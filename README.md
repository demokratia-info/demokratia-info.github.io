...
- Private `author_notice_queue.csv` and `author_notice_history.csv` - editor-controlled author notification queue/history. They are private because they contain author email addresses, source URLs, editor notes, and delivery records.
...
On that same `/page-feedback.html?...` page, a page-specific summary editor is available only after the editor password is verified through the Worker. It loads the source paper page identified by the query string, lets the editor revise the Hebrew title, subtitle, opening line, section headings, and section paragraphs, and saves the result as an approved structured feedback row for the heartbeat to review and apply.
...
Author notification emails are opt-in by editor release. `/author-mailer.html` lets a password-authenticated editor mark queued author-paper notice rows as `ready_to_send`; it does not send mail directly. The local script `python3 scripts/send_author_notices.py --send` sends only `ready_to_send` rows through Gmail SMTP, with `demokratia@tau.ac.il` as `Reply-To` and `Cc`, then moves sent rows into private `author_notice_history.csv`. Blocked authors must never be queued for sending or contacted.
...
When a paper centrally uses a foundational concept such as `פופוליזם`, `שלטון החוק`, `מידתיות`, `נסיגה דמוקרטית`, `ביקורת שיפוטית`, `חוקתנות`, `דמוקרטיה אתנית`, `חברה אזרחית`, `זכויות אדם`, `קיטוב רגשי`, `סבירות`, `שוויון`, or `אזרחות`, add an early `מושגי יסוד בקצרה` section. Explain the concept in plain language, prefer the paper's own framing when available, and otherwise use the accepted meaning reflected in Hebrew/English Wikipedia or similarly stable reference material. Do not add this section merely because a broad topic tag or boilerplate keyword mentions the term.
