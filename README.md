...
After selecting papers, Codex nightly updates should add new papers as `_papers/*.md` files, add or reuse images, update `image_catalog.json`, update the private `suggest_queue.csv` when a suggestion is processed, update public `paper_queue.csv`, bump `_data/site.json` `lastUpdated` and `cacheVersion`, regenerate `_data/paper_index.json`, and then commit/push. At least half of each nightly batch should receive newly created, paper-specific images that fit the article's concrete subject closely; reuse existing catalog images only for the rest and only when the fit is strong. Images should follow the older warm editorial illustration style with people, institutions, documents, civic rooms, public spaces, depth, and texture; avoid flat vector icons, isolated scales/buildings, abstract blobs, sparse diagrams, and other schematic graphics. Update `_data/topics.json` only when adding or changing a topic. They should not edit generated HTML pages manually.
