---
{
  "version": 1,
  "file": "democracy_unpacking_the_politics_of_equality_diversity_and_inclusion_the_case_of_israeli_jewish_and_palestinian_women_summary_he.html",
  "slug": "democracy_unpacking_the_politics_of_equality_diversity_and_inclusion_the_case_of_israeli_jewish_and_palestinian_women_summary_he",
  "order": 457,
  "titleHe": "פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות",
  "subtitleHe": "",
  "descriptionHe": "תמצית בעברית של מאמר על פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות, עם דגש על המשמעות לדמוקרטיה ליברלית, לשלטון החוק, לזכויות ולאחריות ציבורית.",
  "summaryHe": "המאמר בוחן את סוגיית פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות דרך עדשה של זכויות, שוויון אזרחי והיכולת של קבוצות שונות להשתתף בדמוקרטיה.",
  "authorsCardHe": "Helena Desivilya Syna, Michal Palgi, Maha Sabbah Karkabi",
  "paperTitle": "Unpacking the politics of equality, diversity and inclusion: The case of Israeli Jewish and Palestinian women in municipal councils",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-3481-6003\" target=\"_blank\" rel=\"noopener noreferrer\">Helena Desivilya Syna</a>, <a href=\"https://orcid.org/0000-0003-1349-9659\" target=\"_blank\" rel=\"noopener noreferrer\">Michal Palgi</a>, <a href=\"https://orcid.org/0000-0001-9351-7284\" target=\"_blank\" rel=\"noopener noreferrer\">Maha Sabbah Karkabi</a>",
  "journal": "Research in social change",
  "dateText": "פורסם: 2020",
  "doiUrl": "https://doi.org/10.2478/rsc-2020-0001",
  "doiLabel": "https://doi.org/10.2478/rsc-2020-0001",
  "authors": [
    {
      "@type": "Person",
      "name": "Helena Desivilya Syna",
      "url": "https://orcid.org/0000-0002-3481-6003"
    },
    {
      "@type": "Person",
      "name": "Michal Palgi",
      "url": "https://orcid.org/0000-0003-1349-9659"
    },
    {
      "@type": "Person",
      "name": "Maha Sabbah Karkabi",
      "url": "https://orcid.org/0000-0001-9351-7284"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Helena Desivilya Syna",
      "url": "https://orcid.org/0000-0002-3481-6003"
    },
    {
      "@type": "Person",
      "name": "Michal Palgi",
      "url": "https://orcid.org/0000-0003-1349-9659"
    },
    {
      "@type": "Person",
      "name": "Maha Sabbah Karkabi",
      "url": "https://orcid.org/0000-0001-9351-7284"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה",
    "מוסדות",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "מדיניות ציבורית"
  ],
  "image": {
    "src": "html_qa/democracy_unpacking_the_politics_of_equality_diversity_and_inclusion_the_case_of_israeli_jewish_and_palestinian_women_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות מדגים כיצד סוגיה אקדמית ממוקדת הופכת לשאלה על <strong>דמוקרטיה ליברלית</strong>, אחריות ציבורית, שוויון והגבלת כוח.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "שוויון הוא העיקרון שלפיו בני אדם או אזרחים זכאים ליחס שווה, אלא אם קיימת הצדקה עניינית להבחנה ביניהם. בהקשר של המאמר, המושג עוזר לבדוק האם כללים ומוסדות מחלקים זכויות, משאבים והכרה ציבורית באופן הוגן."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "מחקר זה נועד לפרק את הפוליטיקה של שוויון מגדרי והכלה במועצות עירוניות, תוך התמקדות בדפוסי הפרקטיקות הפוליטיות של נשים; ההשפעה המוסדית של התרבויות הלאומיות והמקומיות על התרבויות הארגוניות. ממצאים ראשוניים מצביעים על כך שמעורבותן המשמעותית של נשים בקבלת ההחלטות האסטרטגית העירונית היא חלקית."
      ]
    },
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון בפוליטיקה של שוויון, גיוון והכלה במועצות מקומיות אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון בפוליטיקה של שוויון, גיוון והכלה במועצות מקומיות מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון בפוליטיקה של שוויון, גיוון והכלה במועצות מקומיות עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "פוליטיקה של שוויון, גיוון והכלה במועצות מקומיות",
  "permalink": "/democracy_unpacking_the_politics_of_equality_diversity_and_inclusion_the_case_of_israeli_jewish_and_palestinian_women_summary_he.html",
  "paper_url": "democracy_unpacking_the_politics_of_equality_diversity_and_inclusion_the_case_of_israeli_jewish_and_palestinian_women_summary_he.html",
  "sortKey": 202605220026,
  "sourcePublishedDate": "2020-01-01"
}
---
