---
{
  "version": 1,
  "file": "democracy_nation_state_law_equality_solidarity_summary_he.html",
  "slug": "democracy_nation_state_law_equality_solidarity_summary_he",
  "order": 62,
  "titleHe": "איך חוק הלאום משפיע על שוויון, הגדרה עצמית וסולידריות חברתית?",
  "subtitleHe": "תמר הוסטובסקי ברנדס מנתחת את חוק הלאום דרך שלוש שאלות: זכויות הפרט, זכויות קבוצתיות והיכולת לבנות סולידריות אזרחית מכלילה.",
  "descriptionHe": "המאמר של תמר הוסטובסקי ברנדס ב-Minnesota Journal of International Law בוחן את חוק הלאום ואת השלכותיו על שוויון, הגדרה עצמית, זכויות מיעוטים וסולידריות חברתית בישראל.",
  "summaryHe": "המאמר מסביר מדוע חוק הלאום מעורר לא רק חששות לשוויון משפטי, אלא גם פגיעה עמוקה יותר ביכולת ליצור סולידריות אזרחית משותפת.",
  "authorsCardHe": "תמר הוסטובסקי ברנדס",
  "paperTitle": "Basic Law: Israel as the Nation State of the Jewish People: Implications for Equality, Self-Determination and Social Solidarity",
  "authorsHtml": "<a href=\"https://www.ono.ac.il/lecturer/dr-tamar-ostovsky-brandes/\" target=\"_blank\" rel=\"noopener noreferrer\">Tamar Hostovsky Brandes</a>",
  "journal": "Minnesota Journal of International Law",
  "dateText": "Minnesota Journal of International Law, כרך 29, גיליון 1, עמ׳ 65-107; פורסם בשנת 2020",
  "doiUrl": "https://doi.org/10.24926/19441992.784",
  "doiLabel": "https://doi.org/10.24926/19441992.784",
  "authors": [
    {
      "@type": "Person",
      "name": "תמר הוסטובסקי ברנדס",
      "url": "https://www.ono.ac.il/lecturer/dr-tamar-ostovsky-brandes/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Tamar Hostovsky Brandes",
      "url": "https://www.ono.ac.il/lecturer/dr-tamar-ostovsky-brandes/"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "constitutionalism-courts-rule-of-law",
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "חוק הלאום",
    "שוויון",
    "הגדרה עצמית",
    "סולידריות חברתית",
    "זכויות מיעוטים",
    "מדינת כל אזרחיה"
  ],
  "image": {
    "src": "html_qa/democracy_judicial_overhaul_populist_picture_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על חוק הלאום, שוויון וסולידריות אזרחית בישראל"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-05-21",
  "lastUpdatedHe": "21 במאי 2026",
  "oneLinerHtml": "שורה תחתונה: המאמר טוען שחוק הלאום מעורר שלוש שאלות דמוקרטיות מחוברות: האם הוא פוגע בזכויות פרט ושוויון, האם הוא מצמצם אפשרות להכרה קולקטיבית במיעוטים, והאם הוא מחליש סולידריות אזרחית רחבה. גם אם בית המשפט יכול לצמצם פגיעות מסוימות בשוויון, הפגיעה בסולידריות קשה יותר לתיקון.",
  "sections": [
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מנתח את חוק הלאום על רקע הוויכוח הציבורי והחוקתי שהתפרץ לאחר חקיקתו ב-19 ביולי 2018. הוא אינו מסתפק בשאלה אם החוק פוגע ישירות בזכות אחת, אלא בוחן את מבנה השייכות שהוא יוצר.",
        "הוסטובסקי ברנדס מחלקת את השאלות לשלוש קבוצות: השלכות על זכויות פרט ושוויון; השלכות על הכרה בזכויות קולקטיביות של מיעוטים לא יהודיים; והאופן שבו החוק מעצב זהות לאומית וסולידריות חברתית.",
        "התרומה הדמוקרטית המרכזית היא ההבחנה בין פגיעה משפטית שבית משפט עשוי לתקן לבין פגיעה חברתית-אזרחית עמוקה יותר: צמצום האפשרות לבנות סולידריות שכוללת את כל האזרחים."
      ]
    },
    {
      "headingHe": "מהו ההקשר הפוליטי של החוק?",
      "paragraphsHtml": [
        "המאמר ממקם את חוק הלאום בתוך שני ויכוחים מרכזיים בישראל: הדיון על רעיון ״מדינת כל אזרחיה״ והמאבק בין הכנסת לבין בית המשפט העליון סביב גבולות הביקורת השיפוטית.",
        "ההקשר הזה חשוב מפני שהוא מסביר מדוע חוק קצר יחסית עורר תגובה ציבורית חזקה. החוק אינו רק טקסט, אלא הצהרה על מי נכלל בדמיון החוקתי של המדינה."
      ]
    },
    {
      "headingHe": "אילו נתונים עובדתיים מופיעים במקור?",
      "paragraphsHtml": [
        "התקציר מציין שחוק הלאום נחקק ב-19 ביולי 2018, וכי נכון ל-1 באוקטובר 2018 הוגשו שמונה עתירות נגדו לבית המשפט הגבוה לצדק.",
        "הנתונים האלה חשובים משום שהם ממקמים את המאמר בתוך רגע חוקתי חי, שבו השאלה לא הייתה תיאורטית בלבד אלא חלק ממאבק מוסדי וציבורי ממשי."
      ]
    },
    {
      "headingHe": "למה סולידריות חברתית היא שאלה דמוקרטית?",
      "paragraphsHtml": [
        "סולידריות חברתית אינה רגש נחמד בלבד. היא תנאי לכך שאזרחים מקבוצות שונות יראו עצמם חלק מאותה קהילה פוליטית ויקבלו עליהם כללים משותפים.",
        "כאשר חוק יסוד מדגיש את ההגדרה העצמית של קבוצה אחת בלי להעניק מקום שווה לקבוצות אחרות, הוא עלול להחליש את האפשרות לסולידריות אזרחית שמקיפה את כולם."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד חוק הלאום משפיע על <strong>שוויון</strong>, <strong>הגדרה עצמית</strong> ו<strong>סולידריות חברתית</strong> בישראל."
      ]
    },
    {
      "headingHe": "מהן שלוש קבוצות השאלות שהמאמר מציג?",
      "paragraphsHtml": [
        "הראשונה עוסקת בזכויות פרט ובשוויון; השנייה בהכרה בזכויות קולקטיביות של מיעוטים; השלישית בזהות הלאומית ובתנאי החברות בקהילה הפוליטית."
      ]
    },
    {
      "headingHe": "למה סולידריות קשה יותר לתיקון משפטי?",
      "paragraphsHtml": [
        "פגיעה בשוויון יכולה לעיתים להצטמצם בפסיקה. פגיעה בסולידריות נוגעת למסרים, זהות ושייכות, ולכן היא עמוקה וחברתית יותר."
      ]
    },
    {
      "headingHe": "האם המאמר מתנגד להגדרה עצמית יהודית?",
      "paragraphsHtml": [
        "המאמר אינו שולל הגדרה עצמית יהודית, אלא בוחן כיצד עיגונה החוקתי משפיע על שוויון ועל מקומם של מיעוטים אזרחיים."
      ]
    },
    {
      "headingHe": "מה הקשר לבית המשפט העליון?",
      "paragraphsHtml": [
        "המאמר נכתב על רקע עתירות נגד החוק ועל רקע ויכוח רחב יותר על גבולות הביקורת השיפוטית בישראל."
      ]
    },
    {
      "headingHe": "למה רעיון מדינת כל אזרחיה חשוב כאן?",
      "paragraphsHtml": [
        "הדיון במושג הזה חושף את המתח בין הגדרה לאומית יהודית לבין תפיסה אזרחית שבה המדינה שייכת באופן שווה לכל אזרחיה."
      ]
    },
    {
      "headingHe": "מהי הבעיה הדמוקרטית העמוקה ביותר לפי המאמר?",
      "paragraphsHtml": [
        "מעבר לחשש לפגיעה בשוויון, הבעיה היא צמצום האפשרות לבנות קהילה פוליטית מכלילה שבה מיעוטים מרגישים שותפים מלאים."
      ]
    },
    {
      "headingHe": "מי צריך לקרוא את המאמר?",
      "paragraphsHtml": [
        "משפטנים, אנשי חינוך אזרחי, פעילי זכויות וקוראים העוסקים בחוק הלאום וביחסי רוב-מיעוט בישראל."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין להסיק שכל ביטוי של זהות לאומית פוגע בדמוקרטיה. השאלה היא האם הזהות מעוגנת באופן שמותיר מקום אמיתי לשוויון ולשייכות משותפת."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי להעביר את הדיון מחוק הלאום כסמל לשאלה מעשית יותר: האם כל אזרח יכול לראות עצמו חלק מן המסגרת החוקתית."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך חוק הלאום משפיע על שוויון, הגדרה עצמית וסולידריות חברתית?",
  "permalink": "/democracy_nation_state_law_equality_solidarity_summary_he.html",
  "paper_url": "democracy_nation_state_law_equality_solidarity_summary_he.html",
  "sortKey": 202605210001
}
---
