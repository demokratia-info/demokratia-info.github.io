---
{
  "version": 1,
  "file": "democracy_foreign_aid_volatility_institutional_quality_growth_summary_he.html",
  "slug": "democracy_foreign_aid_volatility_institutional_quality_growth_summary_he",
  "titleHe": "סיוע חוץ תנודתי, מוסדות וצמיחה באפריקה שמדרום לסהרה",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן כיצד תנודתיות בסיוע חוץ ואיכות מוסדית משפיעות על צמיחה במדינות אפריקה שמדרום לסהרה. תרומתו הדמוקרטית היא בהדגשת הבעיה של ממשל שמבוסס על משאבים לא יציבים: גם כסף חיצוני יכול להחליש תכנון ציבורי אם מוסדות אינם מסוגלים להפוך אותו למדיניות אמינה.",
  "summaryHe": "המחקר משתמש בנתונים על 45 מדינות באפריקה שמדרום לסהרה בשנים 1980-2017, ובוחן את ההבדל בין התחייבויות סיוע לבין תנודתיות בסיוע. לפי תקציר המקור, התחייבויות סיוע נקשרות לצמיחה, תנודתיות פוגעת בצמיחה, ואיכות מוסדית משפרת את התוצאה אך אינה בהכרח מבטלת את הנזק של תנודתיות. לכן המאמר חשוב לדיון דמוקרטי על קיבולת מדינתית, תכנון תקציבי ואחריות מוסדית מול תורמים ומול אזרחים.",
  "authorsCardHe": "Elliot Boateng, Frank W. Agbola, Amir Mahmood",
  "paperTitle": "Foreign aid volatility and economic growth in Sub-Saharan Africa: Does institutional quality matter?",
  "authorsHtml": "Elliot Boateng; Frank W. Agbola; Amir Mahmood",
  "journal": "Economic Modelling, 96, 111-127",
  "dateText": "פורסם: מרץ 2021",
  "doiUrl": "https://doi.org/10.1016/j.econmod.2020.12.032",
  "doiLabel": "https://doi.org/10.1016/j.econmod.2020.12.032",
  "authors": [
    {
      "@type": "Person",
      "name": "Elliot Boateng"
    },
    {
      "@type": "Person",
      "name": "Frank W. Agbola"
    },
    {
      "@type": "Person",
      "name": "Amir Mahmood"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Elliot Boateng"
    },
    {
      "@type": "Person",
      "name": "Frank W. Agbola"
    },
    {
      "@type": "Person",
      "name": "Amir Mahmood"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "סיוע חוץ",
    "איכות מוסדית",
    "צמיחה כלכלית",
    "אפריקה שמדרום לסהרה",
    "תנודתיות",
    "קיבולת מדינתית",
    "תכנון ציבורי"
  ],
  "image": {
    "src": "html_qa/democracy_foreign_aid_volatility_institutional_quality_growth_summary_he.jpg",
    "version": "2026-05-26-nightly",
    "altHe": "איור עריכתי על סיוע חוץ, תכנון ציבורי ואיכות מוסדית",
    "fitness": "high"
  },
  "datePublished": "2026-05-26",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן כיצד תנודתיות בסיוע חוץ ואיכות מוסדית משפיעות על צמיחה במדינות אפריקה שמדרום לסהרה. המחקר משתמש בנתונים על 45 מדינות באפריקה שמדרום לסהרה בשנים 1980-2017, ובוחן את ההבדל בין התחייבויות סיוע לבין תנודתיות בסיוע.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>קיבולת מדינתית</strong> היא היכולת של מוסדות ציבור לגבות, לתכנן, לבצע ולפקח. במאמר, איכות המוסדות חשובה משום שסכומי סיוע אינם הופכים מעצמם לצמיחה אם המדינה אינה מסוגלת לייצב תכנון ולנהל תנודתיות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "מקור הפרסום אותר, אך לא נמצא תקציר ציבורי זמין במאגרי המטא־דאטה או בעמוד הנחיתה שנבדקו.",
        "כדי לא להמציא ממצאים, הסיכום הקונקרטי נשען על המידע שכבר אומת בעמוד: המחקר משתמש בנתונים על 45 מדינות באפריקה שמדרום לסהרה בשנים 1980-2017, ובוחן את ההבדל בין התחייבויות סיוע לבין תנודתיות בסיוע. לפי תקציר המקור, התחייבויות סיוע נקשרות לצמיחה, תנודתיות פוגעת בצמיחה, ואיכות מוסדית משפרת את התוצאה אך אינה בהכרח מבטלת את הנזק של תנודתיות. לכן המאמר חשוב לדיון דמוקרטי על קיבולת מדינתית, תכנון תקציבי ואחריות מוסדית מול תורמים ומול אזרחים."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המחקר?",
      "paragraphsHtml": [
        "השאלה היא האם סיוע חוץ תורם לצמיחה כאשר הוא מגיע בסביבה מוסדית שונה, ובעיקר כאשר הוא תנודתי. המחקר מפריד בין עצם ההתחייבות לסיוע לבין אי־הוודאות שמקשה על ממשלות לתכנן."
      ]
    },
    {
      "headingHe": "למה תנודתיות בסיוע היא בעיה דמוקרטית?",
      "paragraphsHtml": [
        "תנודתיות מקשה על תקצוב שירותים, השקעות ותשתיות לאורך זמן. כאשר הממשלה אינה יודעת אם המשאבים יגיעו או ייעלמו, האחריות כלפי האזרחים מתערערת גם אם מקור הכסף חיצוני."
      ]
    },
    {
      "headingHe": "מה תפקיד המוסדות בממצאי המאמר?",
      "paragraphsHtml": [
        "איכות מוסדית משפרת את היכולת להפוך סיוע למדיניות מועילה. עם זאת, המאמר מדגיש שמוסדות טובים אינם בהכרח תרופת פלא לתנודתיות חריפה, ולכן יציבות המשאבים עדיין חשובה."
      ]
    },
    {
      "headingHe": "איך זה קשור לשלטון ציבורי אחראי?",
      "paragraphsHtml": [
        "שלטון אחראי נמדד ביכולת להבטיח שירותים גם כאשר מקורות המימון משתנים. המאמר מדגים שהבטחות סיוע ללא תכנון מוסדי עלולות ליצור פער בין התחייבויות פוליטיות לבין ביצוע בשטח."
      ]
    },
    {
      "headingHe": "מה המאמר מוסיף לדיון על פיתוח?",
      "paragraphsHtml": [
        "הוא מזיז את הדיון מן השאלה אם סיוע טוב או רע לשאלה מדויקת יותר: באיזו צורה, באיזו יציבות ובאיזו סביבה מוסדית הוא פועל. לכן הוא מועיל למי שמבקש להבין מדיניות פיתוח כמערכת מוסדית."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד על יחסי תורמים ומדינות מקבלות?",
      "paragraphsHtml": [
        "סיוע אינו רק העברת כסף אלא גם מערכת יחסים שמעצבת תמריצים. כאשר התורמים יוצרים תנודתיות, הם משפיעים על יכולת המדינה המקבלת לתת דין וחשבון לציבור שלה."
      ]
    },
    {
      "headingHe": "למה חשוב שהמחקר בוחן תקופה ארוכה?",
      "paragraphsHtml": [
        "תקופה ארוכה מאפשרת להבחין בין זעזועים זמניים לבין דפוסים מוסדיים מתמשכים. במדיניות פיתוח, ההבדל הזה חשוב מפני שתוצאה של שנה אחת יכולה להטעות לגבי יכולת ציבורית אמיתית."
      ]
    },
    {
      "headingHe": "האם המאמר אומר שמדינות צריכות לוותר על סיוע?",
      "paragraphsHtml": [
        "לא. הוא מצביע על כך שהתחייבויות סיוע יכולות להיקשר לצמיחה, אך מתריע מפני תנודתיות ומוסדות חלשים. המסקנה המעשית היא לעצב סיוע יציב ומוסדי יותר, לא בהכרח פחות סיוע."
      ]
    },
    {
      "headingHe": "מה המשמעות למדינות דמוקרטיות תורמות?",
      "paragraphsHtml": [
        "מדינות תורמות צריכות לשאול האם מדיניות הסיוע שלהן מחזקת תכנון ואחריות או מייצרת תלות תנודתית. סיוע דמוקרטי צריך לכבד את יכולת המוסדות המקומיים לבנות לגיטימציה פנימית."
      ]
    },
    {
      "headingHe": "מה השורה התחתונה לציבור קוראים רחב?",
      "paragraphsHtml": [
        "כסף ציבורי, גם כשהוא מגיע מבחוץ, זקוק למוסדות שמסוגלים להפוך אותו לתועלת עקבית. בלי יציבות ואחריות, סיוע עלול להוסיף אי־ודאות במקום לבנות צמיחה."
      ]
    }
  ],
  "layout": "paper",
  "title": "סיוע חוץ תנודתי, מוסדות וצמיחה באפריקה שמדרום לסהרה",
  "permalink": "/democracy_foreign_aid_volatility_institutional_quality_growth_summary_he.html",
  "paper_url": "democracy_foreign_aid_volatility_institutional_quality_growth_summary_he.html",
  "sortKey": 202605260003
}
---
