---
{
  "version": 1,
  "file": "democracy_ai_speech_governance_friction_summary_he.html",
  "slug": "democracy_ai_speech_governance_friction_summary_he",
  "order": 1,
  "titleHe": "איך מכניסים חיכוך דמוקרטי לרגולציה של שיח על ידי בינה מלאכותית?",
  "subtitleHe": "מאמר על הצורך לעצב מערכות AI שמאפשרות ערעור, שקיפות והשתתפות ציבורית בהחלטות על ביטוי.",
  "descriptionHe": "המאמר עוסק בדעת קהל, שיח ציבורי, קיטוב ואלימות פוליטית, וממקם את הדיון בתוך שאלות של יכולת מחלוקת דמוקרטית, אמון הדדי וחופש ביטוי.",
  "summaryHe": "המאמר טוען שרגולציה של שיח באמצעות AI צריכה לכלול חיכוך דמוקרטי: יכולת להבין, לערער ולהשתתף בעיצוב כללי ההסרה וההעדפה של תכנים.",
  "authorsCardHe": "ניבה אלקין-קורן, מעיין פרל",
  "paperTitle": "Democratic friction in speech governance by AI",
  "authorsHtml": "<a href=\"https://english.tau.ac.il/profile/elkiniva\" target=\"_blank\" rel=\"noopener noreferrer\">Niva Elkin-Koren</a>; <a href=\"https://ric.org.il/researcher/maayan-pearl/\" target=\"_blank\" rel=\"noopener noreferrer\">Maayan Perel</a>",
  "journal": "Handbook of Critical Studies of Artificial Intelligence",
  "dateText": "פורסם: 14.11.2023",
  "doiUrl": "https://doi.org/10.4337/9781803928562.00066",
  "doiLabel": "https://doi.org/10.4337/9781803928562.00066",
  "authors": [
    {
      "@type": "Person",
      "name": "ניבה אלקין-קורן",
      "url": "https://english.tau.ac.il/profile/elkiniva"
    },
    {
      "@type": "Person",
      "name": "מעיין פרל",
      "url": "https://ric.org.il/researcher/maayan-pearl/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Niva Elkin-Koren",
      "url": "https://english.tau.ac.il/profile/elkiniva"
    },
    {
      "@type": "Person",
      "name": "Maayan Perel",
      "url": "https://ric.org.il/researcher/maayan-pearl/"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "rights-equality-minorities"
  ],
  "keywords": [
    "בינה מלאכותית",
    "חופש ביטוי",
    "ממשל פלטפורמות",
    "שקיפות",
    "דמוקרטיה דיגיטלית",
    "רגולציה של שיח"
  ],
  "image": {
    "src": "html_qa/democracy_civil_society_ngos_backsliding_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על שיח ציבורי, טכנולוגיה וחברה אזרחית דמוקרטית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר טוען שרגולציה של שיח באמצעות AI צריכה לכלול חיכוך דמוקרטי: יכולת להבין, לערער ולהשתתף בעיצוב כללי ההסרה וההעדפה של תכנים. המאמר מחבר בין רגולציה של פלטפורמות, חופש ביטוי ועיצוב מערכות AI, ומציע לראות בחיכוך מוסדי לא תקלה אלא תנאי להגנה על שיח דמוקרטי.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>רגולציה של שיח</strong> היא מכלול הכללים, ההחלטות והטכנולוגיות שקובעים אילו תכנים מוסרים, מקודמים, מדורגים או נשארים גלויים במרחב דיגיטלי. בהקשר של המאמר, המושג חשוב משום שמערכות AI אינן רק כלי טכני: הן מעצבות בפועל את גבולות הביטוי, את אפשרות הערעור ואת מידת השקיפות של כללי השיח.",
        "<strong>חיכוך דמוקרטי</strong> הוא עיכוב או מנגנון ביקורת מכוון שמחייב הסבר, שקיפות, אפשרות לערער והשתתפות של הציבור או של גורמים מפקחים לפני שהחלטות אוטומטיות מתקבעות. המאמר משתמש ברעיון הזה כדי להראות שחיכוך אינו בהכרח תקלה במערכת AI, אלא תנאי להגנה על חופש ביטוי ועל לגיטימיות דמוקרטית."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר ״חיכוך דמוקרטי ברגולציה של שיח באמצעות בינה מלאכותית״ מוצג כאן דרך השאלה הדמוקרטית המרכזית שעולה ממנו.",
        "המאמר מחבר בין רגולציה של פלטפורמות, חופש ביטוי ועיצוב מערכות AI, ומציע לראות בחיכוך מוסדי לא תקלה אלא תנאי להגנה על שיח דמוקרטי."
      ]
    },
    {
      "headingHe": "מהי הטענה המרכזית?",
      "paragraphsHtml": [
        "המאמר טוען שרגולציה של שיח באמצעות AI צריכה לכלול חיכוך דמוקרטי: יכולת להבין, לערער ולהשתתף בעיצוב כללי ההסרה וההעדפה של תכנים.",
        "הקריאה הדמוקרטית של המאמר היא שהגנה על מוסדות, זכויות והשתתפות ציבורית מחייבת תשומת לב גם למנגנונים שנראים טכניים או מקצועיים.",
        "לכן המאמר מתאים לאתר: הוא מסביר כיצד החלטות משפטיות, פוליטיות או מוסדיות משפיעות על היכולת של אזרחים לחיות תחת כללי משחק הוגנים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מערכות בינה מלאכותית (AI) נפרסות יותר ויותר כדי לשלוט במרחב הציבורי הדיגיטלי. עבור הדמוקרטיה, לעומת זאת, חיכוך הוא תכונה מרכזית. בהתאם לכך, פרק זה טוען שמדיניות ציבורית צריכה להתמקד בקידום חיכוך דמוקרטי.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר טוען שרגולציה של שיח באמצעות AI צריכה לכלול חיכוך דמוקרטי: יכולת להבין, לערער ולהשתתף בעיצוב כללי ההסרה וההעדפה של תכנים."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם במוסדות שמרסנים כוח, מגינים על מיעוטים ומאפשרים ביקורת ציבורית.",
        "כאשר אחד מן הרכיבים האלה נחלש, הפגיעה עשויה להופיע בהדרגה: פחות שקיפות, פחות אחריות, פחות הגנה על זכויות ופחות אמון ביכולת של המערכת לתקן את עצמה.",
        "המאמר עוזר לזהות את נקודת החיבור בין סוגיה אקדמית מוגדרת לבין שאלות ציבוריות רחבות על שלטון החוק, שוויון ואחריות."
      ]
    },
    {
      "headingHe": "מה הקורא הישראלי יכול לקחת מכאן?",
      "paragraphsHtml": [
        "הלקח המעשי הוא לבחון לא רק את הכותרת של שינוי מוסדי, אלא את ההשפעה המצטברת שלו על כללי המשחק.",
        "גם כאשר הרפורמה, הפסיקה או התהליך מוצגים כמקצועיים, השאלה הדמוקרטית נשארת: מי מרוויח כוח, מי מאבד הגנה, ואילו מנגנוני ביקורת נשארים פעילים.",
        "המאמר מזמין דיון ציבורי מדויק יותר, שמפריד בין מחלוקת לגיטימית לבין שינוי שמחליש את תנאי המחלוקת עצמה."
      ]
    },
    {
      "headingHe": "מה נושא המאמר?",
      "paragraphsHtml": [
        "המאמר עוסק ב־<strong>איך מכניסים חיכוך דמוקרטי לרגולציה של שיח על ידי בינה מלאכותית?</strong>.",
        "במונחים פשוטים, הוא שואל כיצד המוסדות והכללים משפיעים על איכות הדמוקרטיה ולא רק על התוצאה הפוליטית המיידית."
      ]
    },
    {
      "headingHe": "למה המאמר רלוונטי לישראל?",
      "paragraphsHtml": [
        "המאמר רלוונטי לישראל משום שהוא עוסק במתח בין כוח פוליטי, מוסדות ביקורת, זכויות וזהות ציבורית.",
        "אלו בדיוק השאלות שמעצבות את הוויכוח הישראלי על דמוקרטיה ליברלית, במיוחד בתקופה של קיטוב ומשבר מוסדי."
      ]
    },
    {
      "headingHe": "מהו הסיכון הדמוקרטי המרכזי?",
      "paragraphsHtml": [
        "הסיכון המרכזי הוא ששינוי נקודתי ייתפס כמחלוקת רגילה, אף שהוא משנה את מאזן הכוחות או את רמת ההגנה על זכויות.",
        "כאשר שינויים כאלה מצטברים, קשה יותר להחזיר את המערכת לאיזון דמוקרטי תקין."
      ]
    },
    {
      "headingHe": "האם המאמר נוקט עמדה פוליטית מפלגתית?",
      "paragraphsHtml": [
        "המאמר אינו נדרש להחליף בחירה פוליטית של אזרחים, אלא מציע כלים לבחינת איכות הכללים הדמוקרטיים.",
        "הבחינה מתמקדת בשאלות של <strong>שלטון החוק</strong>, <strong>שוויון</strong>, <strong>אחריות</strong> ויכולת של מוסדות להגביל כוח."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לזכויות?",
      "paragraphsHtml": [
        "גם כאשר המאמר עוסק במוסדות, ההשלכה הסופית נוגעת לעיתים קרובות לזכויות של יחידים וקבוצות.",
        "מוסדות חלשים או מוטים מתקשים להגן על חופש ביטוי, שוויון אזרחי, זכויות מיעוטים והשתתפות פוליטית הוגנת."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק דורש שכללי המשחק יחולו גם על בעלי כוח ולא רק על אזרחים רגילים.",
        "המאמרים בקבוצה הזו מדגישים שכאשר מנגנוני ביקורת נחלשים, שלטון החוק עלול להפוך מסדר מחייב לשפה פוליטית בלבד."
      ]
    },
    {
      "headingHe": "מה הקשר לקיטוב?",
      "paragraphsHtml": [
        "קיטוב משפיע על היכולת של ציבור לקבל הכרעות מוסדיות כהלגיטימיות גם כאשר הן אינן נוחות לו.",
        "כאשר כל מחנה רואה באחר איום קיומי, קל יותר להצדיק פגיעה בכללים בשם ניצחון פוליטי או ביטחון."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "המאמר טוען שרגולציה של שיח באמצעות AI צריכה לכלול חיכוך דמוקרטי: יכולת להבין, לערער ולהשתתף בעיצוב כללי ההסרה וההעדפה של תכנים.",
        "השורה התחתונה לאזרחית ולאזרח היא לבחון כל שינוי לפי השפעתו על כוח, זכויות, אחריות ואמון, ולא רק לפי הכוונה המוצהרת של יוזמיו."
      ]
    },
    {
      "headingHe": "למי כדאי לקרוא את המקור?",
      "paragraphsHtml": [
        "המקור מתאים לקוראים שמתעניינים במשפט חוקתי, מדע המדינה, דעת קהל, זכויות אדם או מוסדות דמוקרטיים.",
        "הוא שימושי במיוחד למי שרוצה להבין את הקשר בין מחקר אקדמי לבין הוויכוחים הציבוריים בישראל על דמוקרטיה ליברלית."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך מכניסים חיכוך דמוקרטי לרגולציה של שיח על ידי בינה מלאכותית?",
  "permalink": "/democracy_ai_speech_governance_friction_summary_he.html",
  "paper_url": "democracy_ai_speech_governance_friction_summary_he.html",
  "sortKey": 202605210011
}
---
