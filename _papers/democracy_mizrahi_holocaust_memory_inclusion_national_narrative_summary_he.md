---
{
  "version": 1,
  "file": "democracy_mizrahi_holocaust_memory_inclusion_national_narrative_summary_he.html",
  "slug": "democracy_mizrahi_holocaust_memory_inclusion_national_narrative_summary_he",
  "titleHe": "חוקי זיכרון אינם מספיקים כדי לכלול קהילות מזרחיות בנרטיב הלאומי",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן אם שילוב זיכרון של יהודים יוצאי צפון אפריקה בחוקי הנצחה בישראל מצליח לחזק שייכות ושוויון. הממצא המרכזי הוא שגם כאשר המדינה מרחיבה את סיפור הזיכרון הרשמי, תחושות הדרה מן הנרטיב הלאומי עשויות להישאר חזקות.",
  "summaryHe": "Kook ו־Solomon מנתחות את הקשר בין <strong>זיכרון לאומי</strong>, מיעוטים ודמוקרטיה דרך חקיקה ומדיניות שמבקשות לכלול את סיפורן של קהילות יהודיות מצפון אפריקה בזיכרון השואה בישראל. על בסיס ראיונות קבוצתיים, הן מראות שהכרה רשמית אינה מחיקה מיידית של חוויית הדרה, משום ששייכות אזרחית נוצרת גם דרך האופן שבו קהילה מרגישה מיוצגת, נשמעת ומכובדת.",
  "authorsCardHe": "Rebecca Kook, Sophia Solomon",
  "paperTitle": "“Don't cry, it doesn't belong to us”; critical thoughts on commemoration as a means of inclusion: Mizrahi Jews and Holocaust memory in Israel",
  "authorsHtml": "<a href=\"https://cris.bgu.ac.il/en/persons/becky-kook/\" target=\"_blank\" rel=\"noopener noreferrer\">Rebecca Kook</a>; <a href=\"https://orcid.org/0000-0001-6907-1550\" target=\"_blank\" rel=\"noopener noreferrer\">Sophia Solomon</a>",
  "journal": "Nations and Nationalism, 29(4), 1273-1288",
  "dateText": "פורסם: 2022",
  "sourcePublishedDate": "2022-11-20",
  "doiUrl": "https://doi.org/10.1111/nana.12908",
  "doiLabel": "https://doi.org/10.1111/nana.12908",
  "authors": [
    {
      "@type": "Person",
      "name": "Rebecca Kook",
      "url": "https://cris.bgu.ac.il/en/persons/becky-kook/"
    },
    {
      "@type": "Person",
      "name": "Sophia Solomon",
      "url": "https://orcid.org/0000-0001-6907-1550"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Rebecca Kook",
      "url": "https://cris.bgu.ac.il/en/persons/becky-kook/"
    },
    {
      "@type": "Person",
      "name": "Sophia Solomon",
      "url": "https://orcid.org/0000-0001-6907-1550"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "זיכרון לאומי",
    "יהודים יוצאי צפון אפריקה",
    "שואה",
    "שוויון",
    "מיעוטים",
    "שייכות אזרחית",
    "חוקי זיכרון"
  ],
  "image": {
    "src": "html_qa/democracy_mizrahi_holocaust_memory_inclusion_national_narrative_summary_he.jpg",
    "version": "2026-06-01-nightly",
    "altHe": "סצנת ארכיון קהילתית על זיכרון מיעוטים ושייכות לנרטיב הלאומי",
    "fitness": "high"
  },
  "datePublished": "2026-06-01",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שהכרה חוקית בזיכרון של קהילות מיעוט היא צעד חשוב, אך אינה מספיקה לבדה ליצירת שייכות דמוקרטית. השאלה היא איך ההכרה מתורגמת לייצוג ממשי בחינוך, בהנצחה ובסיפור הציבורי.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>זיכרון לאומי</strong> הוא הסיפור הציבורי שבאמצעותו מדינה מגדירה אילו אירועים וקבוצות נחשבים מרכזיים בזהותה. כאשר קהילה מרגישה שהסיפור הרשמי אינו כולל אותה, הפער אינו סמלי בלבד אלא נוגע לשוויון אזרחי ולתחושת בעלות על המרחב הציבורי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: תקציר המאמצים לשלב את הזיכרונות הקולקטיביים של קהילות מיעוטים בנרטיבים של זיכרון לאומי נתפסים כחלק בלתי נפרד מהמאמצים להחיות את הדמוקרטיה, תוך תרומה לאינטגרציה חברתית ושוויון. בהקשר זה, הכללת היסטוריות של מיעוטים מחייבת יותר ויותר חוק ומדיניות במדינות רבות. בהתבסס על ראיונות קבוצתיים שנערכו עם בני קהילות יהודיות בצפון אפריקה בישראל, אנו מדגימים שתחושות ההדרה מנרטיב הזיכרון הלאומי מתמשכות, למרות מאמצי המדינה.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: Kook ו־Solomon מנתחות את הקשר בין זיכרון לאומי , מיעוטים ודמוקרטיה דרך חקיקה ומדיניות שמבקשות לכלול את סיפורן של קהילות יהודיות מצפון אפריקה בזיכרון השואה בישראל. על בסיס ראיונות קבוצתיים, הן מראות שהכרה רשמית אינה מחיקה מיידית של חוויית הדרה, משום ששייכות אזרחית נוצרת גם דרך האופן שבו קהילה מרגישה מיוצגת, נשמעת ומכובדת."
      ]
    },
    {
      "headingHe": "איך המאמר מציב את שאלת ההכללה?",
      "paragraphsHtml": [
        "המחברות מתמקדות במאמץ ישראלי להכניס את זיכרון קהילות צפון אפריקה אל נרטיב השואה הרשמי. נקודת המבחן שלהן אינה רק עצם החקיקה, אלא השאלה אם ההכרה משנה את תחושת השייכות של בני הקהילות."
      ]
    },
    {
      "headingHe": "על איזה שינוי בזיכרון הלאומי המאמר מדבר?",
      "paragraphsHtml": [
        "המאמר עוסק בחוקי זיכרון ובמדיניות שמרחיבים את זיכרון השואה מעבר לסיפור ההגמוני הישן. ההרחבה אמורה להכיר בהיסטוריה של יהודים מצפון אפריקה ולהפוך אותה לחלק מן הנרטיב הלאומי הישראלי."
      ]
    },
    {
      "headingHe": "מהי הטענה המרכזית לגבי שילוב מיעוטים?",
      "paragraphsHtml": [
        "הטענה היא ששילוב פורמלי של היסטוריה מודרת אינו מבטיח שהקהילה תרגיש מוכרת. הכרה ציבורית צריכה להיות אמינה, מתמשכת וקשובה לאופן שבו חברי הקהילה מפרשים את מקומם בסיפור הלאומי."
      ]
    },
    {
      "headingHe": "למה הנושא קשור לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתפקת בזכות הצבעה ובשוויון משפטי כתוב. היא נבחנת גם ביכולתה לתת לקבוצות שונות מקום מכבד בזיכרון, בחינוך ובמוסדות שמעצבים אזרחות משותפת."
      ]
    },
    {
      "headingHe": "מה מלמדים ראיונות הקבוצה?",
      "paragraphsHtml": [
        "הראיונות מצביעים על כך שתחושת הדרה יכולה להישאר גם לאחר שינוי מדיניות. משתתפים עשויים לראות בהכרה הרשמית צעד מאוחר או חלקי, ולכן לא לחוות אותה כתיקון מלא של היחס ההיסטורי."
      ]
    },
    {
      "headingHe": "האם חוק זיכרון לבדו יוצר שייכות?",
      "paragraphsHtml": [
        "המאמר מציע תשובה זהירה ושלילית. חוק יכול לפתוח דלת, אך השייכות נוצרת דרך הוראה, טקסים, ייצוג ציבורי והקשבה לחוויות שהודרו מן הסיפור המרכזי."
      ]
    },
    {
      "headingHe": "מהו האתגר במקרה של יהודים יוצאי צפון אפריקה?",
      "paragraphsHtml": [
        "האתגר הוא לשלב את סיפור הקהילות בלי להפוך אותו לתוספת שולית או לטקס ריק. הכרה אמיתית דורשת לראות את הזיכרון המזרחי כחלק מן ההיסטוריה הלאומית ולא כנספח שנועד לסמן הכללה."
      ]
    },
    {
      "headingHe": "איך המאמר מחבר בין שוויון לסיפור לאומי?",
      "paragraphsHtml": [
        "שוויון אינו רק חלוקת משאבים אלא גם הכרה בכבוד ובניסיון חיים של קבוצות שונות. כאשר הסיפור הלאומי קובע מי נחשב חלק מן ה״אנחנו״, הוא משפיע על תחושת האזרחות של מיעוטים."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למדיניות חינוך והנצחה?",
      "paragraphsHtml": [
        "הלקח הוא שמדיניות צריכה להיבחן לפי האופן שבו היא מתקבלת בקהילות עצמן. תכנית לימודים או טקס הנצחה יכולים לקדם שוויון רק אם הם מאפשרים לקולות המודרים להופיע באופן ממשי ולא רק רשמי."
      ]
    },
    {
      "headingHe": "האם המאמר טוען לוותר על נרטיב לאומי משותף?",
      "paragraphsHtml": [
        "לא. הוא מצביע על הצורך לבנות נרטיב משותף שמסוגל להכיל מורכבות, מחלוקת וריבוי זיכרונות בלי למחוק את תחושת השייכות של קבוצה אחת בשם אחדות מדומה."
      ]
    },
    {
      "headingHe": "למי המאמר רלוונטי בישראל?",
      "paragraphsHtml": [
        "המאמר רלוונטי למחנכים, מחוקקים, מוזיאונים וגופי הנצחה שעוסקים בהכרה בקבוצות מודרות. הוא חשוב גם לדיון רחב יותר על הדרך שבה מדינה דמוקרטית הופכת שוויון פורמלי לחוויה אזרחית מורגשת."
      ]
    }
  ],
  "layout": "paper",
  "title": "חוקי זיכרון אינם מספיקים כדי לכלול קהילות מזרחיות בנרטיב הלאומי",
  "permalink": "/democracy_mizrahi_holocaust_memory_inclusion_national_narrative_summary_he.html",
  "paper_url": "democracy_mizrahi_holocaust_memory_inclusion_national_narrative_summary_he.html",
  "sortKey": 202606010010
}
---
