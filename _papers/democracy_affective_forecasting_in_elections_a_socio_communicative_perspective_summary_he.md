---
{
  "version": 1,
  "file": "democracy_affective_forecasting_in_elections_a_socio_communicative_perspective_summary_he.html",
  "slug": "democracy_affective_forecasting_in_elections_a_socio_communicative_perspective_summary_he",
  "order": 440,
  "titleHe": "חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית",
  "subtitleHe": "",
  "descriptionHe": "תמצית בעברית של מאמר על חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית, עם דגש על המשמעות לדמוקרטיה ליברלית, לשלטון החוק, לזכויות ולאחריות ציבורית.",
  "summaryHe": "המאמר בוחן את סוגיית חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית דרך עדשה של דעת קהל, קיטוב והגבולות של מחלוקת פוליטית לגיטימית.",
  "authorsCardHe": "Keren Tenenboim‐Weinblatt, Christian Baden, Tali Aharoni, Maximilian Overbeck",
  "paperTitle": "Affective forecasting in elections: A socio-communicative perspective",
  "authorsHtml": "<a href=\"https://en.communication.huji.ac.il/people/keren-tenenboim-weinblatt\" target=\"_blank\" rel=\"noopener noreferrer\">Keren Tenenboim‐Weinblatt</a>, <a href=\"https://en.communication.huji.ac.il/people/christian-baden\" target=\"_blank\" rel=\"noopener noreferrer\">Christian Baden</a>, <a href=\"https://en.communication.huji.ac.il/people/tali-aharoni\" target=\"_blank\" rel=\"noopener noreferrer\">Tali Aharoni</a>, <a href=\"https://orcid.org/0000-0003-3658-5584\" target=\"_blank\" rel=\"noopener noreferrer\">Maximilian Overbeck</a>",
  "journal": "Human Communication Research",
  "dateText": "פורסם: 2022",
  "doiUrl": "https://doi.org/10.1093/hcr/hqac007",
  "doiLabel": "https://doi.org/10.1093/hcr/hqac007",
  "authors": [
    {
      "@type": "Person",
      "name": "Keren Tenenboim‐Weinblatt",
      "url": "https://en.communication.huji.ac.il/people/keren-tenenboim-weinblatt"
    },
    {
      "@type": "Person",
      "name": "Christian Baden",
      "url": "https://en.communication.huji.ac.il/people/christian-baden"
    },
    {
      "@type": "Person",
      "name": "Tali Aharoni",
      "url": "https://en.communication.huji.ac.il/people/tali-aharoni"
    },
    {
      "@type": "Person",
      "name": "Maximilian Overbeck",
      "url": "https://orcid.org/0000-0003-3658-5584"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Keren Tenenboim‐Weinblatt",
      "url": "https://en.communication.huji.ac.il/people/keren-tenenboim-weinblatt"
    },
    {
      "@type": "Person",
      "name": "Christian Baden",
      "url": "https://en.communication.huji.ac.il/people/christian-baden"
    },
    {
      "@type": "Person",
      "name": "Tali Aharoni",
      "url": "https://en.communication.huji.ac.il/people/tali-aharoni"
    },
    {
      "@type": "Person",
      "name": "Maximilian Overbeck",
      "url": "https://orcid.org/0000-0003-3658-5584"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דעת קהל",
    "קיטוב",
    "אלימות פוליטית",
    "בחירות",
    "מדיה",
    "חיזוי",
    "רגשי",
    "בבחירות",
    "והתקשורת"
  ],
  "image": {
    "src": "html_qa/democracy_affective_forecasting_in_elections_a_socio_communicative_perspective_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית מדגים כיצד סוגיה אקדמית ממוקדת הופכת לשאלה על <strong>דמוקרטיה ליברלית</strong>, אחריות ציבורית, שוויון והגבלת כוח.",
  "sections": [
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מחקר זה נועד להראות את החשיבות של פרספקטיבה חברתית-תקשורתית להבנת המנבאים, הביטויים וההשלכות של חיזוי רגשי, במיוחד כאשר עתידים קולקטיביים עומדים על כף המאזניים. באמצעות מקרה בוחן של הבחירות בישראל 2019–2021 ושילוב של סקר בן 12 גלים ו-25 קבוצות מיקוד, אנו מראים כיצד תחזיות רגשיות פוליטיות קשורות לגורמים חברתיים-תקשורתיים, משמשות באינטראקציות חברתיות ויש להן השלכות על קיטוב והשתתפות פוליטיים. אנו מסיימים בדיון על ההשלכות של הממצאים שלנו על מחקר עתידי על חיזוי רגשי במחקרי תקשורת.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון בחיזוי רגשי בבחירות והתקשורת החברתית־פוליטית אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון בחיזוי רגשי בבחירות והתקשורת החברתית־פוליטית מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון בחיזוי רגשי בבחירות והתקשורת החברתית־פוליטית עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "חיזוי רגשי בבחירות והתקשורת החברתית־פוליטית",
  "permalink": "/democracy_affective_forecasting_in_elections_a_socio_communicative_perspective_summary_he.html",
  "paper_url": "democracy_affective_forecasting_in_elections_a_socio_communicative_perspective_summary_he.html",
  "sortKey": 202605220043,
  "sourcePublishedDate": "2022-04-26"
}
---
