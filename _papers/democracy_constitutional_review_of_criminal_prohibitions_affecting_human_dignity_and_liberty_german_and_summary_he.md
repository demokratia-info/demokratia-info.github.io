---
{
  "version": 1,
  "file": "democracy_constitutional_review_of_criminal_prohibitions_affecting_human_dignity_and_liberty_german_and_summary_he.html",
  "slug": "democracy_constitutional_review_of_criminal_prohibitions_affecting_human_dignity_and_liberty_german_and_summary_he",
  "order": 103,
  "titleHe": "מה הקשר בין מעמדו החוקתי של כבוד האדם בישראל ובהשוואה לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית מעמדו החוקתי של כבוד האדם בישראל ובהשוואה, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית מעמדו החוקתי של כבוד האדם בישראל ובהשוואה דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Miriam Gur-Arye; Thomas Weigend",
  "paperTitle": "Constitutional Review of Criminal Prohibitions Affecting Human Dignity and Liberty: German and Israeli Perspectives",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-7281-8715\" target=\"_blank\" rel=\"noopener noreferrer\">Miriam Gur-Arye</a>; <a href=\"https://orcid.org/0000-0002-6862-0500\" target=\"_blank\" rel=\"noopener noreferrer\">Thomas Weigend</a>",
  "journal": "Israel Law Review",
  "dateText": "פורסם: 2011",
  "doiUrl": "https://doi.org/10.1017/s0021223700000960",
  "doiLabel": "https://doi.org/10.1017/s0021223700000960",
  "authors": [
    {
      "@type": "Person",
      "name": "מירי גור אריה",
      "url": "https://orcid.org/0000-0001-7281-8715"
    },
    {
      "@type": "Person",
      "name": "Thomas Weigend",
      "url": "https://orcid.org/0000-0002-6862-0500"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Miriam Gur-Arye",
      "url": "https://orcid.org/0000-0001-7281-8715"
    },
    {
      "@type": "Person",
      "name": "Thomas Weigend",
      "url": "https://orcid.org/0000-0002-6862-0500"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "מעמדו החוקתי של כבוד האדם בישראל ובהשוואה",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_constitutional_review_of_criminal_prohibitions_affecting_human_dignity_and_liberty_german_and_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על מעמדו החוקתי של כבוד האדם בישראל ובהשוואה",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית מעמדו החוקתי של כבוד האדם בישראל ובהשוואה דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "חוקה היא מערכת כללי יסוד שמגדירה את מבנה השלטון, סמכויותיו וזכויות היסוד; חוקתנות היא התפיסה שלפיה גם רוב פוליטי ומוסדות שלטון מוגבלים על ידי כללים אלה. בהקשר של המאמר, המושג מסייע להבין כיצד כללי יסוד, בתי משפט וחוקי יסוד יכולים להגביל כוח ולייצב מחלוקת פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: המחברים מראים כי קיימות הקבלות ברורות בפסיקה של בתי המשפט הישראלים והגרמנים בכל הנוגע לחירות. המחברים מציעים פתרון ביניים: אין להעניק לכבוד האדם הגנה \"מוחלטת\" אלא יש להתייחס אליו בכבוד רב כאשר החוקים הפליליים נבדקים על חוקתיותם.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית מעמדו החוקתי של כבוד האדם בישראל ובהשוואה, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת מעמדו החוקתי של כבוד האדם בישראל ובהשוואה לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר מעמדו החוקתי של כבוד האדם בישראל ובהשוואה משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין מעמדו החוקתי של כבוד האדם בישראל ובהשוואה לבין שלטון החוק בישראל?",
  "permalink": "/democracy_constitutional_review_of_criminal_prohibitions_affecting_human_dignity_and_liberty_german_and_summary_he.html",
  "paper_url": "democracy_constitutional_review_of_criminal_prohibitions_affecting_human_dignity_and_liberty_german_and_summary_he.html",
  "sortKey": 202605210190
}
---
