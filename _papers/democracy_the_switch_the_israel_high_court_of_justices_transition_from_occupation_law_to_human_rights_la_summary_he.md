---
{
  "version": 1,
  "file": "democracy_the_switch_the_israel_high_court_of_justices_transition_from_occupation_law_to_human_rights_la_summary_he.html",
  "slug": "democracy_the_switch_the_israel_high_court_of_justices_transition_from_occupation_law_to_human_rights_la_summary_he",
  "order": 66,
  "titleHe": "מה הקשר בין המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Amichai Cohen; Yuval Shany",
  "paperTitle": "The switch: The Israel High Court of Justice’s transition from occupation law to human rights law",
  "authorsHtml": "<a href=\"https://en.idi.org.il/experts/1491\" target=\"_blank\" rel=\"noopener noreferrer\">Amichai Cohen</a>, <a href=\"https://en.law.huji.ac.il/people/yuval-shany\" target=\"_blank\" rel=\"noopener noreferrer\">Yuval Shany</a>",
  "journal": "International Journal of Constitutional Law",
  "dateText": "פורסם: 2022",
  "doiUrl": "https://doi.org/10.1093/icon/moac108",
  "doiLabel": "https://doi.org/10.1093/icon/moac108",
  "authors": [
    {
      "@type": "Person",
      "name": "עמיחי כהן",
      "url": "https://en.idi.org.il/experts/1491"
    },
    {
      "@type": "Person",
      "name": "Yuval Shany",
      "url": "https://en.law.huji.ac.il/people/yuval-shany"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Amichai Cohen",
      "url": "https://en.idi.org.il/experts/1491"
    },
    {
      "@type": "Person",
      "name": "Yuval Shany",
      "url": "https://en.law.huji.ac.il/people/yuval-shany"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_the_switch_the_israel_high_court_of_justices_transition_from_occupation_law_to_human_rights_la_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "זכויות אדם הן זכויות בסיסיות שמיוחסות לכל אדם בשל אנושיותו, כגון חירות, שוויון, כבוד, הליך הוגן וחופש ביטוי. בהקשר של המאמר, המושג מסייע לבדוק אם מוסדות המדינה מגינים על יחידים וקבוצות גם כאשר הדבר מגביל כוח שלטוני או רוב פוליטי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: תקציר הוצע כי הפסיקה של בית המשפט הגבוה לצדק (בג\"ץ) המתייחסת לשטחים שנכבשו על ידי ישראל ב-1967, העניקה למעשה לגיטימציה לכיבוש. עם זאת, מהחלטות אחרונות של בג\"ץ עולה כי הפרדיגמה המשפטית שהוא מיישם עברה מהמשפט המנהלי הישראלי למשפט החוקתי הישראלי, ומהמשפט הבינלאומי של כיבוש לוחמני לדיני זכויות אדם פנימיים. פסק הדין של סילוואד - החלטה חשובה מיוני 2020 שבה ביטל בג\"ץ את חוק ההסדרה משנת 2017 - ממחיש את המעבר הזה.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין המעבר של בג״ץ מדיני הכיבוש לשפת זכויות האדם לבין שלטון החוק בישראל?",
  "permalink": "/democracy_the_switch_the_israel_high_court_of_justices_transition_from_occupation_law_to_human_rights_la_summary_he.html",
  "paper_url": "democracy_the_switch_the_israel_high_court_of_justices_transition_from_occupation_law_to_human_rights_la_summary_he.html",
  "sortKey": 202605210227
}
---
