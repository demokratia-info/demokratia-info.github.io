---
{
  "version": 1,
  "file": "democracy_active_participation_of_older_adults_at_political_rallies_as_a_source_of_resilience_the_case_of_the_israeli_summary_he.html",
  "slug": "democracy_active_participation_of_older_adults_at_political_rallies_as_a_source_of_resilience_the_case_of_the_israeli_summary_he",
  "order": 240,
  "titleHe": "השתתפות מבוגרים במחאות פוליטיות כמקור לחוסן",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק במחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית, וממקם את הדיון בתוך שאלות של דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית דרך עדשה של דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
  "authorsCardHe": "Boaz M. Ben‐David, Ortal Shimon-Raz, Yuval Palgi, Lia Ring, Tchelet Bresslet",
  "paperTitle": "ACTIVE PARTICIPATION OF OLDER ADULTS AT POLITICAL RALLIES AS A SOURCE OF RESILIENCE: THE CASE OF THE ISRAELI PROTEST",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-0392-962X\" target=\"_blank\" rel=\"noopener noreferrer\">Boaz M. Ben‐David</a>, <a href=\"https://orcid.org/0000-0002-6464-8370\" target=\"_blank\" rel=\"noopener noreferrer\">Ortal Shimon-Raz</a>, <a href=\"https://orcid.org/0000-0002-8675-5513\" target=\"_blank\" rel=\"noopener noreferrer\">Yuval Palgi</a>, <a href=\"https://orcid.org/0000-0001-5203-4091\" target=\"_blank\" rel=\"noopener noreferrer\">Lia Ring</a>, Tchelet Bresslet",
  "journal": "Innovation in Aging",
  "dateText": "פורסם: 2023",
  "doiUrl": "https://doi.org/10.1093/geroni/igad104.3752",
  "doiLabel": "https://doi.org/10.1093/geroni/igad104.3752",
  "authors": [
    {
      "@type": "Person",
      "name": "Boaz M. Ben‐David",
      "url": "https://orcid.org/0000-0002-0392-962X"
    },
    {
      "@type": "Person",
      "name": "Ortal Shimon-Raz",
      "url": "https://orcid.org/0000-0002-6464-8370"
    },
    {
      "@type": "Person",
      "name": "Yuval Palgi",
      "url": "https://orcid.org/0000-0002-8675-5513"
    },
    {
      "@type": "Person",
      "name": "Lia Ring",
      "url": "https://orcid.org/0000-0001-5203-4091"
    },
    {
      "@type": "Person",
      "name": "Tchelet Bresslet"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Boaz M. Ben‐David",
      "url": "https://orcid.org/0000-0002-0392-962X"
    },
    {
      "@type": "Person",
      "name": "Ortal Shimon-Raz",
      "url": "https://orcid.org/0000-0002-6464-8370"
    },
    {
      "@type": "Person",
      "name": "Yuval Palgi",
      "url": "https://orcid.org/0000-0002-8675-5513"
    },
    {
      "@type": "Person",
      "name": "Lia Ring",
      "url": "https://orcid.org/0000-0001-5203-4091"
    },
    {
      "@type": "Person",
      "name": "Tchelet Bresslet"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דעת קהל",
    "קיטוב",
    "מחאה",
    "בחירות",
    "אלימות פוליטית",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_active_participation_of_older_adults_at_political_rallies_as_a_source_of_resilience_the_case_of_the_israeli_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית ומראה מדוע הדיון קשור לדעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית, וממקם את הסוגיה בתוך הדיון הרחב על דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מעניין לציין שהסקר דיווח על שיעור השתתפות של 37% בקרב מבוגרים, שיעור ההשתתפות הגבוה ביותר בקרב כל קבוצות הגיל שנבדקו. במחקר הנוכחי ערכנו סקר שבדק מדדי בריאות הנפש בקרב ישראלים מבוגרים.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון במחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון במחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "השתתפות מבוגרים במחאות פוליטיות כמקור לחוסן",
  "permalink": "/democracy_active_participation_of_older_adults_at_political_rallies_as_a_source_of_resilience_the_case_of_the_israeli_summary_he.html",
  "paper_url": "democracy_active_participation_of_older_adults_at_political_rallies_as_a_source_of_resilience_the_case_of_the_israeli_summary_he.html",
  "sortKey": 202605210240
}
---
