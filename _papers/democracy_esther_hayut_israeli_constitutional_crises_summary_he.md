---
{
  "version": 1,
  "file": "democracy_esther_hayut_israeli_constitutional_crises_summary_he.html",
  "slug": "democracy_esther_hayut_israeli_constitutional_crises_summary_he",
  "titleHe": "נשיאות אסתר חיות הפכה לזירת מבחן להגנה חוקתית על הדמוקרטיה",
  "subtitleHe": "המאמר מנתח את כהונתה מול משברים חוקתיים מתגלגלים וטוען שבית המשפט פיתח אסטרטגיות לשימור כוחו מול הרשות המבצעת והמחוקקת.",
  "descriptionHe": "המאמר מנתח את נשיאות אסתר חיות בבית המשפט העליון בשנים 2018-2023 מול משברים חוקתיים, מתקפות פוליטיות ושחיקה דמוקרטית בישראל.",
  "summaryHe": "המאמר מציב את כהונת אסתר חיות כנשיאת בית המשפט העליון בתוך סדרה מתמשכת של משברים חוקתיים בישראל. לטענת המחברים, התנועה הפופוליסטית שחקה את הסדר הדמוקרטי, והממשלה והכנסת פעלו לעיתים ככוח פוליטי אחד מול בית המשפט. תחת לחץ אישי ומוסדי, חיות פיתחה אסטרטגיות שיפוטיות שאפשרו לבית המשפט לשמר ואף לחזק את כוחו, בעיקר באמצעות יישום תקדימי ונועז של ביקורת שיפוטית על חוקי יסוד.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Yaniv Roznai, Shani Schnitzer",
  "paperTitle": "Navigating the judicial ship through stormy waters: President Esther Hayut and the Israeli constitutional crises, 2018–23",
  "authorsHtml": "<a href=\"https://www.runi.ac.il/en/faculty/yroznai\" target=\"_blank\" rel=\"noopener noreferrer\">Yaniv Roznai</a>, <a href=\"https://orcid.org/0009-0003-0678-3018\" target=\"_blank\" rel=\"noopener noreferrer\">Shani Schnitzer</a>",
  "journal": "International Journal of Constitutional Law 23(1), 179-202",
  "dateText": "פורסם מקוון: 25 במאי 2025; בגיליון 23(1), 2025",
  "doiUrl": "https://doi.org/10.1093/icon/moaf013",
  "doiLabel": "https://doi.org/10.1093/icon/moaf013",
  "authors": [
    {
      "@type": "Person",
      "name": "Yaniv Roznai",
      "url": "https://www.runi.ac.il/en/faculty/yroznai"
    },
    {
      "@type": "Person",
      "name": "Shani Schnitzer",
      "url": "https://orcid.org/0009-0003-0678-3018"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yaniv Roznai",
      "url": "https://www.runi.ac.il/en/faculty/yroznai"
    },
    {
      "@type": "Person",
      "name": "Shani Schnitzer",
      "url": "https://orcid.org/0009-0003-0678-3018"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "אסתר חיות",
    "בית המשפט העליון",
    "משבר חוקתי",
    "ביקורת שיפוטית",
    "פופוליזם",
    "שלטון החוק",
    "חוקי יסוד"
  ],
  "image": {
    "src": "assets/article-images/real_photo_224_court_democracy_esther_hayut_israeli_constitutional_crises_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לנשיאות אסתר חיות הפכה לזירת מבחן להגנה חוקתית על הדמוקרטיה",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Narciso1224",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Facade_of_Auberge_d%27Auvergne_in_Valletta.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/6/6d/Facade_of_Auberge_d%27Auvergne_in_Valletta.jpg",
    "fileTitle": "Facade of Auberge d'Auvergne in Valletta.jpg",
    "sourceDimensions": "1324x938",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:court",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-06-20",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מציג את נשיאות חיות כמקרה מבחן של הנהגה שיפוטית המגינה על שלטון החוק בזמן משבר חוקתי מתמשך.",
  "sections": [
    {
      "headingHe": "המאמר קורא את בית המשפט כשחקן במשבר חוקתי מתגלגל",
      "paragraphsHtml": [
        "הכותבים מציבים את נשיאות אסתר חיות בתוך סדרה של עימותים בין בית המשפט לבין הממשלה והכנסת. לפי הניתוח, השאלה אינה רק מה פסק בית המשפט, אלא כיצד הנהגה שיפוטית בונה אסטרטגיה בזמן מתקפה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתאר את בית המשפט העליון כמי שניצב במרכזם של עימותים מתמשכים עם הרשויות הפוליטיות. בהקשר הישראלי, המחברים מדגישים שהרשות המבצעת והרוב המחוקק פועלים במידה רבה ככוח אחד, ולכן העימות מול בית המשפט נעשה חריף במיוחד בתקופה של שחיקה פופוליסטית.",
        "הניתוח מתמקד באסתר חיות כנשיאת בית המשפט העליון בשנים 2018-2023. המחברים טוענים שהיא פיתחה אסטרטגיות שיפוטיות מחושבות שאפשרו לרשות השופטת לשמר, ואולי אף לחזק, את מעמדה מול הממשלה והכנסת תחת לחץ אישי ומוסדי.",
        "המורשת המרכזית שמזהה המאמר היא היישום התקדימי והנועז של ביקורת שיפוטית על נורמות חוקתיות. בעיני המחברים, מהלך זה סייע לבית המשפט לשמור על שלטון החוק ועל אופייה הדמוקרטי של ישראל בתקופה שבה המשברים החוקתיים הצטברו."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: ביקורת שיפוטית על תיקונים חוקתיים",
      "paragraphsHtml": [
        "ביקורת שיפוטית על תיקונים חוקתיים היא האפשרות שבית משפט יבחן גם שינוי בנורמות החוקתיות עצמן. בישראל, שבה חוקי היסוד משמשים תשתית חוקתית, השאלה הזאת נעשתה מרכזית במיוחד בתקופת המשבר."
      ]
    },
    {
      "headingHe": "מה טוענים המחברים על התקופה 2018-2023?",
      "paragraphsHtml": [
        "המחברים טוענים שישראל חוותה משברים חוקתיים חסרי תקדים לצד שחיקה פופוליסטית של הסדר הדמוקרטי. בתוך כך בית המשפט נדרש להגן על שלטון החוק מול רשויות פוליטיות שפעלו לעיתים כגוש אחד."
      ]
    },
    {
      "headingHe": "מה היה תפקידה של אסתר חיות לפי המאמר?",
      "paragraphsHtml": [
        "חיות מוצגת כנשיאה שפיתחה אסטרטגיות שיפוטיות מחושבות תחת לחץ. המאמר טוען שהאסטרטגיות האלה אפשרו לבית המשפט לשמר את כוחו ואף להעצים אותו מול הרשויות האחרות."
      ]
    },
    {
      "headingHe": "למה המאמר מדבר על רשות מבצעת ומחוקקת כגוש אחד?",
      "paragraphsHtml": [
        "במשטר פרלמנטרי עם משמעת קואליציונית חזקה, הממשלה והרוב בכנסת יכולים לפעול יחד לקידום שינוי מוסדי. לכן האיזון מול בית המשפט נעשה חשוב במיוחד כאשר שני המרכזים הפוליטיים מתלכדים."
      ]
    },
    {
      "headingHe": "מהי המורשת המרכזית שמזהים המחברים?",
      "paragraphsHtml": [
        "המחברים מצביעים על יישום תקדימי ונועז של ביקורת שיפוטית על נורמות חוקתיות. מבחינתם, זו מורשת מרכזית של חיות משום שהיא נוגעת לגבולות כוחו של הרוב."
      ]
    },
    {
      "headingHe": "איך המאמר מחבר בין פופוליזם לבין בית המשפט?",
      "paragraphsHtml": [
        "פופוליזם מציג לעיתים את בית המשפט כמכשול לרצון העם, ולכן הוא מפעיל לחץ על עצמאות שיפוטית. המאמר מראה כיצד בית המשפט יכול להגיב לא רק בפסקי דין בודדים אלא גם באסטרטגיית הגנה מוסדית."
      ]
    },
    {
      "headingHe": "מה הקשר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק מחייב שהרוב הפוליטי יפעל בתוך גבולות מוסדיים ולא ישנה אותם כרצונו בכל רגע. כאשר בית המשפט בוחן גם תיקונים חוקתיים, הוא מציב גבול לשימוש לרעה בכוח מכונן."
      ]
    },
    {
      "headingHe": "האם המאמר רואה בבית המשפט פתרון מלא?",
      "paragraphsHtml": [
        "המאמר מדגיש את תפקיד בית המשפט, אך אינו הופך אותו לתחליף לפוליטיקה דמוקרטית בריאה. הגנה שיפוטית יכולה לבלום שחיקה, אך היא אינה מחליפה אמון ציבורי ומוסדות פוליטיים אחראיים."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד על הנהגה שיפוטית?",
      "paragraphsHtml": [
        "הנהגה שיפוטית נמדדת לא רק בניהול פנימי של בית המשפט אלא גם בבחירת רגעים, ניסוחים ותיקים שמעצבים את מקומו במשטר. בתקופת משבר, הבחירות האלה משפיעות על יכולת המוסד לשרוד."
      ]
    },
    {
      "headingHe": "מה מגבלת הניתוח?",
      "paragraphsHtml": [
        "זהו ניתוח משפטי־מוסדי של כהונה ונקודת זמן מסוימת, ולכן הוא אינו מספק מדידה אמפירית של כל תגובות הציבור. כוחו הוא בהבנת ההיגיון המשפטי והאסטרטגי של בית המשפט בתקופת משבר."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית של המאמר?",
      "paragraphsHtml": [
        "התרומה היא בהצגת נשיאות חיות כמקרה מבחן להגנה שיפוטית על דמוקרטיה תחת לחץ פופוליסטי. המאמר מסביר כיצד בית משפט יכול לנווט בין איפוק, התערבות ושימור כוח מוסדי."
      ]
    }
  ],
  "layout": "paper",
  "title": "נשיאות אסתר חיות הפכה לזירת מבחן להגנה חוקתית על הדמוקרטיה",
  "permalink": "/democracy_esther_hayut_israeli_constitutional_crises_summary_he.html",
  "paper_url": "democracy_esther_hayut_israeli_constitutional_crises_summary_he.html",
  "sortKey": 202606200002,
  "sourcePublishedDate": "2025-05-25"
}
---
