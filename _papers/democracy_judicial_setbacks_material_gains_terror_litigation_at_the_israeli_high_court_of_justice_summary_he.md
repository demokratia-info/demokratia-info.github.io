---
{
  "version": 1,
  "file": "democracy_judicial_setbacks_material_gains_terror_litigation_at_the_israeli_high_court_of_justice_summary_he.html",
  "slug": "democracy_judicial_setbacks_material_gains_terror_litigation_at_the_israeli_high_court_of_justice_summary_he",
  "order": 109,
  "titleHe": "מה הקשר בין עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Menachem Hofnung; Keren Weinshall Margel",
  "paperTitle": "Judicial Setbacks, Material Gains: Terror Litigation at the Israeli High Court of Justice",
  "authorsHtml": "<a href=\"https://cris.huji.ac.il/en/persons/menachem-hofnung/\" target=\"_blank\" rel=\"noopener noreferrer\">Menachem Hofnung</a>; <a href=\"https://en.law.huji.ac.il/people/keren-weinshall\" target=\"_blank\" rel=\"noopener noreferrer\">Keren Weinshall Margel</a>",
  "journal": "Journal of Empirical Legal Studies",
  "dateText": "פורסם: 2010",
  "doiUrl": "https://doi.org/10.1111/j.1740-1461.2010.01192.x",
  "doiLabel": "https://doi.org/10.1111/j.1740-1461.2010.01192.x",
  "authors": [
    {
      "@type": "Person",
      "name": "Menachem Hofnung",
      "url": "https://cris.huji.ac.il/en/persons/menachem-hofnung/"
    },
    {
      "@type": "Person",
      "name": "קרן וינשל-מרגל",
      "url": "https://en.law.huji.ac.il/people/keren-weinshall"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Menachem Hofnung",
      "url": "https://cris.huji.ac.il/en/persons/menachem-hofnung/"
    },
    {
      "@type": "Person",
      "name": "Keren Weinshall Margel",
      "url": "https://en.law.huji.ac.il/people/keren-weinshall"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_judicial_setbacks_material_gains_terror_litigation_at_the_israeli_high_court_of_justice_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: לפיכך, רוב המחקרים שנעשו על בג\"ץ הישראלי טוענים כי למרות השימוש ברטוריקה של זכויות אדם, בג\"ץ מתערב לעתים רחוקות בהחלטות ביטחוניות שמטרתן למנוע פעילות טרור. המחקר שלנו, המבוסס על מדגם אמפירי של תיקים שהתדיינות בבג\"ץ בשנים 2000–2008, טוען כי התמונה מורכבת יותר, וכי למעשה לבית המשפט בישראל יש תפקיד משמעותי בצמצום הפרות זכויות האדם. למרות שבמרבית המקרים בג\"ץ אינו מתערב באופן גלוי בהחלטות רשויות הביטחון, אנו מראים כי בהחלטות בית המשפט בטרור יש פן מרומז, אך לא פחות חשוב, במניעת החלטות מזיקות רבות.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין עתירות טרור בבג״ץ בין הישגים חומריים וכשלונות משפטיים לבין שלטון החוק בישראל?",
  "permalink": "/democracy_judicial_setbacks_material_gains_terror_litigation_at_the_israeli_high_court_of_justice_summary_he.html",
  "paper_url": "democracy_judicial_setbacks_material_gains_terror_litigation_at_the_israeli_high_court_of_justice_summary_he.html",
  "sortKey": 202605210184
}
---
