---
{
  "version": 1,
  "file": "democracy_the_mother_of_all_protests_meets_israeli_older_persons_when_age_and_gender_intersect_in_political_protests_summary_he.html",
  "slug": "democracy_the_mother_of_all_protests_meets_israeli_older_persons_when_age_and_gender_intersect_in_political_protests_summary_he",
  "order": 291,
  "titleHe": "״אם כל המחאות״ ופוליטיקה של זקנה ומגדר",
  "subtitleHe": "",
  "descriptionHe": "המאמר של ליאת איילון ושרית אוקון בוחן כיצד מחאת 2023 נגד המהפכה המשפטית פגשה אזרחים מבוגרים בישראל. הוא מנתח את הצטלבות הגיל והמגדר במחאה, ואת הדרך שבה זקנה יכולה להפוך ממיקום של שוליות למקור סמכות פוליטית.",
  "summaryHe": "המאמר עוסק במחאות ההמוניות נגד המהפכה המשפטית בישראל ומפנה את המבט אל משתתפים מבוגרים, ובמיוחד אל האופן שבו גיל ומגדר מעצבים השתתפות פוליטית. במקום לראות בזקנים קבוצה פסיבית או מחוץ לפוליטיקה, איילון ואוקון בוחנות כיצד אנשים מבוגרים נכנסו למחאה עם ניסיון חיים, זיכרון היסטורי, דאגה לדורות הבאים ותחושת אחריות אזרחית. הכותרת ״אם כל המחאות״ מאפשרת גם משחק מגדרי: המחאה מוצגת לא רק כמופע פוליטי גדול, אלא כמרחב שבו דימויים של אימהות, סבתאות, אחריות וטיפול מקבלים משמעות ציבורית. המאמר מדגיש שהשתתפות בגיל מבוגר אינה רק המשך של אזרחות רגילה. היא מצטלבת עם תפיסות חברתיות של זקנה, גוף, פגיעות, ניסיון וסמכות מוסרית. מבוגרים במחאה עשויים להיתפס כמי שמגינים על עתיד שאולי לא יחיו לראות במלואו, ולכן השתתפותם מייצרת מסר בין-דורי חזק. המאמר תורם להבנת המחאה הישראלית בכך שהוא מוסיף לה ממד של גילנות, מגדר וסולידריות בין-דורית. דמוקרטיה נבחנת גם בשאלה מי נראה ברחוב, מי מקבל קול, ואיך זהויות של גיל ומגדר משנות את משמעות ההתנגדות.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Liat Ayalon, ‪Sarit Okun‬‏",
  "paperTitle": "The “Mother of All Protests” Meets Israeli Older Persons: When Age and Gender Intersect in Political Protests",
  "authorsHtml": "<a href=\"https://social-work.biu.ac.il/en/Ayalon_Liat_En\" target=\"_blank\" rel=\"noopener noreferrer\">Liat Ayalon</a>, ‪Sarit Okun‬‏",
  "journal": "The Journals of Gerontology Series B",
  "dateText": "פורסם: 2023",
  "doiUrl": "https://doi.org/10.1093/geronb/gbad172",
  "doiLabel": "https://doi.org/10.1093/geronb/gbad172",
  "authors": [
    {
      "@type": "Person",
      "name": "Liat Ayalon",
      "url": "https://social-work.biu.ac.il/en/Ayalon_Liat_En"
    },
    {
      "@type": "Person",
      "name": "‪Sarit Okun‬‏"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Liat Ayalon",
      "url": "https://social-work.biu.ac.il/en/Ayalon_Liat_En"
    },
    {
      "@type": "Person",
      "name": "‪Sarit Okun‬‏"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_766_gender_democracy_the_mother_of_all_protests_meets_israeli_older_persons_when_age_and_gender_inter.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על מחאה, קיטוב והגבולות של פעולה פוליטית דמוקרטית",
    "fitness": "high",
    "license": "CC BY-SA 2.0",
    "creator": "Israel Defense Forces",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_Israel_Defense_Forces_-_The_Life_of_Female_Field_Intelligence_Combat_Soldiers_(6).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/9/95/Flickr_-_Israel_Defense_Forces_-_The_Life_of_Female_Field_Intelligence_Combat_Soldiers_%286%29.jpg",
    "fileTitle": "Flickr - Israel Defense Forces - The Life of Female Field Intelligence Combat Soldiers (6).jpg",
    "sourceDimensions": "1803x1200",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:gender",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/2.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שמבוגרים במחאת 2023 לא היו קהל שולי, אלא שחקנים פוליטיים שגילם ומגדרם העניקו למחאה משמעות בין-דורית ומוסרית.",
  "sections": [
    {
      "headingHe": "למה גיל חשוב במחאה פוליטית?",
      "paragraphsHtml": [
        "גיל משפיע על האופן שבו משתתפים נתפסים ועל השפה שבה הם מסבירים את מעורבותם. מבוגרים מביאים למחאה ניסיון, זיכרון היסטורי ודאגה לדורות הבאים."
      ]
    },
    {
      "headingHe": "איך מגדר נכנס לתמונה?",
      "paragraphsHtml": [
        "המאמר בוחן דימויים של אימהות, סבתאות וטיפול, ואת הדרך שבה נשים מבוגרות יכולות להפוך תפקידים שנתפסים כפרטיים למקור של סמכות ציבורית ופוליטית."
      ]
    },
    {
      "headingHe": "מה מיוחד במחאת 2023?",
      "paragraphsHtml": [
        "המחאה נגד המהפכה המשפטית הייתה רחבה, מתמשכת ורבת-דורות. היא אפשרה לראות כיצד קבוצות גיל שונות מנסחות יחד פחד מפגיעה בדמוקרטיה."
      ]
    },
    {
      "headingHe": "מהי המשמעות הבין-דורית?",
      "paragraphsHtml": [
        "כאשר אנשים מבוגרים מפגינים למען עתיד הדמוקרטיה, הם מציגים את עצמם כשומרי מורשת וכמי שפועלים עבור ילדים ונכדים. כך המחאה מקבלת ממד של אחריות לאורך זמן."
      ]
    },
    {
      "headingHe": "מה התרומה של המאמר?",
      "paragraphsHtml": [
        "המאמר מחבר בין גרונטולוגיה, מגדר ופוליטיקה. הוא מראה שדמוקרטיה אינה רק מוסדות, אלא גם גופים ברחוב, זהויות חברתיות ודרכים שונות של השתתפות אזרחית."
      ]
    }
  ],
  "layout": "paper",
  "title": "״אם כל המחאות״ ופוליטיקה של זקנה ומגדר",
  "permalink": "/democracy_the_mother_of_all_protests_meets_israeli_older_persons_when_age_and_gender_intersect_in_political_protests_summary_he.html",
  "paper_url": "democracy_the_mother_of_all_protests_meets_israeli_older_persons_when_age_and_gender_intersect_in_political_protests_summary_he.html",
  "sortKey": 202605210291
}
---
