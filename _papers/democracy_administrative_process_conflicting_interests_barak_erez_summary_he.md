---
{
  "version": 1,
  "file": "democracy_administrative_process_conflicting_interests_barak_erez_summary_he.html",
  "slug": "democracy_administrative_process_conflicting_interests_barak_erez_summary_he",
  "titleHe": "ההליך המינהלי הוא זירה לחלוקת אינטרסים ולא רק להפעלת סמכות",
  "subtitleHe": "",
  "descriptionHe": "המאמר מציע להבין החלטות מינהליות כזירה של איזון בין אינטרסים מתנגשים, לעיתים בלי שלצדדים יש זכות משפטית מוגדרת מראש לתוצאה. הוא מדגיש את חשיבות ההליך ההוגן, ייצוג האינטרסים וביקורת על ניגודי עניינים.",
  "summaryHe": "המאמר טוען שהמשפט המינהלי צריך לפקח על הדרך שבה רשויות מאזנות בין אינטרסים של יחידים וקבוצות. החלטות מינהליות משפיעות על זכויות, איכות חיים, מחירים, תנאי מחיה והקצאת כספי ציבור, ולכן ההליך עצמו חייב לאפשר השתתפות רחבה ושוויונית. החלק היישומי מנתח את רגולציית הקרקעות בישראל ואת פרשת הקשת הדמוקרטית המזרחית.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Daphne Barak‐Erez",
  "paperTitle": "The Administrative Process as a Domain of Conflicting Interests",
  "authorsHtml": "<a href=\"https://www.tau.ac.il/law/barakerez/\" target=\"_blank\" rel=\"noopener noreferrer\">Daphne Barak‐Erez</a>",
  "journal": "Theoretical Inquiries in Law, 6(1), 193-214",
  "dateText": "פורסם: 24.01.2005",
  "doiUrl": "https://doi.org/10.2202/1565-3404.1104",
  "doiLabel": "https://doi.org/10.2202/1565-3404.1104",
  "authors": [
    {
      "@type": "Person",
      "name": "Daphne Barak‐Erez",
      "url": "https://www.tau.ac.il/law/barakerez/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Daphne Barak‐Erez",
      "url": "https://www.tau.ac.il/law/barakerez/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "משפט מינהלי",
    "אינטרסים מתנגשים",
    "ביקורת שיפוטית",
    "ייצוג אינטרסים",
    "הקצאת קרקע",
    "הקשת הדמוקרטית המזרחית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_027_court_democracy_administrative_process_conflicting_interests_barak_erez_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לההליך המינהלי הוא זירה לחלוקת אינטרסים ולא רק להפעלת סמכות",
    "fitness": "high",
    "license": "CC0",
    "creator": "שי קנדלר",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Zvi_Sukkot_In_supreme_court_hearing_about_Walid_Daqqa_burial,_August_24.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/3/3b/Zvi_Sukkot_In_supreme_court_hearing_about_Walid_Daqqa_burial%2C_August_24.jpg",
    "fileTitle": "Zvi Sukkot In supreme court hearing about Walid Daqqa burial, August 24.jpg",
    "sourceDimensions": "5120x3413",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:court",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/publicdomain/zero/1.0/"
  },
  "datePublished": "2026-05-31",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מציג את ההליך המינהלי כמנגנון דמוקרטי לחלוקת משאבים וסיכונים, שבו הגינות הליכית חיונית במיוחד כשאין תשובה משפטית מוכנה מראש.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>משפט מינהלי</strong> מסדיר את הדרך שבה רשויות ציבוריות מפעילות סמכות, מקבלות החלטות ומאזנות בין אינטרסים. במאמר הזה הוא מוצג לא רק כטכניקה של חוקיות, אלא כמנגנון דמוקרטי לחלוקה הוגנת של כוח ומשאבים."
      ]
    },
    {
      "headingHe": "מהי הטענה המרכזית?",
      "paragraphsHtml": [
        "המאמר טוען שיש להבין החלטות מינהליות כאיזון בין אינטרסים מתנגשים של יחידים וקבוצות. פעמים רבות אין לצדדים זכות משפטית מוגדרת מראש לתוצאה מסוימת, ולכן ההליך עצמו הופך למקור מרכזי של הגינות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר טוען שהחלטה מינהלית היא לעיתים קרובות החלטה בין אינטרסים מתנגשים של יחידים וקבוצות, כאשר לאף צד אין זכות משפטית מוגדרת מראש שמכריעה את התוצאה. החלטות כאלה משפיעות לא רק על זכויות אדם ואזרח, אלא גם על איכות חיים, תנאי מחיה, מחירי מוצרים מפוקחים והקצאת כספי ציבור.",
        "הניתוח מסביר מדוע דוקטרינות של משפט מינהלי צריכות להתמקד בהליך: מניעת ניגודי עניינים של מקבלי החלטות, מתן משקל וקול לקבוצות מושפעות, וביקורת שיפוטית על הדרך שבה הרשות מאזנת בין האינטרסים. המאמר מדגיש שהשתתפות רחבה אינה אמורה לחסום כל יוזמה מינהלית, אך היא צריכה לתת הזדמנות שווה לכל הצדדים הרלוונטיים.",
        "החלק היישומי בוחן את רגולציית משאבי הקרקע בישראל, ובמיוחד החלטות שנגעו לפיתוח קרקעות חקלאיות לשעבר לקראת עיור בפרשת הקשת הדמוקרטית המזרחית. המאמר מצביע על בעיית הייצוג במועצת מקרקעי ישראל, שבה המגזר החקלאי היה מיוצג באופן חריג, ועל כך שהחלטות פיצוי התקבלו בסביבה מוסדית שהעלתה חשש לניגוד עניינים ולשבי רגולטורי."
      ]
    },
    {
      "headingHe": "למה החלטות מינהליות הן חלוקתיות?",
      "paragraphsHtml": [
        "החלטות מינהליות משפיעות על איכות חיים, תנאי מגורים, מחירים, שירותים והקצאת כספי ציבור. כאשר רשות בוחרת בין אינטרסים, היא למעשה מחלקת יתרונות, סיכונים ועלויות בין קבוצות בחברה."
      ]
    },
    {
      "headingHe": "מה תפקידו של המשפט במצב כזה?",
      "paragraphsHtml": [
        "תפקיד המשפט המינהלי הוא להבטיח שהחלטות יתקבלו בהגינות ועם משקל ראוי לאינטרסים של המעורבים. הוא אינו קובע תמיד מי צודק מהותית, אך הוא בונה תנאים שבהם ההחלטה יכולה להיות לגיטימית."
      ]
    },
    {
      "headingHe": "איך איסור ניגוד עניינים משתלב בתמונה?",
      "paragraphsHtml": [
        "איסור ניגוד עניינים מגן על ההליך מפני מצב שבו מקבל ההחלטה מעדיף אינטרס זר או אישי. בהקשר של אינטרסים מתנגשים, הכלל הזה מבטיח שהאיזון ייעשה לטובת הציבור ולא לטובת מי שמחזיק בכוח."
      ]
    },
    {
      "headingHe": "מה החשיבות של ייצוג אינטרסים?",
      "paragraphsHtml": [
        "כאשר החלטה משפיעה על קבוצות רבות, עצם האפשרות להשמיע עמדה היא תנאי להגינות. ייצוג אינטרסים מאפשר לרשות לראות מידע, פגיעות ותוצאות שלא היו נראות מתוך המשרד בלבד."
      ]
    },
    {
      "headingHe": "מה עושה ביקורת שיפוטית בהליך כזה?",
      "paragraphsHtml": [
        "ביקורת שיפוטית יכולה לבדוק אם הרשות שקלה את השיקולים הרלוונטיים, נתנה משקל ראוי ונמנעה מפגמים בהליך. היא אינה חייבת להחליף את שיקול הדעת המינהלי, אך היא מציבה גבולות לאופן הפעלתו."
      ]
    },
    {
      "headingHe": "למה רגולציית קרקעות בישראל חשובה למאמר?",
      "paragraphsHtml": [
        "התקציר מציין שהמאמר מיישם את נקודת המבט על רגולציה רחבה של משאבי קרקע בישראל. קרקע היא משאב ציבורי טעון, ולכן החלטות עליה מדגימות היטב כיצד הליך מינהלי הופך לזירה של צדק חלוקתי."
      ]
    },
    {
      "headingHe": "מהי תרומת פרשת הקשת הדמוקרטית המזרחית לדיון?",
      "paragraphsHtml": [
        "המאמר מתמקד בהחלטת בית המשפט העליון באחת העתירות השנויות במחלוקת נגד החלטות מינהליות על פיתוח קרקעות חקלאיות לשעבר. דרך המקרה הזה הוא בוחן כיצד אינטרסים חברתיים, קנייניים וציבוריים מתנגשים בהליך אחד."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתפקת בכך שהרשות מוסמכת לפעול. היא דורשת שהפעלת הסמכות תיעשה בהליך הוגן, שקוף וקשוב, במיוחד כאשר ההחלטה מחלקת משאבים ופוגעת באינטרסים של קבוצות שונות."
      ]
    },
    {
      "headingHe": "מה לא כדאי להסיק מהמאמר?",
      "paragraphsHtml": [
        "לא כדאי להסיק שכל אינטרס מקבל משקל שווה או שכל החלטה חלוקתית מוכרעת בבית משפט. המאמר מציע מסגרת להבנת ההליך, אך ההכרעה הספציפית תלויה בהקשר, בסמכות, בראיות ובתכליות הציבוריות."
      ]
    },
    {
      "headingHe": "למי המאמר רלוונטי?",
      "paragraphsHtml": [
        "המאמר רלוונטי למשפטנים, מתכננים, רגולטורים וארגוני ציבור. הוא חשוב במיוחד בכל תחום שבו החלטה מינהלית נראית טכנית אך בפועל מחלקת זכויות, משאבים והזדמנויות."
      ]
    }
  ],
  "layout": "paper",
  "title": "ההליך המינהלי הוא זירה לחלוקת אינטרסים ולא רק להפעלת סמכות",
  "permalink": "/democracy_administrative_process_conflicting_interests_barak_erez_summary_he.html",
  "paper_url": "democracy_administrative_process_conflicting_interests_barak_erez_summary_he.html",
  "sortKey": 202605310003,
  "sourcePublishedDate": "2005-01-24"
}
---
