---
{
  "version": 1,
  "file": "democracy_fracturing_exception_violent_interrogation_methods_israel_summary_he.html",
  "slug": "democracy_fracturing_exception_violent_interrogation_methods_israel_summary_he",
  "order": 890,
  "titleHe": "כיצד החריג המשפטי מאפשר שיטות חקירה אלימות בישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר מנתח את המבנים המשפטיים שהסדירו שיטות חקירה אלימות בישראל מאז 1987, וטוען כי הם יצרו משטר מפוצל של חריגים לאיסור העינויים.",
  "summaryHe": "Irit Ballas בוחנת מסמכים רשמיים, הנחיות, ועדות חקירה ופסיקות בג״ץ מ־1987 עד 2018, לצד כללים הנוגעים למעצר ולפיקוח על נחקרים. המאמר טוען שהמשפט לא רק צמצם או הרחיב חריג אחד לאיסור העינויים, אלא יצר מערכת של חריגים נפרדים לפי המבצע, הקורבן, זמן החקירה ומרחב החקירה.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Irit Ballas",
  "paperTitle": "Fracturing the “Exception”: The Legal Sanctioning of Violent Interrogation Methods in Israel since 1987",
  "authorsHtml": "<a href=\"https://cris.biu.ac.il/en/persons/irit-ballas/\" target=\"_blank\" rel=\"noopener noreferrer\">Irit Ballas</a>",
  "journal": "Law & Social Inquiry",
  "dateText": "פורסם: 2020",
  "doiUrl": "https://doi.org/10.1017/lsi.2020.11",
  "doiLabel": "https://doi.org/10.1017/lsi.2020.11",
  "authors": [
    {
      "@type": "Person",
      "name": "Irit Ballas",
      "url": "https://cris.biu.ac.il/en/persons/irit-ballas/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Irit Ballas",
      "url": "https://cris.biu.ac.il/en/persons/irit-ballas/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "עינויים",
    "שיטות חקירה אלימות",
    "חריג משפטי",
    "שלטון החוק",
    "זכויות אדם",
    "ישראל",
    "מצב חריג",
    "אלימות מדינתית",
    "בתי משפט",
    "ביטחון"
  ],
  "image": {
    "src": "html_qa/democracy_fracturing_exception_violent_interrogation_methods_israel_summary_he.jpg",
    "version": "2026-06-25-nightly",
    "altHe": "חדר ארכיון משפטי וכיסא ריק כמטפורה לחריגים משפטיים בחקירות אלימות",
    "fitness": "standard"
  },
  "datePublished": "2026-06-25",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר טוען שריבוי <strong>חריגים משפטיים</strong> לאיסור העינויים יוצר מערכת שמכשירה אלימות מדינתית בשם החוק.",
  "sections": [
    {
      "headingHe": "מה עומד במרכז המאמר?",
      "paragraphsHtml": [
        "המאמר בוחן את המבנים המשפטיים שמסדירים שימוש בשיטות חקירה אלימות בישראל מאז 1987. הוא שואל כיצד משפט שמצהיר על איסור עינויים יכול ליצור חריגים שמאפשרים בפועל אלימות מדינתית.",
        "הנושא הוא ליבת שלטון החוק וזכויות האדם. דמוקרטיה ליברלית נבחנת במיוחד במקרים שבהם המדינה טוענת לצורך ביטחוני חריג."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את ההסדרים המשפטיים המסדירים שימוש בשיטות חקירה אלימות בישראל מאז 1987. הוא מתאר מעבר מהשעיה רחבה של איסור העינויים למשטר משפטי מפוצל, שבו רכיבים שונים של החקירה מוחרגים מן הדין הרגיל באמצעות מבנים משפטיים נפרדים.",
        "המחקר נשען על שני רבדים של מקורות. ברובד הראשון נבחנים מסמכים רשמיים המציגים ומצדיקים את עמדת המדינה ביחס לחוקיות שיטות החקירה של שב״כ, ובהם דוחות ועדות חקירה, הנחיות ממשלתיות ופסיקות בג״ץ מהשנים 1987 עד 2018. ברובד השני נבחנים נהלים והנחיות על מעצר והחזקה של נחקרים, כולל חריגים הנוגעים לזכות להיוועץ בעורך דין, לפיקוח שיפוטי ולמנגנוני בקרה על מצב העצורים.",
        "המאמר טוען שכל אחד מן החריגים נראה צר ומידתי כשהוא נבחן לבדו, אך יחד הם יוצרים מערכת משפטית שמכשירה אלימות מדינתית. ועדת לנדוי ב־1987 עיגנה שימוש באמצעים אלימים באמצעות הגנת הצורך וקוד של כללים מיוחדים; פסק הדין המרכזי של בג״ץ ב־1999 דחה פטור גורף כזה, אך הניתוח מראה שהחריגים המשיכו להתפצל ולהתרבות במסלולים משפטיים אחרים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען?",
      "paragraphsHtml": [
        "המאמר מתאר מעבר מהשעיה רחבה של איסור העינויים למשטר משפטי מפוצל. במקום חריג אחד, נוצרים חריגים לפי מבצע החקירה, האדם הנחקר, זמן החקירה והמקום שבו היא מתרחשת.",
        "הטענה המרכזית היא שכל חריג נראה צר ומידתי בפני עצמו, אך יחד הם יוצרים מערכת שלמה שמכשירה אלימות. כך ריבוי המשפט אינו בהכרח מצמצם כוח אלא יכול לייצר קטגוריות שמצדיקות אותו."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: מצב חריג ושלטון החוק",
      "paragraphsHtml": [
        "מצב חריג הוא מצב שבו המדינה טוענת שיש הצדקה לסטות מן הדין הרגיל בגלל סכנה חמורה. בדמוקרטיה ליברלית, הבעיה היא שהחריג עלול להפוך לשגרה משפטית.",
        "המאמר מראה שהחריג אינו תמיד ביטול של החוק. לעיתים הוא נוצר דווקא דרך פרטים משפטיים רבים שמסווגים, מגבילים לכאורה, ובפועל מאפשרים פעולה אלימה."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בשאלה אם שלטון החוק מגביל את המדינה או מספק לה שפה להצדקת כוח. כאשר איסור מוחלט מתפצל לחריגים רבים, ההגנה על כבוד האדם נעשית פגיעה יותר.",
        "המאמר חשוב משום שהוא אינו מסתפק בשאלה אם יש חריג אלא בוחן את הארכיטקטורה של החריגים. בכך הוא מסייע לזהות כיצד זכויות נשחקות דרך מנגנונים משפטיים מדויקים לכאורה."
      ]
    },
    {
      "headingHe": "איזו שאלה המחקר מציב?",
      "paragraphsHtml": [
        "השאלה היא כיצד המשפט הישראלי מסדיר שיטות חקירה אלימות מאז 1987. זו שאלה על הגבול בין ביטחון, זכויות אדם ושלטון החוק."
      ]
    },
    {
      "headingHe": "באיזו שיטה הוא משתמש?",
      "paragraphsHtml": [
        "המאמר הוא ניתוח משפטי ותיאורטי של מבנים משפטיים, פסיקה וקטגוריות של חריג. הוא בוחן כיצד כל רכיב של החקירה יכול להיות מופרד מן האיסור הכללי."
      ]
    },
    {
      "headingHe": "מה הממצא המרכזי לקורא הישראלי?",
      "paragraphsHtml": [
        "הממצא המרכזי הוא שהחריג התפצל לכמה מנגנונים משפטיים, שכל אחד מהם מוצג כצר ומידתי. יחד, המנגנונים יוצרים מערכת שמאפשרת הצדקה רחבה יותר של אלימות מדינתית."
      ]
    },
    {
      "headingHe": "איך זה קשור לשוויון או לזכויות?",
      "paragraphsHtml": [
        "הקשר לזכויות הוא ברור במיוחד באיסור עינויים ובזכות לכבוד ולשלמות הגוף. כאשר החריגים מתרבים, הזכות אינה נשללת במפורש אך ההגנה עליה נשחקת בפועל."
      ]
    },
    {
      "headingHe": "מה המשמעות למוסדות ציבוריים?",
      "paragraphsHtml": [
        "בתי משפט, יועצים משפטיים וגופי ביטחון צריכים לשים לב לא רק לניסוח של כל חריג אלא לאפקט המצטבר. שלטון החוק דורש לבחון אם המערכת כולה מגבילה כוח או מנרמלת אותו."
      ]
    },
    {
      "headingHe": "איזו זהירות פרשנית נדרשת כאן?",
      "paragraphsHtml": [
        "המאמר אינו טוען שכל הבחנה משפטית היא פסולה. הוא מזהיר מפני מצב שבו הבחנות רבות יוצרות ביחד לגיטימציה למה שהאיסור הכללי היה אמור למנוע."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למדיניות ציבורית?",
      "paragraphsHtml": [
        "למדיניות ציבורית, המאמר תומך בבחינה מחודשת של חריגים ביטחוניים ובחיזוק מנגנוני ביקורת. במקום להסתפק במידתיות נקודתית, יש לבדוק את ההשפעה המצטברת על זכויות."
      ]
    },
    {
      "headingHe": "מה יחפש כאן מי שמתעניין בנושא?",
      "paragraphsHtml": [
        "מי שמחפש על עינויים, שיטות חקירה ושלטון החוק בישראל ימצא כאן ניתוח של משפט החריג. המאמר מתאים במיוחד לשאלות על איך חוק יכול גם להגביל כוח וגם להצדיק אותו."
      ]
    },
    {
      "headingHe": "איזה מושג צריך לזכור כדי להבין את המאמר?",
      "paragraphsHtml": [
        "המושג החשוב הוא מצב חריג, משום שהוא מסביר כיצד סטייה מן הדין יכולה להפוך לדפוס מוסדי. המאמר מחדש בכך שהוא מציג חריג מפוצל ולא רק חריג יחיד."
      ]
    },
    {
      "headingHe": "מה התרומה של המאמר למאגר?",
      "paragraphsHtml": [
        "התרומה למאגר היא העמקת הדיון בזכויות אדם דרך ניתוח משפטי של מנגנוני כוח. הוא מדגים כיצד דמוקרטיה עלולה להחליש זכויות דווקא באמצעות שפה משפטית מורכבת."
      ]
    }
  ],
  "layout": "paper",
  "title": "כיצד החריג המשפטי מאפשר שיטות חקירה אלימות בישראל",
  "permalink": "/democracy_fracturing_exception_violent_interrogation_methods_israel_summary_he.html",
  "paper_url": "democracy_fracturing_exception_violent_interrogation_methods_israel_summary_he.html",
  "sortKey": 202606250003,
  "sourcePublishedDate": "2020-05-19"
}
---
