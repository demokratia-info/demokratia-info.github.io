---
{
  "version": 1,
  "file": "democracy_the_strategic_common_law_court_of_aharon_barak_and_its_aftermath_on_judicially_led_constitutional_summary_he.html",
  "slug": "democracy_the_strategic_common_law_court_of_aharon_barak_and_its_aftermath_on_judicially_led_constitutional_summary_he",
  "order": 391,
  "titleHe": "בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית",
  "subtitleHe": "",
  "descriptionHe": "תמצית בעברית של מאמר על בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית, עם דגש על המשמעות לדמוקרטיה ליברלית, לשלטון החוק, לזכויות ולאחריות ציבורית.",
  "summaryHe": "המאמר בוחן את סוגיית בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית דרך עדשה של חוקתיות, ביקורת על כוח שלטוני ושמירה על שלטון החוק.",
  "authorsCardHe": "Rivka Weill",
  "paperTitle": "The Strategic Common Law Court of Aharon Barak and its Aftermath: On Judicially-Led Constitutional Revolutions and Democratic Backsliding",
  "authorsHtml": "<a href=\"https://www.runi.ac.il/en/faculty/rweill\" target=\"_blank\" rel=\"noopener noreferrer\">Rivka Weill</a>",
  "journal": "Law & Ethics of Human Rights",
  "dateText": "פורסם: 2020",
  "doiUrl": "https://doi.org/10.1515/lehr-2020-2017",
  "doiLabel": "https://doi.org/10.1515/lehr-2020-2017",
  "authors": [
    {
      "@type": "Person",
      "name": "Rivka Weill",
      "url": "https://www.runi.ac.il/en/faculty/rweill"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Rivka Weill",
      "url": "https://www.runi.ac.il/en/faculty/rweill"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "שחיקת מוסדות",
    "רובנות",
    "משבר דמוקרטי",
    "חוקתיות",
    "שלטון החוק",
    "בתי משפט",
    "ביקורת שיפוטית",
    "הגבלת כוח"
  ],
  "image": {
    "src": "html_qa/democracy_the_strategic_common_law_court_of_aharon_barak_and_its_aftermath_on_judicially_led_constitutional_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית מדגים כיצד סוגיה אקדמית ממוקדת הופכת לשאלה על <strong>דמוקרטיה ליברלית</strong>, אחריות ציבורית, שוויון והגבלת כוח.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "נסיגה דמוקרטית היא תהליך שבו מוסדות, זכויות, בחירות חופשיות או בלמים ואיזונים נחלשים בהדרגה, גם כאשר המסגרת הדמוקרטית הפורמלית עדיין קיימת. בהקשר של המאמר, המושג מאפשר לראות שינוי מצטבר בכללי המשחק ולא רק משבר נקודתי או מחלוקת פוליטית רגילה.",
        "חוקה היא מערכת כללי יסוד שמגדירה את מבנה השלטון, סמכויותיו וזכויות היסוד; חוקתנות היא התפיסה שלפיה גם רוב פוליטי ומוסדות שלטון מוגבלים על ידי כללים אלה. בהקשר של המאמר, המושג מסייע להבין כיצד כללי יסוד, בתי משפט וחוקי יסוד יכולים להגביל כוח ולייצב מחלוקת פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: ישראל היא מקרה בוחן חשוב לשתי האג'נדות. המאמר מסתיים בהסברים לתגובת הגב הפוליטית שחווה בית המשפט העליון הישראלי הנוכחי, שחלקם אפיינו כנסיגה דמוקרטית. לטענתה, בית המשפט תורם לפיתוח המשפט כחלק מדיאלוג ואינטראקציה בין רשויות השלטון השונות.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון בבית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון בבית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון בבית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "בית המשפט של אהרן ברק, מהפכות חוקתיות ונסיגה דמוקרטית",
  "permalink": "/democracy_the_strategic_common_law_court_of_aharon_barak_and_its_aftermath_on_judicially_led_constitutional_summary_he.html",
  "paper_url": "democracy_the_strategic_common_law_court_of_aharon_barak_and_its_aftermath_on_judicially_led_constitutional_summary_he.html",
  "sortKey": 202605220092,
  "sourcePublishedDate": "2020-11-01"
}
---
