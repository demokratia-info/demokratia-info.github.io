---
{
  "version": 1,
  "file": "democracy_meso_institutions_unbundling_summary_he.html",
  "slug": "democracy_meso_institutions_unbundling_summary_he",
  "titleHe": "מוסדות ביניים מסבירים איך עקרונות מוסדיים מיושמים בשטח",
  "subtitleHe": "",
  "descriptionHe": "המאמר מציע לפרק את המושג הרחב \"מוסדות\" לשכבות, ולהתמקד במוסדות ביניים שמתרגמים כללים ונורמות כלליים לפעולה של שחקנים בשטח.",
  "summaryHe": "Ménard ו־Martino טוענים שמוסדות אינם ניתנים להבנה או למדידה היטב כמושג כוללני אחד. הם מציעים מסגרת של <strong>meso-institutions</strong> שמחברת בין כללי מאקרו לבין התנהגות מיקרו, וכך מאפשרת מחקר אמפירי מדויק יותר.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Claude Ménard, Gaetano Martino",
  "paperTitle": "Unbundling institutions: the case for meso-institutions",
  "authorsHtml": "<a href=\"https://claudemenard.net/\" target=\"_blank\" rel=\"noopener noreferrer\">Claude Ménard</a>; <a href=\"https://www.unipg.it/personale/gaetano.martino\" target=\"_blank\" rel=\"noopener noreferrer\">Gaetano Martino</a>",
  "journal": "Journal of Institutional Economics, 21",
  "dateText": "פורסם: 2025",
  "doiUrl": "https://doi.org/10.1017/s1744137425100192",
  "doiLabel": "https://doi.org/10.1017/s1744137425100192",
  "authors": [
    {
      "@type": "Person",
      "name": "Claude Ménard",
      "url": "https://claudemenard.net/"
    },
    {
      "@type": "Person",
      "name": "Gaetano Martino",
      "url": "https://www.unipg.it/personale/gaetano.martino"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Claude Ménard",
      "url": "https://claudemenard.net/"
    },
    {
      "@type": "Person",
      "name": "Gaetano Martino",
      "url": "https://www.unipg.it/personale/gaetano.martino"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity",
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "מוסדות",
    "מוסדות ביניים",
    "יישום מדיניות",
    "כללים",
    "נורמות",
    "תאוריה מוסדית",
    "שלטון החוק"
  ],
  "image": {
    "src": "assets/article-images/real_photo_483_court_democracy_meso_institutions_unbundling_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על מוסדות ביניים המחברים בין כללים ציבוריים ליישום מקומי",
    "fitness": "high",
    "license": "Public domain",
    "creator": "Lazar Diner (לזר דינר)",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:First_five_Israeli_Supreme_Court_judges.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/4/42/First_five_Israeli_Supreme_Court_judges.jpg",
    "fileTitle": "First five Israeli Supreme Court judges.jpg",
    "sourceDimensions": "1779x852",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:court",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://commons.wikimedia.org/wiki/Commons:Copyright_tags#Public_domain"
  },
  "datePublished": "2026-05-28",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר טוען שכדי להבין מדוע כללים מוסדיים פועלים אחרת במקומות שונים, צריך לבחון את מוסדות הביניים שמתרגמים אותם ליישום, התאמה או התנגדות.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>מוסדות ביניים</strong> הם מנגנונים שמחברים בין כללי־על לבין פעולות של אנשים, ארגונים וקהילות. הם אינם החוק עצמו ואינם ההתנהגות הפרטית עצמה, אלא השכבה שמאפשרת תרגום, התאמה ויישום."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר טוען שהניתוח הכלכלי של מוסדות התקדם, אך המושג \"מוסדות\" נותר רחב ועמום מדי ולעיתים משמש כקטגוריה כוללת בלי קשר ברור לאסטרטגיית מחקר. לכן הוא מציע לפרק את המושג לשכבות מוסדיות במקום להסתפק בטענה הכללית שמוסדות חשובים.",
        "המסגרת שמוצעת במאמר מבחינה בין שכבת מאקרו, שבה נקבעים כללים מכוננים, נורמות ואמונות, לבין שכבת מיקרו, שבה פועלים השחקנים. בין שתי השכבות נמצאים <strong>meso-institutions</strong>: מנגנוני תמסורת שמתווכים, מפרשים ומיישמים את הכללים או מעצבים את האופן שבו שחקנים מצייתים להם, מתאימים אותם או מתנגדים להם.",
        "המאמר טוען שהדגשת מוסדות הביניים מאפשרת מעבר מתיאוריה כללית מדי לתיאוריות עזר שניתנות לבדיקה אמפירית. הוא מציין שאפשר לחקור את תהליך התמסורת באמצעות חקרי מקרה, ניתוח טקסטואלי, סטטיסטיקה, מבחנים אקונומטריים וניסויי מעבדה או שדה, וכך לבנות מדידה טובה יותר של תפקידי מוסדות שונים."
      ]
    },
    {
      "headingHe": "איזו בעיה מושגית המאמר מזהה?",
      "paragraphsHtml": [
        "המחברים טוענים שהמונח מוסדות הפך לעיתים רחב מדי. כאשר כללים, נורמות, אמונות, ארגונים והתנהגויות נקראים כולם באותו שם, קשה לבנות אסטרטגיית מחקר שמסבירה מנגנונים סיבתיים."
      ]
    },
    {
      "headingHe": "מה מוסדות ביניים מוסיפים לניתוח?",
      "paragraphsHtml": [
        "הם מאפשרים לראות את הדרך שבה כללים רחבים הופכים לשגרות, נהלים, פיקוח ופרשנות מקומית. בלי השכבה הזאת, ניתוח מוסדי עלול לקפוץ מהר מדי מחוקה או מחוק לתוצאה חברתית."
      ]
    },
    {
      "headingHe": "איך המאמר מחבר בין מאקרו למיקרו?",
      "paragraphsHtml": [
        "ברמת המאקרו נמצאים כללים, נורמות ואמונות רחבות. ברמת המיקרו פועלים אזרחים, פקידים, עסקים וארגונים; מוסדות הביניים הם מנגנוני התמסורת שמעצבים את המפגש ביניהם."
      ]
    },
    {
      "headingHe": "למה זה חשוב לרפורמות דמוקרטיות?",
      "paragraphsHtml": [
        "רפורמה דמוקרטית אינה מסתיימת בחקיקה או בהכרזה מוסדית. צריך להבין אילו גופים, נהלים, תמריצים ויכולות יגרמו לכלל החדש לפעול בפועל ולא רק להופיע בספר החוקים."
      ]
    },
    {
      "headingHe": "מה המשמעות לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק תלוי לא רק בניסוח כללים אלא גם בשכבות שמפרשות, אוכפות ומנגישות אותם. בתי משפט, רגולטורים, פקידי רישוי ומנגנוני ביקורת יכולים להיות מוסדות ביניים שמחברים בין עיקרון משפטי לבין חוויה אזרחית."
      ]
    },
    {
      "headingHe": "איך ההבחנה עוזרת להסביר כישלון יישום?",
      "paragraphsHtml": [
        "כאשר כלל פורמלי נכשל, ייתכן שהבעיה אינה בעיקרון עצמו אלא במנגנוני התמסורת. מוסדות ביניים חלשים יכולים לגרום לכך שרפורמה טובה תגיע אל האזרחים כבירוקרטיה, אי־ודאות או אכיפה סלקטיבית."
      ]
    },
    {
      "headingHe": "האם מוסדות ביניים הם תמיד ארגונים רשמיים?",
      "paragraphsHtml": [
        "לא בהכרח. הם יכולים להיות נהלים פורמליים, מנגנוני תיאום, פרקטיקות מקצועיות או מסגרות פרשנות שמכוונות התנהגות. החשוב הוא התפקיד שלהם כמתווכים בין כלל רחב לבין פעולה קונקרטית."
      ]
    },
    {
      "headingHe": "מהי התרומה למחקר כלכלי־מוסדי?",
      "paragraphsHtml": [
        "המאמר מציע מסגרת שמאפשרת לאסוף ראיות באופן מדויק יותר. במקום למדוד איכות מוסדית כמספר כולל בלבד, אפשר לבדוק אילו שכבות תמסורת קיימות, כיצד הן פועלות, ואיפה הן נשברות."
      ]
    },
    {
      "headingHe": "איך זה משנה ניתוח של מדיניות ציבורית?",
      "paragraphsHtml": [
        "מדיניות ציבורית צריכה להיבחן לפי שרשרת היישום שלה. המאמר מזכיר שמוסד מוצלח הוא לא רק כלל נכון, אלא מערכת שמסוגלת לתרגם את הכלל להחלטות עקביות, מובנות וניתנות לביקורת."
      ]
    },
    {
      "headingHe": "איזו תובנה אזרחית עולה מן המאמר?",
      "paragraphsHtml": [
        "כאשר אזרחים חווים פער בין זכויות כתובות לבין שירות בפועל, לעיתים הפער נמצא בשכבת הביניים. לכן ביקורת דמוקרטית צריכה לשאול לא רק מה החוק אומר, אלא מי מתרגם אותו, באילו כלים, ובפני מי הוא נותן דין וחשבון."
      ]
    }
  ],
  "layout": "paper",
  "title": "מוסדות ביניים מסבירים איך עקרונות מוסדיים מיושמים בשטח",
  "permalink": "/democracy_meso_institutions_unbundling_summary_he.html",
  "paper_url": "democracy_meso_institutions_unbundling_summary_he.html",
  "sortKey": 202605280008
}
---
