---
{
  "version": 1,
  "file": "democracy_geopolitics_environmental_issues_geography_education_france_israel_summary_he.html",
  "slug": "democracy_geopolitics_environmental_issues_geography_education_france_israel_summary_he",
  "titleHe": "חינוך סביבתי בלי פוליטיקה מחמיץ את ממד הכוח של משבר האקלים",
  "subtitleHe": "השוואה בין צרפת לישראל מראה שמורים מכירים בקשר בין החלטות סביבתיות לפוליטיקה, אך מתקשים לשלב היבטים פוליטיים מקומיים בהוראת גאוגרפיה.",
  "descriptionHe": "המאמר בוחן כיצד סוגיות סביבתיות ופוליטיות משתלבות בהוראת גאוגרפיה בצרפת ובישראל, ומה מונע ממורים להפוך את הקשר הזה לחלק ברור מן ההכשרה והכיתה.",
  "summaryHe": "Tal Yaar-Waisel ו-Caroline Leininger-Frézal מנתחות תוכניות לימודים וראיונות עם 20 מורים בצרפת ובישראל. הן מראות כי אף שתוכניות הלימודים כוללות גאופוליטיקה סביבתית ומורים מזהים קשר בין סביבה לפוליטיקה, היבטים פוליטיים מקומיים כמעט אינם מודגשים והכשרת המורים אינה תמיד מכינה לכך.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Tal Yaar-Waisel, Caroline Leininger-Frézal",
  "paperTitle": "Bridging Geopolitics and Environmental Issues in Geography Education: Exploratory Teachers’ Insights from France and Israel",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-9817-7290\" target=\"_blank\" rel=\"noopener noreferrer\">Tal Yaar-Waisel</a>, <a href=\"https://orcid.org/0000-0003-0890-6408\" target=\"_blank\" rel=\"noopener noreferrer\">Caroline Leininger-Frézal</a>",
  "journal": "European Journal of Geography 16(2), 184-196",
  "dateText": "פורסם: 19 במאי 2025",
  "doiUrl": "https://doi.org/10.48088/ejg.t.yaa.16.2.184.196",
  "doiLabel": "https://doi.org/10.48088/ejg.t.yaa.16.2.184.196",
  "authors": [
    {
      "@type": "Person",
      "name": "Tal Yaar-Waisel",
      "url": "https://orcid.org/0000-0001-9817-7290"
    },
    {
      "@type": "Person",
      "name": "Caroline Leininger-Frézal",
      "url": "https://orcid.org/0000-0003-0890-6408"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Tal Yaar-Waisel",
      "url": "https://orcid.org/0000-0001-9817-7290"
    },
    {
      "@type": "Person",
      "name": "Caroline Leininger-Frézal",
      "url": "https://orcid.org/0000-0003-0890-6408"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "גאופוליטיקה סביבתית",
    "חינוך גאוגרפי",
    "הכשרת מורים",
    "צרפת",
    "ישראל",
    "אזרחות סביבתית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_267_education_democracy_geopolitics_environmental_issues_geography_education_france_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לחינוך סביבתי בלי פוליטיקה מחמיץ את ממד הכוח של משבר האקלים",
    "fitness": "high",
    "license": "CC BY-SA 2.0",
    "creator": "U.S. Embassy Tel Aviv",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:America_Day_BeerSheva_No.392_(7002922087).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/a/ad/America_Day_BeerSheva_No.392_%287002922087%29.jpg",
    "fileTitle": "America Day BeerSheva No.392 (7002922087).jpg",
    "sourceDimensions": "1200x800",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:education",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/2.0/"
  },
  "datePublished": "2026-06-18",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "הוראת סביבה נעשית אזרחית יותר כאשר היא מסבירה מי מחליט, מי נפגע ומי משתתף בעיצוב המדיניות.",
  "sections": [
    {
      "headingHe": "למה סוגיות סביבתיות צריכות להילמד גם כפוליטיקה?",
      "paragraphsHtml": [
        "המאמר טוען שסוגיות סביבתיות אינן רק תהליכים טבעיים או מדעיים. הן קשורות להחלטות שלטוניות, חלוקת משאבים, אחריות ציבורית והיכולת של אזרחים להבין מי מרוויח ומי נפגע ממדיניות סביבתית."
      ]
    },
    {
      "headingHe": "מה בדקו החוקרות בצרפת ובישראל?",
      "paragraphsHtml": [
        "המחקר משלב ניתוח של תוכנית הלימודים המיועדת ושל מה שמתרחש בפועל אצל מורים. בנוסף לכך נערכו ראיונות עם 20 מורים בשתי המדינות, כדי לזהות חסמים משותפים בהוראה של גאופוליטיקה סביבתית."
      ]
    },
    {
      "headingHe": "מה נמצא בתוכניות הלימודים?",
      "paragraphsHtml": [
        "לפי התקציר, תוכניות הלימודים בשתי המדינות כבר כוללות רכיבים של גאופוליטיקה סביבתית. עם זאת, עצם הופעת הנושא במסמך רשמי אינה מבטיחה שמורים ירגישו מוכנים ללמד אותו באופן פוליטי ומקומי."
      ]
    },
    {
      "headingHe": "מה אמרו המורים על הקשר בין סביבה לפוליטיקה?",
      "paragraphsHtml": [
        "המורים הכירו בקשר בין החלטות סביבתיות לבין החלטות פוליטיות. למרות זאת, הם הזכירו מעט היבטים פוליטיים מקומיים ורבים מהם לא הכירו את החיבור הזה מן ההכשרה האקדמית הקודמת שלהם."
      ]
    },
    {
      "headingHe": "מדוע זמן הוראה מוגבל משנה את התמונה?",
      "paragraphsHtml": [
        "התקציר מציין שהזמן המוקדש לנושא מוגבל, אך מורים בכל זאת משלבים אותו מתוך תחושת שליחות. המשמעות היא שהמחויבות האישית של מורים ממלאת חלל שמערכת ההכשרה ותוכנית הלימודים אינן תמיד ממלאות בצורה מספקת."
      ]
    },
    {
      "headingHe": "מה הקשר לדמוקרטיה ולזכויות?",
      "paragraphsHtml": [
        "חינוך סביבתי דמוקרטי מאפשר לתלמידים להבין את ההשלכות החברתיות של החלטות על מים, קרקע, אנרגיה ותשתיות. כאשר הפוליטיקה של הסביבה אינה נלמדת, קשה יותר לראות כיצד מדיניות סביבתית משפיעה באופן לא שווה על קבוצות וקהילות."
      ]
    },
    {
      "headingHe": "כיצד המאמר תורם להכשרת מורים?",
      "paragraphsHtml": [
        "המאמר מציע מודל שמטרתו להקל על שילוב גאופוליטיקה סביבתית בפרקטיקה ההוראתית. התרומה שלו היא חיבור בין תוכנית לימודים, הכשרת מורים והכיתה עצמה, במקום להטיל את מלוא האחריות על יוזמה אישית של המורה."
      ]
    },
    {
      "headingHe": "מה ניתן ללמוד מן ההשוואה הבין־לאומית?",
      "paragraphsHtml": [
        "ההשוואה בין צרפת לישראל מראה שחסמים דומים יכולים להופיע במערכות חינוך שונות. לכן הבעיה אינה רק מקומית, אלא קשורה לאופן שבו דיסציפלינת הגאוגרפיה מתמודדת עם פוליטיקה, סביבה ואזרחות."
      ]
    },
    {
      "headingHe": "מה תלמידים עשויים להרוויח מהוראה כזאת?",
      "paragraphsHtml": [
        "תלמידים יכולים ללמוד לקרוא מפות, משברים סביבתיים והחלטות תכנון גם כשאלות של כוח והשתתפות. הבנה כזאת מחזקת אוריינות אזרחית משום שהיא מחברת בין ידע מרחבי לבין אחריות ציבורית."
      ]
    },
    {
      "headingHe": "מהו האתגר המעשי המרכזי?",
      "paragraphsHtml": [
        "האתגר הוא להפוך נושא שמופיע לעיתים בשוליים למרכיב יציב בהכשרת מורים ובשיעורי גאוגרפיה. לשם כך נדרשים זמן הוראה, כלים פדגוגיים ושפה מקצועית שמאפשרת לדון בפוליטיקה סביבתית בלי להפוך את הכיתה לזירת הטפה."
      ]
    }
  ],
  "layout": "paper",
  "title": "חינוך סביבתי בלי פוליטיקה מחמיץ את ממד הכוח של משבר האקלים",
  "permalink": "/democracy_geopolitics_environmental_issues_geography_education_france_israel_summary_he.html",
  "paper_url": "democracy_geopolitics_environmental_issues_geography_education_france_israel_summary_he.html",
  "sortKey": 202606180008,
  "sourcePublishedDate": "2025-05-19"
}
---
