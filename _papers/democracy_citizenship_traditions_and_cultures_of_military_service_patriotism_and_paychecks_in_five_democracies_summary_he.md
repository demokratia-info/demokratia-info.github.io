---
{
  "version": 1,
  "file": "democracy_citizenship_traditions_and_cultures_of_military_service_patriotism_and_paychecks_in_five_democracies_summary_he.html",
  "slug": "democracy_citizenship_traditions_and_cultures_of_military_service_patriotism_and_paychecks_in_five_democracies_summary_he",
  "order": 454,
  "titleHe": "מסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות",
  "subtitleHe": "",
  "descriptionHe": "המאמר משווה כיצד ציבור בחמש דמוקרטיות מפרש את מניעי השירות הצבאי: פטריוטיות ואזרחות טובה לעומת שכר, הטבות או היעדר חלופות. הוא קושר את ההבדלים לשיחי אזרחות לאומיים ולמאפייני הפעילות הצבאית.",
  "summaryHe": "המאמר מציג סקרים מייצגים מצרפת, גרמניה, ישראל, בריטניה וארצות הברית שנערכו בין 2018 ל-2021. הממצאים מראים שונות רחבה בין מדינות בתפיסת מניעי השירות, ומציעים שההסבר העיקרי נעוץ בשילוב בין תרבות אזרחות לאומית לקצב המבצעי של הצבא.",
  "authorsCardHe": "Ronald R. Krebs, Robert Ralston, Thierry Balzacq, David Blagden, Shaul R. Shenhav, Markus Steinbrecher",
  "paperTitle": "Citizenship Traditions and Cultures of Military Service: Patriotism and Paychecks in Five Democracies",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-6504-060X\" target=\"_blank\" rel=\"noopener noreferrer\">Ronald R. Krebs</a>, <a href=\"https://orcid.org/0000-0002-4470-1242\" target=\"_blank\" rel=\"noopener noreferrer\">Robert Ralston</a>, <a href=\"https://orcid.org/0000-0002-4242-5088\" target=\"_blank\" rel=\"noopener noreferrer\">Thierry Balzacq</a>, <a href=\"https://orcid.org/0000-0001-6923-4946\" target=\"_blank\" rel=\"noopener noreferrer\">David Blagden</a>, <a href=\"https://orcid.org/0000-0002-8820-3579\" target=\"_blank\" rel=\"noopener noreferrer\">Shaul R. Shenhav</a>, Markus Steinbrecher",
  "journal": "Armed Forces & Society",
  "dateText": "פורסם: 2024",
  "doiUrl": "https://doi.org/10.1177/0095327x241275635",
  "doiLabel": "https://doi.org/10.1177/0095327x241275635",
  "authors": [
    {
      "@type": "Person",
      "name": "Ronald R. Krebs",
      "url": "https://orcid.org/0000-0001-6504-060X"
    },
    {
      "@type": "Person",
      "name": "Robert Ralston",
      "url": "https://orcid.org/0000-0002-4470-1242"
    },
    {
      "@type": "Person",
      "name": "Thierry Balzacq",
      "url": "https://orcid.org/0000-0002-4242-5088"
    },
    {
      "@type": "Person",
      "name": "David Blagden",
      "url": "https://orcid.org/0000-0001-6923-4946"
    },
    {
      "@type": "Person",
      "name": "Shaul R. Shenhav",
      "url": "https://orcid.org/0000-0002-8820-3579"
    },
    {
      "@type": "Person",
      "name": "Markus Steinbrecher"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ronald R. Krebs",
      "url": "https://orcid.org/0000-0001-6504-060X"
    },
    {
      "@type": "Person",
      "name": "Robert Ralston",
      "url": "https://orcid.org/0000-0002-4470-1242"
    },
    {
      "@type": "Person",
      "name": "Thierry Balzacq",
      "url": "https://orcid.org/0000-0002-4242-5088"
    },
    {
      "@type": "Person",
      "name": "David Blagden",
      "url": "https://orcid.org/0000-0001-6923-4946"
    },
    {
      "@type": "Person",
      "name": "Shaul R. Shenhav",
      "url": "https://orcid.org/0000-0002-8820-3579"
    },
    {
      "@type": "Person",
      "name": "Markus Steinbrecher"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה",
    "מוסדות",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "מדיניות ציבורית"
  ],
  "image": {
    "src": "html_qa/democracy_citizenship_traditions_and_cultures_of_military_service_patriotism_and_paychecks_in_five_democracies_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על מסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שתפיסות ציבוריות של שירות צבאי שונות מאוד בין דמוקרטיות, וששירות נתפס לעיתים כשליחות אזרחית ולעיתים כעבודה בשכר או כברירת מחדל.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "אזרחות היא הקשר המשפטי והפוליטי בין אדם למדינה, הכולל זכויות, חובות ויכולת להשתתף בחיים הציבוריים. בהקשר של המאמר, המושג מסייע לבחון מי נחשב שותף מלא בקהילה הפוליטית ומהם גבולות השייכות, הייצוג וההגנה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן מדוע אזרחים חושבים שחיילים וקצינים מצטרפים לצבא, על בסיס סקרים מייצגים בחמש דמוקרטיות: צרפת, גרמניה, ישראל, בריטניה וארצות הברית. הסקרים נערכו בין 2018 ל-2021, ובגרמניה נוספו השאלות לסקר השנתי של המרכז להיסטוריה צבאית ולמדעי החברה של הבונדסוור שבו רואיינו 2,277 משיבים.",
        "המחקר מצא הבדלים לאומיים ברורים. בישראל ובצרפת רוב משמעותי מייחס את השירות למניעים פנימיים כגון פטריוטיות ואזרחות טובה; בארצות הברית הציבור כמעט מחולק בין הסברים פנימיים להסברים חיצוניים כגון שכר, הטבות או רצון להיחלץ מנסיבות קשות; בבריטניה ובגרמניה רוב המשיבים נוטים להסברים חיצוניים. לפי הממצאים המדווחים, בגרמניה ובבריטניה 75.9% ו-62.2% מהמשיבים בהתאמה ייחסו לחיילים מניעים חיצוניים, ואילו בצרפת ובישראל 59.3% ו-62.0% בהתאמה ייחסו להם מניעים פנימיים.",
        "המאמר טוען שההבדלים אינם מוסברים היטב רק באמצעות גודל הצבא, קצב הפעילות המבצעית או הזמן שחלף מאז ביטול הגיוס. ההסבר המשכנע יותר, לפי הניתוח, הוא שילוב בין שיח האזרחות הדומיננטי בכל מדינה לבין הקצב המבצעי של הצבא. המסקנה מרחיבה את הדיון ליחסי צבא־חברה: תפיסות כאלה עשויות להשפיע על תמיכה בהפעלת כוח, על רגישות לנפגעים, על הטבות לחיילים ולווטרנים, ועל יחסים אזרחיים־צבאיים בדמוקרטיה."
      ]
    },
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית מסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון במסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון במסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם מסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון במסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מסורות אזרחות ותרבויות שירות צבאי בדמוקרטיות",
  "permalink": "/democracy_citizenship_traditions_and_cultures_of_military_service_patriotism_and_paychecks_in_five_democracies_summary_he.html",
  "paper_url": "democracy_citizenship_traditions_and_cultures_of_military_service_patriotism_and_paychecks_in_five_democracies_summary_he.html",
  "sortKey": 202605220029,
  "sourcePublishedDate": "2024-09-27"
}
---
