---
{
  "version": 1,
  "file": "democracy_public_private_property_rights_growth_summary_he.html",
  "slug": "democracy_public_private_property_rights_growth_summary_he",
  "titleHe": "הגנה על זכויות קניין בין המדינה לשוק הפרטי",
  "subtitleHe": "",
  "descriptionHe": "המאמר מציע מודל צמיחה אנדוגנית שבו הגנה על זכויות קניין נוצרת גם על ידי המגזר הציבורי וגם על ידי המגזר הפרטי. מבחינה דמוקרטית, הוא מדגיש ששלטון החוק הכלכלי הוא תמהיל מוסדי: המדינה אינה יכולה לוותר על אחריות, אך גם מנגנונים פרטיים פועלים בתוך גבולות שהחוק קובע.",
  "summaryHe": "המחקר שואל מהו חלקם המיטבי של המגזר הציבורי והמגזר הפרטי בהגנה על זכויות קניין, כאשר ההגנה עצמה משפיעה על תמריצים, השקעה וצמיחה. משום שהמקור הוא מודל תאורטי, הסיכום אינו מציג ממצא אמפירי נקודתי אלא את ההיגיון המוסדי: הגנה על קניין דורשת גם אכיפה ציבורית אמינה וגם יכולת פרטית להגן על חוזים, נכסים והשקעות בלי להפוך את הכוח הפרטי לתחליף לשלטון החוק.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Ali Hussein Samadi, Ali Hussein Ostadzad",
  "paperTitle": "Optimum share of public and private sectors in property rights protection: An augmented endogenous growth model",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-8491-6192\" target=\"_blank\" rel=\"noopener noreferrer\">Ali Hussein Samadi</a>; <a href=\"https://orcid.org/0000-0002-2411-8340\" target=\"_blank\" rel=\"noopener noreferrer\">Ali Hussein Ostadzad</a>",
  "journal": "Economic Modelling, 33, 947-955",
  "dateText": "פורסם: יולי 2013",
  "doiUrl": "https://doi.org/10.1016/j.econmod.2013.05.026",
  "doiLabel": "https://doi.org/10.1016/j.econmod.2013.05.026",
  "authors": [
    {
      "@type": "Person",
      "name": "Ali Hussein Samadi",
      "url": "https://orcid.org/0000-0002-8491-6192"
    },
    {
      "@type": "Person",
      "name": "Ali Hussein Ostadzad",
      "url": "https://orcid.org/0000-0002-2411-8340"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ali Hussein Samadi",
      "url": "https://orcid.org/0000-0002-8491-6192"
    },
    {
      "@type": "Person",
      "name": "Ali Hussein Ostadzad",
      "url": "https://orcid.org/0000-0002-2411-8340"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "זכויות קניין",
    "שלטון החוק",
    "אכיפה ציבורית",
    "אכיפה פרטית",
    "צמיחה אנדוגנית",
    "מוסדות כלכליים"
  ],
  "image": {
    "src": "assets/article-images/real_photo_602_economy_democracy_public_private_property_rights_growth_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על הגנה ציבורית ופרטית על זכויות קניין",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Deror_avi",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Bank_of_Agriculture_Building_P1150017.JPG",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/e/e6/Bank_of_Agriculture_Building_P1150017.JPG",
    "fileTitle": "Bank of Agriculture Building P1150017.JPG",
    "sourceDimensions": "4000x3000",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:economy",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-05-27",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מציע מודל צמיחה אנדוגנית שבו הגנה על זכויות קניין נוצרת גם על ידי המגזר הציבורי וגם על ידי המגזר הפרטי. המחקר שואל מהו חלקם המיטבי של המגזר הציבורי והמגזר הפרטי בהגנה על זכויות קניין, כאשר ההגנה עצמה משפיעה על תמריצים, השקעה וצמיחה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>זכויות קניין</strong> הן ההגנות שמאפשרות לאדם, לקהילה או לעסק להחזיק נכס, להשתמש בו ולהסתמך עליו בלי חשש להפקעה שרירותית. בהקשר של המאמר, ההגנה אינה רק סעיף משפטי אלא תשומה שמעצבת השקעה, חדשנות וצמיחה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המחקר שואל מהו חלקם המיטבי של המגזר הציבורי והמגזר הפרטי בהגנה על זכויות קניין, כאשר ההגנה עצמה משפיעה על תמריצים, השקעה וצמיחה. משום שהמקור הוא מודל תאורטי, הסיכום אינו מציג ממצא אמפירי נקודתי אלא את ההיגיון המוסדי: הגנה על קניין דורשת גם אכיפה ציבורית אמינה וגם יכולת פרטית להגן על חוזים, נכסים והשקעות בלי להפוך את הכוח הפרטי לתחליף לשלטון החוק."
      ]
    },
    {
      "headingHe": "מה שואל המודל?",
      "paragraphsHtml": [
        "המודל שואל איך לחלק את מאמץ ההגנה על קניין בין מוסדות ציבוריים לבין שחקנים פרטיים. השאלה חשובה מפני שהגנה טובה מדי בצד אחד וחלשה מדי בצד אחר יכולה ליצור עלויות, כפילות או כוח לא מפוקח."
      ]
    },
    {
      "headingHe": "למה הגנת קניין אינה רק משימה של המדינה?",
      "paragraphsHtml": [
        "המדינה קובעת את המסגרת המשפטית, אך אנשים ועסקים משתמשים גם בחוזים, ביטוח, אבטחה, ארגון מקצועי ומנגנוני בקרה פרטיים. המאמר מזכיר שהשוק אינו פועל מחוץ לחוק אלא נשען עליו כדי להפוך הגנה פרטית לפעולה לגיטימית."
      ]
    },
    {
      "headingHe": "מה מוסיף המגזר הפרטי?",
      "paragraphsHtml": [
        "המגזר הפרטי יכול להגיב מהר יותר לסיכונים נקודתיים וליצור מומחיות מקומית סביב נכסים, חוזים וטכנולוגיות. היתרון הזה אינו מבטל את הצורך באכיפה ציבורית, משום שכוח פרטי לא מרוסן עלול להחליף זכויות בכוח מיקוח."
      ]
    },
    {
      "headingHe": "איפה הסיכון הדמוקרטי בתמהיל כזה?",
      "paragraphsHtml": [
        "כאשר ההגנה על קניין עוברת יותר מדי לשחקנים פרטיים, בעלי משאבים יכולים לקנות לעצמם ביטחון משפטי שאחרים אינם מקבלים. לכן הדיון הדמוקרטי אינו עוסק רק ביעילות אלא גם בשוויון גישה לכללי משחק הוגנים."
      ]
    },
    {
      "headingHe": "איך זה קשור לצמיחה אנדוגנית?",
      "paragraphsHtml": [
        "במודל צמיחה אנדוגנית, מוסדות ותמריצים משפיעים על קצב הצמיחה במקום להיות רק רקע חיצוני. הגנה על קניין יכולה לעודד השקעה וחדשנות, אבל היא גם צורכת משאבים ולכן נדרש תמהיל שאינו יקר או ריכוזי מדי."
      ]
    },
    {
      "headingHe": "מה המשמעות למדינות עם מוסדות חלשים?",
      "paragraphsHtml": [
        "במדינות שבהן בתי משפט, רישום נכסים או משטרה אינם אמינים, שחקנים פרטיים מחפשים פתרונות חלופיים. המאמר מחדד שפתרונות כאלה יכולים לעזור בטווח קצר, אך בלי שיקום מוסדי הם עלולים להעמיק אי־שוויון בגישה לביטחון קנייני."
      ]
    },
    {
      "headingHe": "האם יותר הפרטה תמיד משפרת הגנה?",
      "paragraphsHtml": [
        "לא. הפרטה של מנגנוני הגנה יכולה להקטין עומס ציבורי, אבל היא אינה ערובה לשלטון חוק אם אין פיקוח, שקיפות ויכולת ערעור. ההיגיון הדמוקרטי דורש שההגנה הפרטית תפעל בתוך מערכת ציבורית שמחייבת את כולם."
      ]
    },
    {
      "headingHe": "מה נשאר פתוח לקובעי מדיניות?",
      "paragraphsHtml": [
        "המודל מצביע על הצורך לחשוב על עלויות ההגנה ולא רק על עצם קיומן של זכויות. קובעי מדיניות צריכים לשאול מי מממן את ההגנה, מי נהנה ממנה, ומי נשאר חשוף כאשר השוק הפרטי נכנס לתפקיד אכיפתי."
      ]
    },
    {
      "headingHe": "למה זה רלוונטי לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נמדד גם ביכולת להפוך זכויות קניין לכללים צפויים, שוויוניים ואכיפים. אם ההגנה תלויה בכוח פרטי בלבד, החוק מאבד את תפקידו כמסגרת משותפת שמגבילה את החזקים ומגינה על החלשים."
      ]
    },
    {
      "headingHe": "מה התרומה לקורא ציבורי?",
      "paragraphsHtml": [
        "המאמר עוזר לראות זכויות קניין כתשתית מוסדית ולא כסוגיה טכנית של עסקים. הוא מראה שהשאלה מי מגן על הזכות חשובה כמעט כמו השאלה האם הזכות קיימת על הנייר."
      ]
    }
  ],
  "layout": "paper",
  "title": "הגנה על זכויות קניין בין המדינה לשוק הפרטי",
  "permalink": "/democracy_public_private_property_rights_growth_summary_he.html",
  "paper_url": "democracy_public_private_property_rights_growth_summary_he.html",
  "sortKey": 202605270001
}
---
