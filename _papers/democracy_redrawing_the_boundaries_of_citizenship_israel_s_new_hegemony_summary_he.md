---
{
  "version": 1,
  "file": "democracy_redrawing_the_boundaries_of_citizenship_israel_s_new_hegemony_summary_he.html",
  "slug": "democracy_redrawing_the_boundaries_of_citizenship_israel_s_new_hegemony_summary_he",
  "order": 141,
  "titleHe": "שרטוט מחדש של גבולות האזרחות בישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית שרטוט מחדש של גבולות האזרחות בישראל, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית שרטוט מחדש של גבולות האזרחות בישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Nadim N. Rouhana; Nimer Sultany",
  "paperTitle": "Redrawing the Boundaries of Citizenship: Israel's New Hegemony",
  "authorsHtml": "<a href=\"https://fletcher.tufts.edu/academics/faculty/nadim-rouhana\" target=\"_blank\" rel=\"noopener noreferrer\">Nadim N. Rouhana</a>, <a href=\"https://orcid.org/0000-0002-7922-7683\" target=\"_blank\" rel=\"noopener noreferrer\">Nimer Sultany</a>",
  "journal": "Journal of Palestine Studies",
  "dateText": "פורסם: 2003",
  "doiUrl": "https://doi.org/10.1525/jps.2003.33.1.5",
  "doiLabel": "https://doi.org/10.1525/jps.2003.33.1.5",
  "authors": [
    {
      "@type": "Person",
      "name": "Nadim N. Rouhana",
      "url": "https://fletcher.tufts.edu/academics/faculty/nadim-rouhana"
    },
    {
      "@type": "Person",
      "name": "Nimer Sultany",
      "url": "https://orcid.org/0000-0002-7922-7683"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Nadim N. Rouhana",
      "url": "https://fletcher.tufts.edu/academics/faculty/nadim-rouhana"
    },
    {
      "@type": "Person",
      "name": "Nimer Sultany",
      "url": "https://orcid.org/0000-0002-7922-7683"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "שרטוט מחדש של גבולות האזרחות בישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_redrawing_the_boundaries_of_citizenship_israel_s_new_hegemony_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על שרטוט מחדש של גבולות האזרחות בישראל",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית שרטוט מחדש של גבולות האזרחות בישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "אזרחות היא הקשר המשפטי והפוליטי בין אדם למדינה, הכולל זכויות, חובות ויכולת להשתתף בחיים הציבוריים. בהקשר של המאמר, המושג מסייע לבחון מי נחשב שותף מלא בקהילה הפוליטית ומהם גבולות השייכות, הייצוג וההגנה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מאמר זה מתמקד בהתפתחות מאז האינתיפאדה הפלסטינית השנייה של קונצנזוס חדש בחברה היהודית הישראלית ביחס למיעוט הערבי, אותו מכנים המחברים \"ההגמוניה הציונית החדשה\". לאחר תיאור הגישות והאמונות העומדות בבסיס הקונצנזוס החדש, המאמר מתמקד בארבעה תחומים שבהם הוא בא לידי ביטוי: חקיקה, מדיניות ממשלתית, דעת קהל ושיח ציבורי. התוצאה של המדיניות החדשה היא לשנות את משמעות האזרחות עבור לא-יהודים במדינה יהודית אתנית.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית שרטוט מחדש של גבולות האזרחות בישראל, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת שרטוט מחדש של גבולות האזרחות בישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר שרטוט מחדש של גבולות האזרחות בישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "שרטוט מחדש של גבולות האזרחות בישראל",
  "permalink": "/democracy_redrawing_the_boundaries_of_citizenship_israel_s_new_hegemony_summary_he.html",
  "paper_url": "democracy_redrawing_the_boundaries_of_citizenship_israel_s_new_hegemony_summary_he.html",
  "sortKey": 202605210152
}
---
