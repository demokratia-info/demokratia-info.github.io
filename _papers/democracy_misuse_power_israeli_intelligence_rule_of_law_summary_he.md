---
{
  "version": 1,
  "file": "democracy_misuse_power_israeli_intelligence_rule_of_law_summary_he.html",
  "slug": "democracy_misuse_power_israeli_intelligence_rule_of_law_summary_he",
  "titleHe": "פיקוח על מודיעין מראה כיצד ביטחון עלול לעקוף הליך חוקי",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן שלושה מקרים הקשורים למדיניות העמימות הגרעינית בישראל ולפעילות מלמ״ב, ומציג אותם כמבחן למתח בין ביטחון, סנקציות לא־משפטיות ושלטון החוק. הוא חשוב לדיון דמוקרטי מפני שמנגנוני ביטחון קטנים יחסית יכולים להשפיע עמוקות על חירויות אזרחיות.",
  "summaryHe": "Kahana ו־Sharfman מתארים כיצד מלמ״ב, הגוף המופקד על הגנת מדיניות העמימות הגרעינית, פעל במקרים מסוימים מעבר להליך הפלילי הרגיל. דרך הדיון בחקירות, הטרדות וסנקציות לא־משפטיות, המאמר מציב את שאלת גבולות הביטחון מול דמוקרטיה ושלטון החוק.",
  "authorsCardHe": "Ephraim Kahana, Daphna Sharfman",
  "paperTitle": "Misuse of power in Israeli intelligence",
  "authorsHtml": "Ephraim Kahana; Daphna Sharfman",
  "journal": "Israel Affairs, 20(1), 62-74",
  "dateText": "פורסם: 2014",
  "sourcePublishedDate": "2014-01-02",
  "doiUrl": "https://doi.org/10.1080/13537121.2013.863081",
  "doiLabel": "https://doi.org/10.1080/13537121.2013.863081",
  "authors": [
    {
      "@type": "Person",
      "name": "Ephraim Kahana"
    },
    {
      "@type": "Person",
      "name": "Daphna Sharfman"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ephraim Kahana"
    },
    {
      "@type": "Person",
      "name": "Daphna Sharfman"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "מודיעין",
    "מלמ״ב",
    "עמימות גרעינית",
    "שלטון החוק",
    "פיקוח אזרחי",
    "ביטחון לאומי",
    "סנקציות לא משפטיות"
  ],
  "image": {
    "src": "html_qa/democracy_misuse_power_israeli_intelligence_rule_of_law_summary_he.jpg",
    "version": "2026-06-01-nightly",
    "altHe": "חדר פיקוח אזרחי עם תיקי מודיעין חסויים ודיון בשלטון החוק",
    "fitness": "high"
  },
  "datePublished": "2026-06-01",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה כיצד הגנה על סוד ביטחוני יכולה לעבור מאכיפה חוקית לפעולה אפורה שמרתיעה אזרחים וחוקרים. הדיון ממקם את פיקוח המודיעין בלב שאלת שלטון החוק בדמוקרטיה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>פיקוח אזרחי על מודיעין</strong> הוא הדרישה שמנגנוני ביטחון יפעלו בתוך גבולות חוקיים ויהיו כפופים לביקורת דמוקרטית. ללא פיקוח כזה, סודיות ביטחונית עלולה להגן לא רק על אינטרס ציבורי אלא גם על שימוש עודף בכוח."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: ועדת לנדאו הייתה ועדה בת שלושה אנשים שהוקמה על ידי ממשלת ישראל ב-1987 לבחינת השערוריות הנ\"ל. בראשות שופט העליון לשעבר משה לנדאו, הוועדה מצאה כי חוקרי השב\"כ נהגו להשתמש בכוח פיזי במהלך חקירת אסירים ולאחר מכן ביצעו עדות שקר במשפטים שלאחר מכן. במסקנתו, שאושרה על ידי הקבינט בנובמבר 1987, היא קבעה הנחיות לשימוש ב\"מידה מתונה של לחץ פיזי\".",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: Kahana ו־Sharfman מתארים כיצד מלמ״ב, הגוף המופקד על הגנת מדיניות העמימות הגרעינית, פעל במקרים מסוימים מעבר להליך הפלילי הרגיל. דרך הדיון בחקירות, הטרדות וסנקציות לא־משפטיות, המאמר מציב את שאלת גבולות הביטחון מול דמוקרטיה ושלטון החוק."
      ]
    },
    {
      "headingHe": "מהו מוקד המאמר?",
      "paragraphsHtml": [
        "המאמר מתמקד במלמ״ב ובפעולות שנעשו סביב מדיניות העמימות הגרעינית של ישראל. דרך שלושה מקרים, הוא בוחן כיצד גוף ביטחוני עשוי לעבור מהגנה על סוד למרחב פעולה שאינו מתיישב בנוחות עם ההליך החוקי הרגיל."
      ]
    },
    {
      "headingHe": "מהי מדיניות העמימות הגרעינית בהקשר הזה?",
      "paragraphsHtml": [
        "העמימות הגרעינית היא מדיניות ציבורית־ביטחונית שמונעת דיון מפורש בפרטים רגישים. במאמר היא משמשת רקע לשאלה עד כמה מותר למדינה להפעיל לחץ על מי שנתפסים כמערערים על המדיניות הזאת."
      ]
    },
    {
      "headingHe": "מה הבעיה הדמוקרטית בסנקציות לא־משפטיות?",
      "paragraphsHtml": [
        "כאשר המדינה מפעילה חקירה, הטרדה או איום מחוץ למסלול פלילי ברור, האזרח מתקשה לדעת מהן זכויותיו. הבעיה אינה רק חומרת הפעולה, אלא היעדר כללים שקופים וביקורת עצמאית."
      ]
    },
    {
      "headingHe": "מדוע המאמר חשוב לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק דורש שגם גופי ביטחון יפעלו לפי סמכות מוגדרת, הליך הוגן וביקורת. המאמר מראה שהתחום הביטחוני מפתה במיוחד לחריגה מפני שהמידע חסוי והציבור מתקשה לפקח עליו."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד משלושת המקרים?",
      "paragraphsHtml": [
        "שלושת המקרים מצביעים על דפוס שבו הגנה על סוד לאומי הופכת להתנהלות מול אנשים פרטיים או חוקרים. הדפוס חשוב מפני שהוא מדגים כיצד כוח מוסדי קטן יחסית יכול להשפיע על חופש ביטוי ומחקר."
      ]
    },
    {
      "headingHe": "האם המאמר שולל צורך בסודיות ביטחונית?",
      "paragraphsHtml": [
        "לא. הוא אינו מבטל את הצורך בסודיות, אלא שואל כיצד שומרים עליה בלי להפוך אותה לרישיון לפעול מחוץ להליך תקין."
      ]
    },
    {
      "headingHe": "מה הקשר בין מודיעין לזכויות אזרח?",
      "paragraphsHtml": [
        "מודיעין פועל לעיתים באזור שבו הציבור אינו רואה את מלוא העובדות. לכן ההגנה על זכויות אזרח תלויה במנגנוני פיקוח חזקים שמסוגלים לבדוק שימוש בכוח גם כאשר המידע אינו גלוי."
      ]
    },
    {
      "headingHe": "למה זה רלוונטי למחקר ולתקשורת?",
      "paragraphsHtml": [
        "חוקרים ועיתונאים העוסקים בסוגיות ביטחון יכולים להיתקל בלחצים שאינם מופיעים ככתב אישום או צו רשמי. אם הלחצים האלה אינם מבוקרים, חופש המחקר והדיון הציבורי נפגעים בפועל."
      ]
    },
    {
      "headingHe": "מהו האיזון שהמאמר מבקש לברר?",
      "paragraphsHtml": [
        "האיזון הוא בין הגנת ביטחון לאומי לבין גבולות דמוקרטיים להפעלת סמכות. המאמר מראה שהשאלה אינה רק מה המדינה רוצה להסתיר, אלא כיצד היא מתייחסת למי שמאתגרים את הסתרת המידע."
      ]
    },
    {
      "headingHe": "מה תפקיד הכנסת ובתי המשפט בדיון כזה?",
      "paragraphsHtml": [
        "מוסדות ביקורת יכולים לדרוש הסמכה ברורה, תיעוד ופיקוח על פעולות ביטחוניות. כאשר הם אינם עושים זאת, הגבול בין אכיפה לגיטימית לבין שימוש לרעה בכוח נעשה מטושטש."
      ]
    },
    {
      "headingHe": "מהי המסקנה האזרחית המרכזית?",
      "paragraphsHtml": [
        "חברה דמוקרטית צריכה להיות מסוגלת להגן על סודות ביטחוניים בלי לוותר על הליך חוקי. המאמר מזכיר שהסכנה אינה רק דליפת מידע, אלא גם התרגלות לכך שסודיות מצמצמת דין וחשבון."
      ]
    }
  ],
  "layout": "paper",
  "title": "פיקוח על מודיעין מראה כיצד ביטחון עלול לעקוף הליך חוקי",
  "permalink": "/democracy_misuse_power_israeli_intelligence_rule_of_law_summary_he.html",
  "paper_url": "democracy_misuse_power_israeli_intelligence_rule_of_law_summary_he.html",
  "sortKey": 202606010008
}
---
