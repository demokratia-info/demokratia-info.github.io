---
{
  "version": 1,
  "file": "democracy_evolution_of_brazilian_democracy_unveiling_election_dynamics_in_political_issues_negativity_and_acclaim_summary_he.html",
  "slug": "democracy_evolution_of_brazilian_democracy_unveiling_election_dynamics_in_political_issues_negativity_and_acclaim_summary_he",
  "titleHe": "התפתחות הדמוקרטיה הברזילאית בשיח בחירות",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן כיצד מועמדים לנשיאות בברזיל השתמשו בטוויטר בבחירות 2018 ו-2022, ומקשר בין נושאי קמפיין לבין מסרים שליליים או מסרי שבח עצמי. הוא חשוב להבנת דמוקרטיה משום שהוא מראה כיצד תחרות בחירות ברשתות חברתיות יכולה לעבור מדיון במדיניות לתקשורת שלילית ומקטבת.",
  "summaryHe": "המאמר מנתח 1,191 ציוצים של מועמדים לנשיאות ברזיל בבחירות 2018 ו-2022, ומראה עלייה בשליליות לצד קשר שונה בין נושאי מדיניות לבין אסטרטגיות קמפיין.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Isabella Gonçalves, Mathias-Felipe De-Lima-Santos, Vicente Fenoll, Yossi David",
  "paperTitle": "Evolution of Brazilian Democracy: Unveiling Election Dynamics in Political Issues, Negativity, and Acclaim",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-6492-9236\" target=\"_blank\" rel=\"noopener noreferrer\">Isabella Gonçalves</a>, <a href=\"https://orcid.org/0000-0001-8879-7381\" target=\"_blank\" rel=\"noopener noreferrer\">Mathias-Felipe De-Lima-Santos</a>, <a href=\"https://orcid.org/0000-0002-5851-4237\" target=\"_blank\" rel=\"noopener noreferrer\">Vicente Fenoll</a>, <a href=\"https://www.bgu.ac.il/staffmemberslobby/davidyos/\" target=\"_blank\" rel=\"noopener noreferrer\">Yossi David</a>",
  "journal": "Politics and Governance",
  "dateText": "פורסם: 2024",
  "doiUrl": "https://doi.org/10.17645/pag.8060",
  "doiLabel": "https://doi.org/10.17645/pag.8060",
  "authors": [
    {
      "@type": "Person",
      "name": "Isabella Gonçalves",
      "url": "https://orcid.org/0000-0002-6492-9236"
    },
    {
      "@type": "Person",
      "name": "Mathias-Felipe De-Lima-Santos",
      "url": "https://orcid.org/0000-0001-8879-7381"
    },
    {
      "@type": "Person",
      "name": "Vicente Fenoll",
      "url": "https://orcid.org/0000-0002-5851-4237"
    },
    {
      "@type": "Person",
      "name": "Yossi David",
      "url": "https://www.bgu.ac.il/staffmemberslobby/davidyos/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Isabella Gonçalves",
      "url": "https://orcid.org/0000-0002-6492-9236"
    },
    {
      "@type": "Person",
      "name": "Mathias-Felipe De-Lima-Santos",
      "url": "https://orcid.org/0000-0001-8879-7381"
    },
    {
      "@type": "Person",
      "name": "Vicente Fenoll",
      "url": "https://orcid.org/0000-0002-5851-4237"
    },
    {
      "@type": "Person",
      "name": "Yossi David",
      "url": "https://www.bgu.ac.il/staffmemberslobby/davidyos/"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דמוקרטיה",
    "נסיגה דמוקרטית",
    "פופוליזם",
    "חוסן דמוקרטי",
    "דעת קהל",
    "קיטוב",
    "בחירות",
    "אלימות פוליטית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_233_elections_democracy_evolution_of_brazilian_democracy_unveiling_election_dynamics_in_political_issues.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור להתפתחות הדמוקרטיה הברזילאית בשיח בחירות",
    "fitness": "high",
    "license": "CC0",
    "creator": "Rakoon",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:20190409_091600_Polling_station_in_Israel_(cropped).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/f/f6/20190409_091600_Polling_station_in_Israel_%28cropped%29.jpg",
    "fileTitle": "20190409 091600 Polling station in Israel (cropped).jpg",
    "sourceDimensions": "2592x1850",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:elections",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/publicdomain/zero/1.0/"
  },
  "datePublished": "2026-05-23",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שבבחירות לנשיאות ברזיל ב-2022 הקמפיין בטוויטר נטה יותר לשליליות, בעוד שחינוך ובריאות נקשרו למסרי שבח ושחיתות וקוביד-19 נקשרו למסרים שליליים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "מוסדות דמוקרטיים הם הכללים והארגונים שמתרגמים רצון ציבורי למדיניות ומגבילים את השימוש בכוח. בהקשר של המאמר, המושג חשוב משום שהוא מאפשר לבחון לא רק מי מחזיק בכוח, אלא גם כיצד הכוח מבוקר ומוצדק."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן 1,191 הודעות שפורסמו בטוויטר במהלך הבחירות לנשיאות ברזיל ב-2018 וב-2022. החוקרים דגמו עד 150 פוסטים לכל מועמד ולכל מערכת בחירות מתוך 3,079 פרסומים, וקידדו מסרים שליליים נגד יריבים לצד מסרי שבח עצמי על כישורים, הישגים ועמדות מדיניות.",
        "הניתוח קידד גם נושאים פוליטיים כגון זכויות אזרח, שחיתות, קוביד-19, פשיעה, כלכלה, חינוך, מרוץ הבחירות, סביבה, יחסי חוץ, מדיניות מגדר, בריאות ומדיניות מוסדית. מאחר שכל פוסט יכול היה לכלול יותר מנושא אחד ויותר מאסטרטגיית קמפיין אחת, המחקר בחן קשרים בין נושאים לבין סוגי מסרים ולא רק שכיחות כללית.",
        "המחקר מצא שב-2022 הקמפיין היה נוטה יותר לשליליות מאשר ב-2018. חינוך ובריאות נקשרו למסרי שבח, בעוד שחיתות וקוביד-19 נקשרו למסרים שליליים; מדיניות מגדר נקשרה לשני הכיוונים, כממצא המשקף את אופייה המקוטב בפוליטיקה הברזילאית."
      ]
    },
    {
      "headingHe": "מה עומד במרכז המאמר?",
      "paragraphsHtml": [
        "המאמר עוסק בדעת קהל, בחירות וקיטוב פוליטי, כפי שעולה מכותרת המקור ומן המסגרת הביבליוגרפית שלו בכתב העת Politics and Governance בשנת 2024.  המאמר מצביע על כך שהדיון אינו מסתפק בתיאור כללי, אלא מציב קשר אנליטי בין משתנים מוסדיים לבין תוצאה חברתית, משפטית או כלכלית."
      ]
    },
    {
      "headingHe": "מהי השאלה המוסדית המרכזית?",
      "paragraphsHtml": [
        "השאלה המרכזית היא כיצד כללים ומוסדות משפיעים על התנהגות של אזרחים, רשויות, שווקים או קבוצות פוליטיות. כאשר מוסדות נתפסים כאמינים ושוויוניים יותר, קל יותר להצדיק החלטות ציבוריות גם במצבי מחלוקת.",
        "מן הזווית הדמוקרטית, הדיון חשוב מפני שהוא בוחן את הקשר בין עיצוב מוסדי לבין יכולת של חברה לשמור על גבולות כוח, זכויות והשתתפות. לכן גם מאמר כלכלי או מנהלי יכול להיות רלוונטי להבנת דמוקרטיה ליברלית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק יוצר את התשתית שבה החלטות ציבוריות וכלכליות אינן תלויות רק בכוח רגעי או בקשרים אישיים. כאשר הכללים צפויים וניתנים לאכיפה, גדלה היכולת של יחידים וארגונים לפעול בלי לחשוש מהחלטות שרירותיות.",
        "המאמר רלוונטי משום שהוא מדגיש, במישרין או בעקיפין, את החשיבות של מוסדות שמייצרים יציבות ואמון. זהו רכיב מרכזי גם בצמיחה כלכלית וגם בדמוקרטיה שמכבדת זכויות."
      ]
    },
    {
      "headingHe": "איזו תרומה ציבורית יש לקריאה בו?",
      "paragraphsHtml": [
        "הקריאה במאמר מסייעת לראות שמחלוקות על מוסדות אינן רק ויכוח בין מומחים, אלא שאלה על איכות החיים הדמוקרטית והכלכלית. מוסדות חלשים עלולים להקשות על השקעה, על אכיפת זכויות ועל קבלת החלטות הוגנת.",
        "התרומה הציבורית היא בחיבור בין שיח אקדמי לבין שאלות מדיניות מעשיות. היא מאפשרת לשאול אילו כללים מחזקים אמון ואילו הסדרים עלולים להעמיק אי־שוויון או ריכוז כוח."
      ]
    },
    {
      "headingHe": "כיצד המאמר קשור לצמיחה, רווחה או אמון?",
      "paragraphsHtml": [
        "כאשר המאמר עוסק בצמיחה, מימון או מוסדות כלכליים, נקודת המפתח היא שהשוק אינו פועל בחלל ריק. חוזים, זכויות קניין, בתי משפט, תקציבים ורגולציה הם חלק מן התנאים שמאפשרים פעילות כלכלית יציבה.",
        "גם כאשר המוקד אינו כלכלי בלבד, אמון במוסדות משפיע על נכונות הציבור לקבל החלטות ולשתף פעולה. לכן הקשר בין מוסדות לבין רווחה ציבורית הוא חלק בלתי נפרד מהדיון הדמוקרטי."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על דמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית נשענת על בחירות, אך גם על מגבלות כוח, זכויות, מומחיות מוסדית ואכיפה שווה של כללים. המאמר מסייע להבין מדוע איכות המוסדות משפיעה על עומק הדמוקרטיה ולא רק על יעילות הממשל.",
        "הלקח המרכזי הוא שמדיניות ציבורית אינה נמדדת רק בתוצאה מיידית, אלא גם בדרך שבה היא נוצרת ומי נושא בעלויותיה. מוסדות טובים אמורים להפוך את השאלות האלה לגלויות, ניתנות לביקורת ושוות יותר."
      ]
    },
    {
      "headingHe": "מה הקשר בין מוסדות לבין חופש אזרחי?",
      "paragraphsHtml": [
        "חופש אזרחי אינו תלוי רק בהצהרות ערכיות, אלא גם במוסדות שמסוגלים לאכוף זכויות ולהגביל שרירות. כאשר מוסדות אינם מתפקדים, זכויות עלולות להישאר פורמליות בלבד.",
        "המאמר חשוב משום שהוא מזכיר שהגנה על חירות דורשת תשתית של כללים, משאבים ואחריותיות. לכן שאלות מוסדיות וכלכליות הן לעיתים שאלות זכויות לכל דבר."
      ]
    },
    {
      "headingHe": "מה עשויה להיות נקודת המחלוקת?",
      "paragraphsHtml": [
        "נקודת המחלוקת עשויה להיות האם מוסדות מסוימים הם גורם עצמאי שמייצר תוצאות טובות יותר, או שהם משקפים תנאים חברתיים, היסטוריים וכלכליים מוקדמים יותר. ההבחנה הזאת חשובה במיוחד כאשר משתמשים במחקר כדי להצדיק רפורמות ציבוריות.",
        "מן הבחינה הדמוקרטית, הוויכוח אינו רק טכני אלא גם נורמטיבי. הוא שואל מי נהנה מן הכללים, מי מסוגל להשפיע עליהם, ומי עלול להיפגע כאשר מוסדות משתנים."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "השורה התחתונה היא שדעת קהל, בחירות וקיטוב פוליטי הוא נושא בעל משמעות דמוקרטית רחבה. הוא משפיע על האופן שבו כללים נוצרים, נאכפים ומצדיקים את עצמם מול אזרחים וקבוצות שונות.",
        "המאמר מוסיף נדבך לדיון בכך שהוא מחבר בין מקור אקדמי לבין שאלות של שלטון החוק, אחריותיות, שוויון ואמון. גם כאשר המסקנות המדויקות דורשות קריאה במקור, עצם המסגרת מסייעת להבין למה מוסדות טובים הם תנאי לדמוקרטיה יציבה."
      ]
    }
  ],
  "layout": "paper",
  "title": "התפתחות הדמוקרטיה הברזילאית בשיח בחירות",
  "permalink": "/democracy_evolution_of_brazilian_democracy_unveiling_election_dynamics_in_political_issues_negativity_and_acclaim_summary_he.html",
  "paper_url": "democracy_evolution_of_brazilian_democracy_unveiling_election_dynamics_in_political_issues_negativity_and_acclaim_summary_he.html",
  "sortKey": 202605230056,
  "sourcePublishedDate": "2024-05-09"
}
---
