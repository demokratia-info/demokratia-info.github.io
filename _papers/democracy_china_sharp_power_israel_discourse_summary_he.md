---
{
  "version": 1,
  "file": "democracy_china_sharp_power_israel_discourse_summary_he.html",
  "slug": "democracy_china_sharp_power_israel_discourse_summary_he",
  "titleHe": "העוצמה החדה של סין נבלמת בישראל בידי ספקנות ציבורית ושיח ביקורתי",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את ניסיונות ההשפעה של סין על שיח אקדמי ותקשורתי בישראל דרך מושג העוצמה החדה. הוא טוען שהשפעה זו הייתה מוגבלת, בין היתר בשל חשדנות ציבורית וגיאופוליטיקה אזורית.",
  "summaryHe": "Chaziza ו־Lutmar מציבים את ישראל כמקרה מבחן לחוסן דמוקרטי מול מניפולציה, צנזורה והשפעה חיצונית. לפי התקציר, סין ניסתה לעצב תפיסות ציבוריות, אך תמיכתה בעמדות המנוגדות לאינטרסים ישראליים והמודעות הציבורית הגוברת החלישו את הצלחתה.",
  "authorsCardHe": "Mordechai Chaziza, Carmela Lutmar",
  "paperTitle": "The Limits of China’s Sharp Power in Israel’s Academic and Media Discourse",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-1256-9694\" target=\"_blank\" rel=\"noopener noreferrer\">Mordechai Chaziza</a>; <a href=\"https://cris.haifa.ac.il/en/persons/carmela-lutmar/\" target=\"_blank\" rel=\"noopener noreferrer\">Carmela Lutmar</a>",
  "journal": "China Report, 61(1), 133-152",
  "dateText": "פורסם אונליין: 16.12.2024",
  "sourcePublishedDate": "2024-12-16",
  "doiUrl": "https://doi.org/10.1177/00094455241299974",
  "doiLabel": "https://doi.org/10.1177/00094455241299974",
  "authors": [
    {
      "@type": "Person",
      "name": "Mordechai Chaziza",
      "url": "https://orcid.org/0000-0003-1256-9694"
    },
    {
      "@type": "Person",
      "name": "Carmela Lutmar",
      "url": "https://cris.haifa.ac.il/en/persons/carmela-lutmar/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Mordechai Chaziza",
      "url": "https://orcid.org/0000-0003-1256-9694"
    },
    {
      "@type": "Person",
      "name": "Carmela Lutmar",
      "url": "https://cris.haifa.ac.il/en/persons/carmela-lutmar/"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "עוצמה חדה",
    "סין",
    "חוסן דמוקרטי",
    "תקשורת",
    "אקדמיה",
    "השפעה זרה",
    "צנזורה"
  ],
  "image": {
    "src": "html_qa/democracy_china_sharp_power_israel_discourse_summary_he.jpg",
    "version": "2026-06-03-nightly",
    "altHe": "חוקרים במעבדת מדיה אקדמית בוחנים גזירי עיתונות ותרשימי השפעה זרה ללא טקסט קריא",
    "fitness": "high"
  },
  "datePublished": "2026-06-03",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שחוסן דמוקרטי מול השפעה זרה תלוי לא רק בחוקי ביטחון. הוא תלוי גם בספקנות ציבורית, תקשורת ביקורתית ואקדמיה שאינה מוותרת על עצמאותה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>עוצמה חדה</strong> היא השפעה חיצונית שמנסה לעצב תפיסות ציבוריות בתוך דמוקרטיות באמצעות מניפולציה, צנזורה או חדירה לשיח. בניגוד לעוצמה רכה, היא אינה נשענת רק על משיכה תרבותית אלא גם על עיוות מידע והחלשת ביקורת."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המונח \"כוח חד\" הפך למושג בשימוש נרחב ביחסים בינלאומיים לתיאור השפעה חיצונית המכוונת לדמוקרטיות. מחקר זה בוחן את הטבע וההשפעה של כוחה החד של סין על החוסן הדמוקרטי של החברה הישראלית, במיוחד בשיח האקדמי והתקשורתי. הטענה המרכזית היא שסין ניסתה, אם כי בהצלחה מוגבלת, לבסס השפעה ולעצב את דעת הקהל בישראל באמצעות מניפולציות וצנזורה.",
        "Chaziza ו־Lutmar מציבים את ישראל כמקרה מבחן לחוסן דמוקרטי מול מניפולציה, צנזורה והשפעה חיצונית. לפי התקציר, סין ניסתה לעצב תפיסות ציבוריות, אך תמיכתה בעמדות המנוגדות לאינטרסים ישראליים והמודעות הציבורית הגוברת החלישו את הצלחתה."
      ]
    },
    {
      "headingHe": "מה בוחן המאמר במקרה הישראלי?",
      "paragraphsHtml": [
        "המאמר בוחן כיצד סין ניסתה להשפיע על שיח אקדמי ותקשורתי בישראל. הוא מתעניין לא רק בעצם הניסיון להשפיע, אלא גם בשאלה מדוע ההשפעה נותרה מוגבלת יחסית."
      ]
    },
    {
      "headingHe": "למה שיח אקדמי ותקשורתי חשוב לדמוקרטיה?",
      "paragraphsHtml": [
        "אקדמיה ותקשורת הן זירות מרכזיות לייצור ידע, ביקורת ופיקוח ציבורי. כאשר מדינה זרה מנסה לעצב אותן, הפגיעה האפשרית אינה רק ביחסי חוץ אלא גם ביכולת הציבור להבין מציאות פוליטית."
      ]
    },
    {
      "headingHe": "מה טוענים Chaziza ו־Lutmar על הצלחת סין?",
      "paragraphsHtml": [
        "לפי תקציר המאמר, ניסיונות ההשפעה של סין בישראל לא הצליחו במידה רחבה. הסיבות כוללות ספקנות ציבורית, מודעות גוברת לשאיפות גיאופוליטיות של סין והקשר אזורי שהקשה עליה לזכות באמון."
      ]
    },
    {
      "headingHe": "איך צנזורה ומניפולציה קשורות לעוצמה חדה?",
      "paragraphsHtml": [
        "עוצמה חדה פועלת כאשר מידע מוצג, מוסתר או ממוסגר כדי לשנות התנהגות פוליטית. בדמוקרטיה, הסכנה היא שהציבור יקבל החלטות על בסיס מידע שעוצב בידי גורם שאינו נושא באחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה הספקנות הישראלית חשובה?",
      "paragraphsHtml": [
        "הספקנות הציבורית מתוארת במאמר כחסם בפני השפעה חיצונית. כאשר אזרחים, עיתונאים וחוקרים מודעים לאינטרסים של שחקן זר, קשה יותר להפוך מסר מניפולטיבי לקונצנזוס ציבורי."
      ]
    },
    {
      "headingHe": "מה תפקיד הגיאופוליטיקה האזורית?",
      "paragraphsHtml": [
        "התקציר מציין כי עמדות סין ביחס לסכסוך האזורי ולציר האיראני פגעו ביכולתה להשיג אהדה בישראל. כלומר, השפעה זרה נבחנת גם דרך התאמתה לערכים, אינטרסים ותפיסות ביטחון של החברה המקומית."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למדיניות ציבורית?",
      "paragraphsHtml": [
        "מדיניות מול עוצמה חדה צריכה לשלב שקיפות, חינוך תקשורתי ועצמאות מוסדית. תגובה דמוקרטית טובה אינה רק סגירה או איסור, אלא חיזוק יכולת הציבור לזהות מניפולציה."
      ]
    },
    {
      "headingHe": "מה הסיכון בהפרזה באיום?",
      "paragraphsHtml": [
        "המאמר עצמו מדגיש גבולות להשפעה הסינית, ולכן אין להסיק שכל קשר אקדמי או תקשורתי עם סין הוא מניפולציה. ההבחנה הדמוקרטית החשובה היא בין קשר פתוח ומבוקר לבין ניסיון סמוי לעצב שיח."
      ]
    },
    {
      "headingHe": "איך המאמר תורם לדיון על חוסן דמוקרטי?",
      "paragraphsHtml": [
        "התרומה היא בהצגת חוסן כתוצר של תרבות ביקורתית ומוסדות ידע עצמאיים. דמוקרטיה עמידה אינה חסינה מהשפעה, אך היא מסוגלת לחשוף, לפרש ולתחום אותה."
      ]
    },
    {
      "headingHe": "למה המקרה הישראלי מעניין להשוואה?",
      "paragraphsHtml": [
        "ישראל משלבת פתיחות אקדמית ותקשורתית עם רגישות ביטחונית וגיאופוליטית גבוהה. לכן היא מאפשרת לבחון מתי פתיחות דמוקרטית יוצרת פגיעות, ומתי היא דווקא מחזקת יכולת התנגדות להשפעה זרה."
      ]
    }
  ],
  "layout": "paper",
  "title": "העוצמה החדה של סין נבלמת בישראל בידי ספקנות ציבורית ושיח ביקורתי",
  "permalink": "/democracy_china_sharp_power_israel_discourse_summary_he.html",
  "paper_url": "democracy_china_sharp_power_israel_discourse_summary_he.html",
  "sortKey": 202606030007
}
---
