---
{
  "version": 1,
  "file": "democracy_what_does_national_resilience_mean_in_a_democracy_evidence_from_the_united_states_and_israel_summary_he.html",
  "slug": "democracy_what_does_national_resilience_mean_in_a_democracy_evidence_from_the_united_states_and_israel_summary_he",
  "order": 318,
  "titleHe": "חוסן לאומי בדמוקרטיה: ראיות מארצות הברית ומישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק במוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית, וממקם את הדיון בתוך שאלות של מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית דרך עדשה של מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות.",
  "authorsCardHe": "Daphna Canetti, Israel Waismel-Manor, Naor Cohen, Carmit Rapaport",
  "paperTitle": "What Does National Resilience Mean in a Democracy? Evidence from the United States and Israel",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/daphna-canetti/\" target=\"_blank\" rel=\"noopener noreferrer\">Daphna Canetti</a>, <a href=\"https://cris.haifa.ac.il/en/persons/israel-waismel-manor/\" target=\"_blank\" rel=\"noopener noreferrer\">Israel Waismel-Manor</a>, Naor Cohen, <a href=\"https://cris.haifa.ac.il/en/persons/carmit-rapaport/\" target=\"_blank\" rel=\"noopener noreferrer\">Carmit Rapaport</a>",
  "journal": "Armed Forces & Society",
  "dateText": "פורסם: 2013",
  "doiUrl": "https://doi.org/10.1177/0095327x12466828",
  "doiLabel": "https://doi.org/10.1177/0095327x12466828",
  "authors": [
    {
      "@type": "Person",
      "name": "Daphna Canetti",
      "url": "https://cris.haifa.ac.il/en/persons/daphna-canetti/"
    },
    {
      "@type": "Person",
      "name": "Israel Waismel-Manor",
      "url": "https://cris.haifa.ac.il/en/persons/israel-waismel-manor/"
    },
    {
      "@type": "Person",
      "name": "Naor Cohen"
    },
    {
      "@type": "Person",
      "name": "Carmit Rapaport",
      "url": "https://cris.haifa.ac.il/en/persons/carmit-rapaport/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Daphna Canetti",
      "url": "https://cris.haifa.ac.il/en/persons/daphna-canetti/"
    },
    {
      "@type": "Person",
      "name": "Israel Waismel-Manor",
      "url": "https://cris.haifa.ac.il/en/persons/israel-waismel-manor/"
    },
    {
      "@type": "Person",
      "name": "Naor Cohen"
    },
    {
      "@type": "Person",
      "name": "Carmit Rapaport",
      "url": "https://cris.haifa.ac.il/en/persons/carmit-rapaport/"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "מוסדות ציבור",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "ממשל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_what_does_national_resilience_mean_in_a_democracy_evidence_from_the_united_states_and_israel_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית ומראה מדוע הדיון קשור למוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית, וממקם את הסוגיה בתוך הדיון הרחב על מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: לכן, במאמר זה, אנו מציעים שמאפיינים פוליטיים-פסיכולוגיים מהווים את הבסיס להגדרות שנתפסו על ידי האזרחים לגבי חוסן לאומי. על ידי השוואת הגדרות החוסן הלאומי המורכבות על ידי אזרחים של שתי מדינות דמוקרטיות המתמודדות עם איומים לאומיים של מלחמה וטרור, ארצות הברית וישראל, מצאנו שאיומים נתפסים, אופטימיות ועמדות ציבוריות כמו פטריוטיות ואמון במוסדות ממשלתיים, הם המרכיבים השכיחים ביותר בחוסן הלאומי הנתפס. על בסיס תוצאות אלו מוצגת מושג מחדש של המונח \"חוסן לאומי\".",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון במוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>מוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון במוסדות, זכויות ואחריות ציבורית בדמוקרטיה הישראלית להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "חוסן לאומי בדמוקרטיה: ראיות מארצות הברית ומישראל",
  "permalink": "/democracy_what_does_national_resilience_mean_in_a_democracy_evidence_from_the_united_states_and_israel_summary_he.html",
  "paper_url": "democracy_what_does_national_resilience_mean_in_a_democracy_evidence_from_the_united_states_and_israel_summary_he.html",
  "sortKey": 202605210318
}
---
