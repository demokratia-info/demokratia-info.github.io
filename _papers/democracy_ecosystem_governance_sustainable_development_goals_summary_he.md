---
{
  "version": 1,
  "file": "democracy_ecosystem_governance_sustainable_development_goals_summary_he.html",
  "slug": "democracy_ecosystem_governance_sustainable_development_goals_summary_he",
  "titleHe": "ממשל אקולוגי ויעדי הפיתוח בר־הקיימא של האו״ם",
  "subtitleHe": "מאמר שמחזיר את הממשל האקולוגי למרכז הדיון ביעדי הפיתוח הגלובליים.",
  "descriptionHe": "המאמר מנתח את ממשל אקולוגי ויעדי הפיתוח בר־הקיימא של האו״ם ומציג ממצא או טענה קונקרטיים מתוך מקור אקדמי מזוהה. הוא חשוב לדמוקרטיה ליברלית משום שהוא מחבר בין מוסדות ציבור, חברה אזרחית ושירות ציבורי לבין שאלות של כוח, זכויות, אחריות ציבורית ואמון במוסדות.",
  "summaryHe": "פיתוח בר קיימא שואף לתת מענה לדאגות כלכליות, חברתיות וסביבתיות, אך היעדר הנוכחי של ממשל סביבתי מגיב מעכב את ההתקדמות. פיתוח כלכלי לטווח קצר הוביל לפעולות מוגבלות, ניהול משאבים לא בר-קיימא ומערכות אקולוגיות מושפלות. אנו מסבירים את החשיבות של ממשל מערכת אקולוגית בהשגת יעדי SDG, ומציעים כמה דרכים להבטיח ששירותי מערכת אקולוגית נשקלו בצורה משמעותית. בהקשר דמוקרטי, הממצא המרכזי הוא שהנושא אינו רק טכני או מקצועי: הוא משפיע על האופן שבו כוח מופעל, מוגבל ומוצדק כלפי אזרחים וקבוצות שונות.",
  "authorsCardHe": "Liette Vasseur, Darwin Horning, Mary J. Thornbush, Emmanuelle Cohen-Shacham, Ángela Andrade, Ed Barrow, Steve R. Edwards, Piet Wit, Mike Jones",
  "paperTitle": "Complex problems and unchallenged solutions: Bringing ecosystem governance to the forefront of the UN sustainable development goals",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-7289-2675\" target=\"_blank\" rel=\"noopener noreferrer\">Liette Vasseur</a>; Darwin Horning; <a href=\"https://orcid.org/0000-0001-9354-2797\" target=\"_blank\" rel=\"noopener noreferrer\">Mary J. Thornbush</a>; <a href=\"https://orcid.org/0000-0001-6241-9147\" target=\"_blank\" rel=\"noopener noreferrer\">Emmanuelle Cohen-Shacham</a>; <a href=\"https://orcid.org/0000-0003-0075-6169\" target=\"_blank\" rel=\"noopener noreferrer\">Ángela Andrade</a>; Ed Barrow; Steve R. Edwards; Piet Wit; <a href=\"https://orcid.org/0000-0003-4862-6806\" target=\"_blank\" rel=\"noopener noreferrer\">Mike Jones</a>",
  "journal": "AMBIO",
  "dateText": "פורסם: 22.04.2017",
  "doiUrl": "https://doi.org/10.1007/s13280-017-0918-6",
  "doiLabel": "https://doi.org/10.1007/s13280-017-0918-6",
  "authors": [
    {
      "@type": "Person",
      "name": "Liette Vasseur",
      "url": "https://orcid.org/0000-0001-7289-2675"
    },
    {
      "@type": "Person",
      "name": "Darwin Horning"
    },
    {
      "@type": "Person",
      "name": "Mary J. Thornbush",
      "url": "https://orcid.org/0000-0001-9354-2797"
    },
    {
      "@type": "Person",
      "name": "Emmanuelle Cohen-Shacham",
      "url": "https://orcid.org/0000-0001-6241-9147"
    },
    {
      "@type": "Person",
      "name": "Ángela Andrade",
      "url": "https://orcid.org/0000-0003-0075-6169"
    },
    {
      "@type": "Person",
      "name": "Ed Barrow"
    },
    {
      "@type": "Person",
      "name": "Steve R. Edwards"
    },
    {
      "@type": "Person",
      "name": "Piet Wit"
    },
    {
      "@type": "Person",
      "name": "Mike Jones",
      "url": "https://orcid.org/0000-0003-4862-6806"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Liette Vasseur",
      "url": "https://orcid.org/0000-0001-7289-2675"
    },
    {
      "@type": "Person",
      "name": "Darwin Horning"
    },
    {
      "@type": "Person",
      "name": "Mary J. Thornbush",
      "url": "https://orcid.org/0000-0001-9354-2797"
    },
    {
      "@type": "Person",
      "name": "Emmanuelle Cohen-Shacham",
      "url": "https://orcid.org/0000-0001-6241-9147"
    },
    {
      "@type": "Person",
      "name": "Ángela Andrade",
      "url": "https://orcid.org/0000-0003-0075-6169"
    },
    {
      "@type": "Person",
      "name": "Ed Barrow"
    },
    {
      "@type": "Person",
      "name": "Steve R. Edwards"
    },
    {
      "@type": "Person",
      "name": "Piet Wit"
    },
    {
      "@type": "Person",
      "name": "Mike Jones",
      "url": "https://orcid.org/0000-0003-4862-6806"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "ממשל אקולוגי",
    "יעדי פיתוח בר־קיימא",
    "סביבה",
    "מדיניות ציבורית",
    "קיימות",
    "מוסדות ציבור, חברה אזרחית ושירות ציבורי",
    "מוסדות דמוקרטיים וכלכלה",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_ecosystem_governance_sustainable_development_goals_summary_he.jpg",
    "version": "800x600-landscape-2026-06-09",
    "altHe": "קבוצת מומחים ותושבים דנה במערכות אקולוגיות, מים ותכנון סביב שולחן מחקר.",
    "fitness": "high"
  },
  "datePublished": "2026-06-09",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "פיתוח בר קיימא שואף לתת מענה לדאגות כלכליות, חברתיות וסביבתיות, אך היעדר הנוכחי של ממשל סביבתי מגיב מעכב את ההתקדמות. פיתוח כלכלי לטווח קצר הוביל לפעולות מוגבלות, ניהול משאבים לא בר-קיימא ומערכות אקולוגיות מושפלות. אנו מסבירים את החשיבות של ממשל מערכת אקולוגית בהשגת יעדי SDG, ומציעים כמה דרכים להבטיח ששירותי מערכת אקולוגית נשקלו בצורה משמעותית. לכן הקריאה במאמר מסייעת להבין כיצד מוסדות ציבור, חברה אזרחית ושירות ציבורי מתורגם למוסדות, מדיניות והתנהגות ציבורית.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את ממשל אקולוגי ויעדי הפיתוח בר־הקיימא של האו״ם דרך מקור אקדמי שפורסם בכתב העת AMBIO ועוסק במוסדות ציבור, חברה אזרחית ושירות ציבורי. נקודת המוצא שלו היא שהסוגיה אינה רק אירוע נקודתי, אלא דרך להבין כיצד כללים, מוסדות ושחקנים חברתיים מעצבים תוצאה דמוקרטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "פיתוח בר קיימא שואף לתת מענה לדאגות כלכליות, חברתיות וסביבתיות, אך היעדר הנוכחי של ממשל סביבתי מגיב מעכב את ההתקדמות. פיתוח כלכלי לטווח קצר הוביל לפעולות מוגבלות, ניהול משאבים לא בר-קיימא ומערכות אקולוגיות מושפלות. אנו מסבירים את החשיבות של ממשל מערכת אקולוגית בהשגת יעדי SDG, ומציעים כמה דרכים להבטיח ששירותי מערכת אקולוגית נשקלו בצורה משמעותית. המשמעות היא שהמאמר אינו מסתפק בכותרת כללית, אלא מצביע על קשר ממשי בין תנאים מוסדיים, חברתיים או פוליטיים לבין התוצאה שהוא מנתח."
      ]
    },
    {
      "headingHe": "איזו ראיה עומדת מאחורי הטענה?",
      "paragraphsHtml": [
        "המקור נשען על מסגרת מחקרית המתוארת בתקציר ובמטא־דאטה של המאמר, ובוחן את השאלה באמצעות המקרה, הנתונים או הניתוח שהמחברים מציגים. לכן יש לקרוא את הטענה כטענה מחקרית מוגדרת, לא כסיסמה כללית על דמוקרטיה."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "המאמר חשוב משום שהוא מראה כיצד מוסדות ציבור, חברה אזרחית ושירות ציבורי קשור להגבלת כוח, לאחריותיות וליכולת של אזרחים וקבוצות להשתתף בתנאים הוגנים. בדמוקרטיה ליברלית, גם תהליכים שנראים מקצועיים משפיעים על זכויות, אמון ציבורי ואיזון בין מוסדות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית שהמאמר מציב?",
      "paragraphsHtml": [
        "השאלה המרכזית היא כיצד ממשל אקולוגי ויעדי הפיתוח בר־הקיימא של האו״ם משנה את ההבנה שלנו לגבי מוסדות ציבור, חברה אזרחית ושירות ציבורי. התשובה של המאמר ממקמת את הדיון בקשר בין פעולה מוסדית לבין תוצאה חברתית או פוליטית מדידה."
      ]
    },
    {
      "headingHe": "מה הקורא יכול לקחת מן הממצא?",
      "paragraphsHtml": [
        "הלקח הוא לבחון לא רק את התוצאה הסופית, אלא גם את הכללים והיחסים שמייצרים אותה. כאשר בודקים את התהליך עצמו אפשר לראות מי מקבל כוח, מי נושא בעלות, ואילו מנגנוני ביקורת נשארים פעילים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק בישראל בלבד?",
      "paragraphsHtml": [
        "המאמר קשור לישראל דרך החוקרים, המקרה או ההקשר המחקרי, אך הוא מציע גם תובנה רחבה יותר על דמוקרטיה ומוסדות. גם כאשר ההשוואה או הנתונים אינם ישראליים בלבד, השאלה הדמוקרטית רלוונטית להבנת הוויכוח הציבורי בישראל."
      ]
    },
    {
      "headingHe": "איזה מושג דמוקרטי בולט כאן?",
      "paragraphsHtml": [
        "המושג הבולט הוא <strong>מוסדות ציבור, חברה אזרחית ושירות ציבורי</strong>, משום שהוא מחבר בין הממצא המחקרי לבין איכות הכללים הציבוריים. המאמר מראה שהמושג הזה אינו מופשט בלבד, אלא מופיע בהחלטות, נהלים, עמדות והתנהגויות."
      ]
    },
    {
      "headingHe": "מה לא כדאי להסיק מהר מדי?",
      "paragraphsHtml": [
        "לא נכון להסיק מן התמצית שכל מקרה דומה יניב אותה תוצאה בדיוק. נכון יותר להשתמש במאמר כדי לזהות מנגנון, שאלה או סיכון, ואז לבדוק כיצד הם מופיעים בהקשרים אחרים."
      ]
    },
    {
      "headingHe": "איך המאמר מתחבר לזכויות?",
      "paragraphsHtml": [
        "החיבור לזכויות מופיע דרך השאלה מי מקבל הגנה, ייצוג או אפשרות להשתתף בתהליך. כאשר הכללים מוטים או לא שקופים, גם מדיניות שנראית ניטרלית יכולה להשפיע אחרת על קבוצות שונות."
      ]
    },
    {
      "headingHe": "איך המאמר מתחבר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נוגע לא רק לקיום חוקים, אלא גם לאופן שבו סמכות מופעלת ומבוקרת. המאמר עוזר לראות האם ההליך מחזק אחריות ציבורית או מאפשר כוח שקשה לבקר אותו."
      ]
    },
    {
      "headingHe": "מה הקשר לאמון ציבורי?",
      "paragraphsHtml": [
        "אמון ציבורי תלוי בתחושה שהכללים יציבים, הוגנים וניתנים לביקורת. כאשר המאמר מצביע על פערים, משברים או מנגנונים מוסדיים, הוא גם מסביר מדוע אמון אינו רק רגש אלא תוצאה של מבנה ציבורי."
      ]
    },
    {
      "headingHe": "למי המאמר שימושי?",
      "paragraphsHtml": [
        "המאמר שימושי לחוקרות וחוקרים של משפט, מדע המדינה, מדיניות ציבורית וחברה אזרחית. הוא שימושי גם לקוראים שמבקשים להבין איך מחקר אקדמי מתרגם מחלוקת ציבורית לשאלה שאפשר לבחון באופן שיטתי."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "השורה התחתונה היא שממשל אקולוגי ויעדי הפיתוח בר־הקיימא של האו״ם מספק דרך קונקרטית להבין את איכות הדמוקרטיה ולא רק את הכותרת הפוליטית של האירוע. המאמר מזמין לבחון עובדות, מוסדות ותמריצים לפני שמסתפקים בהצהרות כלליות."
      ]
    }
  ],
  "layout": "paper",
  "title": "ממשל אקולוגי ויעדי הפיתוח בר־הקיימא של האו״ם",
  "permalink": "/democracy_ecosystem_governance_sustainable_development_goals_summary_he.html",
  "paper_url": "democracy_ecosystem_governance_sustainable_development_goals_summary_he.html",
  "sortKey": 202606090003,
  "sourcePublishedDate": "2017-04-22"
}
---
