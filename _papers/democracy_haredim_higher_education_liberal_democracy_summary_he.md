---
{
  "version": 1,
  "file": "democracy_haredim_higher_education_liberal_democracy_summary_he.html",
  "slug": "democracy_haredim_higher_education_liberal_democracy_summary_he",
  "titleHe": "השכלה גבוהה לחרדים בוחנת את גבולות הסובלנות הליברלית",
  "subtitleHe": "",
  "descriptionHe": "המאמר דן בתכניות השכלה גבוהה בהפרדה מגדרית לחרדים בישראל, ובוחן אותן דרך שאלת היחס של דמוקרטיה ליברלית למיעוטים לא־ליברליים. הוא מציב במרכז את המתח בין שילוב, סובלנות, שוויון מגדרי וזכות יציאה מתרבות קהילתית.",
  "summaryHe": "הרטמן וזיכרמן מנתחים את הוויכוח בישראל על תכניות אקדמיות מופרדות מגדרית לחרדים, לרבות הדיון הציבורי והעתירות לבית המשפט העליון. הם מציבים את המקרה בתוך תאוריה של רב־תרבותיות וסובלנות, ושואלים האם דמוקרטיה ליברלית יכולה או צריכה לסבול פרקטיקה לא־ליברלית כאשר היא עשויה לקדם שילוב חברתי.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "טובה הרטמן, חיים זיכרמן",
  "paperTitle": "HIGHER EDUCATION FOR HAREDIM IN ISRAEL",
  "authorsHtml": "<a href=\"https://www.ono.ac.il/lecturer/prof-tova-hartman/\" target=\"_blank\" rel=\"noopener noreferrer\">טובה הרטמן</a>, <a href=\"https://www.ono.ac.il/eng/lecturer-new/?id=6518\" target=\"_blank\" rel=\"noopener noreferrer\">חיים זיכרמן</a>",
  "journal": "Journal of Law and Religion, 34(3), 273-283",
  "dateText": "פורסם: 2019",
  "sourcePublishedDate": "2020-01-20",
  "doiUrl": "https://doi.org/10.1017/jlr.2019.37",
  "doiLabel": "https://doi.org/10.1017/jlr.2019.37",
  "authors": [
    {
      "@type": "Person",
      "name": "Tova Hartman",
      "url": "https://www.ono.ac.il/lecturer/prof-tova-hartman/"
    },
    {
      "@type": "Person",
      "name": "Chaim Zicherman",
      "url": "https://www.ono.ac.il/eng/lecturer-new/?id=6518"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Tova Hartman",
      "url": "https://www.ono.ac.il/lecturer/prof-tova-hartman/"
    },
    {
      "@type": "Person",
      "name": "Chaim Zicherman",
      "url": "https://www.ono.ac.il/eng/lecturer-new/?id=6518"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "democratic-backsliding-populism",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חרדים",
    "השכלה גבוהה",
    "הפרדה מגדרית",
    "רב תרבותיות",
    "דמוקרטיה ליברלית",
    "זכות יציאה",
    "שוויון מגדרי"
  ],
  "image": {
    "src": "assets/article-images/real_photo_279_haredi_democracy_haredim_higher_education_liberal_democracy_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור להשכלה גבוהה לחרדים בוחנת את גבולות הסובלנות הליברלית",
    "fitness": "high",
    "license": "CC BY-SA 2.0",
    "creator": "U.S. Embassy Tel Aviv",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Visit_to_Meah_She%E2%80%99arim_185.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/f/f0/Visit_to_Meah_She%E2%80%99arim_185.jpg",
    "fileTitle": "Visit to Meah She’arim 185.jpg",
    "sourceDimensions": "1200x800",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:haredi",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/2.0/"
  },
  "datePublished": "2026-06-01",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המאמר משתמש בהשכלה גבוהה לחרדים כדי לבחון מתי סובלנות ליברלית כלפי פרקטיקה לא־ליברלית מקדמת שילוב ומתי היא פוגעת בשוויון. הזכות לצאת מן הקהילה הופכת לקריטריון מרכזי בדיון.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>סובלנות ליברלית</strong> היא הנכונות לאפשר אורחות חיים שאינם תואמים בהכרח ערכים ליברליים מלאים. השאלה במאמר היא מתי סובלנות כזו מגנה על מיעוט ומתי היא מאפשרת פגיעה בשוויון, במיוחד שוויון מגדרי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "החיבור בוחן את גבולות הסובלנות של תרבויות לא-ליברליות בתוך חברות דמוקרטיות ליברליות ומוצא את הזכות המכריעה לצאת מתרבות כזו - זכות שכדאיותה תלויה בחינוך הולם. החיבור מסתיים בדיון במודל פיתוח של רב־תרבותיות ובריבוי הפנים והמורכבות של רב־תרבותיות.",
        "הרטמן וזיכרמן מנתחים את הוויכוח בישראל על תכניות אקדמיות מופרדות מגדרית לחרדים, לרבות הדיון הציבורי והעתירות לבית המשפט העליון. הם מציבים את המקרה בתוך תאוריה של רב־תרבותיות וסובלנות, ושואלים האם דמוקרטיה ליברלית יכולה או צריכה לסבול פרקטיקה לא־ליברלית כאשר היא עשויה לקדם שילוב חברתי."
      ]
    },
    {
      "headingHe": "מהו המקרה הישראלי שנבחן?",
      "paragraphsHtml": [
        "המאמר עוסק בתכניות אקדמיות לחרדים שנפתחו בישראל בעשורים האחרונים וחלקן פועלות בהפרדה מגדרית. הצמיחה של התכניות יצרה ויכוח ציבורי, תקשורתי ומשפטי שהגיע גם לבית המשפט העליון."
      ]
    },
    {
      "headingHe": "למה השכלה גבוהה לחרדים היא סוגיה דמוקרטית?",
      "paragraphsHtml": [
        "היא נוגעת לשילוב של מיעוט דתי בשוק העבודה, באקדמיה ובחברה הכללית. במקביל, היא מציבה שאלה האם המדינה רשאית לתמוך במסגרות שמגבילות אינטראקציה מגדרית בשם התאמה תרבותית."
      ]
    },
    {
      "headingHe": "מהי הדילמה של דמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית צריכה לכבד קבוצות מיעוט ולאפשר להן לשמר אורח חיים ייחודי. אך היא גם מחויבת לשוויון, לאוטונומיה אישית ולמניעת כפייה פנימית בתוך הקבוצה."
      ]
    },
    {
      "headingHe": "איך הפרדה מגדרית משתלבת בדיון?",
      "paragraphsHtml": [
        "הפרדה מגדרית יכולה להיתפס כתנאי שמאפשר לחלק מהחרדים לרכוש השכלה גבוהה. מצד אחר, היא עלולה לחזק נורמות שמדירות נשים או מצמצמות את השוויון במרחב ציבורי ואקדמי."
      ]
    },
    {
      "headingHe": "מהי זכות היציאה ולמה היא מרכזית?",
      "paragraphsHtml": [
        "זכות היציאה היא האפשרות הממשית של אדם לעזוב תרבות או פרקטיקה קהילתית שאינה מתאימה לו. המאמר מדגיש שהערכת פרקטיקות לא־ליברליות תלויה בשאלה אם לפרטים יש חלופה אמיתית ולא רק פורמלית."
      ]
    },
    {
      "headingHe": "האם סובלנות פירושה הסכמה לכל פרקטיקה?",
      "paragraphsHtml": [
        "לא. תאוריות רב־תרבותיות מכירות בצורך לסבול שונות תרבותית, אך גם בכך שלא כל פרקטיקה לא־ליברלית ראויה להגנה. השאלה היא כיצד להבחין בין התאמה תרבותית לגיטימית לבין פגיעה בזכויות."
      ]
    },
    {
      "headingHe": "מה תפקיד בית המשפט העליון במחלוקת?",
      "paragraphsHtml": [
        "העתירות לבית המשפט ממחישות שהוויכוח אינו רק חינוכי אלא חוקתי וציבורי. בית המשפט נדרש לאזן בין חופש דת ותרבות, שוויון מגדרי, סמכות המדינה ומדיניות שילוב."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לרב־תרבותיות?",
      "paragraphsHtml": [
        "המאמר מציב את ישראל בתוך דיון תאורטי רחב על מיעוטים לא־ליברליים במדינות ליברליות. הוא בוחן האם התאמות מוסדיות יכולות לחזק השתתפות בלי לוותר על עקרונות שוויון ואוטונומיה."
      ]
    },
    {
      "headingHe": "מהי התועלת האפשרית של התכניות?",
      "paragraphsHtml": [
        "התכניות עשויות להרחיב נגישות להשכלה ולתעסוקה עבור אוכלוסייה שהשתתפותה באקדמיה הייתה מוגבלת. תועלת זו חשובה דמוקרטית משום שהיא יכולה לצמצם הדרה חברתית וכלכלית."
      ]
    },
    {
      "headingHe": "מהו הסיכון האפשרי במדיניות כזו?",
      "paragraphsHtml": [
        "הסיכון הוא שהמדינה תעניק לגיטימציה לפרקטיקה שמחלישה נשים או מקבעת גבולות קהילתיים נוקשים. אם אין חלופות שוות ופתוחות, ההתאמה התרבותית עלולה להפוך להגבלה מוסדית."
      ]
    },
    {
      "headingHe": "מהי השאלה שהמאמר משאיר לקובעי מדיניות?",
      "paragraphsHtml": [
        "השאלה היא כיצד לעצב השכלה גבוהה שמכבדת זהות חרדית אך אינה מוותרת על שוויון ליברלי. מדיניות טובה צריכה למדוד לא רק מספרי שילוב, אלא גם חופש בחירה, איכות לימודים וזכויות של נשים וגברים בתוך המסגרת."
      ]
    }
  ],
  "layout": "paper",
  "title": "השכלה גבוהה לחרדים בוחנת את גבולות הסובלנות הליברלית",
  "permalink": "/democracy_haredim_higher_education_liberal_democracy_summary_he.html",
  "paper_url": "democracy_haredim_higher_education_liberal_democracy_summary_he.html",
  "sortKey": 202606010001
}
---
