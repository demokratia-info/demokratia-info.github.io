---
{
  "version": 1,
  "file": "democracy_conflict_regulation_deeply_divided_societies_summary_he.html",
  "slug": "democracy_conflict_regulation_deeply_divided_societies_summary_he",
  "order": 496,
  "titleHe": "חברות שסועות מנהלות קונפליקט באמצעות דפוסים שונים של הסדרה פוליטית",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בדרכים שונות שבהן חברות שסועות מווסתות קונפליקט פוליטי, ומדגיש כי אין מודל יחיד להסדרת יחסים בין קבוצות בחברה מחולקת.",
  "summaryHe": "המאמר של Sammy Smooha ו-Theodor Hanf מתמקד באופני הסדרה של קונפליקט בחברות שסועות לעומק. הוא רלוונטי במיוחד לדיון דמוקרטי משום שהוא עוסק בשאלה כיצד מוסדות, הסכמות ויחסי כוח מאפשרים חיים פוליטיים משותפים למרות חלוקות חברתיות עמוקות.",
  "authorsCardHe": "Sammy Smooha, Theodor Hanf",
  "paperTitle": "The Diverse Modes of Conflict-Regulation in Deeply Divided Societies",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/sammy-smooha/\" target=\"_blank\" rel=\"noopener noreferrer\">Sammy Smooha</a>, Theodor Hanf",
  "journal": "International Journal of Comparative Sociology",
  "dateText": "פורסם: 1992",
  "doiUrl": "https://doi.org/10.1163/002071592x00031",
  "doiLabel": "https://doi.org/10.1163/002071592x00031",
  "authors": [
    {
      "@type": "Person",
      "name": "Sammy Smooha",
      "url": "https://cris.haifa.ac.il/en/persons/sammy-smooha/"
    },
    {
      "@type": "Person",
      "name": "Theodor Hanf"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Sammy Smooha",
      "url": "https://cris.haifa.ac.il/en/persons/sammy-smooha/"
    },
    {
      "@type": "Person",
      "name": "Theodor Hanf"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "חברות שסועות",
    "הסדרת קונפליקט",
    "דמוקרטיה השוואתית",
    "יחסי קבוצות",
    "הסדרים מוסדיים",
    "פלורליזם",
    "קיטוב"
  ],
  "image": {
    "src": "html_qa/democracy_conflict_regulation_deeply_divided_societies_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "חדר תיווך אזרחי עם נציגי קהילות שונות סביב שולחן עגול",
    "fitness": "high"
  },
  "datePublished": "2026-06-08",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "בחברה שסועה, דמוקרטיה אינה נמדדת רק בבחירות אלא גם במנגנונים שמאפשרים לקבוצות יריבות לחיות בתוך מסגרת פוליטית אחת.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן דרכים שונות שבהן חברות שסועות מנסות להסדיר קונפליקט בין קבוצות מרכזיות. נקודת המוצא היא שחברה מחולקת אינה חייבת להתפרק, אך היא זקוקה למנגנונים פוליטיים שמתאימים לעומק החלוקה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: סקירה של כמה חברות עכשוויות והיסטוריות מפוצלות עמוקות מראה מגוון גדול של אופנים של ויסות סכסוכים המורכבים מוריאציות של חלוקה, אלימות, שליטה והתאמות. מגוון רחב זה של חלופות מצביע על כך שניהול חד-צדדי או אלים של סכסוכים קהילתיים אינו בלתי נמנע.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר של Sammy Smooha ו-Theodor Hanf מתמקד באופני הסדרה של קונפליקט בחברות שסועות לעומק. הוא רלוונטי במיוחד לדיון דמוקרטי משום שהוא עוסק בשאלה כיצד מוסדות, הסכמות ויחסי כוח מאפשרים חיים פוליטיים משותפים למרות חלוקות חברתיות עמוקות."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>חברה שסועה</strong> היא חברה שבה זהויות קבוצתיות מרכזיות חופפות למחלוקות פוליטיות עמוקות. בהקשר כזה, הסדרת קונפליקט פירושה יצירת כללים, מוסדות והבנות שמאפשרים תחרות פוליטית בלי להפוך אותה למאבק בלתי נשלט."
      ]
    },
    {
      "headingHe": "למה אין מודל אחד שמתאים לכל חברה?",
      "paragraphsHtml": [
        "חלוקות חברתיות נבדלות זו מזו בעומק, בהיסטוריה, ביחסי הכוח ובמידת האמון בין קבוצות. לכן מנגנון שעובד בחברה אחת עלול להיכשל בחברה אחרת אם הוא מתעלם ממבנה הקונפליקט המקומי."
      ]
    },
    {
      "headingHe": "מהי חשיבות ההשוואה הסוציולוגית?",
      "paragraphsHtml": [
        "הגישה ההשוואתית מאפשרת לראות שהסדרת קונפליקט אינה רק שאלה חוקתית מופשטת. היא קשורה גם לדפוסי זהות, לארגון קבוצות, למנהיגות וליכולת של מוסדות לייצר תחושת ביטחון פוליטי."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית דורשת שמחלוקת תתנהל בתוך כללים המגינים על זכויות ומונעים שליטה מוחלטת של קבוצה אחת. בחברה שסועה, הדרישה הזאת קשה במיוחד משום שהפסד פוליטי עלול להיתפס כאיום על קיום קבוצתי."
      ]
    },
    {
      "headingHe": "מה ההבדל בין ניהול קונפליקט לפתרון קונפליקט?",
      "paragraphsHtml": [
        "פתרון קונפליקט מבקש לעיתים לסיים את המחלוקת עצמה, אך ניהול או הסדרה מכירים בכך שחלק מן המחלוקות ימשיכו להתקיים. המאמר חשוב מפני שהוא מתמקד ביכולת לקיים חיים פוליטיים גם כאשר ההסכמה העמוקה אינה זמינה."
      ]
    },
    {
      "headingHe": "אילו מוסדות יכולים לסייע בהסדרה?",
      "paragraphsHtml": [
        "מוסדות ייצוג, חלוקת סמכויות, מנגנוני וטו, אוטונומיה תרבותית, פדרליזם או הסכמי כוח יכולים כולם להיות חלק מארגז הכלים. השאלה אינה אם כלי אחד טוב תמיד, אלא אם הוא מתאים למבנה היחסים בין הקבוצות."
      ]
    },
    {
      "headingHe": "מה הסיכון בהסדרה לא מאוזנת?",
      "paragraphsHtml": [
        "הסדרה שמקפיאה יחסי כוח לא שוויוניים יכולה להיראות יציבה אך להנציח הדרה. לכן הסדרת קונפליקט דמוקרטית צריכה לשלב יציבות עם זכויות, ייצוג ואפשרות לשינוי פוליטי."
      ]
    },
    {
      "headingHe": "איך המאמר רלוונטי לישראל?",
      "paragraphsHtml": [
        "ישראל היא דוגמה מובהקת לחברה שבה זהויות לאומיות, דתיות, אתניות ואידאולוגיות משפיעות על הפוליטיקה. לכן מסגרת שמבחינה בין דפוסי הסדרה שונים יכולה לסייע בבחינת מוסדות ופשרות בלי להניח פתרון אחיד."
      ]
    },
    {
      "headingHe": "מה הקשר לקיטוב פוליטי?",
      "paragraphsHtml": [
        "קיטוב הופך מחלוקת לתפיסה של איום הדדי, ובחברה שסועה האיום הזה עלול להתלבש על זהות קבוצתית עמוקה. הסדרה פוליטית נועדה להפחית את הסיכון שמחלוקת תתורגם לאי־לגיטימציה של עצם השותפות."
      ]
    },
    {
      "headingHe": "מה לא כדאי להסיק מן המאמר?",
      "paragraphsHtml": [
        "לא נכון להסיק שהסדרים קבוצתיים הם תמיד ליברליים או תמיד אנטי־ליברליים. הערכתם תלויה בשאלה האם הם מגינים על בני אדם וקבוצות מפני דיכוי, או להפך מקבעים שליטה ופגיעה בזכויות."
      ]
    },
    {
      "headingHe": "איזו שאלה מוסדית המאמר מציב?",
      "paragraphsHtml": [
        "השאלה המוסדית היא כיצד לבנות כללים שמאפשרים לקבוצות להרגיש בטוחות מספיק כדי להשתתף במשחק הפוליטי. בלי תחושת ביטחון כזאת, מנגנוני בחירות רגילים עלולים להחמיר את הקונפליקט במקום לווסת אותו."
      ]
    },
    {
      "headingHe": "מהי התרומה למחקר דמוקרטי?",
      "paragraphsHtml": [
        "המאמר מזכיר שמוסדות דמוקרטיים נבחנים בתנאי עומס, לא רק במצבי הסכמה. בחברות שסועות, איכות הדמוקרטיה תלויה ביכולת להפוך יריבות עמוקה לתחרות פוליטית מוגבלת ומוסדרת."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה הדמוקרטית?",
      "paragraphsHtml": [
        "חברה שסועה זקוקה לדמוקרטיה שמכירה במחלוקת ולא מדחיקה אותה. הסדרה פוליטית טובה אינה מבטלת זהויות, אלא יוצרת מסגרת שבה זהויות יכולות להתקיים בלי לפרק את המרחב הציבורי המשותף."
      ]
    }
  ],
  "layout": "paper",
  "title": "חברות שסועות מנהלות קונפליקט באמצעות דפוסים שונים של הסדרה פוליטית",
  "permalink": "/democracy_conflict_regulation_deeply_divided_societies_summary_he.html",
  "paper_url": "democracy_conflict_regulation_deeply_divided_societies_summary_he.html",
  "sortKey": 202606080007
}
---
