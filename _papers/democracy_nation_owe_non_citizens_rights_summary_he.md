---
{
  "version": 1,
  "file": "democracy_nation_owe_non_citizens_rights_summary_he.html",
  "slug": "democracy_nation_owe_non_citizens_rights_summary_he",
  "titleHe": "זכויות לא־אזרחים תלויות באופן שבו הציבור מפרש איום ושייכות לאומית",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן נכונות של אזרחים להעניק זכויות אזרחות למהגרים בצרפת, גרמניה, ארצות הברית וישראל. הוא מראה שהקשר בין פטריוטיות, שוביניזם, תפיסת איום והקצאת זכויות מורכב יותר מן ההבחנה הרגילה בין מדינות אתנו־לאומיות וליברליות.",
  "summaryHe": "Raijman, Davidov, Schmidt ו־Hochman בוחנים כיצד התקשרות לאומית ותפיסת איום משפיעות על נכונות להעניק זכויות למהגרים. ממצאיהם מצביעים על דפוסים פרדוקסליים: ההשפעות של שוביניזם ופטריוטיות חלשות יותר בגרמניה ובישראל, ואילו השפעת האיום חלשה יותר בישראל וחזקה יותר במדינות ליברליות.",
  "authorsCardHe": "Rebeca Raijman, Eldad Davidov, Peter Schmidt, Oshrat Hochman",
  "paperTitle": "What Does a Nation Owe Non-Citizens?",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/rebeca-raijman/\" target=\"_blank\" rel=\"noopener noreferrer\">Rebeca Raijman</a>; <a href=\"https://dss.uni-koeln.de/de/institut/personen/d/prof-dr-eldad-davidov\" target=\"_blank\" rel=\"noopener noreferrer\">Eldad Davidov</a>; <a href=\"https://www.uni-giessen.de/de/fbz/zentren/zeu/Team/zeumembersall/Peter_Schmidt/dpschmidt\" target=\"_blank\" rel=\"noopener noreferrer\">Peter Schmidt</a>; <a href=\"https://www.gesis.org/institut/ueber-uns/mitarbeitendenverzeichnis/person/Oshrat.Hochman\" target=\"_blank\" rel=\"noopener noreferrer\">Oshrat Hochman</a>",
  "journal": "International Journal of Comparative Sociology, 49(2-3), 195-220",
  "dateText": "פורסם: אפריל 2008",
  "sourcePublishedDate": "2008-04-01",
  "doiUrl": "https://doi.org/10.1177/0020715208088912",
  "doiLabel": "https://doi.org/10.1177/0020715208088912",
  "authors": [
    {
      "@type": "Person",
      "name": "Rebeca Raijman",
      "url": "https://cris.haifa.ac.il/en/persons/rebeca-raijman/"
    },
    {
      "@type": "Person",
      "name": "Eldad Davidov",
      "url": "https://dss.uni-koeln.de/de/institut/personen/d/prof-dr-eldad-davidov"
    },
    {
      "@type": "Person",
      "name": "Peter Schmidt",
      "url": "https://www.uni-giessen.de/de/fbz/zentren/zeu/Team/zeumembersall/Peter_Schmidt/dpschmidt"
    },
    {
      "@type": "Person",
      "name": "Oshrat Hochman",
      "url": "https://www.gesis.org/institut/ueber-uns/mitarbeitendenverzeichnis/person/Oshrat.Hochman"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Rebeca Raijman",
      "url": "https://cris.haifa.ac.il/en/persons/rebeca-raijman/"
    },
    {
      "@type": "Person",
      "name": "Eldad Davidov",
      "url": "https://dss.uni-koeln.de/de/institut/personen/d/prof-dr-eldad-davidov"
    },
    {
      "@type": "Person",
      "name": "Peter Schmidt",
      "url": "https://www.uni-giessen.de/de/fbz/zentren/zeu/Team/zeumembersall/Peter_Schmidt/dpschmidt"
    },
    {
      "@type": "Person",
      "name": "Oshrat Hochman",
      "url": "https://www.gesis.org/institut/ueber-uns/mitarbeitendenverzeichnis/person/Oshrat.Hochman"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "rights-equality-minorities",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "לא־אזרחים",
    "זכויות מהגרים",
    "איום נתפס",
    "פטריוטיות",
    "שוביניזם",
    "אזרחות",
    "השוואה בין מדינות"
  ],
  "image": {
    "src": "html_qa/democracy_nation_owe_non_citizens_rights_summary_he.jpg",
    "version": "2026-06-02-nightly",
    "altHe": "חוקרים בוחנים סקרי דעת קהל על זכויות מהגרים ושייכות אזרחית",
    "fitness": "high"
  },
  "datePublished": "2026-06-02",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שהנכונות לתת זכויות למהגרים אינה נובעת רק מסוג המשטר או ממדיניות ההגירה הרשמית. היא מושפעת מאופן שבו אזרחים מפרשים שייכות, איום וזהות לאומית.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>איום נתפס</strong> הוא תחושה שקבוצה אחרת מסכנת משאבים, זהות או ביטחון, גם כאשר האיום אינו נמדד באופן אובייקטיבי. במאמר הוא משמש להסבר נכונות או התנגדות להענקת זכויות למהגרים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: במאמר זה אנו בודקים את ההשפעות של התקשרויות לאומיות (פטריוטיות ושוביניזם) ותפיסת האיום על נכונות האזרחים לוותר על זכויות אזרחות למהגרים בצרפת, גרמניה (מערב ומזרח), ארה\"ב וישראל. הממצאים שלנו מראים שלמרות הבדלים ניכרים במדיניות ההגירה של מדינות ובתפיסות הלאום, לא נמצאו הבדלים משמעותיים בעמדות כלפי הקצאת זכויות אזרחות למהגרים. לסיכום, אנו מציעים הסברים אפשריים לממצאים המסקרנים והפרדוקסאליים הללו.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: Raijman, Davidov, Schmidt ו־Hochman בוחנים כיצד התקשרות לאומית ותפיסת איום משפיעות על נכונות להעניק זכויות למהגרים. ממצאיהם מצביעים על דפוסים פרדוקסליים: ההשפעות של שוביניזם ופטריוטיות חלשות יותר בגרמניה ובישראל, ואילו השפעת האיום חלשה יותר בישראל וחזקה יותר במדינות ליברליות."
      ]
    },
    {
      "headingHe": "מה בוחן המאמר בארבע מדינות?",
      "paragraphsHtml": [
        "המאמר בוחן את עמדות הציבור בצרפת, גרמניה, ארצות הברית וישראל כלפי הקצאת זכויות אזרחות למהגרים. ההשוואה מאפשרת לבדוק אם דגמי לאומיות שונים מייצרים הבדלים ברורים ביחס ללא־אזרחים."
      ]
    },
    {
      "headingHe": "מה ההבדל בין פטריוטיות לשוביניזם?",
      "paragraphsHtml": [
        "פטריוטיות יכולה לבטא התקשרות חיובית למדינה, ואילו שוביניזם מצביע על עליונות לאומית או יחס מדיר יותר. ההבחנה חשובה משום ששני המושגים עשויים להשפיע אחרת על נכונות להכיר בזכויות של מי שאינו חלק מן הקבוצה הלאומית."
      ]
    },
    {
      "headingHe": "מה מפתיע בממצאים לגבי ישראל וגרמניה?",
      "paragraphsHtml": [
        "לפי תקציר המאמר, ההשפעות של שוביניזם ופטריוטיות על נכונות לתת זכויות למהגרים היו נמוכות יחסית בגרמניה ובישראל. זה ממצא מפתיע משום ששתי המדינות מתוארות בהקשר המחקרי כאתנו־לאומיות יותר."
      ]
    },
    {
      "headingHe": "מה מפתיע בממצאים לגבי איום?",
      "paragraphsHtml": [
        "השפעת תפיסת האיום הייתה חלשה יותר בישראל וחזקה יותר במדינות ליברליות יותר. הממצא הזה מערער הסבר פשוט שלפיו חברות ליברליות תמיד פתוחות יותר לזכויות מהגרים כאשר הציבור חש איום."
      ]
    },
    {
      "headingHe": "למה השאלה חשובה לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית נמדדת גם באופן שבו היא מתייחסת למי שחי בתחומה אך אינו אזרח מלא. זכויות לא־אזרחים מגלות אם ערכי חירות ושוויון נשמרים רק לחברי הקבוצה או גם למי שנמצא בשוליה."
      ]
    },
    {
      "headingHe": "איך עמדות ציבור משפיעות על מדיניות?",
      "paragraphsHtml": [
        "עמדות ציבור אינן קובעות לבדן את החוק, אך הן יוצרות מרחב פוליטי שבתוכו נבחרים מקבלים החלטות. כאשר הציבור תופס מהגרים כאיום, קל יותר להצדיק הגבלות גם כאשר הן פוגעות בשוויון."
      ]
    },
    {
      "headingHe": "מה הקשר בין לאומיות לבין אזרחות?",
      "paragraphsHtml": [
        "לאומיות מגדירה מי נתפס כחלק מן ה״אנחנו״, ואזרחות מגדירה זכויות וחובות פורמליות. המאמר מראה שהיחס בין השתיים אינו קבוע, משום שמדינות בעלות מודלים שונים יכולות להציג דפוסי עמדות בלתי צפויים."
      ]
    },
    {
      "headingHe": "מה יכול ללמוד מכך מחקר השוואתי?",
      "paragraphsHtml": [
        "מחקר השוואתי צריך להיזהר מהנחות רחבות מדי על סוגי מדינות. אותו מושג, כמו איום או פטריוטיות, עשוי לפעול אחרת לפי היסטוריה, מוסדות והגדרות שייכות מקומיות."
      ]
    },
    {
      "headingHe": "מה המשמעות למדיניות הגירה?",
      "paragraphsHtml": [
        "מדיניות הגירה צריכה להתמודד לא רק עם כללים משפטיים אלא גם עם פחדים ותפיסות ציבוריות. הסבר ציבורי, צמצום סטיגמה ומנגנוני זכויות ברורים יכולים להפחית את הפער בין נוכחות של מהגרים לבין הכרה בזכויותיהם."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית של המאמר?",
      "paragraphsHtml": [
        "התרומה היא הצגת תמונה מורכבת של הקצאת זכויות ללא־אזרחים. המאמר מסביר מדוע אי אפשר להסיק באופן אוטומטי ממודל הלאומיות של מדינה כיצד אזרחיה יתייחסו לזכויות מהגרים."
      ]
    }
  ],
  "layout": "paper",
  "title": "זכויות לא־אזרחים תלויות באופן שבו הציבור מפרש איום ושייכות לאומית",
  "permalink": "/democracy_nation_owe_non_citizens_rights_summary_he.html",
  "paper_url": "democracy_nation_owe_non_citizens_rights_summary_he.html",
  "sortKey": 202606020008
}
---
