---
{
  "version": 1,
  "file": "democracy_political_polarization_israel_1992_2022_summary_he.html",
  "slug": "democracy_political_polarization_israel_1992_2022_summary_he",
  "order": 47,
  "titleHe": "איך השתנה הקיטוב הפוליטי בישראל בין 1992 ל-2022?",
  "subtitleHe": "הפרק ממקם את הבחירות של 2022 בתוך שלושה עשורים של קיטוב פוליטי בישראל, ומחבר בין זהות מפלגתית, גושים ושינוי מוסדי.",
  "descriptionHe": "הפרק מספק מבט היסטורי על קיטוב בישראל ומזכיר שמשבר פוליטי עכשווי נשען על תהליכים ארוכי טווח. החשיבות הציבורית של המאמר היא בכך שהוא מראה כיצד עמדות הציבור, רגשות פוליטיים וקיטוב משפיעים על נכונות לקבל כללי משחק משותפים, לכבד יריבים פוליטיים ולהגן על מוסדות גם כאשר הם פועלים נגד העדפה מחנאית רגעית.",
  "summaryHe": "הפרק מספק מבט היסטורי על קיטוב בישראל ומזכיר שמשבר פוליטי עכשווי נשען על תהליכים ארוכי טווח.",
  "authorsCardHe": "יאיר אמיתי, נעם גדרון ועומר יאיר",
  "paperTitle": "Political Polarization in Israel, 1992–2022",
  "authorsHtml": "<a href=\"https://en.politics.huji.ac.il/people/yair-amitai\" target=\"_blank\" rel=\"noopener noreferrer\">Yair Amitai</a>; <a href=\"https://noamgidron.com/\" target=\"_blank\" rel=\"noopener noreferrer\">Noam Gidron</a>; <a href=\"https://omeryair.com/\" target=\"_blank\" rel=\"noopener noreferrer\">Omer Yair</a>",
  "journal": "The Elections in Israel 2022",
  "dateText": "פרק בספר בעריכת Routledge, פורסם ב-17 בינואר 2025, עמ׳ 21-41",
  "doiUrl": "https://doi.org/10.4324/9781032633299-3",
  "doiLabel": "https://doi.org/10.4324/9781032633299-3",
  "authors": [
    {
      "@type": "Person",
      "name": "יאיר אמיתי",
      "url": "https://en.politics.huji.ac.il/people/yair-amitai"
    },
    {
      "@type": "Person",
      "name": "נעם גדרון",
      "url": "https://noamgidron.com/"
    },
    {
      "@type": "Person",
      "name": "עומר יאיר",
      "url": "https://omeryair.com/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yair Amitai",
      "url": "https://en.politics.huji.ac.il/people/yair-amitai"
    },
    {
      "@type": "Person",
      "name": "Noam Gidron",
      "url": "https://noamgidron.com/"
    },
    {
      "@type": "Person",
      "name": "Omer Yair",
      "url": "https://omeryair.com/"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "institutions-civil-society-public-service",
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "קיטוב פוליטי",
    "בחירות בישראל",
    "1992-2022",
    "גושים פוליטיים",
    "דמוקרטיה פרלמנטרית"
  ],
  "image": {
    "src": "html_qa/democracy_constitutional_moment_west_bank_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על ציר זמן פוליטי וקיטוב בישראל לאורך שלושה עשורים",
    "fitness": "standard"
  },
  "datePublished": "2026-05-20",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "הפרק מספק מבט היסטורי על קיטוב בישראל ומזכיר שמשבר פוליטי עכשווי נשען על תהליכים ארוכי טווח. מבחינה דמוקרטית, ההיסטוריה של הקיטוב עוזרת להבין מדוע פשרות מוסדיות נעשות קשות יותר ככל שהיריב הפוליטי נתפס כאיום על זהות ושלטון.",
  "sections": [
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "הפרק מדגיש שקיטוב אינו אירוע נקודתי של מערכת בחירות אחת, אלא תהליך היסטורי שמצטבר ומשנה את היחס בין מחנות, מפלגות ומוסדות.",
        "הפרק הוא ניתוח פוליטי-היסטורי בספר על בחירות 2022, ומתייחס לתקופה הרחבה שבין 1992 ל-2022.",
        "במקום להתייחס לדמוקרטיה רק כמערכת בחירות, המאמר בוחן את התנאים שמאפשרים לחברה לשמר אמון, זכויות, אחריות ומחלוקת לגיטימית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: אבל האם יש עדויות ברורות לקיטוב מוגבר שכזה בקרב ישראלים רגילים? בפרק זה אנו מציגים עדויות מ-30 שנות סקרי דעת קהל בישראל ובוחנים מגמות בקיטוב אידיאולוגי וכן בקיטוב רגשי (כלומר, לא אוהב מעבר לקווי המפלגה). אנו מגלים שבסך הכל, הציבור הישראלי אינו מקוטב אידיאולוגית כיום יותר מאשר בשנות ה-90.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: הפרק מספק מבט היסטורי על קיטוב בישראל ומזכיר שמשבר פוליטי עכשווי נשען על תהליכים ארוכי טווח."
      ]
    },
    {
      "headingHe": "מהי נקודת המבט הליברלית-דמוקרטית?",
      "paragraphsHtml": [
        "מבחינה דמוקרטית, ההיסטוריה של הקיטוב עוזרת להבין מדוע פשרות מוסדיות נעשות קשות יותר ככל שהיריב הפוליטי נתפס כאיום על זהות ושלטון.",
        "הזווית הליברלית חשובה משום שהיא שואלת לא רק מי שולט, אלא גם מה מגביל את השלטון, איך נשמרת ביקורת, ואיך מוגנות קבוצות או יריבים פוליטיים מפני כוח עודף."
      ]
    },
    {
      "headingHe": "למה המאמר חשוב לקוראי האתר?",
      "paragraphsHtml": [
        "הפרק מתאים לאתר משום שהוא מחבר בין דעת קהל, מפלגות, בחירות ומוסדות דמוקרטיים בישראל.",
        "הוא מסייע לתרגם מחקר אקדמי לשאלות ציבוריות מעשיות: איך לזהות שחיקה דמוקרטית, איך למדוד קיטוב, ואיך לחשוב על מוסדות וזכויות בלי להסתפק בסיסמאות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד קיטוב פוליטי לאורך זמן משפיע על איכות הדמוקרטיה בישראל ועל היכולת לשמור על אמון, זכויות ומוסדות עצמאיים.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "מה החידוש העיקרי?",
      "paragraphsHtml": [
        "תרומתו היא בהצבת הבחירות האחרונות בתוך רצף ארוך יותר של שינויי גושים, זהויות מפלגתיות ותחרות על אופייה של הדמוקרטיה בישראל.",
        "החידוש צריך להיבחן מול הטענה והראיות במאמר המקורי, ולא רק לפי הדרך שבה התמצית מנגישה אותו."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "מבחינה דמוקרטית, ההיסטוריה של הקיטוב עוזרת להבין מדוע פשרות מוסדיות נעשות קשות יותר ככל שהיריב הפוליטי נתפס כאיום על זהות ושלטון.",
        "החשיבות הדמוקרטית נובעת מן הקשר בין מוסדות, זכויות ואחריות ציבורית, אך גבולות הטענה נשארים גבולות המקור."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לישראל?",
      "paragraphsHtml": [
        "הקשר הישראלי נמצא בלב המאמר: הפרק הוא ניתוח פוליטי-היסטורי בספר על בחירות 2022, ומתייחס לתקופה הרחבה שבין 1992 ל-2022.",
        "הקשר הישראלי צריך להיקרא דרך המוסדות, הזכויות וההקשרים שהמאמר עצמו מנתח, ולא כהכללה על החברה הישראלית כולה."
      ]
    },
    {
      "headingHe": "איזה מושג כדאי לזכור?",
      "paragraphsHtml": [
        "המושג המרכזי הוא <strong>קיטוב פוליטי לאורך זמן</strong>. הוא עוזר לראות כיצד תהליך שנראה פוליטי או משפטי צר משפיע על מערכת היחסים בין אזרחים, מוסדות ושלטון."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על מוסדות?",
      "paragraphsHtml": [
        "המוסדות הדמוקרטיים אינם פועלים בחלל ריק. הם תלויים בנורמות, באמון ציבורי, בהתנהגות מפלגות וביכולת להציב גבולות לכוח פוליטי."
      ]
    },
    {
      "headingHe": "האם המאמר מציע פתרון פשוט?",
      "paragraphsHtml": [
        "לא. התרומה שלו היא בעיקר אנליטית: הוא מחדד את הבעיה ומראה אילו תנאים פוליטיים, חברתיים או משפטיים צריכים להשתנות כדי לשפר את החוסן הדמוקרטי."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין לקרוא את המאמר כהוכחה שכל מחלוקת פוליטית היא סכנה לדמוקרטיה. החשיבות היא בזיהוי התנאים שבהם מחלוקת הופכת לפגיעה באמון, בזכויות או באיזונים מוסדיים."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול שאלות מדויקות יותר: אילו מוסדות נפגעים, אילו זכויות עומדות על הפרק, ואיך קיטוב פוליטי לאורך זמן משנה את התנהגות הציבור או השלטון.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך השתנה הקיטוב הפוליטי בישראל בין 1992 ל-2022?",
  "permalink": "/democracy_political_polarization_israel_1992_2022_summary_he.html",
  "paper_url": "democracy_political_polarization_israel_1992_2022_summary_he.html",
  "sortKey": 202605200011
}
---
