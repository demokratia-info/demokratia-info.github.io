---
{
  "version": 1,
  "file": "democracy_world_legislatures_covid_first_wave_open_close_summary_he.html",
  "slug": "democracy_world_legislatures_covid_first_wave_open_close_summary_he",
  "titleHe": "פרלמנטים דמוקרטיים היו חסינים יותר מסגירה בגל הקורונה הראשון",
  "subtitleHe": "",
  "descriptionHe": "תקציר בעברית של מחקר השוואתי על פעילות בתי מחוקקים ב־159 מדינות בגל הראשון של Covid-19, ועל הקשר בין סיכון בריאותי, משטר פוליטי, טכנולוגיה וסגירת פרלמנטים.",
  "summaryHe": "המאמר מנתח נתונים השוואתיים על פעילות בתי מחוקקים ב־159 מדינות בגל הראשון של Covid-19. הוא מצא שחומרת התחלואה לא הסבירה את הגבלת הפרלמנטים, ושבתי מחוקקים בדמוקרטיות שבריריות היו חשופים יותר לסגירה או לצמצום פעילות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Israel Waismel-Manor, Ittai Bar-Siman-Tov, Olivier Rozenberg, Asaf Levanon, Cyril Benoît, Gal Ifergane",
  "paperTitle": "Should I Stay (Open) or Should I Close? World Legislatures during the First Wave of Covid-19",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/israel-waismel-manor/\" target=\"_blank\" rel=\"noopener noreferrer\">Israel Waismel-Manor</a>, <a href=\"https://www.ittai.com/\" target=\"_blank\" rel=\"noopener noreferrer\">Ittai Bar-Siman-Tov</a>, <a href=\"https://orcid.org/0000-0003-2468-7639\" target=\"_blank\" rel=\"noopener noreferrer\">Olivier Rozenberg</a>, <a href=\"https://cris.haifa.ac.il/en/persons/asaf-levanon/\" target=\"_blank\" rel=\"noopener noreferrer\">Asaf Levanon</a>, <a href=\"https://orcid.org/0000-0001-9247-4702\" target=\"_blank\" rel=\"noopener noreferrer\">Cyril Benoît</a>, <a href=\"https://orcid.org/0000-0001-7450-3776\" target=\"_blank\" rel=\"noopener noreferrer\">Gal Ifergane</a>",
  "journal": "Political Studies, 72(1), 200-226",
  "dateText": "פורסם אונליין: 12.05.2022",
  "sourcePublishedDate": "2022-05-12",
  "doiUrl": "https://doi.org/10.1177/00323217221090615",
  "doiLabel": "https://doi.org/10.1177/00323217221090615",
  "authors": [
    {
      "@type": "Person",
      "name": "Israel Waismel-Manor",
      "url": "https://cris.haifa.ac.il/en/persons/israel-waismel-manor/"
    },
    {
      "@type": "Person",
      "name": "Ittai Bar-Siman-Tov",
      "url": "https://www.ittai.com/"
    },
    {
      "@type": "Person",
      "name": "Olivier Rozenberg",
      "url": "https://orcid.org/0000-0003-2468-7639"
    },
    {
      "@type": "Person",
      "name": "Asaf Levanon",
      "url": "https://cris.haifa.ac.il/en/persons/asaf-levanon/"
    },
    {
      "@type": "Person",
      "name": "Cyril Benoît",
      "url": "https://orcid.org/0000-0001-9247-4702"
    },
    {
      "@type": "Person",
      "name": "Gal Ifergane",
      "url": "https://orcid.org/0000-0001-7450-3776"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Israel Waismel-Manor",
      "url": "https://cris.haifa.ac.il/en/persons/israel-waismel-manor/"
    },
    {
      "@type": "Person",
      "name": "Ittai Bar-Siman-Tov",
      "url": "https://www.ittai.com/"
    },
    {
      "@type": "Person",
      "name": "Olivier Rozenberg",
      "url": "https://orcid.org/0000-0003-2468-7639"
    },
    {
      "@type": "Person",
      "name": "Asaf Levanon",
      "url": "https://cris.haifa.ac.il/en/persons/asaf-levanon/"
    },
    {
      "@type": "Person",
      "name": "Cyril Benoît",
      "url": "https://orcid.org/0000-0001-9247-4702"
    },
    {
      "@type": "Person",
      "name": "Gal Ifergane",
      "url": "https://orcid.org/0000-0001-7450-3776"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "פרלמנטים",
    "Covid-19",
    "פיקוח דמוקרטי",
    "מצבי חירום",
    "טכנולוגיה פרלמנטרית",
    "דמוקרטיות שבריריות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_858_civic_democracy_world_legislatures_covid_first_wave_open_close_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לפרלמנטים דמוקרטיים היו חסינים יותר מסגירה בגל הקורונה הראשון",
    "fitness": "high",
    "license": "Public domain",
    "creator": "President (1969-1974 : Nixon). White House Photo Office. 1969-1974, General Services Administration. National Archives and Records Service. Office of Presidential Libraries. Office of Presidential Papers. 1/20/1969-ca. 12/1974",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:President_Richard_Nixon_and_Israeli_Prime_Minister_Golda_Meir_Meeting_in_the_White_House_Oval_Office_-_DPLA_-_af9bb4c689e2b7c87f733acc0bbc6280.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/c/c2/President_Richard_Nixon_and_Israeli_Prime_Minister_Golda_Meir_Meeting_in_the_White_House_Oval_Office_-_DPLA_-_af9bb4c689e2b7c87f733acc0bbc6280.jpg",
    "fileTitle": "President Richard Nixon and Israeli Prime Minister Golda Meir Meeting in the White House Oval Office - DPLA - af9bb4c689e2b7c87f733acc0bbc6280.jpg",
    "sourceDimensions": "3084x2061",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:civic",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://commons.wikimedia.org/wiki/Commons:Copyright_tags#Public_domain"
  },
  "datePublished": "2026-06-07",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כי בגל הראשון של Covid-19, סגירת פרלמנטים לא נבעה בהכרח מחומרת המגפה; בחלק מהמקרים משבר בריאותי יצר סיכון ממשי ל<strong>החלשת פיקוח דמוקרטי</strong>.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>פיקוח פרלמנטרי</strong> הוא יכולת המחוקקים לבקר את הממשלה, לדרוש מידע ולהמשיך לייצג ציבור גם בזמן משבר. במאמר הוא חשוב משום שמצבי חירום יכולים להפוך בקלות לעילה לצמצום כוחם של בתי מחוקקים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המחקר בוחן פעילות של בתי מחוקקים ב־159 מדינות בגל הראשון של מגפת Covid-19 באמצעות ניתוח כמותי השוואתי. הוא משלב מקורות מידע מפרויקטים בין־לאומיים על פרלמנטים, אתרי מחוקקים, דיווחים עיתונאיים ונתוני מעקב על תגובות פרלמנטריות למגפה, כדי להשוות פתיחות מוסדית, התאמה טכנולוגית ומאפייני משטר.",
        "הניתוח מצא שאין קשר בין חומרת Covid-19 לבין הגבלות על פעילות בתי המחוקקים. מכאן המאמר טוען שבתי מחוקקים עלולים להיסגר או להצטמצם לא רק בגלל צורך בריאותי, אלא גם בשל תגובת יתר מוסדית או שימוש בסיכון הבריאותי כהצדקה להחלשת הזירה המחוקקת.",
        "המאמר מצא שבתי מחוקקים במדינות דמוקרטיות היו יחסית חסינים מפני סגירה או הגבלה, בעוד שבדמוקרטיות שבריריות ובקבוצת הביניים של התפתחות דמוקרטית הם היו חשופים יותר. במדינות חופשיות חלקית, שימוש בטכנולוגיה יכול היה למתן את הסיכון ולסייע בשימור פעילות פרלמנטרית, וגם מאפיינים קואליציוניים של הממשלה נמצאו כגורם שעשוי להשפיע על סגירת מחוקקים."
      ]
    },
    {
      "headingHe": "מה השאלה שהמאמר בודק?",
      "paragraphsHtml": [
        "המאמר שואל מדוע פרלמנטים במדינות שונות נסגרו, הוגבלו או נותרו פעילים בגל הראשון של הקורונה. הוא אינו מניח שחומרת המגפה לבדה הסבירה את ההבדלים בין מדינות."
      ]
    },
    {
      "headingHe": "מהו בסיס הנתונים של המחקר?",
      "paragraphsHtml": [
        "המחקר משתמש בניתוח כמותי רוחבי של פעילות מחוקקים ב־159 מדינות בזמן התגובה הראשונית למגפה. היקף כזה מאפשר להשוות בין משטרים, רמות חופש ומאפיינים מוסדיים שונים."
      ]
    },
    {
      "headingHe": "מה הממצא הבולט על חומרת המגפה?",
      "paragraphsHtml": [
        "הממצא הבולט הוא שלא נמצא קשר בין חומרת Covid-19 לבין הגבלות על פעילות פרלמנטרית. לכן המאמר מציע שהסיכון אינו רק בריאותי, אלא גם פוליטי: מדינות עלולות להגיב ביתר ולצמצם פיקוח."
      ]
    },
    {
      "headingHe": "למה דמוקרטיות היו חסינות יותר?",
      "paragraphsHtml": [
        "פרלמנטים במדינות דמוקרטיות היו יחסית חסינים מפני סגירה או הגבלה. ההסבר האפשרי הוא שמוסדות דמוקרטיים חזקים מקשים יותר על הממשלה להשתמש במשבר כדי להשתיק את הזירה המחוקקת."
      ]
    },
    {
      "headingHe": "מה קורה בדמוקרטיות שבריריות?",
      "paragraphsHtml": [
        "דמוקרטיות שבריריות חשופות יותר לסיכון שהמגפה תשמש הצדקה להחלשת מחוקקים. במצבים כאלה, גם צעדים שמוצגים כהכרחיים לבריאות הציבור צריכים להיבחן דרך שאלת הפיקוח והמידתיות."
      ]
    },
    {
      "headingHe": "מה תפקיד הטכנולוגיה לפי המאמר?",
      "paragraphsHtml": [
        "המאמר מציין שבמדינות חופשיות חלקית שימוש בטכנולוגיה יכול למתן את הסיכון להגבלת פעילות מחוקקים. טכנולוגיה אינה תחליף לדמוקרטיה, אך היא יכולה לאפשר דיונים, הצבעות ופיקוח כאשר התכנסות פיזית מוגבלת."
      ]
    },
    {
      "headingHe": "איך מאפייני קואליציה משפיעים?",
      "paragraphsHtml": [
        "המאמר מוצא שגם מאפיינים קואליציוניים של הממשלה עשויים להוביל לסגירת מחוקקים. המשמעות היא שהחלטה על פעילות פרלמנטרית בזמן חירום קשורה למבנה הכוח הפוליטי ולא רק להנחיות בריאות."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למשברים עתידיים?",
      "paragraphsHtml": [
        "משבר עתידי צריך לכלול מראש מנגנונים להפעלת הכנסת או הפרלמנט בתנאים משתנים. אם הכללים נקבעים רק בתוך המשבר, הממשלה עלולה לקבל יתרון מוסדי שמקשה על ביקורת אפקטיבית."
      ]
    },
    {
      "headingHe": "איך המאמר רלוונטי לזכויות אזרח?",
      "paragraphsHtml": [
        "כאשר פרלמנט מוגבל, נפגע גם הערוץ שמפקח על פגיעה בזכויות בזמן חירום. לכן הדיון אינו רק על נוחות מוסדית, אלא על יכולת הציבור לקבל ייצוג וביקורת כאשר המדינה מפעילה סמכויות חריגות."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית לשיח הדמוקרטי?",
      "paragraphsHtml": [
        "התרומה היא הבחנה בין צורך בריאותי אמיתי לבין תגובת־יתר מוסדית. המאמר נותן בסיס אמפירי לטענה שגם בשעת מגפה, פתיחות המחוקק היא חלק מן החוסן הדמוקרטי ולא מכשול בפני ניהול משבר."
      ]
    }
  ],
  "layout": "paper",
  "title": "פרלמנטים דמוקרטיים היו חסינים יותר מסגירה בגל הקורונה הראשון",
  "permalink": "/democracy_world_legislatures_covid_first_wave_open_close_summary_he.html",
  "paper_url": "democracy_world_legislatures_covid_first_wave_open_close_summary_he.html",
  "sortKey": 202606070003
}
---
