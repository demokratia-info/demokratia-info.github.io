---
{
  "version": 1,
  "file": "democracy_regime_classification_backsliding_summary_he.html",
  "slug": "democracy_regime_classification_backsliding_summary_he",
  "order": 15,
  "titleHe": "האם נסיגה דמוקרטית היא רק ויכוח על בית המשפט?",
  "subtitleHe": "",
  "descriptionHe": "המאמר מסביר כיצד הוויכוח על הרפורמה המשפטית קשור לשאלה רחבה יותר: איזה סוג של דמוקרטיה ישראל היא, ומה גבולות המשטר שאותו מסווגים.",
  "summaryHe": "למה סיווג המשטר הישראלי משנה את האופן שבו מבינים את המשבר.",
  "authorsCardHe": "גל אריאלי",
  "paperTitle": "The Democratic Backsliding Debate and the Controversy over Regime Classification in Israel",
  "authorsHtml": "<a href=\"https://cris.bgu.ac.il/en/persons/gal-ariely/\" target=\"_blank\" rel=\"noopener noreferrer\">Gal Ariely</a>",
  "journal": "Israel Studies Review",
  "dateText": "2024",
  "doiUrl": "https://doi.org/10.3167/isr.2024.390104",
  "doiLabel": "https://doi.org/10.3167/isr.2024.390104",
  "authors": [
    {
      "@type": "Person",
      "name": "גל אריאלי",
      "url": "https://www.bgu.ac.il/en/people/galariel/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Gal Ariely",
      "url": "https://cris.bgu.ac.il/en/persons/gal-ariely/"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "institutions-civil-society-public-service",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דמוקרטיה ליברלית",
    "נסיגה דמוקרטית",
    "מוסדות דמוקרטיים",
    "שוויון"
  ],
  "image": {
    "src": "html_qa/democracy_regime_classification_backsliding_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על סיווג משטר ונסיגה דמוקרטית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-07",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מסביר כיצד הוויכוח על הרפורמה המשפטית קשור לשאלה רחבה יותר: איזה סוג של דמוקרטיה ישראל היא, ומה גבולות המשטר שאותו מסווגים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "נסיגה דמוקרטית היא תהליך שבו מוסדות, זכויות, בחירות חופשיות או בלמים ואיזונים נחלשים בהדרגה, גם כאשר המסגרת הדמוקרטית הפורמלית עדיין קיימת. בהקשר של המאמר, המושג מאפשר לראות שינוי מצטבר בכללי המשחק ולא רק משבר נקודתי או מחלוקת פוליטית רגילה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: תקציר תוך שימוש במחלוקת 2023 על השיפוץ השיפוטי של ישראל כמקרה, מאמר זה מנתח את הוויכוח הרחב יותר, בן עשרות שנים, על אופיו של המשטר הישראלי. הוא מדגים כיצד הנחות סותרות לגבי דמוקרטיה והמשטר הישראלי עומדות בבסיס פרשנויות שונות לשינוי השיפוטי המוצע.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: למה סיווג המשטר הישראלי משנה את האופן שבו מבינים את המשבר."
      ]
    },
    {
      "headingHe": "מה נבדק במחקר",
      "paragraphsHtml": [
        "גל אריאלי משתמש במחלוקת סביב הרפורמה המשפטית של 2023 כמקרה מבחן לוויכוח ותיק יותר על אופי המשטר בישראל.",
        "המאמר מראה שהשאלה האם מדובר בנסיגה דמוקרטית תלויה גם בהנחות מוקדמות על דמוקרטיה: האם מדגישים שלטון רוב, או גם זכויות, שוויון, מוסדות מגבילים והגנה על מיעוטים.",
        "המחקר מחבר את הדיון העכשווי לבית המשפט לוויכוחים קודמים על ישראל כדמוקרטיה ליברלית, דמוקרטיה אתנית או משטר דמוקרטי מסוג מוגבל יותר."
      ]
    },
    {
      "headingHe": "הממצאים המרכזיים",
      "paragraphsHtml": [
        "המאמר מצביע על כך שתומכים ומתנגדים לרפורמה המשפטית לא רק חלוקים על פרטי החקיקה. הם משתמשים בתפיסות שונות של דמוקרטיה כדי לפרש את משמעותה.",
        "גישות רוביות נוטות לראות את המהלך כהחזרת כוח לנבחרי הציבור, בעוד גישות ליברליות מדגישות את הסכנה שבהחלשת בלמים, זכויות וביקורת שיפוטית.",
        "אריאלי מוסיף ששני הצדדים בדיון הישראלי נוטים לעיתים להתייחס לישראל כאילו היא מקבילה לדמוקרטיות ליברליות מערביות, בלי להתמודד מספיק עם שאלת גבולות המשטר והשלכות השליטה בשטחים על סיווגו."
      ]
    },
    {
      "headingHe": "למה זה חשוב לישראל",
      "paragraphsHtml": [
        "כאשר מדברים על דמוקרטיה רק כעל הכרעת רוב, קל להמעיט בחשיבותם של בתי משפט, זכויות מיעוטים, שוויון אזרחי ושלטון החוק.",
        "המאמר מציע שהדיון הישראלי צריך לשאול לא רק מי מנצח בהליך הפוליטי, אלא גם מי נכלל בקהילה הפוליטית, אילו זכויות מוגנות, ואילו גבולות יש לכוח השלטוני.",
        "במובן הזה, המאבק על הרפורמה המשפטית הוא חלק משאלה עמוקה יותר על תנאי הקיום של דמוקרטיה ליברלית בישראל."
      ]
    },
    {
      "headingHe": "במילים פשוטות",
      "paragraphsHtml": [
        "אפשר לקיים בחירות ועדיין להתווכח האם המשטר דמוקרטי במובן מלא.",
        "המאמר מזכיר שדמוקרטיה אינה רק מי מקבל רוב, אלא גם אילו זכויות נשמרות, מי נספר כשווה, ואילו מוסדות יכולים לעצור שימוש לרעה בכוח."
      ]
    }
  ],
  "layout": "paper",
  "title": "האם נסיגה דמוקרטית היא רק ויכוח על בית המשפט?",
  "permalink": "/democracy_regime_classification_backsliding_summary_he.html",
  "paper_url": "democracy_regime_classification_backsliding_summary_he.html",
  "sortKey": 202605070010
}
---
