---
{
  "version": 1,
  "file": "democracy_agents_of_the_nation_state_or_transformative_intellectuals_exploring_the_conflicting_roles_of_civics_summary_he.html",
  "slug": "democracy_agents_of_the_nation_state_or_transformative_intellectuals_exploring_the_conflicting_roles_of_civics_summary_he",
  "order": 435,
  "titleHe": "מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות",
  "subtitleHe": "",
  "descriptionHe": "תמצית בעברית של מאמר על מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות, עם דגש על המשמעות לדמוקרטיה ליברלית, לשלטון החוק, לזכויות ולאחריות ציבורית.",
  "summaryHe": "המאמר בוחן את סוגיית מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות דרך עדשה של זכויות, שוויון אזרחי והיכולת של קבוצות שונות להשתתף בדמוקרטיה. חשיבותו הציבורית היא בכך שהוא מחבר בין מקור אקדמי לבין שאלות מעשיות על גבולות כוח, השתתפות אזרחית, זכויות ואמון במוסדות. הסיכום מדגיש את המתח הדמוקרטי המרכזי: כיצד לשמור על כללים הוגנים והגנות ליברליות גם כאשר הסוגיה טעונה מבחינה פוליטית, ביטחונית, חברתית או זהותית.",
  "authorsCardHe": "Aline Muff, Zvi Bekerman",
  "paperTitle": "Agents of the nation-state or transformative intellectuals? Exploring the conflicting roles of civics teachers in Israel",
  "authorsHtml": "<a href=\"https://www.polsoz.fu-berlin.de/polwiss/forschung/sozialkunde/politikdidaktik/Team/Ehemalige_Mitarbeiter_innen/Muff/index.html\" target=\"_blank\" rel=\"noopener noreferrer\">Aline Muff</a>, <a href=\"https://orcid.org/0000-0002-3493-0770\" target=\"_blank\" rel=\"noopener noreferrer\">Zvi Bekerman</a>",
  "journal": "Education Citizenship and Social Justice",
  "dateText": "פורסם: 2017",
  "doiUrl": "https://doi.org/10.1177/1746197917743752",
  "doiLabel": "https://doi.org/10.1177/1746197917743752",
  "authors": [
    {
      "@type": "Person",
      "name": "Aline Muff",
      "url": "https://www.polsoz.fu-berlin.de/polwiss/forschung/sozialkunde/politikdidaktik/Team/Ehemalige_Mitarbeiter_innen/Muff/index.html"
    },
    {
      "@type": "Person",
      "name": "Zvi Bekerman",
      "url": "https://orcid.org/0000-0002-3493-0770"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Aline Muff",
      "url": "https://www.polsoz.fu-berlin.de/polwiss/forschung/sozialkunde/politikdidaktik/Team/Ehemalige_Mitarbeiter_innen/Muff/index.html"
    },
    {
      "@type": "Person",
      "name": "Zvi Bekerman",
      "url": "https://orcid.org/0000-0002-3493-0770"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה",
    "מוסדות",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "מדיניות ציבורית"
  ],
  "image": {
    "src": "html_qa/democracy_agents_of_the_nation_state_or_transformative_intellectuals_exploring_the_conflicting_roles_of_civics_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-05-23",
  "lastUpdatedHe": "23 במאי 2026",
  "oneLinerHtml": "שורה תחתונה: מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות מדגים כיצד סוגיה אקדמית ממוקדת הופכת לשאלה על <strong>דמוקרטיה ליברלית</strong>, אחריות ציבורית, שוויון והגבלת כוח.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות דרך שאלות של זכויות, שוויון אזרחי והיכולת של קבוצות שונות להשתתף בדמוקרטיה.",
        "הסיכום נזהר לא לייחס למחברים טענות שאינן מופיעות במקור, ולכן הוא מתמקד במשמעות הדמוקרטית שניתן ללמוד מן הנושא ומן המטא־דאטה האקדמי המאומת."
      ]
    },
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון במורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון במורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון במורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מורי אזרחות בישראל בין סוכני מדינה לאינטלקטואלים משני מציאות",
  "permalink": "/democracy_agents_of_the_nation_state_or_transformative_intellectuals_exploring_the_conflicting_roles_of_civics_summary_he.html",
  "paper_url": "democracy_agents_of_the_nation_state_or_transformative_intellectuals_exploring_the_conflicting_roles_of_civics_summary_he.html",
  "sortKey": 202605220048,
  "sourcePublishedDate": "2017-12-12"
}
---
