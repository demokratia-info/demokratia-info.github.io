---
{
  "version": 1,
  "file": "democracy_the_stranger_that_dwelleth_with_you_shall_be_unto_you_as_one_born_among_you_israeli_media_law_summary_he.html",
  "slug": "democracy_the_stranger_that_dwelleth_with_you_shall_be_unto_you_as_one_born_among_you_israeli_media_law_summary_he",
  "order": 70,
  "titleHe": "דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני־ישראלי",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Amit Schejter",
  "paperTitle": "The Stranger That Dwelleth with You Shall Be unto You as One Born among You: Israeli Media Law and the Cultural Rights of the Palestinian-Israeli Minority",
  "authorsHtml": "<a href=\"https://in.bgu.ac.il/en/humsos/comm-Studies/pages/staff/amit_schejter.aspx\" target=\"_blank\" rel=\"noopener noreferrer\">Amit Schejter</a>",
  "journal": "Middle East Journal of Culture and Communication",
  "dateText": "פורסם: 2008",
  "doiUrl": "https://doi.org/10.1163/187398608x335810",
  "doiLabel": "https://doi.org/10.1163/187398608x335810",
  "authors": [
    {
      "@type": "Person",
      "name": "Amit Schejter",
      "url": "https://in.bgu.ac.il/en/humsos/comm-Studies/pages/staff/amit_schejter.aspx"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Amit Schejter",
      "url": "https://in.bgu.ac.il/en/humsos/comm-Studies/pages/staff/amit_schejter.aspx"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_the_stranger_that_dwelleth_with_you_shall_be_unto_you_as_one_born_among_you_israeli_media_law_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: באמצעות ניתוח ביקורתי של המסמכים המעצבים את תעשיית התקשורת בישראל ואת התפתחותם ההיסטורית, מאמר זה חושף את ההנחות שבבסיס מדיניות התקשורת הישראלית. העיתון טוען שמדיניות זו נובעת מפרשנות צרה של 'דמוקרטיה' הדוחה הזדהות עם המזרח ומחבקת את הניאו-ליברליזם.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני בישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "דיני תקשורת וזכויות תרבות של המיעוט הפלסטיני־ישראלי",
  "permalink": "/democracy_the_stranger_that_dwelleth_with_you_shall_be_unto_you_as_one_born_among_you_israeli_media_law_summary_he.html",
  "paper_url": "democracy_the_stranger_that_dwelleth_with_you_shall_be_unto_you_as_one_born_among_you_israeli_media_law_summary_he.html",
  "sortKey": 202605210223
}
---
