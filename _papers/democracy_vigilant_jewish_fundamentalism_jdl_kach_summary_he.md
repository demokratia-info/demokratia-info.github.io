---
{
  "version": 1,
  "file": "democracy_vigilant_jewish_fundamentalism_jdl_kach_summary_he.html",
  "slug": "democracy_vigilant_jewish_fundamentalism_jdl_kach_summary_he",
  "order": 497,
  "titleHe": "הפונדמנטליזם היהודי הערני מציב מבחן גבול לדמוקרטיה ולביטחון הציבורי",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק במעבר ובקשרים בין תנועות פונדמנטליסטיות יהודיות מיליטנטיות, וממקם אותן בתוך שאלות של אלימות פוליטית, קיצוניות וגבולות ההשתתפות הדמוקרטית.",
  "summaryHe": "המאמר של Raphael Cohen-Almagor מנתח תופעות של פונדמנטליזם יהודי ערני מן ה-JDL ועד כך, על רקע שאלות של קיצוניות פוליטית ואלימות. חשיבותו לאתר היא בדיון בגבול שבין מחאה, ארגון פוליטי, מיליטנטיות ופגיעה בסדר דמוקרטי.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Raphael Cohen-Almagor",
  "paperTitle": "Vigilant Jewish fundamentalism: From the JDL to Kach (or ‘Shalom Jews, Shalom Dogs')",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-5869-9243\" target=\"_blank\" rel=\"noopener noreferrer\">Raphael Cohen-Almagor</a>",
  "journal": "Terrorism and Political Violence",
  "dateText": "פורסם: מרץ 1992",
  "doiUrl": "https://doi.org/10.1080/09546559208427137",
  "doiLabel": "https://doi.org/10.1080/09546559208427137",
  "authors": [
    {
      "@type": "Person",
      "name": "Raphael Cohen-Almagor",
      "url": "https://orcid.org/0000-0002-5869-9243"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Raphael Cohen-Almagor",
      "url": "https://orcid.org/0000-0002-5869-9243"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "פונדמנטליזם יהודי",
    "קיצוניות פוליטית",
    "אלימות פוליטית",
    "כך",
    "JDL",
    "ביטחון ציבורי",
    "דמוקרטיה מתגוננת"
  ],
  "image": {
    "src": "assets/article-images/real_photo_835_security_democracy_vigilant_jewish_fundamentalism_jdl_kach_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור להפונדמנטליזם היהודי הערני מציב מבחן גבול לדמוקרטיה ולביטחון הציבורי",
    "fitness": "high",
    "license": "CC BY 2.0",
    "creator": "Israel Defense Forces",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_Israel_Defense_Forces_-_Chief_of_Staff_Visits_Paratrooper_Exercise.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/8/88/Flickr_-_Israel_Defense_Forces_-_Chief_of_Staff_Visits_Paratrooper_Exercise.jpg",
    "fileTitle": "Flickr - Israel Defense Forces - Chief of Staff Visits Paratrooper Exercise.jpg",
    "sourceDimensions": "2134x1417",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:security",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/2.0/"
  },
  "datePublished": "2026-06-08",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "קיצוניות פוליטית מאורגנת בוחנת את יכולתה של דמוקרטיה להגן על חופש פוליטי בלי לאפשר אלימות, הדרה ואיום על יריבים.",
  "sections": [
    {
      "headingHe": "מה עומד במרכז המאמר?",
      "paragraphsHtml": [
        "המאמר מתמקד בתופעה של פונדמנטליזם יהודי ערני ובזיקות בין ארגונים ותנועות מיליטנטיות. הוא בוחן קיצוניות פוליטית כבעיה של דמוקרטיה, ביטחון ציבורי וגבולות הלגיטימיות הפוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "החיבור נועד להסביר את הופעת הכהניזם בישראל ומדוע הדמוקרטיה הישראלית ראתה צורך להתגונן מפני רעיונותיו המעין־פשיסטיים של כהנא ולצמצם את הלגיטימיות שלו. המחבר דן בתופעה הכהניסטית, באידיאולוגיה שלה ובתוכנית הפוליטית שהביאה את הרב כהנא לכנסת. קריאת ההצעות הגזעניות של כך מסבירה מדוע נקטו המערכת הפוליטית, התקשורת ומערכת החינוך צעדים יוצאי דופן נגד כהנא.",
        "המאמר של Raphael Cohen-Almagor מנתח תופעות של פונדמנטליזם יהודי ערני מן ה-JDL ועד כך, על רקע שאלות של קיצוניות פוליטית ואלימות. חשיבותו לאתר היא בדיון בגבול שבין מחאה, ארגון פוליטי, מיליטנטיות ופגיעה בסדר דמוקרטי."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>דמוקרטיה מתגוננת</strong> היא תפיסה שלפיה משטר דמוקרטי רשאי ולעיתים חייב להגן על עצמו מפני כוחות שמבקשים להשתמש בחירויות דמוקרטיות כדי להרוס אותן. בהקשר של המאמר, השאלה היא כיצד להבחין בין ביטוי פוליטי קשה לבין פעילות שמאיימת באלימות או בשלילת זכויות."
      ]
    },
    {
      "headingHe": "למה קיצוניות פוליטית היא בעיה מוסדית?",
      "paragraphsHtml": [
        "קיצוניות אינה רק עמדה חריפה; היא יכולה להפוך לרשת ארגונית, לשפת לגיטימציה לאלימות וללחץ על מוסדות ציבור. לכן ההתמודדות הדמוקרטית דורשת כללים ברורים, אכיפה שוויונית והגנה על זכויות גם בזמן עימות."
      ]
    },
    {
      "headingHe": "מה הקשר בין JDL וכך בשאלת המאמר?",
      "paragraphsHtml": [
        "כותרת המאמר מציבה רצף או זיקה היסטורית בין JDL לבין כך, ומכאן החשיבות של מעקב אחר מעבר רעיונות, פרקטיקות וסגנונות פעולה. הדגש אינו רק על ארגון אחד, אלא על דפוסי קיצוניות שמקבלים ביטוי פוליטי וחברתי."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לאלימות פוליטית?",
      "paragraphsHtml": [
        "כתב העת שבו פורסם המאמר עוסק בטרור ובאלימות פוליטית, והכותרת ממקמת את התופעה בתוך מרחב זה. השאלה הדמוקרטית היא מתי אידאולוגיה מיליטנטית עוברת ממחאה או רטוריקה לסיכון ממשי לשלום הציבור ולזכויות אחרים."
      ]
    },
    {
      "headingHe": "למה חשוב לא להאדיר את התופעה?",
      "paragraphsHtml": [
        "מחקר על קיצוניות חייב להבין את התופעה בלי להפוך אותה למושכת או לגיבורה. לכן התקציר מדגישה את ההקשר המוסדי והדמוקרטי, ולא את הסמלים או השפה הפנימית של התנועות."
      ]
    },
    {
      "headingHe": "מהי הדילמה של חופש הביטוי?",
      "paragraphsHtml": [
        "דמוקרטיה מגינה על ביטוי פוליטי חריף, גם כאשר הוא מקומם או לא פופולרי. עם זאת, כאשר הביטוי כרוך בהסתה, איום או שלילת זכויות בסיסיות מאחרים, ההגנה הדמוקרטית נתקלת בגבולותיה."
      ]
    },
    {
      "headingHe": "איך המאמר רלוונטי לחברה אזרחית?",
      "paragraphsHtml": [
        "חברה אזרחית כוללת ארגונים, תנועות ורשתות פעולה שאינן המדינה, אך היא אינה חסינה מפני קיצוניות. המאמר מזכיר שהמרחב האזרחי עצמו יכול להיות מקור להשתתפות דמוקרטית או לאיום על יריבים וקבוצות מיעוט."
      ]
    },
    {
      "headingHe": "מה הקשר לזכויות מיעוטים?",
      "paragraphsHtml": [
        "קיצוניות פוליטית מיליטנטית נוטה לעיתים להגדיר קבוצות אחרות כאויב או כמי שאינן ראויות לשוויון. לכן בחינתה קשורה ישירות ליכולת של דמוקרטיה להגן על מיעוטים מפני אלימות, הפחדה והדרה."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למדיניות אכיפה?",
      "paragraphsHtml": [
        "מדיניות אכיפה צריכה להיות ממוקדת בהתנהגות, הסתה וסיכון ממשי, ולא בזהות פוליטית כללית. כך ניתן להגן על הסדר הדמוקרטי בלי להפוך את ההתמודדות לקולקטיבית או שרירותית."
      ]
    },
    {
      "headingHe": "מה לא נכון להסיק מן המאמר?",
      "paragraphsHtml": [
        "לא נכון להסיק שכל דתיות פוליטית היא פונדמנטליזם או שכל מחאה חריפה היא אלימות. המאמר עוסק בדפוס מסוים של מיליטנטיות פוליטית ולכן יש להבחין בינו לבין השתתפות פוליטית לגיטימית."
      ]
    },
    {
      "headingHe": "איזה לקח מחקרי עולה מן הכותרת?",
      "paragraphsHtml": [
        "הכותרת מצביעה על חשיבות של מחקר היסטורי־ארגוני ולא רק של ניתוח רעיוני. כדי להבין קיצוניות, צריך לבדוק גם מעברים בין תנועות, קשרים בין פעילים ותנאים שמאפשרים להן להתבסס."
      ]
    },
    {
      "headingHe": "מהי השאלה הדמוקרטית המעשית?",
      "paragraphsHtml": [
        "השאלה היא כיצד משטר דמוקרטי יכול להציב גבולות לפעילות מאיימת בלי לוותר על עקרונות חופש הביטוי, ההתאגדות וההליך ההוגן. התשובה מחייבת איזון זהיר בין הגנה על הציבור לבין מניעת שימוש יתר בכוח המדינה."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מזכיר שקיצוניות מאורגנת אינה שולית רק משום שהיא מתחילה בשוליים. דמוקרטיה יציבה צריכה לזהות מוקדם דפוסים שמאיימים על שוויון, ביטחון ולגיטימיות פוליטית, ולפעול נגדם בכלים חוקיים ומידתיים."
      ]
    }
  ],
  "layout": "paper",
  "title": "הפונדמנטליזם היהודי הערני מציב מבחן גבול לדמוקרטיה ולביטחון הציבורי",
  "permalink": "/democracy_vigilant_jewish_fundamentalism_jdl_kach_summary_he.html",
  "paper_url": "democracy_vigilant_jewish_fundamentalism_jdl_kach_summary_he.html",
  "sortKey": 202606080006
}
---
