---
{
  "version": 1,
  "file": "democracy_authoritarianism_curbed_populism_war_israel_summary_he.html",
  "slug": "democracy_authoritarianism_curbed_populism_war_israel_summary_he",
  "order": 13,
  "titleHe": "האם מלחמה בולמת סמכותנות או משנה את צורתה?",
  "subtitleHe": "בן-פורת ופילק בוחנים את היחס בין פופוליזם, מלחמה ודמוקרטיה בישראל.",
  "descriptionHe": "המאמר עוסק בדעת קהל, שיח ציבורי, קיטוב ואלימות פוליטית, וממקם את הדיון בתוך שאלות של יכולת מחלוקת דמוקרטית, אמון הדדי וחופש ביטוי.",
  "summaryHe": "המאמר שואל האם מלחמה מצמצמת פרויקט פופוליסטי-סמכותני או מאפשרת לו להתארגן מחדש סביב ביטחון, זהות ולגיטימציה ציבורית.",
  "authorsCardHe": "גיא בן-פורת, דני פילק",
  "paperTitle": "Authoritarianism Curbed? Populism, Democracy and War in Israel",
  "authorsHtml": "<a href=\"https://cris.bgu.ac.il/en/persons/guy-ben-porat\" target=\"_blank\" rel=\"noopener noreferrer\">Guy Ben-Porat</a>; <a href=\"https://cris.bgu.ac.il/en/persons/dani-filc\" target=\"_blank\" rel=\"noopener noreferrer\">Dani Filc</a>",
  "journal": "",
  "dateText": "פורסם: 24.09.2025",
  "sourcePublishedDate": "2025-09-24",
  "doiUrl": "https://doi.org/10.55271/jps000118",
  "doiLabel": "https://doi.org/10.55271/jps000118",
  "authors": [
    {
      "@type": "Person",
      "name": "גיא בן-פורת",
      "url": "https://cris.bgu.ac.il/en/persons/guy-ben-porat"
    },
    {
      "@type": "Person",
      "name": "דני פילק",
      "url": "https://cris.bgu.ac.il/en/persons/dani-filc"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Guy Ben-Porat",
      "url": "https://cris.bgu.ac.il/en/persons/guy-ben-porat"
    },
    {
      "@type": "Person",
      "name": "Dani Filc",
      "url": "https://cris.bgu.ac.il/en/persons/dani-filc"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "פופוליזם",
    "מלחמה",
    "סמכותנות",
    "דמוקרטיה ישראלית",
    "ביטחון"
  ],
  "image": {
    "src": "html_qa/democracy_political_violence_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על מלחמה, פופוליזם וקיטוב פוליטי",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר שואל האם מלחמה מצמצמת פרויקט פופוליסטי-סמכותני או מאפשרת לו להתארגן מחדש סביב ביטחון, זהות ולגיטימציה ציבורית. התרומה היא התבוננות בזהירות כפולה: מצב חירום יכול לבלום חלק מן המאבק הפוליטי, אך גם לפתוח דרכים חדשות לריכוז כוח.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "פופוליזם הוא סגנון או תפיסה פוליטית שמציגה את הפוליטיקה כמאבק בין <strong>העם</strong> לבין <strong>אליטה</strong>, וטוענת שיש רצון עממי אחד שאפשר וצריך לבטא ישירות. בהקשר של המאמר, ההסבר חשוב כדי להבין מתי פנייה בשם הציבור מחזקת השתתפות פוליטית ומתי היא מחלישה פלורליזם, ביקורת מוסדית וזכויות של מי שאינם חלק מן הרוב."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר ״האם הסמכותנות נבלמה? פופוליזם, דמוקרטיה ומלחמה בישראל״ מוצג כאן דרך השאלה הדמוקרטית המרכזית שעולה ממנו.",
        "התרומה היא התבוננות בזהירות כפולה: מצב חירום יכול לבלום חלק מן המאבק הפוליטי, אך גם לפתוח דרכים חדשות לריכוז כוח."
      ]
    },
    {
      "headingHe": "מהי הטענה המרכזית?",
      "paragraphsHtml": [
        "המאמר שואל האם מלחמה מצמצמת פרויקט פופוליסטי-סמכותני או מאפשרת לו להתארגן מחדש סביב ביטחון, זהות ולגיטימציה ציבורית.",
        "הקריאה הדמוקרטית של המאמר היא שהגנה על מוסדות, זכויות והשתתפות ציבורית מחייבת תשומת לב גם למנגנונים שנראים טכניים או מקצועיים.",
        "לכן המאמר מתאים לאתר: הוא מסביר כיצד החלטות משפטיות, פוליטיות או מוסדיות משפיעות על היכולת של אזרחים לחיות תחת כללי משחק הוגנים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מאז ינואר 2023 מאות אלפי ישראלים יצאו לרחובות בגל מחאה חסר תקדים נגד תוכנית הממשלות להגביל את כוחו של בית המשפט העליון. הממשלה, קואליציה בין המפלגה הפופוליסטית של הליכוד, החרדים והימין הדתי הקיצוני הכריזה על חבילת חקיקה המאיימת על החוקתיות הליברלית -המוגבלת- של מוסדות ישראל, ופותחת אפשרות לסמכותיות. על אף כוונות הממשלות, אנו טוענים, באמצעות המקרה הישראלי, כי ה\"גלישה\" מפופוליזם ימני לסמכותיות אינה בלתי נמנעת.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר שואל האם מלחמה מצמצמת פרויקט פופוליסטי-סמכותני או מאפשרת לו להתארגן מחדש סביב ביטחון, זהות ולגיטימציה ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם במוסדות שמרסנים כוח, מגינים על מיעוטים ומאפשרים ביקורת ציבורית.",
        "כאשר אחד מן הרכיבים האלה נחלש, הפגיעה עשויה להופיע בהדרגה: פחות שקיפות, פחות אחריות, פחות הגנה על זכויות ופחות אמון ביכולת של המערכת לתקן את עצמה.",
        "המאמר עוזר לזהות את נקודת החיבור בין סוגיה אקדמית מוגדרת לבין שאלות ציבוריות רחבות על שלטון החוק, שוויון ואחריות."
      ]
    },
    {
      "headingHe": "מה הקורא הישראלי יכול לקחת מכאן?",
      "paragraphsHtml": [
        "הלקח המעשי הוא לבחון לא רק את הכותרת של שינוי מוסדי, אלא את ההשפעה המצטברת שלו על כללי המשחק.",
        "גם כאשר הרפורמה, הפסיקה או התהליך מוצגים כמקצועיים, השאלה הדמוקרטית נשארת: מי מרוויח כוח, מי מאבד הגנה, ואילו מנגנוני ביקורת נשארים פעילים.",
        "המאמר מזמין דיון ציבורי מדויק יותר, שמפריד בין מחלוקת לגיטימית לבין שינוי שמחליש את תנאי המחלוקת עצמה."
      ]
    },
    {
      "headingHe": "מה נושא המאמר?",
      "paragraphsHtml": [
        "המאמר עוסק ב־<strong>האם מלחמה בולמת סמכותנות או משנה את צורתה?</strong>.",
        "במונחים פשוטים, הוא שואל כיצד המוסדות והכללים משפיעים על איכות הדמוקרטיה ולא רק על התוצאה הפוליטית המיידית."
      ]
    },
    {
      "headingHe": "למה המאמר רלוונטי לישראל?",
      "paragraphsHtml": [
        "המאמר רלוונטי לישראל משום שהוא עוסק במתח בין כוח פוליטי, מוסדות ביקורת, זכויות וזהות ציבורית.",
        "אלו בדיוק השאלות שמעצבות את הוויכוח הישראלי על דמוקרטיה ליברלית, במיוחד בתקופה של קיטוב ומשבר מוסדי."
      ]
    },
    {
      "headingHe": "מהו הסיכון הדמוקרטי המרכזי?",
      "paragraphsHtml": [
        "הסיכון המרכזי הוא ששינוי נקודתי ייתפס כמחלוקת רגילה, אף שהוא משנה את מאזן הכוחות או את רמת ההגנה על זכויות.",
        "כאשר שינויים כאלה מצטברים, קשה יותר להחזיר את המערכת לאיזון דמוקרטי תקין."
      ]
    },
    {
      "headingHe": "האם המאמר נוקט עמדה פוליטית מפלגתית?",
      "paragraphsHtml": [
        "המאמר אינו נדרש להחליף בחירה פוליטית של אזרחים, אלא מציע כלים לבחינת איכות הכללים הדמוקרטיים.",
        "הבחינה מתמקדת בשאלות של <strong>שלטון החוק</strong>, <strong>שוויון</strong>, <strong>אחריות</strong> ויכולת של מוסדות להגביל כוח."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לזכויות?",
      "paragraphsHtml": [
        "גם כאשר המאמר עוסק במוסדות, ההשלכה הסופית נוגעת לעיתים קרובות לזכויות של יחידים וקבוצות.",
        "מוסדות חלשים או מוטים מתקשים להגן על חופש ביטוי, שוויון אזרחי, זכויות מיעוטים והשתתפות פוליטית הוגנת."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק דורש שכללי המשחק יחולו גם על בעלי כוח ולא רק על אזרחים רגילים.",
        "המאמרים בקבוצה הזו מדגישים שכאשר מנגנוני ביקורת נחלשים, שלטון החוק עלול להפוך מסדר מחייב לשפה פוליטית בלבד."
      ]
    },
    {
      "headingHe": "מה הקשר לקיטוב?",
      "paragraphsHtml": [
        "קיטוב משפיע על היכולת של ציבור לקבל הכרעות מוסדיות כהלגיטימיות גם כאשר הן אינן נוחות לו.",
        "כאשר כל מחנה רואה באחר איום קיומי, קל יותר להצדיק פגיעה בכללים בשם ניצחון פוליטי או ביטחון."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "המאמר שואל האם מלחמה מצמצמת פרויקט פופוליסטי-סמכותני או מאפשרת לו להתארגן מחדש סביב ביטחון, זהות ולגיטימציה ציבורית.",
        "השורה התחתונה לאזרחית ולאזרח היא לבחון כל שינוי לפי השפעתו על כוח, זכויות, אחריות ואמון, ולא רק לפי הכוונה המוצהרת של יוזמיו."
      ]
    },
    {
      "headingHe": "למי כדאי לקרוא את המקור?",
      "paragraphsHtml": [
        "המקור מתאים לקוראים שמתעניינים במשפט חוקתי, מדע המדינה, דעת קהל, זכויות אדם או מוסדות דמוקרטיים.",
        "הוא שימושי במיוחד למי שרוצה להבין את הקשר בין מחקר אקדמי לבין הוויכוחים הציבוריים בישראל על דמוקרטיה ליברלית."
      ]
    }
  ],
  "layout": "paper",
  "title": "האם מלחמה בולמת סמכותנות או משנה את צורתה?",
  "permalink": "/democracy_authoritarianism_curbed_populism_war_israel_summary_he.html",
  "paper_url": "democracy_authoritarianism_curbed_populism_war_israel_summary_he.html",
  "sortKey": 202605210023
}
---
