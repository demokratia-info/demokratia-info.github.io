---
{
  "version": 1,
  "file": "democracy_partition_and_the_arab_palestinian_minority_in_israel_summary_he.html",
  "slug": "democracy_partition_and_the_arab_palestinian_minority_in_israel_summary_he",
  "order": 135,
  "titleHe": "החלוקה והמיעוט הפלסטיני־ערבי בישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית החלוקה והמיעוט הפלסטיני בישראל, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית החלוקה והמיעוט הפלסטיני בישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Asher Susser",
  "paperTitle": "Partition and the Arab Palestinian Minority in Israel",
  "authorsHtml": "Asher Susser",
  "journal": "Israel Studies",
  "dateText": "פורסם: 2009",
  "doiUrl": "https://doi.org/10.2979/isr.2009.14.2.105",
  "doiLabel": "https://doi.org/10.2979/isr.2009.14.2.105",
  "authors": [
    {
      "@type": "Person",
      "name": "Asher Susser"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Asher Susser"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "החלוקה והמיעוט הפלסטיני בישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_partition_and_the_arab_palestinian_minority_in_israel_summary_he.jpg",
    "version": "2026-05-21-image-audit",
    "altHe": "איור עריכתי על החלוקה והמיעוט הפלסטיני בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית החלוקה והמיעוט הפלסטיני בישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית החלוקה והמיעוט הפלסטיני בישראל, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: בסוף 2006 ערכו אינטלקטואלים ערבים פלסטינים מובילים, מנהיגים פוליטיים ופעילים בישראל את חזון העתיד של הערבים הפלסטינים בישראל. המסמך דחה את ייעודה של ישראל כמדינה יהודית, שלטענתם הוא מדיר ושלל שוויון מלא מהערבים. מאמר זה טוען כי בעוד שהכרה בפלסטינים בישראל כמיעוט לאומי היא דרישה סבירה בהחלט, הרוב היהודי יתקשה לקבל כל עוד הזכויות הלאומיות של הרוב היהודי נשללו בו-זמנית על ידי אזרחי ישראל הפלסטינים.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת החלוקה והמיעוט הפלסטיני בישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר החלוקה והמיעוט הפלסטיני בישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "החלוקה והמיעוט הפלסטיני־ערבי בישראל",
  "permalink": "/democracy_partition_and_the_arab_palestinian_minority_in_israel_summary_he.html",
  "paper_url": "democracy_partition_and_the_arab_palestinian_minority_in_israel_summary_he.html",
  "sortKey": 202605210158
}
---
