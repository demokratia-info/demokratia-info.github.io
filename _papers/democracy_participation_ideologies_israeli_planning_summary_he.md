---
{
  "version": 1,
  "file": "democracy_participation_ideologies_israeli_planning_summary_he.html",
  "slug": "democracy_participation_ideologies_israeli_planning_summary_he",
  "titleHe": "אידיאולוגיות תכנון בישראל מעצבות את גבולות השתתפות הציבור",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן השתתפות ציבורית בתכנון הישראלי דרך מקורות אידיאולוגיים של פרקטיקות תכנון. הוא מציע להבין אד־הוקיזם, קליינטליזם, פופוליזם ודמוקרטיה קיבוצית כמסגרות שמעצבות את הקשר בין מתכננים לציבור.",
  "summaryHe": "Gertel ו־Law-Yone טוענים שמחקרי השתתפות בתכנון נוטים להתעלם מן התפקיד של אידיאולוגיה בפרקטיקה מקצועית. דרך המקרה הישראלי, הם מראים כיצד דפוסי פיתוח פוליטיים וחברתיים משפיעים על השתתפות הציבור ועל תחושת האחריות של המתכנן.",
  "authorsCardHe": "S Gertel, Hubert Law-Yone",
  "paperTitle": "Participation Ideologies in Israeli Planning",
  "authorsHtml": "S Gertel; Hubert Law-Yone",
  "journal": "Environment and Planning C: Government and Policy, 9(2), 173-188",
  "dateText": "פורסם: יוני 1991",
  "sourcePublishedDate": "1991-06-01",
  "doiUrl": "https://doi.org/10.1068/c090173",
  "doiLabel": "https://doi.org/10.1068/c090173",
  "authors": [
    {
      "@type": "Person",
      "name": "S Gertel"
    },
    {
      "@type": "Person",
      "name": "Hubert Law-Yone"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "S Gertel"
    },
    {
      "@type": "Person",
      "name": "H Law-Yone"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "השתתפות ציבורית",
    "תכנון ישראלי",
    "אידיאולוגיה תכנונית",
    "אד־הוקיזם",
    "קליינטליזם",
    "פופוליזם",
    "דמוקרטיה קיבוצית"
  ],
  "image": {
    "src": "html_qa/democracy_participation_ideologies_israeli_planning_summary_he.jpg",
    "version": "2026-06-02-nightly",
    "altHe": "מתכננים ונציגי ציבור בוחנים מפות ארכיון בדיון על השתתפות בתכנון הישראלי",
    "fitness": "high"
  },
  "datePublished": "2026-06-02",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שהשתתפות ציבורית בתכנון אינה רק טכניקה של שיתוף. היא מעוצבת בידי אידיאולוגיות פיתוח, יחסי מתכנן־ציבור ודימויים שונים של דמוקרטיה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>השתתפות ציבורית</strong> בתכנון היא היכולת של תושבים וקבוצות להשפיע על החלטות מרחביות לפני שהן מתקבעות. במאמר ההשתתפות נבחנת כפרקטיקה אידיאולוגית, כלומר כמשהו שמושפע מרעיונות על חברה, פיתוח וסמכות מקצועית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: את סוגיית שיתוף הציבור בתהליך התכנון ניתן לבחון מנקודת מבטם של הקדמה האידיאולוגית הכללית של רעיונות תכנונים. אף על פי שחשיבותה של האידיאולוגיה בהתפתחותה של ישראל מוזכרת לעתים קרובות, מחקרים על תכנון ישראלי אינם כוללים אידיאולוגיה במודלים שלהם. במאמר זה מסתכלים על מספר כיוונים כלפי נושא ההשתתפות, המוגדרים בתוך ההקשר הישראלי.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: Gertel ו־Law-Yone טוענים שמחקרי השתתפות בתכנון נוטים להתעלם מן התפקיד של אידיאולוגיה בפרקטיקה מקצועית. דרך המקרה הישראלי, הם מראים כיצד דפוסי פיתוח פוליטיים וחברתיים משפיעים על השתתפות הציבור ועל תחושת האחריות של המתכנן."
      ]
    },
    {
      "headingHe": "מה הבעיה שהמאמר מזהה במחקרי השתתפות?",
      "paragraphsHtml": [
        "המאמר טוען שמחקרי השתתפות נוטים להתעלם מן השורשים האידיאולוגיים של רעיונות תכנוניים. כאשר מתכננים אינם מודעים לאידיאולוגיה שמנחה אותם, הם עלולים לראות התנגדות ציבורית כבעיה טכנית במקום כחלק מדמוקרטיה."
      ]
    },
    {
      "headingHe": "מדוע המקרה הישראלי מתאים לשאלה הזו?",
      "paragraphsHtml": [
        "הפיתוח בישראל הושפע ממחנות פוליטיים, תפיסות לאומיות, פרקטיקות התיישבות ומסורות מוסדיות שונות. לכן הוא מאפשר לראות כיצד השתתפות ציבורית נוצרת בתוך שדה אידיאולוגי ולא רק בתוך נהלים מקצועיים."
      ]
    },
    {
      "headingHe": "מהו אד־הוקיזם בהקשר התכנוני?",
      "paragraphsHtml": [
        "אד־הוקיזם מתאר נטייה לפתור בעיות נקודתית וללא מסגרת עקרונית יציבה. בדמוקרטיה תכנונית, גישה כזו יכולה להחליש שקיפות ושוויון משום שהציבור מתקשה לדעת לפי אילו כללים מתקבלות החלטות."
      ]
    },
    {
      "headingHe": "מהו קליינטליזם בתכנון?",
      "paragraphsHtml": [
        "קליינטליזם מתאר יחסי תלות וחליפין בין בעלי כוח לבין קבוצות או יחידים. כאשר הוא משפיע על תכנון, ההשתתפות עלולה להתקיים דרך קשרים ומתווכים במקום דרך הליך פתוח ושוויוני."
      ]
    },
    {
      "headingHe": "איך פופוליזם משפיע על השתתפות?",
      "paragraphsHtml": [
        "פופוליזם יכול להציג עצמו כהקשבה ישירה לעם, אך לעיתים הוא עוקף מוסדות, ידע מקצועי וקולות מיעוט. בתכנון עירוני, הסכנה היא שהשתתפות תהפוך לאישור רטורי של החלטות שכבר התקבלו."
      ]
    },
    {
      "headingHe": "מהי דמוקרטיה קיבוצית במאמר?",
      "paragraphsHtml": [
        "דמוקרטיה קיבוצית מתייחסת למסורת של השתתפות קולקטיבית ותפיסה קהילתית של החלטות. המאמר משתמש בה כאחת האוריינטציות שמעצבות את היחס בין ציבור, מוסדות ותכנון בישראל."
      ]
    },
    {
      "headingHe": "למה מתכננים צריכים מודעות עצמית?",
      "paragraphsHtml": [
        "מודעות עצמית מאפשרת למתכננים להבין שהחלטותיהם אינן ניטרליות לגמרי. כאשר הם מזהים את ההנחות האידיאולוגיות שלהם, הם יכולים ליצור קשר מקצועי יותר הוגן עם הציבור."
      ]
    },
    {
      "headingHe": "מה הקשר בין השתתפות לבין אחריות מקצועית?",
      "paragraphsHtml": [
        "אחריות מקצועית אינה רק הפקת תכנית איכותית מבחינה טכנית. היא כוללת גם הסבר, הקשבה והכרה בכך שהציבור מחזיק ידע מקומי וזכות להשפיע על סביבתו."
      ]
    },
    {
      "headingHe": "איך המאמר תורם לשלטון מקומי?",
      "paragraphsHtml": [
        "המאמר מספק שפה לניתוח פרקטיקות השתתפות ברשויות ובמוסדות תכנון. במקום לשאול רק אם נערך שיתוף ציבור, הוא מזמין לבדוק איזה דגם אידיאולוגי של השתתפות הופעל בפועל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית לדמוקרטיה תכנונית?",
      "paragraphsHtml": [
        "התרומה היא הבנה שהשתתפות ציבורית דורשת בירור של רעיונות יסוד, לא רק הוספת שלב פרוצדורלי. דמוקרטיה תכנונית חזקה יותר כאשר המתכננים והציבור יודעים אילו הנחות עומדות מאחורי ההליך."
      ]
    }
  ],
  "layout": "paper",
  "title": "אידיאולוגיות תכנון בישראל מעצבות את גבולות השתתפות הציבור",
  "permalink": "/democracy_participation_ideologies_israeli_planning_summary_he.html",
  "paper_url": "democracy_participation_ideologies_israeli_planning_summary_he.html",
  "sortKey": 202606020010
}
---
