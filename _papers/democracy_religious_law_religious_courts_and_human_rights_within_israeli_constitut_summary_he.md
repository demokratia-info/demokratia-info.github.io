---
{
  "version": 1,
  "file": "democracy_religious_law_religious_courts_and_human_rights_within_israeli_constitut_summary_he.html",
  "slug": "democracy_religious_law_religious_courts_and_human_rights_within_israeli_constitut_summary_he",
  "order": 13,
  "titleHe": "דין דתי, בתי דין דתיים וזכויות אדם במבנה החוקתי הישראלי",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את המתח בין דין דתי, בתי דין דתיים וזכויות אדם במבנה החוקתי הישראלי, דרך דיני מעמד אישי ושוויון בין בני זוג.",
  "summaryHe": "המאמר מנתח התנגשות בתוך מערכת משפט אחת: בתי דין דתיים ובתי משפט אזרחיים פועלים שניהם כחלק מן הסדר המשפטי הישראלי, אך מחויבים לעיתים למקורות נורמטיביים שונים. המוקד הוא פסיקת בית המשפט העליון שלפיה בתי הדין הדתיים חייבים להחיל עקרונות סטטוטוריים של שוויון בין בני זוג, אלא אם הצדדים הסכימו במפורש להחלת הדין הדתי. דרך פרשות כגון בבלי וימיני, המאמר מראה כיצד חלוקת רכוש בין בני זוג הפכה לזירת עימות בין שוויון מגדרי לבין סמכות דתית.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "ענת סקולניקוב",
  "paperTitle": "Religious law, religious Courts and human rights within Israeli constitutional structure",
  "authorsHtml": "<a href=\"https://www.winchester.ac.uk/about-us/leadership-and-governance/staff-directory/staff-profiles/scolnicov.php\" target=\"_blank\" rel=\"noopener noreferrer\">Anat Scolnicov</a>",
  "journal": "International Journal of Constitutional Law",
  "dateText": "פורסם: 01.10.2006",
  "doiUrl": "https://doi.org/10.1093/icon/mol037",
  "doiLabel": "https://doi.org/10.1093/icon/mol037",
  "authors": [
    {
      "@type": "Person",
      "name": "ענת סקולניקוב",
      "url": "https://www.winchester.ac.uk/about-us/leadership-and-governance/staff-directory/staff-profiles/scolnicov.php"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Anat Scolnicov",
      "url": "https://www.winchester.ac.uk/about-us/leadership-and-governance/staff-directory/staff-profiles/scolnicov.php"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חוקתיות ושלטון החוק",
    "דמוקרטיה ליברלית",
    "שלטון החוק",
    "זכויות",
    "מוסדות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_639_religion_democracy_religious_law_religious_courts_and_human_rights_within_israeli_constitut_summary.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על בית משפט, חוקה ושלטון החוק",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Godot13",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Jerusalem-2013(2)-Temple_Mount-Dome_of_the_Rock_(SE_exposure).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/8/8b/Jerusalem-2013%282%29-Temple_Mount-Dome_of_the_Rock_%28SE_exposure%29.jpg",
    "fileTitle": "Jerusalem-2013(2)-Temple Mount-Dome of the Rock (SE exposure).jpg",
    "sourceDimensions": "7500x5425",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:religion",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מציג את דיני המעמד האישי בישראל כזירה שבה שוויון בין בני זוג מתנגש עם סמכותם של בתי הדין הדתיים.",
  "sections": [
    {
      "headingHe": "איך דיני מעמד אישי הופכים לשאלה חוקתית?",
      "paragraphsHtml": [
        "המאמר פותח מן המתח של המתח בין סמכות דתית בענייני מעמד אישי לבין זכויות אדם ושוויון מגדרי. במקום להציג את הנושא כמחלוקת מופשטת, הוא ממקם אותו בתוך מוסדות, פרקטיקות ושחקנים שפועלים בתנאים פוליטיים ממשיים.",
        "כך דין דתי ובתי דין דתיים במבנה החוקתי נעשה דרך לקרוא את היחסים בין כללים פורמליים לבין ניסיון אזרחי חי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתמקד במבנה חוקתי שבו בתי דין דתיים ובתי משפט חילוניים הם חלק מאותה מערכת משפטית, אך עשויים להגיע להתנגשות חזיתית. ההקשר המרכזי הוא דיני מעמד אישי, ובייחוד שאלות של רכוש וגירושין שבהן עקרונות שוויון מגדרי מקבלים משמעות מעשית.",
        "נקודת המוצא היא פסיקת בית המשפט העליון שלפיה בתי הדין הדתיים חייבים להחיל עקרונות חוקיים של שוויון בין בני זוג, אלא אם בני הזוג הסכימו במפורש שהדין הדתי יחול. בפרשת ימיני, אשה בהליכי גירושין ביקשה חלוקה שוויונית של הרכוש לפי חוק יחסי ממון בין בני זוג משנת 1973.",
        "המאמר מציג את פרשת בבלי כרקע מרכזי: בית המשפט העליון הורה לבתי הדין הרבניים להחיל את עקרון שיתוף הנכסים, שאינו חלק מן הדין היהודי. בפועל, בתי הדין הרבניים לא קיבלו את ההנחיה, ואף הצהירו שימשיכו לפסוק לפי הדין היהודי בלבד; מכאן נחשף הסיכון החוקתי במאבק סמכויות בין ערכאות."
      ]
    },
    {
      "headingHe": "איפה סמכות דתית מתנגשת עם זכויות אדם?",
      "paragraphsHtml": [
        "המאמר מזהה במבנה החוקתי שני קשיים שלובים: מאבק סמכויות בין בתי הדין הדתיים למערכת המשפט האזרחית, ופגיעה בזכויות אדם, ובפרט בחופש הדת.",
        "מתוך הזירה הזאת אפשר לראות כיצד שפה משפטית, מנהלית או ציבורית מקבלת תוצאות שונות עבור קבוצות, מוסדות ואזרחים."
      ]
    },
    {
      "headingHe": "למה שוויון מגדרי עומד במרכז הוויכוח?",
      "paragraphsHtml": [
        "המהלך המרכזי של המאמר הוא הטענה כי הפלורליזם הדתי בישראל מוגבל ומעוצב דרך מאבקי סמכות בין מערכות משפטיות. זהו ניסוח שמרחיק את הדיון מהסבר חד-סיבתי ומדגיש תנאים, איזונים ויחסי כוח.",
        "הטענה חשובה במיוחד משום שהיא מראה שהבעיה אינה רק אירוע נקודתי. היא נטועה במבנה שמארגן אפשרויות פעולה ומגבלות."
      ]
    },
    {
      "headingHe": "כיצד מערכת המשפט האזרחית מגבילה בתי דין דתיים?",
      "paragraphsHtml": [
        "מן הזווית הדמוקרטית, דמוקרטיה חוקתית נבחנת ביכולתה להכיל דת בלי להפקיר חופש דת וזכויות נשים. לכן המאמר עוסק לא רק בנושא המקצועי שעל פני השטח, אלא גם בשאלה מי זוכה להכרה, מי נדרש להצדיק את מקומו, ומי מסוגל להשפיע על הכללים.",
        "הקריאה הזאת הופכת את המקרה למבחן של איכות דמוקרטית: לא רק קיומם של מוסדות, אלא דרך פעולתם מול אנשים וקבוצות ממשיים."
      ]
    },
    {
      "headingHe": "מה המקרה הישראלי מלמד על פלורליזם דתי דמוקרטי?",
      "paragraphsHtml": [
        "התרומה הרחבה של המאמר היא מיפוי בתי הדין הדתיים כזירת יסוד של זכויות אדם בישראל. הוא מציע לקורא שפה מדויקת יותר לקריאת המקרה, בלי להסתפק בסיסמאות על זכויות, ביטחון, ריבונות או ייצוג.",
        "בכך הוא משנה את מוקד הדיון: מן השאלה אם הערך הדמוקרטי קיים באופן פורמלי, אל השאלה כיצד הוא מופעל, מוגבל או מתעצב בתוך מציאות מוסדית וחברתית."
      ]
    }
  ],
  "layout": "paper",
  "title": "דין דתי, בתי דין דתיים וזכויות אדם במבנה החוקתי הישראלי",
  "permalink": "/democracy_religious_law_religious_courts_and_human_rights_within_israeli_constitut_summary_he.html",
  "paper_url": "democracy_religious_law_religious_courts_and_human_rights_within_israeli_constitut_summary_he.html",
  "sortKey": 202605210043
}
---
