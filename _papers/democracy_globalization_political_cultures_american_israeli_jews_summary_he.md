---
{
  "version": 1,
  "file": "democracy_globalization_political_cultures_american_israeli_jews_summary_he.html",
  "slug": "democracy_globalization_political_cultures_american_israeli_jews_summary_he",
  "titleHe": "גלובליזציה מקרבת בין שיחי זכויות של יהודים אמריקאים וישראלים",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן כיצד גלובליזציה מעצבת עמדות של יהודים אמריקאים וישראלים כלפי פלורליזם דתי וזכויות אזרח של מיעוטים בישראל.",
  "summaryHe": "המאמר מתמקד ביהודים ישראלים ואמריקאים ממעמד בינוני וממוצא אירופי, ומוצא גם חפיפה בין הקבוצות וגם בידול פנימי בתוכן. הוא מייחס את ההתקרבות הפוליטית לנסיעות בין־לאומיות, למוסדות יהודיים טרנס־לאומיים, להשתלבות ישראל ביחסים קפיטליסטיים גלובליים ולתפיסות ביטחון שמקרבות חלק מיהודי ארצות הברית לדאגות ביטחוניות ביחס לעצמם ולישראל.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Theodore Sasson, Ephraim Tabory",
  "paperTitle": "Converging Political Cultures: How Globalization is Shaping the Discourses of American and Israeli Jews",
  "authorsHtml": "<a href=\"https://www.brandeis.edu/cmjs/about/staff/theodore-sasson.html\" target=\"_blank\" rel=\"noopener noreferrer\">Theodore Sasson</a>; <a href=\"https://cris.biu.ac.il/en/persons/ephraim-tabory\" target=\"_blank\" rel=\"noopener noreferrer\">Ephraim Tabory</a>",
  "journal": "Nationalism and Ethnic Politics, 16(1), 22-41",
  "dateText": "פורסם: מרץ 2010",
  "doiUrl": "https://doi.org/10.1080/13537110903583328",
  "doiLabel": "https://doi.org/10.1080/13537110903583328",
  "authors": [
    {
      "@type": "Person",
      "name": "Theodore Sasson",
      "url": "https://www.brandeis.edu/cmjs/about/staff/theodore-sasson.html"
    },
    {
      "@type": "Person",
      "name": "Ephraim Tabory",
      "url": "https://cris.biu.ac.il/en/persons/ephraim-tabory"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Theodore Sasson",
      "url": "https://www.brandeis.edu/cmjs/about/staff/theodore-sasson.html"
    },
    {
      "@type": "Person",
      "name": "Ephraim Tabory",
      "url": "https://cris.biu.ac.il/en/persons/ephraim-tabory"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "גלובליזציה",
    "תרבות פוליטית",
    "יהדות אמריקה",
    "יהודים ישראלים",
    "פלורליזם דתי",
    "זכויות מיעוטים"
  ],
  "image": {
    "src": "assets/article-images/real_photo_271_minorities_democracy_globalization_political_cultures_american_israeli_jews_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לגלובליזציה מקרבת בין שיחי זכויות של יהודים אמריקאים וישראלים",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Romayan",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Rahat_view.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/7/74/Rahat_view.jpg",
    "fileTitle": "Rahat view.jpg",
    "sourceDimensions": "4288x2848",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:minorities",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-05-31",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שעמדות יהודיות כלפי פלורליזם וזכויות מיעוטים בישראל נוצרות גם דרך רשתות גלובליות ולא רק בתוך גבולות המדינה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>תרבות פוליטית</strong> היא מערכת של תפיסות, ערכים ודפוסי שיח שדרכם קבוצה מבינה סמכות, זכויות וזהות ציבורית. במאמר הזה המושג אינו נשאר בתוך גבולות המדינה, אלא נבחן כמרחב שבו קהילות יהודיות בישראל ובארצות הברית משפיעות זו על זו."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את השפעת הגלובליזציה על עמדות של יהודים ישראלים ואמריקאים כלפי פלורליזם דתי וזכויות אזרח של מיעוטים בישראל. המחקר מתמקד ביהודים ממעמד בינוני וממוצא אירופי, ולכן הממצאים מתארים קבוצות מוגדרות ולא את כלל היהודים בישראל ובארצות הברית.",
        "המחקר מצא חפיפה בין חלק מן העמדות של שתי הקבוצות, אך גם בידול פנימי בתוך כל אחת מהן. במקום לתאר שתי תרבויות פוליטיות נפרדות לחלוטין, המאמר מצביע על דפוס שבו ישראלים ואמריקאים יהודים מחפשים שותפים רעיוניים דומים להם מעבר לגבול הלאומי.",
        "המאמר מייחס את ההתקרבות לארבעה מנגנוני גלובליזציה: נסיעות בין־לאומיות, התפשטות מוסדות יהודיים טרנס־לאומיים, שילוב גובר של ישראל ביחסים קפיטליסטיים גלובליים ותפיסות של איום ביטחוני שמגבירות אצל יהודים אמריקאים דאגה לביטחונם ולביטחון ישראל. כך שיח הזכויות והפלורליזם מוצג כתוצר של קשרים חברתיים ומוסדיים חוצי גבולות."
      ]
    },
    {
      "headingHe": "מהי נקודת המוצא של המאמר?",
      "paragraphsHtml": [
        "המאמר יוצא מהשאלה כיצד גלובליזציה משנה עמדות יהודיות ביחס לפלורליזם דתי ולזכויות מיעוטים בישראל. במקום להניח שקיימות שתי תרבויות פוליטיות נפרדות, הוא מחפש אזורי חפיפה והבדלים בתוך כל קהילה."
      ]
    },
    {
      "headingHe": "אילו מנגנוני גלובליזציה מופיעים בניתוח?",
      "paragraphsHtml": [
        "התקציר מצביע על נסיעות בין-לאומיות, מוסדות יהודיים טרנס-לאומיים, השתלבות ישראל ביחסים קפיטליסטיים גלובליים ותפיסות ביטחון משותפות. כל מנגנון כזה מעביר רעיונות, פחדים ושפה פוליטית בין קהילות שאינן חיות באותה מערכת מוסדית."
      ]
    },
    {
      "headingHe": "מדוע פלורליזם דתי הוא סוגיה דמוקרטית?",
      "paragraphsHtml": [
        "פלורליזם דתי נוגע לשאלה אם המדינה מכירה במגוון זהויות וזרמים בתוך קהילה לאומית אחת. כאשר הסוגיה נקשרת לזכויות מיעוטים, היא בוחנת אם רוב תרבותי יכול להחזיק כוח בלי לסגור את המרחב האזרחי בפני אחרים."
      ]
    },
    {
      "headingHe": "מה נמצא ביחס ליהודים אמריקאים וישראלים?",
      "paragraphsHtml": [
        "החוקרים מוצאים חפיפה בין עמדות של יהודים אמריקאים וישראלים ממעמד בינוני וממוצא אירופי. החפיפה אינה מבטלת הבדלים, אך היא מצביעה על שיח פוליטי משותף יותר מכפי שתיאור לאומי פשוט היה מנבא."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לזכויות מיעוטים בישראל?",
      "paragraphsHtml": [
        "הסוגיות שנבחנות כוללות עמדות כלפי זכויות אזרח של מיעוטים בישראל. בכך המאמר מחבר בין זהות יהודית טרנס-לאומית לבין מבחנים ליברליים בסיסיים של שוויון, הכרה והגבלת כוח רובני."
      ]
    },
    {
      "headingHe": "מהי המשמעות של מוסדות יהודיים טרנס-לאומיים?",
      "paragraphsHtml": [
        "מוסדות טרנס-לאומיים יוצרים זירות שבהן יהודים ממדינות שונות מדברים, מתווכחים ופועלים יחד. הם יכולים להפיץ תפיסות ליברליות או ביטחוניות, ולכן הם חשובים להבנת עמדות אזרחיות שאינן נוצרות רק דרך הפוליטיקה המקומית."
      ]
    },
    {
      "headingHe": "האם ההתקרבות מבטלת קיטוב פנימי?",
      "paragraphsHtml": [
        "לא. התקציר מדגיש גם בידול בתוך הקבוצות, ולכן ההתקרבות אינה מתארת הסכמה כללית. היא מתארת יצירת בריתות רעיוניות בין אנשים דומים בשתי קהילות, לצד המשך מחלוקות פנימיות בכל אחת מהן."
      ]
    },
    {
      "headingHe": "למה ביטחון מופיע בתוך דיון בזכויות?",
      "paragraphsHtml": [
        "המאמר מציין שתפיסות של איומים ביטחוניים יכולות לקרב חלק מיהודי ארצות הברית לעמדות ביטחוניות יותר ביחס לישראל ולעצמם. זה חשוב משום שדיונים בזכויות מיעוטים בישראל מתקיימים לעיתים בתוך שפה של ביטחון וזהות קולקטיבית."
      ]
    },
    {
      "headingHe": "מה התרומה למחקר על דמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "התרומה היא בהצגת דמוקרטיה וזכויות כמושגים שנדונים גם בקהילות תפוצה ולא רק בתוך מוסדות המדינה. כך אפשר להבין כיצד שיח חוץ-מדינתי משפיע על תמיכה בפלורליזם, בשוויון ובגבולות כוח הרוב."
      ]
    },
    {
      "headingHe": "מה לא כדאי להסיק מהמאמר?",
      "paragraphsHtml": [
        "לא כדאי להסיק שכל היהודים האמריקאים והישראלים מחזיקים באותה תרבות פוליטית. המאמר מצביע על התקרבות ודמיון בין חלקים מוגדרים, ולכן הקריאה הזהירה היא של תהליך מורכב ולא של אחידות מלאה."
      ]
    },
    {
      "headingHe": "למי המאמר רלוונטי?",
      "paragraphsHtml": [
        "המאמר רלוונטי לחוקרי תפוצות, דמוקרטיה ישראלית, זכויות מיעוטים ויחסי ישראל-ארצות הברית. הוא מועיל במיוחד למי שמבקש להבין כיצד רעיונות ליברליים וביטחוניים נעים בין קהילות ולא רק בין ממשלות."
      ]
    }
  ],
  "layout": "paper",
  "title": "גלובליזציה מקרבת בין שיחי זכויות של יהודים אמריקאים וישראלים",
  "permalink": "/democracy_globalization_political_cultures_american_israeli_jews_summary_he.html",
  "paper_url": "democracy_globalization_political_cultures_american_israeli_jews_summary_he.html",
  "sortKey": 202605310010
}
---
