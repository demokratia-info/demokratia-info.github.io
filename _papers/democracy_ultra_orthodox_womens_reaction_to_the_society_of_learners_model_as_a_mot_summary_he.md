---
{
  "version": 1,
  "file": "democracy_ultra_orthodox_womens_reaction_to_the_society_of_learners_model_as_a_mot_summary_he.html",
  "slug": "democracy_ultra_orthodox_womens_reaction_to_the_society_of_learners_model_as_a_mot_summary_he",
  "order": 39,
  "titleHe": "מה המאמר מלמד על זכויות ושוויון בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק במעמדן הפוליטי והאזרחי של קבוצות מיעוט בישראל, וממקם את הדיון בתוך שאלות של שוויון אזרחי, השתתפות פוליטית והגנה על זכויות.",
  "summaryHe": "המאמר ממקם את הדיון בתוך תחום זכויות, שוויון ומיעוטים, ומראה כיצד סוגיה ממוקדת יכולה להשפיע על מוסדות, זכויות ואמון ציבורי בדמוקרטיה.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "אשר סוזין, איילת בנאי, לי כהנר",
  "paperTitle": "Ultra-Orthodox Women's Reaction to the \"Society of Learners\" Model as a Motive among Israel's Ultra-Orthodox Feminists",
  "authorsHtml": "<a href=\"https://www.ono.ac.il/lecturer/dr-asher-suzin/\" target=\"_blank\" rel=\"noopener noreferrer\">Asher Suzin</a>; <a href=\"https://politics.haifa.ac.il/index.php/en/people/faculty/54-ayelet-banai\" target=\"_blank\" rel=\"noopener noreferrer\">Ayelet Banai</a>; <a href=\"https://www.oranim.ac.il/sites/heb/academic-units/social-sciences-faculty/geography-studies/about/staff/pages/lee-cahaner.aspx\" target=\"_blank\" rel=\"noopener noreferrer\">Lee Cahaner</a>",
  "journal": "Israel Studies",
  "dateText": "פורסם: 09.2023",
  "doiUrl": "https://doi.org/10.2979/is.2023.a903077",
  "doiLabel": "https://doi.org/10.2979/is.2023.a903077",
  "authors": [
    {
      "@type": "Person",
      "name": "אשר סוזין",
      "url": "https://www.ono.ac.il/lecturer/dr-asher-suzin/"
    },
    {
      "@type": "Person",
      "name": "איילת בנאי",
      "url": "https://politics.haifa.ac.il/index.php/en/people/faculty/54-ayelet-banai"
    },
    {
      "@type": "Person",
      "name": "לי כהנר",
      "url": "https://www.oranim.ac.il/sites/heb/academic-units/social-sciences-faculty/geography-studies/about/staff/pages/lee-cahaner.aspx"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Asher Suzin",
      "url": "https://www.ono.ac.il/lecturer/dr-asher-suzin/"
    },
    {
      "@type": "Person",
      "name": "Ayelet Banai",
      "url": "https://politics.haifa.ac.il/index.php/en/people/faculty/54-ayelet-banai"
    },
    {
      "@type": "Person",
      "name": "Lee Cahaner",
      "url": "https://www.oranim.ac.il/sites/heb/academic-units/social-sciences-faculty/geography-studies/about/staff/pages/lee-cahaner.aspx"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות ושוויון",
    "דמוקרטיה ליברלית",
    "שלטון החוק",
    "זכויות",
    "מוסדות"
  ],
  "image": {
    "src": "html_qa/democracy_majority_nationalism_equal_citizenship_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על שוויון אזרחי וזכויות מיעוט",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר ממקם את הדיון בתוך תחום זכויות, שוויון ומיעוטים, ומראה כיצד סוגיה ממוקדת יכולה להשפיע על מוסדות, זכויות ואמון ציבורי בדמוקרטיה.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר Ultra-Orthodox Women&#x27;s Reaction to the &quot;Society of Learners&quot; Model as a Motive among Israel&#x27;s Ultra-Orthodox Feminists מוצג כאן דרך השאלה הדמוקרטית המרכזית שעולה ממנו.",
        "המאמר ממקם את הדיון בתוך תחום זכויות, שוויון ומיעוטים, ומראה כיצד סוגיה ממוקדת יכולה להשפיע על מוסדות, זכויות ואמון ציבורי בדמוקרטיה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "תקציר: \"חברת הלומדים\" היא מודל מחקרי מקובל המסביר מאפיינים קהילתיים, דתיים וסוציולוגיים בקרב קהילות ליטאיות ומזרחיות בישראל. במאמר זה נדון בתגובות למודל חברת הלומדים כמניע להקמת התנועה הפמיניסטית החרדית שנוצרה לקראת הבחירות לכנסת 2013, ובוחנים את תפקידה בהקמת התנועה. מחקר זה מוסיף פן חדש לתיאוריה של חברת הלומדים ותומך במחקרים אחרים של נשים מקבוצות מיעוט כסוכניות של שימור ושינוי.",
        "המאמר ממקם את הדיון בתוך תחום זכויות, שוויון ומיעוטים, ומראה כיצד סוגיה ממוקדת יכולה להשפיע על מוסדות, זכויות ואמון ציבורי בדמוקרטיה."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת מחלוקת משפטית או פוליטית לכמה שאלות בדיקות: מי מחזיק כוח, מי מפקח עליו, ואילו זכויות מוגנות בפועל.",
        "המאמר מדגיש שהדמוקרטיה נמדדת לא רק בתוצאה מוסדית אחת אלא במערכת היחסים בין בחירות, ביקורת, זכויות והשתתפות ציבורית.",
        "לכן התמצית קוראת את המקור כתרומה להבנת התנאים שמאפשרים דמוקרטיה ליברלית יציבה."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית צריכה גם הכרעת רוב וגם הגבלות שמונעות מרוב רגעי למחוק זכויות או להשתיק ביקורת.",
        "כאשר מוסדות ביקורת, שוויון אזרחי או אמון ציבורי נחלשים, הפגיעה עשויה להיראות תחילה כמחלוקת מקצועית בלבד.",
        "המאמר עוזר לזהות את המשמעות הדמוקרטית של אותם שינויים לפני שהם מצטברים למשבר רחב יותר."
      ]
    },
    {
      "headingHe": "מה הקורא הישראלי יכול לקחת מכאן?",
      "paragraphsHtml": [
        "הלקח המעשי הוא לבדוק כל שינוי לפי השפעתו על כללי המשחק ולא רק לפי הכותרת הפוליטית שלו.",
        "שאלות של חוקה, מיעוטים, בתי משפט, בחירות וחברה אזרחית קשורות זו לזו: כל אחת מהן משנה את יכולת האזרחים להשתתף כשווים."
      ]
    },
    {
      "headingHe": "מה נושא המאמר?",
      "paragraphsHtml": [
        "המאמר עוסק ב־<strong>מה המאמר מלמד על זכויות ושוויון בישראל?</strong>.",
        "הוא שייך בעיקר לתחום <strong>זכויות, שוויון ומיעוטים</strong>, אבל נוגע גם לשאלות רחבות של כוח, זכויות ואחריות."
      ]
    },
    {
      "headingHe": "למה הוא רלוונטי לישראל?",
      "paragraphsHtml": [
        "המאמר רלוונטי משום שהוויכוחים בישראל על דמוקרטיה אינם מתנהלים רק סביב בחירות, אלא גם סביב מוסדות, זהות, שוויון וביקורת.",
        "הוא מספק שפה שמאפשרת להבין את המחלוקת בלי לצמצם אותה לעמדה מפלגתית אחת."
      ]
    },
    {
      "headingHe": "מהו הסיכון הדמוקרטי המרכזי?",
      "paragraphsHtml": [
        "הסיכון המרכזי הוא שינוי הדרגתי של כללי המשחק כך שקשה יותר לבקר כוח, להגן על זכויות או לאפשר השתתפות שווה.",
        "כאשר השינוי נתפס כטכני בלבד, קל להחמיץ את משמעותו המצטברת."
      ]
    },
    {
      "headingHe": "איך זה קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק מחייב שגם רשויות חזקות יפעלו לפי כללים, ביקורת וסמכות מוגבלת.",
        "המאמר מראה מדוע מוסדות משפטיים וציבוריים אינם רק מנגנונים טכניים אלא תנאי לאמון דמוקרטי."
      ]
    },
    {
      "headingHe": "איך זה קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר נושא המאמר אינו מוגדר כמאמר על מיעוטים, ההשלכות שלו עשויות לקבוע מי נהנה מהגנה שווה ומי נשאר תלוי ברצון הרוב.",
        "זכויות מיעוטים הן מבחן מרכזי לדמוקרטיה ליברלית משום שהן בודקות את גבולות כוחו של הרוב."
      ]
    },
    {
      "headingHe": "האם זה מאמר משפטי או פוליטי?",
      "paragraphsHtml": [
        "ברבים מן המקרים ההבחנה עצמה מטעה: משפט, מוסדות ופוליטיקה פועלים יחד.",
        "המאמר חשוב דווקא משום שהוא מראה איך הכרעות משפטיות ומוסדיות משנות תמריצים פוליטיים ודפוסי השתתפות."
      ]
    },
    {
      "headingHe": "מה הקשר לאמון הציבור?",
      "paragraphsHtml": [
        "אמון ציבורי תלוי בתחושה שהכללים הוגנים, שהמוסדות אינם כלי של צד אחד, ושיש דרך אמינה לתקן עוולות.",
        "כאשר האמון נשחק, גם הכרעות חוקיות פורמלית עלולות להיתפס כבלתי לגיטימיות."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "המאמר ממקם את הדיון בתוך תחום זכויות, שוויון ומיעוטים, ומראה כיצד סוגיה ממוקדת יכולה להשפיע על מוסדות, זכויות ואמון ציבורי בדמוקרטיה.",
        "השורה התחתונה היא שמוסדות דמוקרטיים נבחנים ביכולתם להגן על מחלוקת חופשית, זכויות שוות ואחריות של בעלי כוח."
      ]
    },
    {
      "headingHe": "למי כדאי לקרוא את המקור?",
      "paragraphsHtml": [
        "המקור מתאים לקוראים שמתעניינים במשפט חוקתי, מדע המדינה, זכויות, בחירות, מיעוטים או מוסדות ציבור.",
        "הוא שימושי במיוחד למי שרוצה להבין כיצד מחקר אקדמי מאיר את המשבר הדמוקרטי בישראל מזווית רחבה ומדויקת יותר."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה המאמר מלמד על זכויות ושוויון בישראל?",
  "permalink": "/democracy_ultra_orthodox_womens_reaction_to_the_society_of_learners_model_as_a_mot_summary_he.html",
  "paper_url": "democracy_ultra_orthodox_womens_reaction_to_the_society_of_learners_model_as_a_mot_summary_he.html",
  "sortKey": 202605210069
}
---
