---
{
  "version": 1,
  "file": "democracy_fighting_terrorism_political_arena_summary_he.html",
  "slug": "democracy_fighting_terrorism_political_arena_summary_he",
  "permalink": "/democracy_fighting_terrorism_political_arena_summary_he.html",
  "paper_url": "democracy_fighting_terrorism_political_arena_summary_he.html",
  "titleHe": "פסילת מפלגות בשל תמיכה בטרור בוחנת את גבולות הדמוקרטיה המתגוננת",
  "subtitleHe": "",
  "descriptionHe": "המאמר של Suzie Navot מנתח את המסגרת המשפטית לפסילת מפלגות ומועמדים בישראל בשל תמיכה בטרור או אלימות. הוא מציב את הדילמה של דמוקרטיה מתגוננת: כיצד להגן על ההליך הדמוקרטי בלי לצמצם יתר על המידה השתתפות פוליטית.",
  "summaryHe": "Navot בוחנת את התיקון האנטי־טרוריסטי בדיני הבחירות בישראל ומשווה אותו גם לחקיקה הספרדית משנת 2002. המאמר מדגיש שהמאבק בטרור בזירה הפוליטית מחייב קווים ברורים, ראיות אמינות ופיקוח שיפוטי כדי שלא להפוך הגנה על הדמוקרטיה לפגיעה בה.",
  "authorsCardHe": "Suzie Navot",
  "paperTitle": "Fighting Terrorism in the Political Arena",
  "authorsHtml": "<a href=\"https://en.idi.org.il/experts/38336\" target=\"_blank\" rel=\"noopener noreferrer\">Suzie Navot</a>",
  "journal": "Party Politics, 14(6), 745-762",
  "dateText": "פורסם אונליין: 01.11.2008",
  "sourcePublishedDate": "2008-11-01",
  "doiUrl": "https://doi.org/10.1177/1354068808093409",
  "doiLabel": "https://doi.org/10.1177/1354068808093409",
  "authors": [
    {
      "@type": "Person",
      "name": "Suzie Navot",
      "url": "https://en.idi.org.il/experts/38336"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Suzie Navot",
      "url": "https://en.idi.org.il/experts/38336"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "public-opinion-polarization-violence",
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "דמוקרטיה מתגוננת",
    "פסילת מפלגות",
    "טרור",
    "בחירות",
    "שלטון החוק",
    "ישראל",
    "אלימות פוליטית"
  ],
  "image": {
    "src": "html_qa/democracy_fighting_terrorism_political_arena_summary_he.jpg",
    "version": "2026-06-04-nightly",
    "altHe": "ועדה משפטית לבחירות דנה בתיקי ראיות ובפתקי הצבעה אנונימיים ללא סמלי מפלגות",
    "fitness": "high"
  },
  "datePublished": "2026-06-04",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מציב את שאלת הגבול: מתי הגנה על בחירות מפני אלימות ותמיכה בטרור היא תנאי לדמוקרטיה, ומתי היא עלולה להפוך לכלי שמצמצם ייצוג פוליטי לגיטימי.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>דמוקרטיה מתגוננת</strong> היא תפיסה שלפיה דמוקרטיה רשאית להציב גבולות לכוחות שפועלים להריסתה מבפנים. במאמר, המושג משמש לניתוח פסילת רשימות ומועמדים כאשר הטענה היא שהם תומכים בטרור או באלימות פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: דיונים אחרונים בספרד, גרמניה, טורקיה וישראל מספקים כמה דוגמאות לכך. העילות החדשות לאיסור מפלגות נדונות גם בפרספקטיבה השוואתית, מול החוק הספרדי החדש מ-2002. המאמר חושף את הדילמות של ישראל, דמוקרטיה 'מתגוננת', בניסיונה להתמודד עם תמיכה בטרור על ידי הוצאת מפלגות פוליטיות מחוץ לחוק.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: Navot בוחנת את התיקון האנטי־טרוריסטי בדיני הבחירות בישראל ומשווה אותו גם לחקיקה הספרדית משנת 2002. המאמר מדגיש שהמאבק בטרור בזירה הפוליטית מחייב קווים ברורים, ראיות אמינות ופיקוח שיפוטי כדי שלא להפוך הגנה על הדמוקרטיה לפגיעה בה."
      ]
    },
    {
      "headingHe": "מהי הדילמה המרכזית בפסילת מפלגות?",
      "paragraphsHtml": [
        "הדילמה היא שמפלגות הן כלי מרכזי להשתתפות וייצוג בדמוקרטיה. פסילה עשויה להגן על ההליך מפני אלימות, אך היא גם עלולה לשלול מן הבוחרים אפשרות לבחור נציגים ולדחוק קונפליקט פוליטי אל מחוץ לזירה החוקית."
      ]
    },
    {
      "headingHe": "מה מוסיף התיקון האנטי־טרוריסטי בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד בעילה שמאפשרת לפסול רשימות או מועמדים בשל תמיכה במעשי טרור ובשימוש באלימות. עילה כזו מרחיבה את ההגנה על הדמוקרטיה, אך היא מחייבת פרשנות זהירה כדי שלא תבלבל בין עמדה פוליטית חריפה לבין תמיכה באלימות."
      ]
    },
    {
      "headingHe": "למה השוואה לספרד חשובה כאן?",
      "paragraphsHtml": [
        "המאמר משווה את ישראל גם לחקיקה הספרדית משנת 2002 נגד מפלגות הקשורות לטרור. ההשוואה מראה שדמוקרטיות נוספות מתמודדות עם אותה בעיה, אך כל אחת מעצבת איזון שונה בין ביטחון, חופש התאגדות ותחרות פוליטית."
      ]
    },
    {
      "headingHe": "מה תפקידו של בית המשפט בסוגיה?",
      "paragraphsHtml": [
        "בית המשפט נדרש לוודא שהפסילה נשענת על ראיות ועל עילות מוגדרות, ולא על אי־נוחות פוליטית בלבד. פיקוח שיפוטי חשוב במיוחד משום שהחלטה על פסילה מתקבלת בזירה שבה לשחקנים פוליטיים יש אינטרס ישיר בתוצאה."
      ]
    },
    {
      "headingHe": "איך המאמר מבחין בין ביטוי פוליטי לבין תמיכה באלימות?",
      "paragraphsHtml": [
        "המאמר מדגיש שהדמוקרטיה צריכה לסבול מחלוקת עמוקה, ביקורת חריפה ותפיסות לא פופולריות. הגבול נעשה מוצדק יותר כאשר יש תמיכה ממשית בטרור או בשימוש באלימות, ולא רק עמדה אידיאולוגית שמקוממת את הרוב."
      ]
    },
    {
      "headingHe": "מדוע פסילה יכולה לפגוע בלגיטימציה של הבחירות?",
      "paragraphsHtml": [
        "בחירות מקבלות לגיטימציה לא רק מן ההצבעה אלא גם מן התחושה שכללים חלים באופן שווה. אם פסילה נתפסת ככלי מפלגתי, היא עלולה לפגוע באמון הציבור ולהחליש את הטענה שההליך פתוח ותחרותי."
      ]
    },
    {
      "headingHe": "מהו הסיכון ההפוך של אי־פסילה?",
      "paragraphsHtml": [
        "אי־פסילה מוחלטת עלולה לאפשר לשחקנים שמקדמים אלימות להשתמש במוסדות הדמוקרטיים כדי לערער אותם. לכן המאמר אינו מציע אדישות לסכנה, אלא איזון שמבחין בין הגנה הכרחית לבין הרחבת יתר של איסורים."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לאלימות פוליטית?",
      "paragraphsHtml": [
        "המאמר עוסק במפגש בין אלימות לבין זירה מפלגתית. כאשר תמיכה באלימות נכנסת לתחרות בחירות, השאלה אינה רק ביטחונית אלא גם מוסדית: האם התחרות עדיין מתנהלת לפי כללים דמוקרטיים."
      ]
    },
    {
      "headingHe": "מהי המסקנה המעשית למדינה דמוקרטית?",
      "paragraphsHtml": [
        "הגנה על דמוקרטיה מפני טרור מחייבת כללים צרים, שקופים וברי ביקורת. ככל שהעילה חמורה יותר, כך נדרש בסיס ראייתי ברור יותר כדי לשמר גם ביטחון וגם חירות פוליטית."
      ]
    }
  ],
  "layout": "paper",
  "title": "פסילת מפלגות בשל תמיכה בטרור בוחנת את גבולות הדמוקרטיה המתגוננת",
  "sortKey": 202606040008
}
---
