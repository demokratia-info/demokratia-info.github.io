---
{
  "version": 1,
  "file": "democracy_the_impact_of_immigrants_characteristics_on_anti_immigrant_sentiment_among_the_jewish_majority_and_the_arab_summary_he.html",
  "slug": "democracy_the_impact_of_immigrants_characteristics_on_anti_immigrant_sentiment_among_the_jewish_majority_and_the_arab_summary_he",
  "order": 255,
  "titleHe": "אילו מהגרים מעוררים התנגדות בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן כיצד מאפייני מהגרים משפיעים על עמדות נגד הגירה בקרב יהודים וערבים בישראל. הדיון ממקם את המקרה בתוך שאלות רחבות של מוסדות, זכויות, ייצוג ושוויון דמוקרטי. הסיכום מדגיש את המקרה, המושגים וההשלכות המוסדיות של המאמר בלי להפוך אותו להצהרה כללית על דמוקרטיה.",
  "summaryHe": "המחקר משתמש בניסוי סקר פקטוריאלי עם תיאורים משתנים של מהגרים היפותטיים. הוא מראה שדת וזהות קבוצתית משפיעות במיוחד: יהודים נטו להעדיף מהגרים יהודים, ואילו ערבים נטו להיות פתוחים יותר למהגרים לא-יהודים, במיוחד מוסלמים. בכך המחקר מראה שעמדות כלפי הגירה בישראל מעוצבות לא רק דרך חשש כלכלי, אלא גם דרך גבולות לאומיים, דתיים וקבוצתיים. המאמר בוחן עמדות כלפי מהגרים באמצעות ניסוי סקר פקטוריאלי. המחקר משווה בין יהודים, כקבוצת רוב, לבין ערבים, כמיעוט אזרחי בישראל. התרומה של המקור היא בהעברת הדיון מן הכותרת הכללית אל מנגנונים קונקרטיים: מי מגדיר את הבעיה, אילו מוסדות מקבלים סמכות, כיצד קבוצות שונות מושפעות מן ההכרעה, ומה נחשף על היחסים בין כוח, זכויות ולגיטימציה. לכן הקריאה בו חשובה לא רק להבנת המקרה הנדון, אלא גם להבנת הדרך שבה דמוקרטיה פועלת בפועל דרך חקיקה, פסיקה, חינוך, מדיניות ציבורית או פעולה אזרחית. כך המחקר מציב את ההגירה כמראה של יחסי שייכות בישראל: מאפייני המהגר מפעילים אצל קבוצות שונות דמיונות שונים של איום, קרבה וגבול פוליטי.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Moshe Semyonov, Rebeca Raijman, Anastasia Gorodzeisky, Thomas Hinz",
  "paperTitle": "The impact of immigrants’ characteristics on anti-immigrant sentiment among the Jewish majority and the Arab minority in Israel",
  "authorsHtml": "<a href=\"https://people1.socsci.tau.ac.il/mu/semyonov/\" target=\"_blank\" rel=\"noopener noreferrer\">Moshe Semyonov</a>, <a href=\"https://cris.haifa.ac.il/en/persons/rebeca-raijman/\" target=\"_blank\" rel=\"noopener noreferrer\">Rebeca Raijman</a>, <a href=\"https://orcid.org/0000-0002-9244-8501\" target=\"_blank\" rel=\"noopener noreferrer\">Anastasia Gorodzeisky</a>, <a href=\"https://orcid.org/0000-0002-8940-3003\" target=\"_blank\" rel=\"noopener noreferrer\">Thomas Hinz</a>",
  "journal": "Journal of Ethnic and Migration Studies",
  "dateText": "פורסם: 2023",
  "doiUrl": "https://doi.org/10.1080/1369183x.2023.2172716",
  "doiLabel": "https://doi.org/10.1080/1369183x.2023.2172716",
  "authors": [
    {
      "@type": "Person",
      "name": "Moshe Semyonov",
      "url": "https://people1.socsci.tau.ac.il/mu/semyonov/"
    },
    {
      "@type": "Person",
      "name": "Rebeca Raijman",
      "url": "https://cris.haifa.ac.il/en/persons/rebeca-raijman/"
    },
    {
      "@type": "Person",
      "name": "Anastasia Gorodzeisky",
      "url": "https://orcid.org/0000-0002-9244-8501"
    },
    {
      "@type": "Person",
      "name": "Thomas Hinz",
      "url": "https://orcid.org/0000-0002-8940-3003"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Moshe Semyonov",
      "url": "https://people1.socsci.tau.ac.il/mu/semyonov/"
    },
    {
      "@type": "Person",
      "name": "Rebeca Raijman",
      "url": "https://cris.haifa.ac.il/en/persons/rebeca-raijman/"
    },
    {
      "@type": "Person",
      "name": "Anastasia Gorodzeisky",
      "url": "https://orcid.org/0000-0002-9244-8501"
    },
    {
      "@type": "Person",
      "name": "Thomas Hinz",
      "url": "https://orcid.org/0000-0002-8940-3003"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה",
    "אזרחים ערבים",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_745_minorities_democracy_the_impact_of_immigrants_characteristics_on_anti_immigrant_sentiment_among_the_j.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על אזרחות, שוויון ומעמדן של קבוצות מיעוט בישראל",
    "fitness": "high",
    "license": "Public domain",
    "creator": "Andreas Ramer",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:PikiWiki_Israel_47088_Nazareth_market.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/9/9c/PikiWiki_Israel_47088_Nazareth_market.jpg",
    "fileTitle": "PikiWiki Israel 47088 Nazareth market.jpg",
    "sourceDimensions": "5181x3475",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:minorities",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://commons.wikimedia.org/wiki/Commons:Copyright_tags#Public_domain"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המחקר מראה שעמדות כלפי הגירה בישראל מעוצבות בעיקר סביב דת וזהות קבוצתית, ולא רק סביב חשש כלכלי.",
  "sections": [
    {
      "headingHe": "איך ניסוי וינייטות חושף גבולות קבלה של מהגרים?",
      "paragraphsHtml": [
        "המאמר בוחן עמדות כלפי מהגרים באמצעות ניסוי סקר פקטוריאלי.",
        "המחקר משתמש בניסוי סקר פקטוריאלי עם תיאורים משתנים של מהגרים היפותטיים."
      ]
    },
    {
      "headingHe": "למה דת המהגר משנה ליהודים ולערבים אחרת?",
      "paragraphsHtml": [
        "המשתתפים נחשפו לתיאורים של מהגרים היפותטיים שמאפייניהם השתנו, וכך ניתן היה לבדוק אילו תכונות משפיעות על התנגדות או קבלה.",
        "הוא מראה שדת וזהות קבוצתית משפיעות במיוחד: יהודים נטו להעדיף מהגרים יהודים, ואילו ערבים נטו להיות פתוחים יותר למהגרים לא-יהודים, במיוחד מוסלמים."
      ]
    },
    {
      "headingHe": "מה מעבר להסבר הכלכלי של התנגדות להגירה?",
      "paragraphsHtml": [
        "המחקר משווה בין יהודים, כקבוצת רוב, לבין ערבים, כמיעוט אזרחי בישראל.",
        "בכך המחקר מראה שעמדות כלפי הגירה בישראל מעוצבות לא רק דרך חשש כלכלי, אלא גם דרך גבולות לאומיים, דתיים וקבוצתיים."
      ]
    },
    {
      "headingHe": "איך יחסי רוב-מיעוט מעצבים מדיניות הגירה?",
      "paragraphsHtml": [
        "ההשוואה מאפשרת לראות כיצד מיקום קבוצתי משנה את האופן שבו אנשים מעריכים מהגרים.",
        "המאמר בוחן עמדות כלפי מהגרים באמצעות ניסוי סקר פקטוריאלי."
      ]
    },
    {
      "headingHe": "מה המחקר מגלה על שייכות לקהילה הפוליטית?",
      "paragraphsHtml": [
        "הממצא המרכזי הוא שלדת וזהות קבוצתית יש משקל חזק.",
        "המחקר משווה בין יהודים, כקבוצת רוב, לבין ערבים, כמיעוט אזרחי בישראל.",
        "הוא מראה שדת וזהות קבוצתית משפיעות במיוחד: יהודים נטו להעדיף מהגרים יהודים, ואילו ערבים נטו להיות פתוחים יותר למהגרים לא-יהודים, במיוחד מוסלמים."
      ]
    }
  ],
  "layout": "paper",
  "title": "אילו מהגרים מעוררים התנגדות בישראל?",
  "permalink": "/democracy_the_impact_of_immigrants_characteristics_on_anti_immigrant_sentiment_among_the_jewish_majority_and_the_arab_summary_he.html",
  "paper_url": "democracy_the_impact_of_immigrants_characteristics_on_anti_immigrant_sentiment_among_the_jewish_majority_and_the_arab_summary_he.html",
  "sortKey": 202605210255
}
---
