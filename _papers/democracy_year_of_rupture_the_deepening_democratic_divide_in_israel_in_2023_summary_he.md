---
{
  "version": 1,
  "file": "democracy_year_of_rupture_the_deepening_democratic_divide_in_israel_in_2023_summary_he.html",
  "slug": "democracy_year_of_rupture_the_deepening_democratic_divide_in_israel_in_2023_summary_he",
  "order": 78,
  "titleHe": "כיצד העמקת השבר הדמוקרטי בישראל בשנת 2023 קשור לסיכון לנסיגה דמוקרטית?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית העמקת השבר הדמוקרטי בישראל בשנת 2023, וממקם את הדיון בתוך שאלות של נסיגה דמוקרטית ופופוליזם, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית העמקת השבר הדמוקרטי בישראל בשנת 2023 דרך עדשה של נסיגה דמוקרטית ופופוליזם.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Yuval Feinstein",
  "paperTitle": "Year of Rupture: The Deepening Democratic Divide in Israel in 2023",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-6970-6317\" target=\"_blank\" rel=\"noopener noreferrer\">Yuval Feinstein</a>",
  "journal": "Center for Open Science",
  "dateText": "פורסם: 2025",
  "sourcePublishedDate": "2025-09-01",
  "doiUrl": "https://doi.org/10.31235/osf.io/h28va_v1",
  "doiLabel": "https://doi.org/10.31235/osf.io/h28va_v1",
  "authors": [
    {
      "@type": "Person",
      "name": "Yuval Feinstein",
      "url": "https://orcid.org/0000-0001-6970-6317"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yuval Feinstein",
      "url": "https://orcid.org/0000-0001-6970-6317"
    }
  ],
  "topics": [
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "משבר חוקתי",
    "איזונים ובלמים",
    "ריכוז כוח",
    "העמקת השבר הדמוקרטי בישראל בשנת 2023",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_year_of_rupture_the_deepening_democratic_divide_in_israel_in_2023_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על העמקת השבר הדמוקרטי בישראל בשנת 2023",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית העמקת השבר הדמוקרטי בישראל בשנת 2023 דרך עדשה של נסיגה דמוקרטית ופופוליזם.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית העמקת השבר הדמוקרטי בישראל בשנת 2023, וממקם את הסוגיה בתוך הדיון הרחב על נסיגה דמוקרטית ופופוליזם.",
        "נקודת המוצא של התמצית היא השאלה כיצד שחיקת נורמות דמוקרטיות, ריכוז כוח ופוליטיקה פופוליסטית משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "משבר הרפורמה השיפוטית בישראל בשנת 2023 העצימה את הפער הדמוקרטי הגובר. מאמר זה משתמש בעדשה תרבותית-סוציולוגית כדי לפרש את הקונפליקט כחלק ממאבק רחב יותר על זהות לאומית. בהסתמך על נתוני סקר פאנלים מקוריים וניתוח מעמדות סמויים, הוא מזהה חמישה אשכולות אמונות לאומניות מובהקות בקרב ישראלים יהודים, אשר בנו תגובות שונות ליוזמת הרפורמה.",
        "המאמר בוחן את סוגיית העמקת השבר הדמוקרטי בישראל בשנת 2023 דרך עדשה של נסיגה דמוקרטית ופופוליזם."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת העמקת השבר הדמוקרטי בישראל בשנת 2023 לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד נסיגה דמוקרטית ופופוליזם מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר העמקת השבר הדמוקרטי בישראל בשנת 2023 משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "כיצד העמקת השבר הדמוקרטי בישראל בשנת 2023 קשור לסיכון לנסיגה דמוקרטית?",
  "permalink": "/democracy_year_of_rupture_the_deepening_democratic_divide_in_israel_in_2023_summary_he.html",
  "paper_url": "democracy_year_of_rupture_the_deepening_democratic_divide_in_israel_in_2023_summary_he.html",
  "sortKey": 202605210215
}
---
