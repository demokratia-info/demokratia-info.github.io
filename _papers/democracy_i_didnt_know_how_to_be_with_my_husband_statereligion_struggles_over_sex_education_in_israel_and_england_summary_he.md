---
{
  "version": 1,
  "file": "democracy_i_didnt_know_how_to_be_with_my_husband_statereligion_struggles_over_sex_education_in_israel_and_england_summary_he.html",
  "slug": "democracy_i_didnt_know_how_to_be_with_my_husband_statereligion_struggles_over_sex_education_in_israel_and_england_summary_he",
  "order": 263,
  "titleHe": "מאבקי דת ומדינה סביב חינוך מיני בישראל ובאנגליה",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בדת, זהות יהודית וזכויות אזרחיות בישראל, וממקם את הדיון בתוך שאלות של שוויון אזרחי, זכויות מיעוטים, אזרחות, הכלה והדרה פוליטית. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את דת, זהות יהודית וזכויות אזרחיות בישראל דרך עדשה של שוויון אזרחי, זכויות מיעוטים, אזרחות, הכלה והדרה פוליטית.",
  "authorsCardHe": "Lea Taragin‐Zeller, Ben Kasstan",
  "paperTitle": "‘I Didn’t Know How to Be with My Husband’: State‐Religion Struggles over Sex Education in Israel and England",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-1886-4080\" target=\"_blank\" rel=\"noopener noreferrer\">Lea Taragin‐Zeller</a>, <a href=\"https://orcid.org/0000-0002-9020-2954\" target=\"_blank\" rel=\"noopener noreferrer\">Ben Kasstan</a>",
  "journal": "Anthropology & Education Quarterly",
  "dateText": "פורסם: 2020",
  "doiUrl": "https://doi.org/10.1111/aeq.12358",
  "doiLabel": "https://doi.org/10.1111/aeq.12358",
  "authors": [
    {
      "@type": "Person",
      "name": "Lea Taragin‐Zeller",
      "url": "https://orcid.org/0000-0002-1886-4080"
    },
    {
      "@type": "Person",
      "name": "Ben Kasstan",
      "url": "https://orcid.org/0000-0002-9020-2954"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Lea Taragin‐Zeller",
      "url": "https://orcid.org/0000-0002-1886-4080"
    },
    {
      "@type": "Person",
      "name": "Ben Kasstan",
      "url": "https://orcid.org/0000-0002-9020-2954"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה",
    "דת ומדינה",
    "חינוך אזרחי",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_i_didnt_know_how_to_be_with_my_husband_statereligion_struggles_over_sex_education_in_israel_and_england_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על דת, זהות יהודית וזכויות אזרחיות בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את דת, זהות יהודית וזכויות אזרחיות בישראל ומראה מדוע הדיון קשור לשוויון אזרחי, זכויות מיעוטים, אזרחות, הכלה והדרה פוליטית. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את דת, זהות יהודית וזכויות אזרחיות בישראל, וממקם את הסוגיה בתוך הדיון הרחב על שוויון אזרחי, זכויות מיעוטים, אזרחות, הכלה והדרה פוליטית.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: חינוך מיני מהווה דילמה מרכזית ביחסי מדינה-מיעוט, המשקפת התנגשות בין זכויות יסוד לחינוך וחופש דת. באתנוגרפיה השוואתית זו של חינוך מיני בלתי פורמלי בקרב יהודים חרדים (חרדים) בישראל ובאנגליה, אנו מסגרות את ההבדל הקריטי בין מודלים \"מתאימים לגיל\" ו\"שלב החיים\" (נישואין ולידה) של חינוך מיני. בהמשגה של הגישות המתחרות הללו כמחלוקות על \"אחריות ידע\", אנו קוראים להבנה ספציפית יותר להקשר של האופן שבו נשקפת אחריות חינוכית באוכלוסיות מגוונות יותר ויותר.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת דת, זהות יהודית וזכויות אזרחיות בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד שוויון אזרחי, זכויות מיעוטים, אזרחות, הכלה והדרה פוליטית מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בדת, זהות יהודית וזכויות אזרחיות בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את דת, זהות יהודית וזכויות אזרחיות בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>דת, זהות יהודית וזכויות אזרחיות בישראל</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בדת, זהות יהודית וזכויות אזרחיות בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "מאבקי דת ומדינה סביב חינוך מיני בישראל ובאנגליה",
  "permalink": "/democracy_i_didnt_know_how_to_be_with_my_husband_statereligion_struggles_over_sex_education_in_israel_and_england_summary_he.html",
  "paper_url": "democracy_i_didnt_know_how_to_be_with_my_husband_statereligion_struggles_over_sex_education_in_israel_and_england_summary_he.html",
  "sortKey": 202605210263
}
---
