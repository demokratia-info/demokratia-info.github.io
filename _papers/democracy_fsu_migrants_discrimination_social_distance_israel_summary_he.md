---
{
  "version": 1,
  "file": "democracy_fsu_migrants_discrimination_social_distance_israel_summary_he.html",
  "slug": "democracy_fsu_migrants_discrimination_social_distance_israel_summary_he",
  "titleHe": "מהגרי FSU נוצרים חושפים את גבולות השייכות האזרחית בישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן מהגרים מחבר המדינות לשעבר שהגיעו לישראל לאחר 1989 ומבקשים לשמור על זהותם הנוצרית. הוא מתמקד בתפיסות אפליה מוסדית ובאופן שבו דת, מוצא וגבולות חברתיים מעצבים שייכות אזרחית.",
  "summaryHe": "Raijman ו־Pinsky מנתחות את חוויותיהם של מהגרים נוצרים מחבר המדינות לשעבר בישראל. המחקר מצא כי דת נעשית סמן מרכזי של זהות ושייכות, וכי מוצא לאומי משותף אינו מבטיח סולידריות או יחסים חברתיים קרובים בין מהגרים.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Rebeca Raijman, Janina Pinsky",
  "paperTitle": "‘Non-Jewish and Christian’: perceived discrimination and social distance among FSU migrants in Israel",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/rebeca-raijman/\" target=\"_blank\" rel=\"noopener noreferrer\">Rebeca Raijman</a>; Janina Pinsky",
  "journal": "Israel Affairs, 17(1), 125-141",
  "dateText": "פורסם: ינואר 2011",
  "doiUrl": "https://doi.org/10.1080/13537121.2011.522074",
  "doiLabel": "https://doi.org/10.1080/13537121.2011.522074",
  "authors": [
    {
      "@type": "Person",
      "name": "Rebeca Raijman",
      "url": "https://cris.haifa.ac.il/en/persons/rebeca-raijman/"
    },
    {
      "@type": "Person",
      "name": "Janina Pinsky"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Rebeca Raijman",
      "url": "https://cris.haifa.ac.il/en/persons/rebeca-raijman/"
    },
    {
      "@type": "Person",
      "name": "Janina Pinsky"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "democratic-backsliding-populism",
    "rights-equality-minorities"
  ],
  "keywords": [
    "מהגרים מחבר המדינות",
    "אפליה נתפסת",
    "מרחק חברתי",
    "דת ולאומיות",
    "שוויון אזרחי",
    "ישראל",
    "זכויות מיעוטים"
  ],
  "image": {
    "src": "assets/article-images/real_photo_262_court_democracy_fsu_migrants_discrimination_social_distance_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור למהגרי FSU נוצרים חושפים את גבולות השייכות האזרחית בישראל",
    "fitness": "high",
    "license": "Public domain",
    "creator": "Alexey Mokeev",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Seat_of_the_Universal_House_of_Justice.JPG",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/5/5f/Seat_of_the_Universal_House_of_Justice.JPG",
    "fileTitle": "Seat of the Universal House of Justice.JPG",
    "sourceDimensions": "2592x1944",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:court",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://commons.wikimedia.org/wiki/Commons:Copyright_tags#Public_domain"
  },
  "datePublished": "2026-06-02",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כי בקרב מהגרי FSU בישראל, זהות נוצרית יכולה לחשוף גבולות שייכות שאזרחות או מוצא משותף אינם מעלימים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>מרחק חברתי</strong> הוא מדד לדרגת הקרבה או הריחוק שקבוצה אחת מוכנה לקיים כלפי קבוצה אחרת. במאמר הוא עוזר להבין כיצד גבולות חברתיים נשמרים גם כאשר קבוצות חיות באותו מרחב אזרחי ומשפטי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתמקד בקבוצה מובחנת של מהגרים מחבר המדינות לשעבר שהגיעו לישראל לאחר 1989 וממשיכים לזהות עצמם כנוצרים במדינה בעלת אופי יהודי מוצהר. מוקד המחקר הוא תפיסותיהם לגבי אפליה מצד המדינה ומוסדותיה כלפי מהגרים שאינם יהודים.",
        "המחקר מצא כי מהגרים נוצרים מערערים על ההגדרה ההגמונית של מיהו יהודי, מתנגדים למשטר קליטה אתנו־לאומי שמפלה אזרחים לא־יהודים, ומנסחים תביעות לשוויון זכויות. הדת מופיעה בניתוח כסמן החשוב ביותר לבניית זהות בתוך הקבוצה הנחקרת.",
        "המאמר מראה שמוצא משותף מחבר המדינות לשעבר אינו מבטיח קרבה חברתית או סולידריות בין מהגרים. גם בתוך אוכלוסיית הגירה רחבה, גבולות דתיים ואתנו־תרבותיים ממשיכים לעצב מרחק חברתי, יחסים יומיומיים ותפיסות של שייכות."
      ]
    },
    {
      "headingHe": "איזו קבוצה עומדת במרכז המאמר?",
      "paragraphsHtml": [
        "המאמר מתמקד במהגרים מחבר המדינות לשעבר בישראל המזוהים כנוצרים ולא יהודים. הבחירה בקבוצה זו חשובה משום שהיא מערערת את ההנחה שכל יוצאי FSU משתלבים באותו אופן בתוך הקטגוריה הרחבה של עלייה."
      ]
    },
    {
      "headingHe": "מהי שאלת השוויון שהמאמר מעלה?",
      "paragraphsHtml": [
        "השאלה היא האם מעמד אזרחי או זיקה משפחתית למדינה מספיקים כדי להבטיח שייכות חברתית שווה. המאמר מראה שהגבול בין יהודי ללא־יהודי ממשיך להשפיע על חוויות אפליה ועל קשרים חברתיים."
      ]
    },
    {
      "headingHe": "איך דת הופכת לסמן של גבול אזרחי?",
      "paragraphsHtml": [
        "דת אינה מופיעה רק כאמונה פרטית אלא כסימן ציבורי לשייכות או זרות. כאשר נוצרים יוצאי FSU נתפסים כלא־יהודים, הם עלולים להיתקל בספק לגבי מקומם בתוך קולקטיב שמגדיר עצמו גם במונחים לאומיים וגם במונחים דתיים."
      ]
    },
    {
      "headingHe": "מה ההבדל בין אפליה משפטית לאפליה נתפסת?",
      "paragraphsHtml": [
        "אפליה משפטית עוסקת בכללים רשמיים, ואפליה נתפסת עוסקת באופן שבו אנשים חווים יחס יומיומי. דמוקרטיה ליברלית צריכה להתייחס לשני הממדים, משום שחוויית הדרה יכולה לפגוע באמון ובשייכות גם כאשר הזכויות הפורמליות קיימות."
      ]
    },
    {
      "headingHe": "מדוע המאמר חשוב לדיון על חוק השבות?",
      "paragraphsHtml": [
        "המאמר אינו רק דיון הגירה טכני אלא מבט על תוצאות חברתיות של שער כניסה לאומי ייחודי. הוא מאפשר לשאול מה קורה לאחר הכניסה למדינה, כאשר אנשים נדרשים להוכיח שייכות מעבר למעמד החוקי שלהם."
      ]
    },
    {
      "headingHe": "מה הקשר בין מרחק חברתי לבין זכויות מיעוטים?",
      "paragraphsHtml": [
        "מרחק חברתי יכול לצמצם בפועל את יכולת ההשתתפות של קבוצה, גם בלי שלילת זכויות רשמית. אם קשרי שכנות, עבודה ונישואים נמדדים דרך חשד או ריחוק, השוויון האזרחי נעשה חלש יותר."
      ]
    },
    {
      "headingHe": "איך המאמר מתחבר לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית מבקשת להגן על יחידים וקבוצות גם כאשר הם אינם מתאימים לזהות הרוב. המאמר מדגיש שהגנה כזו דורשת להכיר בגבולות החברתיים שמייצרים הדרה ולא להסתפק בהצהרה כללית על שוויון."
      ]
    },
    {
      "headingHe": "מה ניתן ללמוד על מדיניות קליטה?",
      "paragraphsHtml": [
        "מדיניות קליטה צריכה להתייחס לזהות דתית ולאומית כאל גורם המשפיע על השתלבות, ולא כאל פרט שולי. שירותים, חינוך ושיח ציבורי צריכים לצמצם סטיגמה ולתת מקום לזהויות מורכבות."
      ]
    },
    {
      "headingHe": "מדוע חוויות ההדרה אינן אחידות?",
      "paragraphsHtml": [
        "לא נכון להסיק שכל מהגר מקבוצה זו חווה את אותה רמת הדרה או שכל החברה הישראלית מגיבה באותו אופן. התרומה היא זיהוי מנגנוני גבול והדרה, ולא תיאור אחיד של כל יחיד או קהילה."
      ]
    },
    {
      "headingHe": "למה העמוד רלוונטי גם מחוץ לשאלת ההגירה?",
      "paragraphsHtml": [
        "הדיון מלמד כיצד דמוקרטיה מתמודדת עם פער בין אזרחות פורמלית לבין הכרה חברתית. זהו פער שחוזר גם ביחס לקבוצות מיעוט אחרות, ולכן המאמר תורם לשיחה רחבה על שוויון ושייכות."
      ]
    }
  ],
  "layout": "paper",
  "title": "מהגרי FSU נוצרים חושפים את גבולות השייכות האזרחית בישראל",
  "permalink": "/democracy_fsu_migrants_discrimination_social_distance_israel_summary_he.html",
  "paper_url": "democracy_fsu_migrants_discrimination_social_distance_israel_summary_he.html",
  "sortKey": 202606020003
}
---
