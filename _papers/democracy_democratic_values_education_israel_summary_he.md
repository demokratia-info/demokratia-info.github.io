---
{
  "version": 1,
  "file": "democracy_democratic_values_education_israel_summary_he.html",
  "slug": "democracy_democratic_values_education_israel_summary_he",
  "order": 44,
  "titleHe": "איך מחנכים לערכים דמוקרטיים במדינה המצויה בסכסוך?",
  "subtitleHe": "המאמר עוסק בחינוך לדמוקרטיה בישראל ומדגיש את הקושי לטפח ערכים דמוקרטיים, זכויות וסובלנות בתוך מציאות של סכסוך וזהויות מתחרות.",
  "descriptionHe": "המאמר מזכיר שחינוך לדמוקרטיה אינו רק לימוד מוסדות, אלא תהליך של טיפוח סובלנות, זכויות, ביקורתיות והכרה בלגיטימיות של האחר. החשיבות הציבורית של המאמר היא בכך שהוא מדגיש שהדמוקרטיה נשענת על מוסדות, שירות ציבורי וחברה אזרחית מתפקדים; כאשר המרחבים האלה נחלשים, גם ההכרעה האלקטורלית עצמה מאבדת חלק מכוחה המתקן.",
  "summaryHe": "המאמר מזכיר שחינוך לדמוקרטיה אינו רק לימוד מוסדות, אלא תהליך של טיפוח סובלנות, זכויות, ביקורתיות והכרה בלגיטימיות של האחר. החשיבות הציבורית של המאמר היא בכך שהוא מדגיש שהדמוקרטיה נשענת על מוסדות, שירות ציבורי וחברה אזרחית מתפקדים; כאשר המרחבים האלה נחלשים, גם ההכרעה האלקטורלית עצמה מאבדת חלק מכוחה המתקן.",
  "authorsCardHe": "ערן הלפרין ודניאל בר-טל",
  "paperTitle": "Democratic Values and Education for Democracy in the State of Israel",
  "authorsHtml": "<a href=\"https://en.psychology.huji.ac.il/people/eran-halperin-0\" target=\"_blank\" rel=\"noopener noreferrer\">Eran Halperin</a>; <a href=\"https://english.tau.ac.il/profile/daniel\" target=\"_blank\" rel=\"noopener noreferrer\">Daniel Bar-Tal</a>",
  "journal": "Democracy and Security",
  "dateText": "כרך 2(2), דצמבר 2006, עמ׳ 169-200",
  "doiUrl": "https://doi.org/10.1080/17419160600954441",
  "doiLabel": "https://doi.org/10.1080/17419160600954441",
  "authors": [
    {
      "@type": "Person",
      "name": "ערן הלפרין",
      "url": "https://en.psychology.huji.ac.il/people/eran-halperin-0"
    },
    {
      "@type": "Person",
      "name": "דניאל בר-טל",
      "url": "https://english.tau.ac.il/profile/daniel"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Eran Halperin",
      "url": "https://en.psychology.huji.ac.il/people/eran-halperin-0"
    },
    {
      "@type": "Person",
      "name": "Daniel Bar-Tal",
      "url": "https://english.tau.ac.il/profile/daniel"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "rights-equality-minorities",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "חינוך לדמוקרטיה",
    "ערכים דמוקרטיים",
    "ישראל",
    "סובלנות",
    "זכויות",
    "סכסוך"
  ],
  "image": {
    "src": "html_qa/democracy_trade_unions_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על חינוך אזרחי, חברה אזרחית וערכים דמוקרטיים",
    "fitness": "standard"
  },
  "datePublished": "2026-05-20",
  "dateModified": "2026-05-24",
  "lastUpdatedHe": "24 במאי 2026",
  "oneLinerHtml": "המאמר מזכיר שחינוך לדמוקרטיה אינו רק לימוד מוסדות, אלא תהליך של טיפוח סובלנות, זכויות, ביקורתיות והכרה בלגיטימיות של האחר. בדמוקרטיה ליברלית, חינוך לדמוקרטיה נמדד ביכולת להכיר בזכויות גם של יריבים ומיעוטים, ולא רק בהצהרה כללית בעד שלטון הרוב. החשיבות הציבורית של המאמר היא בכך שהוא מדגיש שהדמוקרטיה נשענת על מוסדות, שירות ציבורי וחברה אזרחית מתפקדים; כאשר המרחבים האלה נחלשים, גם ההכרעה האלקטורלית עצמה מאבדת חלק מכוחה המתקן.",
  "sections": [
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מציג את החינוך לדמוקרטיה כתנאי עומק לחוסן ליברלי, ולא רק כתוספת אזרחית למערכת הלימודים.",
        "זהו מאמר ב-Democracy and Security העוסק בערכים דמוקרטיים ובחינוך לדמוקרטיה במדינת ישראל.",
        "במקום להתייחס לדמוקרטיה רק כמערכת בחירות, המאמר בוחן את התנאים שמאפשרים לחברה לשמר אמון, זכויות, אחריות ומחלוקת לגיטימית."
      ]
    },
    {
      "headingHe": "מהי נקודת המבט הליברלית-דמוקרטית?",
      "paragraphsHtml": [
        "בדמוקרטיה ליברלית, חינוך לדמוקרטיה נמדד ביכולת להכיר בזכויות גם של יריבים ומיעוטים, ולא רק בהצהרה כללית בעד שלטון הרוב.",
        "הזווית הליברלית חשובה משום שהיא שואלת לא רק מי שולט, אלא גם מה מגביל את השלטון, איך נשמרת ביקורת, ואיך מוגנות קבוצות או יריבים פוליטיים מפני כוח עודף."
      ]
    },
    {
      "headingHe": "למה המאמר חשוב לקוראי האתר?",
      "paragraphsHtml": [
        "המאמר מתאים לאתר משום שהוא עוסק בתשתית הערכית והחינוכית של חברה דמוקרטית בישראל.",
        "הוא מסייע לתרגם מחקר אקדמי לשאלות ציבוריות מעשיות: איך לזהות שחיקה דמוקרטית, איך למדוד קיטוב, ואיך לחשוב על מוסדות וזכויות בלי להסתפק בסיסמאות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד חינוך לדמוקרטיה משפיע על איכות הדמוקרטיה בישראל ועל היכולת לשמור על אמון, זכויות ומוסדות עצמאיים.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "מה החידוש העיקרי?",
      "paragraphsHtml": [
        "תרומתו היא בחיבור בין פסיכולוגיה פוליטית, סכסוך מתמשך וחינוך אזרחי, ובהדגשת המתח בין זהות לאומית לבין מחויבות לזכויות ולפלורליזם.",
        "החידוש צריך להיבחן מול הטענה והראיות במאמר המקורי, ולא רק לפי הדרך שבה התמצית מנגישה אותו."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "בדמוקרטיה ליברלית, חינוך לדמוקרטיה נמדד ביכולת להכיר בזכויות גם של יריבים ומיעוטים, ולא רק בהצהרה כללית בעד שלטון הרוב.",
        "החשיבות הדמוקרטית נובעת מן הקשר בין מוסדות, זכויות ואחריות ציבורית, אך גבולות הטענה נשארים גבולות המקור."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לישראל?",
      "paragraphsHtml": [
        "הקשר הישראלי נמצא בלב המאמר: זהו מאמר ב-Democracy and Security העוסק בערכים דמוקרטיים ובחינוך לדמוקרטיה במדינת ישראל.",
        "הקשר הישראלי צריך להיקרא דרך המוסדות, הזכויות וההקשרים שהמאמר עצמו מנתח, ולא כהכללה על החברה הישראלית כולה."
      ]
    },
    {
      "headingHe": "איזה מושג כדאי לזכור?",
      "paragraphsHtml": [
        "המושג המרכזי הוא <strong>חינוך לדמוקרטיה</strong>. הוא עוזר לראות כיצד תהליך שנראה פוליטי או משפטי צר משפיע על מערכת היחסים בין אזרחים, מוסדות ושלטון."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על מוסדות?",
      "paragraphsHtml": [
        "המוסדות הדמוקרטיים אינם פועלים בחלל ריק. הם תלויים בנורמות, באמון ציבורי, בהתנהגות מפלגות וביכולת להציב גבולות לכוח פוליטי."
      ]
    },
    {
      "headingHe": "האם המאמר מציע פתרון פשוט?",
      "paragraphsHtml": [
        "לא. התרומה שלו היא בעיקר אנליטית: הוא מחדד את הבעיה ומראה אילו תנאים פוליטיים, חברתיים או משפטיים צריכים להשתנות כדי לשפר את החוסן הדמוקרטי."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין לקרוא את המאמר כהוכחה שכל מחלוקת פוליטית היא סכנה לדמוקרטיה. החשיבות היא בזיהוי התנאים שבהם מחלוקת הופכת לפגיעה באמון, בזכויות או באיזונים מוסדיים."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול שאלות מדויקות יותר: אילו מוסדות נפגעים, אילו זכויות עומדות על הפרק, ואיך חינוך לדמוקרטיה משנה את התנהגות הציבור או השלטון.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך מחנכים לערכים דמוקרטיים במדינה המצויה בסכסוך?",
  "permalink": "/democracy_democratic_values_education_israel_summary_he.html",
  "paper_url": "democracy_democratic_values_education_israel_summary_he.html",
  "sortKey": 202605200008
}
---
