---
{
  "version": 1,
  "file": "democracy_the_democratic_self_defence_of_constitutional_courts_summary_he.html",
  "slug": "democracy_the_democratic_self_defence_of_constitutional_courts_summary_he",
  "order": 409,
  "titleHe": "הגנה עצמית דמוקרטית של בתי משפט חוקתיים",
  "subtitleHe": "",
  "descriptionHe": "תמצית בעברית של מאמר על הגנה עצמית דמוקרטית של בתי משפט חוקתיים, עם דגש על המשמעות לדמוקרטיה ליברלית, לשלטון החוק, לזכויות ולאחריות ציבורית.",
  "summaryHe": "המאמר בוחן את סוגיית הגנה עצמית דמוקרטית של בתי משפט חוקתיים דרך עדשה של חוקתיות, ביקורת על כוח שלטוני ושמירה על שלטון החוק.",
  "authorsCardHe": "Pablo José Castillo Ortiz, Yaniv Roznai",
  "paperTitle": "The Democratic Self-Defence of Constitutional Courts",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-4540-1855\" target=\"_blank\" rel=\"noopener noreferrer\">Pablo José Castillo Ortiz</a>, <a href=\"https://www.runi.ac.il/en/faculty/yroznai\" target=\"_blank\" rel=\"noopener noreferrer\">Yaniv Roznai</a>",
  "journal": "ICL Journal",
  "dateText": "פורסם: 2024",
  "doiUrl": "https://doi.org/10.1515/icl-2024-0001",
  "doiLabel": "https://doi.org/10.1515/icl-2024-0001",
  "authors": [
    {
      "@type": "Person",
      "name": "Pablo José Castillo Ortiz",
      "url": "https://orcid.org/0000-0003-4540-1855"
    },
    {
      "@type": "Person",
      "name": "Yaniv Roznai",
      "url": "https://www.runi.ac.il/en/faculty/yroznai"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Pablo José Castillo Ortiz",
      "url": "https://orcid.org/0000-0003-4540-1855"
    },
    {
      "@type": "Person",
      "name": "Yaniv Roznai",
      "url": "https://www.runi.ac.il/en/faculty/yroznai"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "שלטון החוק",
    "בתי משפט",
    "ביקורת שיפוטית",
    "הגבלת כוח",
    "הגנה",
    "עצמית",
    "דמוקרטית",
    "משפט"
  ],
  "image": {
    "src": "html_qa/democracy_the_democratic_self_defence_of_constitutional_courts_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על הגנה עצמית דמוקרטית של בתי משפט חוקתיים",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "הגנה עצמית דמוקרטית של בתי משפט חוקתיים מדגים כיצד סוגיה אקדמית ממוקדת הופכת לשאלה על <strong>דמוקרטיה ליברלית</strong>, אחריות ציבורית, שוויון והגבלת כוח.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "חוקה היא מערכת כללי יסוד שמגדירה את מבנה השלטון, סמכויותיו וזכויות היסוד; חוקתנות היא התפיסה שלפיה גם רוב פוליטי ומוסדות שלטון מוגבלים על ידי כללים אלה. בהקשר של המאמר, המושג מסייע להבין כיצד כללי יסוד, בתי משפט וחוקי יסוד יכולים להגביל כוח ולייצב מחלוקת פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: בתי משפט מופשטים ברחבי העולם מותקפים. ברגע שבתי משפט נחלשים או נתפסים, קל יותר לממשלות הללו לערער מוסדות דמוקרטיים אחרים ואף להעמיד בתי משפט בעצמם נגד הדמוקרטיה. בהתאם לכך, תוך שימוש ברציונל של \"דמוקרטיה לוחמנית\", מאמר זה מציע את התיאוריה של הגנה עצמית דמוקרטית של בתי משפט לחוקה.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית הגנה עצמית דמוקרטית של בתי משפט חוקתיים משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון בהגנה עצמית דמוקרטית של בתי משפט חוקתיים אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון בהגנה עצמית דמוקרטית של בתי משפט חוקתיים מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם הגנה עצמית דמוקרטית של בתי משפט חוקתיים בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון בהגנה עצמית דמוקרטית של בתי משפט חוקתיים עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "הגנה עצמית דמוקרטית של בתי משפט חוקתיים",
  "permalink": "/democracy_the_democratic_self_defence_of_constitutional_courts_summary_he.html",
  "paper_url": "democracy_the_democratic_self_defence_of_constitutional_courts_summary_he.html",
  "sortKey": 202605220074,
  "sourcePublishedDate": "2024-03-01"
}
---
