---
{
  "version": 1,
  "file": "democracy_noncompliance_law_institutional_maintenance_ultra_religious_schools_summary_he.html",
  "slug": "democracy_noncompliance_law_institutional_maintenance_ultra_religious_schools_summary_he",
  "order": 901,
  "titleHe": "אי־ציות לחוק כשימור מוסדי בבתי ספר חרדיים לבנים",
  "subtitleHe": "",
  "descriptionHe": "תקציר בעברית של מחקר על אי־ציות לרגולציית לימודי הליבה בבתי ספר חרדיים לבנים בישראל, ועל האופן שבו בתי ספר ומפקחים משמרים יחד אוטונומיה מוסדית ואכיפה מוגבלת.",
  "summaryHe": "המאמר בוחן כיצד בתי ספר חרדיים לבנים מגיבים לדרישות המדינה ללימודי ליבה. על בסיס ראיונות ונתוני בתי ספר ממדגם מייצג, הוא מראה שהחוק נוכח כקו בסיס בהחלטות, אך אי־ציות נרחב ואכיפה חלשה יוצרים דפוס יציב של שימור מוסדי.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Lotem Perry-Hazan; Netta Barak-Corren; Gil Nachmani",
  "paperTitle": "Noncompliance with the law as institutional maintenance at ultra‐religious schools",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-6560-1136\" target=\"_blank\" rel=\"noopener noreferrer\">Lotem Perry-Hazan</a>; <a href=\"https://en.law.huji.ac.il/people/netta-barak-corren\" target=\"_blank\" rel=\"noopener noreferrer\">Netta Barak-Corren</a>; Gil Nachmani",
  "journal": "Regulation & Governance",
  "dateText": "פורסם: 2024; כרך 18(2), עמ׳ 612-636",
  "doiUrl": "https://doi.org/10.1111/rego.12545",
  "doiLabel": "https://doi.org/10.1111/rego.12545",
  "authors": [
    {
      "@type": "Person",
      "name": "Lotem Perry-Hazan",
      "url": "https://orcid.org/0000-0002-6560-1136"
    },
    {
      "@type": "Person",
      "name": "Netta Barak-Corren",
      "url": "https://en.law.huji.ac.il/people/netta-barak-corren"
    },
    {
      "@type": "Person",
      "name": "Gil Nachmani"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Lotem Perry-Hazan",
      "url": "https://orcid.org/0000-0002-6560-1136"
    },
    {
      "@type": "Person",
      "name": "Netta Barak-Corren",
      "url": "https://en.law.huji.ac.il/people/netta-barak-corren"
    },
    {
      "@type": "Person",
      "name": "Gil Nachmani"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "אי־ציות לחוק",
    "חינוך חרדי",
    "לימודי ליבה",
    "רגולציה",
    "שלטון החוק",
    "אכיפה",
    "אוטונומיה מוסדית",
    "מפקחים",
    "חינוך בישראל",
    "תחזוקה מוסדית"
  ],
  "image": {
    "src": "html_qa/democracy_noncompliance_law_institutional_maintenance_ultra_religious_schools_summary_he.jpg",
    "version": "2026-06-26-nightly",
    "altHe": "חדר הנהלה בבית ספר עם ספרים ותיקי רגולציה כמטפורה לאי־ציות ולימודי ליבה",
    "fitness": "standard"
  },
  "datePublished": "2026-06-26",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כיצד אי־ציות ללימודי ליבה בבתי ספר חרדיים פועל כ<strong>שימור מוסדי מתואם</strong> של אוטונומיה ואכיפה מוגבלת.",
  "sections": [
    {
      "headingHe": "אי־ציות כחלק ממערכת יחסים מוסדית",
      "paragraphsHtml": [
        "המאמר שואל כיצד בתי ספר חרדיים מגיבים לרגולציה שמנוגדת לנורמות תרבותיות עמוקות. במקום לראות אי־ציות ככשל נקודתי, הוא מנתח אותו כמערכת יחסים בין חוק, קהילה, רשתות חינוך, הורים ומפקחים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את החלטותיהם של בתי ספר חרדיים לבנים בישראל ביחס לרגולציית לימודי הליבה. המחקר נשען על מערך נתונים ראשון מסוגו: ראיונות ונתוני בתי ספר ממדגם מייצג של 82 מנהלים ומורים בבתי ספר שבהם לומדים 18,000 תלמידים, לצד שישה מפקחי ממשלה האחראים לעשרות בתי ספר.",
        "הניתוח מראה שהחוק נמצא בסביבת קבלת ההחלטות של בתי הספר ומשמש מסגרת כללית לתוכניות הלימודים, אך הוא מתחרה עם מקורות סמכות אחרים: רבנים, מפקחי רשתות לא־ממשלתיות, יועצים פרטיים, דגמים של בתי ספר אחרים, אילוצי משאבים, הורים ודעות אישיות.",
        "הממצאים מצביעים על אי־ציות רחב לצד אכיפה מוגבלת, כאשר למפקחי המדינה יש תפקיד שולי בלבד בהחלטות הלימודיות. המאמר מפרש את הדפוס הזה כשימור מוסדי מצומד: בתי הספר והמפקחים מקיימים יחד מצב יציב שבו אי־ציות ואוטונומיה נשמרים לאורך זמן."
      ]
    },
    {
      "headingHe": "נתונים נדירים על בתי ספר, מורים ומפקחים",
      "paragraphsHtml": [
        "לפי התקציר, המחקר נשען על ראיונות ונתוני בתי ספר ממדגם מייצג של 82 מנהלים ומורים בבתי ספר המשרתים 18,000 תלמידים, וכן על שישה מפקחי ממשלה. היקף הנתונים מאפשר לבחון את האכיפה מבפנים ולא רק דרך הצהרות משפטיות."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: שלטון החוק ואכיפה חסרה",
      "paragraphsHtml": [
        "שלטון החוק אינו רק כתיבת נורמה בספר החוקים אלא גם יישום עקבי, שוויוני ומוסדי שלה. כאשר החוק קובע קו בסיס אך האכיפה חלשה או מתואמת בפועל עם אי־ציות, נוצרת מערכת שבה הנורמה קיימת אך מוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "המשמעות הדמוקרטית של אוטונומיה חינוכית",
      "paragraphsHtml": [
        "אוטונומיה קהילתית היא ערך חשוב בחברה ליברלית, אך היא אינה פוטרת את המדינה משאלות של שוויון הזדמנויות, זכויות ילדים ושלטון חוק. המאמר מציב בדיוק את המתח בין הכרה בקהילה לבין אחריות ציבורית לחינוך בסיסי."
      ]
    },
    {
      "headingHe": "מהי רגולציית לימודי הליבה בהקשר המאמר?",
      "paragraphsHtml": [
        "המאמר עוסק בהחלטות של בתי ספר חרדיים לבנים ביחס לדרישות המדינה ללימודי ליבה. התקציר מציג את הדרישות האלה כדוגמה לרגולציה שמתנגשת עם נורמות תרבותיות ומוסדיות עמוקות."
      ]
    },
    {
      "headingHe": "מה פירוש שימור מוסדי?",
      "paragraphsHtml": [
        "שימור מוסדי הוא פעולה שמחזיקה מבנה קיים ומונעת שינוי עמוק בו. במאמר, אי־ציות והאכיפה החלקית מתוארים כדרך שבה בתי ספר ומפקחים משמרים יחד אוטונומיה ואי־ציות."
      ]
    },
    {
      "headingHe": "אילו מקורות משפיעים על החלטות בתי הספר?",
      "paragraphsHtml": [
        "התקציר מזכיר רבנים, מפקחי רשתות לא־ממשלתיות, יועצים פרטיים, מודלים של בתי ספר אחרים, אילוצי משאבים, הורים ודעות אישיות. החוק הוא מקור חשוב, אך הוא מתחרה עם מקורות סמכות אחרים."
      ]
    },
    {
      "headingHe": "למה המאמר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המאמר מראה שחוק יכול לקבוע קו בסיס ועדיין להיות חלש כאשר האכיפה אינה עקבית. שלטון החוק נבחן לא רק בסמכות הפורמלית של המדינה אלא גם ביכולת המעשית שלה להפוך נורמה לכלל מחייב."
      ]
    },
    {
      "headingHe": "האם המחקר תוקף את הקהילה החרדית?",
      "paragraphsHtml": [
        "התקציר אינו מציג התקפה כוללת על קהילה אלא ניתוח מוסדי של ציות ואי־ציות. הוא בוחן כיצד שחקנים שונים פועלים בתוך מערכת רגולטורית מורכבת שיש בה ערכים, אינטרסים ואילוצים."
      ]
    },
    {
      "headingHe": "מה תפקיד המפקחים בממצאים?",
      "paragraphsHtml": [
        "לפי התקציר, המפקחים אינם רק גורם חיצוני שמנסה לאכוף את החוק. הם חלק ממערכת יחסים שבה גבולות האכיפה והאוטונומיה מתוחזקים בפועל יחד עם בתי הספר."
      ]
    },
    {
      "headingHe": "למה זכויות ילדים מופיעות ברקע הדיון?",
      "paragraphsHtml": [
        "לימודי ליבה קשורים ליכולת עתידית להשתתף בשוק העבודה, בהשכלה ובחברה אזרחית. כאשר דרישות אלה אינן נאכפות, הדיון אינו רק על סמכות המדינה אלא גם על הזדמנויות הילדים."
      ]
    },
    {
      "headingHe": "מה ההבדל בין אוטונומיה לבין פטור מחוק?",
      "paragraphsHtml": [
        "אוטונומיה מאפשרת לקהילה לעצב חלקים מחייה לפי ערכיה. פטור מעשי מחוק מתחיל כאשר נורמות כלליות אינן חלות בפועל, ולכן הוא מעורר שאלות של שוויון, פיקוח ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "מה יכול ללמוד מכאן רגולטור?",
      "paragraphsHtml": [
        "רגולטור יכול ללמוד שאכיפה פורמלית אינה מספיקה כאשר מוסדות מקומיים מחזיקים מקורות סמכות חזקים יותר. שינוי דורש להבין את הרשתות, התמריצים וההסכמות השקטות שמתחזקות אי־ציות."
      ]
    },
    {
      "headingHe": "מהי תרומת המאמר למאגר?",
      "paragraphsHtml": [
        "המאמר מוסיף למאגר ניתוח חד של שלטון החוק במפגש עם חינוך, דת ואוטונומיה קהילתית. הוא מראה כיצד מוסדות דמוקרטיים נבחנים לא רק בחוקים שהם מחוקקים אלא ביכולת לאכוף אותם באופן הוגן ומידתי."
      ]
    }
  ],
  "layout": "paper",
  "title": "אי־ציות לחוק כשימור מוסדי בבתי ספר חרדיים לבנים",
  "permalink": "/democracy_noncompliance_law_institutional_maintenance_ultra_religious_schools_summary_he.html",
  "paper_url": "democracy_noncompliance_law_institutional_maintenance_ultra_religious_schools_summary_he.html",
  "sortKey": 202606260002
}
---
