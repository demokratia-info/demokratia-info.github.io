---
{
  "version": 1,
  "file": "democracy_legislative_executive_relations_crisis_summary_he.html",
  "slug": "democracy_legislative_executive_relations_crisis_summary_he",
  "order": 2,
  "titleHe": "איך משבר חוקתי מעביר כוח מהכנסת לממשלה?",
  "subtitleHe": "",
  "descriptionHe": "המאמר של אסנת עקירב מסביר כיצד בין 2019 ל-2024 נחלשה הכנסת מול הממשלה בישראל, ומדוע בית המשפט נשאר אחד הבלמים המרכזיים על כוח ביצועי.",
  "summaryHe": "איך שינוי ביחסי הכנסת והממשלה יכול להעמיק משבר חוקתי ולשחוק בלמים דמוקרטיים.",
  "authorsCardHe": "אסנת עקירב",
  "paperTitle": "Legislative-Executive Relations in Israel: Constitutional Crisis 2023-2024",
  "authorsHtml": "<a href=\"https://ric.org.il/researcher/osnat-akirav/\" target=\"_blank\" rel=\"noopener noreferrer\">Osnat Akirav</a>",
  "journal": "PS: Political Science & Politics",
  "dateText": "פורסם אונליין: 31 בינואר 2025; גיליון: ינואר 2025",
  "doiUrl": "https://doi.org/10.1017/S1049096524000532",
  "doiLabel": "https://doi.org/10.1017/S1049096524000532",
  "authors": [
    {
      "@type": "Person",
      "name": "אסנת עקירב",
      "url": "https://ric.org.il/researcher/osnat-akirav/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Osnat Akirav",
      "url": "https://ric.org.il/researcher/osnat-akirav/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "democratic-backsliding-populism",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "יחסי כנסת-ממשלה",
    "משבר חוקתי",
    "כוח ביצועי",
    "בית המשפט העליון",
    "איזונים ובלמים"
  ],
  "image": {
    "src": "html_qa/democracy_regime_classification_backsliding_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על שינוי יחסי כוח בין מוסדות שלטון, כנסת, ממשלה ובית משפט",
    "fitness": "standard"
  },
  "datePublished": "2026-05-07",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שבין 2019 ל-2024 עבר בישראל כוח פוליטי מן הכנסת אל הממשלה, במיוחד סביב משברים אישיים, ביטחוניים וקואליציוניים. לפי אסנת עקירב מהמכללה האקדמית גליל מערבי, היחלשות הכנסת הפכה את בית המשפט העליון לאחד המוסדות היחידים שיכולים עדיין להגביל באופן משמעותי את הכוח הביצועי.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "חוקה היא מערכת כללי יסוד שמגדירה את מבנה השלטון, סמכויותיו וזכויות היסוד; חוקתנות היא התפיסה שלפיה גם רוב פוליטי ומוסדות שלטון מוגבלים על ידי כללים אלה. בהקשר של המאמר, המושג מסייע להבין כיצד כללי יסוד, בתי משפט וחוקי יסוד יכולים להגביל כוח ולייצב מחלוקת פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "בין שני השותפים הקואליציוניים הקטנים יותר (כלומר, הירוקים וה-FDP). התגובות למלחמתה של רוסיה באוקראינה הובילו לעימות גלוי, כאשר חברי פרלמנט בודדים מתוך הרוב התמקמו בפומבי נגד עמדות ה-PPG שלהם והקנצלר. החום כך התגבר עוד יותר. המאמר רואה את ה\"פוליטיקה החמה\" הנוכחית במערכת שמעריכה \"פוליטיקה קרה\".",
        "איך שינוי ביחסי הכנסת והממשלה יכול להעמיק משבר חוקתי ולשחוק בלמים דמוקרטיים."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "אסנת עקירב מנתחת את יחסי הכנסת והממשלה בישראל בתקופת משבר חוקתי מתמשך בין 2019 ל-2024.",
        "המאמר ב-PS: Political Science &amp; Politics מתמקד בשאלה כיצד כתבי האישום נגד ראש הממשלה בנימין נתניהו, משברים חופפים, קיטוב פוליטי ורטוריקה פופוליסטית שינו את האיזון בין הרשות המחוקקת לרשות המבצעת.",
        "נקודת המוצא היא שישראל היא מערכת שבה הכנסת חלשה יחסית והממשלה שולטת בדרך כלל בסדר היום הפרלמנטרי."
      ]
    },
    {
      "headingHe": "איך הכנסת נחלשה מול הממשלה?",
      "paragraphsHtml": [
        "המאמר טוען שהממשלה השתמשה בכוחה הקואליציוני כדי לצמצם דיון פרלמנטרי מקצועי בסוגיות יסוד.",
        "עקירב מתארת כיצד תיקונים לחוקי יסוד, תקציבים קואליציוניים והחלטות סביב גיוס תלמידי ישיבות הדגימו דפוס שבו הרשות המבצעת דוחקת את הכנסת לשוליים.",
        "כאשר הרוב הקואליציוני מצביע כמעט אוטומטית עם הממשלה, הכנסת מתקשה לפקח, להתנות או לעכב מהלכים שמשנים את כללי המשחק."
      ]
    },
    {
      "headingHe": "מה השתנה אחרי 7 באוקטובר 2023?",
      "paragraphsHtml": [
        "אחרי 7 באוקטובר 2023, המאמר מזהה החרפה בדפוס שבו הישרדות הקואליציה קיבלה עדיפות על דיון ציבורי ומוסדי רחב.",
        "בדיונים תקציביים ובסוגיית גיוס חרדים, הממשלה והקואליציה פעלו כך שהכנסת לא מילאה תפקיד עצמאי מספיק בבחינת סדרי עדיפויות בזמן מלחמה.",
        "הטענה אינה שהכנסת נעלמה, אלא שהיא התקשתה לתפקד כבלם אמיתי על החלטות הממשלה דווקא ברגעים שבהם פיקוח דמוקרטי חיוני במיוחד."
      ]
    },
    {
      "headingHe": "למה בית המשפט מופיע כבלם מרכזי?",
      "paragraphsHtml": [
        "המאמר מדגיש שבישראל בית המשפט העליון חשוב במיוחד משום שאין בית עליון, אין פדרליזם, ואין חוקה שלמה וקשיחה שמגבילה בקלות את הממשלה.",
        "בכמה מקרים בית המשפט הצליח לאזן את כוח הממשלה, למשל כאשר בחן תיקוני יסוד או החלטות שהוציאו את הכנסת מן התהליך.",
        "אבל המסקנה אינה שבית המשפט לבדו מספיק. אם הכנסת חלשה מדי והקואליציה ממושמעת מדי, גם בית משפט עצמאי פועל תחת לחץ פוליטי כבד."
      ]
    },
    {
      "headingHe": "מה השאלה הציבורית שצריך לשאול?",
      "paragraphsHtml": [
        "השאלה הציבורית היא האם הכנסת מסוגלת להיות מוסד עצמאי שמפקח על הממשלה, או שהיא הופכת לכלי ביצוע של הקואליציה.",
        "כאשר כוח עובר לרשות המבצעת בלי דיון, שקיפות ופיקוח, הדמוקרטיה מאבדת את אחד הבלמים הבסיסיים שלה. לכן המאמר חשוב לא רק למשפטנים, אלא לכל מי שחושב על אחריות שלטונית בזמן משבר."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך משבר חוקתי מעביר כוח מהכנסת לממשלה?",
  "permalink": "/democracy_legislative_executive_relations_crisis_summary_he.html",
  "paper_url": "democracy_legislative_executive_relations_crisis_summary_he.html",
  "sortKey": 202605070023
}
---
