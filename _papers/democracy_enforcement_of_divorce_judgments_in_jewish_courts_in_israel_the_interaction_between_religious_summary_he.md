---
{
  "version": 1,
  "file": "democracy_enforcement_of_divorce_judgments_in_jewish_courts_in_israel_the_interaction_between_religious_summary_he.html",
  "slug": "democracy_enforcement_of_divorce_judgments_in_jewish_courts_in_israel_the_interaction_between_religious_summary_he",
  "order": 102,
  "titleHe": "מה הקשר בין המשפט החוקתי בישראל לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית המשפט החוקתי בישראל, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית המשפט החוקתי בישראל דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Yehiel S. Kaplan",
  "paperTitle": "Enforcement of Divorce Judgments in Jewish Courts in Israel: The Interaction Between Religious and Constitutional Law*",
  "authorsHtml": "<a href=\"https://law.haifa.ac.il/en/lecturers_eng/kaplane/\" target=\"_blank\" rel=\"noopener noreferrer\">Yehiel S. Kaplan</a>",
  "journal": "Middle East Law and Governance",
  "dateText": "פורסם: 2012",
  "doiUrl": "https://doi.org/10.1163/187633712x631246",
  "doiLabel": "https://doi.org/10.1163/187633712x631246",
  "authors": [
    {
      "@type": "Person",
      "name": "Yehiel S. Kaplan",
      "url": "https://law.haifa.ac.il/en/lecturers_eng/kaplane/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yehiel S. Kaplan",
      "url": "https://law.haifa.ac.il/en/lecturers_eng/kaplane/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "המשפט החוקתי בישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_enforcement_of_divorce_judgments_in_jewish_courts_in_israel_the_interaction_between_religious_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על המשפט החוקתי בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית המשפט החוקתי בישראל דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "חוקה היא מערכת כללי יסוד שמגדירה את מבנה השלטון, סמכויותיו וזכויות היסוד; חוקתנות היא התפיסה שלפיה גם רוב פוליטי ומוסדות שלטון מוגבלים על ידי כללים אלה. בהקשר של המאמר, המושג מסייע להבין כיצד כללי יסוד, בתי משפט וחוקי יסוד יכולים להגביל כוח ולייצב מחלוקת פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: במדינת ישראל מוענקת לבתי הדין הרבניים סמכות שיפוט בלעדית בפסיקת נישואין וגירושין של יהודים. עם זאת, בית המשפט העליון בישראל לא התייחס בצורה ברורה או ספציפית לאיזון בין הזכויות והחובות של הבעל והאישה בתהליך אכיפת פסקי הגירושין, לא לפני ולא לאחר חקיקתם של שני חוקי היסוד החוקתיים החשובים שנחקקו ב-1992. טרם נערך ניתוח מדיניות מפורט של הסנקציות נגד בני זוג סוררים בבתי הדין הרבניים בישראל - לאור עקרונות המשפט העברי והחוקתי במדינה.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית המשפט החוקתי בישראל, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת המשפט החוקתי בישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר המשפט החוקתי בישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין המשפט החוקתי בישראל לבין שלטון החוק בישראל?",
  "permalink": "/democracy_enforcement_of_divorce_judgments_in_jewish_courts_in_israel_the_interaction_between_religious_summary_he.html",
  "paper_url": "democracy_enforcement_of_divorce_judgments_in_jewish_courts_in_israel_the_interaction_between_religious_summary_he.html",
  "sortKey": 202605210191
}
---
