---
{
  "version": 1,
  "file": "democracy_finance_and_growth_schumpeter_might_be_right_summary_he.html",
  "slug": "democracy_finance_and_growth_schumpeter_might_be_right_summary_he",
  "titleHe": "מימון וצמיחה: האם שומפטר צדק?",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן אם פיתוח פיננסי קודם לצמיחה ומסייע לחזות אותה, באמצעות ראיות בין-מדינתיות על כ-80 מדינות בשנים 1960-1989.",
  "summaryHe": "המאמר מציג ראיות התומכות בטענה השומפטריאנית שמערכת פיננסית יכולה לקדם צמיחה, ולא רק להגיב אליה. המחקר בונה ארבעה מדדים לפיתוח פיננסי ובוחן את הקשר שלהם לצמיחת תוצר לנפש, צבירת הון ושיפור ביעילות כלכלית. הוא מוצא שפיתוח פיננסי גבוה יותר קשור לצמיחה מהירה יותר, וגם שהרכיב המוקדם של הפיתוח הפיננסי חוזה צמיחה ארוכת טווח בעשורים הבאים.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Robert G. King, Ross Levine",
  "paperTitle": "Finance and Growth: Schumpeter Might Be Right",
  "authorsHtml": "<a href=\"https://www.bu.edu/urop/profile/robert-king/\" target=\"_blank\" rel=\"noopener noreferrer\">Robert G. King</a>; <a href=\"https://profiles.stanford.edu/ross-levine\" target=\"_blank\" rel=\"noopener noreferrer\">Ross Levine</a>",
  "journal": "The Quarterly Journal of Economics",
  "dateText": "פורסם: 1.8.1993",
  "doiUrl": "https://doi.org/10.2307/2118406",
  "doiLabel": "https://doi.org/10.2307/2118406",
  "authors": [
    {
      "@type": "Person",
      "name": "Robert G. King",
      "url": "https://www.bu.edu/urop/profile/robert-king/"
    },
    {
      "@type": "Person",
      "name": "Ross Levine",
      "url": "https://profiles.stanford.edu/ross-levine"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Robert G. King",
      "url": "https://www.bu.edu/urop/profile/robert-king/"
    },
    {
      "@type": "Person",
      "name": "Ross Levine",
      "url": "https://profiles.stanford.edu/ross-levine"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "מימון",
    "צמיחה כלכלית",
    "מוסדות",
    "בנקים",
    "אשראי",
    "שלטון החוק",
    "השקעה"
  ],
  "image": {
    "src": "assets/article-images/real_photo_245_economy_democracy_finance_and_growth_schumpeter_might_be_right_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על מימון, השקעה וצמיחה במוסדות ציבוריים",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Tsvika Israeli",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_Government_Press_Office_(GPO)_-_Stockbrokers_at_the_Tel_Aviv_Stock_Exchange.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/1/16/Flickr_-_Government_Press_Office_%28GPO%29_-_Stockbrokers_at_the_Tel_Aviv_Stock_Exchange.jpg",
    "fileTitle": "Flickr - Government Press Office (GPO) - Stockbrokers at the Tel Aviv Stock Exchange.jpg",
    "sourceDimensions": "3072x2048",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:economy",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-05-24",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "בנתוני כ-80 מדינות בשנים 1960-1989, פיתוח פיננסי גבוה יותר קשור לצמיחה, לצבירת הון ולשיפור יעילות, ואף חוזה צמיחה עתידית לטווח של 10 עד 30 שנים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>פיתוח פיננסי</strong> הוא היכולת של בנקים, שווקים ומוסדות אשראי לאסוף מידע, להפנות חסכונות להשקעות ולנהל סיכונים. בהקשר של המאמר, זהו מנגנון מוסדי שמחבר בין כללים, אמון וצמיחה ולא רק ענף טכני של הכלכלה.",
        "<strong>צמיחה כלכלית</strong> אינה רק עלייה בתוצר; היא גם שאלה על מי מקבל גישה לאשראי, להשקעה ולהזדמנות. כאשר המימון פתוח, מפוקח ואמין יותר, יש בסיס רחב יותר למדיניות ציבורית שמצמצמת שרירות ומרחיבה יכולת פעולה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר משתמש בנתונים על כ-80 מדינות בתקופה 1960-1989 כדי לבחון את טענת Schumpeter שלפיה מתווכים פיננסיים תורמים לחדשנות ולפיתוח כלכלי. השירותים המרכזיים שנבחנים בהקשר זה הם גיוס חסכונות, הערכת פרויקטים, ניהול סיכונים, פיקוח על מנהלים והקלה על עסקאות.",
        "המחקר בונה ארבעה מדדים לפיתוח פיננסי. אחד מהם מודד עומק פיננסי באמצעות יחס ההתחייבויות הנזילות של המערכת הפיננסית לתוצר; מדד נוסף בוחן את חלקם של נכסי בנקים לפיקדונות ביחס לנכסים של בנקים והבנק המרכזי יחד. שני מדדים נוספים נועדו לבחון לאן האשראי המקומי מופנה, מתוך הבחנה בין מערכת שמזרימה אשראי לממשלה או לחברות מדינה לבין מערכת שמקצה אשראי למגזר הפרטי.",
        "המחקר מצא שפיתוח פיננסי גבוה יותר קשור באופן חיובי לצמיחת תוצר ריאלי לנפש, לצבירת הון פיזי ולשיפור ביעילות הכלכלית, גם לאחר בקרה על מאפייני מדינה ומדיניות. נוסף על כך, הרכיב המוקדם של הפיתוח הפיננסי נמצא כמנבא טוב של צמיחה ארוכת טווח ב-10 עד 30 השנים הבאות, ממצא שמנוגד לטענה שלפיה הפיננסים רק עוקבים אחר צמיחה קיימת."
      ]
    },
    {
      "headingHe": "מה עומד במרכז המחקר?",
      "paragraphsHtml": [
        "המחקר בוחן אם מימון הוא מנוע עצמאי של צמיחה או רק תוצר לוואי של כלכלה שכבר מתפתחת. הוא מחזיר את הדיון במערכת הפיננסית אל שאלות של מוסדות, מידע ואכיפה.",
        "הזווית הציבורית חשובה מפני שמימון קובע מי יכול להשקיע, מי יכול להתרחב ומי נשאר תלוי במקורות כוח מצומצמים. דמוקרטיה ליברלית זקוקה לכלכלה שבה גישה למשאבים אינה נשלטת רק בידי קשרים, משפחות או קרבה פוליטית."
      ]
    },
    {
      "headingHe": "איזו שאלה המחקר מציב על מוסדות וצמיחה?",
      "paragraphsHtml": [
        "השאלה היא האם שירותים פיננסיים כגון הערכת סיכון, ניהול חסכונות והקצאת אשראי מקדימים צמיחה ומסייעים לה. זו שאלה מוסדית מפני שהשוק הפיננסי נשען על חוזים, מידע ואמון ולא רק על היצע כסף."
      ]
    },
    {
      "headingHe": "איזה מנגנון מסביר את הקשר שהמאמר בוחן?",
      "paragraphsHtml": [
        "המנגנון המרכזי הוא הפיכת חסכונות מפוזרים להשקעות בעלות פוטנציאל פריון גבוה. כאשר מוסדות פיננסיים מסוגלים לזהות מיזמים טובים ולפקח עליהם, הם יכולים לצמצם בזבוז ולהגדיל את התשואה החברתית של השקעה."
      ]
    },
    {
      "headingHe": "איך זה קשור לשלטון החוק ולאחריות ציבורית?",
      "paragraphsHtml": [
        "מימון יעיל תלוי באכיפת חוזים, הגנה על זכויות קניין ויכולת לתבוע או לגבות חובות באופן לא שרירותי. לכן שלטון החוק אינו רק רקע משפטי; הוא תנאי לכך שאשראי יינתן לפי סיכון ויכולת ולא לפי כוח פוליטי."
      ]
    },
    {
      "headingHe": "למה המאמר חשוב לדיון על שוויון?",
      "paragraphsHtml": [
        "כאשר אשראי זמין רק לקבוצות מקורבות, צמיחה יכולה להעמיק ריכוז כוח במקום לפתוח הזדמנויות. המאמר מאפשר לשאול אם מערכת פיננסית מפותחת משרתת רק בעלי הון קיימים או גם יזמים, עובדים וקהילות שהגישה שלהן למשאבים מוגבלת."
      ]
    },
    {
      "headingHe": "מה המשמעות למדיניות ציבורית?",
      "paragraphsHtml": [
        "המשמעות למדיניות היא שפיתוח פיננסי צריך להיבחן לצד פיקוח, שקיפות ואחריות. לא די להגדיל אשראי; צריך לוודא שהכללים שמחלקים אותו אינם יוצרים תלות פוליטית או העדפה לא הוגנת."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על אמון במוסדות?",
      "paragraphsHtml": [
        "אמון במוסדות פיננסיים מאפשר לאזרחים ולחברות לתכנן לטווח ארוך. כאשר הציבור חושד שהמימון מוטה או לא מפוקח, גם השקעה כלכלית וגם אמון במוסדות המדינה נפגעים."
      ]
    },
    {
      "headingHe": "איך המחקר משנה קריאה רגילה של צמיחה כלכלית?",
      "paragraphsHtml": [
        "המאמר מזיז את הדיון מצמיחה כנתון מאקרו־כלכלי אל צמיחה כתוצאה של תשתיות מוסדיות. הוא מדגיש שהדרך שבה אשראי, מידע וחוזים מאורגנים משפיעה על איכות הצמיחה ועל חלוקתה."
      ]
    },
    {
      "headingHe": "איזה סוג ראיות או מודל עומד ברקע?",
      "paragraphsHtml": [
        "ברקע עומדת השוואה בין מדינות ולא רק תיאור של שוק פיננסי אחד. סוג ראיות כזה מאפשר לבחון אם רמת הפיתוח הפיננסי מופיעה לפני צמיחה מאוחרת יותר, אך עדיין מחייב זהירות בפרשנות סיבתית."
      ]
    },
    {
      "headingHe": "למה המאמר רלוונטי גם מחוץ למקרה שנבדק?",
      "paragraphsHtml": [
        "הדיון רלוונטי גם למדינות שאינן עומדות במרכז המחקר מפני שכל דמוקרטיה צריכה להחליט כיצד להסדיר אשראי, בנקאות וסיכון. במקומות שבהם מוסדות פיננסיים חלשים, גם זכויות פורמליות עלולות להישאר חסרות יכולת מימוש."
      ]
    },
    {
      "headingHe": "מה המסר המרכזי לקוראים שמבקשים להבין דמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "המסר המרכזי הוא שמימון הוא מוסד ציבורי במובן הרחב, גם כאשר הוא פועל דרך גופים פרטיים. דמוקרטיה ליברלית צריכה לשאול לא רק כמה צמיחה נוצרת, אלא אילו כללים מאפשרים אותה ומי יכול להשתתף בה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מימון וצמיחה: האם שומפטר צדק?",
  "permalink": "/democracy_finance_and_growth_schumpeter_might_be_right_summary_he.html",
  "paper_url": "democracy_finance_and_growth_schumpeter_might_be_right_summary_he.html",
  "sortKey": 202605240010,
  "sourcePublishedDate": "1993-08-01"
}
---
