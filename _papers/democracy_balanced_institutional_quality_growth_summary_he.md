---
{
  "version": 1,
  "file": "democracy_balanced_institutional_quality_growth_summary_he.html",
  "slug": "democracy_balanced_institutional_quality_growth_summary_he",
  "titleHe": "צמיחה כלכלית דורשת איזון בין מוסדות, לא רק ציון ממוצע",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן אם מדינות צומחות יותר כאשר תחומי האיכות המוסדית שלהן מאוזנים, ולא רק כאשר המדד הממוצע שלהן גבוה. מבחינה דמוקרטית, הוא מדגיש שרפורמה מוסדית אינה יכולה להסתפק באי של מצוינות לצד חולשות עמוקות בשלטון החוק, זכויות קניין או רגולציה.",
  "summaryHe": "לפי תקציר המקור, הספרות נטתה למדוד מוסדות בעזרת מדדי חירות כלכלית אך להתעלם מן היחסים בין תחומי המשנה. Bolen ו־Sobel מראים שהשונות הפנימית בין תחומי המדד משפרת את הסברי הצמיחה, ושאינטראקציות עם המערכת המשפטית וזכויות הקניין חשובות במיוחד. השורה הדמוקרטית היא שמוסד חזק אחד אינו מפצה בקלות על מוסד חלש אחר.",
  "authorsCardHe": "J. Brandon Bolen, Russell S. Sobel",
  "paperTitle": "Does Balance Among Areas of Institutional Quality Matter for Economic Growth?",
  "authorsHtml": "<a href=\"https://www.mc.edu/faculty/u/bbolen\" target=\"_blank\" rel=\"noopener noreferrer\">J. Brandon Bolen</a>; <a href=\"https://www.citadel.edu/marketing-supply-chain-economics/faculty-staff/dr-russell-s-sobel/\" target=\"_blank\" rel=\"noopener noreferrer\">Russell S. Sobel</a>",
  "journal": "Southern Economic Journal, 86(4), 1418-1445",
  "dateText": "פורסם אונליין: 20 במרץ 2020; גיליון: אפריל 2020",
  "doiUrl": "https://doi.org/10.1002/soej.12428",
  "doiLabel": "https://doi.org/10.1002/soej.12428",
  "authors": [
    {
      "@type": "Person",
      "name": "J. Brandon Bolen",
      "url": "https://www.mc.edu/faculty/u/bbolen"
    },
    {
      "@type": "Person",
      "name": "Russell S. Sobel",
      "url": "https://www.citadel.edu/marketing-supply-chain-economics/faculty-staff/dr-russell-s-sobel/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "J. Brandon Bolen",
      "url": "https://www.mc.edu/faculty/u/bbolen"
    },
    {
      "@type": "Person",
      "name": "Russell S. Sobel",
      "url": "https://www.citadel.edu/marketing-supply-chain-economics/faculty-staff/dr-russell-s-sobel/"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "איכות מוסדית",
    "צמיחה כלכלית",
    "חירות כלכלית",
    "זכויות קניין",
    "מערכת משפטית",
    "רפורמה מוסדית"
  ],
  "image": {
    "src": "html_qa/democracy_balanced_institutional_quality_growth_summary_he.jpg",
    "version": "2026-05-27-nightly",
    "altHe": "איור עריכתי על איזון בין תחומי איכות מוסדית וצמיחה",
    "fitness": "high"
  },
  "datePublished": "2026-05-27",
  "dateModified": "2026-05-27",
  "lastUpdatedHe": "27 במאי 2026",
  "oneLinerHtml": "המאמר בוחן אם מדינות צומחות יותר כאשר תחומי האיכות המוסדית שלהן מאוזנים, ולא רק כאשר המדד הממוצע שלהן גבוה. לפי תקציר המקור, הספרות נטתה למדוד מוסדות בעזרת מדדי חירות כלכלית אך להתעלם מן היחסים בין תחומי המשנה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>איכות מוסדית</strong> מתארת את יכולתם של כללים, רשויות ושווקים לפעול באופן צפוי, הוגן ואכיף. במאמר הזה הדגש הוא על כך שאיכות אינה רק ממוצע: חולשה בתחום אחד יכולה להגביל את התרומה של תחומים אחרים."
      ]
    },
    {
      "headingHe": "מה החידוש בשאלת האיזון?",
      "paragraphsHtml": [
        "המאמר אינו מסתפק בשאלה אם למדינה יש ציון מוסדי גבוה בממוצע. הוא שואל אם פערים בין תחומי המשנה של המוסדות פוגעים בצמיחה גם כאשר חלק מן התחומים נראים חזקים."
      ]
    },
    {
      "headingHe": "למה ממוצע מוסדי יכול להטעות?",
      "paragraphsHtml": [
        "ממוצע גבוה יכול להסתיר מערכת משפט חלשה, זכויות קניין רעועות או רגולציה לא יציבה. אם תחום חלש אחד חוסם השקעה ואמון, תחומים חזקים אחרים אינם בהכרח מפצים עליו."
      ]
    },
    {
      "headingHe": "איזה מדד עומד ברקע המחקר?",
      "paragraphsHtml": [
        "תקציר המקור מציין את מדד Economic Freedom of the World כבסיס למדידת מוסדות. המדד מחלק איכות מוסדית לכמה תחומים, ולכן הוא מתאים לבחינת יחסים בין תחומים ולא רק לציון כולל."
      ]
    },
    {
      "headingHe": "מה מצאו המחברים על שונות פנימית?",
      "paragraphsHtml": [
        "הם מצאו שמדד פשוט של סטיית תקן בין תחומי המשנה משפר באופן משמעותי את מפרטי הצמיחה האמפיריים. המשמעות היא שחוסר איזון מוסדי אינו רעש סטטיסטי בלבד אלא מידע מהותי על יכולת המשק לצמוח."
      ]
    },
    {
      "headingHe": "למה המערכת המשפטית וזכויות הקניין מרכזיות?",
      "paragraphsHtml": [
        "לפי התקציר, האינטראקציות החשובות ביותר קשורות למערכת המשפטית ולזכויות הקניין. זה הגיוני מבחינה דמוקרטית, מפני שתחומים אלה קובעים אם שחקנים יכולים להסתמך על כללים ולא רק על קשרים או כוח."
      ]
    },
    {
      "headingHe": "מה המשמעות לרפורמות מוסדיות?",
      "paragraphsHtml": [
        "המאמר תומך בגישה של שיפור התחומים החלשים ביותר במקום השקעה רק בתחומים שכבר מתפקדים היטב. רפורמה דמוקרטית טובה צריכה לזהות צווארי בקבוק מוסדיים ולצמצם אותם באופן עקבי."
      ]
    },
    {
      "headingHe": "האם מוסדות יכולים להחליף זה את זה?",
      "paragraphsHtml": [
        "המחברים מערערים על הנחת התחליפיות הפשוטה בין תחומי מוסדות. אם אכיפת חוזים חלשה, למשל, סביבה עסקית חופשית יותר לא בהכרח תיצור את אותה השפעה על השקעה וצמיחה."
      ]
    },
    {
      "headingHe": "איך זה קשור לשוויון אזרחי?",
      "paragraphsHtml": [
        "מערכת לא מאוזנת יוצרת יתרון למי שיודע לנווט בין חולשות מוסדיות. אזרחים ועסקים קטנים תלויים יותר בכללים ברורים, ולכן איזון מוסדי הוא גם שאלה של השתתפות שווה."
      ]
    },
    {
      "headingHe": "מה קובעי מדיניות צריכים לקחת מכאן?",
      "paragraphsHtml": [
        "המסר הוא למדוד מוסדות כארכיטקטורה שלמה ולא כאוסף ציונים מבודדים. כאשר תחום אחד חלש במיוחד, השקעה בו עשויה להיות חשובה יותר מהמשך שיפור של תחום שכבר מתפקד היטב."
      ]
    },
    {
      "headingHe": "מה השורה התחתונה לציבור?",
      "paragraphsHtml": [
        "צמיחה בת קיימא אינה נובעת רק ממדד מוסדי גבוה בכותרת. היא דורשת מערכת שבה משפט, קניין, רגולציה, מסחר ומדיניות ציבורית עובדים יחד ולא מושכים לכיוונים מנוגדים."
      ]
    }
  ],
  "layout": "paper",
  "title": "צמיחה כלכלית דורשת איזון בין מוסדות, לא רק ציון ממוצע",
  "permalink": "/democracy_balanced_institutional_quality_growth_summary_he.html",
  "paper_url": "democracy_balanced_institutional_quality_growth_summary_he.html",
  "sortKey": 202605270002,
  "sourcePublishedDate": "2020-03-20"
}
---
