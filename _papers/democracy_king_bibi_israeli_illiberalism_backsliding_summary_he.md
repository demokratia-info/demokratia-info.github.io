---
{
  "version": 1,
  "file": "democracy_king_bibi_israeli_illiberalism_backsliding_summary_he.html",
  "slug": "democracy_king_bibi_israeli_illiberalism_backsliding_summary_he",
  "titleHe": "הנסיגה הדמוקרטית בישראל תחת נתניהו הייתה ממשית אך מוגבלת מוסדית",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את ישראל בשנים 2009-2021 כמקרה של נסיגה דמוקרטית בהשוואה להונגריה ולטורקיה. הוא מדגיש כי דפוסי רטוריקה וטקטיקה דמו למקרים אחרים, אך הקיטוב האפקטיבי, השיטה הפרלמנטרית והמערכת האלקטורלית הגבילו את עומק ההצלחה.",
  "summaryHe": "Oren ו-Waxman מנתחים את שנות נתניהו השנייה, מ-2009 עד 2021, דרך מסגרת של נסיגה דמוקרטית ואי-ליברליזם. לפי התקציר, ישראל דמתה בחלק מן הרטוריקה והטקטיקות להונגריה ולטורקיה, אך התוצאה הייתה מוגבלת יותר. ההסבר שהם מדגישים קשור לאופי הקיטוב בישראל, למבנה הפרלמנטרי ולמערכת הבחירות.",
  "authorsCardHe": "Neta Oren, Dov Waxman",
  "paperTitle": "\"King Bibi\" and Israeli Illiberalism: Assessing Democratic Backsliding in Israel during the Second Netanyahu Era (2009–2021)",
  "authorsHtml": "Neta Oren; <a href=\"https://polisci.ucla.edu/person/dov-waxman/\" target=\"_blank\" rel=\"noopener noreferrer\">Dov Waxman</a>",
  "journal": "The Middle East Journal, 76(3), 303-326",
  "dateText": "פורסם: 01.12.2022",
  "doiUrl": "https://doi.org/10.3751/76.3.11",
  "doiLabel": "https://doi.org/10.3751/76.3.11",
  "authors": [
    {
      "@type": "Person",
      "name": "Neta Oren"
    },
    {
      "@type": "Person",
      "name": "Dov Waxman",
      "url": "https://polisci.ucla.edu/person/dov-waxman/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Neta Oren"
    },
    {
      "@type": "Person",
      "name": "Dov Waxman",
      "url": "https://polisci.ucla.edu/person/dov-waxman/"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "אי-ליברליזם",
    "נתניהו",
    "קיטוב אפקטיבי",
    "שיטה פרלמנטרית",
    "ישראל 2009-2021"
  ],
  "image": {
    "src": "html_qa/democracy_king_bibi_israeli_illiberalism_backsliding_summary_he.jpg",
    "version": "2026-05-30-nightly",
    "altHe": "חוקרים בוחנים מסמכי אחריותיות מוסדית במסדרון פרלמנטרי בעת משבר דמוקרטי",
    "fitness": "high"
  },
  "datePublished": "2026-05-30",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מציב את ישראל לצד הונגריה וטורקיה כדי להבין מה דומה ומה שונה במסלול של נסיגה דמוקרטית. הוא מראה שמבנה מוסדי וקיטוב מסוג מסוים יכולים להגביל גם מנהיגות שמפעילה רטוריקה וטקטיקות אי-ליברליות.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>נסיגה דמוקרטית</strong> היא שחיקה הדרגתית של איזונים, זכויות וכללי אחריותיות, לרוב דרך צעדים חוקיים לכאורה. <strong>אי-ליברליזם</strong> הוא תפיסה שמחלישה הגנות ליברליות כמו זכויות מיעוט, ביקורת שיפוטית ועצמאות מוסדית גם כאשר מתקיימות בחירות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מאמר זה מעריך את הנסיגה הדמוקרטית בישראל במהלך שנות כהונתו של ראש הממשלה בנימין נתניהו בין 2009 ל-2021, תוך השוואה להתפתחויות דומות בהונגריה ובטורקיה. בעוד שהנסיגה הדמוקרטית שהתרחשה בישראל בתקופה זו דמתה למתרחש במדינות אחרות אלו, נתניהו הצליח פחות בהשגת מטרותיו מאשר ויקטור אורבן ורג'פ טאיפ ארדואן, למרות שהשתמש ברטוריקה וטקטיקות דומות. חוסר ההצלחה של נתניהו מוסבר בכמות ואופי הקיטוב האפקטיבי בישראל, בשיטת הממשל הפרלמנטרית שלה ובשיטת הבחירות שלה.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: Oren ו-Waxman מנתחים את שנות נתניהו השנייה, מ-2009 עד 2021, דרך מסגרת של נסיגה דמוקרטית ואי-ליברליזם. לפי התקציר, ישראל דמתה בחלק מן הרטוריקה והטקטיקות להונגריה ולטורקיה, אך התוצאה הייתה מוגבלת יותר. ההסבר שהם מדגישים קשור לאופי הקיטוב בישראל, למבנה הפרלמנטרי ולמערכת הבחירות."
      ]
    },
    {
      "headingHe": "מה הטענה המרכזית על שנות 2009-2021?",
      "paragraphsHtml": [
        "המאמר טוען שבתקופת נתניהו השנייה התקיימו בישראל תהליכים שניתן להבין כנסיגה דמוקרטית. עם זאת, הוא מדגיש שההצלחה של המהלך הייתה חלקית יותר מאשר במקרים מובהקים כמו הונגריה וטורקיה."
      ]
    },
    {
      "headingHe": "למה ההשוואה להונגריה ולטורקיה חשובה?",
      "paragraphsHtml": [
        "ההשוואה מאפשרת להפריד בין דמיון בטקטיקות לבין דמיון בתוצאות. לפי התקציר, נתניהו השתמש ברטוריקה ובשיטות דומות לאלה של אורבן וארדואן, אך לא הגיע לאותה רמת שליטה מוסדית."
      ]
    },
    {
      "headingHe": "איזה תפקיד ממלא הקיטוב האפקטיבי?",
      "paragraphsHtml": [
        "המאמר מציג את הכמות והאופי של הקיטוב בישראל כהסבר מרכזי למגבלות המהלך. קיטוב יכול לסייע למנהיגות שמבקשת לגייס נאמנות, אך הוא גם יכול ליצור התנגדות עיקשת ומחנות שאינם מתכנסים תחת שליטה אחת."
      ]
    },
    {
      "headingHe": "איך השיטה הפרלמנטרית הגבילה את הנסיגה?",
      "paragraphsHtml": [
        "במשטר פרלמנטרי מנהיג תלוי בקואליציות, במפלגות שותפות ובאפשרות של בחירות תכופות. התלות הזאת אינה מבטיחה הגנה דמוקרטית, אך היא מקשה על ריכוז כוח מלא כאשר המערכת המפלגתית מפוצלת."
      ]
    },
    {
      "headingHe": "מה תפקידה של מערכת הבחירות בישראל?",
      "paragraphsHtml": [
        "מערכת הבחירות היחסית בישראל יוצרת ריבוי מפלגות ושותפים קואליציוניים. לפי המסגור של המאמר, המבנה הזה היה חלק מן הסיבה שנתניהו הצליח פחות ממנהיגים במדינות שבהן ניתן לרכז שליטה פוליטית במהירות רבה יותר."
      ]
    },
    {
      "headingHe": "האם המאמר מציג את ישראל כדמוקרטיה חסינה?",
      "paragraphsHtml": [
        "לא. הוא מזהה נסיגה דמוקרטית ממשית ומזהיר מפני אי-ליברליזם, אך מסביר מדוע התהליך לא הגיע לאותה עוצמה של מקרים השוואתיים אחרים."
      ]
    },
    {
      "headingHe": "מה ניתן ללמוד ממנו על מוסדות חלשים וחזקים?",
      "paragraphsHtml": [
        "מוסדות אינם פועלים לבד, אלא בתוך מערכת מפלגתית, ציבורית ואלקטורלית. המאמר מדגים שכוחות בלימה יכולים לנבוע לא רק מבית משפט או חוקה, אלא גם ממבנה תחרות פוליטית שמקשה על השתלטות אחידה."
      ]
    },
    {
      "headingHe": "למה זה חשוב לוויכוח הישראלי העכשווי?",
      "paragraphsHtml": [
        "המאמר מספק שפה השוואתית לדיון שאינו מסתפק בשאלה אם ישראל דמוקרטית או לא. הוא מאפשר לשאול אילו מנגנונים נחלשו, אילו החזיקו מעמד, ומהם התנאים שעלולים לשנות את האיזון בעתיד."
      ]
    },
    {
      "headingHe": "מהי התרומה הציבורית של המאמר?",
      "paragraphsHtml": [
        "התרומה היא בהבחנה בין סכנה לבין תוצאה סופית. במקום לומר שישראל כבר דומה לחלוטין להונגריה או לטורקיה, המאמר בוחן את מסלול השחיקה ואת המחסומים שמנעו ממנו להסתיים באותה צורה."
      ]
    }
  ],
  "layout": "paper",
  "title": "הנסיגה הדמוקרטית בישראל תחת נתניהו הייתה ממשית אך מוגבלת מוסדית",
  "permalink": "/democracy_king_bibi_israeli_illiberalism_backsliding_summary_he.html",
  "paper_url": "democracy_king_bibi_israeli_illiberalism_backsliding_summary_he.html",
  "sortKey": 202605300002,
  "sourcePublishedDate": "2022-12-01"
}
---
