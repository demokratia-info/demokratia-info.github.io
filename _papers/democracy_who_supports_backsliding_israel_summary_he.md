---
{
  "version": 1,
  "file": "democracy_who_supports_backsliding_israel_summary_he.html",
  "slug": "democracy_who_supports_backsliding_israel_summary_he",
  "order": 5,
  "titleHe": "מי מוכן לתמוך בנסיגה דמוקרטית בישראל?",
  "subtitleHe": "מחקר אמפירי על התנאים שבהם אזרחים תומכים בפגיעה בכללי המשחק הדמוקרטיים.",
  "descriptionHe": "המחקר בוחן מי בציבור הישראלי תומך בנסיגה דמוקרטית סביב תוכנית הממשלה לצמצום כוחם של בתי המשפט, באמצעות נתוני סקר פאנל שנאספו לפני ואחרי הכרזת התוכנית.",
  "summaryHe": "המחקר מצא שהתמיכה בתוכנית המשפטית קשורה בעיקר לזיקה חזקה לנתניהו ולסלידה ממתנגדים מפלגתיים, ופחות לתפיסה רובנית של דמוקרטיה או להתנסות שלילית עם מערכת המשפט.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "נעם גדרון, יותם מרגלית, ליאור שפר, איתמר יקיר",
  "paperTitle": "Who Supports Democratic Backsliding? Evidence from Israel",
  "authorsHtml": "<a href=\"https://noamgidron.com/\" target=\"_blank\" rel=\"noopener noreferrer\">Noam Gidron</a>; <a href=\"https://www.ymargalit.net/\" target=\"_blank\" rel=\"noopener noreferrer\">Yotam Margalit</a>; <a href=\"https://english.tau.ac.il/profile/liorsheffer\" target=\"_blank\" rel=\"noopener noreferrer\">Lior Sheffer</a>; <a href=\"https://en.idi.org.il/experts/36238\" target=\"_blank\" rel=\"noopener noreferrer\">Itamar Yakir</a>",
  "journal": "",
  "dateText": "פורסם: 19.09.2023",
  "doiUrl": "https://doi.org/10.31219/osf.io/zxukm",
  "doiLabel": "https://doi.org/10.31219/osf.io/zxukm",
  "authors": [
    {
      "@type": "Person",
      "name": "נעם גדרון",
      "url": "https://noamgidron.com/"
    },
    {
      "@type": "Person",
      "name": "יותם מרגלית",
      "url": "https://www.ymargalit.net/"
    },
    {
      "@type": "Person",
      "name": "ליאור שפר",
      "url": "https://english.tau.ac.il/profile/liorsheffer"
    },
    {
      "@type": "Person",
      "name": "איתמר יקיר",
      "url": "https://en.idi.org.il/experts/36238"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Noam Gidron",
      "url": "https://noamgidron.com/"
    },
    {
      "@type": "Person",
      "name": "Yotam Margalit",
      "url": "https://www.ymargalit.net/"
    },
    {
      "@type": "Person",
      "name": "Lior Sheffer",
      "url": "https://english.tau.ac.il/profile/liorsheffer"
    },
    {
      "@type": "Person",
      "name": "Itamar Yakir",
      "url": "https://en.idi.org.il/experts/36238"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "דעת קהל",
    "נסיגה דמוקרטית",
    "תמיכה ציבורית",
    "קיטוב",
    "ישראל"
  ],
  "image": {
    "src": "assets/article-images/real_photo_847_protest_democracy_who_supports_backsliding_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על דעת קהל ותמיכה בנסיגה דמוקרטית",
    "fitness": "high",
    "license": "CC0",
    "creator": "Eman",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Israel_Housing_Protests_Beersheba_Aug_13_2011e373.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/3/33/Israel_Housing_Protests_Beersheba_Aug_13_2011e373.jpg",
    "fileTitle": "Israel Housing Protests Beersheba Aug 13 2011e373.jpg",
    "sourceDimensions": "1230x923",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:protest",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/publicdomain/zero/1.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המחקר בוחן מי תמך בתוכנית לצמצום כוחם של בתי המשפט בישראל, ומראה שהתמיכה הוסברה בעיקר בזיקה למנהיג ובקיטוב רגשי בין מחנות פוליטיים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "נסיגה דמוקרטית היא תהליך שבו מוסדות, זכויות, בחירות חופשיות או בלמים ואיזונים נחלשים בהדרגה, גם כאשר המסגרת הדמוקרטית הפורמלית עדיין קיימת. בהקשר של המאמר, המושג מאפשר לראות שינוי מצטבר בכללי המשחק ולא רק משבר נקודתי או מחלוקת פוליטית רגילה."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר ״מי תומך בנסיגה דמוקרטית? ראיות מישראל״ מוצג כאן דרך השאלה הדמוקרטית המרכזית שעולה ממנו.",
        "התרומה החשובה היא הורדת הדיון מהאליטה אל הציבור: דמוקרטיה נשחקת לא רק מלמעלה, אלא גם כאשר אזרחים מצדיקים חריגה מן הכללים."
      ]
    },
    {
      "headingHe": "מהי הטענה המרכזית?",
      "paragraphsHtml": [
        "המחקר בוחן אילו עמדות וזהויות קשורות לנכונות לתמוך בצעדים שמחלישים דמוקרטיה, ומדוע תמיכה כזו אינה מוגבלת לאליטות בלבד.",
        "הקריאה הדמוקרטית של המאמר היא שהגנה על מוסדות, זכויות והשתתפות ציבורית מחייבת תשומת לב גם למנגנונים שנראים טכניים או מקצועיים.",
        "לכן המאמר מתאים לאתר: הוא מסביר כיצד החלטות משפטיות, פוליטיות או מוסדיות משפיעות על היכולת של אזרחים לחיות תחת כללי משחק הוגנים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן חמישה הסברים אפשריים לתמיכה ציבורית בנסיגה דמוקרטית: מנהיגות פרסונליסטית, קיטוב רגשי, פופוליזם, רובנות והסתבכות עם מערכת המשפט. הוא עושה זאת בהקשר של התוכנית הממשלתית לצמצום כוחם של בתי המשפט בישראל, שנתפסה כצעד המשנה את מבנה הבלמים והאיזונים הדמוקרטיים.",
        "המחקר מתבסס על נתוני Israel Polarization Panel, מחקר אורך שכלל גלים בין מרץ 2019 לאפריל 2023. הניתוח מתמקד בעמדות שנמדדו לפני ואחרי ההכרזה על התוכנית, ובפרט מנצל את תזמון התשובות סביב ההכרזה הציבורית כדי לצמצם את הסיכון שעמדות כלפי התוכנית כבר עיצבו מחדש עמדות כלפי נתניהו או כלפי המחנות הפוליטיים.",
        "הניתוח מראה שהתמיכה בתוכנית קשורה במיוחד לסלידה ממתנגדים מפלגתיים ולזיקה לראש הממשלה בנימין נתניהו. תפיסה רובנית של דמוקרטיה והתנסות שלילית עם מערכת המשפט קשורות לתמיכה באופן חלש יותר, ואילו עמדות פופוליסטיות אינן קשורות לתמיכה בתוכנית. המאמר מדגיש גם את המגבלה של ניתוחים המבוססים רק על נתוני חתך שנאספו לאחר הכרזת הרפורמה."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם במוסדות שמרסנים כוח, מגינים על מיעוטים ומאפשרים ביקורת ציבורית.",
        "כאשר אחד מן הרכיבים האלה נחלש, הפגיעה עשויה להופיע בהדרגה: פחות שקיפות, פחות אחריות, פחות הגנה על זכויות ופחות אמון ביכולת של המערכת לתקן את עצמה.",
        "המאמר עוזר לזהות את נקודת החיבור בין סוגיה אקדמית מוגדרת לבין שאלות ציבוריות רחבות על שלטון החוק, שוויון ואחריות."
      ]
    },
    {
      "headingHe": "מה הקורא הישראלי יכול לקחת מכאן?",
      "paragraphsHtml": [
        "הלקח המעשי הוא לבחון לא רק את הכותרת של שינוי מוסדי, אלא את ההשפעה המצטברת שלו על כללי המשחק.",
        "גם כאשר הרפורמה, הפסיקה או התהליך מוצגים כמקצועיים, השאלה הדמוקרטית נשארת: מי מרוויח כוח, מי מאבד הגנה, ואילו מנגנוני ביקורת נשארים פעילים.",
        "המאמר מזמין דיון ציבורי מדויק יותר, שמפריד בין מחלוקת לגיטימית לבין שינוי שמחליש את תנאי המחלוקת עצמה."
      ]
    },
    {
      "headingHe": "מה נושא המאמר?",
      "paragraphsHtml": [
        "המאמר עוסק בשאלה <strong>מי מוכן לתמוך בנסיגה דמוקרטית בישראל?</strong>",
        "במונחים פשוטים, הוא שואל כיצד המוסדות והכללים משפיעים על איכות הדמוקרטיה ולא רק על התוצאה הפוליטית המיידית."
      ]
    },
    {
      "headingHe": "למה המאמר רלוונטי לישראל?",
      "paragraphsHtml": [
        "המאמר רלוונטי לישראל משום שהוא עוסק במתח בין כוח פוליטי, מוסדות ביקורת, זכויות וזהות ציבורית.",
        "אלו בדיוק השאלות שמעצבות את הוויכוח הישראלי על דמוקרטיה ליברלית, במיוחד בתקופה של קיטוב ומשבר מוסדי."
      ]
    },
    {
      "headingHe": "מהו הסיכון הדמוקרטי המרכזי?",
      "paragraphsHtml": [
        "הסיכון המרכזי הוא ששינוי נקודתי ייתפס כמחלוקת רגילה, אף שהוא משנה את מאזן הכוחות או את רמת ההגנה על זכויות.",
        "כאשר שינויים כאלה מצטברים, קשה יותר להחזיר את המערכת לאיזון דמוקרטי תקין."
      ]
    },
    {
      "headingHe": "האם המאמר נוקט עמדה פוליטית מפלגתית?",
      "paragraphsHtml": [
        "המאמר אינו נדרש להחליף בחירה פוליטית של אזרחים, אלא מציע כלים לבחינת איכות הכללים הדמוקרטיים.",
        "הבחינה מתמקדת בשאלות של <strong>שלטון החוק</strong>, <strong>שוויון</strong>, <strong>אחריות</strong> ויכולת של מוסדות להגביל כוח."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לזכויות?",
      "paragraphsHtml": [
        "גם כאשר המאמר עוסק במוסדות, ההשלכה הסופית נוגעת לעיתים קרובות לזכויות של יחידים וקבוצות.",
        "מוסדות חלשים או מוטים מתקשים להגן על חופש ביטוי, שוויון אזרחי, זכויות מיעוטים והשתתפות פוליטית הוגנת."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק דורש שכללי המשחק יחולו גם על בעלי כוח ולא רק על אזרחים רגילים.",
        "המאמרים בקבוצה הזו מדגישים שכאשר מנגנוני ביקורת נחלשים, שלטון החוק עלול להפוך מסדר מחייב לשפה פוליטית בלבד."
      ]
    },
    {
      "headingHe": "מה הקשר לקיטוב?",
      "paragraphsHtml": [
        "קיטוב משפיע על יכולתו של הציבור לקבל הכרעות מוסדיות כלגיטימיות גם כאשר הן אינן נוחות לו.",
        "כאשר כל מחנה רואה באחר איום קיומי, קל יותר להצדיק פגיעה בכללים בשם ניצחון פוליטי או ביטחון."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "המחקר בוחן אילו עמדות וזהויות קשורות לנכונות לתמוך בצעדים שמחלישים דמוקרטיה, ומדוע תמיכה כזו אינה מוגבלת לאליטות בלבד.",
        "השורה התחתונה לאזרחית ולאזרח היא לבחון כל שינוי לפי השפעתו על כוח, זכויות, אחריות ואמון, ולא רק לפי הכוונה המוצהרת של יוזמיו."
      ]
    },
    {
      "headingHe": "למי כדאי לקרוא את המקור?",
      "paragraphsHtml": [
        "המקור מתאים לקוראים שמתעניינים במשפט חוקתי, מדע המדינה, דעת קהל, זכויות אדם או מוסדות דמוקרטיים.",
        "הוא שימושי במיוחד למי שרוצה להבין את הקשר בין מחקר אקדמי לבין הוויכוחים הציבוריים בישראל על דמוקרטיה ליברלית."
      ]
    }
  ],
  "layout": "paper",
  "title": "מי מוכן לתמוך בנסיגה דמוקרטית בישראל?",
  "permalink": "/democracy_who_supports_backsliding_israel_summary_he.html",
  "paper_url": "democracy_who_supports_backsliding_israel_summary_he.html",
  "sortKey": 202605210015
}
---
