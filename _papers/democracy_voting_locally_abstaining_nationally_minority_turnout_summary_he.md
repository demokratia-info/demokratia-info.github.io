---
{
  "version": 1,
  "file": "democracy_voting_locally_abstaining_nationally_minority_turnout_summary_he.html",
  "slug": "democracy_voting_locally_abstaining_nationally_minority_turnout_summary_he",
  "titleHe": "מדוע מיעוטים מצביעים מקומית אך נמנעים בבחירות ארציות?",
  "subtitleHe": "מחקר על ייצוג תיאורי, ייצוג מהותי ודפוסי השתתפות של מצביעים ערבים־מוסלמים בישראל.",
  "descriptionHe": "המאמר בוחן כיצד ייצוג תיאורי לעומת ייצוג מהותי משפיעים על שיעורי הצבעה של מיעוטים, דרך המקרה של ערבים־מוסלמים בישראל בהשוואה לרוב היהודי ולמיעוט הדרוזי.",
  "summaryHe": "המאמר טוען שנוכחות של נציגים מקבוצת מיעוט אינה מספיקה אם היא אינה משפרת בפועל את הגישה לטובין ציבוריים. כאשר הייצוג הוא תיאורי בלבד, חישוב ההצבעה של בוחרים עשוי להקטין השתתפות; כאשר הייצוג מהותי ומקדם מדיניות שמיטיבה עם הקבוצה, הוא עשוי להגדיל השתתפות. בישראל, ערבים־מוסלמים הפחיתו השתתפות בבחירות ארציות והגדילו השתתפות בבחירות מקומיות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Maoz Rosenthal; Hani Zubida; David Nachmias",
  "paperTitle": "Voting locally abstaining nationally: descriptive representation, substantive representation and minority voters’ turnout",
  "authorsHtml": "<a href=\"https://www.jmc.ac.il/en/about/contact-list/maoz-rosenthal/\" target=\"_blank\" rel=\"noopener noreferrer\">Maoz Rosenthal</a>; <a href=\"https://ric.org.il/researcher/hani-zubida-2/\" target=\"_blank\" rel=\"noopener noreferrer\">Hani Zubida</a>; <a href=\"https://www.runi.ac.il/en/faculty/davidna\" target=\"_blank\" rel=\"noopener noreferrer\">David Nachmias</a>",
  "journal": "Ethnic and Racial Studies, 41(9), 1632-1650",
  "dateText": "פורסם: 15 ביולי 2018",
  "doiUrl": "https://doi.org/10.1080/01419870.2017.1295161",
  "doiLabel": "https://doi.org/10.1080/01419870.2017.1295161",
  "authors": [
    {
      "@type": "Person",
      "name": "Maoz Rosenthal",
      "url": "https://www.jmc.ac.il/en/about/contact-list/maoz-rosenthal/"
    },
    {
      "@type": "Person",
      "name": "Hani Zubida",
      "url": "https://ric.org.il/researcher/hani-zubida-2/"
    },
    {
      "@type": "Person",
      "name": "David Nachmias",
      "url": "https://www.runi.ac.il/en/faculty/davidna"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Maoz Rosenthal",
      "url": "https://www.jmc.ac.il/en/about/contact-list/maoz-rosenthal/"
    },
    {
      "@type": "Person",
      "name": "Hani Zubida",
      "url": "https://ric.org.il/researcher/hani-zubida-2/"
    },
    {
      "@type": "Person",
      "name": "David Nachmias",
      "url": "https://www.runi.ac.il/en/faculty/davidna"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "השתתפות פוליטית",
    "ייצוג תיאורי",
    "ייצוג מהותי",
    "מיעוט ערבי־מוסלמי",
    "בחירות מקומיות",
    "בחירות ארציות"
  ],
  "image": {
    "src": "html_qa/democracy_voting_locally_abstaining_nationally_minority_turnout_summary_he.jpg",
    "version": "800x600-landscape-2026-07-01",
    "altHe": "מרכז קהילתי המשמש קלפי מקומית עם אזרחים מגוונים ליד שולחן הצבעה וללא טקסט קריא.",
    "fitness": "high"
  },
  "datePublished": "2026-07-01",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שמיעוטים מצביעים יותר כאשר הייצוג מתורגם להשפעה ממשית על טובין ציבוריים.",
  "sections": [
    {
      "headingHe": "מה ההבחנה שהמאמר מציע?",
      "paragraphsHtml": [
        "המאמר מבחין בין ייצוג תיאורי לבין ייצוג מהותי. ייצוג תיאורי מתקיים כאשר הנציגים דומים לזהות הקבוצה, ואילו ייצוג מהותי נמדד בשאלה אם הם מקדמים בפועל מדיניות וטובין ציבוריים עבור הקבוצה.",
        "ההבחנה הזאת חשובה במיוחד למיעוטים מוחלשים. כאשר נציגים קיימים אך אינם מצליחים להשפיע על השירותים והמשאבים, עצם הנראות הפוליטית עלולה לא להספיק כדי לעודד השתתפות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מבחין בין ייצוג תיאורי, שבו לנציגים יש זהות קבוצתית דומה לזו של הבוחרים, לבין ייצוג מהותי, שבו נציגי המיעוט מקדמים מדיניות המשפרת את אספקת הטובין הציבוריים לקבוצה. הטענה היא שהבוחרים מגיבים לא רק לזהות הנציגים אלא גם ליכולת שלהם להשיג תוצאות.",
        "המקרה הישראלי מתמקד בערבים־מוסלמים ומשווה אותם לרוב היהודי ולמיעוט הדרוזי. תחום הדיון מוגדר לישראל שבין הים התיכון לקו הירוק, ואינו כולל את הפלסטינים בגדה המערבית וברצועת עזה; אוכלוסיות שאינן זכאיות להצביע בבחירות הארציות לא נכללו בניתוח ההצבעה הארצי.",
        "הממצא המרכזי הוא שאיכות שונה של ייצוג במוסדות שונים קשורה להתנהגות הצבעה שונה בתוך אותה קבוצת מיעוט. ערבים־מוסלמים הורידו את שיעור ההשתתפות בבחירות הארציות והעלו אותו בבחירות המקומיות, דפוס שמחבר בין השתתפות פוליטית לבין הערכה של השפעה מוסדית על חיים יומיומיים."
      ]
    },
    {
      "headingHe": "מה קורה במעבר בין מקומי לארצי?",
      "paragraphsHtml": [
        "לפי המאמר, המאמר מראה כי ערבים־מוסלמים בישראל הפחיתו השתתפות בבחירות ארציות והגדילו השתתפות בבחירות מקומיות. ההבדל הזה קשור לאיכות הייצוג ולתחושה שהצבעה במוסד מסוים יכולה להשפיע על טובין ציבוריים.",
        "הבחירות המקומיות עשויות להיראות קרובות יותר לבעיות יומיומיות כמו תשתיות, חינוך ושירותים. הבחירות הארציות עשויות להיתפס כמרוחקות יותר כאשר הייצוג אינו מתורגם להשפעה מהותית."
      ]
    },
    {
      "headingHe": "מה המשמעות של שיעור הצבעה?",
      "paragraphsHtml": [
        "המאמר מזמין לקרוא שיעור הצבעה לא רק כמדד של עניין פוליטי או זהות לאומית. הוא מציע לראות בו תגובה רציונלית לאיכות הקשר בין נציגות, מדיניות ותוצאה מוסדית.",
        "לכן הימנעות אינה בהכרח אדישות. היא יכולה להיות איתות לכך שמוסד מסוים אינו נתפס כערוץ אפקטיבי להשפעה של קבוצת מיעוט."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>ייצוג תיאורי</strong> מתאר מצב שבו לנבחר ציבור יש זהות קבוצתית הדומה לזהות הבוחרים שהוא מייצג. הוא חשוב לסמליות ולנראות, אך אינו מבטיח שהקבוצה תקבל שירותים או הגנה טובים יותר.",
        "<strong>ייצוג מהותי</strong> מתמקד בפעולה ובתוצאה: האם הנציגים מקדמים מדיניות שמשפרת את מצב הקבוצה. במאמר זה ההבחנה בין שני סוגי הייצוג מסבירה מדוע אותה קבוצה יכולה להתנהג אחרת בבחירות שונות."
      ]
    },
    {
      "headingHe": "מה שאלת המחקר של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל אם איכות הייצוג של מיעוטים משפיעה על שיעורי ההצבעה שלהם במוסדות שונים. זו שאלה אמפירית ונורמטיבית גם יחד, מפני שהיא בודקת כיצד מוסדות וכללים מתרגמים זהות, כוח ואמון להתנהגות ציבורית.",
        "במקום להסתפק בתיאור כללי של השתתפות פוליטית של מיעוטים, המאמר ממקד את הדיון בההשוואה בין בחירות מקומיות לבין בחירות ארציות בישראל. לכן הוא מתאים לקריאה דמוקרטית שמחפשת את הקשר בין מדיניות יומיומית לבין שוויון אזרחי."
      ]
    },
    {
      "headingHe": "איך המאמר בוחן השתתפות?",
      "paragraphsHtml": [
        "המאמר משווה בין מוסדות בחירה שונים ובין קבוצות שונות בישראל כדי להבין מדוע אותה קבוצת מיעוט עשויה להשתתף יותר בזירה אחת ופחות בזירה אחרת. ההבחנה בין בחירות מקומיות לארציות מאפשרת לבחון את הקשר בין ייצוג לבין תוצאה מורגשת.",
        "המסגרת התאורטית מבחינה בין נציגים הדומים לקבוצה מבחינת זהות לבין נציגים שמצליחים לקדם עבורה טובין ציבוריים. כך שיעור הצבעה הופך למדד של אמון מוסדי ולא רק למדד של עניין בפוליטיקה."
      ]
    },
    {
      "headingHe": "איזו קבוצה או מוסד עומדים במרכז?",
      "paragraphsHtml": [
        "במרכז המאמר עומדים מצביעים ערבים־מוסלמים בישראל, בהשוואה לרוב היהודי ולמיעוט הדרוזי. הבחירה הזו חשובה משום שהיא מעבירה את הדיון מן העיקרון המופשט של שוויון אל זירה שבה אזרחים פוגשים את המדינה או את הרוב החברתי בפועל.",
        "המאמר מראה שמיעוטים עשויים להגיב לא רק לזהות הנציגים אלא גם ליכולת המוסד לספק השפעה ממשית. לכן המקרה אינו רק סיפור מגזרי, אלא מבחן ליכולת של דמוקרטיה להכיל אזרחות מורכבת."
      ]
    },
    {
      "headingHe": "מהי הטענה המרכזית?",
      "paragraphsHtml": [
        "הטענה המרכזית היא שייצוג תיאורי שאינו מלווה בייצוג מהותי עשוי להפחית השתתפות, בעוד ייצוג אפקטיבי יותר עשוי לעודד אותה. המאמר מציב את הטענה בתוך הקשר מוסדי וחברתי, ולא רק כהעדפה אישית של משתתפים.",
        "המשמעות היא שהצבעה היא גם הערכה של יעילות מוסדית ולא רק ביטוי של זהות או חובה אזרחית. כך הדיון עובר משאלה של כוונות לשאלה של תנאים, תמריצים ומנגנוני אחריות."
      ]
    },
    {
      "headingHe": "מה הממצא או הדגש המרכזי?",
      "paragraphsHtml": [
        "הדגש המרכזי הוא שערבים־מוסלמים הפחיתו השתתפות בבחירות ארציות והגדילו השתתפות בבחירות מקומיות. זהו ממצא חשוב משום שהוא מראה כיצד חוויה מוסדית או חברתית מסוימת יכולה לשנות אמון, השתתפות או תחושת שייכות.",
        "בקריאה זהירה, הממצא אינו אומר שכל חברי הקבוצה חושבים אותו דבר. הוא מצביע על דפוס או מתח שצריך להיבחן בתוך ההקשר המקומי של הפוליטיקה המקומית והארצית בישראל."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "החשיבות הדמוקרטית נובעת מכך שמיעוטים צריכים לא רק זכות הצבעה פורמלית אלא ערוצים שבהם ההצבעה משפיעה על חלוקת משאבים והכרה. דמוקרטיה ליברלית נמדדת גם בדרך שבה היא מגינה על מי שאינו שייך למרכז הכוח הפוליטי או התרבותי.",
        "כאשר נוכחות של נציגים אינה מלווה ביכולת ממשית לשנות מדיניות, הזכות הפורמלית אינה תמיד מספיקה. נדרש גם עיצוב מוסדי שמאפשר אמון, גישה, השפעה והכרה."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לשוויון אזרחי?",
      "paragraphsHtml": [
        "הקשר לשוויון אזרחי נמצא בהפער בין נראות פוליטית לבין השפעה על טובין ציבוריים. המאמר מזכיר ששוויון אינו רק איסור אפליה, אלא גם שאלה של נגישות, קול, ייצוג והגנה מפני הדרה.",
        "כאשר השוויון נבחן דרך קלפיות מקומיות וארציות, אפשר לראות אם הכללים מייצרים השתתפות ממשית או רק הכרה סמלית. זו נקודת מבחן מרכזית לכל חברה מפוצלת."
      ]
    },
    {
      "headingHe": "איזו טעות פרשנית כדאי להימנע ממנה?",
      "paragraphsHtml": [
        "לא כדאי לקרוא את המאמר כאילו הוא קובע שמיעוט שנמנע מהצבעה בהכרח אדיש לדמוקרטיה או מתנגד להשתתפות פוליטית. המידע הזמין תומך במסקנה ממוקדת יותר, שנוגעת לתנאים שבהם מתפתחים אמון, שייכות או הדרה.",
        "קריאה אחראית תבחן את סוג הבחירות, איכות הייצוג והטובין הציבוריים הזמינים לפני הסקת מסקנות כלליות. כך נשמר ההבדל בין ממצא מחקרי לבין עמדה פוליטית רחבה."
      ]
    },
    {
      "headingHe": "מה ההשלכה למדיניות ציבורית?",
      "paragraphsHtml": [
        "ההשלכה למדיניות היא שחיזוק השתתפות דורש להפוך ייצוג להשפעה ממשית על שירותים, תקציבים ומדיניות. אם רוצים לחזק דמוקרטיה ולא רק לנהל משבר, צריך לטפל במנגנונים שמייצרים פערי אמון או פערי גישה.",
        "במונחים מעשיים, המאמר מצביע על צורך בשקיפות, אחריות נבחרים ומנגנונים שמחברים בין הצבעה לבין תוצאות קונקרטיות. אלו צעדים שמתרגמים עיקרון דמוקרטי למפגש יומיומי בין אזרח, קהילה ומוסד."
      ]
    },
    {
      "headingHe": "מה הקורא הישראלי יכול לקחת מן המאמר?",
      "paragraphsHtml": [
        "הלקח לקורא הישראלי הוא ששיעור הצבעה של מיעוטים הוא מדד לאמון במוסדות ולא רק למדד נאמנות או עניין. המאמר מאפשר לראות את המחלוקת לא רק דרך עמדות זהותיות, אלא דרך האופן שבו מוסדות מעודדים או מחלישים השתתפות.",
        "בדיון ציבורי על ייצוג ערבים בישראל והשתתפות בבחירות, זו תזכורת לכך ששאלות של אמון, ייצוג והגינות אינן עניין משני. הן חלק מן התשתית שמאפשרת מחלוקת דמוקרטית בלי להוציא קבוצות שלמות מן המשחק."
      ]
    }
  ],
  "layout": "paper",
  "title": "מדוע מיעוטים מצביעים מקומית אך נמנעים בבחירות ארציות?",
  "permalink": "/democracy_voting_locally_abstaining_nationally_minority_turnout_summary_he.html",
  "paper_url": "democracy_voting_locally_abstaining_nationally_minority_turnout_summary_he.html",
  "sortKey": 202607010009,
  "sourcePublishedDate": "2018-07-15"
}
---
