---
{
  "version": 1,
  "file": "democracy_opportunistic_legislation_natural_emergency_covid_summary_he.html",
  "slug": "democracy_opportunistic_legislation_natural_emergency_covid_summary_he",
  "titleHe": "חקיקת הזדמנות בזמן חירום טבעי: צבירת כוח ממשלתי בקורונה",
  "subtitleHe": "מחקר על האופן שבו מצב חירום טבעי יכול להפוך להזדמנות להרחבת סמכות שלטונית.",
  "descriptionHe": "המאמר בוחן כיצד מצב חירום טבעי יכול לאפשר חקיקת הזדמנות וצבירת כוח ממשלתי גם בדמוקרטיה. הוא מנתח הגבלות על הפלות במדינות ארצות הברית בראשית מגפת הקורונה.",
  "summaryHe": "Udi Sommer, Jonathan Parent ו-Quan Li מראים שבחלק ממדינות ארצות הברית משבר COVID-19 שימש לא רק לניהול סיכון בריאותי אלא גם לקידום יעדי מדיניות קיימים בתחום זכויות הרבייה. ניתוח השוואתי איכותני מצביע על רמת החירום, דתיות המדינה ודומיננטיות רפובליקנית כגורמים שניבאו היטב את הסבירות לחקיקת הזדמנות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Udi Sommer, Jonathan Parent, Quan Li",
  "paperTitle": "Opportunistic legislation under a natural emergency: Grabbing government power in a democracy during COVID ‐19",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-9284-5291\" target=\"_blank\" rel=\"noopener noreferrer\">Udi Sommer</a>; Jonathan Parent; <a href=\"https://orcid.org/0000-0002-8851-3585\" target=\"_blank\" rel=\"noopener noreferrer\">Quan Li</a>",
  "journal": "Regulation & Governance",
  "dateText": "פורסם: 23.03.2023",
  "doiUrl": "https://doi.org/10.1111/rego.12519",
  "doiLabel": "https://doi.org/10.1111/rego.12519",
  "authors": [
    {
      "@type": "Person",
      "name": "Udi Sommer",
      "url": "https://orcid.org/0000-0002-9284-5291"
    },
    {
      "@type": "Person",
      "name": "Jonathan Parent"
    },
    {
      "@type": "Person",
      "name": "Quan Li",
      "url": "https://orcid.org/0000-0002-8851-3585"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Udi Sommer",
      "url": "https://orcid.org/0000-0002-9284-5291"
    },
    {
      "@type": "Person",
      "name": "Jonathan Parent"
    },
    {
      "@type": "Person",
      "name": "Quan Li",
      "url": "https://orcid.org/0000-0002-8851-3585"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "חקיקת חירום",
    "קורונה",
    "ריכוז כוח",
    "רגולציה",
    "משבר טבעי",
    "זכויות, שוויון ומיעוטים",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_527_knesset_democracy_opportunistic_legislation_natural_emergency_covid_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לחקיקת הזדמנות בזמן חירום טבעי: צבירת כוח ממשלתי בקורונה",
    "fitness": "high",
    "license": "CC BY 4.0",
    "creator": "IPPA photographer",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:The_Knesset_plenum_held_a_sitting_today_(FL63610088).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/2/23/The_Knesset_plenum_held_a_sitting_today_%28FL63610088%29.jpg",
    "fileTitle": "The Knesset plenum held a sitting today (FL63610088).jpg",
    "sourceDimensions": "3319x2039",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:knesset",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/4.0/"
  },
  "datePublished": "2026-06-09",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כיצד מצב חירום בריאותי יכול להפוך למסלול מהיר לקידום מדיניות שנויה במחלוקת ולצמצום זכויות.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את חקיקת הזדמנות בזמן חירום טבעי: צבירת כוח ממשלתי בקורונה דרך מקור אקדמי שפורסם בכתב העת Regulation & Governance ועוסק בזכויות, שוויון ומיעוטים. נקודת המוצא שלו היא שהסוגיה אינה רק אירוע נקודתי, אלא דרך להבין כיצד כללים, מוסדות ושחקנים חברתיים מעצבים תוצאה דמוקרטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתמקד בהגבלות על הפלות שהוטלו ברמת המדינות בארצות הברית בתחילת COVID-19. נקודת המוצא היא שמצב חירום מאפשר לעקוף דיון ציבורי ורגולציה ממושכת, ולכן הוא עלול לפתוח פתח לקידום מטרות מדיניות שהקשר שלהן למשבר הבריאותי חלקי בלבד.",
        "המחקר משתמש בניתוח השוואתי איכותני של כלל 50 המדינות ומכייל תנאים בינאריים כגון מושל רפובליקני, בית מחוקקים רפובליקני, שיעור תמיכה בטראמפ ב-2016, רמת הידבקות, מספר מרפאות ורמת דתיות. בין היתר, מדינה סווגה כנגועה כאשר חלקה במקרי COVID-19 עבר 10%, וכמדינה עם תשתית הפלות מוגבלת כאשר היו בה פחות משלוש מרפאות.",
        "הניתוח מצא שבהקשר של זכויות רבייה בזמן החירום של COVID-19, שילוב של רמת חירום, דתיות ודומיננטיות רפובליקנית חזה היטב חקיקה אופורטוניסטית. המאמר מתאר מנגנון שבו קבוצות אינטרס מעצבות מסר של בריאות הציבור, שלוחות מדינתיות מפעילות לחץ על פקידים מקומיים ותומכים מגויסים כדי לדחוף את הרשות המבצעת להטיל הגבלות."
      ]
    },
    {
      "headingHe": "איזו ראיה עומדת מאחורי הטענה?",
      "paragraphsHtml": [
        "המקור נשען על מסגרת מחקרית המתוארת בתקציר ובמטא־דאטה של המאמר, ובוחן את השאלה באמצעות המקרה, הנתונים או הניתוח שהמחברים מציגים. לכן יש לקרוא את הטענה כטענה מחקרית מוגדרת, לא כסיסמה כללית על דמוקרטיה."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "המאמר חשוב משום שהוא מראה כיצד זכויות, שוויון ומיעוטים קשור להגבלת כוח, לאחריותיות וליכולת של אזרחים וקבוצות להשתתף בתנאים הוגנים. בדמוקרטיה ליברלית, גם תהליכים שנראים מקצועיים משפיעים על זכויות, אמון ציבורי ואיזון בין מוסדות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית שהמאמר מציב?",
      "paragraphsHtml": [
        "השאלה המרכזית היא כיצד חקיקת הזדמנות בזמן חירום טבעי: צבירת כוח ממשלתי בקורונה משנה את ההבנה המחקרית לגבי זכויות, שוויון ומיעוטים. התשובה של המאמר ממקמת את הדיון בקשר בין פעולה מוסדית לבין תוצאה חברתית או פוליטית מדידה."
      ]
    },
    {
      "headingHe": "מה הקורא יכול לקחת מן הממצא?",
      "paragraphsHtml": [
        "הלקח הוא לבחון לא רק את התוצאה הסופית, אלא גם את הכללים והיחסים שמייצרים אותה. כאשר בודקים את התהליך עצמו אפשר לראות מי מקבל כוח, מי נושא בעלות, ואילו מנגנוני ביקורת נשארים פעילים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק בישראל בלבד?",
      "paragraphsHtml": [
        "המאמר קשור לישראל דרך החוקרים, המקרה או ההקשר המחקרי, אך הוא מציע גם תובנה רחבה יותר על דמוקרטיה ומוסדות. גם כאשר ההשוואה או הנתונים אינם ישראליים בלבד, השאלה הדמוקרטית רלוונטית להבנת הוויכוח הציבורי בישראל."
      ]
    },
    {
      "headingHe": "איזה מושג דמוקרטי בולט כאן?",
      "paragraphsHtml": [
        "המושג הבולט הוא <strong>זכויות, שוויון ומיעוטים</strong>, משום שהוא מחבר בין הממצא המחקרי לבין איכות הכללים הציבוריים. המאמר מראה שהמושג הזה אינו מופשט בלבד, אלא מופיע בהחלטות, נהלים, עמדות והתנהגויות."
      ]
    },
    {
      "headingHe": "מה לא כדאי להסיק מהר מדי?",
      "paragraphsHtml": [
        "לא נכון להסיק מן המאמר שכל מקרה דומה יניב אותה תוצאה בדיוק. נכון יותר להשתמש במאמר כדי לזהות מנגנון, שאלה או סיכון, ואז לבדוק כיצד הם מופיעים בהקשרים אחרים."
      ]
    },
    {
      "headingHe": "איך המאמר מתחבר לזכויות?",
      "paragraphsHtml": [
        "החיבור לזכויות מופיע דרך השאלה מי מקבל הגנה, ייצוג או אפשרות להשתתף בתהליך. כאשר הכללים מוטים או לא שקופים, גם מדיניות שנראית ניטרלית יכולה להשפיע אחרת על קבוצות שונות."
      ]
    },
    {
      "headingHe": "איך המאמר מתחבר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נוגע לא רק לקיום חוקים, אלא גם לאופן שבו סמכות מופעלת ומבוקרת. המאמר עוזר לראות האם ההליך מחזק אחריות ציבורית או מאפשר כוח שקשה לבקר אותו."
      ]
    },
    {
      "headingHe": "מה הקשר לאמון ציבורי?",
      "paragraphsHtml": [
        "אמון ציבורי תלוי בתחושה שהכללים יציבים, הוגנים וניתנים לביקורת. כאשר המאמר מצביע על פערים, משברים או מנגנונים מוסדיים, הוא גם מסביר מדוע אמון אינו רק רגש אלא תוצאה של מבנה ציבורי."
      ]
    },
    {
      "headingHe": "למי המאמר שימושי?",
      "paragraphsHtml": [
        "המאמר שימושי לחוקרות וחוקרים של משפט, מדע המדינה, מדיניות ציבורית וחברה אזרחית. הוא שימושי גם לקוראים שמבקשים להבין איך מחקר אקדמי מתרגם מחלוקת ציבורית לשאלה שאפשר לבחון באופן שיטתי."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה?",
      "paragraphsHtml": [
        "השורה התחתונה היא שחקיקת הזדמנות בזמן חירום טבעי: צבירת כוח ממשלתי בקורונה מספק דרך קונקרטית להבין את איכות הדמוקרטיה ולא רק את הכותרת הפוליטית של האירוע. המאמר מזמין לבחון עובדות, מוסדות ותמריצים לפני שמסתפקים בהצהרות כלליות."
      ]
    }
  ],
  "layout": "paper",
  "title": "חקיקת הזדמנות בזמן חירום טבעי: צבירת כוח ממשלתי בקורונה",
  "permalink": "/democracy_opportunistic_legislation_natural_emergency_covid_summary_he.html",
  "paper_url": "democracy_opportunistic_legislation_natural_emergency_covid_summary_he.html",
  "sortKey": 202606090010,
  "sourcePublishedDate": "2023-03-23"
}
---
