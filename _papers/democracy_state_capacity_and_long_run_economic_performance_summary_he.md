---
{
  "version": 1,
  "file": "democracy_state_capacity_and_long_run_economic_performance_summary_he.html",
  "slug": "democracy_state_capacity_and_long_run_economic_performance_summary_he",
  "titleHe": "יכולת מדינתית וביצועים כלכליים ארוכי טווח",
  "subtitleHe": "",
  "descriptionHe": "דינצ'קו וכץ בוחנים את הקשר בין יכולת מדינתית לבין ביצועים כלכליים ארוכי טווח באירופה. המאמר מתמקד בשתי תמורות פוליטיות, ריכוז פיסקלי וממשלה מוגבלת, ובוחן כיצד הן שיפרו את יכולת המדינה לגבות מסים ולתמוך בצמיחה.",
  "summaryHe": "המאמר טוען כי מדינות אירופיות פתרו בעיות היסטוריות של יכולת מדינתית באמצעות שתי תמורות: ריכוז פיסקלי וממשלה מוגבלת. הממצאים מראים קשר ישיר מובהק בין ריכוז פיסקלי לבין צמיחה כלכלית, וכן מצביעים על יכולת מוגברת לגבות הכנסות מס כמנגנון שקישר בין התמורות הפוליטיות לבין ביצועים כלכליים טובים יותר. לפי הניתוח, יכולת מדינתית אינה רק כוח כפייה או גבייה, אלא גם תשתית מנהלית שמאפשרת זכויות קניין בטוחות יותר, רגולציה בסיסית של שווקים ופתרון סכסוכים בבתי משפט. המאמר מציב את השילוב בין מדינה בעלת יכולת לבין מגבלות שלטוניות כמרכיב מרכזי בצמיחה ארוכת טווח.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Mark Dincecco, Gabriel Katz",
  "paperTitle": "State Capacity and Long‐Run Economic Performance",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-2860-7069\" target=\"_blank\" rel=\"noopener noreferrer\">Mark Dincecco</a>, <a href=\"https://orcid.org/0000-0001-5970-2769\" target=\"_blank\" rel=\"noopener noreferrer\">Gabriel Katz</a>",
  "journal": "The Economic Journal",
  "dateText": "פורסם: 2016",
  "doiUrl": "https://doi.org/10.1111/ecoj.12161",
  "doiLabel": "https://doi.org/10.1111/ecoj.12161",
  "authors": [
    {
      "@type": "Person",
      "name": "Mark Dincecco",
      "url": "https://orcid.org/0000-0003-2860-7069"
    },
    {
      "@type": "Person",
      "name": "Gabriel Katz",
      "url": "https://orcid.org/0000-0001-5970-2769"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Mark Dincecco",
      "url": "https://orcid.org/0000-0003-2860-7069"
    },
    {
      "@type": "Person",
      "name": "Gabriel Katz",
      "url": "https://orcid.org/0000-0001-5970-2769"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "מוסדות",
    "ממשל",
    "שירות ציבורי",
    "אחריותיות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_673_economy_democracy_state_capacity_and_long_run_economic_performance_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור ליכולת מדינתית וביצועים כלכליים ארוכי טווח",
    "fitness": "high",
    "license": "CC BY 4.0",
    "creator": "Nizzan Cohen",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Solomon_Souza_and_Lost-Koi_in_Mahane_Yehuda_Market_11.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/8/8d/Solomon_Souza_and_Lost-Koi_in_Mahane_Yehuda_Market_11.jpg",
    "fileTitle": "Solomon Souza and Lost-Koi in Mahane Yehuda Market 11.jpg",
    "sourceDimensions": "4624x3472",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:economy",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/4.0/"
  },
  "datePublished": "2026-05-23",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שבאירופה ההיסטורית ריכוז פיסקלי וממשלה מוגבלת חיזקו את יכולת המדינה לגבות מסים, לבנות תשתית מנהלית ולתמוך בצמיחה ארוכת טווח.",
  "sections": [
    {
      "headingHe": "למה יכולת מדינתית לבדה אינה מספיקה?",
      "paragraphsHtml": [
        "המאמר פותח במתח שבשילוב בין כוח פיסקלי-מנהלי לבין מגבלות ייצוגיות באירופה ההיסטורית. במקום להציג את הנושא כמחלוקת מופשטת, הוא ממקם אותו בתוך מוסדות, פרקטיקות ושחקנים שפועלים בתנאים פוליטיים ממשיים.",
        "כך יכולת מדינתית וביצועים כלכליים ארוכי טווח נעשה דרך לקרוא את היחסים בין כללים פורמליים לבין ניסיון אזרחי חי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את אירופה כזירה שבה נולדה הצמיחה הכלכלית המודרנית, ומעמיד במרכז שתי תמורות פוליטיות: ריכוז פיסקלי וממשלה מוגבלת. הטענה היא ששתי התמורות הגדילו את יכולת הממשלות הלאומיות לגייס הכנסות ממסים, ובכך פתרו חלק מבעיות היכולת של מדינות מתהוות.",
        "המחקר מצא קשר ישיר ומובהק בין ריכוז פיסקלי לבין צמיחה כלכלית. משטרים מרוכזים פיסקלית הציגו שיעור צמיחה שנתי ממוצע בתוצר הריאלי לנפש שהיה גבוה ב-0.17 עד 0.43 נקודות אחוז מזה של משטרים מפוצלים פיסקלית; בהשוואה לשיעור צמיחה ממוצע של 0.62% במדגם בשנים 1650-1913, מדובר בתוספת משמעותית.",
        "הניתוח מציג את יכולת גביית המסים כמנגנון מרכזי שדרכו התמורות הפוליטיות שיפרו ביצועים כלכליים. יכולת זו נקשרת גם לבניית תשתית מנהלית שמספקת זכויות קניין בטוחות יותר, כללי שוק בסיסיים ופתרון סכסוכים בבתי משפט, ולכן המאמר מבחין בין עצם כוח המדינה לבין האופן המוסדי שבו הכוח מופעל ומוגבל."
      ]
    },
    {
      "headingHe": "איך מגבלות ייצוגיות משנות את השפעת הכוח הפיסקלי?",
      "paragraphsHtml": [
        "הראיות שנרשמו בשלב ההכנה מצביעות על ניתוח היסטורי ארוך טווח של מוסדות וכלכלה. הבחירה הזו חשובה מפני שהיא מאפשרת לעקוב אחר האופן שבו הטענה מתממשת בפועל, ולא רק אחר ההצהרה הנורמטיבית שלה.",
        "מתוך הזירה הזאת אפשר לראות כיצד שפה משפטית, מנהלית או ציבורית מקבלת תוצאות שונות עבור קבוצות, מוסדות ואזרחים."
      ]
    },
    {
      "headingHe": "מה מחברת ההיסטוריה הכלכלית לתאוריה דמוקרטית?",
      "paragraphsHtml": [
        "המהלך המרכזי של המאמר הוא הטענה כי מדינה חזקה מועילה במיוחד כאשר יכולתה לפעול נתונה לריסון ולפיקוח ייצוגי. זהו ניסוח שמרחיק את הדיון מהסבר חד-סיבתי ומדגיש תנאים, איזונים ויחסי כוח.",
        "הטענה חשובה במיוחד משום שהיא מראה שהבעיה אינה רק אירוע נקודתי. היא נטועה במבנה שמארגן אפשרויות פעולה ומגבלות."
      ]
    },
    {
      "headingHe": "מתי מדינה חזקה מפתחת ומתי היא מנצלת?",
      "paragraphsHtml": [
        "מן הזווית הדמוקרטית, כוח מדינתי בלי מגבלות עלול לשרת שליטים או קבוצות צרות במקום פיתוח רחב. לכן המאמר עוסק לא רק בנושא המקצועי שעל פני השטח, אלא גם בשאלה מי זוכה להכרה, מי נדרש להצדיק את מקומו, ומי מסוגל להשפיע על הכללים.",
        "הקריאה הזאת הופכת את המקרה למבחן של איכות דמוקרטית: לא רק קיומם של מוסדות, אלא דרך פעולתם מול אנשים וקבוצות ממשיים."
      ]
    },
    {
      "headingHe": "כיצד המאמר מחדד את רעיון הממשלה המוגבלת?",
      "paragraphsHtml": [
        "התרומה הרחבה של המאמר היא הבחנה בין יכולת פעולה לבין איכות השימוש בכוח המדינה. הוא מציע לקורא שפה מדויקת יותר לקריאת המקרה, בלי להסתפק בסיסמאות על זכויות, ביטחון, ריבונות או ייצוג.",
        "בכך הוא משנה את מוקד הדיון: מן השאלה אם הערך הדמוקרטי קיים באופן פורמלי, אל השאלה כיצד הוא מופעל, מוגבל או מתעצב בתוך מציאות מוסדית וחברתית."
      ]
    }
  ],
  "layout": "paper",
  "title": "יכולת מדינתית וביצועים כלכליים ארוכי טווח",
  "permalink": "/democracy_state_capacity_and_long_run_economic_performance_summary_he.html",
  "paper_url": "democracy_state_capacity_and_long_run_economic_performance_summary_he.html",
  "sortKey": 202605230104,
  "sourcePublishedDate": "2016-02-01"
}
---
