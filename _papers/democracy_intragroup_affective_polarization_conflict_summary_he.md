---
{
  "version": 1,
  "file": "democracy_intragroup_affective_polarization_conflict_summary_he.html",
  "slug": "democracy_intragroup_affective_polarization_conflict_summary_he",
  "order": 51,
  "titleHe": "האם איום ביטחוני מפחית קיטוב רגשי בתוך החברה?",
  "subtitleHe": "המחקר משווה חברות המצויות בסכסוך מתמשך ומראה שאיום חיצוני אינו מבטל בהכרח איבה רגשית בין קבוצות אידיאולוגיות בתוך אותה חברה.",
  "descriptionHe": "טל אוריאן הראל, נמרוד ניר, דאן ונדרמולן, יפעת מעוז וערן הלפרין בוחנים קיטוב רגשי פנימי בישראל ובדרום קוריאה בתוך הקשר של סכסוכים מתמשכים.",
  "summaryHe": "המאמר מראה שקיטוב רגשי פנימי יכול להישאר איום על לכידות דמוקרטית גם כאשר החברה מתמודדת עם איום חיצוני מתמשך.",
  "authorsCardHe": "טל אוריאן הראל, נמרוד ניר, דאן ונדרמולן, יפעת מעוז וערן הלפרין",
  "paperTitle": "A Threat to Cohesion: Intragroup Affective Polarization in the Context of Intractable Intergroup Conflict",
  "authorsHtml": "<a href=\"https://www.asc.upenn.edu/people/faculty/tal-orian-harel\" target=\"_blank\" rel=\"noopener noreferrer\">Tal Orian Harel</a>; <a href=\"https://www.eranhalperin.com/our-lab\" target=\"_blank\" rel=\"noopener noreferrer\">Nimrod Nir</a>; <a href=\"https://www.eranhalperin.com/our-lab\" target=\"_blank\" rel=\"noopener noreferrer\">Daan Vandermeulen</a>; <a href=\"https://en.communication.huji.ac.il/people/prof-ifat-maoz\" target=\"_blank\" rel=\"noopener noreferrer\">Ifat Maoz</a>; <a href=\"https://en.psychology.huji.ac.il/people/eran-halperin-0\" target=\"_blank\" rel=\"noopener noreferrer\">Eran Halperin</a>",
  "journal": "Journal of Conflict Resolution",
  "dateText": "כרך 69, גליונות 2-3, מרץ 2025, עמ׳ 491-517",
  "doiUrl": "https://doi.org/10.1177/00220027241247033",
  "doiLabel": "https://doi.org/10.1177/00220027241247033",
  "authors": [
    {
      "@type": "Person",
      "name": "טל אוריאן הראל",
      "url": "https://www.asc.upenn.edu/people/faculty/tal-orian-harel"
    },
    {
      "@type": "Person",
      "name": "נמרוד ניר",
      "url": "https://www.eranhalperin.com/our-lab"
    },
    {
      "@type": "Person",
      "name": "דאן ונדרמולן",
      "url": "https://www.eranhalperin.com/our-lab"
    },
    {
      "@type": "Person",
      "name": "יפעת מעוז",
      "url": "https://en.communication.huji.ac.il/people/prof-ifat-maoz"
    },
    {
      "@type": "Person",
      "name": "ערן הלפרין",
      "url": "https://en.psychology.huji.ac.il/people/eran-halperin-0"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Tal Orian Harel",
      "url": "https://www.asc.upenn.edu/people/faculty/tal-orian-harel"
    },
    {
      "@type": "Person",
      "name": "Nimrod Nir",
      "url": "https://www.eranhalperin.com/our-lab"
    },
    {
      "@type": "Person",
      "name": "Daan Vandermeulen",
      "url": "https://www.eranhalperin.com/our-lab"
    },
    {
      "@type": "Person",
      "name": "Ifat Maoz",
      "url": "https://en.communication.huji.ac.il/people/prof-ifat-maoz"
    },
    {
      "@type": "Person",
      "name": "Eran Halperin",
      "url": "https://en.psychology.huji.ac.il/people/eran-halperin-0"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "democratic-backsliding-populism",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "קיטוב רגשי",
    "סכסוך מתמשך",
    "לכידות חברתית",
    "דעת קהל",
    "ישראל",
    "דרום קוריאה"
  ],
  "image": {
    "src": "html_qa/democracy_political_violence_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על קיטוב רגשי ולכידות חברתית בזמן סכסוך",
    "fitness": "standard"
  },
  "datePublished": "2026-05-20",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מראה שקיטוב רגשי פנימי יכול להישאר איום על לכידות דמוקרטית גם כאשר החברה מתמודדת עם איום חיצוני מתמשך. המשמעות הדמוקרטית היא שאחדות מול איום חיצוני אינה תחליף לעבודה מוסדית וחינוכית נגד דה-לגיטימציה של יריבים פוליטיים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "קיטוב רגשי הוא מצב שבו יריבים פוליטיים אינם רק חולקים בדעותיהם אלא מפתחים חשד, ריחוק או עוינות זה כלפי זה. בהקשר של המאמר, המושג חשוב כדי להבין כיצד רגשות כלפי מחנות פוליטיים משפיעים על אמון, אלימות פוליטית ונכונות לשתף פעולה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: עם זאת, במחקר הנוכחי, אנו מנתחים מערכי סקרים משתי חברות המשובצות בסכסוכים בלתי פתירים, דרום קוריאה (N=897) וישראל (N=504), ומדגימים כי פערים בתפיסת האיום החיצוני בין קבוצות אידיאולוגיות מתחרות קשורים לרמות גבוהות יותר של קיטוב רגשי בתוך חברות אלו. אנו מוצאים תימוכין גם למנגנון שמסביר מגמה זו: איום פנימי מצד החוץ האידיאולוגי. אנו דנים בהשלכות של הממצאים שלנו על חקר ההשפעה של קונפליקטים על תהליכים תוך-קבוצתיים, במיוחד קיטוב רגשי, ועל ההבנה כיצד תהליכים כאלה עשויים להנציח את הסכסוך עצמו.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר מראה שקיטוב רגשי פנימי יכול להישאר איום על לכידות דמוקרטית גם כאשר החברה מתמודדת עם איום חיצוני מתמשך."
      ]
    },
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מערער על ההנחה שאיום חיצוני יוצר אוטומטית אחדות פנימית, ומראה כי איבה בין מחנות פוליטיים יכולה להמשיך לשחוק אמון ולכידות.",
        "המחקר מנתח סקרי דעת קהל בשתי חברות המצויות בסכסוך מתמשך: דרום קוריאה עם N=897 וישראל עם N=504, לפי תקציר המאמר.",
        "במקום להתייחס לדמוקרטיה רק כמערכת בחירות, המאמר בוחן את התנאים שמאפשרים לחברה לשמר אמון, זכויות, אחריות ומחלוקת לגיטימית."
      ]
    },
    {
      "headingHe": "מהי נקודת המבט הליברלית-דמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית היא שאחדות מול איום חיצוני אינה תחליף לעבודה מוסדית וחינוכית נגד דה-לגיטימציה של יריבים פוליטיים.",
        "הזווית הליברלית חשובה משום שהיא שואלת לא רק מי שולט, אלא גם מה מגביל את השלטון, איך נשמרת ביקורת, ואיך מוגנות קבוצות או יריבים פוליטיים מפני כוח עודף."
      ]
    },
    {
      "headingHe": "למה המאמר חשוב לקוראי האתר?",
      "paragraphsHtml": [
        "המאמר מתאים לאתר משום שהוא מחבר בין סכסוך ביטחוני, קיטוב פוליטי ולכידות דמוקרטית בישראל.",
        "הוא מסייע לתרגם מחקר אקדמי לשאלות ציבוריות מעשיות: איך לזהות שחיקה דמוקרטית, איך למדוד קיטוב, ואיך לחשוב על מוסדות וזכויות בלי להסתפק בסיסמאות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד קיטוב רגשי משפיע על איכות הדמוקרטיה בישראל ועל היכולת לשמור על אמון, זכויות ומוסדות עצמאיים.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "מה החידוש העיקרי?",
      "paragraphsHtml": [
        "תרומתו היא בהעברת הדיון בקיטוב מהקשר אמריקאי או אלקטורלי צר אל חברות שחיות תחת איומי ביטחון מתמשכים.",
        "החידוש צריך להיבחן מול הטענה והראיות במאמר המקורי, ולא רק לפי הדרך שבה התמצית מנגישה אותו."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית היא שאחדות מול איום חיצוני אינה תחליף לעבודה מוסדית וחינוכית נגד דה-לגיטימציה של יריבים פוליטיים.",
        "החשיבות הדמוקרטית נובעת מן הקשר בין מוסדות, זכויות ואחריות ציבורית, אך גבולות הטענה נשארים גבולות המקור."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לישראל?",
      "paragraphsHtml": [
        "הקשר הישראלי נמצא בלב המאמר: המחקר מנתח סקרי דעת קהל בשתי חברות המצויות בסכסוך מתמשך: דרום קוריאה עם N=897 וישראל עם N=504, לפי תקציר המאמר.",
        "הקשר הישראלי צריך להיקרא דרך המוסדות, הזכויות וההקשרים שהמאמר עצמו מנתח, ולא כהכללה על החברה הישראלית כולה."
      ]
    },
    {
      "headingHe": "איזה מושג כדאי לזכור?",
      "paragraphsHtml": [
        "המושג המרכזי הוא <strong>קיטוב רגשי</strong>. הוא עוזר לראות כיצד תהליך שנראה פוליטי או משפטי צר משפיע על מערכת היחסים בין אזרחים, מוסדות ושלטון."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על מוסדות?",
      "paragraphsHtml": [
        "המוסדות הדמוקרטיים אינם פועלים בחלל ריק. הם תלויים בנורמות, באמון ציבורי, בהתנהגות מפלגות וביכולת להציב גבולות לכוח פוליטי."
      ]
    },
    {
      "headingHe": "האם המאמר מציע פתרון פשוט?",
      "paragraphsHtml": [
        "לא. התרומה שלו היא בעיקר אנליטית: הוא מחדד את הבעיה ומראה אילו תנאים פוליטיים, חברתיים או משפטיים צריכים להשתנות כדי לשפר את החוסן הדמוקרטי."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין לקרוא את המאמר כהוכחה שכל מחלוקת פוליטית היא סכנה לדמוקרטיה. החשיבות היא בזיהוי התנאים שבהם מחלוקת הופכת לפגיעה באמון, בזכויות או באיזונים מוסדיים."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול שאלות מדויקות יותר: אילו מוסדות נפגעים, אילו זכויות עומדות על הפרק, ואיך קיטוב רגשי משנה את התנהגות הציבור או השלטון.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "האם איום ביטחוני מפחית קיטוב רגשי בתוך החברה?",
  "permalink": "/democracy_intragroup_affective_polarization_conflict_summary_he.html",
  "paper_url": "democracy_intragroup_affective_polarization_conflict_summary_he.html",
  "sortKey": 202605200015
}
---
