---
{
  "version": 1,
  "file": "democracy_are_citizens_more_politically_engaged_when_candidate_selection_is_democratic_analysis_of_seven_parliamentary_summary_he.html",
  "slug": "democracy_are_citizens_more_politically_engaged_when_candidate_selection_is_democratic_analysis_of_seven_parliamentary_summary_he",
  "order": 279,
  "titleHe": "דמוקרטיה בבחירת מועמדים ומעורבות פוליטית של אזרחים",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן האם תהליכי בחירת מועמדים דמוקרטיים יותר בתוך מפלגות בישראל קשורים למעורבות פוליטית של אזרחים, מעבר לעצם ההצבעה או הפעילות המפלגתית.",
  "summaryHe": "המחקר משלב נתוני סקרי INES משבע מערכות בחירות בישראל בשנים 1996-2015 עם נתונים על שיטות בחירת מועמדים במפלגות. הוא מוצא שתהליכים מכילים יותר קשורים לרמות גבוהות יותר של עניין ומעורבות פוליטית, אך אינם מתורגמים לעלייה בהצבעה או בפעילות מפלגתית.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Gert-Jan Put, Yael Shomer",
  "paperTitle": "Are Citizens More Politically Engaged when Candidate Selection is Democratic? Analysis of Seven Parliamentary Election Cycles in Israel (1996–2015)",
  "authorsHtml": "<a href=\"https://soc.kuleuven.be/io/personeel/staf/00056161\" target=\"_blank\" rel=\"noopener noreferrer\">Gert-Jan Put</a>, <a href=\"https://people.socsci.tau.ac.il/mu/yshomer/\" target=\"_blank\" rel=\"noopener noreferrer\">Yael Shomer</a>",
  "journal": "Political Research Quarterly",
  "dateText": "פורסם: 2022",
  "doiUrl": "https://doi.org/10.1177/10659129221119203",
  "doiLabel": "https://doi.org/10.1177/10659129221119203",
  "authors": [
    {
      "@type": "Person",
      "name": "Gert-Jan Put",
      "url": "https://soc.kuleuven.be/io/personeel/staf/00056161"
    },
    {
      "@type": "Person",
      "name": "Yael Shomer",
      "url": "https://people.socsci.tau.ac.il/mu/yshomer/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Gert-Jan Put",
      "url": "https://soc.kuleuven.be/io/personeel/staf/00056161"
    },
    {
      "@type": "Person",
      "name": "Yael Shomer",
      "url": "https://people.socsci.tau.ac.il/mu/yshomer/"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דעת קהל",
    "קיטוב",
    "מחאה",
    "בחירות",
    "אלימות פוליטית",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_054_elections_democracy_are_citizens_more_politically_engaged_when_candidate_selection_is_democratic_ana.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על בחירות, ייצוג פוליטי ואמון הציבור בישראל",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Laliv g",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Israeli_legislative_election_2019_ballots_(8).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/f/f1/Israeli_legislative_election_2019_ballots_%288%29.jpg",
    "fileTitle": "Israeli legislative election 2019 ballots (8).jpg",
    "sourceDimensions": "3650x2240",
    "localDimensions": "800x600",
    "specificityTier": "literal_or_near_literal_subject_match",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:elections",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שתהליכי בחירת מועמדים פתוחים יותר במפלגות ישראליות קשורים ליותר קשב ודיון פוליטי, אך לא בהכרח ליותר הצבעה או השתתפות מפלגתית.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את בחירות, ייצוג פוליטי ואמון הציבור בישראל, וממקם את הסוגיה בתוך הדיון הרחב על דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
        "נקודת המוצא של התקציר היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את הקשר בין דמוקרטיה פנים-מפלגתית לבין מעורבות אזרחית בישראל באמצעות שילוב בין נתוני סקר הבחירות הלאומי בישראל לבין נתונים על שיטות בחירת מועמדים במפלגות. הניתוח מכסה שבע מערכות בחירות בשנים 1996-2015, תקופה שבה מפלגות ישראליות נעו בין פריימריז פתוחים יחסית, בחירה בידי מוסדות מפלגתיים, ובחירה ריכוזית בידי הנהגה או גורמים מצומצמים.",
        "המחקר מבחין בין השתתפות פוליטית לבין מעורבות פוליטית. בהשתתפות, המודלים אינם מוצאים שתומכי מפלגות המשתמשות בפריימריז או בנציגים נוטים יותר להצביע או להשתתף בפעילות מפלגתית מאשר תומכי מפלגות שבהן הרשימה נקבעת בידי הנהגה מצומצמת. בישראל שיעורי ההצבעה במדגם היו ממילא גבוהים, ורק חלק קטן מן המשיבים דיווחו שלא הצביעו.",
        "לעומת זאת, הניתוח מוצא קשר חיובי בין בחירת מועמדים מכילה יותר לבין מדדי מעורבות: דיון בפוליטיקה עם חברים ומשפחה, צפייה בתשדירי קמפיין וקריאת עיתונים. המסקנה המרכזית היא שיש נתק מבני בין מעורבות להשתתפות: פתיחת תהליך בחירת המועמדים יכולה להגביר עניין וקשב פוליטי, אך אינה מבטיחה תרגום של המעורבות הזו להצבעה או לפעילות מפלגתית."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת בחירות, ייצוג פוליטי ואמון הציבור בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בבחירות, ייצוג פוליטי ואמון הציבור בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את בחירות, ייצוג פוליטי ואמון הציבור בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>בחירות, ייצוג פוליטי ואמון הציבור בישראל</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בבחירות, ייצוג פוליטי ואמון הציבור בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "דמוקרטיה בבחירת מועמדים ומעורבות פוליטית של אזרחים",
  "permalink": "/democracy_are_citizens_more_politically_engaged_when_candidate_selection_is_democratic_analysis_of_seven_parliamentary_summary_he.html",
  "paper_url": "democracy_are_citizens_more_politically_engaged_when_candidate_selection_is_democratic_analysis_of_seven_parliamentary_summary_he.html",
  "sortKey": 202605210279
}
---
