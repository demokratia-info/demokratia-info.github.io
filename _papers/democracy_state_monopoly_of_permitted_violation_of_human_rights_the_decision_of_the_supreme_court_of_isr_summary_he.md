---
{
  "version": 1,
  "file": "democracy_state_monopoly_of_permitted_violation_of_human_rights_the_decision_of_the_supreme_court_of_isr_summary_he.html",
  "slug": "democracy_state_monopoly_of_permitted_violation_of_human_rights_the_decision_of_the_supreme_court_of_isr_summary_he",
  "order": 67,
  "titleHe": "מה הקשר בין גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Richard Harding",
  "paperTitle": "State monopoly of permitted violation of human rights: The decision of the Supreme Court of Israel prohibiting the private operation and management of prisons",
  "authorsHtml": "<a href=\"https://research-repository.uwa.edu.au/en/persons/richard-harding/\" target=\"_blank\" rel=\"noopener noreferrer\">Richard Harding</a>",
  "journal": "Punishment & Society",
  "dateText": "פורסם: 2012",
  "doiUrl": "https://doi.org/10.1177/1462474511434429",
  "doiLabel": "https://doi.org/10.1177/1462474511434429",
  "authors": [
    {
      "@type": "Person",
      "name": "Richard Harding",
      "url": "https://research-repository.uwa.edu.au/en/persons/richard-harding/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Richard Harding",
      "url": "https://research-repository.uwa.edu.au/en/persons/richard-harding/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_state_monopoly_of_permitted_violation_of_human_rights_the_decision_of_the_supreme_court_of_isr_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "זכויות אדם הן זכויות בסיסיות שמיוחסות לכל אדם בשל אנושיותו, כגון חירות, שוויון, כבוד, הליך הוגן וחופש ביטוי. בהקשר של המאמר, המושג מסייע לבדוק אם מוסדות המדינה מגינים על יחידים וקבוצות גם כאשר הדבר מגביל כוח שלטוני או רוב פוליטי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: הופעתו של ניהול המגזר הפרטי של בתי הכלא במדינה במהלך 30 השנים האחרונות הייתה שנויה במחלוקת. תומכי תפיסה זו טוענים כי על המדינה לנהל ישירות את בתי הכלא שלה גם אם ניתן להוכיח כי בתי הכלא המנוהלים באופן פרטי עדיפים על בתי הכלא המנוהלים באופן ציבורי בהשגת תוצאות תיקון חיוביות או בעלי משטרים פחות דיכויים או רגישים יותר לזכויות אדם. בית המשפט העליון אישר טענה זו, ולמעשה קיבל כי המאסר הוא תפקיד ליבה של המדינה.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין גבולות הפרטת בתי הסוהר וחובת המדינה להגן על זכויות לבין שלטון החוק בישראל?",
  "permalink": "/democracy_state_monopoly_of_permitted_violation_of_human_rights_the_decision_of_the_supreme_court_of_isr_summary_he.html",
  "paper_url": "democracy_state_monopoly_of_permitted_violation_of_human_rights_the_decision_of_the_supreme_court_of_isr_summary_he.html",
  "sortKey": 202605210226
}
---
