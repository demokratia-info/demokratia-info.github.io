---
{
  "version": 1,
  "file": "democracy_political_regimes_economic_freedom_growth_summary_he.html",
  "slug": "democracy_political_regimes_economic_freedom_growth_summary_he",
  "titleHe": "חירות כלכלית, דמוקרטיה וצמיחה",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את הקשר בין משטר פוליטי, חירות כלכלית וצמיחה, ומפריד בין דמוקרטיה פוליטית לבין מוסדות כלכליים.",
  "summaryHe": "המאמר משתמש במדדי החירות הכלכלית של Gwartney-Lawson-Block כדי לבדוק אם צמיחה מוסברת רק באמצעות ההבחנה בין משטרים דמוקרטיים ולא־דמוקרטיים. הממצאים מראים שחירות כלכלית תורמת לצמיחה בלי תלות באופי המשטר, בעוד השפעת הדמוקרטיה על הצמיחה אינה חד־משמעית ועשויה לפעול בעקיפין דרך השקעה.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Abdiweli M. Ali, W. Mark Crain",
  "paperTitle": "Political Regimes, Economic Freedom, Institutions and Growth",
  "authorsHtml": "Abdiweli M. Ali; <a href=\"https://economics.lafayette.edu/people/w-mark-crain/\" target=\"_blank\" rel=\"noopener noreferrer\">W. Mark Crain</a>",
  "journal": "Journal of Public Finance and Public Choice, 19(1), 3-21",
  "dateText": "פורסם: אפריל 2001",
  "doiUrl": "https://doi.org/10.1332/251569201x15668905707096",
  "doiLabel": "https://doi.org/10.1332/251569201x15668905707096",
  "authors": [
    {
      "@type": "Person",
      "name": "Abdiweli M. Ali"
    },
    {
      "@type": "Person",
      "name": "W. Mark Crain",
      "url": "https://economics.lafayette.edu/people/w-mark-crain/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Abdiweli M. Ali"
    },
    {
      "@type": "Person",
      "name": "W. Mark Crain",
      "url": "https://economics.lafayette.edu/people/w-mark-crain/"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "חירות כלכלית",
    "דמוקרטיה",
    "צמיחה כלכלית",
    "מוסדות",
    "השקעה",
    "זכויות אזרח"
  ],
  "image": {
    "src": "assets/article-images/real_photo_565_economy_democracy_political_regimes_economic_freedom_growth_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על חירות כלכלית, משטר פוליטי וצמיחה",
    "fitness": "high",
    "license": "CC BY 4.0",
    "creator": "IPPA photographer",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:David_Rockefeller_was_a_guest_of_honour_of_the_Bank_of_Israel_(FL45811654).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/a/a3/David_Rockefeller_was_a_guest_of_honour_of_the_Bank_of_Israel_%28FL45811654%29.jpg",
    "fileTitle": "David Rockefeller was a guest of honour of the Bank of Israel (FL45811654).jpg",
    "sourceDimensions": "2900x1824",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:economy",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/4.0/"
  },
  "datePublished": "2026-05-27",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שחירות כלכלית ודמוקרטיה אינן אותו מדד, ושחירות כלכלית קשורה לצמיחה גם מעבר לסוג המשטר.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>חירות כלכלית</strong> מתייחסת לכללים כמו זכויות קניין, חופש מסחר, יציבות כספית ורגולציה מוגבלת. <strong>דמוקרטיה</strong> מתייחסת לזכויות פוליטיות, בחירות והשתתפות אזרחית, ולכן שני המושגים חופפים בחלקם אך אינם זהים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר טוען שאי אפשר להבין צמיחה כלכלית רק דרך סיווג מדינות לדמוקרטיות ולא־דמוקרטיות. הוא מפריד בין חירות פוליטית וזכויות אזרח לבין חירות כלכלית, ומשתמש במדדי Gwartney-Lawson-Block כדי לבחון את המרכיב הכלכלי בנפרד.",
        "הניתוח מצא שחירות כלכלית תורמת לצמיחה ללא תלות בטיב המשטר הפוליטי. לעומת זאת, הקשר הישיר בין דמוקרטיה לצמיחה מתואר כעמום, ולכן המאמר מזהיר מפני ייחוס ביצועים כלכליים לעצם סוג המשטר בלבד.",
        "המאמר מוסיף שדמוקרטיה עשויה להשפיע על הצמיחה בעקיפין דרך ערוץ ההשקעה. לכן המסקנה הציבורית היא מוסדית: יש לבחון יחד כללים כלכליים, זכויות פוליטיות ותמריצי השקעה, בלי להפוך אחד מהם להסבר יחיד."
      ]
    },
    {
      "headingHe": "מה המאמר מנסה לברר?",
      "paragraphsHtml": [
        "המאמר מנסה להפריד בין השפעת המשטר הפוליטי לבין השפעת החירות הכלכלית על צמיחה. ההפרדה חשובה מפני שמדינות יכולות להיות דמוקרטיות יותר או פחות, וגם חופשיות כלכלית יותר או פחות, בשילובים שונים."
      ]
    },
    {
      "headingHe": "למה הבחנה בין דמוקרטיה לאוטוקרטיה אינה מספיקה?",
      "paragraphsHtml": [
        "המאמר מדגיש שהשפעת מערכות פוליטיות לא מובנת רק דרך סיווג בינארי. מדינה לא דמוקרטית יכולה לאמץ חלק מכללי החירות הכלכלית, ודמוקרטיה יכולה לסבול ממוסדות כלכליים חלשים."
      ]
    },
    {
      "headingHe": "מה תפקיד מדדי Gwartney-Lawson-Block?",
      "paragraphsHtml": [
        "המחברים משתמשים במדדים של חירות כלכלית שנבנו בשנות התשעים. המדדים מאפשרים לבחון כללי משחק כלכליים באופן נפרד יחסית מן הזכויות הפוליטיות והאזרחיות."
      ]
    },
    {
      "headingHe": "מה נמצא לגבי חירות כלכלית?",
      "paragraphsHtml": [
        "חירות כלכלית תורמת לצמיחה בלי תלות באופי המשטר הפוליטי. המשמעות היא שכללים כלכליים אמינים יכולים להשפיע על השקעה וייצור גם כאשר הדמוקרטיה הפוליטית אינה מלאה."
      ]
    },
    {
      "headingHe": "מה נמצא לגבי דמוקרטיה?",
      "paragraphsHtml": [
        "התקציר מתאר את השפעת הדמוקרטיה על הצמיחה כעמומה במקרה הטוב. עם זאת, הוא מוסיף שדמוקרטיה עשויה להשפיע על צמיחה בעקיפין דרך ערוץ ההשקעה."
      ]
    },
    {
      "headingHe": "האם המאמר טוען שדמוקרטיה אינה חשובה?",
      "paragraphsHtml": [
        "לא. המאמר עוסק בצמיחה כלכלית ולא בערך הכולל של חירות פוליטית, זכויות אדם או שלטון נבחר. גם אם התרומה הישירה לצמיחה אינה חד־משמעית, לדמוקרטיה יש תפקיד נורמטיבי ומוסדי רחב יותר."
      ]
    },
    {
      "headingHe": "איך ערוץ ההשקעה מחבר דמוקרטיה לצמיחה?",
      "paragraphsHtml": [
        "דמוקרטיה יכולה להשפיע על ביטחון, יציבות מדיניות, שקיפות והגבלת שרירותיות. גורמים אלה עשויים להשפיע על נכונות להשקיע, גם אם הקשר הכולל בין דמוקרטיה לצמיחה אינו פשוט."
      ]
    },
    {
      "headingHe": "מה הסיכון בפרשנות פוליטית מהירה?",
      "paragraphsHtml": [
        "אפשר לטעות ולקרוא את הממצא כהעדפה למשטר לא דמוקרטי אם הוא מציג חירות כלכלית מסוימת. זו פרשנות צרה מדי, משום שהמאמר מפריד בין מדד צמיחה לבין שאלות של זכויות, אחריותיות ושוויון פוליטי."
      ]
    },
    {
      "headingHe": "מה קובעי מדיניות צריכים ללמוד?",
      "paragraphsHtml": [
        "מדיניות צמיחה צריכה לבחון גם זכויות קניין, יציבות כללים, מסחר ורגולציה. במקביל, מדיניות דמוקרטית צריכה לזכור שחירות פוליטית וחירות כלכלית אינן תחליף מלא זו לזו."
      ]
    },
    {
      "headingHe": "מה השורה התחתונה לציבור?",
      "paragraphsHtml": [
        "המאמר מבקש דיוק: דמוקרטיה, מוסדות וחירות כלכלית אינם אותו דבר. הבנה רצינית של צמיחה צריכה לבדוק את כל המרכיבים האלה, בלי להפוך את אחד מהם להסבר יחיד."
      ]
    }
  ],
  "layout": "paper",
  "title": "חירות כלכלית, דמוקרטיה וצמיחה",
  "permalink": "/democracy_political_regimes_economic_freedom_growth_summary_he.html",
  "paper_url": "democracy_political_regimes_economic_freedom_growth_summary_he.html",
  "sortKey": 202605270007
}
---
