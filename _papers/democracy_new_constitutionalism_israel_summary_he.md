---
{
  "version": 1,
  "file": "democracy_new_constitutionalism_israel_summary_he.html",
  "slug": "democracy_new_constitutionalism_israel_summary_he",
  "order": 10,
  "titleHe": "החוקתיות החדשה בישראל ושאלת הדמוקרטיה",
  "subtitleHe": "מנדל מציע ביקורת חריפה על העברת כוח ממוסדות נבחרים אל בתי משפט במסגרת החוקתיות החדשה.",
  "descriptionHe": "המאמר של מייקל מנדל מציג ביקורת חריפה על החוקתיות החדשה בישראל ועל העברת כוח מן הכנסת אל בתי המשפט באמצעות אכיפה שיפוטית של מגילות זכויות נוקשות.",
  "summaryHe": "המאמר בוחן את החוקתיות החדשה בישראל כחלק מתהליך רחב שבו אכיפה שיפוטית של מגילות זכויות וחוקות נוקשות מעבירה כוח ממחוקקים אל בתי המשפט. מנדל מתנגד לתיאור התהליך כהתפתחות דמוקרטית פשוטה, וטוען שהוא משנה את מיקום הפוליטיקה ואת אופייה באמצעות משפטיזציה. ביקורתו חריפה במיוחד: החוקתיות החדשה מוצגת כאמצעי שמצמצם את כוחם של מוסדות ייצוגיים ומגן על סדר חברתי המבוסס על קניין פרטי מפני הכרעות של רוב דמוקרטי.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "מייקל מנדל",
  "paperTitle": "Democracy and the New Constitutionalism in Israel",
  "authorsHtml": "Michael Mandel",
  "journal": "Israel Law Review",
  "dateText": "פורסם: 1999",
  "doiUrl": "https://doi.org/10.1017/s0021223700015995",
  "doiLabel": "https://doi.org/10.1017/s0021223700015995",
  "authors": [
    {
      "@type": "Person",
      "name": "מייקל מנדל"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Michael Mandel"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "חוקתיות חדשה",
    "ביקורת שיפוטית",
    "ייצוג דמוקרטי",
    "בתי משפט",
    "ישראל"
  ],
  "image": {
    "src": "assets/article-images/real_photo_514_court_democracy_new_constitutionalism_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על חוקתיות חדשה, מוסדות וייצוג דמוקרטי",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Avi1111 dr. avishai teicher",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Dovetailed_sculpture_by_Israel_Hadany_in_the_Israel_supreme_court_building.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/5/50/Dovetailed_sculpture_by_Israel_Hadany_in_the_Israel_supreme_court_building.jpg",
    "fileTitle": "Dovetailed sculpture by Israel Hadany in the Israel supreme court building.jpg",
    "sourceDimensions": "4160x3120",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:court",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר טוען שהחוקתיות החדשה בישראל אינה רק הגנה על זכויות, אלא גם העברת כוח דמוקרטי מן המחוקק אל בית המשפט.",
  "sections": [
    {
      "headingHe": "איך החוקתיות החדשה משנה את מאזן הכוחות בישראל?",
      "paragraphsHtml": [
        "המאמר בוחן את התפתחות החוקתיות החדשה בישראל, במיוחד לאחר חקיקת חוקי היסוד העוסקים בזכויות אדם וביסוס ביקורת שיפוטית על חקיקה. הנקודה החשובה היא שהדיון נשאר מחובר למקרה ולמנגנון שהמאמר מנתח, ולא לטענה כללית בלבד על דמוקרטיה.",
        "הוא מתמקד בשאלה כיצד שינוי זה משפיע על הדמוקרטיה: האם הוא מחזק מגבלות הכרחיות על הרוב, או יוצר בעיית לגיטימציה כאשר בית המשפט מקבל כוח חוקתי רחב."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתאר את עליית כוחם של בתי המשפט ואת ירידת מעמדן של רשויות מחוקקות כתוצאה מאכיפה שיפוטית של מגילות זכויות וחוקות נוקשות. תהליך זה, לטענת המאמר, הפך מחלוקות פוליטיות לשאלות משפטיות ושינה הן את אופי הפוליטיקה והן את המקום שבו היא מוכרעת.",
        "מנדל חולק על הטענה שהחוקתיות החדשה היא בהכרח התפתחות דמוקרטית. הוא מציג אותה כמהלך הפועל נגד הדמוקרטיה הייצוגית, משום שהוא מרחיק הכרעות ממוסדות שנבחרים בידי הציבור ומעביר אותן לזירה שיפוטית.",
        "הביקורת המוסדית קשורה גם לביקורת חברתית: המאמר טוען שהחוקתיות החדשה נועדה להגן על אוליגרכיה של קניין פרטי מפני הסיכון שמציבים מוסדות ייצוגיים הנבחרים בידי ציבור רחב שאינו מחזיק באותו כוח רכושי."
      ]
    },
    {
      "headingHe": "מדוע הגנה על זכויות מעלה שאלת לגיטימציה?",
      "paragraphsHtml": [
        "המאמר טוען שיש לבחון את החוקתיות החדשה גם דרך שאלת הסמכות. הגנה על זכויות היא מטרה דמוקרטית חשובה, אך בישראל היא התפתחה בתוך מסגרת חוקתית חלקית, ללא רגע מכונן מוסכם וברור.",
        "לכן הוויכוח סביב בית המשפט אינו רק מאבק בין זכויות לרוב. הוא גם מאבק על הדרך שבה חברה דמוקרטית יוצרת כללים עליונים ומעניקה להם תוקף ציבורי. הנקודה החשובה היא שהדיון נשאר מחובר למקרה ולמנגנון שהמאמר מנתח, ולא לטענה כללית בלבד על דמוקרטיה."
      ]
    },
    {
      "headingHe": "מה חסר כאשר אין רגע חוקתי מוסכם?",
      "paragraphsHtml": [
        "המאמר מסייע להבין מדוע הדיון הישראלי בביקורת שיפוטית נשאר טעון כל כך. הוא נוגע לזכויות, אך גם לשאלת מקור הכוח החוקתי ולחלוקת הסמכות בין מוסדות נבחרים ושופטים.",
        "בסיכום האתר חשוב להציג את המאמר כניתוח של לגיטימציה חוקתית, לא רק כהצדקה או התנגדות כללית לבית המשפט. הנקודה החשובה היא שהדיון נשאר מחובר למקרה ולמנגנון שהמאמר מנתח, ולא לטענה כללית בלבד על דמוקרטיה."
      ]
    },
    {
      "headingHe": "איפה הכנסת, הממשלה ובית המשפט מתנגשים?",
      "paragraphsHtml": [
        "המאמר בוחן את עליית החוקתיות החדשה בישראל בעקבות חוקי היסוד של שנות התשעים והתחזקות הביקורת השיפוטית. הוא שואל האם המהלך מעמיק הגנה על זכויות או מעביר כוח פוליטי משמעותי מבוחרים ונציגים אל בית המשפט.",
        "Sources support a critique of Israel's new constitutionalism and its relationship to democratic decision-making, judicial power, and Basic Laws."
      ]
    },
    {
      "headingHe": "איך המאמר מסבך את הוויכוח על ביקורת שיפוטית?",
      "paragraphsHtml": [
        "החשיבות הפרשנית של המקור היא בכך שהוא מעביר את הדיון מן הסיסמה הכללית אל המנגנונים המוסדיים, המשפטיים או החברתיים שבהם הדמוקרטיה נבחנת בפועל. לכן כדאי לקרוא אותו כמקור שמחדד לא רק עמדה נורמטיבית, אלא גם את הדרך שבה מושגים כמו שוויון, זכויות, ייצוג, סמכות ואחריות ציבורית מקבלים צורה מוסדית קונקרטית.",
        "המאמר בוחן אם החוקתיות החדשה בישראל מאזנת בין הגנת זכויות לבין לגיטימציה דמוקרטית. הנקודה החשובה היא שהדיון נשאר מחובר למקרה ולמנגנון שהמאמר מנתח, ולא לטענה כללית בלבד על דמוקרטיה."
      ]
    },
    {
      "headingHe": "מה הוא מלמד על חוקתיות בלי חוקה שלמה?",
      "paragraphsHtml": [
        "החשיבות הפרשנית של המקור היא בכך שהוא מעביר את הדיון מן הסיסמה הכללית אל המנגנונים המוסדיים, המשפטיים או החברתיים שבהם הדמוקרטיה נבחנת בפועל. לכן כדאי לקרוא אותו כמקור שמחדד לא רק עמדה נורמטיבית, אלא גם את הדרך שבה מושגים כמו שוויון, זכויות, ייצוג, סמכות ואחריות ציבורית מקבלים צורה מוסדית קונקרטית."
      ]
    }
  ],
  "layout": "paper",
  "title": "החוקתיות החדשה בישראל ושאלת הדמוקרטיה",
  "permalink": "/democracy_new_constitutionalism_israel_summary_he.html",
  "paper_url": "democracy_new_constitutionalism_israel_summary_he.html",
  "sortKey": 202605210020
}
---
