---
{
  "version": 1,
  "file": "democracy_judicial_rhetoric_government_lawyers_and_human_rights_the_case_of_the_israeli_high_court_of_ju_summary_he.html",
  "slug": "democracy_judicial_rhetoric_government_lawyers_and_human_rights_the_case_of_the_israeli_high_court_of_ju_summary_he",
  "order": 64,
  "titleHe": "מה הקשר בין שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Yoav Dotan",
  "paperTitle": "Judicial Rhetoric, Government Lawyers, and Human Rights: The Case of the Israeli High Court of Justice during the Intifada",
  "authorsHtml": "<a href=\"https://law.huji.ac.il/people/%D7%99%D7%95%D7%90%D7%91-%D7%93%D7%95%D7%AA%D7%9F\" target=\"_blank\" rel=\"noopener noreferrer\">Yoav Dotan</a>",
  "journal": "Law & Society Review",
  "dateText": "פורסם: 1999",
  "doiUrl": "https://doi.org/10.2307/3115167",
  "doiLabel": "https://doi.org/10.2307/3115167",
  "authors": [
    {
      "@type": "Person",
      "name": "יואב דותן",
      "url": "https://law.huji.ac.il/people/%D7%99%D7%95%D7%90%D7%91-%D7%93%D7%95%D7%AA%D7%9F"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yoav Dotan",
      "url": "https://law.huji.ac.il/people/%D7%99%D7%95%D7%90%D7%91-%D7%93%D7%95%D7%AA%D7%9F"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_judicial_rhetoric_government_lawyers_and_human_rights_the_case_of_the_israeli_high_court_of_ju_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "זכויות אדם הן זכויות בסיסיות שמיוחסות לכל אדם בשל אנושיותו, כגון חירות, שוויון, כבוד, הליך הוגן וחופש ביטוי. בהקשר של המאמר, המושג מסייע לבדוק אם מוסדות המדינה מגינים על יחידים וקבוצות גם כאשר הדבר מגביל כוח שלטוני או רוב פוליטי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "מחקרים רבים מצביעים על כך שבתי משפט אינם מצליחים להגן על זכויות הפרט מאחר שהם תומכים ומקיימים שיטות דיכוי של המדינה בתקופות חירום או עימות. המאמר טוען כי חקר ההסדרים מחוץ לבית המשפט וההשוואה בין תוצאות ההסדרים לבין הרטוריקה השיפוטית הם מפתח להבנת התנהגות בתי המשפט בעתות משבר לאומי. טיעון זה מודגם בתיאור מקרה של בית המשפט הגבוה לצדק הישראלי בתקופת האינתיפאדה הפלסטינית."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין שפת המשפטנים הממשלתיים והגנת זכויות האדם בבג״ץ לבין שלטון החוק בישראל?",
  "permalink": "/democracy_judicial_rhetoric_government_lawyers_and_human_rights_the_case_of_the_israeli_high_court_of_ju_summary_he.html",
  "paper_url": "democracy_judicial_rhetoric_government_lawyers_and_human_rights_the_case_of_the_israeli_high_court_of_ju_summary_he.html",
  "sortKey": 202605210229
}
---
