---
{
  "version": 1,
  "file": "democracy_jewish_and_israeli_law_an_introduction_summary_he.html",
  "slug": "democracy_jewish_and_israeli_law_an_introduction_summary_he",
  "order": 124,
  "titleHe": "מה הקשר בין הקשרים בין משפט יהודי למשפט ישראלי לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית הקשרים בין משפט יהודי למשפט ישראלי, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית הקשרים בין משפט יהודי למשפט ישראלי דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Shimon Shetreet; Walter Homolka",
  "paperTitle": "Jewish and Israeli Law - An Introduction",
  "authorsHtml": "<a href=\"https://en.law.huji.ac.il/people/shimon-shetrit\" target=\"_blank\" rel=\"noopener noreferrer\">Shimon Shetreet</a>; <a href=\"https://orcid.org/0000-0003-1021-2341\" target=\"_blank\" rel=\"noopener noreferrer\">Walter Homolka</a>",
  "journal": "De Gruyter",
  "dateText": "פורסם: 2017",
  "doiUrl": "https://doi.org/10.1515/9783899497946",
  "doiLabel": "https://doi.org/10.1515/9783899497946",
  "authors": [
    {
      "@type": "Person",
      "name": "Shimon Shetreet",
      "url": "https://en.law.huji.ac.il/people/shimon-shetrit"
    },
    {
      "@type": "Person",
      "name": "Walter Homolka",
      "url": "https://orcid.org/0000-0003-1021-2341"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Shimon Shetreet",
      "url": "https://en.law.huji.ac.il/people/shimon-shetrit"
    },
    {
      "@type": "Person",
      "name": "Walter Homolka",
      "url": "https://orcid.org/0000-0003-1021-2341"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "הקשרים בין משפט יהודי למשפט ישראלי",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_jewish_and_israeli_law_an_introduction_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על הקשרים בין משפט יהודי למשפט ישראלי",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית הקשרים בין משפט יהודי למשפט ישראלי דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית הקשרים בין משפט יהודי למשפט ישראלי, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: הוא נותן ניתוח מפורט של המשפט הציבורי והפרטי העכשווי במדינת ישראל, כמו גם התרבות המשפטית של ישראל, שיטת הממשל שלה ותפקידי המוסדות הדמוקרטיים שלה: הרשות המבצעת, הפרלמנט והרשות השופטת. הספר בוחן סוגיות של שואה, משפט ודת, חוקה ושוויון.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת הקשרים בין משפט יהודי למשפט ישראלי לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הקשרים בין משפט יהודי למשפט ישראלי משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין הקשרים בין משפט יהודי למשפט ישראלי לבין שלטון החוק בישראל?",
  "permalink": "/democracy_jewish_and_israeli_law_an_introduction_summary_he.html",
  "paper_url": "democracy_jewish_and_israeli_law_an_introduction_summary_he.html",
  "sortKey": 202605210169
}
---
