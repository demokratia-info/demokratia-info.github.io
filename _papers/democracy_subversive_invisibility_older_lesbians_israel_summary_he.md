---
{
  "version": 1,
  "file": "democracy_subversive_invisibility_older_lesbians_israel_summary_he.html",
  "slug": "democracy_subversive_invisibility_older_lesbians_israel_summary_he",
  "titleHe": "אי-נראות של לסביות מבוגרות בישראל יכולה להיות גם פגיעה וגם בחירה",
  "subtitleHe": "ראיונות עם 21 נשים בגילאי 50-79 מציעים קריאה מורכבת של גיל, מגדר, מיניות והתנגדות חלשה.",
  "descriptionHe": "המאמר בוחן כיצד לסביות מבוגרות בישראל חוות אי־נראות חברתית, וכיצד חלקן משתמשות בה כמרחב של חירות, קבלה עצמית והתנגדות יומיומית.",
  "summaryHe": "סקטורה־ז׳וקוביץ׳ ואיילון מנתחות ראיונות חצי־מובנים עם 21 נשים לסביות בישראל בגילאי 50 עד 79. המאמר מציע להבין אי־נראות לא רק כמחיקה הנוצרת מצטלבות של גילנות, סקסיזם והומופוביה, אלא גם כמצב שחלק מהמשתתפות הופכות למקור של פעולה. הוא מזהה שני ממדים: אי־נראות בין־אישית כמקור לחירות יחסית מנורמות חברתיות, ונראות תוך־אישית כתהליך של קבלה בזהות המינית. בכך המאמר מציג אי־נראות חתרנית כאסטרטגיה של התמודדות והתנגדות חלשה, בלי להתעלם מההדרה שמייצרת אותה.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Olga Edyta Saktura-Żukowicz, Liat Ayalon",
  "paperTitle": "‘I am so happy to be invisible’: subversive invisibility of older lesbians in Israel",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-4976-5894\" target=\"_blank\" rel=\"noopener noreferrer\">Olga Edyta Saktura-Żukowicz</a>; <a href=\"https://social-work.biu.ac.il/en/Ayalon_Liat_En\" target=\"_blank\" rel=\"noopener noreferrer\">Liat Ayalon</a>",
  "journal": "Ageing and Society 46, Article e42",
  "dateText": "פורסם: 2026",
  "doiUrl": "https://doi.org/10.1017/s0144686x26100713",
  "doiLabel": "https://doi.org/10.1017/s0144686x26100713",
  "authors": [
    {
      "@type": "Person",
      "name": "Olga Edyta Saktura-Żukowicz",
      "url": "https://orcid.org/0000-0002-4976-5894"
    },
    {
      "@type": "Person",
      "name": "Liat Ayalon",
      "url": "https://social-work.biu.ac.il/en/Ayalon_Liat_En"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Olga Edyta Saktura-Żukowicz",
      "url": "https://orcid.org/0000-0002-4976-5894"
    },
    {
      "@type": "Person",
      "name": "Liat Ayalon",
      "url": "https://social-work.biu.ac.il/en/Ayalon_Liat_En"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "לסביות מבוגרות",
    "אי־נראות",
    "אפליה מצטלבת",
    "זהות מינית",
    "התנגדות חלשה",
    "ישראל"
  ],
  "image": {
    "src": "assets/article-images/real_photo_689_minorities_democracy_subversive_invisibility_older_lesbians_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לאי־נראות של לסביות מבוגרות בישראל יכולה להפוך למרחב חירות",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Golan2072",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Rahat_Beighborhood_19_1.JPG",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/8/80/Rahat_Beighborhood_19_1.JPG",
    "fileTitle": "Rahat Beighborhood 19 1.JPG",
    "sourceDimensions": "960x720",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:minorities",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-06-15",
  "dateModified": "2026-07-07",
  "lastUpdatedHe": "7 ביולי 2026",
  "oneLinerHtml": "עבור לסביות מבוגרות בישראל, אי־נראות יכולה להיות גם תוצאה של הדרה וגם כלי יומיומי לחירות, קבלה עצמית והתנגדות.",
  "sections": [
    {
      "headingHe": "איך אי-נראות יכולה להיות גם הדרה וגם טקטיקה?",
      "paragraphsHtml": [
        "המאמר מתמקד בלסביות מבוגרות בישראל, קבוצה שנמצאת בצומת של גיל, מגדר, מיניות, משפחה ונורמות ציבוריות.",
        "הוא מבקש להבין את חווייתן לא רק דרך קטגוריות של אפליה, אלא דרך האופן שבו הן עצמן מתארות את המקום שהן תופסות, את מה שהן חושפות ואת מה שהן בוחרות להשאיר מחוץ למבט הציבורי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המחקר מבוסס על ראיונות חצי־מובנים עם 21 נשים לסביות בישראל בגילאי 50 עד 79. הוא בוחן כיצד המשתתפות מתארות נראות ואי־נראות בהקשרים חברתיים שונים, וכיצד גיל, מגדר וזהות מינית מצטלבים בעיצוב חוויות של הדרה וסוכנות.",
        "הניתוח התמטי זיהה נושא מרכזי של אי־נראות חתרנית: מצב שבו אדם נעשה בולט דווקא דרך התנהגויות לא צפויות שאינן מיוחסות בדרך כלל למעמדו החברתי, וכך יוצר בלבול או הפתעה אצל הסובבים. המאמר מפרק זאת לשני ממדים: אי־נראות בין־אישית כמקור לחירות אישית מנורמות חברתיות, ונראות תוך־אישית כמקדמת קבלה של הזהות המינית.",
        "המאמר מפרש את העדויות דרך תאוריות של הצטלבות, פסיכולוגיה של אי־נראות והתנגדות חלשה. הממצא המרכזי הוא שלסביות מבוגרות אינן רק נפגעות מאי־נראות חברתית; חלקן משתמשות בה באופנים מרדניים וחתרניים כדי להתמודד עם ההשלכות של הדרה, לשמור על מרחב תמרון ולנסח את זהותן בתנאים שלהן."
      ]
    },
    {
      "headingHe": "מה נשים לסביות מבוגרות עושות עם מרחב הפרטיות?",
      "paragraphsHtml": [
        "הטענה המרכזית היא שאי-נראות יכולה לפעול בשני כיוונים. היא עלולה למחוק נשים מן השיחה הציבורית, להסתיר צרכים ולהעמיק תחושה של שוליות בגיל מבוגר.",
        "אבל אותה אי-נראות יכולה גם להפוך לטקטיקה שמאפשרת להן להתרחק מציפיות חברתיות, לשמור על אינטימיות ולנוע במרחבים שונים בביטחון יחסי. לכן המאמר משתמש במושג “חתרנית” כדי להצביע על שימוש פעיל במצב שנראה מבחוץ כפסיבי."
      ]
    },
    {
      "headingHe": "למה זקנה להטבית אינה רק סיפור של פגיעות?",
      "paragraphsHtml": [
        "המאמר אינו מציג בחירה באי-נראות כחירות מלאה. האפשרות לבחור מתי להיראות ומתי לא תלויה בתנאים חברתיים, משפחתיים וכלכליים, וביחס של הסביבה לזקנה וללהט״ביות.",
        "דווקא משום כך החוויה מורכבת: מה שנראה כהיעדר הכרה עשוי להיות גם מנגנון הגנה, ומה שנראה כפרטיות עשוי להיווצר מתוך חשש, עייפות או ניסיון חיים של פיקוח ושיפוט."
      ]
    },
    {
      "headingHe": "איפה בחירה ואילוץ מתקיימים יחד?",
      "paragraphsHtml": [
        "המחקר מרחיב את הדיון על שוויון וזכויות להט״ב מעבר לשאלת ההכרה הפורמלית. הוא מזכיר שחירות אזרחית נמדדת גם ביכולת של אנשים מבוגרים לחיות את זהותם בלי להיחשף תמיד לפיקוח, בושה או הסבר כפוי.",
        "במובן הזה, דמוקרטיה אינה רק זכות להיראות במרחב הציבורי. היא גם האפשרות לשלוט במידת הנראות, בפרטיות ובאופן שבו זהות מוצגת לאחרים."
      ]
    },
    {
      "headingHe": "מה המאמר משנה בשיחה על זכויות ונראות?",
      "paragraphsHtml": [
        "במוקד הקריאה עומדת אי-נראות חתרנית של לסביות מבוגרות בישראל: לא כתגית רחבה, אלא כמקרה שמצריך הבחנה בין טענה, ראיות ומשמעות ציבורית.",
        "הניסוח נשאר צמוד למסגרת המקור: הוא מבליט את הטענה המרכזית ואת משמעותה הציבורית, אך נמנע מהוספת פרטים שלא הופיעו בתיעוד הקיים."
      ]
    }
  ],
  "layout": "paper",
  "title": "אי-נראות של לסביות מבוגרות בישראל יכולה להיות גם פגיעה וגם בחירה",
  "permalink": "/democracy_subversive_invisibility_older_lesbians_israel_summary_he.html",
  "paper_url": "democracy_subversive_invisibility_older_lesbians_israel_summary_he.html",
  "sortKey": 202606150002
}
---
