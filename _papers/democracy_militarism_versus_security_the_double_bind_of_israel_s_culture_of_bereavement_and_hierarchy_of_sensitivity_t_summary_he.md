---
{
  "version": 1,
  "file": "democracy_militarism_versus_security_the_double_bind_of_israel_s_culture_of_bereavement_and_hierarchy_of_sensitivity_t_summary_he.html",
  "slug": "democracy_militarism_versus_security_the_double_bind_of_israel_s_culture_of_bereavement_and_hierarchy_of_sensitivity_t_summary_he",
  "order": 243,
  "titleHe": "מיליטריזם מול ביטחון בתרבות השכול הישראלית",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי, וממקם את הדיון בתוך שאלות של דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי דרך עדשה של דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
  "authorsCardHe": "Udi Lebel",
  "paperTitle": "Militarism versus Security? The Double-Bind of Israel's Culture of Bereavement and Hierarchy of Sensitivity to Loss",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-0964-019X\" target=\"_blank\" rel=\"noopener noreferrer\">Udi Lebel</a>",
  "journal": "Mediterranean Politics",
  "dateText": "פורסם: 2011",
  "doiUrl": "https://doi.org/10.1080/13629395.2011.613670",
  "doiLabel": "https://doi.org/10.1080/13629395.2011.613670",
  "authors": [
    {
      "@type": "Person",
      "name": "Udi Lebel",
      "url": "https://orcid.org/0000-0002-0964-019X"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Udi Lebel",
      "url": "https://orcid.org/0000-0002-0964-019X"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דעת קהל",
    "קיטוב",
    "מחאה",
    "בחירות",
    "אלימות פוליטית",
    "ביטחון",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_militarism_versus_security_the_double_bind_of_israel_s_culture_of_bereavement_and_hierarchy_of_sensitivity_t_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי ומראה מדוע הדיון קשור לדעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי, וממקם את הסוגיה בתוך הדיון הרחב על דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מאמר זה בוחן את ההשלכות של נגזרת של מיליטריזם תרבותי בישראל - הפאניקה שמעוררים נפגעים צבאיים - ומראה כי היא מערערת את ערך הביטחון ותוך כדי כך מונעת מהצבא לספק ביטחון לאזרחים. אוריינטציה זו חדרה לדרגים המדיניים והצבאיים הגבוהים ביותר בישראל וחושפת היפוך ערכים יוצא דופן בקרב האליטה הישראלית, שבאופן מסורתי קידמה גבורה והקרבה במילוי תפקידה. בכל אחת מהפרדיגמות, המאמר מצביע על תרבות ולא על אינטרסים גיאו-אסטרטגיים כגורם הקובע העיקרי בגיבוש מדיניות והתנהגות צבאית.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>ביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בביטחון, כיבוש והגבלות חוקתיות על כוח ציבורי להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "מיליטריזם מול ביטחון בתרבות השכול הישראלית",
  "permalink": "/democracy_militarism_versus_security_the_double_bind_of_israel_s_culture_of_bereavement_and_hierarchy_of_sensitivity_t_summary_he.html",
  "paper_url": "democracy_militarism_versus_security_the_double_bind_of_israel_s_culture_of_bereavement_and_hierarchy_of_sensitivity_t_summary_he.html",
  "sortKey": 202605210243
}
---
