---
{
  "version": 1,
  "file": "democracy_suspending_voluntary_reserve_service_israeli_military_ethics_summary_he.html",
  "slug": "democracy_suspending_voluntary_reserve_service_israeli_military_ethics_summary_he",
  "titleHe": "השעיית שירות מילואים התנדבותי מציבה דילמה אתית־דמוקרטית",
  "subtitleHe": "",
  "descriptionHe": "המאמר דן בשאלות אתיות שהתעוררו כאשר אנשי מילואים מתנדבים בישראל השעו שירות מול מה שנתפס כניסיון לשנות את אופייה הדמוקרטי של המדינה.",
  "summaryHe": "המאמר בוחן האם השעיית שירות מילואים התנדבותי יכולה להיות מוצדקת מוסרית, אתית ודמוקרטית כאשר המשרתים רואים במהלכי הממשלה פגיעה ביסודות המשטר הדמוקרטי.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Asa Kasher",
  "paperTitle": "Suspending Voluntary Reserve Service: New Questions in Israeli Military Ethics",
  "authorsHtml": "<a href=\"https://english.tau.ac.il/profile/kasher\" target=\"_blank\" rel=\"noopener noreferrer\">Asa Kasher</a>",
  "journal": "Conatus, 8(2), 241-256",
  "dateText": "פורסם: 31.12.2023",
  "sourcePublishedDate": "2023-12-31",
  "doiUrl": "https://doi.org/10.12681/cjp.35404",
  "doiLabel": "https://doi.org/10.12681/cjp.35404",
  "authors": [
    {
      "@type": "Person",
      "name": "Asa Kasher",
      "url": "https://english.tau.ac.il/profile/kasher"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Asa Kasher",
      "url": "https://english.tau.ac.il/profile/kasher"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "אתיקה צבאית",
    "מילואים",
    "מחאה דמוקרטית",
    "חובה אזרחית",
    "ישראל",
    "שירות התנדבותי"
  ],
  "image": {
    "src": "assets/article-images/real_photo_692_security_democracy_suspending_voluntary_reserve_service_israeli_military_ethics_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור להשעיית שירות מילואים התנדבותי מציבה דילמה אתית־דמוקרטית",
    "fitness": "high",
    "license": "CC BY-SA 2.0",
    "creator": "Israel Defense Forces",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_Israel_Defense_Forces_-_Package_of_Nails_to_be_Used_in_Bomb.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/b/bf/Flickr_-_Israel_Defense_Forces_-_Package_of_Nails_to_be_Used_in_Bomb.jpg",
    "fileTitle": "Flickr - Israel Defense Forces - Package of Nails to be Used in Bomb.jpg",
    "sourceDimensions": "1823x1169",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:security",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/2.0/"
  },
  "datePublished": "2026-06-07",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר בוחן את המתח בין שירות מילואים התנדבותי לבין אחריות אזרחית כאשר משרתים סבורים שהצבא עלול להפוך לצבא של משטר שאינו דמוקרטי.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>אתיקה צבאית</strong> עוסקת בכללים ובשיקולים שמנחים שימוש בכוח, נאמנות למסגרת וחובות של משרתים. במאמר היא נבחנת דרך מקרה מיוחד שבו השירות הוא התנדבותי והמחאה מוצגת כהגנה על יסודות דמוקרטיים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מציב את הסוגיה על רקע הממשלה שהוקמה בישראל לאחר בחירות סוף 2022 והציגה ״רפורמה משפטית״ שנועדה, לפי התיאור במאמר, להחליש מאוד את מעמדם של שומרי הסף של המשטר הדמוקרטי: בית המשפט העליון והיועץ המשפטי לממשלה. ההתנגשות הפוליטית מתוארת כעימות בין ממשלה שנתפסה כמנסה לשנות את אופייה הדמוקרטי של המדינה לבין אופוזיציה ציבורית שביקשה להגן על מערכת המשפט ועל יסודות דמוקרטיים אחרים.",
        "המאמר מתמקד בקבוצת משרתי מילואים שהתנדבותם נמשכה מעבר לשירות מילואים רגיל, ובהם טייסים וקציני יחידות מיוחדות. מול המשבר הפוליטי, רבים מהם השעו את שירותם ההתנדבותי או פרשו, בעיקר ביחידות כמו חיל האוויר וחיל המודיעין, והדבר העלה שאלה אם החלטה כזאת יכולה להיות מוצדקת מבחינה מוסרית, אתית ודמוקרטית.",
        "הניתוח קושר את הדילמה גם לקוד האתי של צה״ל, ״רוח צה״ל״, שבו ישראל מזוהה כמדינה יהודית ודמוקרטית. המאמר מדגיש במיוחד את ערך כבוד האדם, שממנו ניתן לגזור עקרונות של שוויון, זכויות אדם ואזרח והגנה מוסדית עליהם, ולכן שאלת ההשעיה אינה מוצגת רק כפעולת מחאה פוליטית אלא כשאלה על גבולות המחויבות האתית של משרתים במסגרת צבאית של מדינה דמוקרטית."
      ]
    },
    {
      "headingHe": "מה הדילמה שהמאמר מציב?",
      "paragraphsHtml": [
        "הדילמה היא האם משרתים מתנדבים רשאים להשעות שירות כאשר הם מאמינים שהממשלה פוגעת באופייה הדמוקרטי של המדינה. זו אינה רק שאלה משפטית אלא גם מוסרית, אתית ודמוקרטית."
      ]
    },
    {
      "headingHe": "מה מיוחד בשירות מילואים התנדבותי?",
      "paragraphsHtml": [
        "שירות התנדבותי שונה משירות חובה רגיל משום שהוא נשען על מחויבות מתמשכת מעבר לפקודה נקודתית. כאשר המחויבות הזאת נפסקת או מושעית, עולה שאלה קשה על גבולות הנאמנות למדינה, לצבא ולמשטר הדמוקרטי."
      ]
    },
    {
      "headingHe": "למה ההקשר של מחאה דמוקרטית חשוב?",
      "paragraphsHtml": [
        "המאמר מתייחס לעימות שבו הממשלה נתפסה בידי מתנגדיה כמי שמנסה לשנות את טבע המדינה הדמוקרטית. לכן ההשעיה אינה מוצגת רק כאינטרס אישי, אלא כפעולה שמבקשת להגן על מערכת מוסדית רחבה."
      ]
    },
    {
      "headingHe": "איך זה קשור ליחסי צבא־חברה?",
      "paragraphsHtml": [
        "יחסי צבא־חברה בדמוקרטיה תלויים בכך שהצבא נשאר כפוף לסמכות אזרחית לגיטימית. כאשר חלק מן המשרתים מערערים על הלגיטימיות הדמוקרטית של השינויים הפוליטיים, הגבול בין ציות, מחאה ואחריות אזרחית נעשה טעון במיוחד."
      ]
    },
    {
      "headingHe": "מה הסיכון הדמוקרטי שבהשעיית שירות?",
      "paragraphsHtml": [
        "הסיכון הוא שיחידות קריטיות או קבוצות מקצועיות יקבלו כוח וטו בלתי פורמלי על הכרעות פוליטיות. גם כאשר המניע הוא הגנה על דמוקרטיה, פעולה כזאת עלולה לשנות את מאזן הכוח בין אזרחים, נבחרים וצבא."
      ]
    },
    {
      "headingHe": "מה הסיכון הדמוקרטי באיסור מוחלט על מחאה כזאת?",
      "paragraphsHtml": [
        "איסור מוחלט על כל השעיה או פרישה מתעלם מכך שמשרתים הם גם אזרחים בעלי מצפון ואחריות ציבורית. אם הם מאמינים שהמסגרת הדמוקרטית עצמה נפגעת, התעלמות מן הטענה עלולה לצמצם את האתיקה הצבאית לציות בלבד."
      ]
    },
    {
      "headingHe": "איזו הבחנה חשובה עולה מן המאמר?",
      "paragraphsHtml": [
        "הבחנה חשובה היא בין סירוב לפקודה מבצעית לבין השעיית שירות התנדבותי עתידי. ההבחנה אינה פותרת את הדילמה, אך היא מבהירה מדוע המקרה דורש ניתוח נפרד ולא רק שימוש בקטגוריות רגילות של סרבנות."
      ]
    },
    {
      "headingHe": "איך המאמר רלוונטי לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק בדמוקרטיה דורש שגם הצבא וגם הממשלה יפעלו בתוך מגבלות נורמטיביות. כאשר מחאה צבאית־אזרחית מתעוררת סביב שינוי חוקתי, השאלה היא כיצד להגן על המגבלות בלי לפרק את עקרון הכפיפות האזרחית."
      ]
    },
    {
      "headingHe": "איזו שאלה נשארת פתוחה?",
      "paragraphsHtml": [
        "שאלה פתוחה היא אילו תנאים צריכים להתקיים כדי שהשעיית שירות התנדבותי תיחשב מוצדקת ולא רק לחץ פוליטי. תשובה רצינית צריכה לכלול את חומרת הפגיעה הדמוקרטית, טיב השירות, הסיכון הביטחוני והאפשרות למחאה אזרחית חלופית."
      ]
    },
    {
      "headingHe": "מהי תרומת המאמר לדיון הישראלי?",
      "paragraphsHtml": [
        "התרומה היא ניסוח הסוגיה כשאלה אתית מורכבת ולא כהכרעה סיסמתית בעד או נגד המוחים. בכך המאמר מאפשר דיון ציבורי שמכיר גם בחשיבות הביטחון וגם בחשיבות ההגנה על מוסדות דמוקרטיים."
      ]
    }
  ],
  "layout": "paper",
  "title": "השעיית שירות מילואים התנדבותי מציבה דילמה אתית־דמוקרטית",
  "permalink": "/democracy_suspending_voluntary_reserve_service_israeli_military_ethics_summary_he.html",
  "paper_url": "democracy_suspending_voluntary_reserve_service_israeli_military_ethics_summary_he.html",
  "sortKey": 202606070005
}
---
