---
{
  "version": 1,
  "file": "democracy_religious_liberty_in_the_state_of_israel_summary_he.html",
  "slug": "democracy_religious_liberty_in_the_state_of_israel_summary_he",
  "order": 82,
  "titleHe": "חירות דתית במדינת ישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית חירות דתית במדינת ישראל, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית חירות דתית במדינת ישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Natan Lerner",
  "paperTitle": "Religious Liberty in the State of Israel",
  "authorsHtml": "<a href=\"https://en-law.tau.ac.il/profile/lerner\" target=\"_blank\" rel=\"noopener noreferrer\">Natan Lerner</a>",
  "journal": "Religion and Equality Law",
  "dateText": "פורסם: 2017",
  "doiUrl": "https://doi.org/10.4324/9781315088945-9",
  "doiLabel": "https://doi.org/10.4324/9781315088945-9",
  "authors": [
    {
      "@type": "Person",
      "name": "Natan Lerner",
      "url": "https://en-law.tau.ac.il/profile/lerner"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Natan Lerner",
      "url": "https://en-law.tau.ac.il/profile/lerner"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "חירות דתית במדינת ישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_religious_liberty_in_the_state_of_israel_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על חירות דתית במדינת ישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית חירות דתית במדינת ישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית חירות דתית במדינת ישראל, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: גבולות חופש הדת במדינת ישראל קשים לרוב לתיחום. לא פחות קשה להמשגה הוא כיצד הסדרת חופש הדת בישראל משתלבת במסגרת המקובלת במדינה של יחסי דת ומדינה. 1 היא גם לא אימצה שיטה של ​​הפרדת דת ומדינה או העניקה שוויון מלא לדת של רוב האוכלוסייה ולאמונות של אחרים.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת חירות דתית במדינת ישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר חירות דתית במדינת ישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "חירות דתית במדינת ישראל",
  "permalink": "/democracy_religious_liberty_in_the_state_of_israel_summary_he.html",
  "paper_url": "democracy_religious_liberty_in_the_state_of_israel_summary_he.html",
  "sortKey": 202605210211
}
---
