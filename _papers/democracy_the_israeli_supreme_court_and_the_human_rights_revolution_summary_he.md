---
{
  "version": 1,
  "file": "democracy_the_israeli_supreme_court_and_the_human_rights_revolution_summary_he.html",
  "slug": "democracy_the_israeli_supreme_court_and_the_human_rights_revolution_summary_he",
  "order": 12,
  "titleHe": "בית המשפט העליון כמהנדס סדר יום של זכויות אדם",
  "subtitleHe": "",
  "descriptionHe": "הספר בוחן את בית המשפט העליון בישראל כשחקן הקובע סדר יום בתחום זכויות האדם, תוך שילוב ניתוח אמפירי כמותי ומקרי בוחן איכותניים.",
  "summaryHe": "הספר מציג את בית המשפט העליון לא רק כערכאה המכריעה בסכסוכים, אלא כמוסד המשפיע על סדר היום הציבורי והנורמטיבי של זכויות אדם בישראל. הניתוח קושר את התנהגות בית המשפט לשני משתנים מרכזיים: אי־משילות של המערכת הפוליטית ותרבות פוליטית חלופית. בתוך המודל הזה, החלטות בג״ץ מבטאות את ערכי המדינה, מעצבות כיוון חינוכי־נורמטיבי ומשקפות את אופייה הדמוקרטי של ישראל ביחס לזכויות אדם.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "אסף מידני",
  "paperTitle": "The Israeli Supreme Court and the Human Rights Revolution",
  "authorsHtml": "<a href=\"https://www.int.mta.ac.il/faculty-members/prof.-meydani-assaf\" target=\"_blank\" rel=\"noopener noreferrer\">Assaf Meydani</a>",
  "journal": "Cambridge University Press",
  "dateText": "פורסם: 25.07.2011",
  "doiUrl": "https://doi.org/10.1017/cbo9780511998423",
  "doiLabel": "https://doi.org/10.1017/cbo9780511998423",
  "authors": [
    {
      "@type": "Person",
      "name": "אסף מידני",
      "url": "https://www.int.mta.ac.il/faculty-members/prof.-meydani-assaf"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Assaf Meydani",
      "url": "https://www.int.mta.ac.il/faculty-members/prof.-meydani-assaf"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חוקתיות ושלטון החוק",
    "דמוקרטיה ליברלית",
    "שלטון החוק",
    "זכויות",
    "מוסדות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_757_knesset_democracy_the_israeli_supreme_court_and_the_human_rights_revolution_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על בית משפט, חוקה ושלטון החוק",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Tsvika Israeli",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_Government_Press_Office_(GPO)_-_The_Knesset_Building_in_Jerusalem.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/9/9c/Flickr_-_Government_Press_Office_%28GPO%29_-_The_Knesset_Building_in_Jerusalem.jpg",
    "fileTitle": "Flickr - Government Press Office (GPO) - The Knesset Building in Jerusalem.jpg",
    "sourceDimensions": "3072x2048",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_broad_contextual_match_review",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:knesset",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "הספר מציג את בג״ץ כקובע סדר יום שמתרגם חולשות פוליטיות ותרבות מוסדית להכרעות זכויות אדם.",
  "sections": [
    {
      "headingHe": "איך בית המשפט העליון נעשה שחקן של זכויות אדם?",
      "paragraphsHtml": [
        "הספר בוחן את עליית בית המשפט העליון כמוסד מרכזי בהגנה על זכויות אדם בישראל ואת הדרך שבה עתירות, פסיקות ונגישות שיפוטית מעצבות את גבולות הכוח השלטוני.",
        "הספר מנתח את בית המשפט העליון כשחקן שמזהה בעיות מדיניות, פותח ערוצי נגישות לעותרים ומתרגם מחלוקות חברתיות לשפה חוקתית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "הספר מתאר את בית המשפט העליון בישראל כ־<strong>קובע סדר יום</strong> בתחום זכויות האדם. במקום לראות בפסיקה רק תגובה לעתירות, הניתוח מייחס לבית המשפט תפקיד פעיל בעיצוב השאלות הנורמטיביות שהמערכת הפוליטית והציבורית נדרשות להתמודד איתן.",
        "המודל שמוצג בספר מדגיש שני גורמים המשפיעים על שחקנים פוליטיים בישראל: אי־משילות של המערכת הפוליטית ותרבות פוליטית חלופית. כל עוד המבנה הזה אינו משתנה משמעותית, החלטות בג״ץ מוצגות כמנגנון שמבטא את ערכי המדינה ותומך במחויבות לזכויות אדם.",
        "הספר נשען על ניתוח אמפירי כמותי לצד מקרי בוחן איכותניים. מתוך כך הוא מצביע על קשרי גומלין בין בג״ץ לבין שחקנים אחרים במערכת הפוליטית, ועל תפקיד הפסיקה בקביעת כיוון חינוכי־נורמטיבי המשקף את אופייה הדמוקרטי של ישראל בתחום זכויות האדם."
      ]
    },
    {
      "headingHe": "מה פירוש לקבוע סדר יום משפטי?",
      "paragraphsHtml": [
        "הטענה היא שבית המשפט אינו רק פוסק בסכסוכים קיימים, אלא גם קובע סדר יום: הוא מגדיר אילו תביעות הופכות לשאלות חוקתיות ומאותת לרשויות האחרות אילו סוגיות אינן יכולות להישאר מחוץ לדיון.",
        "במקום להציג את מהפכת זכויות האדם רק כמעבר משפטי פורמלי, מידני מדגיש את יחסי הגומלין בין בית המשפט, הכנסת, הממשלה וקבוצות אזרחיות."
      ]
    },
    {
      "headingHe": "כיצד עתירות מחברות חברה אזרחית ומוסדות שלטון?",
      "paragraphsHtml": [
        "הספר בוחן את עליית בית המשפט העליון כמוסד מרכזי בהגנה על זכויות אדם בישראל ואת הדרך שבה עתירות, פסיקות ונגישות שיפוטית מעצבות את גבולות הכוח השלטוני.",
        "כך מתקבלת תמונה של מוסד שיפוטי שמרחיב הגנה על זכויות, אך פועל בתוך תנאים פוליטיים, מוסדיים וציבוריים משתנים."
      ]
    },
    {
      "headingHe": "למה מהפכת זכויות האדם אינה רק חוקי יסוד?",
      "paragraphsHtml": [
        "הטענה היא שבית המשפט אינו רק פוסק בסכסוכים קיימים, אלא גם קובע סדר יום: הוא מגדיר אילו תביעות הופכות לשאלות חוקתיות ומאותת לרשויות האחרות אילו סוגיות אינן יכולות להישאר מחוץ לדיון.",
        "התרומה של המקור היא בהעברת הדיון מן הכותרת הכללית אל מנגנונים קונקרטיים: מי מגדיר את הבעיה, אילו מוסדות מקבלים סמכות, כיצד קבוצות שונות מושפעות מן ההכרעה, ומה נחשף על היחסים בין כוח, זכויות ולגיטימציה."
      ]
    },
    {
      "headingHe": "מה הדילמה הדמוקרטית של בית משפט פתוח?",
      "paragraphsHtml": [
        "הספר בוחן את עליית בית המשפט העליון כמוסד מרכזי בהגנה על זכויות אדם בישראל ואת הדרך שבה עתירות, פסיקות ונגישות שיפוטית מעצבות את גבולות הכוח השלטוני.",
        "לכן הקריאה בו חשובה לא רק להבנת המקרה הנדון, אלא גם להבנת הדרך שבה דמוקרטיה פועלת בפועל דרך חקיקה, פסיקה, חינוך, מדיניות ציבורית או פעולה אזרחית."
      ]
    }
  ],
  "layout": "paper",
  "title": "בית המשפט העליון כמהנדס סדר יום של זכויות אדם",
  "permalink": "/democracy_the_israeli_supreme_court_and_the_human_rights_revolution_summary_he.html",
  "paper_url": "democracy_the_israeli_supreme_court_and_the_human_rights_revolution_summary_he.html",
  "sortKey": 202605210042
}
---
