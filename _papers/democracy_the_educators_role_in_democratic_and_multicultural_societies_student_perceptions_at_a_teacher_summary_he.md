---
{
  "version": 1,
  "file": "democracy_the_educators_role_in_democratic_and_multicultural_societies_student_perceptions_at_a_teacher_summary_he.html",
  "slug": "democracy_the_educators_role_in_democratic_and_multicultural_societies_student_perceptions_at_a_teacher_summary_he",
  "order": 148,
  "titleHe": "תפקיד המחנך בחברות דמוקרטיות ורב־תרבותיות",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית תפקיד המחנך בחברה דמוקרטית ורב־תרבותית, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית תפקיד המחנך בחברה דמוקרטית ורב־תרבותית דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Michal Hisherik; Ilana Paul-Binyamin",
  "paperTitle": "The educator’s role in democratic and multicultural societies: student perceptions at a teacher training college in Israel",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-4017-5713\" target=\"_blank\" rel=\"noopener noreferrer\">Michal Hisherik</a>; <a href=\"https://orcid.org/0000-0001-5566-2763\" target=\"_blank\" rel=\"noopener noreferrer\">Ilana Paul-Binyamin</a>",
  "journal": "Journal for Multicultural Education",
  "dateText": "פורסם: 2024",
  "doiUrl": "https://doi.org/10.1108/jme-08-2023-0077",
  "doiLabel": "https://doi.org/10.1108/jme-08-2023-0077",
  "authors": [
    {
      "@type": "Person",
      "name": "Michal Hisherik",
      "url": "https://orcid.org/0000-0003-4017-5713"
    },
    {
      "@type": "Person",
      "name": "Ilana Paul-Binyamin",
      "url": "https://orcid.org/0000-0001-5566-2763"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Michal Hisherik",
      "url": "https://orcid.org/0000-0003-4017-5713"
    },
    {
      "@type": "Person",
      "name": "Ilana Paul-Binyamin",
      "url": "https://orcid.org/0000-0001-5566-2763"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "תפקיד המחנך בחברה דמוקרטית ורב־תרבותית",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_the_educators_role_in_democratic_and_multicultural_societies_student_perceptions_at_a_teacher_summary_he.jpg",
    "version": "2026-05-21-image-audit",
    "altHe": "איור עריכתי על תפקיד המחנך בחברה דמוקרטית ורב־תרבותית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית תפקיד המחנך בחברה דמוקרטית ורב־תרבותית דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית תפקיד המחנך בחברה דמוקרטית ורב־תרבותית, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מעבר להקניית ידע, למורים יש חשיבות מכרעת בהתמודדות עם אתגרים חברתיים כמו קיימות, דמוקרטיה ושוויון חברתי. עיצוב/מתודולוגיה/גישה מחקר זה בוחן את עמדותיהם של סטודנטים בקבוצות רוב ומיעוט במכללה ישראלית להכשרת מורים כלפי מימוש ערכים דמוקרטיים וקידום אזרחות משותפת. המדגם כלל 382 תלמידים יהודים וערבים, שענו על שאלון לגבי עמדות לגבי חינוך לדמוקרטיה וחברה משותפת, ותפיסתם את תפקידם בקידום חינוך זה.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת תפקיד המחנך בחברה דמוקרטית ורב־תרבותית לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר תפקיד המחנך בחברה דמוקרטית ורב־תרבותית משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "תפקיד המחנך בחברות דמוקרטיות ורב־תרבותיות",
  "permalink": "/democracy_the_educators_role_in_democratic_and_multicultural_societies_student_perceptions_at_a_teacher_summary_he.html",
  "paper_url": "democracy_the_educators_role_in_democratic_and_multicultural_societies_student_perceptions_at_a_teacher_summary_he.html",
  "sortKey": 202605210145
}
---
