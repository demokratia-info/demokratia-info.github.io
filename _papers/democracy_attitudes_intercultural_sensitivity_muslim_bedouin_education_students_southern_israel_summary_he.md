---
{
  "version": 1,
  "file": "democracy_attitudes_intercultural_sensitivity_muslim_bedouin_education_students_southern_israel_summary_he.html",
  "slug": "democracy_attitudes_intercultural_sensitivity_muslim_bedouin_education_students_southern_israel_summary_he",
  "titleHe": "רגישות בין־תרבותית בקרב סטודנטים בדואים לחינוך בדרום ישראל",
  "subtitleHe": "מחקר איכותני על 16 סטודנטים לחינוך בדרום ישראל מציע מודל פדגוגי להתמודדות עם מרחק, שפה ואי־שוויון.",
  "descriptionHe": "עופר גת בוחן עמדות של סטודנטים מוסלמים־בדואים לחינוך בדרום ישראל כלפי רגישות בין־תרבותית. המחקר מציב את הסטודנטים בתוך ההיסטוריה והאי־שוויון של החברה הבדואית בנגב, ובודק מה קורה כאשר הם משתתפים בקורס הכולל עבודה פדגוגית מעורבת של ערבים ויהודים.",
  "summaryHe": "המאמר הוא מחקר איכותני קטן המבוסס על רפלקציות של 16 סטודנטים בדואים בשנת הלימודים השנייה לתואר בחינוך. הסטודנטים השתתפו בקורס עם עבודה קבוצתית מעורבת וכתבו רפלקציות לאורך הסמסטר. הממצאים מציגים עמדות מורכבות: רצון להכיר את האחר ולנהל דיאלוג, לצד מצוקה, חשד, קושי בשפה העברית, מרחק תרבותי ופוליטי, ותחושה שמפגש לא מתווך עלול לייצר שיפוטיות. המאמר מציע מודל בן שישה שלבים לטיפוח אכפתיות ורגישות בין־תרבותית מתמשכת. עופר גת בוחן עמדות של סטודנטים מוסלמים־בדואים לחינוך בדרום ישראל כלפי רגישות בין־תרבותית. המחקר מציב את הסטודנטים בתוך ההיסטוריה והאי־שוויון של החברה הבדואית בנגב, ובודק מה קורה כאשר הם משתתפים בקורס הכולל עבודה פדגוגית מעורבת של ערבים ויהודים. המאמר מראה שמפגש ערבי־יהודי באקדמיה אינו מספיק כשלעצמו; הוא דורש תיווך, הכשרה ועבודה מודעת עם זהות וקונפליקט. החשיבות של הערך באתר היא בהצגת המנגנון הספציפי של המאמר, ולא בהחלפתו בסיסמה כללית על דמוקרטיה, מוסדות או זכויות. לכן הקריאה הנכונה במאמר היא קריאה ממוקדת: לזהות את השאלה המחקרית, את הזירה המוסדית או החברתית שבה היא נבחנת, ואת האופן שבו הממצאים או הטיעון משנים את ההבנה של כוח, שוויון, משילות או צמיחה. הרחבה זו נשארת בתוך גבולות המידע שכבר תועד בערך ואינה מוסיפה עובדות חיצוניות.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Ofer Gat",
  "paperTitle": "Attitudes towards Intercultural Sensitivity among Muslim-Bedouin Education Students in Southern Israel: A Proposed Educational Mode",
  "authorsHtml": "Ofer Gat",
  "journal": "Social Science and Humanities Journal 10(01), 9650-9679",
  "dateText": "פורסם אונליין: 1 בינואר 2026",
  "doiUrl": "https://doi.org/10.18535/sshj.v10i01.2163",
  "doiLabel": "https://doi.org/10.18535/sshj.v10i01.2163",
  "authors": [
    {
      "@type": "Person",
      "name": "Ofer Gat"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ofer Gat"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "רגישות בין־תרבותית",
    "סטודנטים בדואים",
    "חינוך",
    "מיעוט ערבי־פלסטיני",
    "עברית",
    "אי־שוויון"
  ],
  "image": {
    "src": "assets/article-images/real_photo_060_education_democracy_attitudes_intercultural_sensitivity_muslim_bedouin_education_students_southern_i.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לרגישות בין־תרבותית של סטודנטים בדואים דורשת תיווך מתמשך",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "JJ Fortress",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Yishai_Fleisher_with_Hebrew_University_students_at_the_Voice_of_Israel_studios.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/f/ff/Yishai_Fleisher_with_Hebrew_University_students_at_the_Voice_of_Israel_studios.jpg",
    "fileTitle": "Yishai Fleisher with Hebrew University students at the Voice of Israel studios.jpg",
    "sourceDimensions": "1642x1095",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:education",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-06-15",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שמפגש ערבי־יהודי באקדמיה אינו מספיק כשלעצמו; הוא דורש תיווך, הכשרה ועבודה מודעת עם זהות וקונפליקט.",
  "sections": [
    {
      "headingHe": "למה מפגש בין־תרבותי צריך תיווך?",
      "paragraphsHtml": [
        "המחקר בוחן כיצד סטודנטים מוסלמים־בדואים לחינוך בדרום ישראל מבינים רגישות בין־תרבותית ואכפתיות בין־תרבותית. נקודת המוצא היא שהחברה הבדואית היא חלק מן המיעוט הערבי־פלסטיני בישראל, עם היסטוריה ייחודית של עקירה, ריכוז מרחבי ואי־שוויון.",
        "המאמר שואל אם קורס פדגוגי שמדגיש למידה מעורבת ורפלקציה יכול לחשוף שכבות חיוביות ושליליות ביחסים בין קבוצות."
      ]
    },
    {
      "headingHe": "מה חושפות הרפלקציות של סטודנטים בדואים?",
      "paragraphsHtml": [
        "המחקר מבוסס על 16 סטודנטים בדואים לתואר ראשון בחינוך, 14 נשים ושני גברים, שלמדו בקורס מתודולוגי בן סמסטר. הקורס נלמד על ידי מרצים ערבי ויהודי, וכלל עבודה בקבוצות מעורבות של סטודנטים ערבים ויהודים.",
        "הנתונים הם רפלקציות אנונימיות של הסטודנטים, שנכתבו לפי שאלות מנחות ונותחו באמצעות ניתוח תוכן איכותני וקידוד קטגוריות."
      ]
    },
    {
      "headingHe": "איך ריחוק יכול להיות חלק מאכפתיות?",
      "paragraphsHtml": [
        "הממצאים מצביעים על “מאבק בין־תרבותי” ו”מצוקה בין־תרבותית”: מצד אחד רצון בקרבה, דיאלוג ולמידה של תרבות האחר; מצד שני צורך במרחק מחושב בגלל היסטוריה, פוליטיקה, שפה ופערי כוח.",
        "המאמר טוען שהכשרת מורים לעתיד צריכה ללמד לא רק קרבה בין־תרבותית אלא גם יכולת לקחת מרחק אחראי, להבין אי־שוויון היסטורי ולעבוד עם זהות מורכבת ודינמית."
      ]
    },
    {
      "headingHe": "איך שפה ופוליטיקה יוצרות מצוקה במפגש?",
      "paragraphsHtml": [
        "הסטודנטים מתארים מפגש שבו עברית, היסטוריה ופערי כוח אינם נעלמים. הרצון להכיר את האחר מתקיים לצד חשש משיפוטיות ומתח פוליטי."
      ]
    },
    {
      "headingHe": "למה פרחי הוראה הם קבוצה חשובה?",
      "paragraphsHtml": [
        "כסטודנטים לחינוך, המשתתפים עתידים לתווך לילדים מפגש בין־תרבותי. לכן העמדות שלהם אינן רק חוויה אישית, אלא בסיס לפדגוגיה עתידית בחברה שסועה."
      ]
    }
  ],
  "layout": "paper",
  "title": "רגישות בין־תרבותית בקרב סטודנטים בדואים לחינוך בדרום ישראל",
  "permalink": "/democracy_attitudes_intercultural_sensitivity_muslim_bedouin_education_students_southern_israel_summary_he.html",
  "paper_url": "democracy_attitudes_intercultural_sensitivity_muslim_bedouin_education_students_southern_israel_summary_he.html",
  "sortKey": 202606150001,
  "sourcePublishedDate": "2026-01-01"
}
---
