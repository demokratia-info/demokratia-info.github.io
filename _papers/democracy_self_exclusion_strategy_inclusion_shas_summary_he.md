---
{
  "version": 1,
  "file": "democracy_self_exclusion_strategy_inclusion_shas_summary_he.html",
  "slug": "democracy_self_exclusion_strategy_inclusion_shas_summary_he",
  "permalink": "/democracy_self_exclusion_strategy_inclusion_shas_summary_he.html",
  "paper_url": "democracy_self_exclusion_strategy_inclusion_shas_summary_he.html",
  "titleHe": "ש״ס מדגימה כיצד הסתגרות קהילתית יכולה לשמש אסטרטגיית הכלה פוליטית",
  "subtitleHe": "",
  "descriptionHe": "המאמר של David Lehmann ו-Batia Siebzehner מנתח את ש״ס כמקרה שבו הדגשת ייחוד דתי וקהילתי פועלת לצד השתלבות בפוליטיקה הישראלית. הוא מציע להבין הכלה לא רק כהיטמעות אלא גם כיכולת לייצג קבוצה מתוך גבולות זהות ברורים.",
  "summaryHe": "המאמר מראה כיצד קבוצה שמדגישה נבדלות יכולה להשתמש בנבדלות עצמה כדי להשיג קול פוליטי, משאבים והכרה. מבחינה דמוקרטית, המקרה מאתגר תפיסה של אזרחות אחידה ומעלה שאלה כיצד מערכת פוליטית מכלילה קהילות שאינן מבקשות להיטמע במלואן.",
  "authorsCardHe": "David Lehmann, Batia Siebzehner",
  "paperTitle": "Self-exclusion as a strategy of inclusion: the case of Shas",
  "authorsHtml": "<a href=\"https://www.davidlehmann.org/\" target=\"_blank\" rel=\"noopener noreferrer\">David Lehmann</a>; <a href=\"https://truman.huji.ac.il/people/batia-siebzehner\" target=\"_blank\" rel=\"noopener noreferrer\">Batia Siebzehner</a>",
  "journal": "Citizenship Studies, 12(3), 233-247",
  "dateText": "פורסם: יוני 2008",
  "doiUrl": "https://doi.org/10.1080/13621020802015396",
  "doiLabel": "https://doi.org/10.1080/13621020802015396",
  "authors": [
    {
      "@type": "Person",
      "name": "David Lehmann",
      "url": "https://www.davidlehmann.org/"
    },
    {
      "@type": "Person",
      "name": "Batia Siebzehner",
      "url": "https://truman.huji.ac.il/people/batia-siebzehner"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "David Lehmann",
      "url": "https://www.davidlehmann.org/"
    },
    {
      "@type": "Person",
      "name": "Batia Siebzehner",
      "url": "https://truman.huji.ac.il/people/batia-siebzehner"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "ש״ס",
    "אזרחות",
    "הכלה פוליטית",
    "מיעוטים",
    "דת ופוליטיקה",
    "זהות קהילתית",
    "ישראל"
  ],
  "image": {
    "src": "html_qa/democracy_self_exclusion_strategy_inclusion_shas_summary_he.jpg",
    "version": "2026-06-04-nightly",
    "altHe": "מפגש קהילתי דתי ומוניציפלי שבו פעילים ונציגי ציבור דנים בהשתתפות ובהכללה פוליטית",
    "fitness": "high"
  },
  "datePublished": "2026-06-04",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המקרה של ש״ס מחדד שהשתתפות דמוקרטית אינה תמיד מתחילה בהיטמעות. לפעמים קבוצה נכנסת אל הפוליטיקה דווקא דרך שפה של גבול, זהות ותביעה להכרה.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>אזרחות</strong> היא לא רק מעמד משפטי אלא גם השתייכות, הכרה ויכולת להשפיע על החלטות ציבוריות. במאמר, ש״ס משמשת מקרה שבו אזרחות פוליטית מתחזקת דווקא דרך הדגשת זהות קהילתית נפרדת."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: בין הפרדוקסים הרבים של הפוליטיקה הישראלית, ישנן אסטרטגיות של הכללה פוליטית המשמשות ארגונים ומפלגות המייצגות קבוצות השוללות את האוניברסליזם שהדמוקרטיה הישראלית היא יורשת לו. מאמר זה מפתח מודל של 'הכלה פוליטית בסגנון ישראלי', המומחש על ידי מפלגה אחת, ש\"ס, שמאז 1984 מכריזה על הקול של האוכלוסייה הספרדית המודרת מבחינה חברתית ותרבותית של יהודי צפון אפריקה והמזרח התיכון, המייצגת למעלה מ-40% מהאוכלוסייה היהודית. ש\"ס היא גם תנועה של תחייה דתית ואתנית, שבאמצעות אימוץ אסטרטגיה חברתית של הדרה עצמית המבוססת על שמירה על מצוות דתית, ושל עצמאות מול פוליטיקה חרדית אשכנזית מבוססת, זכתה ל-11 מתוך 120 מושבים בכנסת, להכללה בממשלה ושליטה על נתח החינוך והרווחה.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר מראה כיצד קבוצה שמדגישה נבדלות יכולה להשתמש בנבדלות עצמה כדי להשיג קול פוליטי, משאבים והכרה. מבחינה דמוקרטית, המקרה מאתגר תפיסה של אזרחות אחידה ומעלה שאלה כיצד מערכת פוליטית מכלילה קהילות שאינן מבקשות להיטמע במלואן."
      ]
    },
    {
      "headingHe": "מה פירוש הביטוי הסתגרות כאסטרטגיית הכלה?",
      "paragraphsHtml": [
        "הביטוי מתאר מצב שבו קבוצה אינה מוותרת על גבולותיה התרבותיים או הדתיים כדי להשתתף בפוליטיקה. במקום זאת, היא משתמשת בגבולות האלה כדי לבנות כוח ייצוגי ולדרוש מקום בתוך המערכת."
      ]
    },
    {
      "headingHe": "למה ש״ס היא מקרה חשוב לדיון בדמוקרטיה?",
      "paragraphsHtml": [
        "ש״ס מציגה שילוב בין זהות דתית, מעמדית ועדתית לבין פעולה מפלגתית בזירה ארצית. לכן היא מאפשרת לבחון כיצד דמוקרטיה מתמודדת עם קבוצות שמבקשות הכרה ושירותים ציבוריים בלי לאמץ דגם חילוני או ליברלי אחיד."
      ]
    },
    {
      "headingHe": "איך המאמר מאתגר תפיסת הכלה רגילה?",
      "paragraphsHtml": [
        "תפיסת הכלה רגילה מניחה לעיתים שקבוצה מודרת תיכנס למרכז אם תאמץ את שפתו. המאמר מציע אפשרות אחרת: השתתפות יכולה לצמוח גם מתוך מוסדות קהילתיים, סמכות דתית ושפה פוליטית שמדגישה הבדל."
      ]
    },
    {
      "headingHe": "מה הקשר בין נבדלות לבין כוח פוליטי?",
      "paragraphsHtml": [
        "נבדלות יכולה ליצור משמעת ארגונית, תחושת שליחות וקהל בוחרים מזוהה. כאשר היא מתורגמת למפלגה, היא יכולה להפוך מחוויה של שוליות לכלי מיקוח והשפעה בתוך מוסדות המדינה."
      ]
    },
    {
      "headingHe": "האם המאמר רואה בהסתגרות פתרון פשוט?",
      "paragraphsHtml": [
        "המאמר אינו מציג הסתגרות כפתרון נטול מחיר. נבדלות יכולה לחזק ייצוג, אך היא גם עלולה להעמיק גבולות בין קבוצות וליצור מתח סביב שוויון מגדרי, חינוך, סמכות רבנית וחובות אזרחיות משותפות."
      ]
    },
    {
      "headingHe": "מה המשמעות לזכויות מיעוטים?",
      "paragraphsHtml": [
        "המאמר מדגיש שזכויות מיעוטים כוללות לעיתים גם הכרה בזהות קולקטיבית ובמוסדות פנימיים. עם זאת, דמוקרטיה ליברלית צריכה לשאול כיצד הכרה כזו משתלבת עם זכויות הפרט בתוך הקהילה ומחוץ לה."
      ]
    },
    {
      "headingHe": "כיצד המקרה קשור לשירותים ציבוריים?",
      "paragraphsHtml": [
        "כוח מפלגתי של קהילה יכול להשפיע על חלוקת משאבים, חינוך ורווחה. לכן השאלה אינה רק סמלית אלא גם מוסדית: מי מקבל גישה למדינה, דרך אילו מתווכים, ועל בסיס אילו תנאים."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד על פוליטיקה של זהות?",
      "paragraphsHtml": [
        "המאמר מציג פוליטיקה של זהות כזירה מורכבת ולא רק כבעיה. זהות יכולה לחסום דיאלוג, אך היא גם יכולה לאפשר לאנשים שקולם חלש במרכז הפוליטי להתארגן, לנסח תביעות ולהיכנס למשחק הדמוקרטי."
      ]
    },
    {
      "headingHe": "מהי המסקנה לדמוקרטיה רב־קבוצתית?",
      "paragraphsHtml": [
        "דמוקרטיה רב־קבוצתית צריכה להכיל צורות שונות של השתתפות בלי לוותר על שוויון וזכויות. המקרה של ש״ס מראה שהאתגר הוא לא רק לפתוח דלתות, אלא להבין באילו תנאים קבוצות בוחרות להיכנס דרכן."
      ]
    }
  ],
  "layout": "paper",
  "title": "ש״ס מדגימה כיצד הסתגרות קהילתית יכולה לשמש אסטרטגיית הכלה פוליטית",
  "sortKey": 202606040007
}
---
