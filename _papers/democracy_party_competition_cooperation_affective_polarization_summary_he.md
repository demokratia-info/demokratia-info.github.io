---
{
  "version": 1,
  "file": "democracy_party_competition_cooperation_affective_polarization_summary_he.html",
  "slug": "democracy_party_competition_cooperation_affective_polarization_summary_he",
  "order": 50,
  "titleHe": "איך תחרות ושיתוף פעולה בין מפלגות מעצבים קיטוב רגשי?",
  "subtitleHe": "המאמר משתמש בניסוי טבעי ובניסויי סקר בישראל כדי להראות שתחרות מפלגתית מחריפה קיטוב רגשי, בעוד שיתוף פעולה בין מפלגות יכול לרכך אותו.",
  "descriptionHe": "לוטם בסן-נייגט וחגי מ׳ וייס בוחנים כיצד תחרות ושיתוף פעולה בין מפלגות בישראל משפיעים על רגשות הבוחרים כלפי מחנות פוליטיים אחרים.",
  "summaryHe": "המאמר מראה שתמריצים מפלגתיים אינם רק תוצאה של קיטוב רגשי; הם גם מנגנון שמייצר או מפחית אותו. החשיבות הציבורית של המאמר היא בכך שהוא מראה כיצד עמדות הציבור, רגשות פוליטיים וקיטוב משפיעים על נכונות לקבל כללי משחק משותפים, לכבד יריבים פוליטיים ולהגן על מוסדות גם כאשר הם פועלים נגד העדפה מחנאית רגעית.",
  "authorsCardHe": "לוטם בסן-נייגט וחגי מ׳ וייס",
  "paperTitle": "Party Competition and Cooperation Shape Affective Polarization: Evidence from Natural and Survey Experiments in Israel",
  "authorsHtml": "<a href=\"https://www.hks.harvard.edu/faculty/lotem-bassan-nygate\" target=\"_blank\" rel=\"noopener noreferrer\">Lotem Bassan-Nygate</a>; <a href=\"https://www.chagaiweiss.com/\" target=\"_blank\" rel=\"noopener noreferrer\">Chagai M. Weiss</a>",
  "journal": "Comparative Political Studies",
  "dateText": "כרך 55(2), פברואר 2022, עמ׳ 287-318",
  "doiUrl": "https://doi.org/10.1177/00104140211024283",
  "doiLabel": "https://doi.org/10.1177/00104140211024283",
  "authors": [
    {
      "@type": "Person",
      "name": "לוטם בסן-נייגט",
      "url": "https://www.hks.harvard.edu/faculty/lotem-bassan-nygate"
    },
    {
      "@type": "Person",
      "name": "חגי מ׳ וייס",
      "url": "https://www.chagaiweiss.com/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Lotem Bassan-Nygate",
      "url": "https://www.hks.harvard.edu/faculty/lotem-bassan-nygate"
    },
    {
      "@type": "Person",
      "name": "Chagai M. Weiss",
      "url": "https://www.chagaiweiss.com/"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "קיטוב רגשי",
    "תחרות מפלגתית",
    "שיתוף פעולה בין מפלגות",
    "בחירות בישראל",
    "זהות מפלגתית"
  ],
  "image": {
    "src": "html_qa/democracy_israeli_democracy_crisis_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על מפלגות, תחרות פוליטית ושיתוף פעולה דמוקרטי",
    "fitness": "standard"
  },
  "datePublished": "2026-05-20",
  "dateModified": "2026-05-24",
  "lastUpdatedHe": "24 במאי 2026",
  "oneLinerHtml": "המאמר מראה שתמריצים מפלגתיים אינם רק תוצאה של קיטוב רגשי; הם גם מנגנון שמייצר או מפחית אותו. מבחינה דמוקרטית, הוא מזכיר שהפחתת קיטוב אינה רק עניין של חינוך אזרחי; היא תלויה גם בהתנהגות השחקנים המפלגתיים עצמם. החשיבות הציבורית של המאמר היא בכך שהוא מראה כיצד עמדות הציבור, רגשות פוליטיים וקיטוב משפיעים על נכונות לקבל כללי משחק משותפים, לכבד יריבים פוליטיים ולהגן על מוסדות גם כאשר הם פועלים נגד העדפה מחנאית רגעית.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "קיטוב רגשי הוא מצב שבו יריבים פוליטיים אינם רק חולקים בדעותיהם אלא מפתחים חשד, ריחוק או עוינות זה כלפי זה. בהקשר של המאמר, המושג חשוב כדי להבין כיצד רגשות כלפי מחנות פוליטיים משפיעים על אמון, אלימות פוליטית ונכונות לשתף פעולה."
      ]
    },
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מצביע על כך שמנהיגים ומפלגות יכולים להחריף איבה בין מחנות אך גם לשדר מסרים של שיתוף פעולה שמחלישים אותה.",
        "המחקר נשען על ניסוי טבעי שנפרש על פני שבעה מחקרי בחירות בישראל ועל ניסויי סקר הבוחנים תגובות לשיתוף פעולה בין מפלגות.",
        "במקום להתייחס לדמוקרטיה רק כמערכת בחירות, המאמר בוחן את התנאים שמאפשרים לחברה לשמר אמון, זכויות, אחריות ומחלוקת לגיטימית."
      ]
    },
    {
      "headingHe": "מהי נקודת המבט הליברלית-דמוקרטית?",
      "paragraphsHtml": [
        "מבחינה דמוקרטית, הוא מזכיר שהפחתת קיטוב אינה רק עניין של חינוך אזרחי; היא תלויה גם בהתנהגות השחקנים המפלגתיים עצמם.",
        "הזווית הליברלית חשובה משום שהיא שואלת לא רק מי שולט, אלא גם מה מגביל את השלטון, איך נשמרת ביקורת, ואיך מוגנות קבוצות או יריבים פוליטיים מפני כוח עודף."
      ]
    },
    {
      "headingHe": "למה המאמר חשוב לקוראי האתר?",
      "paragraphsHtml": [
        "המאמר מתאים לאתר משום שהוא מסביר כיצד מוסדות הבחירות והמפלגות משפיעים על איכות המרחב הדמוקרטי.",
        "הוא מסייע לתרגם מחקר אקדמי לשאלות ציבוריות מעשיות: איך לזהות שחיקה דמוקרטית, איך למדוד קיטוב, ואיך לחשוב על מוסדות וזכויות בלי להסתפק בסיסמאות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד תחרות מפלגתית משפיע על איכות הדמוקרטיה בישראל ועל היכולת לשמור על אמון, זכויות ומוסדות עצמאיים.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "מה החידוש העיקרי?",
      "paragraphsHtml": [
        "תרומתו היא בהצגת קשר סיבתי יותר בין מבנה התחרות הפוליטית לבין רגשות הבוחרים, מעבר למדידה תיאורית של קיטוב.",
        "החידוש צריך להיבחן מול הטענה והראיות במאמר המקורי, ולא רק לפי הדרך שבה התמצית מנגישה אותו."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "מבחינה דמוקרטית, הוא מזכיר שהפחתת קיטוב אינה רק עניין של חינוך אזרחי; היא תלויה גם בהתנהגות השחקנים המפלגתיים עצמם.",
        "החשיבות הדמוקרטית נובעת מן הקשר בין מוסדות, זכויות ואחריות ציבורית, אך גבולות הטענה נשארים גבולות המקור."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לישראל?",
      "paragraphsHtml": [
        "הקשר הישראלי נמצא בלב המאמר: המחקר נשען על ניסוי טבעי שנפרש על פני שבעה מחקרי בחירות בישראל ועל ניסויי סקר הבוחנים תגובות לשיתוף פעולה בין מפלגות.",
        "הקשר הישראלי צריך להיקרא דרך המוסדות, הזכויות וההקשרים שהמאמר עצמו מנתח, ולא כהכללה על החברה הישראלית כולה."
      ]
    },
    {
      "headingHe": "איזה מושג כדאי לזכור?",
      "paragraphsHtml": [
        "המושג המרכזי הוא <strong>תחרות מפלגתית</strong>. הוא עוזר לראות כיצד תהליך שנראה פוליטי או משפטי צר משפיע על מערכת היחסים בין אזרחים, מוסדות ושלטון."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על מוסדות?",
      "paragraphsHtml": [
        "המוסדות הדמוקרטיים אינם פועלים בחלל ריק. הם תלויים בנורמות, באמון ציבורי, בהתנהגות מפלגות וביכולת להציב גבולות לכוח פוליטי."
      ]
    },
    {
      "headingHe": "האם המאמר מציע פתרון פשוט?",
      "paragraphsHtml": [
        "לא. התרומה שלו היא בעיקר אנליטית: הוא מחדד את הבעיה ומראה אילו תנאים פוליטיים, חברתיים או משפטיים צריכים להשתנות כדי לשפר את החוסן הדמוקרטי."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין לקרוא את המאמר כהוכחה שכל מחלוקת פוליטית היא סכנה לדמוקרטיה. החשיבות היא בזיהוי התנאים שבהם מחלוקת הופכת לפגיעה באמון, בזכויות או באיזונים מוסדיים."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול שאלות מדויקות יותר: אילו מוסדות נפגעים, אילו זכויות עומדות על הפרק, ואיך תחרות מפלגתית משנה את התנהגות הציבור או השלטון.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך תחרות ושיתוף פעולה בין מפלגות מעצבים קיטוב רגשי?",
  "permalink": "/democracy_party_competition_cooperation_affective_polarization_summary_he.html",
  "paper_url": "democracy_party_competition_cooperation_affective_polarization_summary_he.html",
  "sortKey": 202605200014
}
---
