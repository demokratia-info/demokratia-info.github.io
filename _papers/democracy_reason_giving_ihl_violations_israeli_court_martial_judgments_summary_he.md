---
{
  "version": 1,
  "file": "democracy_reason_giving_ihl_violations_israeli_court_martial_judgments_summary_he.html",
  "slug": "democracy_reason_giving_ihl_violations_israeli_court_martial_judgments_summary_he",
  "titleHe": "למה אוכפים את הדין נגד הפרות של המשפט ההומניטרי הבינלאומי?",
  "subtitleHe": "ניתוח שיטתי של 60 שנות פסקי דין בוחן ערכים הומניטריים, שליטה בכוח ואחריות צבאית.",
  "descriptionHe": "המאמר בוחן כיצד בתי דין צבאיים בישראל מנמקים אכיפה של עבירות הקשורות למשפט הלחימה והמשפט ההומניטרי הבינלאומי. הוא מבוסס על ניתוח תוכן שיטתי של 60 שנות פסקי דין צבאיים.",
  "summaryHe": "המחקר שואל אילו ערכים מופיעים בפסקי דין צבאיים כאשר נאכפות הפרות של דיני הלחימה: הגנה הומניטרית על בני אדם, שליטה בכוחות הצבא, או שילוב ביניהם. הממצאים מצביעים על הבחנה בין סוגי עבירות: בעבירות נגד חיים ושלמות הגוף בולטים נימוקים הומניטריים, ואילו בעבירות רכוש כמו ביזה בולטים יותר שיקולים של פיקוח, משמעת ושליטה בכוחות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Liron A. Libman, Amichai Cohen",
  "paperTitle": "Why do we enforce the law against IHL violations? Reason-giving in Israeli court-martial judgments",
  "authorsHtml": "<a href=\"https://orcid.org/0009-0003-2661-2101\" target=\"_blank\" rel=\"noopener noreferrer\">Liron A. Libman</a>; <a href=\"https://en.idi.org.il/experts/1491\" target=\"_blank\" rel=\"noopener noreferrer\">Amichai Cohen</a>",
  "journal": "The Military Law and the Law of War Review, 63(2), 161-195",
  "dateText": "פורסם: 1 במרץ 2026",
  "doiUrl": "https://doi.org/10.4337/mllwr.2025.01.02",
  "doiLabel": "https://doi.org/10.4337/mllwr.2025.01.02",
  "authors": [
    {
      "@type": "Person",
      "name": "Liron A. Libman",
      "url": "https://orcid.org/0009-0003-2661-2101"
    },
    {
      "@type": "Person",
      "name": "Amichai Cohen",
      "url": "https://en.idi.org.il/experts/1491"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Liron A. Libman",
      "url": "https://orcid.org/0009-0003-2661-2101"
    },
    {
      "@type": "Person",
      "name": "Amichai Cohen",
      "url": "https://en.idi.org.il/experts/1491"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "משפט הומניטרי בין לאומי",
    "בתי דין צבאיים",
    "נימוק שיפוטי",
    "אחריות צבאית",
    "שלטון החוק",
    "ישראל"
  ],
  "image": {
    "src": "assets/article-images/real_photo_609_security_democracy_reason_giving_ihl_violations_israeli_court_martial_judgments_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לנימוקי בתי הדין הצבאיים חושפים מדוע נאכף המשפט ההומניטרי",
    "fitness": "high",
    "license": "CC BY-SA 2.0",
    "creator": "U.S. Embassy Tel Aviv",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_U.S._Embassy_Tel_Aviv_-_Visit_to_Iron_Dome_No.161FL.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/8/8a/Flickr_-_U.S._Embassy_Tel_Aviv_-_Visit_to_Iron_Dome_No.161FL.jpg",
    "fileTitle": "Flickr - U.S. Embassy Tel Aviv - Visit to Iron Dome No.161FL.jpg",
    "sourceDimensions": "1200x799",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:security",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/2.0/"
  },
  "datePublished": "2026-06-13",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מנתח 60 שנות פסקי דין צבאיים בישראל ומראה שאכיפת דיני הלחימה נשענת על שילוב משתנה של ערכים הומניטריים ושליטה צבאית.",
  "sections": [
    {
      "headingHe": "מהי שאלת המחקר המרכזית?",
      "paragraphsHtml": [
        "המאמר שואל אילו ערכים מופיעים בנימוקי בתי הדין כאשר הם אוכפים את המשפט ההומניטרי הבינלאומי. האם האכיפה מוצגת כהגנה על בני אדם, כאינטרס של הצבא לשלוט בכוחותיו, או כשילוב בין השניים?"
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את הנימוקים שבתי דין צבאיים בישראל נותנים לאכיפת עבירות הקשורות לדיני הלחימה. מסגרת המחקר כוללת פסקי דין של ערכאות מחוזיות ושל בית הדין הצבאי לערעורים, ומארגנת אותם לפי תקופות, סוגי עבירות, דרגות נאשמים והיחס בין עבירות בתוך הצבא לבין עבירות כלפי אזרחים.",
        "המחקר מצא הבחנה מרכזית בין עבירות. כאשר מדובר בעבירות נגד חיים ושלמות הגוף, הנימוקים השיפוטיים מדגישים יותר ערכים הומניטריים והגנה על הקורבן. כאשר מדובר בעבירות רכוש, ובמיוחד ביזה, הנימוקים נוטים להדגיש שליטה בכוחות הצבא, משמעת ומניעת התפרקות של סמכות הפיקוד.",
        "הניתוח מראה שהצדקות לאכיפה אינן קבועות ואינן נשענות על עיקרון יחיד. בתי הדין משנים את שפת ההצדקה לפי סוג העבירה, לפי תקופה ולפי ההקשר השיפוטי, ובוחנים גם את מידת ההתייחסות הישירה למשפט הבינלאומי. מכאן שהמאמר מציג את אכיפת דיני הלחימה כזירה שבה נפגשים מוסר הומניטרי, ניהול כוח צבאי ותרבות משפטית משתנה."
      ]
    },
    {
      "headingHe": "איך נבדקה השאלה?",
      "paragraphsHtml": [
        "המחקר מבצע ניתוח תוכן שיטתי של פסקי דין של בתי דין צבאיים ישראליים לאורך 60 שנה. הוא בוחן אילו ערכים מופיעים בנימוקים, באילו סוגי עבירות, וכיצד הם משתנים לאורך זמן."
      ]
    },
    {
      "headingHe": "מה ההבדל בין עבירות גוף לעבירות רכוש?",
      "paragraphsHtml": [
        "לפי התקציר, בעבירות נגד חיים ושלמות הגוף בולטים יותר ערכים הומניטריים. לעומת זאת, בעבירות רכוש כמו ביזה, נימוקים של שליטה בכוחות הצבא ודיסציפלינה מקבלים משקל גדול יותר."
      ]
    },
    {
      "headingHe": "למה הממצא מסבך את התאוריה?",
      "paragraphsHtml": [
        "אם רק הומניטריות הייתה מסבירה את האכיפה, היינו מצפים לשפה דומה בכל סוגי העבירות. אם רק אינטרס צבאי היה מסביר אותה, זכויות הקורבנות היו נדחקות. בפועל, המאמר מראה שילוב משתנה לפי הקשר."
      ]
    },
    {
      "headingHe": "מה הערך הרחב של המחקר?",
      "paragraphsHtml": [
        "המאמר מלמד כיצד משפט בינלאומי מתורגם לפרקטיקה שיפוטית צבאית. הוא מראה שאכיפת הדין היא גם פעולה מוסרית, גם מנגנון שליטה צבאי, וגם זירה שבה ערכים משתנים לאורך זמן."
      ]
    }
  ],
  "layout": "paper",
  "title": "למה אוכפים את הדין נגד הפרות של המשפט ההומניטרי הבינלאומי?",
  "permalink": "/democracy_reason_giving_ihl_violations_israeli_court_martial_judgments_summary_he.html",
  "paper_url": "democracy_reason_giving_ihl_violations_israeli_court_martial_judgments_summary_he.html",
  "sortKey": 202606130002,
  "sourcePublishedDate": "2026-03-01"
}
---
