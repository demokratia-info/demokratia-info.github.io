---
{
  "version": 1,
  "file": "democracy_asymmetry_in_israeli_academia_summary_he.html",
  "slug": "democracy_asymmetry_in_israeli_academia_summary_he",
  "order": 893,
  "titleHe": "א־סימטריה במפגש בין סטודנטיות בדואיות ויהודיות באקדמיה הישראלית",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את עמדותיהן של סטודנטיות ערביות־בדואיות כלפי עמיתותיהן היהודיות בקורסים רב־תרבותיים במכללות להכשרת מורים בישראל, ואת האופן שבו יחסי רוב ומיעוט מעצבים את המפגש.",
  "summaryHe": "המאמר מבוסס על מדגם של 30 משתתפות ובוחן עמדות של סטודנטיות בדואיות כלפי סטודנטיות יהודיות בקורסים רב־תרבותיים במכללות להכשרת מורים בישראל. הממצאים מצביעים על רגשות מעורבים: הסטודנטיות הבדואיות מעריכות גיוון תרבותי והזדמנויות למפגש בין־תרבותי, אך מתארות גם דינמיקה א־סימטרית שבה סטודנטיות יהודיות שומרות לעיתים על עמדה דומיננטית ונמנעות ממעורבות עמוקה ביחסי הקבוצות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Aref Abu-Gweder",
  "paperTitle": "Asymmetry in Israeli academia",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-2050-2150\" target=\"_blank\" rel=\"noopener noreferrer\">Aref Abu-Gweder</a>",
  "journal": "Journal of International Students",
  "dateText": "פורסם: 28 בפברואר 2025; כרך 15(3), עמ׳ 91-110",
  "doiUrl": "https://doi.org/10.32674/3z6vvp28",
  "doiLabel": "https://doi.org/10.32674/3z6vvp28",
  "authors": [
    {
      "@type": "Person",
      "name": "Aref Abu-Gweder",
      "url": "https://orcid.org/0000-0002-2050-2150"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Aref Abu-Gweder",
      "url": "https://orcid.org/0000-0002-2050-2150"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "סטודנטיות בדואיות",
    "אקדמיה ישראלית",
    "רב־תרבותיות",
    "מכללות לחינוך",
    "יחסי רוב ומיעוט",
    "שוויון",
    "דיאלוג בין־קבוצתי",
    "זהות מיעוט",
    "הכשרת מורות",
    "הדרה סמויה"
  ],
  "image": {
    "src": "html_qa/democracy_asymmetry_in_israeli_academia_summary_he.jpg",
    "version": "2026-06-26-nightly",
    "altHe": "בניין מכללה אקדמית לחינוך כמסגרת למפגש בין סטודנטיות בדואיות ויהודיות",
    "fitness": "standard"
  },
  "datePublished": "2026-06-26",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כיצד קורסים רב־תרבותיים יכולים לקדם שיתוף פעולה, אך גם לחשוף <strong>א־סימטריה בין רוב למיעוט</strong> באקדמיה הישראלית.",
  "sections": [
    {
      "headingHe": "מפגש לימודי שבו יחסי הכוח נשארים נוכחים",
      "paragraphsHtml": [
        "המחקר מתמקד בסטודנטיות בדואיות המשתתפות בקורסים רב־תרבותיים במכללות לחינוך בישראל. המפגש עם סטודנטיות יהודיות מתואר כמרחב שיש בו פוטנציאל לשותפות, אך הוא אינו מתקיים מחוץ ליחסי הכוח הרחבים של החברה הישראלית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן מדגם של 30 סטודנטיות בדואיות ואת עמדותיהן כלפי עמיתות יהודיות בקורסים רב־תרבותיים במכללות להכשרת מורים בישראל. הוא מתמקד במפגש בין קבוצת מיעוט נשית לבין קבוצת רוב יהודית, ובאופן שבו יחסי כוח חברתיים נכנסים אל הכיתה הרב־תרבותית.",
        "הניתוח מראה שהסטודנטיות הבדואיות מייחסות ערך לגיוון תרבותי ולהזדמנויות למפגש בין־תרבותי, אך חוות את המפגש בתוך דינמיקות חברתיות רחבות יותר בישראל. קבוצות רב־תרבותיות מיועדות לעודד דיאלוג ונקודות מבט משותפות, אך המאמר מדגיש שהמטרה הזו מתקיימת לצד פערי כוח.",
        "המאמר טוען שהא־סימטריה מתבטאת בכך שסטודנטיות יהודיות שומרות לעיתים על עמדה דומיננטית ונמנעות ממעורבות עמוקה ביחסים בין הקבוצות. מנגד, הסטודנטיות הבדואיות מדגישות את זהותן כמיעוט המעוצב בידי גורמים דתיים, חברתיים ותרבותיים; עם זאת, שתי הקבוצות גם משתפות פעולה ותומכות זו בזו בהצלחה אקדמית."
      ]
    },
    {
      "headingHe": "ממצא מרכזי: שיתוף פעולה לצד הימנעות מדיאלוג עמוק",
      "paragraphsHtml": [
        "לפי המאמר, הסטודנטיות הבדואיות מעריכות את האפשרות להכיר תרבויות אחרות ולפעול יחד בתוך הכיתה. עם זאת, הן מתארות מצב שבו סטודנטיות יהודיות מחזיקות בעמדה דומיננטית ולעיתים נמנעות מהתמודדות עמוקה עם יחסי יהודים־ערבים ועם זהות המיעוט."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: רב־תרבותיות וא־סימטריה",
      "paragraphsHtml": [
        "רב־תרבותיות במוסד חינוכי אינה רק נוכחות של קבוצות שונות באותו חדר. כאשר קבוצה אחת מחזיקה בכוח חברתי רב יותר, המפגש עלול להפוך לא־סימטרי גם אם הוא מנוהל בשפה של סובלנות ושיתוף."
      ]
    },
    {
      "headingHe": "המשמעות הדמוקרטית של הכשרת מורות",
      "paragraphsHtml": [
        "מכללות לחינוך מעצבות את מי שיובילו כיתות וגני ילדים בעתיד, ולכן הן זירה חשובה לשוויון אזרחי. אם ההכשרה מאפשרת למיעוט לדבר אך אינה מחייבת את הרוב להקשיב באופן ממשי, גם החינוך לדמוקרטיה נשאר חלקי."
      ]
    },
    {
      "headingHe": "מה נבדק במחקר על סטודנטיות בדואיות?",
      "paragraphsHtml": [
        "המחקר בדק עמדות וחוויות של סטודנטיות בדואיות כלפי עמיתות יהודיות בקורסים רב־תרבותיים. הוא נשען על מדגם של 30 משתתפות ומתמקד באופן שבו זהות מיעוט פוגשת מרחב לימודי יהודי־דומיננטי."
      ]
    },
    {
      "headingHe": "למה המאמר קשור לשוויון אזרחי?",
      "paragraphsHtml": [
        "המאמר קשור לשוויון משום שהוא מראה שהשתתפות פורמלית באקדמיה אינה מבטיחה מעמד שווה בשיחה. סטודנטיות מיעוט עשויות להיכנס למרחב משותף ועדיין לשאת לבדן את הנטל של הסבר, תרגום והתמודדות עם סטריאוטיפים."
      ]
    },
    {
      "headingHe": "מה תפקיד הקורסים הרב־תרבותיים לפי המאמר?",
      "paragraphsHtml": [
        "הקורסים הרב־תרבותיים יכולים לפתוח מקום למפגש, היכרות ושיתוף פעולה בין קבוצות. כוחם הדמוקרטי תלוי בכך שהם אינם מסתפקים בחגיגת שונות אלא מאפשרים דיון גלוי על אי־שוויון ועל יחסי רוב ומיעוט."
      ]
    },
    {
      "headingHe": "האם המחקר מתאר רק קונפליקט בין הקבוצות?",
      "paragraphsHtml": [
        "לא, התקציר מתאר גם שיתופי פעולה ותמיכה הדדית בהצלחה אקדמית. דווקא המורכבות הזאת חשובה, משום שהיא מראה שמרחב משותף יכול להיות גם מועיל וגם בלתי שוויוני בעת ובעונה אחת."
      ]
    },
    {
      "headingHe": "איזה חסם מופיע ביחס לקבוצת הרוב?",
      "paragraphsHtml": [
        "החסם המרכזי הוא הימנעות של סטודנטיות יהודיות ממעורבות עמוקה בשאלות של יחסים בין־קבוצתיים. כאשר הרוב יכול להישאר במרחק נוח מהשיחה, המיעוט נדרש להנכיח את עצמו יותר כדי לקבל הכרה."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למוסדות להשכלה גבוהה?",
      "paragraphsHtml": [
        "מוסדות להשכלה גבוהה צריכים לבחון את איכות הדיאלוג ולא רק את עצם ההרשמה של סטודנטיות מקבוצות שונות. הכשרה טובה דורשת הנחיה שמאזנת בין הקולות ומונעת מצב שבו צד אחד נושא את רוב העבודה הרגשית והאזרחית."
      ]
    },
    {
      "headingHe": "איך זה נוגע להכשרת מורות וגננות?",
      "paragraphsHtml": [
        "מי שלומדות במכללות לחינוך יפגשו בעתיד כיתות מגוונות, ולכן דרך ההתנסות שלהן ברב־תרבותיות חשובה במיוחד. אם הן לומדות להתמודד עם א־סימטריה במקום להסתיר אותה, הן עשויות לבנות מרחבים חינוכיים שוויוניים יותר."
      ]
    },
    {
      "headingHe": "מה ההבדל בין גיוון לבין שוויון במקרה הזה?",
      "paragraphsHtml": [
        "גיוון מתאר את עצם נוכחותן של קבוצות שונות באותו מוסד או קורס. שוויון דורש לבחון מי מדבר, מי מוגדר כנורמה, מי מתבקש להסביר את עצמו ומי נהנה מהיכולת להישאר בלתי מסומן."
      ]
    },
    {
      "headingHe": "למי המאמר רלוונטי במיוחד?",
      "paragraphsHtml": [
        "המאמר רלוונטי למנהלות מוסדות אקדמיים, למרצות בקורסים רב־תרבותיים ולמי שעוסקים בהכשרת מורות. הוא רלוונטי גם לחוקרי דמוקרטיה משום שהוא מתרגם שאלות גדולות של רוב ומיעוט לפרקטיקה יומיומית בכיתה."
      ]
    },
    {
      "headingHe": "מהי תרומת המאמר למאגר?",
      "paragraphsHtml": [
        "התרומה למאגר היא הרחבת הדיון בזכויות מיעוטים אל תוך מוסדות הכשרת המורים. המאמר מציג את האקדמיה לא רק כמנוע ניידות אלא גם כזירה שבה דמוקרטיה אזרחית נבחנת ביחסי דיבור, הקשבה והכרה."
      ]
    }
  ],
  "layout": "paper",
  "title": "א־סימטריה במפגש בין סטודנטיות בדואיות ויהודיות באקדמיה הישראלית",
  "permalink": "/democracy_asymmetry_in_israeli_academia_summary_he.html",
  "paper_url": "democracy_asymmetry_in_israeli_academia_summary_he.html",
  "sortKey": 202606260010,
  "sourcePublishedDate": "2025-02-28"
}
---
