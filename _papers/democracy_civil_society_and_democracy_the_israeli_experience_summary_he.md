---
{
  "version": 1,
  "file": "democracy_civil_society_and_democracy_the_israeli_experience_summary_he.html",
  "slug": "democracy_civil_society_and_democracy_the_israeli_experience_summary_he",
  "order": 242,
  "titleHe": "חברה אזרחית ודמוקרטיה: הניסיון הישראלי",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את תרומת החברה האזרחית לדמוקרטיה בישראל באמצעות סקר של עמותות וארגונים התנדבותיים, ומתמקד בביצועיהן הדמוקרטיים של התאגדויות אזרחיות.",
  "summaryHe": "המאמר בוחן האם החברה האזרחית בישראל מתפקדת כתשתית לדמוקרטיה, ומראה שבסקר בקרב 360 התאגדויות התנדבותיות בישראל, סוג הקבוצה לא הסביר הבדלים בביצוע הדמוקרטי והתחום החלש ביותר היה אינטגרציה.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Yael Yishai",
  "paperTitle": "Civil Society and Democracy: The Israeli Experience",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/yael-yishai/\" target=\"_blank\" rel=\"noopener noreferrer\">Yael Yishai</a>",
  "journal": "VOLUNTAS International Journal of Voluntary and Nonprofit Organizations",
  "dateText": "פורסם: 2002",
  "doiUrl": "https://doi.org/10.1023/a:1020303908944",
  "doiLabel": "https://doi.org/10.1023/a:1020303908944",
  "authors": [
    {
      "@type": "Person",
      "name": "Yael Yishai",
      "url": "https://cris.haifa.ac.il/en/persons/yael-yishai/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yael Yishai",
      "url": "https://cris.haifa.ac.il/en/persons/yael-yishai/"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "מוסדות ציבור",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "ממשל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_107_civic_democracy_civil_society_and_democracy_the_israeli_experience_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על חברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל",
    "fitness": "high",
    "license": "Public domain",
    "creator": "Photography department - Government Press Office",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Kfar_Kama_municipal_council_1966-01-10.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/f/f2/Kfar_Kama_municipal_council_1966-01-10.jpg",
    "fileTitle": "Kfar Kama municipal council 1966-01-10.jpg",
    "sourceDimensions": "3072x2048",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:civic",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://commons.wikimedia.org/wiki/Commons:Copyright_tags#Public_domain"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שחברה אזרחית אינה מחזקת דמוקרטיה באופן אוטומטי: בסקר התאגדויות בישראל, הביצוע הדמוקרטי היה דומה בין סוגי קבוצות והחולשה הבולטת הייתה <strong>אינטגרציה</strong>.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "חברה אזרחית היא מרחב הפעולה של ארגונים, עמותות, תנועות וקבוצות אזרחים שאינם חלק ישיר ממוסדות המדינה או מן השוק המסחרי. בהקשר של המאמר, המושג עוזר להבין כיצד אזרחים וקבוצות פועלים בין המדינה לבין החיים הפרטיים כדי לקדם זכויות, ביקורת ציבורית ושינוי חברתי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את הקשר בין חברה אזרחית לדמוקרטיה בישראל דרך סקר בקרב 360 התאגדויות התנדבותיות. נקודת המוצא היא שהחברה האזרחית נחשבת חשובה לדמוקרטיה, אך קיימת מחלוקת על הגורמים שמסבירים את איכות הביצוע הדמוקרטי שלה.",
        "המחקר מצא שההתאגדויות קיבלו ציונים דומים במדד הדמוקרטי, ושסוג הקבוצה לא השפיע על ציון הביצוע הדמוקרטי. לכן המאמר אינו מציג הבדל חד בין קטגוריות ארגוניות כגורם המרכזי שמסביר את תרומתן לדמוקרטיה.",
        "הממצא החלש ביותר היה בתחום האינטגרציה. מכאן שהמאמר מצביע על מגבלה חשובה של החברה האזרחית בישראל: עצם קיומן של התאגדויות אזרחיות אינו מבטיח חיזוק שווה של כל רכיבי הדמוקרטיה."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את חברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל, וממקם את הסוגיה בתוך הדיון הרחב על מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות.",
        "נקודת המוצא של המאמר היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת חברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בחברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את חברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>חברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בחברה אזרחית, ארגונים ומרחב פעולה דמוקרטי בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "חברה אזרחית ודמוקרטיה: הניסיון הישראלי",
  "permalink": "/democracy_civil_society_and_democracy_the_israeli_experience_summary_he.html",
  "paper_url": "democracy_civil_society_and_democracy_the_israeli_experience_summary_he.html",
  "sortKey": 202605210242
}
---
