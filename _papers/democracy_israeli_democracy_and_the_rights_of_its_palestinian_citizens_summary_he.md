---
{
  "version": 1,
  "file": "democracy_israeli_democracy_and_the_rights_of_its_palestinian_citizens_summary_he.html",
  "slug": "democracy_israeli_democracy_and_the_rights_of_its_palestinian_citizens_summary_he",
  "order": 142,
  "titleHe": "הדמוקרטיה הישראלית וזכויותיהם של אזרחיה הפלסטינים",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את הדמוקרטיה הישראלית דרך מעמדם של האזרחים הפלסטינים, ומציג את זכויות המיעוט כמבחן לעומק הדמוקרטיזציה בישראל.",
  "summaryHe": "המאמר טוען כי קיים פער חד בין הצהרות ישראליות על שוויון לבין מדיניות שאינה מעניקה שוויון אזרחי מקיף לאזרחים הפלסטינים. הוא מציב את הדיון בתוך נתונים על גודל המיעוט הפלסטיני, עבר של ממשל צבאי, פערים בחינוך, השכלה גבוהה, קרקע, תכנון ודיור. מסקנתו היא שישראל צריכה להתאים את מוסדותיה ואינטרסיה הציבוריים גם לאזרחיה הפלסטינים ולהעניק להם אזרחות שווה בפועל.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Raphael Cohen-Almagor",
  "paperTitle": "Israeli Democracy and the Rights of Its Palestinian Citizens",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-5869-9243\" target=\"_blank\" rel=\"noopener noreferrer\">Raphael Cohen-Almagor</a>",
  "journal": "Ragion pratica",
  "dateText": "פורסם: 2015",
  "doiUrl": "https://doi.org/10.1415/81523",
  "doiLabel": "https://doi.org/10.1415/81523",
  "authors": [
    {
      "@type": "Person",
      "name": "Raphael Cohen-Almagor",
      "url": "https://orcid.org/0000-0002-5869-9243"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Raphael Cohen-Almagor",
      "url": "https://orcid.org/0000-0002-5869-9243"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "זכויות האזרחים הפלסטינים בישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_390_minorities_democracy_israeli_democracy_and_the_rights_of_its_palestinian_citizens_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על זכויות האזרחים הפלסטינים בישראל",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "HelenOnline",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Israel_supporters_in_Cape_Town_5.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/2/27/Israel_supporters_in_Cape_Town_5.jpg",
    "fileTitle": "Israel supporters in Cape Town 5.jpg",
    "sourceDimensions": "4776x3184",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:minorities",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מציב את זכויות האזרחים הפלסטינים כמבחן מרכזי לפער בין שוויון מוצהר לבין מדיניות ישראלית בפועל.",
  "sections": [
    {
      "headingHe": "למה זכויות מיעוט הן מבחן לדמוקרטיה?",
      "paragraphsHtml": [
        "נקודת המוצא של המאמר היא שמעמד המיעוטים אינו נספח למדינה דמוקרטית אלא דרך מרכזית למדוד את עומק הדמוקרטיזציה שלה.",
        "כאשר מיעוט הוא חלק בלתי נפרד מן המדינה אך אינו מקבל יחס שווה, הדמוקרטיה נפגעת לא רק במישור הזכויות אלא גם במישור הלגיטימציה הפוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מציג את האזרחים הפלסטינים כמיעוט גדול בתוך ישראל: ערב 2015 מנתה האוכלוסייה בישראל 8,296,000 נפש, והפלסטינים היו 20.7 אחוז ממנה, 1,709,900 בני אדם. רובם מוסלמים, ולצדם נוצרים, דרוזים ובדואים; רוב האוכלוסייה הערבית מתגוררת בגליל, וחלקים קטנים יותר במשולש ובנגב.",
        "הניתוח מזכיר כי בין 1948 ל-1966 חיו אזרחי ישראל הערבים תחת ממשל צבאי, עם הגבלות חמורות על זכויות וחירויות ותפיסה ביטחונית חשדנית כלפיהם. לאחר ביטול הממשל הצבאי החל תהליך השתלבות אזרחית, אך המאמר טוען שהיחסים בין הרוב היהודי למיעוט הערבי נותרו רחוקים משוויון ממשי.",
        "המאמר מפרט פערים מוסדיים בתחומי חינוך, השכלה גבוהה, קרקע ותכנון. הוא מציין כי ב-2009 רק 31 אחוז מן הפלסטינים עמדו בתנאי קבלה לאוניברסיטה בבחינות הבגרות לעומת 76 אחוז מן היהודים, וכי ב-2011-2012 היו הערבים 11 אחוז מתלמידי התואר הראשון, 7 אחוז מתלמידי התואר השני, 3 אחוז מתלמידי הדוקטורט ו-2 אחוז מהסגל האקדמי. בתחום הקרקע והתכנון הוא מצביע על בעלות ערבית של 2.5 אחוז בלבד מקרקעות ישראל, על מגבלות של הקרן הקיימת על מכירה או החכרה ללא-יהודים, ועל פערים גדולים במכרזי תעשייה, מסחר ודיור לטובת יישובים יהודיים."
      ]
    },
    {
      "headingHe": "מה הפער בין הצהרות לבין מעשים?",
      "paragraphsHtml": [
        "לפי המאמר, כהן־אלמגור מדגיש פער בין מחויבות מוצהרת לעקרונות שוויון לבין הימנעות ממימוש מדיניות שוויונית מקיפה.",
        "הפער הזה חשוב משום שהוא אינו מתמצה באירוע נקודתי. הוא מתואר כדפוס עקבי שבו השפה הרשמית מכירה בשוויון, אך המוסדות והמדיניות אינם מעניקים אותו במלואו."
      ]
    },
    {
      "headingHe": "אילו אזרחים עומדים במרכז הניתוח?",
      "paragraphsHtml": [
        "המאמר עוסק באזרחים הפלסטינים של ישראל, המכונים לעתים גם אזרחים ערבים. הוא אינו עוסק רק ביחסי ישראל והפלסטינים בשטחים, אלא במיעוט אזרחי החי בתוך גבולות המדינה ובתוך מסגרת אזרחות ישראלית.",
        "בחירה זו משנה את מוקד הדיון: השאלה אינה רק ביטחון או סכסוך לאומי, אלא שוויון אזרחי, כבוד, השתתפות וזכויות בתוך המדינה עצמה."
      ]
    },
    {
      "headingHe": "מהי הביקורת המרכזית על ישראל?",
      "paragraphsHtml": [
        "התקציר קובע שהביקורת על יחס ישראל לאזרחיה הפלסטינים מגיעה הן מיריבים והן מידידים, וכי היא מוצדקת.",
        "הטענה היא שאפליה בתחומי חיים שונים מחייבת לא רק תיקונים נקודתיים אלא תפיסה אזרחית רחבה יותר, המבקשת להעניק זכויות שוות ללא הבדל דת, לאום, מין או גזע."
      ]
    },
    {
      "headingHe": "איך מגילת העצמאות נכנסת לתמונה?",
      "paragraphsHtml": [
        "המאמר קושר את דרישת השוויון לעקרונות המוצהרים של מדינת ישראל עצמה. לכן הוא אינו מציב את השוויון כיבוא חיצוני, אלא כמבחן למימוש ההבטחה הפוליטית שהמדינה ניסחה לעצמה.",
        "במובן זה, המאמר קורא את הדמוקרטיה הישראלית דרך המתח בין זהות לאומית יהודית לבין חובה אזרחית שוויונית כלפי כלל אזרחי המדינה."
      ]
    },
    {
      "headingHe": "מה אסור לטשטש בסיכום המאמר?",
      "paragraphsHtml": [
        "אין להציג את המאמר כדיון כללי במיעוטים או כדף הסברה על דמוקרטיה ישראלית. מוקד המאמר הוא הפער המוסדי והאזרחי ביחס לאזרחים הפלסטינים.",
        "הסיכום צריך לשמור על החריפות הנורמטיבית של הטענה: זכויות שוות אינן מחווה פוליטית אלא תנאי יסוד להערכת איכותה הדמוקרטית של ישראל."
      ]
    }
  ],
  "layout": "paper",
  "title": "הדמוקרטיה הישראלית וזכויותיהם של אזרחיה הפלסטינים",
  "permalink": "/democracy_israeli_democracy_and_the_rights_of_its_palestinian_citizens_summary_he.html",
  "paper_url": "democracy_israeli_democracy_and_the_rights_of_its_palestinian_citizens_summary_he.html",
  "sortKey": 202605210151
}
---
