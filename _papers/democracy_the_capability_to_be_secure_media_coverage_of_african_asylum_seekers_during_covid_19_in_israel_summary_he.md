---
{
  "version": 1,
  "file": "democracy_the_capability_to_be_secure_media_coverage_of_african_asylum_seekers_during_covid_19_in_israel_summary_he.html",
  "slug": "democracy_the_capability_to_be_secure_media_coverage_of_african_asylum_seekers_during_covid_19_in_israel_summary_he",
  "order": 231,
  "titleHe": "סיקור מבקשי מקלט אפריקאים בקורונה והיכולת להיות בטוחים",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בתקשורת, דעת קהל ואחריות דמוקרטית בישראל, וממקם את הדיון בתוך שאלות של דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את תקשורת, דעת קהל ואחריות דמוקרטית בישראל דרך עדשה של דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
  "authorsCardHe": "Baruch Shomron",
  "paperTitle": "The Capability ‘To Be Secure’: Media Coverage of African Asylum Seekers During Covid-19 in Israel",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-3943-782X\" target=\"_blank\" rel=\"noopener noreferrer\">Baruch Shomron</a>",
  "journal": "Journal of Refugee Studies",
  "dateText": "פורסם: 2020",
  "doiUrl": "https://doi.org/10.1093/jrs/feaa131",
  "doiLabel": "https://doi.org/10.1093/jrs/feaa131",
  "authors": [
    {
      "@type": "Person",
      "name": "Baruch Shomron",
      "url": "https://orcid.org/0000-0002-3943-782X"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Baruch Shomron",
      "url": "https://orcid.org/0000-0002-3943-782X"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דעת קהל",
    "קיטוב",
    "מחאה",
    "בחירות",
    "אלימות פוליטית",
    "תקשורת",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_the_capability_to_be_secure_media_coverage_of_african_asylum_seekers_during_covid_19_in_israel_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על תקשורת, דעת קהל ואחריות דמוקרטית בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את תקשורת, דעת קהל ואחריות דמוקרטית בישראל ומראה מדוע הדיון קשור לדעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את תקשורת, דעת קהל ואחריות דמוקרטית בישראל, וממקם את הסוגיה בתוך הדיון הרחב על דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: תקציר ההשלכות הבריאותיות והכלכליות של קוביד-19 לא השפיעו באופן שווה על הקבוצות השונות בחברה, במיוחד על הקהילות החלשות ביותר, ובראשם מבקשי מקלט שנפגעו בצורה הקשה ביותר. מחקר זה מתמקד בתפקידה של התקשורת הישראלית במהלך המשבר הזה, והאם היא דגלה או הפריעה למבקשי מקלט אפריקאים את היכולת \"להיות בטוחים\". המחקר חושף תוצאות מעורבות מכיוון שהתקשורת הישראלית גם מעכבת וגם דוגלת בביטחון מבקשי המקלט במהלך מגיפת קוביד-19.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת תקשורת, דעת קהל ואחריות דמוקרטית בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בתקשורת, דעת קהל ואחריות דמוקרטית בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את תקשורת, דעת קהל ואחריות דמוקרטית בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>תקשורת, דעת קהל ואחריות דמוקרטית בישראל</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בתקשורת, דעת קהל ואחריות דמוקרטית בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "סיקור מבקשי מקלט אפריקאים בקורונה והיכולת להיות בטוחים",
  "permalink": "/democracy_the_capability_to_be_secure_media_coverage_of_african_asylum_seekers_during_covid_19_in_israel_summary_he.html",
  "paper_url": "democracy_the_capability_to_be_secure_media_coverage_of_african_asylum_seekers_during_covid_19_in_israel_summary_he.html",
  "sortKey": 202605210231
}
---
