---
{
  "version": 1,
  "file": "democracy_enshrining_discrimination_israel_s_nation_state_law_summary_he.html",
  "slug": "democracy_enshrining_discrimination_israel_s_nation_state_law_summary_he",
  "order": 33,
  "titleHe": "עיגון האפליה בחוק הלאום",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בחוק הלאום, זהות לאומית, שוויון אזרחי והמסגרת החוקתית בישראל, וממקם את הדיון בתוך שאלות של זהות חוקתית, זכויות שוות ומעמדן של קבוצות מיעוט. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא בוחן כיצד הסדרים מוסדיים וחברתיים משפיעים על זכויות, אחריות ציבורית וכללי משחק משותפים.",
  "summaryHe": "המאמר עוסק בחוק הלאום, זהות לאומית, שוויון אזרחי והמסגרת החוקתית בישראל.",
  "authorsCardHe": "Nadia Ben-Youssef, Sandra Samaan Tamari",
  "paperTitle": "Enshrining Discrimination: Israel's Nation-State Law",
  "authorsHtml": "Nadia Ben-Youssef; Sandra Samaan Tamari",
  "journal": "",
  "dateText": "פורסם: 2018",
  "doiUrl": "https://doi.org/10.1525/jps.2018.48.1.73",
  "doiLabel": "https://doi.org/10.1525/jps.2018.48.1.73",
  "authors": [
    {
      "@type": "Person",
      "name": "Nadia Ben-Youssef"
    },
    {
      "@type": "Person",
      "name": "Sandra Samaan Tamari"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Nadia Ben-Youssef"
    },
    {
      "@type": "Person",
      "name": "Sandra Samaan Tamari"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה"
  ],
  "image": {
    "src": "html_qa/democracy_enshrining_discrimination_israel_s_nation_state_law_summary_he.jpg",
    "version": "2026-05-21-topic-image-diversity",
    "altHe": "איור עריכתי: עיגון האפליה בחוק הלאום",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר עוסק בחוק הלאום, זהות לאומית, שוויון אזרחי והמסגרת החוקתית בישראל, ומראה מדוע הדיון בנושא קשור לזהות חוקתית, זכויות שוות ומעמדן של קבוצות מיעוט. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר עוסק בחוק הלאום, זהות לאומית, שוויון אזרחי והמסגרת החוקתית בישראל.",
        "הקריאה בו רלוונטית משום שהנושא משפיע על זהות חוקתית, זכויות שוות ומעמדן של קבוצות מיעוט, ולא רק על שאלה משפטית או פוליטית נקודתית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מאמר זה מדגיש שלוש מהנחות היסוד המרכזיות של החוק: עליונותם המושרשת של מתיישבים יהודים; מחיקת הפלסטינים הילידים; ובהתייחס לגבולות, סיפוח יעיל של אותם חלקים של פלסטין ההיסטורית שנכבשו ב-1967. המחברים טוענים כי האלטרנטיבה לחוק מדינת הלאום המדיר, מסגרת מבוססת זכויות, ממוקדת באנשים, היא דרך מבטיחה לא רק להבטיח את זכויות הפלסטינים, אלא גם לקדם מאבק אוניברסלי לשוויון וצדק היסטורי.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי השאלה הדמוקרטית המרכזית?",
      "paragraphsHtml": [
        "השאלה המרכזית היא כיצד הנושא משנה את האיזון בין כוח שלטוני, זכויות, ביקורת ציבורית ואמון במוסדות.",
        "המסגור מתמקד בשאלה המוסדית והאזרחית שעולה מן המקור."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית תלויה לא רק בבחירות, אלא גם בזהות חוקתית, זכויות שוות ומעמדן של קבוצות מיעוט.",
        "לכן גם נושא שנראה מקצועי או מוסדי יכול להשפיע על השאלה אם כללי המשחק נשארים הוגנים, יציבים ופתוחים לביקורת."
      ]
    },
    {
      "headingHe": "אילו מוסדות או זכויות נמצאים ברקע?",
      "paragraphsHtml": [
        "ברקע עומדות שאלות על חלוקת כוח, אחריות שלטונית, זכויות שוות, עצמאות מוסדית והיכולת של קבוצות שונות להשתתף במרחב הציבורי.",
        "הדגש אינו על הכרעה מפלגתית, אלא על התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לישראל?",
      "paragraphsHtml": [
        "החיבור הישראלי נמצא בכך שהנושא משפיע על הדרך שבה מוסדות המדינה, קבוצות אזרחיות והציבור הרחב מבינים דמוקרטיה בפועל.",
        "במיוחד חשוב לשים לב להשפעה המצטברת של כללים, פרקטיקות ונורמות על אמון הציבור ועל זכויות שוות."
      ]
    },
    {
      "headingHe": "מה הקשר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק דורש שכללים יחולו גם על בעלי כוח, ושיהיו מנגנונים שיכולים לבקר החלטות שלטוניות.",
        "כאשר הדיון עוסק בחוק הלאום, זהות לאומית, שוויון אזרחי והמסגרת החוקתית בישראל, שאלת שלטון החוק היא האם נשמרים גבולות אפקטיביים לכוח והאם קיימת הגנה ממשית על זכויות."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה הציבורית?",
      "paragraphsHtml": [
        "השורה התחתונה היא שהנושא הוא חלק מן האופן שבו דמוקרטיה שומרת על גבולות כוח, זכויות ואמון ציבורי.",
        "התרומה של העמוד היא לסמן את השאלה הדמוקרטית ולכוון לקריאת המקור בלי להוסיף שמות, ציטוטים או ייחוסים שאינם מבוססים."
      ]
    }
  ],
  "layout": "paper",
  "title": "עיגון האפליה בחוק הלאום",
  "permalink": "/democracy_enshrining_discrimination_israel_s_nation_state_law_summary_he.html",
  "paper_url": "democracy_enshrining_discrimination_israel_s_nation_state_law_summary_he.html",
  "sortKey": 202605210103
}
---
