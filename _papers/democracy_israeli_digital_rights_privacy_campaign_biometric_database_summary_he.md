---
{
  "version": 1,
  "file": "democracy_israeli_digital_rights_privacy_campaign_biometric_database_summary_he.html",
  "slug": "democracy_israeli_digital_rights_privacy_campaign_biometric_database_summary_he",
  "titleHe": "קמפיין הזכויות הדיגיטליות בישראל מראה איך חברה אזרחית מגינה על פרטיות",
  "subtitleHe": "",
  "descriptionHe": "המאמר מנתח את קמפיין התנועה לזכויות דיגיטליות בישראל נגד המאגר הביומטרי בין השנים 2009 ל-2017. הוא מצביע על שלוש מסגרות שכנוע: חוסר נחיצות של המאגר בדמוקרטיה, חוסר יעילותו ואי-כשירות ממשלתית באבטחתו.",
  "summaryHe": "Daskal חוקרת כיצד תנועה אזרחית לזכויות דיגיטליות ניסתה לשכנע נגד מאגר ביומטרי בישראל. המחקר מבוסס על פרסומי התנועה ועל השיח העיתונאי סביב הקמפיין בזירות הפוליטית, המשפטית והציבורית בשנים 2009-2017. התוצאה היא ניתוח של פרטיות כתחום שבו חברה אזרחית יכולה לתרגם מומחיות טכנולוגית למסגור דמוקרטי של כוח מדינתי.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Efrat Daskal",
  "paperTitle": "The Israeli Digital Rights Movement's campaign for privacy",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-3772-2173\" target=\"_blank\" rel=\"noopener noreferrer\">Efrat Daskal</a>",
  "journal": "Internet Policy Review, 6(3)",
  "dateText": "פורסם: 19.09.2017",
  "doiUrl": "https://doi.org/10.14763/2017.3.711",
  "doiLabel": "https://doi.org/10.14763/2017.3.711",
  "authors": [
    {
      "@type": "Person",
      "name": "Efrat Daskal",
      "url": "https://orcid.org/0000-0003-3772-2173"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Efrat Daskal",
      "url": "https://orcid.org/0000-0003-3772-2173"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "זכויות דיגיטליות",
    "פרטיות",
    "מאגר ביומטרי",
    "חברה אזרחית",
    "ממשל דיגיטלי",
    "ישראל 2009-2017"
  ],
  "image": {
    "src": "assets/article-images/real_photo_396_elections_democracy_israeli_digital_rights_privacy_campaign_biometric_database_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לקמפיין הזכויות הדיגיטליות בישראל מראה איך חברה אזרחית מגינה על פרטיות",
    "fitness": "high",
    "license": "CC0",
    "creator": "Rakoon",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:20190409_Israeli_legislative_election_2019.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/8/88/20190409_Israeli_legislative_election_2019.jpg",
    "fileTitle": "20190409 Israeli legislative election 2019.jpg",
    "sourceDimensions": "4608x2592",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:elections",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/publicdomain/zero/1.0/"
  },
  "datePublished": "2026-05-31",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המאמר מציג את פרטיות המידע כזירה דמוקרטית מובהקת. הוא מראה כיצד קמפיין אזרחי יכול לתקוף מאגר ביומטרי לא רק בשם הזכות לפרטיות, אלא גם בשם נחיצות, יעילות ואחריות שלטונית.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>פרטיות דיגיטלית</strong> היא ההגנה על מידע אישי כאשר המדינה או גופים אחרים אוספים, מעבדים ומחברים נתונים. במקרה של מאגר ביומטרי, הפגיעה הפוטנציאלית עמוקה משום שהמידע קשור לגוף ולא ניתן להחלפה כמו סיסמה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המחקר בוחן את טכניקות השכנוע שבהן משתמשת התנועה הישראלית לזכויות דיגיטליות בקמפיין שלה נגד המאגר הביומטרי של ישראל. התוצאות מדגימות כיצד הארגון ניווט בשלוש מסגרות שכנוע כדי להשיג את מטרותיו: חוסר הצורך במאגר ביומטרי בדמוקרטיה; חוסר היעילות של בסיס הנתונים; וחוסר יכולת שלטונית באבטחתו. המאמר מסכם בדיון כיצד ניתוח קמפיינים לפרטיות בחברה האזרחית יכול לשפוך אור על משטרים שונים של ניהול פרטיות.",
        "Daskal חוקרת כיצד תנועה אזרחית לזכויות דיגיטליות ניסתה לשכנע נגד מאגר ביומטרי בישראל. המחקר מבוסס על פרסומי התנועה ועל השיח העיתונאי סביב הקמפיין בזירות הפוליטית, המשפטית והציבורית בשנים 2009-2017. התוצאה היא ניתוח של פרטיות כתחום שבו חברה אזרחית יכולה לתרגם מומחיות טכנולוגית למסגור דמוקרטי של כוח מדינתי."
      ]
    },
    {
      "headingHe": "מה חקר המאמר?",
      "paragraphsHtml": [
        "המאמר חוקר את קמפיין התנועה הישראלית לזכויות דיגיטליות נגד המאגר הביומטרי. הוא בוחן כיצד פעילים ניסחו טיעונים, פעלו מול זירות מוסדיות והשפיעו על שיח ציבורי סביב פרטיות."
      ]
    },
    {
      "headingHe": "איזו תקופה נבחנה?",
      "paragraphsHtml": [
        "המחקר מתמקד בשנים 2009-2017. זו תקופה ארוכה מספיק כדי לראות קמפיין שמתנהל בו זמנית מול פוליטיקה, משפט, תקשורת וציבור רחב."
      ]
    },
    {
      "headingHe": "מה היו שלוש מסגרות השכנוע המרכזיות?",
      "paragraphsHtml": [
        "התקציר מציין שלוש מסגרות: המאגר אינו נחוץ בדמוקרטיה, המאגר אינו יעיל, והממשלה אינה כשירה להבטיח את אבטחתו. השילוב בין מסגרות ערכיות, מעשיות ומוסדיות חיזק את הטענה נגד הרחבת איסוף הנתונים."
      ]
    },
    {
      "headingHe": "למה חוסר נחיצות הוא טיעון דמוקרטי?",
      "paragraphsHtml": [
        "כאשר המדינה מבקשת לאסוף מידע רגיש, נטל ההצדקה צריך להיות גבוה. טענת חוסר הנחיצות אומרת שגם אם טכנולוגיה אפשרית, דמוקרטיה ליברלית צריכה לשאול אם היא הכרחית ומידתית."
      ]
    },
    {
      "headingHe": "איך טיעון היעילות שונה מטיעון הזכויות?",
      "paragraphsHtml": [
        "טיעון היעילות אינו אומר רק שהזכות לפרטיות חשובה, אלא גם שהמדיניות אינה משיגה את יעדיה. הוא מאפשר לקמפיין לדבר גם אל מי שאינם מתחילים מערכי פרטיות, אך מודאגים ממדיניות ציבורית לא יעילה."
      ]
    },
    {
      "headingHe": "מדוע אי-כשירות ממשלתית היא מסגרת חזקה?",
      "paragraphsHtml": [
        "מאגר ביומטרי מחייב אמון ביכולת המדינה לשמור מידע רגיש לאורך זמן. כאשר הקמפיין מדגיש כשלים אפשריים באבטחה, הוא מציב את שאלת הפרטיות בתוך דיון רחב יותר על יכולת, אחריות ובקרה ממשלתית."
      ]
    },
    {
      "headingHe": "מה התפקיד של חברה אזרחית בקמפיין כזה?",
      "paragraphsHtml": [
        "חברה אזרחית יכולה לתרגם ידע טכנולוגי ומומחיות משפטית לשפה ציבורית נגישה. בכך היא מאזנת פערי כוח בין אזרחים לבין מדינה שמחזיקה בכלים מתקדמים לאיסוף מידע."
      ]
    },
    {
      "headingHe": "מה המאמר מוסיף להבנת משטרי פרטיות?",
      "paragraphsHtml": [
        "המאמר מראה שמשטרי פרטיות אינם נקבעים רק בחוק או בטכנולוגיה. הם נוצרים גם דרך קמפיינים, מסגור תקשורתי, התנגדות אזרחית ויכולת להציג איסוף מידע כבעיה דמוקרטית."
      ]
    },
    {
      "headingHe": "איך הקמפיין קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק מחייב שהפעלת כוח מדינתי תהיה מוסמכת, מידתית ומבוקרת. מאגר ביומטרי מציב מבחן חד במיוחד לשאלה אם פיקוח משפטי וציבורי יכולים לרסן איסוף מידע רגיש."
      ]
    },
    {
      "headingHe": "מה לא כדאי להסיק מהמחקר?",
      "paragraphsHtml": [
        "לא כדאי להסיק שכל שימוש ממשלתי במידע ביומטרי פסול בהכרח לפי המאמר. הקריאה המדויקת היא שהמאבק הציבורי סביב המאגר חשף כיצד נחיצות, יעילות ואמון מוסדי חייבים להיבחן יחד."
      ]
    },
    {
      "headingHe": "למי המאמר רלוונטי?",
      "paragraphsHtml": [
        "המאמר רלוונטי למחוקקים, ארגוני זכויות דיגיטליות, מומחי פרטיות וחוקרי מדיניות ציבורית. הוא חשוב במיוחד לכל דיון על זיהוי דיגיטלי, מאגרי מידע ומנגנוני פיקוח על טכנולוגיות מדינה."
      ]
    }
  ],
  "layout": "paper",
  "title": "קמפיין הזכויות הדיגיטליות בישראל מראה איך חברה אזרחית מגינה על פרטיות",
  "permalink": "/democracy_israeli_digital_rights_privacy_campaign_biometric_database_summary_he.html",
  "paper_url": "democracy_israeli_digital_rights_privacy_campaign_biometric_database_summary_he.html",
  "sortKey": 202605310006,
  "sourcePublishedDate": "2017-09-19"
}
---
