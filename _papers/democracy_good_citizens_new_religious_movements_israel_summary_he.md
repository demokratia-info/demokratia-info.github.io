---
{
  "version": 1,
  "file": "democracy_good_citizens_new_religious_movements_israel_summary_he.html",
  "slug": "democracy_good_citizens_new_religious_movements_israel_summary_he",
  "titleHe": "תנועות דתיות חדשות בישראל חיפשו לגיטימציה דרך שפת האזרחות הטובה",
  "subtitleHe": "המאמר מראה כיצד קבוצות שנתפסו כזרות או ככתות אימצו אסטרטגיות רפובליקניות ונאו־ליברליות כדי להפחית התנגדות ציבורית.",
  "descriptionHe": "המאמר מנתח כיצד תנועות דתיות חדשות בישראל ביקשו להשיג לגיטימציה ציבורית ומוסדית באמצעות שיח של אזרחות טובה, ציונות, שירות ותרומה חברתית.",
  "summaryHe": "המאמר בוחן את האנתרופוסופיה ואת תנועת אמין בישראל על בסיס ראיונות, דיווחים תקשורתיים, מסמכים רשמיים ודיונים ציבוריים. הניתוח מראה ששתי אסטרטגיות לגיטימציה מרכזיות סייעו לתנועות להפחית התנגדות: התאמה לנרטיב אזרחות רפובליקני-ציוני של התיישבות ושירות, והתאמה לשיח נאו-ליברלי של חינוך, הצלחה וניידות חברתית.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Guy Ben‐Porat, Boaz Huss",
  "paperTitle": "Good citizens: legitimization strategies of new religious movements in Israel",
  "authorsHtml": "<a href=\"https://cris.bgu.ac.il/en/persons/guy-ben-porat\" target=\"_blank\" rel=\"noopener noreferrer\">Guy Ben‐Porat</a>, <a href=\"https://cris.bgu.ac.il/en/persons/boaz-huss\" target=\"_blank\" rel=\"noopener noreferrer\">Boaz Huss</a>",
  "journal": "Politics and Religion 18(1), 130-148",
  "dateText": "פורסם מקוון: 11 בפברואר 2025; בגיליון 18(1), מרץ 2025",
  "doiUrl": "https://doi.org/10.1017/s175504832400035x",
  "doiLabel": "https://doi.org/10.1017/s175504832400035x",
  "authors": [
    {
      "@type": "Person",
      "name": "Guy Ben‐Porat",
      "url": "https://cris.bgu.ac.il/en/persons/guy-ben-porat"
    },
    {
      "@type": "Person",
      "name": "Boaz Huss",
      "url": "https://cris.bgu.ac.il/en/persons/boaz-huss"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Guy Ben‐Porat",
      "url": "https://cris.bgu.ac.il/en/persons/guy-ben-porat"
    },
    {
      "@type": "Person",
      "name": "Boaz Huss",
      "url": "https://cris.bgu.ac.il/en/persons/boaz-huss"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "rights-equality-minorities"
  ],
  "keywords": [
    "תנועות דתיות חדשות",
    "אזרחות",
    "לגיטימציה",
    "דת ומדינה",
    "ציונות",
    "נאו־ליברליזם",
    "ישראל"
  ],
  "image": {
    "src": "assets/article-images/real_photo_273_religion_democracy_good_citizens_new_religious_movements_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לתנועות דתיות חדשות בישראל חיפשו לגיטימציה דרך שפת האזרחות הטובה",
    "fitness": "high",
    "license": "CC BY 2.0",
    "creator": "zeevveez from Jerusalem, Israel",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Religious_Jews_at_Mount_of_Olives_Cemetery_(10454803283).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/b/bc/Religious_Jews_at_Mount_of_Olives_Cemetery_%2810454803283%29.jpg",
    "fileTitle": "Religious Jews at Mount of Olives Cemetery (10454803283).jpg",
    "sourceDimensions": "2048x1536",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:religion",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/2.0/"
  },
  "datePublished": "2026-06-19",
  "dateModified": "2026-07-07",
  "lastUpdatedHe": "7 ביולי 2026",
  "oneLinerHtml": "תנועות דתיות חדשות בישראל צמצמו התנגדות כאשר תרגמו שונות דתית לשפת תרומה אזרחית מוכרת.",
  "sections": [
    {
      "headingHe": "המאבק אינו רק על אמונה אלא על לגיטימציה אזרחית",
      "paragraphsHtml": [
        "המאמר מציג תנועות דתיות חדשות בישראל כקבוצות שנאלצו להצדיק את עצם נוכחותן במרחב הציבורי. ההתנגדות אליהן לא נבעה רק מתיאולוגיה, אלא גם מהשאלה מי נחשב אזרח מועיל, נאמן ומוכר."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן תנועות דתיות חדשות בישראל בהקשר שבו אין הפרדה מלאה בין דת למדינה, והאורתודוקסיה היהודית נהנית ממעמד פורמלי ולא-פורמלי חזק. הוא מתמקד באנתרופוסופיה ובתנועת אמין, ובודק כיצד הן התמודדו עם חשד ציבורי, ביקורת ושיח הרואה בתנועות כאלה גורמים זרים או 'כתות'.",
        "הניתוח מבוסס על ראיונות עם פעילים וחברים, דיווחים תקשורתיים, דוחות רשמיים ודיונים בוועדות פרלמנטריות. המאמר מזהה אסטרטגיה אחת של לגיטימציה דרך אזרחות רפובליקנית: אימוץ שפה ציונית, השתלבות באתוס ההתיישבות והדגשת שירות צבאי או תרומה למדינה.",
        "האסטרטגיה השנייה נשענת על מושג נאו-ליברלי של אזרחות טובה: חינוך, יכולות אישיות, הצלחה וניידות חברתית. במקרה האנתרופוסופי, למשל, הוצגו חינוך ולדורף ובוגריו כמי שמפתחים עצמאות, חוסן, אחריות ויכולת התמודדות, ובכך מספקים לא רק מסלול אישי להצלחה אלא גם תרומה רפובליקנית בדמות הכנה לחיים אזרחיים ולשירות."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: אזרחות רפובליקנית ונאו־ליברלית",
      "paragraphsHtml": [
        "אזרחות רפובליקנית מדגישה תרומה לקולקטיב דרך שירות, נאמנות והשתתפות במפעלים לאומיים. אזרחות נאו־ליברלית מדגישה הצלחה אישית, חינוך, יזמות וניידות חברתית, ובמאמר שתי השפות משמשות תנועות דתיות כדי להיראות לגיטימיות."
      ]
    },
    {
      "headingHe": "למה ישראל היא מקרה חשוב?",
      "paragraphsHtml": [
        "בישראל יש דומיננטיות מוסדית וחברתית של דת אורתודוקסית, ולכן קבוצות דתיות חדשות נתפסות לעיתים כזרות או מאיימות. המקרה הישראלי מחדד כיצד פלורליזם דתי נבחן לא רק בחוק, אלא גם בזירה של דימוי ציבורי וחשד חברתי."
      ]
    },
    {
      "headingHe": "שתי אסטרטגיות של אזרחות טובה",
      "paragraphsHtml": [
        "האסטרטגיה הראשונה מדגישה ציונות, אתוס התיישבות ושירות צבאי כדי להראות השתייכות לרפובליקה הלאומית. האסטרטגיה השנייה מדגישה חינוך, הצלחה וניידות כלכלית כדי להתאים לשיח עכשווי יותר של אחריות אישית ותרומה לשוק."
      ]
    },
    {
      "headingHe": "איך תנועות דתיות חדשות הפחיתו התנגדות?",
      "paragraphsHtml": [
        "הן ניסו להראות שהן אינן גוף זר או מסוכן אלא חלק תורם מן החברה הישראלית. לפי המאמר, הדימוי של אזרח טוב סייע להן לנטרל שיח של כתות ואיום."
      ]
    },
    {
      "headingHe": "למה הצבא וההתיישבות חשובים בניתוח?",
      "paragraphsHtml": [
        "הצבא וההתיישבות משמשים בישראל סמנים חזקים של נאמנות קולקטיבית. כאשר תנועה דתית חדשה מאמצת סמנים כאלה, היא מתרגמת שונות דתית לשפה פוליטית מוכרת."
      ]
    },
    {
      "headingHe": "מה חדש באסטרטגיה הנאו־ליברלית?",
      "paragraphsHtml": [
        "האסטרטגיה הנאו־ליברלית אינה מבקשת רק להוכיח נאמנות לאומית, אלא להציג את הקבוצה כמסלול להצלחה ולשיפור עצמי. היא מתאימה לחברה שבה חינוך, יזמות ותוצאות כלכליות נעשו מקור מרכזי ללגיטימציה."
      ]
    },
    {
      "headingHe": "האם המאמר מצדיק את התנועות עצמן?",
      "paragraphsHtml": [
        "המאמר אינו נדרש להצדיק או לשלול את האמונות של התנועות. הוא מנתח את הדרכים שבהן הן מתמודדות עם התנגדות וממקמות את עצמן בשיח האזרחות הישראלי."
      ]
    },
    {
      "headingHe": "מה הקשר לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית נבחנת ביחסה לקבוצות דתיות לא מוכרות ולא פופולריות. אם לגיטימציה תלויה בהתאמה לדגם צר של אזרחות טובה, הפלורליזם הדתי נשאר מוגבל."
      ]
    },
    {
      "headingHe": "האם מיעוט מספרי יכול להיות איום ציבורי?",
      "paragraphsHtml": [
        "המאמר מדגיש שגם קבוצות קטנות יכולות לעורר התנגדות כאשר הן מסומנות כזרות או ככתות. האיום אינו רק מספרי, אלא קשור למעמד הדת, לזהות הלאומית ולגבולות הנורמליות."
      ]
    },
    {
      "headingHe": "מה תפקיד התקשורת במחקר?",
      "paragraphsHtml": [
        "המחברים משתמשים בין היתר בדיווחים תקשורתיים כדי להבין כיצד התנועות והמתנגדים להן ממסגרים את המחלוקת. התקשורת היא חלק מן הזירה שבה נקבע אם קבוצה מוצגת כאיום או כשותפה אזרחית."
      ]
    },
    {
      "headingHe": "למה חשוב להבחין בין לגיטימציה לזכויות?",
      "paragraphsHtml": [
        "זכויות משפטיות יכולות להתקיים גם כאשר קבוצה סובלת מדימוי ציבורי שלילי. המאמר מראה שהמאבק על לגיטימציה קובע בפועל עד כמה קבוצה יכולה לפעול בלי מגבלות חברתיות ופוליטיות."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד על דת ומדינה בישראל?",
      "paragraphsHtml": [
        "המאמר מלמד שהיחס לקבוצות דתיות חדשות אינו נקבע רק על ידי יחסי חילונים ודתיים מוכרים. הוא מושפע גם מן השאלה איזו דת נתפסת כחלק מהלאום ואיזו דת נתפסת כייבוא זר."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית של המאמר?",
      "paragraphsHtml": [
        "התרומה המרכזית היא זיהוי שתי שפות אזרחות שמסבירות כיצד קבוצות שוליות מבקשות הכרה. בכך המאמר מחבר בין דת, פלורליזם, אזרחות ושינויים בשיח הציבורי הישראלי."
      ]
    }
  ],
  "layout": "paper",
  "title": "תנועות דתיות חדשות בישראל חיפשו לגיטימציה דרך שפת האזרחות הטובה",
  "permalink": "/democracy_good_citizens_new_religious_movements_israel_summary_he.html",
  "paper_url": "democracy_good_citizens_new_religious_movements_israel_summary_he.html",
  "sortKey": 202606190009,
  "sourcePublishedDate": "2025-02-11"
}
---
