---
{
  "version": 1,
  "file": "democracy_extraterritorial_militarism_emigrants_soldiers_israel_summary_he.html",
  "slug": "democracy_extraterritorial_militarism_emigrants_soldiers_israel_summary_he",
  "titleHe": "שירות צבאי ממשיך לעצב אזרחות ישראלית גם מעבר לגבולות המדינה",
  "subtitleHe": "ניתוח דיוני כנסת מראה כיצד שיח מיליטריסטי מדרג ישראלים בחו״ל לפי שירות צבאי בעבר, בהווה או בעתיד.",
  "descriptionHe": "המאמר מציע את המושג מיליטריזם חוץ־טריטוריאלי כדי להסביר כיצד מדינות מעריכות אזרחים החיים מחוץ לשטחן דרך שירותם הצבאי או נכונותם לשרת.",
  "summaryHe": "Jonathan Grossman מנתח דיונים פרלמנטריים בישראל מ־1977 עד 2023 ומראה כיצד מהגרים ישראלים וילדיהם הובנו בשיח הרשמי דרך היחס שלהם לשירות בצה״ל. המאמר מזהה שלוש צורות של מיליטריזם חוץ־טריטוריאלי: אזרחים בחו״ל כלא־חיילים, כחיילים לשעבר וכחיילים עתידיים.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Jonathan Grossman",
  "paperTitle": "Extraterritorial militarism: emigrants as soldiers in Israel",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-7244-2498\" target=\"_blank\" rel=\"noopener noreferrer\">Jonathan Grossman</a>",
  "journal": "Comparative Migration Studies 13(1), Article 50",
  "dateText": "פורסם: 8 ביולי 2025",
  "doiUrl": "https://doi.org/10.1186/s40878-025-00465-9",
  "doiLabel": "https://doi.org/10.1186/s40878-025-00465-9",
  "authors": [
    {
      "@type": "Person",
      "name": "Jonathan Grossman",
      "url": "https://orcid.org/0000-0001-7244-2498"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Jonathan Grossman",
      "url": "https://orcid.org/0000-0001-7244-2498"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "אזרחות",
    "מיליטריזם",
    "מהגרים",
    "שירות צבאי",
    "כנסת",
    "ישראל"
  ],
  "image": {
    "src": "assets/article-images/real_photo_238_security_democracy_extraterritorial_militarism_emigrants_soldiers_israel_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לשירות צבאי ממשיך לעצב אזרחות ישראלית גם מעבר לגבולות המדינה",
    "fitness": "high",
    "license": "CC BY-SA 2.0",
    "creator": "Israel Defense Forces",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Flickr_-_Israel_Defense_Forces_-_US-Israel_Military_Cooperation_During_Juniper_Cobra.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/c/c5/Flickr_-_Israel_Defense_Forces_-_US-Israel_Military_Cooperation_During_Juniper_Cobra.jpg",
    "fileTitle": "Flickr - Israel Defense Forces - US-Israel Military Cooperation During Juniper Cobra.jpg",
    "sourceDimensions": "4256x2832",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:security",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/2.0/"
  },
  "datePublished": "2026-06-17",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כיצד שירות צבאי נעשה מדד לאזרחות טובה גם ביחס לאזרחים ישראלים החיים מחוץ לגבולות המדינה.",
  "sections": [
    {
      "headingHe": "מהו מיליטריזם חוץ־טריטוריאלי?",
      "paragraphsHtml": [
        "<strong>מיליטריזם חוץ־טריטוריאלי</strong> הוא שיח שמחיל ערך צבאי על אזרחים החיים מחוץ למדינה. במקום לראות בהם רק מהגרים או פזורה, המדינה והאליטות הפוליטיות עשויות למדוד אותם לפי עברם הצבאי או נכונותם העתידית לשירות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר טוען שמדינות יכולות להשליך שיח מיליטריסטי גם על אזרחים שחיים מחוץ לשטחן. במקרה הישראלי, אליטות פוליטיות העריכו מהגרים וילדי מהגרים לפי עברם הצבאי או לפי פוטנציאל השירות העתידי שלהם.",
        "המחקר מבוסס על ניתוח שיח של רשומות פרלמנטריות מ־1977 עד 2023, כולל מליאת הכנסת, ועדת העלייה והקליטה, הצעות חוק על הצבעה מחוץ לישראל ודיונים בוועדת החוקה, חוק ומשפט. מתוך החומר מזוהות שלוש צורות של מיליטריזם חוץ־טריטוריאלי: מי שמוצגים כלא־חיילים, כחיילים לשעבר או כחיילים עתידיים.",
        "הניתוח מראה כי מהגרים וילדי מהגרים ששירתו בצה״ל או הביעו נכונות לשרת הוצגו כאזרחים חוץ־טריטוריאליים טובים יותר ממי שלא שירתו או לא צפויים לשרת. בהקשר זה נדונו גם תוכניות לנוער ישראלי בחו״ל, כגון צופי צבר וחץ וקשת, שנועדו לחזק זיקה לישראל ולעודד עלייה וגיוס."
      ]
    },
    {
      "headingHe": "למה ישראל היא מקרה מרכזי לדיון הזה?",
      "paragraphsHtml": [
        "ישראל היא מקרה מתאים משום ששירות צבאי ממלא תפקיד עמוק בשפת האזרחות, השייכות והתרומה הציבורית. כאשר השפה הזאת נמתחת אל אזרחים בחו״ל, היא חושפת גבולות חדשים של הכלה והדרה."
      ]
    },
    {
      "headingHe": "אילו שלוש דמויות אזרחיות מזהה המאמר?",
      "paragraphsHtml": [
        "המאמר מתאר שלושה מסגורים של אזרחים בחו״ל: מי שאינם חיילים, מי שכבר שירתו, ומי שמוצגים כחיילים עתידיים. ההבחנה הזאת חשובה משום שכל מסגור מייצר ציפייה אחרת כלפי אזרחות ונאמנות."
      ]
    },
    {
      "headingHe": "איך דיוני כנסת משמשים מקור מחקרי?",
      "paragraphsHtml": [
        "המחקר נשען על ניתוח שיח של דיונים פרלמנטריים, ולכן הוא בוחן את השפה הפוליטית שבה נציגי ציבור מגדירים אזרחים בחו״ל. המקור הזה מתאים לשאלה משום שהוא מציג כיצד המדינה מדברת על שייכות ולא רק כיצד יחידים חווים אותה."
      ]
    },
    {
      "headingHe": "מה הקשר בין שירות צבאי לאזרחות מלאה?",
      "paragraphsHtml": [
        "במסגור שמנתח המאמר, שירות צבאי הופך לסימן של אזרחות טובה ושל תרומה למדינה. הבעיה הדמוקרטית היא שכאשר סימן אחד נעשה מרכזי מדי, אזרחים שאינם מתאימים לו עלולים להיתפס כפחות שייכים."
      ]
    },
    {
      "headingHe": "מי עלול להישאר מחוץ לשיח הזה?",
      "paragraphsHtml": [
        "המאמר מדגיש שחשוב גם לשאול מי מודר מן השיח של מיליטריזם חוץ־טריטוריאלי. אזרחים שאינם צפויים לשרת, אינם יכולים לשרת או אינם מזוהים עם שירות צבאי עלולים לקבל מעמד סמלי שונה בשפת השייכות."
      ]
    },
    {
      "headingHe": "מדוע זו סוגיה של זכויות ושוויון?",
      "paragraphsHtml": [
        "אזרחות ליברלית אמורה להבטיח זכויות בסיסיות שאינן תלויות במידת התאמה למודל תרומה אחד. כאשר שיח ציבורי מדרג אזרחים לפי שירות, הוא עשוי לערער את ההבחנה בין זכויות אזרח לבין הוקרה חברתית."
      ]
    },
    {
      "headingHe": "איך המאמר משנה את הדיון על פזורה ישראלית?",
      "paragraphsHtml": [
        "הדיון בפזורה מתמקד לעיתים בקשר רגשי, כלכלי או פוליטי למדינת המקור. המאמר מוסיף שכוח צבאי ושפת שירות יכולים להפוך לחלק מן הקשר הזה גם כאשר החיים עצמם מתנהלים מעבר לים."
      ]
    },
    {
      "headingHe": "כיצד צריך לקרוא את הטענה על שיח פוליטי?",
      "paragraphsHtml": [
        "הטענה אינה שכל ישראלי בחו״ל נתפס בפועל דרך השירות הצבאי בלבד. היא עוסקת בשיח פרלמנטרי ובמסגור מוסדי, ולכן היא מזמינה לבדוק מתי, כלפי מי ובאילו תקופות מסגור השירות נעשה חזק יותר."
      ]
    },
    {
      "headingHe": "מהי התרומה הרחבה לחקר אזרחות?",
      "paragraphsHtml": [
        "התרומה היא הרחבת מושג האזרחות אל מעבר לגבול הטריטוריאלי ואל תוך שפת הביטחון. כך המאמר מראה שאזרחות אינה רק סטטוס משפטי, אלא גם מערכת ציפיות חברתיות שמוסדות פוליטיים מייצרים ומתחזקים."
      ]
    }
  ],
  "layout": "paper",
  "title": "שירות צבאי ממשיך לעצב אזרחות ישראלית גם מעבר לגבולות המדינה",
  "permalink": "/democracy_extraterritorial_militarism_emigrants_soldiers_israel_summary_he.html",
  "paper_url": "democracy_extraterritorial_militarism_emigrants_soldiers_israel_summary_he.html",
  "sortKey": 202606170008,
  "sourcePublishedDate": "2025-07-08"
}
---
