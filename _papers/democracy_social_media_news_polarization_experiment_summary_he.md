---
{
  "version": 1,
  "file": "democracy_social_media_news_polarization_experiment_summary_he.html",
  "slug": "democracy_social_media_news_polarization_experiment_summary_he",
  "order": 60,
  "titleHe": "האם חשיפה לחדשות ברשתות חברתיות משנה קיטוב פוליטי?",
  "subtitleHe": "רועי לוי משתמש בניסוי שדה כדי לבדוק כיצד חשיפה לחדשות בפייסבוק משפיעה על אתרי חדשות, עמדות כלפי המפלגה היריבה ודעות פוליטיות.",
  "descriptionHe": "מאמרו של רועי לוי ב-American Economic Review מציג ניסוי שדה על צריכת חדשות ברשתות חברתיות, חשיפה לתוכן תואם או מנוגד לעמדות, והשפעת אלגוריתמים על קיטוב פוליטי.",
  "summaryHe": "המאמר מראה שרשתות חברתיות משפיעות על סוג החדשות שאנשים צורכים ועל רגשות כלפי היריב הפוליטי, אך לא בהכרח משנות דעות מדיניות.",
  "authorsCardHe": "רועי לוי",
  "paperTitle": "Social Media, News Consumption, and Polarization: Evidence from a Field Experiment",
  "authorsHtml": "<a href=\"https://english.tau.ac.il/profile/roeelevy\" target=\"_blank\" rel=\"noopener noreferrer\">Roee Levy</a>",
  "journal": "American Economic Review",
  "dateText": "American Economic Review, כרך 111, גיליון 3, עמ׳ 831-870; פורסם ב-1 במרץ 2021",
  "doiUrl": "https://doi.org/10.1257/aer.20191777",
  "doiLabel": "https://doi.org/10.1257/aer.20191777",
  "authors": [
    {
      "@type": "Person",
      "name": "רועי לוי",
      "url": "https://english.tau.ac.il/profile/roeelevy"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ro'ee Levy",
      "url": "https://english.tau.ac.il/profile/roeelevy"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "רשתות חברתיות",
    "צריכת חדשות",
    "קיטוב פוליטי",
    "פייסבוק",
    "ניסוי שדה",
    "אלגוריתמים",
    "עמדות כלפי יריבים"
  ],
  "image": {
    "src": "html_qa/democracy_civil_society_ngos_backsliding_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על חדשות ברשתות חברתיות, שיח ציבורי וקיטוב פוליטי",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "הניסוי של לוי מראה שחשיפה לחדשות ברשתות חברתיות משנה את מסלולי החשיפה למידע ואת היחס הרגשי ליריבים פוליטיים, אך אינה בהכרח משנה דעות מדיניות. הדמוקרטיה נפגעת במיוחד כאשר אלגוריתמים מצמצמים חשיפה לתוכן מנוגד לעמדות ומחזקים הפרדה בין מחנות.",
  "sections": [
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מציע ראיה ניסויית לשאלה שמעסיקה דמוקרטיות רבות: האם רשתות חברתיות רק משקפות קיטוב, או גם מייצרות אותו. לוי בוחן זאת באמצעות ניסוי שדה שבו אנשים קיבלו אפשרות להיחשף לעמודי חדשות ליברליים או שמרניים בפייסבוק.",
        "המחקר עוקב אחר שרשרת ההשפעה: מנוי לעמודים, חשיפה בפיד, ביקורים באתרי חדשות, שיתוף פוסטים, עמדות פוליטיות ורגשות כלפי המפלגה היריבה. בכך הוא אינו מסתפק במדידה של קליקים, אלא מחבר בין התנהגות מידע לבין קיטוב דמוקרטי.",
        "התרומה המרכזית היא הבחנה בין דעות לבין רגשות. חשיפה לתוכן מנוגד יכולה להפחית עוינות כלפי הצד השני גם כאשר היא אינה משנה עמדות מדיניות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: אני מעריך את ההשפעות של חשיפת חדשות במדיה החברתית על ידי ביצוע ניסוי שטח גדול המציע למשתתפים הרשמות לרשתות חדשות שמרניות או ליברליות בפייסבוק. אני אוספת נתונים על השרשרת הסיבתית של השפעות התקשורת: מנויים לשקעים, חשיפה לחדשות בפייסבוק, ביקורים באתרי חדשות מקוונים ושיתוף פוסטים וכן שינויים בדעות וגישות פוליטיות. יחד, התוצאות מצביעות על כך שאלגוריתמים של מדיה חברתית עשויים להגביל את החשיפה לחדשות נגד גישה ובכך להגביר את הקיטוב.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר מראה שרשתות חברתיות משפיעות על סוג החדשות שאנשים צורכים ועל רגשות כלפי היריב הפוליטי, אך לא בהכרח משנות דעות מדיניות."
      ]
    },
    {
      "headingHe": "מהם הממצאים המרכזיים?",
      "paragraphsHtml": [
        "הניסוי מצא שחשיפה אקראית לחדשות ברשתות חברתיות משפיעה באופן משמעותי על הנטייה האידיאולוגית של אתרי החדשות שבהם אנשים מבקרים.",
        "חשיפה לחדשות מנוגדות לעמדות הפחיתה רגשות שליליים כלפי המפלגה היריבה, אך המחקר לא מצא ראיה לכך שהנטייה הפוליטית של כלי החדשות שינתה דעות פוליטיות."
      ]
    },
    {
      "headingHe": "איזה תפקיד יש לאלגוריתם?",
      "paragraphsHtml": [
        "תקציר המאמר מציין שאלגוריתם פייסבוק היה פחות נוטה לספק לאנשים פוסטים ממקורות מנוגדים לעמדותיהם, גם כאשר הם נרשמו אליהם.",
        "זה ממצא דמוקרטי חשוב: גם בחירה מודעת של משתמש להיחשף לקול אחר יכולה להיחסם או להיחלש על ידי מנגנון המלצה שאינו ניטרלי מבחינה ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לשיחה על קיטוב?",
      "paragraphsHtml": [
        "קיטוב אינו רק פער בעמדות. הוא גם עוינות, חשדנות וחוסר נכונות לראות את הצד השני כשותף לגיטימי במשחק הדמוקרטי.",
        "המאמר מראה שאפשר להשפיע על הרכיב הרגשי של הקיטוב באמצעות חשיפה לתוכן מנוגד, ולכן עיצוב סביבת המידע הוא חלק מן התשתית של דמוקרטיה מתפקדת."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד חשיפה לחדשות ברשתות חברתיות משפיעה על <strong>צריכת מידע</strong>, <strong>קיטוב רגשי</strong> ודעות פוליטיות.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "מהו ניסוי שדה בהקשר הזה?",
      "paragraphsHtml": [
        "ניסוי שדה בוחן התנהגות בעולם האמיתי. כאן המשתתפים נחשפו באופן אקראי לאפשרות לעקוב אחר מקורות חדשות שונים בפייסבוק."
      ]
    },
    {
      "headingHe": "האם חדשות מנוגדות לעמדות משנות דעות?",
      "paragraphsHtml": [
        "לפי הממצאים, לא נמצאה ראיה ברורה לשינוי בדעות פוליטיות עצמן. ההשפעה הבולטת יותר הייתה על רגשות כלפי המפלגה היריבה."
      ]
    },
    {
      "headingHe": "למה הפחתת רגשות שליליים חשובה?",
      "paragraphsHtml": [
        "בדמוקרטיה, יריבים פוליטיים צריכים להיתפס כלגיטימיים. הפחתת עוינות יכולה לעזור לשמר מחלוקת בלי להפוך אותה לאיבה אנטי-דמוקרטית."
      ]
    },
    {
      "headingHe": "מה הבעיה באלגוריתמים של רשתות חברתיות?",
      "paragraphsHtml": [
        "הבעיה היא שהם עשויים להפחית חשיפה לתוכן מנוגד לעמדות, גם כאשר המשתמש בחר לעקוב אחר מקור כזה. כך הם משמרים בועות מידע."
      ]
    },
    {
      "headingHe": "האם המאמר מוכיח שפייסבוק מקטבת את כולם?",
      "paragraphsHtml": [
        "לא. הוא מציע ראיות ממוקדות לגבי מנגנוני חשיפה והשפעות מסוימות, ולא טענה גורפת שכל משתמש נעשה מקוטב יותר."
      ]
    },
    {
      "headingHe": "איך זה קשור לדמוקרטיה בישראל?",
      "paragraphsHtml": [
        "גם בישראל, שיח פוליטי נעשה דרך פלטפורמות דיגיטליות. הבנת מנגנוני חשיפה חשובה לכל דיון על קיטוב, אמון ויכולת לנהל מחלוקת."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין להסיק שחשיפה לתוכן מנוגד תפתור לבדה קיטוב. היא יכולה להשפיע על רגשות, אך יש צורך גם במוסדות, נורמות ופלטפורמות אחראיות."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול כיצד עיצוב אלגוריתמים משפיע על יכולתם של אזרחים לפגוש מידע שלא נבחר עבורם מראש לפי המחנה שלהם.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "האם חשיפה לחדשות ברשתות חברתיות משנה קיטוב פוליטי?",
  "permalink": "/democracy_social_media_news_polarization_experiment_summary_he.html",
  "paper_url": "democracy_social_media_news_polarization_experiment_summary_he.html",
  "sortKey": 202605210003
}
---
