---
{
  "version": 1,
  "file": "democracy_affective_polarization_parliamentary_proceedings_summary_he.html",
  "slug": "democracy_affective_polarization_parliamentary_proceedings_summary_he",
  "order": 498,
  "titleHe": "קיטוב רגשי בכנסת נמדד דרך סגנון רגשי ולא רק דרך עמדות אידאולוגיות",
  "subtitleHe": "",
  "descriptionHe": "המאמר מציע מדידה חישובית של קיטוב רגשי באמצעות הטון הרגשי של דיבור פרלמנטרי, ומיישם אותה על קורפוס רחב של דיוני הכנסת בעברית.",
  "summaryHe": "המאמר בוחן קיטוב רגשי בדיבור פרלמנטרי באמצעות מדדי ערכיות, עוררות ודומיננטיות. הוא מפתח משאבי VAD לעברית ומיישם אותם על קורפוס הכנסת, הכולל יותר מ-30 מיליון משפטים מדיוני מליאה וועדות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Gili Goldin, Ella Rabinovich, Shuly Wintner",
  "paperTitle": "Unveiling Affective Polarization Trends in Parliamentary Proceedings",
  "authorsHtml": "Gili Goldin, Ella Rabinovich, <a href=\"https://cris.haifa.ac.il/en/persons/shuly-wintner/\" target=\"_blank\" rel=\"noopener noreferrer\">Shuly Wintner</a>",
  "journal": "Computational Linguistics",
  "dateText": "פורסם: 8 בינואר 2026",
  "sourcePublishedDate": "2026-01-08",
  "doiUrl": "https://doi.org/10.1162/coli.a.600",
  "doiLabel": "https://doi.org/10.1162/coli.a.600",
  "authors": [
    {
      "@type": "Person",
      "name": "Gili Goldin"
    },
    {
      "@type": "Person",
      "name": "Ella Rabinovich"
    },
    {
      "@type": "Person",
      "name": "Shuly Wintner",
      "url": "https://cris.haifa.ac.il/en/persons/shuly-wintner/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Gili Goldin"
    },
    {
      "@type": "Person",
      "name": "Ella Rabinovich"
    },
    {
      "@type": "Person",
      "name": "Shuly Wintner",
      "url": "https://cris.haifa.ac.il/en/persons/shuly-wintner/"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "קיטוב רגשי",
    "כנסת",
    "בלשנות חישובית",
    "דיון פרלמנטרי",
    "ממשלה ואופוזיציה",
    "ערכיות",
    "עוררות",
    "דומיננטיות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_030_civic_democracy_affective_polarization_parliamentary_proceedings_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לקיטוב רגשי בכנסת נמדד דרך סגנון רגשי ולא רק דרך עמדות אידאולוגיות",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Arielinson , uploaded by Chenspec-WMIL for Wikimedia Israel",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:WMIL_Community_meeting_with_Jimmy_Wales_19.5_(123).JPG",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/d/db/WMIL_Community_meeting_with_Jimmy_Wales_19.5_%28123%29.JPG",
    "fileTitle": "WMIL Community meeting with Jimmy Wales 19.5 (123).JPG",
    "sourceDimensions": "6000x4000",
    "localDimensions": "800x600",
    "specificityTier": "literal_or_near_literal_subject_match",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:civic",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-06-08",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר הופך את הסגנון הרגשי של דיבור בכנסת לכלי מדידה של קיטוב, ומחבר בין בלשנות חישובית לבין ניתוח מוסדי של שיח פרלמנטרי.",
  "sections": [
    {
      "headingHe": "מה החידוש המרכזי במאמר?",
      "paragraphsHtml": [
        "המאמר מציע למדוד קיטוב רגשי דרך האופן שבו פוליטיקאים מדברים, ולא רק דרך השאלה אילו עמדות אידאולוגיות הם מחזיקים. בכך הוא מעביר את הדיון מקיטוב תוכני בלבד אל סגנון רגשי של יריבות פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן קיטוב רגשי לא דרך עמדות אידיאולוגיות מוצהרות, אלא דרך הסגנון הרגשי של הדיבור הפרלמנטרי. לשם כך הוא משתמש בשלושה ממדים רגשיים: ערכיות, עוררות ודומיננטיות, ומיישם אותם על עברית כדי לזהות הבדלים רגשיים בשיח פוליטי.",
        "המחקר נשען על קורפוס הכנסת, קורפוס גדול של הליכים פרלמנטריים בעברית הכולל יותר מ-30 מיליון משפטים. הנתונים כוללים דיוני מליאה מן השנים 1992-2024 ודיוני ועדות מן השנים 1998-2024, כך שהניתוח מאפשר לעקוב אחר שינויים בשיח הפרלמנטרי לאורך זמן.",
        "מכיוון שלא היו לקסיקוני VAD ידניים לעברית, המאמר מתאים לעברית לקסיקון אנגלי קיים של 19,972 ערכים, ומוסיף נתוני אימון ממשפטים מתורגמים ומטקסטים קצרים מן הכנסת שסומנו ידנית. בהחלה על הכנסת, הניתוח מזהה הבדלים בין סגנון הדיבור הרגשי של חברי ממשלה וחברי אופוזיציה, ומציג עלייה משמעותית בקיטוב הרגשי לאורך זמן."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "<strong>קיטוב רגשי</strong> הוא מצב שבו קבוצות פוליטיות אינן רק חלוקות בדעותיהן, אלא גם מרגישות עוינות, חוסר אמון או זרות כלפי הצד השני. במאמר, הקיטוב נמדד באמצעות סימנים לשוניים של ערכיות, עוררות ודומיננטיות בדיבור פרלמנטרי."
      ]
    },
    {
      "headingHe": "מה מקור הנתונים?",
      "paragraphsHtml": [
        "המחברים מיישמים את השיטה על קורפוס חדש של דיוני הכנסת, הפרלמנט הישראלי, בעברית. הבחירה בכנסת מאפשרת לבחון קיטוב רגשי בתוך מוסד דמוקרטי מרכזי ולא רק ברשתות חברתיות או בסקרים."
      ]
    },
    {
      "headingHe": "מהם ערכיות, עוררות ודומיננטיות?",
      "paragraphsHtml": [
        "ערכיות מתייחסת למטען חיובי או שלילי של ביטוי, עוררות מתייחסת לעוצמת הרגש, ודומיננטיות מתייחסת לתחושת כוח או שליטה המשתקפת בשפה. שלושת הממדים מאפשרים לתאר סגנון רגשי באופן עשיר יותר מספירת מילים שליליות בלבד."
      ]
    },
    {
      "headingHe": "מה נמצא לגבי ממשלה ואופוזיציה?",
      "paragraphsHtml": [
        "המאמר מציין שהסגנון הרגשי של חברי ממשלה שונה מזה של חברי אופוזיציה. הבדל זה מצביע על קשר בין השתייכות לממשלה או לאופוזיציה לבין הסגנון הרגשי של הדיבור; הניתוח המתואר אינו מוכיח שהמיקום המוסדי הוא שגרם להבדל."
      ]
    },
    {
      "headingHe": "למה קיטוב רגשי בפרלמנט חשוב לדמוקרטיה?",
      "paragraphsHtml": [
        "פרלמנט הוא זירה שבה מחלוקת אמורה להתנהל בגלוי אך גם במסגרת כללים משותפים. כאשר השפה עצמה נעשית טעונה יותר בעוינות או בדומיננטיות, היכולת לשכנע, לפקח ולשתף פעולה עלולה להישחק."
      ]
    },
    {
      "headingHe": "איך מדידה חישובית מסייעת למחקר פוליטי?",
      "paragraphsHtml": [
        "מדידה חישובית מאפשרת לעקוב אחר כמויות גדולות של טקסט לאורך זמן ובאופן שיטתי. היא אינה מחליפה פרשנות פוליטית, אך היא מספקת עדות רחבה לדפוסי שיח שקשה לזהות בקריאה ידנית בלבד."
      ]
    },
    {
      "headingHe": "מה היתרון בבדיקת עברית?",
      "paragraphsHtml": [
        "שיח פוליטי בעברית מציב אתגרי שפה, הקשר ותרבות שאינם נפתרים על ידי כלים שנבנו רק לאנגלית. לכן יישום על קורפוס עברי של הכנסת מוסיף ידע גם לבלשנות חישובית וגם למחקר ישראלי של דמוקרטיה."
      ]
    },
    {
      "headingHe": "האם קיטוב רגשי שווה לקיטוב אידאולוגי?",
      "paragraphsHtml": [
        "לא; אדם יכול לחלוק אידאולוגית על יריב בלי לחוש עוינות עמוקה כלפיו. המאמר חשוב בדיוק משום שהוא מפריד בין עמדות לבין סגנון רגשי, ומאפשר לשאול מתי מחלוקת הופכת ליחס רגשי מאיים יותר."
      ]
    },
    {
      "headingHe": "מהן מגבלות הזהירות של כלי כזה?",
      "paragraphsHtml": [
        "מדידה לשונית תלויה בהקשר, אירוניה, ציטוטים וכללי דיבור פרלמנטריים. לכן תוצאה חישובית צריכה להיקרא לצד ידע פוליטי והיסטורי, ולא כתחליף להבנת האירועים שעליהם דיברו חברי הכנסת."
      ]
    },
    {
      "headingHe": "איך המאמר יכול לשמש ניטור דמוקרטי?",
      "paragraphsHtml": [
        "אם ניתן למדוד מגמות של קיטוב רגשי לאורך זמן, אפשר לזהות תקופות שבהן השיח המוסדי נעשה טעון יותר. ניטור כזה עשוי לסייע לחוקרים, עיתונאים ואזרחים להבין מתי המרחב הפרלמנטרי עובר משיח יריבות לשיח עוינות."
      ]
    },
    {
      "headingHe": "מה לא נכון להסיק מן המאמר?",
      "paragraphsHtml": [
        "לא נכון להסיק שרגש בדיבור פוליטי הוא תמיד בעיה. רגש יכול לבטא מחויבות, כאב או אחריות ציבורית; הבעיה הדמוקרטית מתעוררת כאשר הסגנון הרגשי מעמיק אי־אמון ומקשה על הכרה בלגיטימיות של הצד השני."
      ]
    },
    {
      "headingHe": "איזו שאלה חדשה המאמר פותח?",
      "paragraphsHtml": [
        "המאמר פותח אפשרות לשאול כיצד אירועים פוליטיים, משברים מוסדיים או חילופי ממשלה משפיעים על טון הדיבור בכנסת. זו שאלה חשובה משום שהטון הפרלמנטרי מעצב גם את התרבות הפוליטית שמחוץ למליאה."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר מראה שאיכות הדמוקרטיה ניכרת גם בממד הרגשי של השפה הציבורית. כאשר מודדים את הממד הזה בזהירות, אפשר להבין טוב יותר את התנאים שבהם מחלוקת דמוקרטית נשארת מחלוקת ואינה הופכת לניכור פוליטי עמוק."
      ]
    }
  ],
  "layout": "paper",
  "title": "קיטוב רגשי בכנסת נמדד דרך סגנון רגשי ולא רק דרך עמדות אידאולוגיות",
  "permalink": "/democracy_affective_polarization_parliamentary_proceedings_summary_he.html",
  "paper_url": "democracy_affective_polarization_parliamentary_proceedings_summary_he.html",
  "sortKey": 202606080005
}
---
