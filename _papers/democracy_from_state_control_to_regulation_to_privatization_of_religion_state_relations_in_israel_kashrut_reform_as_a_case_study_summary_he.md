---
{
  "version": 1,
  "layout": "paper",
  "file": "democracy_from_state_control_to_regulation_to_privatization_of_religion_state_relations_in_israel_kashrut_reform_as_a_case_study_summary_he.html",
  "slug": "democracy_from_state_control_to_regulation_to_privatization_of_religion_state_relations_in_israel_kashrut_reform_as_a_case_study_summary_he",
  "order": 488,
  "title": "רפורמת הכשרות ושינוי יחסי דת ומדינה בישראל",
  "titleHe": "רפורמת הכשרות ושינוי יחסי דת ומדינה בישראל",
  "subtitleHe": "",
  "descriptionHe": "תקציר בעברית של מאמר על רפורמת הכשרות בישראל כמקרה מבחן למעבר משליטה מדינתית ישירה להסדרים היברידיים של רגולציה והפרטה ביחסי דת ומדינה.",
  "summaryHe": "המאמר טוען שרפורמת הכשרות של 2021 היא חלק משינוי רחב יותר ביחסי דת ומדינה בישראל. במקום מודל סטטוס־קוו אחד, שבו קבוצות דתיות מוכרות מחזיקות מונופול בתחומים מסוימים והמדינה מספקת טובין דתיים, מתפתח מודל היברידי המשלב שרידי סטטוס־קוו, רגולציה, נסיגה מדינתית והפרטה חלקית.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Nahshon Perez, Elisheva Rosman",
  "paperTitle": "From State Control to Regulation to Privatization of Religion–State Relations in Israel: Kashrut Reform as a Case-Study",
  "authorsHtml": "<a href=\"https://cris.biu.ac.il/en/persons/nahshon-perez\" target=\"_blank\" rel=\"noopener noreferrer\">Nahshon Perez</a>; <a href=\"https://politics.biu.ac.il/en/node/596\" target=\"_blank\" rel=\"noopener noreferrer\">Elisheva Rosman</a>",
  "journal": "Religions",
  "dateText": "פורסם: 2022",
  "sourcePublishedDate": "2022-05-18",
  "doiUrl": "https://doi.org/10.3390/rel13050455",
  "doiLabel": "https://doi.org/10.3390/rel13050455",
  "authors": [
    {
      "@type": "Person",
      "name": "Nahshon Perez",
      "url": "https://cris.biu.ac.il/en/persons/nahshon-perez"
    },
    {
      "@type": "Person",
      "name": "Elisheva Rosman",
      "url": "https://politics.biu.ac.il/en/node/596"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Nahshon Perez",
      "url": "https://cris.biu.ac.il/en/persons/nahshon-perez"
    },
    {
      "@type": "Person",
      "name": "Elisheva Rosman",
      "url": "https://politics.biu.ac.il/en/node/596"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "rights-equality-minorities"
  ],
  "keywords": [
    "מוסדות",
    "מדיניות ציבורית",
    "חברה אזרחית",
    "רגולציה",
    "אחריות ציבורית",
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "חירות",
    "אזרחות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_258_religion_democracy_from_state_control_to_regulation_to_privatization_of_religion_state_relations_in.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לרפורמת הכשרות ושינוי יחסי דת ומדינה בישראל",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Simon Mannweiler",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Prayers_at_the_Western_Wall_(2008-01).jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/a/ad/Prayers_at_the_Western_Wall_%282008-01%29.jpg",
    "fileTitle": "Prayers at the Western Wall (2008-01).jpg",
    "sourceDimensions": "3576x2232",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:religion",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "רפורמת הכשרות מדגימה מעבר מ<strong>שליטה מדינתית בדת</strong> למודל היברידי של סטטוס־קוו, רגולציה והפרטה.",
  "sections": [
    {
      "headingHe": "רפורמת הכשרות ושינוי יחסי דת ומדינה בישראל",
      "paragraphsHtml": [
        "המאמר בוחן כיצד שינוי כלכלי ומוסדי משפיע על הסדרים דתיים שנתפסו במשך שנים כחלק מן הסטטוס־קוו הישראלי.",
        "התרומה הדמוקרטית היא הפניית תשומת הלב לאופן שבו שירות דתי הופך לזירת רגולציה, תחרות ושאלות של אחריות ציבורית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר פותח מן הטענה שיחסי דת ומדינה בישראל הובנו לאורך שנים דרך הסכם הסטטוס־קוו. הסדר זה העניק לקבוצות דתיות מוכרות מונופול בענייני מעמד אישי והבטיח טובין דתיים ופטורים, בעיקר אך לא רק ליהדות האורתודוקסית.",
        "הניתוח קושר את השינוי ביחסי דת ומדינה לשינוי כלכלי רחב שהחל מאמצע שנות השמונים, כאשר ישראל עברה מכלכלה ריכוזית למערכת מופרטת וליברלית יותר. בתוך הקשר זה מופיעות רפורמת הכשרות והתרחבות פעילות מסחרית בשבת כדוגמאות לנסיגה של המדינה מהספקה ישירה של שירותים דתיים או למיקומה מחדש כרגולטור.",
        "רפורמת הכשרות של 2021 משמשת מקרה מבחן מפני שהיא צמצמה מעורבות ממשלתית בתחום מרכזי של מוסדות דת ומדינה, ונוסחה בשפה כלכלית. המאמר מסיק שמודל הסטטוס־קוו כבר אינו מתאר לבדו את המציאות, ושנדרש מודל היברידי שבו חלק מן ההסדרים נותרו סטטוס־קוו, חלקם מבוססי רגולציה וחלקם מופרטים במידה רבה יותר או מופרטים לחלוטין."
      ]
    },
    {
      "headingHe": "הטענה המרכזית והמשמעות הציבורית",
      "paragraphsHtml": [
        "המאמר משתמש ברפורמת הכשרות כמקרה מבחן לשינוי רחב ביחסי דת ומדינה בישראל. הוא מתאר מעבר מהספקה מדינתית ישירה של שירותים דתיים ומשליטה מדינתית ישירה בהם אל מודלים של רגולציה, נסיגה והפרטה חלקית. הטענה המרכזית היא שמודל הסטטוס־קוו כבר אינו מתאר כראוי את המציאות, מפני שיחסי דת ומדינה נעשים היברידיים ומשלבים שוק, פיקוח, מונופולים ישנים ומנגנונים חדשים.",
        "לקורא הישראלי המשמעות היא שרפורמות טכניות לכאורה, כמו כשרות, משנות את חלוקת הכוח בין מדינה, רבנות, עסקים, צרכנים וקבוצות דתיות."
      ]
    },
    {
      "headingHe": "איזה משפט מן המאמר מאיר את הדיון?",
      "paragraphsHtml": [
        "המאמר מסכם את נקודת המפנה כך: <q>מודל הסטטוס־קוו כבר אינו מגדיר כראוי את יחסי הדת והמדינה בישראל.</q>",
        "המשמעות היא שיש לבחון את מערך הכשרות לפי שילוב של סמכות ציבורית, רגולציה ושחקנים פרטיים, ולא רק לפי הנוסחה הישנה של הסדר קבוע."
      ]
    },
    {
      "headingHe": "האם הסטטוס־קוו נעלם?",
      "paragraphsHtml": [
        "לא בהכרח. לפי המאמר, הוא משתלב במודל היברידי לצד הפרטה ורגולציה."
      ]
    }
  ],
  "permalink": "/democracy_from_state_control_to_regulation_to_privatization_of_religion_state_relations_in_israel_kashrut_reform_as_a_case_study_summary_he.html",
  "paper_url": "democracy_from_state_control_to_regulation_to_privatization_of_religion_state_relations_in_israel_kashrut_reform_as_a_case_study_summary_he.html",
  "sortKey": 202605220105
}
---
