---
{
  "version": 1,
  "file": "democracy_populism_and_political_appointments_summary_he.html",
  "slug": "democracy_populism_and_political_appointments_summary_he",
  "order": 400,
  "titleHe": "פופוליזם ומינויים פוליטיים",
  "subtitleHe": "",
  "descriptionHe": "תמצית בעברית של מאמר על פופוליזם ומינויים פוליטיים, עם דגש על המשמעות לדמוקרטיה ליברלית, לשלטון החוק, לזכויות ולאחריות ציבורית.",
  "summaryHe": "המאמר בוחן את סוגיית פופוליזם ומינויים פוליטיים דרך עדשה של שחיקת מוסדות, פופוליזם והגנות מפני ריכוז כוח.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Nissim Cohen, Ron Duhl",
  "paperTitle": "Populism and Political Appointments",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/nissim-cohen/\" target=\"_blank\" rel=\"noopener noreferrer\">Nissim Cohen</a>, Ron Duhl",
  "journal": "Public Administration Review",
  "dateText": "פורסם: 2025",
  "doiUrl": "https://doi.org/10.1111/puar.70023",
  "doiLabel": "https://doi.org/10.1111/puar.70023",
  "authors": [
    {
      "@type": "Person",
      "name": "Nissim Cohen",
      "url": "https://cris.haifa.ac.il/en/persons/nissim-cohen/"
    },
    {
      "@type": "Person",
      "name": "Ron Duhl"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Nissim Cohen",
      "url": "https://cris.haifa.ac.il/en/persons/nissim-cohen/"
    },
    {
      "@type": "Person",
      "name": "Ron Duhl"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "שחיקת מוסדות",
    "רובנות",
    "משבר דמוקרטי",
    "מוסדות",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "מדיניות ציבורית"
  ],
  "image": {
    "src": "html_qa/democracy_populism_and_political_appointments_summary_he.jpg",
    "version": "2026-05-22-nightly-100c",
    "altHe": "איור או צילום עריכתי חם ומפורט על פופוליזם ומינויים פוליטיים",
    "fitness": "standard"
  },
  "datePublished": "2026-05-22",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "פופוליזם ומינויים פוליטיים מדגים כיצד סוגיה אקדמית ממוקדת הופכת לשאלה על <strong>דמוקרטיה ליברלית</strong>, אחריות ציבורית, שוויון והגבלת כוח.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "פופוליזם הוא סגנון או תפיסה פוליטית שמציגה את הפוליטיקה כמאבק בין <strong>העם</strong> לבין <strong>אליטה</strong>, וטוענת שיש רצון עממי אחד שאפשר וצריך לבטא ישירות. במאמר הזה המושג נקשר לשאלה האם פוליטיקאים פופוליסטים מגבירים מינויים פוליטיים כשהם מגיעים לשלטון: המחברים בוחנים פטורים מתחרות או מהליך מינוי מבוסס כישורים כאינדיקציה לפוליטיזציה של השירות הציבורי. לכן ההסבר חשוב כדי להבין כיצד רטוריקה בשם הציבור יכולה להחליש פלורליזם, מקצועיות מינהלית וביקורת מוסדית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "בעוד שהספרות הקיימת מזהה פוליטיזציה ומינויים פוליטיים כאסטרטגיות פופוליסטיות מובילות, העדויות האמפיריות נותרו מוגבלות. בהתחשב באופי החמקמק של מינויים פוליטיים, מאתגר להעריך את היקפם האמיתי בהקשרים שונים. באמצעות ניתוח שיטתי של כל הפטורים מתחרות או תהליך מיון מבוסס-כשרון בשירות המדינה הישראלי מה-1 בינואר 2000 ועד ה-30 באפריל 2024, המחקר מספק ראיות אמפיריות המקשרות בין פופוליזם למינויים פוליטיים, המצביעות על פוליטיזציה עמוקה ונרחבת בתוך שירות המדינה הישראלי."
      ]
    },
    {
      "headingHe": "מה עומד במרכז הדיון המחקרי?",
      "paragraphsHtml": [
        "במרכז הדיון נמצאת השאלה כיצד סוגיית פופוליזם ומינויים פוליטיים משפיעה על היחסים בין אזרחים, מוסדות וקבוצות חברתיות.",
        "המאמר חשוב משום שהוא מזמין קריאה שאינה מסתפקת בתוצאה הפוליטית, אלא בוחנת גם את ההליך, את יחסי הכוח ואת התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית נמצאת בכך שהדיון בפופוליזם ומינויים פוליטיים אינו נושא טכני בלבד; הוא נוגע לשאלה מי יכול להשפיע, מי מוגן מפני שרירות, ואילו מוסדות מסוגלים להציב גבולות לכוח.",
        "מנקודת מבט ליברלית, הערך המרכזי הוא שמירה על כללים שווים גם כאשר הזירה הציבורית מפוצלת או טעונה."
      ]
    },
    {
      "headingHe": "מה חשוב במיוחד לקורא הישראלי?",
      "paragraphsHtml": [
        "לקורא הישראלי חשוב לשים לב לאופן שבו המאמר מחבר בין מציאות פוליטית וחברתית לבין שאלות כלליות יותר על דמוקרטיה ליברלית.",
        "גם כאשר המאמר משווה בין מדינות או עוסק במקרה שאינו רק ישראלי, הדיון בפופוליזם ומינויים פוליטיים מספק זווית לבחינת גבולות הכוח, ההשתתפות והאחריות הציבורית בישראל."
      ]
    },
    {
      "headingHe": "איזו בעיה דמוקרטית עולה מן המאמר?",
      "paragraphsHtml": [
        "הבעיה הדמוקרטית היא כיצד להתמודד עם פופוליזם ומינויים פוליטיים בלי לוותר על <strong>שוויון אזרחי</strong>, ביקורת ציבורית וכללים החלים גם על בעלי כוח.",
        "השאלה אינה רק אם התוצאה רצויה, אלא אם הדרך אליה שומרת על מגבלות כוח, שקיפות ואפשרות לביקורת."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לזכויות ולשוויון?",
      "paragraphsHtml": [
        "הקשר לזכויות ולשוויון נובע מכך שהדיון בפופוליזם ומינויים פוליטיים עשוי להשפיע על גישה למשאבים, הכרה, הגנה משפטית או השתתפות פוליטית. לכן השאלה אינה רק מי צודק בוויכוח, אלא האם כל הקבוצות נהנות ממעמד אזרחי הוגן."
      ]
    },
    {
      "headingHe": "איך הנושא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק נבחן ביכולת של כללים ברורים, ביקורת עצמאית והנמקה ציבורית להגביל החלטות גם במצבי לחץ. כאשר הכללים מוחלים באופן לא שווה או נחלשים מול כוח פוליטי, הדמוקרטיה מאבדת חלק מן ההגנות הליברליות שלה."
      ]
    }
  ],
  "layout": "paper",
  "title": "פופוליזם ומינויים פוליטיים",
  "permalink": "/democracy_populism_and_political_appointments_summary_he.html",
  "paper_url": "democracy_populism_and_political_appointments_summary_he.html",
  "sortKey": 202605220083,
  "sourcePublishedDate": "2025-08-17"
}
---
