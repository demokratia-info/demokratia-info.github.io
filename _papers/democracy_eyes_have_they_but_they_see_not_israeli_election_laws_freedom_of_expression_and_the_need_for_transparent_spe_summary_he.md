---
{
  "version": 1,
  "file": "democracy_eyes_have_they_but_they_see_not_israeli_election_laws_freedom_of_expression_and_the_need_for_transparent_spe_summary_he.html",
  "slug": "democracy_eyes_have_they_but_they_see_not_israeli_election_laws_freedom_of_expression_and_the_need_for_transparent_spe_summary_he",
  "order": 330,
  "titleHe": "חוקי הבחירות בישראל, חופש הביטוי והצורך בשקיפות",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בבחירות, ייצוג פוליטי ואמון הציבור בישראל, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט, שלטון החוק, ביקורת שיפוטית והגבלת כוח ציבורי. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את בחירות, ייצוג פוליטי ואמון הציבור בישראל דרך עדשה של חוקתיות, בתי משפט, שלטון החוק, ביקורת שיפוטית והגבלת כוח ציבורי.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Amit M. Schejter, Moran Yemini",
  "paperTitle": "“Eyes Have They, But They See Not”: Israeli Election Laws, Freedom of Expression, and the Need for Transparent Speech",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0002-3670-2165\" target=\"_blank\" rel=\"noopener noreferrer\">Amit M. Schejter</a>, Moran Yemini",
  "journal": "Communication Law and Policy",
  "dateText": "פורסם: 2009",
  "doiUrl": "https://doi.org/10.1080/10811680903238043",
  "doiLabel": "https://doi.org/10.1080/10811680903238043",
  "authors": [
    {
      "@type": "Person",
      "name": "Amit M. Schejter",
      "url": "https://orcid.org/0000-0002-3670-2165"
    },
    {
      "@type": "Person",
      "name": "Moran Yemini"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Amit M. Schejter",
      "url": "https://orcid.org/0000-0002-3670-2165"
    },
    {
      "@type": "Person",
      "name": "Moran Yemini"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "בחירות",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_eyes_have_they_but_they_see_not_israeli_election_laws_freedom_of_expression_and_the_need_for_transparent_spe_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על בחירות, ייצוג פוליטי ואמון הציבור בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את בחירות, ייצוג פוליטי ואמון הציבור בישראל ומראה מדוע הדיון קשור לחוקתיות, בתי משפט, שלטון החוק, ביקורת שיפוטית והגבלת כוח ציבורי. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את בחירות, ייצוג פוליטי ואמון הציבור בישראל, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט, שלטון החוק, ביקורת שיפוטית והגבלת כוח ציבורי.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "תיאור ביקורתי-היסטורי וניתוח של התפתחות החוקים המסדירים את מערכת הבחירות בשידור ופרשנותם על ידי קובעי מדיניות ובתי משפט לאורך תקופה של קרוב לחמישים שנה בישראל, מלמדים כיצד היסוד הרעיוני לתכנית ההסדרה שהציע החוק במקור כבר ב-1959 התהפך על ראשו. התוצאה היא מערכת כאוטית שבה הכללים של, והגבול בין המותר והאסור אינם ברורים. הפניה לתיקון נוסף ניתן למצוא בכתובת http://www.ynet.co.il/articles/0,7340,L-3593108,00.html."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת בחירות, ייצוג פוליטי ואמון הציבור בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט, שלטון החוק, ביקורת שיפוטית והגבלת כוח ציבורי מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בבחירות, ייצוג פוליטי ואמון הציבור בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את בחירות, ייצוג פוליטי ואמון הציבור בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>בחירות, ייצוג פוליטי ואמון הציבור בישראל</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בבחירות, ייצוג פוליטי ואמון הציבור בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "חוקי הבחירות בישראל, חופש הביטוי והצורך בשקיפות",
  "permalink": "/democracy_eyes_have_they_but_they_see_not_israeli_election_laws_freedom_of_expression_and_the_need_for_transparent_spe_summary_he.html",
  "paper_url": "democracy_eyes_have_they_but_they_see_not_israeli_election_laws_freedom_of_expression_and_the_need_for_transparent_spe_summary_he.html",
  "sortKey": 202605210330
}
---
