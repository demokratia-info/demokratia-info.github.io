---
{
  "version": 1,
  "file": "democracy_the_politics_of_equality_the_limits_of_collective_rights_litigation_and_the_case_of_the_palest_summary_he.html",
  "slug": "democracy_the_politics_of_equality_the_limits_of_collective_rights_litigation_and_the_case_of_the_palest_summary_he",
  "order": 129,
  "titleHe": "פוליטיקת השוויון והתדיינות זכויות קבוצתיות",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Yousef T. Jabareen",
  "paperTitle": "The Politics Of Equality: The Limits Of Collective Rights Litigation And The Case Of The Palestinian-Arab Minority In Israel",
  "authorsHtml": "<a href=\"https://ric.org.il/researcher/yousef-jabareen/\" target=\"_blank\" rel=\"noopener noreferrer\">Yousef T. Jabareen</a>",
  "journal": "Columbia Journal of Race and Law",
  "dateText": "פורסם: 2013",
  "doiUrl": "https://doi.org/10.7916/cjrl.v4i1.2292",
  "doiLabel": "https://doi.org/10.7916/cjrl.v4i1.2292",
  "authors": [
    {
      "@type": "Person",
      "name": "Yousef T. Jabareen",
      "url": "https://ric.org.il/researcher/yousef-jabareen/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yousef T. Jabareen",
      "url": "https://ric.org.il/researcher/yousef-jabareen/"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "מעמד המיעוט הערבי בדמוקרטיה הישראלית",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_the_politics_of_equality_the_limits_of_collective_rights_litigation_and_the_case_of_the_palest_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על מעמד המיעוט הערבי בדמוקרטיה הישראלית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "שוויון הוא העיקרון שלפיו בני אדם או אזרחים זכאים ליחס שווה, אלא אם קיימת הצדקה עניינית להבחנה ביניהם. בהקשר של המאמר, המושג עוזר לבדוק האם כללים ומוסדות מחלקים זכויות, משאבים והכרה ציבורית באופן הוגן."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: בהתמקדות בישראל ובמיעוט הפלסטיני-ערבי, המהווה כמעט חמישית מאוכלוסיית המדינה, מאמר זה בוחן את המגבלות והחסרונות של התדיינות משפטית כאסטרטגיה להשגת זכויות קולקטיביות. סעיף זה מדגים כי בעוד שהתוצאה של כל תיק משפטי הייתה \"הצלחה\" - ברמה הטכנית - פסקי הדין לא הצליחו להשיג את השוויון המהותי והזכויות המבוססות על קבוצות שהעותרים ביקשו. לפיכך, מאמר זה טוען כי ליטיגציה הייתה עד כה כלי לא מספק להגנה על הזכויות הקולקטיביות של המיעוט הפלסטיני-ערבי, שכן בתי המשפט לא הצליחו לנסח מסגרות משמעותיות ובר-קיימא לפעולה ואכיפה לתוך פסקי הדין שלהם.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת מעמד המיעוט הערבי בדמוקרטיה הישראלית לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר מעמד המיעוט הערבי בדמוקרטיה הישראלית משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "פוליטיקת השוויון והתדיינות זכויות קבוצתיות",
  "permalink": "/democracy_the_politics_of_equality_the_limits_of_collective_rights_litigation_and_the_case_of_the_palest_summary_he.html",
  "paper_url": "democracy_the_politics_of_equality_the_limits_of_collective_rights_litigation_and_the_case_of_the_palest_summary_he.html",
  "sortKey": 202605210164
}
---
