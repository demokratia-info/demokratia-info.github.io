---
{
  "version": 1,
  "file": "democracy_neo_zionist_right_wing_populist_discourse_and_activism_in_the_israel_education_system_summary_he.html",
  "slug": "democracy_neo_zionist_right_wing_populist_discourse_and_activism_in_the_israel_education_system_summary_he",
  "order": 316,
  "titleHe": "שיח ופעילות פופוליסטיים ניאו־ציוניים במערכת החינוך",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בפופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני, וממקם את הדיון בתוך שאלות של נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני דרך עדשה של נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי.",
  "authorsCardHe": "Halleli Pinson",
  "paperTitle": "Neo Zionist right-wing populist discourse and activism in the Israel education system",
  "authorsHtml": "<a href=\"https://www.bgu.ac.il/en/people/halleli/\" target=\"_blank\" rel=\"noopener noreferrer\">Halleli Pinson</a>",
  "journal": "Globalisation Societies and Education",
  "dateText": "פורסם: 2021",
  "doiUrl": "https://doi.org/10.1080/14767724.2021.1872372",
  "doiLabel": "https://doi.org/10.1080/14767724.2021.1872372",
  "authors": [
    {
      "@type": "Person",
      "name": "Halleli Pinson",
      "url": "https://www.bgu.ac.il/en/people/halleli/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Halleli Pinson",
      "url": "https://www.bgu.ac.il/en/people/halleli/"
    }
  ],
  "topics": [
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "שחיקת מוסדות",
    "חוסן דמוקרטי",
    "משבר חוקתי",
    "חינוך אזרחי",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_neo_zionist_right_wing_populist_discourse_and_activism_in_the_israel_education_system_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני ומראה מדוע הדיון קשור לנסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "פופוליזם הוא סגנון או תפיסה פוליטית שמציגה את הפוליטיקה כמאבק בין <strong>העם</strong> לבין <strong>אליטה</strong>, וטוענת שיש רצון עממי אחד שאפשר וצריך לבטא ישירות. בהקשר של המאמר, ההסבר חשוב כדי להבין מתי פנייה בשם הציבור מחזקת השתתפות פוליטית ומתי היא מחלישה פלורליזם, ביקורת מוסדית וזכויות של מי שאינם חלק מן הרוב."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מאמר זה בוחן כיצד שיח חינוכי, מדיניות ושינויים בתכנית הלימודים מעוצבים על ידי ארגון פופוליסט ימני ופוליטיקאים כדי לחזק את הציונות החדשה, תוך דה-לגיטימציה לערכים הליברלים-דמוקרטיים. המאמר מסתמך על שלוש דוגמאות. דוגמה שלישית מתמקדת בשינויים בתכנית הלימודים האזרחית החובה - דוגמה לדרכים שבהן נעשה שימוש בחינוך כדי להנציח מושג מסוים של לאומיות יהודית. מקרים אלו מסייעים לחשוף את תגובות המוסדות האקדמיים לפעולות כאלה, ואת ההקבלה החלקית שהם זכו בשנים האחרונות. הם גם מראים כיצד שורטטו מחדש גבולות הפוליטי ואת התפקיד שממלאים מדיניות החינוך והמוסדות בדה-לגיטימציה לביקורת השמאל.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני, וממקם את הסוגיה בתוך הדיון הרחב על נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בפופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בפופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "שיח ופעילות פופוליסטיים ניאו־ציוניים במערכת החינוך",
  "permalink": "/democracy_neo_zionist_right_wing_populist_discourse_and_activism_in_the_israel_education_system_summary_he.html",
  "paper_url": "democracy_neo_zionist_right_wing_populist_discourse_and_activism_in_the_israel_education_system_summary_he.html",
  "sortKey": 202605210316
}
---
