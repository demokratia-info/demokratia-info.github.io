---
{
  "version": 1,
  "file": "democracy_affective_gap_polarization_emotions_summary_he.html",
  "slug": "democracy_affective_gap_polarization_emotions_summary_he",
  "order": 46,
  "titleHe": "אילו רגשות באמת עומדים מאחורי קיטוב פוליטי?",
  "subtitleHe": "המאמר קורא לעבור ממדידת חיבה וסלידה כללית למיפוי רגשות מובחנים כמו פחד, כעס, בוז ואכזבה, כדי להבין טוב יותר קיטוב רגשי.",
  "descriptionHe": "המאמר מסביר שקיטוב רגשי אינו רגש אחד אלא משפחה של רגשות שונים, שכל אחד מהם עשוי להוביל להתנהגות פוליטית אחרת. החשיבות הציבורית של המאמר היא בכך שהוא מראה כיצד עמדות הציבור, רגשות פוליטיים וקיטוב משפיעים על נכונות לקבל כללי משחק משותפים, לכבד יריבים פוליטיים ולהגן על מוסדות גם כאשר הם פועלים נגד העדפה מחנאית רגעית.",
  "summaryHe": "המאמר מסביר שקיטוב רגשי אינו רגש אחד אלא משפחה של רגשות שונים, שכל אחד מהם עשוי להוביל להתנהגות פוליטית אחרת.",
  "authorsCardHe": "ערן הלפרין, מבל קרצ׳נר, סיון הירש-הופלר ויוליה אלעד-שטרנגר",
  "paperTitle": "The affective gap: a call for a comprehensive examination of the discrete emotions underlying affective polarization",
  "authorsHtml": "<a href=\"https://en.psychology.huji.ac.il/people/eran-halperin-0\" target=\"_blank\" rel=\"noopener noreferrer\">Eran Halperin</a>; <a href=\"https://mores-horizon.eu/experts/mabelle-kretchner\" target=\"_blank\" rel=\"noopener noreferrer\">Mabelle Kretchner</a>; <a href=\"https://www.runi.ac.il/en/faculty/hsivan\" target=\"_blank\" rel=\"noopener noreferrer\">Sivan Hirsch-Hoefler</a>; <a href=\"https://cris.biu.ac.il/en/persons/julia-elad-strenger/\" target=\"_blank\" rel=\"noopener noreferrer\">Julia Elad-Strenger</a>",
  "journal": "Cognition and Emotion",
  "dateText": "כרך 38(4), 2024, עמ׳ 442-450; פורסם אונליין ב-18 במאי 2024",
  "doiUrl": "https://doi.org/10.1080/02699931.2024.2348028",
  "doiLabel": "https://doi.org/10.1080/02699931.2024.2348028",
  "authors": [
    {
      "@type": "Person",
      "name": "ערן הלפרין",
      "url": "https://en.psychology.huji.ac.il/people/eran-halperin-0"
    },
    {
      "@type": "Person",
      "name": "מבל קרצ׳נר",
      "url": "https://mores-horizon.eu/experts/mabelle-kretchner"
    },
    {
      "@type": "Person",
      "name": "סיון הירש-הופלר",
      "url": "https://www.runi.ac.il/en/faculty/hsivan"
    },
    {
      "@type": "Person",
      "name": "יוליה אלעד-שטרנגר",
      "url": "https://cris.biu.ac.il/en/persons/julia-elad-strenger/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Eran Halperin",
      "url": "https://en.psychology.huji.ac.il/people/eran-halperin-0"
    },
    {
      "@type": "Person",
      "name": "Mabelle Kretchner",
      "url": "https://mores-horizon.eu/experts/mabelle-kretchner"
    },
    {
      "@type": "Person",
      "name": "Sivan Hirsch-Hoefler",
      "url": "https://www.runi.ac.il/en/faculty/hsivan"
    },
    {
      "@type": "Person",
      "name": "Julia Elad-Strenger",
      "url": "https://cris.biu.ac.il/en/persons/julia-elad-strenger/"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "רגשות פוליטיים",
    "קיטוב רגשי",
    "כעס",
    "פחד",
    "בוז",
    "דמוקרטיה"
  ],
  "image": {
    "src": "html_qa/democracy_gender_justice_backsliding_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על רגשות פוליטיים וקיטוב בין קבוצות",
    "fitness": "standard"
  },
  "datePublished": "2026-05-20",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר מסביר שקיטוב רגשי אינו רגש אחד אלא משפחה של רגשות שונים, שכל אחד מהם עשוי להוביל להתנהגות פוליטית אחרת. מבחינה דמוקרטית, טיפול בקיטוב דורש להבין איזה רגש מפעיל את הציבור: פחד עשוי לדרוש ביטחון, בוז דורש הכרה הדדית, וכעס דורש ערוצי השפעה לגיטימיים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "קיטוב רגשי הוא מצב שבו יריבים פוליטיים אינם רק חולקים בדעותיהם אלא מפתחים חשד, ריחוק או עוינות זה כלפי זה. בהקשר של המאמר, המושג חשוב כדי להבין כיצד רגשות כלפי מחנות פוליטיים משפיעים על אמון, אלימות פוליטית ונכונות לשתף פעולה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: מתוך הכרה במורכבות ההשפעה בפער האידיאולוגי, אנו טוענים כי היעדר בחינה יסודית של תפקידם המובהק של רגשות בדידים מהווה את ה\"פער האפקטיבי\" העיקרי. אנו טוענים שגישה כה ניואנסית, לגבי המרכיב הרגשי של קיטוב רגשי משפרת את הבנתנו את התופעה ושופכת אור על השלכותיה על חברות.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר מסביר שקיטוב רגשי אינו רגש אחד אלא משפחה של רגשות שונים, שכל אחד מהם עשוי להוביל להתנהגות פוליטית אחרת."
      ]
    },
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר טוען שמדידה כללית של חיבה או סלידה מחמיצה את המנגנונים הפסיכולוגיים שמניעים קיטוב פוליטי.",
        "זהו מאמר מושגי-מחקרי ב-Cognition and Emotion הקורא לבחון רגשות מובחנים שעומדים מאחורי קיטוב רגשי.",
        "במקום להתייחס לדמוקרטיה רק כמערכת בחירות, המאמר בוחן את התנאים שמאפשרים לחברה לשמר אמון, זכויות, אחריות ומחלוקת לגיטימית."
      ]
    },
    {
      "headingHe": "מהי נקודת המבט הליברלית-דמוקרטית?",
      "paragraphsHtml": [
        "מבחינה דמוקרטית, טיפול בקיטוב דורש להבין איזה רגש מפעיל את הציבור: פחד עשוי לדרוש ביטחון, בוז דורש הכרה הדדית, וכעס דורש ערוצי השפעה לגיטימיים.",
        "הזווית הליברלית חשובה משום שהיא שואלת לא רק מי שולט, אלא גם מה מגביל את השלטון, איך נשמרת ביקורת, ואיך מוגנות קבוצות או יריבים פוליטיים מפני כוח עודף."
      ]
    },
    {
      "headingHe": "למה המאמר חשוב לקוראי האתר?",
      "paragraphsHtml": [
        "המאמר מתאים לאתר משום שהוא מעמיק את ההבנה של דעת קהל וקיטוב רגשי בזירה דמוקרטית.",
        "הוא מסייע לתרגם מחקר אקדמי לשאלות ציבוריות מעשיות: איך לזהות שחיקה דמוקרטית, איך למדוד קיטוב, ואיך לחשוב על מוסדות וזכויות בלי להסתפק בסיסמאות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל כיצד רגשות מובחנים משפיע על איכות הדמוקרטיה בישראל ועל היכולת לשמור על אמון, זכויות ומוסדות עצמאיים.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "מה החידוש העיקרי?",
      "paragraphsHtml": [
        "תרומתו היא בהבחנה בין סוגי רגשות שונים, משום שכעס, פחד, בוז ואכזבה אינם מובילים בהכרח לאותה תגובה פוליטית.",
        "החידוש צריך להיבחן מול הטענה והראיות במאמר המקורי, ולא רק לפי הדרך שבה התמצית מנגישה אותו."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "מבחינה דמוקרטית, טיפול בקיטוב דורש להבין איזה רגש מפעיל את הציבור: פחד עשוי לדרוש ביטחון, בוז דורש הכרה הדדית, וכעס דורש ערוצי השפעה לגיטימיים.",
        "החשיבות הדמוקרטית נובעת מן הקשר בין מוסדות, זכויות ואחריות ציבורית, אך גבולות הטענה נשארים גבולות המקור."
      ]
    },
    {
      "headingHe": "איך המאמר קשור לישראל?",
      "paragraphsHtml": [
        "הקשר הישראלי נמצא בלב המאמר: זהו מאמר מושגי-מחקרי ב-Cognition and Emotion הקורא לבחון רגשות מובחנים שעומדים מאחורי קיטוב רגשי.",
        "הקשר הישראלי צריך להיקרא דרך המוסדות, הזכויות וההקשרים שהמאמר עצמו מנתח, ולא כהכללה על החברה הישראלית כולה."
      ]
    },
    {
      "headingHe": "איזה מושג כדאי לזכור?",
      "paragraphsHtml": [
        "המושג המרכזי הוא <strong>רגשות מובחנים</strong>. הוא עוזר לראות כיצד תהליך שנראה פוליטי או משפטי צר משפיע על מערכת היחסים בין אזרחים, מוסדות ושלטון."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על מוסדות?",
      "paragraphsHtml": [
        "המוסדות הדמוקרטיים אינם פועלים בחלל ריק. הם תלויים בנורמות, באמון ציבורי, בהתנהגות מפלגות וביכולת להציב גבולות לכוח פוליטי."
      ]
    },
    {
      "headingHe": "האם המאמר מציע פתרון פשוט?",
      "paragraphsHtml": [
        "לא. התרומה שלו היא בעיקר אנליטית: הוא מחדד את הבעיה ומראה אילו תנאים פוליטיים, חברתיים או משפטיים צריכים להשתנות כדי לשפר את החוסן הדמוקרטי."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין לקרוא את המאמר כהוכחה שכל מחלוקת פוליטית היא סכנה לדמוקרטיה. החשיבות היא בזיהוי התנאים שבהם מחלוקת הופכת לפגיעה באמון, בזכויות או באיזונים מוסדיים."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול שאלות מדויקות יותר: אילו מוסדות נפגעים, אילו זכויות עומדות על הפרק, ואיך רגשות מובחנים משנה את התנהגות הציבור או השלטון.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "אילו רגשות באמת עומדים מאחורי קיטוב פוליטי?",
  "permalink": "/democracy_affective_gap_polarization_emotions_summary_he.html",
  "paper_url": "democracy_affective_gap_polarization_emotions_summary_he.html",
  "sortKey": 202605200010
}
---
