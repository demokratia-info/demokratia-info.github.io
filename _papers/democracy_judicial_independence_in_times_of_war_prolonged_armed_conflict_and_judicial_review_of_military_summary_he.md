---
{
  "version": 1,
  "file": "democracy_judicial_independence_in_times_of_war_prolonged_armed_conflict_and_judicial_review_of_military_summary_he.html",
  "slug": "democracy_judicial_independence_in_times_of_war_prolonged_armed_conflict_and_judicial_review_of_military_summary_he",
  "order": 125,
  "titleHe": "מה הקשר בין ביקורת שיפוטית והגבלת כוח שלטוני בישראל לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית ביקורת שיפוטית והגבלת כוח שלטוני בישראל, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית ביקורת שיפוטית והגבלת כוח שלטוני בישראל דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Amnon Reichman",
  "paperTitle": "Judicial Independence in Times of War: Prolonged Armed Conflict and Judicial Review of Military Actions in Israel",
  "authorsHtml": "<a href=\"https://law.haifa.ac.il/en/our-staff/lecturers/amnon-reichman/\" target=\"_blank\" rel=\"noopener noreferrer\">Amnon Reichman</a>",
  "journal": "Utah Law Review",
  "dateText": "פורסם: 2011",
  "doiUrl": "",
  "doiLabel": "",
  "authors": [
    {
      "@type": "Person",
      "name": "אמנון רייכמן",
      "url": "https://law.haifa.ac.il/en/our-staff/lecturers/amnon-reichman/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Amnon Reichman",
      "url": "https://law.haifa.ac.il/en/our-staff/lecturers/amnon-reichman/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "ביקורת שיפוטית והגבלת כוח שלטוני בישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_judicial_independence_in_times_of_war_prolonged_armed_conflict_and_judicial_review_of_military_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על ביקורת שיפוטית והגבלת כוח שלטוני בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית ביקורת שיפוטית והגבלת כוח שלטוני בישראל דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "ביקורת שיפוטית היא סמכות בית המשפט לבחון אם חוק, החלטה מנהלית או פעולה שלטונית עומדים בדין או בעקרונות חוקתיים. בהקשר של המאמר, המושג עוזר להבין את מקומם של בתי המשפט באיזון בין הכרעת רוב, שלטון אפקטיבי והגנה על זכויות."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לאחר מכן, חלק שלישי יבחן את סוגיית העצמאות השיפוטית בעתות מלחמה בישראל, מדינה שנקלעה לסכסוך מזוין מאז הקמתה, ונכון לשנת 1967 עומדת בפני ניהול שטחים המאוכלסים על ידי למעלה ממיליון תושבים המוחזקים תחת משטר של כיבוש לוחמני. הסעיף ידגים את יכולתו של בית המשפט העליון הישראלי (בית המשפט) לשמור על עצמאותו בעת הפעלת ביקורת שיפוטית על מעשיו של המפקד הצבאי בשטחים הכבושים, כפי שמתגלה מנימוקי בית המשפט והחלטותיו. הוא יציע כי נוכחות הקהילה הבינלאומית והטרנס-לאומית מציבה את מערכת המשפט הישראלית במדינה המתוארת בצורה הטובה ביותר כעצמאות \"מוגבלת\", שבה על בית המשפט לנווט בין הקהלים הפנימיים שלו (ואזורי הבחירה) לבין הקהילות המשפטיות הבינלאומיות והטרנס-לאומיות."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית ביקורת שיפוטית והגבלת כוח שלטוני בישראל, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת ביקורת שיפוטית והגבלת כוח שלטוני בישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר ביקורת שיפוטית והגבלת כוח שלטוני בישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין ביקורת שיפוטית והגבלת כוח שלטוני בישראל לבין שלטון החוק בישראל?",
  "permalink": "/democracy_judicial_independence_in_times_of_war_prolonged_armed_conflict_and_judicial_review_of_military_summary_he.html",
  "paper_url": "democracy_judicial_independence_in_times_of_war_prolonged_armed_conflict_and_judicial_review_of_military_summary_he.html",
  "sortKey": 202605210168
}
---
