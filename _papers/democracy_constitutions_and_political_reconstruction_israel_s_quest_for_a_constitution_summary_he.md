---
{
  "version": 1,
  "file": "democracy_constitutions_and_political_reconstruction_israel_s_quest_for_a_constitution_summary_he.html",
  "slug": "democracy_constitutions_and_political_reconstruction_israel_s_quest_for_a_constitution_summary_he",
  "order": 25,
  "titleHe": "החיפוש הישראלי אחר חוקה כשיקום פוליטי",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן מדוע ישראל לא השלימה חוקה פורמלית וכיצד הוויכוח החוקתי משקף מאבק עמוק יותר על בניית המדינה, זהות יהודית ודמוקרטית, גבולות אזרחות וסמכות מוסדית. החוקה מוצגת לא רק כמסמך משפטי חסר, אלא כזירה שבה החברה הישראלית מתקשה להסכים על כללי יסוד משותפים.",
  "summaryHe": "המאמר קורא את החיפוש הישראלי אחר חוקה כפרויקט של שיקום ובנייה פוליטית. הוא חוזר להחלטה שלא לכונן חוקה מלאה בראשית המדינה, ולדרך שבה חוקי היסוד, פסיקת בית המשפט והמאבקים הפוליטיים סביב דת, לאום וביטחון מילאו חלק מן החלל. הטענה המרכזית היא שהבעיה אינה טכנית בלבד: חוקה בישראל הייתה אמורה להכריע או לפחות למסגר מחלוקות עמוקות על זהות המדינה, זכויות מיעוטים, יחסי דת ומדינה וסמכות הרשויות. לכן הכישלון להשלים חוקה מלמד על קושי מתמשך לבנות הסכמה אזרחית רחבה, אך גם על ניסיון ישראלי הדרגתי ליצור חוקתיות חלקית מתוך פשרות, דחיות ומאבקים. המאמר בוחן מדוע ישראל לא השלימה חוקה פורמלית וכיצד הוויכוח החוקתי משקף מאבק עמוק יותר על בניית המדינה, זהות יהודית ודמוקרטית, גבולות אזרחות וסמכות מוסדית. החוקה מוצגת לא רק כמסמך משפטי חסר, אלא כזירה שבה החברה הישראלית מתקשה להסכים על כללי יסוד משותפים. החשיבות הפרשנית של המקור היא בכך שהוא מעביר את הדיון מן הסיסמה הכללית אל המנגנונים המוסדיים, המשפטיים או החברתיים שבהם הדמוקרטיה נבחנת בפועל.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Ruth Gavison",
  "paperTitle": "Constitutions and Political Reconstruction? Israel's Quest for a Constitution",
  "authorsHtml": "<a href=\"https://en.law.huji.ac.il/people/ruth-gavison\" target=\"_blank\" rel=\"noopener noreferrer\">Ruth Gavison</a>",
  "journal": "International Sociology",
  "dateText": "פורסם: 2003",
  "doiUrl": "https://doi.org/10.1177/0268580903018001004",
  "doiLabel": "https://doi.org/10.1177/0268580903018001004",
  "authors": [
    {
      "@type": "Person",
      "name": "Ruth Gavison",
      "url": "https://en.law.huji.ac.il/people/ruth-gavison"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ruth Gavison",
      "url": "https://en.law.huji.ac.il/people/ruth-gavison"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "איזונים ובלמים"
  ],
  "image": {
    "src": "assets/article-images/133_court_democracy_constitutions_and_political_reconstruction_israel_s_quest_for_a_constitution_sum.png",
    "version": "2026-07-05-final-generated-image-set",
    "altHe": "איור עריכתי: החיפוש הישראלי אחר חוקה ושיקום פוליטי",
    "fitness": "high",
    "license": "project-generated; permitted for Demokratia website handoff",
    "creator": "Codex-generated editorial asset",
    "provider": "Demokratia generated asset",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "generated_article_specific_unique",
    "sourceMethod": "local_generated_asset_full_set",
    "visualAnchor": "generated:court"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "החוקה הישראלית החסרה מוצגת כחלון למחלוקות היסוד על זהות, זכויות וסמכות פוליטית בישראל.",
  "sections": [
    {
      "headingHe": "איך החוקה החסרה נעשית בעיית שיקום פוליטי?",
      "paragraphsHtml": [
        "המאמר בוחן את מסלול החוקה בישראל מאז הקמת המדינה: ההבטחה לכונן חוקה, ההחלטה להתקדם באמצעות חוקי יסוד, והמשמעות הפוליטית של חוקתיות שאינה מתכנסת למסמך אחד מוסכם.",
        "הדיון אינו עוסק רק במבנה משפטי. הוא שואל מדוע חוקה נעשתה בישראל לסמל של מחלוקות סביב יהדות ודמוקרטיה, דת ומדינה, גבולות הקולקטיב וזכויות אזרחיות."
      ]
    },
    {
      "headingHe": "מדוע חוקי היסוד אינם סוגרים את המחלוקת החוקתית?",
      "paragraphsHtml": [
        "הטענה היא שהיעדר חוקה משקף קושי לבנות הסכמה פוליטית עמוקה לאחר הקמת המדינה. חוקה אמורה לקבוע כללי יסוד, אך בישראל עצם הכללים הללו נוגעים לשאלות שלא הוכרעו: מי העם החוקתי, מה מעמד הדת, וכיצד מאזנים בין רוב יהודי לזכויות שוות.",
        "לצד זאת, המאמר אינו מתאר ריק מוחלט. הוא מראה כיצד חוקי יסוד, מוסדות פוליטיים וביקורת שיפוטית יצרו חוקתיות הדרגתית וחלקית, גם אם היא נשארה חשופה למחלוקת ציבורית חריפה."
      ]
    },
    {
      "headingHe": "איפה זהות יהודית ודמוקרטית מכבידה על רגע מכונן?",
      "paragraphsHtml": [
        "המאמר חשוב משום שהוא מציב את הדיון בחוקה בתוך ההיסטוריה הפוליטית של ישראל, ולא רק בתוך משפט חוקתי פורמלי. הוא מסביר מדוע כל ויכוח על חוקה בישראל נוגע גם לשאלת הלגיטימיות של מוסדות, זכויות ומגבלות על רוב פוליטי.",
        "לסיכום באתר כדאי להדגיש שהמאמר עוסק בחוקה כזירה של בניית אמון וכללים משותפים, ולא בסיסמה כללית על דמוקרטיה. הנקודה החשובה היא שהדיון נשאר מחובר למקרה ולמנגנון שהמאמר מנתח, ולא לטענה כללית בלבד על דמוקרטיה."
      ]
    },
    {
      "headingHe": "כיצד בית המשפט והכנסת ממלאים חלל חוקתי?",
      "paragraphsHtml": [
        "לכן הכישלון להשלים חוקה מלמד על קושי מתמשך לבנות הסכמה אזרחית רחבה, אך גם על ניסיון ישראלי הדרגתי ליצור חוקתיות חלקית מתוך פשרות, דחיות ומאבקים. המאמר בוחן מדוע ישראל לא השלימה חוקה פורמלית וכיצד הוויכוח החוקתי משקף מאבק עמוק יותר על בניית המדינה, זהות יהודית ודמוקרטית, גבולות אזרחות וסמכות מוסדית.",
        "The article frames Israel's constitution-making as an unfinished project tied to state-building, identity conflict, and political reconstruction rather than a simple absence of legal drafting."
      ]
    },
    {
      "headingHe": "מה החיפוש אחר חוקה מלמד על הסכמה אזרחית בישראל?",
      "paragraphsHtml": [
        "החוקה מוצגת לא רק כמסמך משפטי חסר, אלא כזירה שבה החברה הישראלית מתקשה להסכים על כללי יסוד משותפים. החשיבות הפרשנית של המקור היא בכך שהוא מעביר את הדיון מן הסיסמה הכללית אל המנגנונים המוסדיים, המשפטיים או החברתיים שבהם הדמוקרטיה נבחנת בפועל.",
        "החוקה הישראלית החסרה מוצגת כחלון למחלוקות היסוד על זהות, זכויות וסמכות פוליטית בישראל. הנקודה החשובה היא שהדיון נשאר מחובר למקרה ולמנגנון שהמאמר מנתח, ולא לטענה כללית בלבד על דמוקרטיה."
      ]
    },
    {
      "headingHe": "איזו קריאה זהירה מציע המאמר לוויכוח החוקתי?",
      "paragraphsHtml": [
        "החוקה מוצגת לא רק כמסמך משפטי חסר, אלא כזירה שבה החברה הישראלית מתקשה להסכים על כללי יסוד משותפים. החשיבות הפרשנית של המקור היא בכך שהוא מעביר את הדיון מן הסיסמה הכללית אל המנגנונים המוסדיים, המשפטיים או החברתיים שבהם הדמוקרטיה נבחנת בפועל."
      ]
    }
  ],
  "layout": "paper",
  "title": "החיפוש הישראלי אחר חוקה כשיקום פוליטי",
  "permalink": "/democracy_constitutions_and_political_reconstruction_israel_s_quest_for_a_constitution_summary_he.html",
  "paper_url": "democracy_constitutions_and_political_reconstruction_israel_s_quest_for_a_constitution_summary_he.html",
  "sortKey": 202605210095
}
---
