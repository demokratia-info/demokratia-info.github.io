---
{
  "version": 1,
  "file": "democracy_ultrarich_media_ownership_political_investment_summary_he.html",
  "slug": "democracy_ultrarich_media_ownership_political_investment_summary_he",
  "order": 59,
  "titleHe": "איך בעלות של מיליארדרים על עיתונים יכולה להפוך להשקעה פוליטית?",
  "subtitleHe": "גיא גרוסמן, יותם מרגלית ותמר מיטס בוחנים את ישראל היום כמקרה שבו בעלות תקשורתית של עשירי-על השפיעה על התחרות הפוליטית בישראל.",
  "descriptionHe": "המאמר ב-The Journal of Politics מנתח כיצד בעלות תקשורתית של עשירי-על יכולה להשפיע על תוצאות בחירות, דרך המקרה של ישראל היום ושלדון אדלסון בישראל.",
  "summaryHe": "המאמר מראה שבעלות תקשורתית אינה רק שאלה עסקית, אלא יכולה לשמש כלי בעל השפעה אלקטורלית ופוליטית עמוקה.",
  "authorsCardHe": "גיא גרוסמן, יותם מרגלית ותמר מיטס",
  "paperTitle": "How the Ultrarich Use Media Ownership as a Political Investment",
  "authorsHtml": "<a href=\"https://live-sas-www-polisci.pantheon.sas.upenn.edu/people/standing-faculty/guy-grossman\" target=\"_blank\" rel=\"noopener noreferrer\">Guy Grossman</a>; <a href=\"https://www.ymargalit.net/\" target=\"_blank\" rel=\"noopener noreferrer\">Yotam Margalit</a>; <a href=\"https://polisci.columbia.edu/content/tamar-mitts\" target=\"_blank\" rel=\"noopener noreferrer\">Tamar Mitts</a>",
  "journal": "The Journal of Politics",
  "dateText": "The Journal of Politics, כרך 84, גיליון 4, עמ׳ 1913-1931; פורסם ב-1 באוקטובר 2022",
  "doiUrl": "https://doi.org/10.1086/719415",
  "doiLabel": "https://doi.org/10.1086/719415",
  "authors": [
    {
      "@type": "Person",
      "name": "גיא גרוסמן",
      "url": "https://live-sas-www-polisci.pantheon.sas.upenn.edu/people/standing-faculty/guy-grossman"
    },
    {
      "@type": "Person",
      "name": "יותם מרגלית",
      "url": "https://www.ymargalit.net/"
    },
    {
      "@type": "Person",
      "name": "תמר מיטס",
      "url": "https://polisci.columbia.edu/content/tamar-mitts"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Guy Grossman",
      "url": "https://live-sas-www-polisci.pantheon.sas.upenn.edu/people/standing-faculty/guy-grossman"
    },
    {
      "@type": "Person",
      "name": "Yotam Margalit",
      "url": "https://www.ymargalit.net/"
    },
    {
      "@type": "Person",
      "name": "Tamar Mitts",
      "url": "https://polisci.columbia.edu/content/tamar-mitts"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "בעלות תקשורתית",
    "עשירי-על",
    "ישראל היום",
    "השפעה פוליטית",
    "בחירות",
    "נתניהו",
    "ליכוד"
  ],
  "image": {
    "src": "html_qa/democracy_constitutional_reform_economy_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור על כוח כלכלי, בעלות תקשורתית והשפעה פוליטית בדמוקרטיה",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המחקר מציג את בעלות התקשורת של עשירי-על כהשקעה פוליטית שעשויה לעצב התנהגות בוחרים ותחרות מפלגתית. במקרה הישראלי, ישראל היום שהושק בשנת 2007 ומחולק בחינם הפך למקרה מבחן מרכזי להשפעת בעלות תקשורתית על כוח פוליטי.",
  "sections": [
    {
      "headingHe": "מה המאמר מוסיף לשיחה הדמוקרטית?",
      "paragraphsHtml": [
        "המאמר שואל האם עשירי-על יכולים להשפיע על תוצאות בחירות באמצעות שליטה בכלי תקשורת שמקדמים בגלוי אינטרסים פוליטיים. זו שאלה דמוקרטית יסודית, משום שתקשורת חופשית אמורה לשרת ציבור, לא רק בעלים חזקים.",
        "החוקרים בוחנים את ישראל היום, עיתון חינמי שהקים שלדון אדלסון בשנת 2007 והפך במהירות לעיתון הנקרא ביותר בישראל. הם משתמשים בנתוני חשיפה מקומיים כדי לבדוק האם חדירת העיתון שינתה דפוסי הצבעה.",
        "התרומה המרכזית היא בהצגת בעלות תקשורתית לא רק כריכוזיות כלכלית, אלא כערוץ שבו כסף פרטי יכול להתגלגל להשפעה פוליטית רחבה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: האם העשירים האולטרה יכולים לעצב את תוצאות הבחירות על ידי שליטה בכלי תקשורת שמפיצים בגלוי את האינטרסים הפוליטיים שלהם? באמצעות נתוני החשיפה של התקשורת המקומית מאז ההשקה, הניתוח שלנו מצביע על כך שהעיתון הפעיל השפעה אלקטורלית משמעותית, בעיקר לטובת נתניהו ומפלגתו הליכוד. הממצאים מדגישים את ההשפעה העצומה שעשירים האולטרה יכולים להפעיל בעיצוב פוליטיקה באמצעות בעלות על התקשורת.",
        "בהקשר הדמוקרטי של האתר, המשמעות היא: המאמר מראה שבעלות תקשורתית אינה רק שאלה עסקית, אלא יכולה לשמש כלי בעל השפעה אלקטורלית ופוליטית עמוקה."
      ]
    },
    {
      "headingHe": "מה הממצא הפוליטי המרכזי?",
      "paragraphsHtml": [
        "התקציר מציין שהניתוח מצביע על השפעה אלקטורלית משמעותית של העיתון, בעיקר לטובת בנימין נתניהו ומפלגת הליכוד.",
        "המשמעות הדמוקרטית אינה רק שמצביעים נחשפו לעיתון מסוים. היא שהמבנה הכלכלי של שוק התקשורת יכול לשנות את מאזן הכוח בין מפלגות, במיוחד כאשר כלי התקשורת מחולק בחינם ומגיע לקהלים רחבים."
      ]
    },
    {
      "headingHe": "איזו ראיה מרכזית מופיעה במקור?",
      "paragraphsHtml": [
        "תקציר המחקר מתאר את ישראל היום כעיתון ימני שחולק בחינם והפך ל״הנקרא ביותר במדינה״.",
        "המקור גם קובע שההשפעה שנמצאה סייעה ליצירת ״שינוי ים״ בדומיננטיות של הימין בפוליטיקה הלאומית. שני הניסוחים ממקמים את התקשורת כחלק ממבנה הכוח הפוליטי."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית זקוקה לתקשורת מגוונת, ביקורתית ועצמאית. כאשר בעל הון יכול לסבסד כלי תקשורת כדי לקדם יעד פוליטי, השוק החופשי לבדו אינו מבטיח פלורליזם ציבורי.",
        "המאמר אינו טוען שכל עיתון בעל קו אידיאולוגי הוא בעיה. הבעיה מתחילה כאשר כוח כלכלי קיצוני מאפשר לעקוף מגבלות שוק, להציף את המרחב הציבורי במסר פוליטי, ולהשפיע על תחרות הבחירות."
      ]
    },
    {
      "headingHe": "מה השאלה המרכזית של המאמר?",
      "paragraphsHtml": [
        "המאמר שואל האם בעלות תקשורתית של עשירי-על יכולה לשמש <strong>השקעה פוליטית</strong> ולהשפיע על תוצאות בחירות.",
        "השאלה הזאת צריכה להיקרא בתוך ההקשר המחקרי של המאמר, ולא כהצהרה כללית שאינה תלויה במקור."
      ]
    },
    {
      "headingHe": "למה ישראל היום הוא מקרה מבחן חשוב?",
      "paragraphsHtml": [
        "ישראל היום הושק בשנת 2007, חולק בחינם והפך במהירות לשחקן מרכזי. לכן הוא מאפשר לבחון כיצד כניסת כלי תקשורת חדש משנה חשיפה ומאזן פוליטי."
      ]
    },
    {
      "headingHe": "מה הקשר בין בעלות לתוכן?",
      "paragraphsHtml": [
        "בעלות יכולה להשפיע על קו מערכתי, סדר יום, בחירת נושאים ומסגור פוליטי. כאשר הבעלות פוליטית במוצהר, ההשפעה יכולה להיות מכוונת במיוחד."
      ]
    },
    {
      "headingHe": "האם הציבור פשוט מתעלם מתקשורת מוטה?",
      "paragraphsHtml": [
        "המאמר בוחן בדיוק את ההנחה הזו. הממצאים מציעים שגם כאשר כלי תקשורת נתפס כמוטה, הוא עדיין יכול להשפיע על התנהגות פוליטית."
      ]
    },
    {
      "headingHe": "איך זה קשור לפלורליזם תקשורתי?",
      "paragraphsHtml": [
        "פלורליזם אינו רק מספר כלי התקשורת. הוא תלוי גם בשאלה מי מממן אותם, מי שולט בגישה לקהלים, והאם קולות שונים יכולים להתחרות בתנאים הוגנים."
      ]
    },
    {
      "headingHe": "האם המאמר מציע רגולציה מסוימת?",
      "paragraphsHtml": [
        "המאמר בעיקר אמפירי ואנליטי. הוא מספק בסיס לדיון על שקיפות בעלות, ריכוזיות, סבסוד פוליטי וכללי תחרות במרחב התקשורתי."
      ]
    },
    {
      "headingHe": "מה חשוב לא להסיק ממנו?",
      "paragraphsHtml": [
        "אין להסיק שכל עיתון חינמי או ימני הוא פסול. השאלה היא האם בעלות וסבסוד משמשים כלי להשפעה פוליטית לא סימטרית."
      ]
    },
    {
      "headingHe": "איך אפשר להשתמש בו בדיון ציבורי?",
      "paragraphsHtml": [
        "אפשר להשתמש בו כדי לשאול מי מממן את המידע שאזרחים מקבלים, ומה קורה כאשר בעלות תקשורתית הופכת לכלי אסטרטגי במאבק על שלטון.",
        "שימוש ציבורי אחראי צריך להבחין בין מה שהמאמר מראה לבין מסקנות מדיניות רחבות יותר שדורשות נימוק נוסף."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך בעלות של מיליארדרים על עיתונים יכולה להפוך להשקעה פוליטית?",
  "permalink": "/democracy_ultrarich_media_ownership_political_investment_summary_he.html",
  "paper_url": "democracy_ultrarich_media_ownership_political_investment_summary_he.html",
  "sortKey": 202605210004
}
---
