---
{
  "version": 1,
  "file": "democracy_dynamics_of_dis_trust_between_the_news_media_and_their_audience_the_case_of_the_april_2019_israeli_exit_poll_summary_he.html",
  "slug": "democracy_dynamics_of_dis_trust_between_the_news_media_and_their_audience_the_case_of_the_april_2019_israeli_exit_poll_summary_he",
  "order": 298,
  "titleHe": "אמון ואי־אמון בין תקשורת החדשות לקהל",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את משבר האמון סביב מדגם היציאה השגוי של ערוץ 12 בבחירות אפריל 2019 בישראל, ומראה כיצד סקרים ליל הבחירות מתפקדים גם ככלי מידע וגם כטקס אזרחי טעון.",
  "summaryHe": "המאמר מנתח את הדינמיקה של אמון ואי־אמון בין מומחים, עיתונאים וקהל סביב מדגמי הבחירות בישראל באפריל 2019. הוא משלב קבוצות מיקוד עם אזרחים, ראיונות עומק עם עיתונאים, סוקרים ומומחים, וניתוח איכותני של דיווחי חדשות. הממצאים מזהים שלושה נרטיבי אשמה סביב הפער בין מדגם ערוץ 12 לתוצאות הרשמיות: כשל סטטיסטי של הסוקרים, הטיה של התקשורת, והונאה מצד חלק מן המצביעים. המאמר טוען שאמון בתקשורת נוצר בתוך יחסי גומלין בין מי שמייצרים, מקבלים ומפרשים חדשות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Tali Aharoni, Keren Tenenboim‐Weinblatt, Christian Baden, Maximilian Overbeck",
  "paperTitle": "Dynamics of (dis)trust between the news media and their audience: The case of the April 2019 Israeli exit polls",
  "authorsHtml": "<a href=\"https://en.communication.huji.ac.il/people/tali-aharoni\" target=\"_blank\" rel=\"noopener noreferrer\">Tali Aharoni</a>, <a href=\"https://en.communication.huji.ac.il/people/keren-tenenboim-weinblatt\" target=\"_blank\" rel=\"noopener noreferrer\">Keren Tenenboim‐Weinblatt</a>, <a href=\"https://en.communication.huji.ac.il/people/christian-baden\" target=\"_blank\" rel=\"noopener noreferrer\">Christian Baden</a>, <a href=\"https://orcid.org/0000-0003-3658-5584\" target=\"_blank\" rel=\"noopener noreferrer\">Maximilian Overbeck</a>",
  "journal": "Journalism",
  "dateText": "פורסם: 2020",
  "doiUrl": "https://doi.org/10.1177/1464884920978105",
  "doiLabel": "https://doi.org/10.1177/1464884920978105",
  "authors": [
    {
      "@type": "Person",
      "name": "Tali Aharoni",
      "url": "https://en.communication.huji.ac.il/people/tali-aharoni"
    },
    {
      "@type": "Person",
      "name": "Keren Tenenboim‐Weinblatt",
      "url": "https://en.communication.huji.ac.il/people/keren-tenenboim-weinblatt"
    },
    {
      "@type": "Person",
      "name": "Christian Baden",
      "url": "https://en.communication.huji.ac.il/people/christian-baden"
    },
    {
      "@type": "Person",
      "name": "Maximilian Overbeck",
      "url": "https://orcid.org/0000-0003-3658-5584"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Tali Aharoni",
      "url": "https://en.communication.huji.ac.il/people/tali-aharoni"
    },
    {
      "@type": "Person",
      "name": "Keren Tenenboim‐Weinblatt",
      "url": "https://en.communication.huji.ac.il/people/keren-tenenboim-weinblatt"
    },
    {
      "@type": "Person",
      "name": "Christian Baden",
      "url": "https://en.communication.huji.ac.il/people/christian-baden"
    },
    {
      "@type": "Person",
      "name": "Maximilian Overbeck",
      "url": "https://orcid.org/0000-0003-3658-5584"
    }
  ],
  "topics": [
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "דעת קהל",
    "קיטוב",
    "מחאה",
    "בחירות",
    "אלימות פוליטית",
    "תקשורת",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_200_media_democracy_dynamics_of_dis_trust_between_the_news_media_and_their_audience_the_case_of_the.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על תקשורת, דעת קהל ואחריות דמוקרטית בישראל",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Danny-w",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Israeli_Educational_Television_-a_%22flash_news%22_sign.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/a/a4/Israeli_Educational_Television_-a_%22flash_news%22_sign.jpg",
    "fileTitle": "Israeli Educational Television -a \"flash news\" sign.jpg",
    "sourceDimensions": "4160x3120",
    "localDimensions": "800x600",
    "specificityTier": "literal_or_near_literal_subject_match",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:media",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה שמדגמי יציאה הם גם חיזוי פוליטי וגם טקס אזרחי, ולכן כשל במדגם עלול לערער אמון בעיתונות, בסוקרים ובקהל עצמו.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את תקשורת, דעת קהל ואחריות דמוקרטית בישראל, וממקם את הסוגיה בתוך הדיון הרחב על דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית.",
        "נקודת המוצא של התקציר היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר אומר בפועל על מדגמי אפריל 2019?",
      "paragraphsHtml": [
        "המאמר מנתח את משבר האמון שנוצר סביב מדגמי הבחירות של אפריל 2019 בישראל, ומראה כיצד יחסי אמון ואי־אמון בין תקשורת החדשות לבין הקהל אינם נוצרים רק מאי־דיוק מספרי. המסר המרכזי הוא שמדגמי יציאה הם רגע מבחן ציבורי: כאשר התחזית, ההסבר העיתונאי והחוויה הפוליטית של הצופים אינם מתלכדים, האמון בתקשורת נפגע גם אם העבודה המקצועית מורכבת יותר מן הכותרת. לכן המאמר מציע לראות אמון תקשורתי כתהליך דינמי של ציפיות, פרשנות ואחריות ציבורית, ולא כעמדה קבועה בעד או נגד כלי תקשורת."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתמקד בליל הבחירות של 9 באפריל 2019, כאשר שלושת ערוצי הטלוויזיה המרכזיים בישראל פרסמו מדגמי יציאה. ערוצים 11 ו-13 הציגו יתרון ברור לגוש הימין בראשות בנימין נתניהו והליכוד, ואילו ערוץ 12 חזה איזון בין הגושים ואף יתרון קל לכחול לבן. בבוקר שלאחר מכן התוצאות הרשמיות הפריכו את מדגם ערוץ 12 והציתו ויכוח ציבורי חריף.",
        "המחקר נשען על שילוב של קבוצות מיקוד עם אזרחים, ראיונות עומק חצי-מובנים עם עיתונאים, סוקרים ומומחים, וניתוח איכותני של דיווחי חדשות. מתוך החומרים האלה המאמר מזהה שלושה הסברים ציבוריים לכשל: נרטיב סטטיסטי שהאשים את שיטת הסקר והסוקרים, נרטיב של תקשורת מוטה שהרחיב את האשמה לגוף החדשות, ונרטיב הונאה שטען כי מצביעי ימין שיקרו לסוקרים במכוון.",
        "המאמר טוען שאי־אמון תקשורתי אינו נובע רק מטעות מספרית. מדגמי יציאה בליל בחירות פועלים גם כמקור מידע וגם כטקס לאומי שבו אזרחים ועיתונאים מפרשים יחד את משמעות התוצאה. כאשר החשד מופנה לסוקרים, לתקשורת או לציבור הבוחרים, הוא פוגע לא רק בדיוק הנתפס של המדגם אלא גם במשמעות האזרחית של אירוע הבחירות המשודר."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת תקשורת, דעת קהל ואחריות דמוקרטית בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד דעת קהל, קיטוב, מחאה, בחירות ואלימות פוליטית מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בתקשורת, דעת קהל ואחריות דמוקרטית בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את תקשורת, דעת קהל ואחריות דמוקרטית בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>תקשורת, דעת קהל ואחריות דמוקרטית בישראל</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בתקשורת, דעת קהל ואחריות דמוקרטית בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "אמון ואי־אמון בין תקשורת החדשות לקהל",
  "permalink": "/democracy_dynamics_of_dis_trust_between_the_news_media_and_their_audience_the_case_of_the_april_2019_israeli_exit_poll_summary_he.html",
  "paper_url": "democracy_dynamics_of_dis_trust_between_the_news_media_and_their_audience_the_case_of_the_april_2019_israeli_exit_poll_summary_he.html",
  "sortKey": 202605210298
}
---
