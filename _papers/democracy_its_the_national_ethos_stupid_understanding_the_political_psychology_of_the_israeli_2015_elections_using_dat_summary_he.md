---
{
  "version": 1,
  "file": "democracy_its_the_national_ethos_stupid_understanding_the_political_psychology_of_the_israeli_2015_elections_using_dat_summary_he.html",
  "slug": "democracy_its_the_national_ethos_stupid_understanding_the_political_psychology_of_the_israeli_2015_elections_using_dat_summary_he",
  "order": 235,
  "titleHe": "האתוס הלאומי והפסיכולוגיה הפוליטית של בחירות 2015",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את בחירות 2015 בישראל דרך עמדות הבוחרים כלפי אתוס לאומי, ומראה כיצד מפלגות שונות ממוקמות על רצף שבין דגשים רפובליקניים לליברליים.",
  "summaryHe": "ניתוח של דפוסי הצבעה בישראל ב־2015 דרך אתוס לאומי, אמון במוסדות ועמדות כלפי ביטחון, פטריוטיות ובית המשפט העליון.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Eyal Lewin",
  "paperTitle": "“It’s the National Ethos, Stupid”! – Understanding the Political Psychology of the Israeli 2015 Elections Using Data from the National Resilience Survey",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-5461-6634\" target=\"_blank\" rel=\"noopener noreferrer\">Eyal Lewin</a>",
  "journal": "International Journal of Social Science Studies",
  "dateText": "פורסם: 2016",
  "doiUrl": "https://doi.org/10.11114/ijsss.v4i7.1651",
  "doiLabel": "https://doi.org/10.11114/ijsss.v4i7.1651",
  "authors": [
    {
      "@type": "Person",
      "name": "Eyal Lewin",
      "url": "https://orcid.org/0000-0001-5461-6634"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Eyal Lewin",
      "url": "https://orcid.org/0000-0001-5461-6634"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service",
    "public-opinion-polarization-violence"
  ],
  "keywords": [
    "מוסדות ציבור",
    "חברה אזרחית",
    "שירות ציבורי",
    "אחריותיות",
    "ממשל",
    "בחירות",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_408_elections_democracy_its_the_national_ethos_stupid_understanding_the_political_psychology_of_the_isra.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על בחירות, ייצוג פוליטי ואמון הציבור בישראל",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "DGtal",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:2020_elections_poling_station_in_Lod_02.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/a/a4/2020_elections_poling_station_in_Lod_02.jpg",
    "fileTitle": "2020 elections poling station in Lod 02.jpg",
    "sourceDimensions": "4160x3120",
    "localDimensions": "800x600",
    "specificityTier": "literal_or_near_literal_subject_match",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:elections",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר בוחן את בחירות 2015 בישראל באמצעות נתוני סקר החוסן הלאומי, ומראה כיצד עמדות על פטריוטיות, איום ביטחוני, אמון פוליטי ואמון בבית המשפט העליון מבדילות בין מצביעי מפלגות.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את הבחירות, הייצוג הפוליטי ואמון הציבור בישראל, וממקם את הסוגיה בתוך הדיון הרחב על מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות.",
        "נקודת המוצא של המאמר היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר משתמש בנתוני סקר החוסן הלאומי של המרכז למחקרי ביטחון לאומי באוניברסיטת חיפה. בכל נקודת זמן נדגמו 2,000 משיבים, מהם כ-1,600 יהודים, באמצעות סקרי טלפון רחבי היקף וסולם ליקרט מ-1 עד 6. הניתוח התמקד בשאלות שמבחינות בין דגשים רפובליקניים, כמו פטריוטיות, הצדקה עצמית קולקטיבית, תחושת מצור ואמון בהנהגה פוליטית, לבין דגש ליברלי שנמדד דרך אמון בבית המשפט העליון.",
        "הניתוח מראה פערים ברורים בין מצביעי הליכוד למצביעי העבודה. מצביעי הליכוד קיבלו ממוצעים גבוהים יותר בפטריוטיות, בהצדקה עצמית קולקטיבית, בפחד מאויבים, במיליטנטיות ובאמון פוליטי; למשל 5.2828 בפטריוטיות לעומת 4.6177 בעבודה, ו-4.9084 במיליטנטיות לעומת 3.4799. באמון בבית המשפט העליון התמונה התהפכה: מצביעי העבודה קיבלו 5.1316 לעומת 3.9755 בקרב מצביעי הליכוד.",
        "המאמר טוען שהעימות בין אתוסים אינו דיכוטומי לגמרי אלא רצף מפלגתי. מרצ קרובה יותר לקוטב הליברלי מן העבודה, יש עתיד ממוקמת בתוך גבולות מחנה המרכז-שמאל אך בחלק מהמדדים דומה יותר לימין, והמפלגות הדתיות והחרדיות קרובות יותר לאתוס הרפובליקני. בקרב ש״ס ויהדות התורה נמצא אמון נמוך במיוחד בבית המשפט העליון, ובתוך הימין ניכרים הבדלים בין הליכוד, ישראל ביתנו, מפד״ל, ש״ס ויהדות התורה במדדי פטריוטיות, תחושת מצור ומיליטנטיות."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת בחירות, ייצוג פוליטי ואמון הציבור בישראל לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד מוסדות ציבור, חברה אזרחית, שירות ציבורי, ממשל ואחריותיות מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בבחירות, ייצוג פוליטי ואמון הציבור בישראל מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את הבחירות, הייצוג הפוליטי ואמון הציבור בישראל כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>בחירות, ייצוג פוליטי ואמון הציבור בישראל</strong> מושפעים מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בבחירות, ייצוג פוליטי ואמון הציבור בישראל להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "האתוס הלאומי והפסיכולוגיה הפוליטית של בחירות 2015",
  "permalink": "/democracy_its_the_national_ethos_stupid_understanding_the_political_psychology_of_the_israeli_2015_elections_using_dat_summary_he.html",
  "paper_url": "democracy_its_the_national_ethos_stupid_understanding_the_political_psychology_of_the_israeli_2015_elections_using_dat_summary_he.html",
  "sortKey": 202605210235
}
---
