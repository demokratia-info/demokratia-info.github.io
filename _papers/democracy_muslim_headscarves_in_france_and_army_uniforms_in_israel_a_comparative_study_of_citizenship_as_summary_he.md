---
{
  "version": 1,
  "file": "democracy_muslim_headscarves_in_france_and_army_uniforms_in_israel_a_comparative_study_of_citizenship_as_summary_he.html",
  "slug": "democracy_muslim_headscarves_in_france_and_army_uniforms_in_israel_a_comparative_study_of_citizenship_as_summary_he",
  "order": 147,
  "titleHe": "כיסויי ראש בצרפת ומדי צבא בישראל: אזרחות כמסכה",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית אזרחות כמסכה בהשוואה בין צרפת לישראל, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית אזרחות כמסכה בהשוואה בין צרפת לישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Leora Bilsky",
  "paperTitle": "Muslim headscarves in France and army uniforms in Israel: a comparative study of citizenship as mask",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-4357-3696\" target=\"_blank\" rel=\"noopener noreferrer\">Leora Bilsky</a>",
  "journal": "Patterns of Prejudice",
  "dateText": "פורסם: 2009",
  "doiUrl": "https://doi.org/10.1080/00313220903109193",
  "doiLabel": "https://doi.org/10.1080/00313220903109193",
  "authors": [
    {
      "@type": "Person",
      "name": "ליאורה בילסקי",
      "url": "https://orcid.org/0000-0003-4357-3696"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Leora Bilsky",
      "url": "https://orcid.org/0000-0003-4357-3696"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "אזרחות כמסכה בהשוואה בין צרפת לישראל",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_muslim_headscarves_in_france_and_army_uniforms_in_israel_a_comparative_study_of_citizenship_as_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על אזרחות כמסכה בהשוואה בין צרפת לישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית אזרחות כמסכה בהשוואה בין צרפת לישראל דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "אזרחות היא הקשר המשפטי והפוליטי בין אדם למדינה, הכולל זכויות, חובות ויכולת להשתתף בחיים הציבוריים. בהקשר של המאמר, המושג מסייע לבחון מי נחשב שותף מלא בקהילה הפוליטית ומהם גבולות השייכות, הייצוג וההגנה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: האיסור היה תוצאה של מחלוקת מתמשכת בצרפת לגבי קבילות החיג'אב שלבשו תלמידות בית ספר מוסלמיות. היא טוענת שההתמקדות בצביעות חושפת היבט חשוב באזרחות שהתפרש לא נכון בשני המקרים, כלומר \"אזרחות כמסכה\". בילסקי חוזר להרהורים של חנה ארנדט על אזרחות כדרך להבין את גבולותיה של תיאוריית שוויון המבוססת על זהות, ומשתמש בשתי המחלוקות כדי להדגים את הצורך בפיתוח תיאוריית אזרחות שתוכל להגיב טוב יותר הן לשוויון והן לריבוי.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית אזרחות כמסכה בהשוואה בין צרפת לישראל, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת אזרחות כמסכה בהשוואה בין צרפת לישראל לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר אזרחות כמסכה בהשוואה בין צרפת לישראל משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "כיסויי ראש בצרפת ומדי צבא בישראל: אזרחות כמסכה",
  "permalink": "/democracy_muslim_headscarves_in_france_and_army_uniforms_in_israel_a_comparative_study_of_citizenship_as_summary_he.html",
  "paper_url": "democracy_muslim_headscarves_in_france_and_army_uniforms_in_israel_a_comparative_study_of_citizenship_as_summary_he.html",
  "sortKey": 202605210146
}
---
