---
{
  "version": 1,
  "file": "democracy_from_integration_to_segregation_the_role_of_religion_in_the_idf_summary_he.html",
  "slug": "democracy_from_integration_to_segregation_the_role_of_religion_in_the_idf_summary_he",
  "titleHe": "מהשתלבות להפרדה: תפקידה של הדת בצה״ל",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את תפקידה המורכב של הדת בשירות הצבאי בישראל. לפי התקציר, הדת היהודית אינה מוגבלת לרבנות הצבאית אלא חודרת לתחומים רבים בחיי הצבא. בניגוד לחוכמה המקובלת שראתה בדת גורם מלכד, המאמר מצביע על מתחים חדשים: אי־שירות מטעמים דתיים, שירות נפרד ושירות מותנה.",
  "summaryHe": "המאמר טוען שתפקידה של הדת בצה״ל מורכב מכדי להתאים לפרדיגמות רגילות של צבא במדינה חילונית. אף שצה״ל הוא זרועה הצבאית של מדינה חילונית וליברלית, הוא אינו מגביל את הדת לתפקיד צר של רבנות צבאית. מוטיבים יהודיים מסורתיים מופיעים בתחומים רבים של חיי הצבא ולעיתים מגויסים למטרות צבאיות. במשך שנים רווחה ההנחה שהדת ממלאת בצה״ל תפקיד אינטגרטיבי, מחזקת לכידות ומחברת בין קבוצות. המאמר בוחן את בסיס הטענה הזו ואז מטיל ספק בעדכניותה. לפי התקציר, מתחים בחברה הישראלית סביב השקפות ואורחות חיים דתיים מתחילים להשפיע גם על הצבא. שלוש תופעות עומדות במרכז: אי־שירות מטעמים דתיים, שירות נפרד של חיילים מן הציבור הדתי־לאומי, וסימנים ראשונים לשירות מותנה. לכן הסיכום צריך להציג את המאמר כמעבר מדת כמנגנון שילוב לדת כמקור אפשרי להפרדה, משא ומתן ותנאים בתוך הצבא.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Stuart A. Cohen",
  "paperTitle": "From Integration to Segregation The Role of Religion in the IDF",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-2931-4495\" target=\"_blank\" rel=\"noopener noreferrer\">Stuart A. Cohen</a>",
  "journal": "Armed Forces & Society",
  "dateText": "פורסם: 1999",
  "doiUrl": "https://doi.org/10.1177/0095327x9902500303",
  "doiLabel": "https://doi.org/10.1177/0095327x9902500303",
  "authors": [
    {
      "@type": "Person",
      "name": "Stuart A. Cohen",
      "url": "https://orcid.org/0000-0003-2931-4495"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Stuart A. Cohen",
      "url": "https://orcid.org/0000-0003-2931-4495"
    }
  ],
  "topics": [
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "מוסדות",
    "ממשל",
    "שירות ציבורי",
    "אחריותיות"
  ],
  "image": {
    "src": "assets/article-images/real_photo_255_religion_democracy_from_integration_to_segregation_the_role_of_religion_in_the_idf_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לדת בצה״ל: מאינטגרציה להפרדה",
    "fitness": "high",
    "license": "CC BY-SA 3.0",
    "creator": "Wknight94 talk",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Western_Wall_tunnel_prayer_hall_2010.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/6/60/Western_Wall_tunnel_prayer_hall_2010.jpg",
    "fileTitle": "Western Wall tunnel prayer hall 2010.jpg",
    "sourceDimensions": "4272x2848",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:religion",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/3.0/"
  },
  "datePublished": "2026-05-23",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כיצד הדת בצה״ל עברה מדימוי של כוח מלכד למקור של הפרדה, אי־שירות ושירות מותנה.",
  "sections": [
    {
      "headingHe": "למה הדת בצה״ל מורכבת במיוחד?",
      "paragraphsHtml": [
        "צה״ל פועל במדינה חילונית וליברלית, אך הדת היהודית אינה נותרת בשוליים. היא מופיעה בטקסים, סמלים, חינוך, זהות יחידתית והצדקות ערכיות לשירות."
      ]
    },
    {
      "headingHe": "מה הייתה החוכמה המקובלת על דת וצבא?",
      "paragraphsHtml": [
        "ההנחה הרווחת הייתה שהדת ממלאת תפקיד אינטגרטיבי: היא מעניקה משמעות לשירות, מחזקת לכידות ומסייעת לחיבור בין חיילים שונים סביב זהות יהודית־לאומית."
      ]
    },
    {
      "headingHe": "איזה שינוי המאמר מזהה?",
      "paragraphsHtml": [
        "המאמר מצביע על כך שמתחים דתיים בחברה הישראלית חודרים לצבא. במקום דת שמאחדת בלבד, מופיעות תופעות של הסתייגות, הפרדה והתניה של השירות."
      ]
    },
    {
      "headingHe": "אילו שלוש תופעות נבחנות?",
      "paragraphsHtml": [
        "התקציר מונה אי־שירות מטעמים דתיים, שירות נפרד בקרב חיילים דתיים־לאומיים, וסימנים לשירות מותנה. אלה מצביעים על שינוי ביחסי דת, אזרחות וצבא."
      ]
    },
    {
      "headingHe": "מה המשמעות הדמוקרטית של הטענה?",
      "paragraphsHtml": [
        "צבא אזרחי במדינה דמוקרטית נשען על שוויון, סמכות משותפת ולכידות. כאשר קבוצות מגדירות תנאים דתיים לשירות או מבקשות הפרדה, מתחדדת השאלה כיצד צה״ל יכול להישאר מוסד משותף."
      ]
    }
  ],
  "layout": "paper",
  "title": "מהשתלבות להפרדה: תפקידה של הדת בצה״ל",
  "permalink": "/democracy_from_integration_to_segregation_the_role_of_religion_in_the_idf_summary_he.html",
  "paper_url": "democracy_from_integration_to_segregation_the_role_of_religion_in_the_idf_summary_he.html",
  "sortKey": 202605230079
}
---
