---
{
  "version": 1,
  "file": "democracy_constitutional_reform_economy_summary_he.html",
  "slug": "democracy_constitutional_reform_economy_summary_he",
  "order": 17,
  "titleHe": "כשהוויכוח על הדמוקרטיה פוגש את הכלכלה",
  "subtitleHe": "",
  "descriptionHe": "המאמר טוען שהמשבר סביב השינויים החוקתיים בישראל לא נשאר בתוך עולם המשפט והפוליטיקה; הוא השפיע גם על תפיסת הסיכון של הכלכלה הישראלית.",
  "summaryHe": "מה קורה לאמון משקיעים ולתפיסת סיכון בזמן משבר חוקתי.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "ארז כהן",
  "paperTitle": "Political and public discourse surrounding constitutional reform and its implications for the Israeli economy",
  "authorsHtml": "<a href=\"https://cris.ariel.ac.il/en/persons/erez-cohen-2/\" target=\"_blank\" rel=\"noopener noreferrer\">Erez Cohen</a>",
  "journal": "The Social Science Journal",
  "dateText": "פורסם אונליין: 10 ביולי 2025",
  "sourcePublishedDate": "2025-07-10",
  "doiUrl": "https://doi.org/10.1080/03623319.2025.2529592",
  "doiLabel": "https://doi.org/10.1080/03623319.2025.2529592",
  "authors": [
    {
      "@type": "Person",
      "name": "ארז כהן",
      "url": "https://cris.ariel.ac.il/en/persons/erez-cohen-2/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Erez Cohen",
      "url": "https://cris.ariel.ac.il/en/persons/erez-cohen-2/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "institutions-civil-society-public-service",
    "democratic-institutions-economic-prosperity"
  ],
  "keywords": [
    "דמוקרטיה ליברלית",
    "זכויות",
    "מוסדות דמוקרטיים",
    "שלטון החוק"
  ],
  "image": {
    "src": "assets/article-images/real_photo_128_economy_democracy_constitutional_reform_economy_summary_he.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום הקשור לכשהוויכוח על הדמוקרטיה פוגש את הכלכלה",
    "fitness": "high",
    "license": "CC BY 2.5",
    "creator": "Ilana Shkolnik",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:PikiWiki_Israel_33048_Economy_of_Israel.JPG",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/5/5a/PikiWiki_Israel_33048_Economy_of_Israel.JPG",
    "fileTitle": "PikiWiki Israel 33048 Economy of Israel.JPG",
    "sourceDimensions": "2592x1944",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:economy",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/2.5/"
  },
  "datePublished": "2026-05-07",
  "dateModified": "2026-07-05",
  "lastUpdatedHe": "5 ביולי 2026",
  "oneLinerHtml": "המאמר טוען שהמשבר סביב השינויים החוקתיים בישראל לא נשאר בתוך עולם המשפט והפוליטיקה; הוא השפיע גם על תפיסת הסיכון של הכלכלה הישראלית.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "חוקה היא מערכת כללי יסוד שמגדירה את מבנה השלטון, סמכויותיו וזכויות היסוד; חוקתנות היא התפיסה שלפיה גם רוב פוליטי ומוסדות שלטון מוגבלים על ידי כללים אלה. בהקשר של המאמר, המושג מסייע להבין כיצד כללי יסוד, בתי משפט וחוקי יסוד יכולים להגביל כוח ולייצב מחלוקת פוליטית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן כיצד שיח פוליטי וציבורי סביב רפורמה חוקתית משפיע על אמון משקיעים ועל תפיסת סיכון בזמן משבר חוקתי.",
        "הדגש הוא על הקשר בין אי־יציבות מוסדית לבין ציפיות כלכליות, ולא רק על ההיבט המשפטי של הרפורמה."
      ]
    },
    {
      "headingHe": "מה נבדק במחקר",
      "paragraphsHtml": [
        "ארז כהן בוחן כיצד הדיון הציבורי והפוליטי סביב הרפורמה החוקתית של 2023 השפיע על מדדים כלכליים ועל אמון משקיעים ומוסדות בינלאומיים בישראל.",
        "המחקר משלב מקורות רשמיים, כמו הצעות חוק, דוחות ממשלתיים ופרוטוקולים בכנסת, לצד ניתוח אינדיקטורים כלכליים.",
        "המאמר מתמקד בתקופה שבה הרפורמה יצרה אי-ודאות חריגה לגבי כללי המשחק המוסדיים בישראל."
      ]
    },
    {
      "headingHe": "הממצאים המרכזיים",
      "paragraphsHtml": [
        "קידום החקיקה נקשר לעלייה באי-הוודאות לגבי הכלכלה הישראלית ולפגיעה בתדמית היציבות של ישראל.",
        "לפי המאמר, הדבר התבטא בהורדות דירוג אשראי, האטה ביבוא ובהשקעות זרות ישירות, וכן בירידה בגיוסי הון ובהשקעות בחברות הייטק ישראליות.",
        "הטענה אינה שכל שינוי כלכלי נגרם רק מהרפורמה, אלא שהמשבר המוסדי הפך לרכיב מרכזי בהערכת הסיכון של ישראל."
      ]
    },
    {
      "headingHe": "למה זה חשוב לישראל",
      "paragraphsHtml": [
        "המאמר הופך דיון שנשמע לעיתים מופשט - הפרדת רשויות, ביקורת שיפוטית ואיזונים ובלמים - לשאלה יומיומית של השקעות, תעסוקה, צמיחה ויוקר הון.",
        "הוא מדגיש שמשקיעים ומוסדות אשראי מסתכלים לא רק על נתוני מאקרו, אלא גם על אמינות מוסדות השלטון ועל יציבות כללי המשחק.",
        "עבור הציבור הישראלי, המסר הוא שדמוקרטיה מתפקדת היא גם נכס כלכלי: כשהמוסדות נתפסים כלא יציבים, המחיר עלול להגיע לשוק העבודה ולכיס."
      ]
    },
    {
      "headingHe": "במילים פשוטות",
      "paragraphsHtml": [
        "אם בעולם חוששים שהמערכת המשפטית והפוליטית בישראל משתנה במהירות וללא הסכמה רחבה, הם עשויים לדרוש מחיר גבוה יותר על השקעה בישראל או לבחור להשקיע במקום אחר.",
        "אמון הוא אחד המשאבים החשובים ביותר של כלכלה מודרנית. לכן, ויכוח חוקתי אינו רק עניין של משפטנים."
      ]
    }
  ],
  "layout": "paper",
  "title": "כשהוויכוח על הדמוקרטיה פוגש את הכלכלה",
  "permalink": "/democracy_constitutional_reform_economy_summary_he.html",
  "paper_url": "democracy_constitutional_reform_economy_summary_he.html",
  "sortKey": 202605070008
}
---
