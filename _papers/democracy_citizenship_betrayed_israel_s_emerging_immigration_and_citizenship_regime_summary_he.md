---
{
  "version": 1,
  "file": "democracy_citizenship_betrayed_israel_s_emerging_immigration_and_citizenship_regime_summary_he.html",
  "slug": "democracy_citizenship_betrayed_israel_s_emerging_immigration_and_citizenship_regime_summary_he",
  "order": 24,
  "titleHe": "אזרחות נבגדת: משטר ההגירה והאזרחות המתהווה בישראל",
  "subtitleHe": "",
  "descriptionHe": "המאמר בוחן את התפתחות משטר ההגירה והאזרחות בישראל בראשית המאה ה-21, תוך התמקדות בזכויות האזרחים הפלסטינים. הוא טוען שישראל נעה מדגם של דמוקרטיה אתנית לעבר מבנה הדומה יותר לאתנוקרטיה.",
  "summaryHe": "Peled מנתח את הפגיעה בזכויות אזרחות של פלסטינים אזרחי ישראל דרך חוק איחוד המשפחות, פסיקת בג״ץ, אירועי אוקטובר 2000 ותוכניות לשינוי גבול ואזרחות. המאמר מציג את הפגיעה באזרחות כיעד מדיניות מצטבר ולא רק כתוצר לוואי ביטחוני או דמוגרפי.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Yoav Peled",
  "paperTitle": "Citizenship Betrayed: Israel's Emerging Immigration and Citizenship Regime",
  "authorsHtml": "<a href=\"https://english.tau.ac.il/profile/yoavpel\" target=\"_blank\" rel=\"noopener noreferrer\">Yoav Peled</a>",
  "journal": "Theoretical Inquiries in Law",
  "dateText": "פורסם: 2007",
  "doiUrl": "https://doi.org/10.2202/1565-3404.1162",
  "doiLabel": "https://doi.org/10.2202/1565-3404.1162",
  "authors": [
    {
      "@type": "Person",
      "name": "Yoav Peled",
      "url": "https://english.tau.ac.il/profile/yoavpel"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Yoav Peled",
      "url": "https://english.tau.ac.il/profile/yoavpel"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "הדרה"
  ],
  "image": {
    "src": "assets/article-images/real_photo_099_religion_democracy_citizenship_betrayed_israel_s_emerging_immigration_and_citizenship_regime_summar.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום: אזרחות נבגדת: משטר ההגירה והאזרחות המתהווה בישראל",
    "fitness": "high",
    "license": "CC BY 2.5",
    "creator": "רחל וייסלר",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:143908_prayers_at_the_western_wall_jerusalem_PikiWiki_Israel.jpg",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/e/ef/143908_prayers_at_the_western_wall_jerusalem_PikiWiki_Israel.jpg",
    "fileTitle": "143908 prayers at the western wall jerusalem PikiWiki Israel.jpg",
    "sourceDimensions": "1980x1320",
    "localDimensions": "800x600",
    "specificityTier": "strong_contextual_anchor",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:religion",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "ready_to_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by/2.5/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר טוען שמשטר האזרחות המתעצב בישראל מטשטש את הקו בין פלסטינים אזרחים ללא־אזרחים, ובכך מחליש את עצם מעמד האזרחות של המיעוט הפלסטיני.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "אזרחות היא הקשר המשפטי והפוליטי בין אדם למדינה, הכולל זכויות, חובות ויכולת להשתתף בחיים הציבוריים. בהקשר של המאמר, המושג מסייע לבחון מי נחשב שותף מלא בקהילה הפוליטית ומהם גבולות השייכות, הייצוג וההגנה."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר בוחן את מעמדם של פלסטינים אזרחי ישראל דרך שני מבחנים: מידת השוויון בזכויות האזרחות, והיכולת לשפר את מעמדם במסגרת החוק. על בסיס מבחנים אלה הוא טוען כי בשנים הראשונות של המאה ה-21 ישראל נעה, בגבולותיה שלפני 1967, מדגם של דמוקרטיה אתנית לעבר צורה המזכירה אתנוקרטיה.",
        "הניתוח מתמקד בשלילת הזכות לאיחוד משפחות של אזרחים או תושבי קבע בישראל עם בני זוג, ילדים או הורים מהשטחים הכבושים, ובפסק דין בג״ץ מ-2006 שאישר את חוקתיות ההסדר. המאמר מציב זאת לצד אירועי אוקטובר 2000, דו״ח ועדת אור, ותוכנית פוליטית לשינוי גבול כך שחלק מהאזרחים הפלסטינים יאבדו את אזרחותם הישראלית.",
        "המסקנה המרכזית היא שהפגיעה בזכויות האזרחים הפלסטינים אינה מוצגת במאמר רק כמחיר שנועד להשיג ביטחון או יעד דמוגרפי. המאמר טוען כי טשטוש הקו בין פלסטינים אזרחים ללא־אזרחים, שלילת איחוד משפחות ותוכניות שלילת אזרחות מצטברים לכיוון מדיניות שבו החלשת האזרחות עצמה נעשית יעד."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר עוסק במעמדן הפוליטי והאזרחי של קבוצות מיעוט בישראל.",
        "הקריאה בו רלוונטית משום שהנושא משפיע על שוויון אזרחי, השתתפות פוליטית והגנה על זכויות, ולא רק על שאלה משפטית או פוליטית נקודתית."
      ]
    },
    {
      "headingHe": "מהי השאלה הדמוקרטית המרכזית?",
      "paragraphsHtml": [
        "השאלה המרכזית היא כיצד הנושא משנה את האיזון בין כוח שלטוני, זכויות, ביקורת ציבורית ואמון במוסדות.",
        "המסגור מתמקד בשאלה המוסדית והאזרחית שעולה מן המקור."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית תלויה לא רק בבחירות, אלא גם בשוויון אזרחי, השתתפות פוליטית והגנה על זכויות.",
        "לכן גם נושא שנראה מקצועי או מוסדי יכול להשפיע על השאלה אם כללי המשחק נשארים הוגנים, יציבים ופתוחים לביקורת."
      ]
    },
    {
      "headingHe": "אילו מוסדות או זכויות נמצאים ברקע?",
      "paragraphsHtml": [
        "ברקע עומדות שאלות על חלוקת כוח, אחריות שלטונית, זכויות שוות, עצמאות מוסדית והיכולת של קבוצות שונות להשתתף במרחב הציבורי.",
        "הדגש אינו על הכרעה מפלגתית, אלא על התנאים שמאפשרים מחלוקת דמוקרטית הוגנת."
      ]
    },
    {
      "headingHe": "איך זה מתחבר לישראל?",
      "paragraphsHtml": [
        "החיבור הישראלי נמצא בכך שהנושא משפיע על הדרך שבה מוסדות המדינה, קבוצות אזרחיות והציבור הרחב מבינים דמוקרטיה בפועל.",
        "במיוחד חשוב לשים לב להשפעה המצטברת של כללים, פרקטיקות ונורמות על אמון הציבור ועל זכויות שוות."
      ]
    },
    {
      "headingHe": "מה הקשר לשלטון החוק?",
      "paragraphsHtml": [
        "שלטון החוק דורש שכללים יחולו גם על בעלי כוח, ושיהיו מנגנונים שיכולים לבקר החלטות שלטוניות.",
        "כאשר הדיון עוסק במעמדן הפוליטי והאזרחי של קבוצות מיעוט בישראל, שאלת שלטון החוק היא האם נשמרים גבולות אפקטיביים לכוח והאם קיימת הגנה ממשית על זכויות."
      ]
    },
    {
      "headingHe": "מהי השורה התחתונה הציבורית?",
      "paragraphsHtml": [
        "השורה התחתונה היא שהנושא הוא חלק מן האופן שבו דמוקרטיה שומרת על גבולות כוח, זכויות ואמון ציבורי.",
        "התרומה של העמוד היא לסמן את השאלה הדמוקרטית ולכוון לקריאת המקור בלי להוסיף שמות, ציטוטים או ייחוסים שאינם מבוססים."
      ]
    }
  ],
  "layout": "paper",
  "title": "אזרחות נבגדת: משטר ההגירה והאזרחות המתהווה בישראל",
  "permalink": "/democracy_citizenship_betrayed_israel_s_emerging_immigration_and_citizenship_regime_summary_he.html",
  "paper_url": "democracy_citizenship_betrayed_israel_s_emerging_immigration_and_citizenship_regime_summary_he.html",
  "sortKey": 202605210094
}
---
