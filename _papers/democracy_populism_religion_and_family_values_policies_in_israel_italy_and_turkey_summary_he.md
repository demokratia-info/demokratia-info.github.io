---
{
  "version": 1,
  "file": "democracy_populism_religion_and_family_values_policies_in_israel_italy_and_turkey_summary_he.html",
  "slug": "democracy_populism_religion_and_family_values_policies_in_israel_italy_and_turkey_summary_he",
  "order": 325,
  "titleHe": "איך פופוליזם דתי מעצב מדיניות משפחה?",
  "subtitleHe": "",
  "descriptionHe": "המאמר משווה בין ישראל, איטליה וטורקיה כדי לבחון כיצד מפלגות פופוליסטיות ימניות משתמשות בערכי משפחה בדיונים על שוויון מגדרי, תכנון משפחה וזכויות להט״ב. הוא מראה שהשימוש במשפחה פועל גם נגד אליטות המוצגות כמושחתות מוסרית וגם נגד זרים ומהגרים המוצגים כאיום דמוגרפי או תרבותי.",
  "summaryHe": "המאמר בוחן את פוליטיקת ערכי המשפחה בקרב הליכוד וש״ס בישראל, הלגה והאחים של איטליה באיטליה, וה-AKP בטורקיה. הוא מציע להבין את ערכי המשפחה דרך שני צירי פופוליזם: ציר אנכי שמעמיד את העם מול אליטות, וציר אופקי שמגדיר זרים או קבוצות חיצוניות כאיום. ההשוואה מראה שמפלגות פופוליסטיות ימניות אינן פועלות תמיד באופן אחיד: חלקן מאמצות עמדות שמרניות מובהקות, ואחרות מציגות עמדות ליברליות יחסית כאשר הדבר משרת גבולות לאומיים, דתיים או תרבותיים.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Guy Ben‐Porat, Dani Filc, Ahmet Erdi Öztürk, Luca Ozzano",
  "paperTitle": "Populism, religion and family values policies in Israel, Italy and Turkey",
  "authorsHtml": "<a href=\"https://cris.bgu.ac.il/en/persons/guy-ben-porat\" target=\"_blank\" rel=\"noopener noreferrer\">Guy Ben‐Porat</a>, <a href=\"https://cris.bgu.ac.il/en/persons/dani-filc\" target=\"_blank\" rel=\"noopener noreferrer\">Dani Filc</a>, <a href=\"https://orcid.org/0000-0003-1749-6682\" target=\"_blank\" rel=\"noopener noreferrer\">Ahmet Erdi Öztürk</a>, <a href=\"https://orcid.org/0000-0002-2675-9344\" target=\"_blank\" rel=\"noopener noreferrer\">Luca Ozzano</a>",
  "journal": "Mediterranean Politics",
  "dateText": "פורסם: 2021",
  "doiUrl": "https://doi.org/10.1080/13629395.2021.1901484",
  "doiLabel": "https://doi.org/10.1080/13629395.2021.1901484",
  "authors": [
    {
      "@type": "Person",
      "name": "Guy Ben‐Porat",
      "url": "https://cris.bgu.ac.il/en/persons/guy-ben-porat"
    },
    {
      "@type": "Person",
      "name": "Dani Filc",
      "url": "https://cris.bgu.ac.il/en/persons/dani-filc"
    },
    {
      "@type": "Person",
      "name": "Ahmet Erdi Öztürk",
      "url": "https://orcid.org/0000-0003-1749-6682"
    },
    {
      "@type": "Person",
      "name": "Luca Ozzano",
      "url": "https://orcid.org/0000-0002-2675-9344"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Guy Ben‐Porat",
      "url": "https://cris.bgu.ac.il/en/persons/guy-ben-porat"
    },
    {
      "@type": "Person",
      "name": "Dani Filc",
      "url": "https://cris.bgu.ac.il/en/persons/dani-filc"
    },
    {
      "@type": "Person",
      "name": "Ahmet Erdi Öztürk",
      "url": "https://orcid.org/0000-0003-1749-6682"
    },
    {
      "@type": "Person",
      "name": "Luca Ozzano",
      "url": "https://orcid.org/0000-0002-2675-9344"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "rights-equality-minorities"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "שחיקת מוסדות",
    "חוסן דמוקרטי",
    "משבר חוקתי",
    "דת ומדינה",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "assets/article-images/real_photo_580_religion_democracy_populism_religion_and_family_values_policies_in_israel_italy_and_turkey_summary.jpg",
    "version": "2026-07-05-real-photo-handoff",
    "altHe": "תצלום על פופוליזם, מוסדות דמוקרטיים והגבלת כוח שלטוני",
    "fitness": "high",
    "license": "CC BY-SA 4.0",
    "creator": "Yagasi",
    "provider": "Wikimedia Commons",
    "sourcePage": "https://commons.wikimedia.org/wiki/File:Mikveh_Israel_-_Interior_of_the_Synagogue_in_May_2015.JPG",
    "originalUrl": "https://upload.wikimedia.org/wikipedia/commons/d/d6/Mikveh_Israel_-_Interior_of_the_Synagogue_in_May_2015.JPG",
    "fileTitle": "Mikveh Israel - Interior of the Synagogue in May 2015.JPG",
    "sourceDimensions": "3072x2304",
    "localDimensions": "800x600",
    "specificityTier": "contextual_anchor_review_recommended",
    "matchType": "wikimedia_commons_real_photo",
    "matchQuality": "real_photo_strong_contextual_match",
    "sourceMethod": "wikimedia_commons_real_photo",
    "visualAnchor": "real_photo:religion",
    "handoffStatus": "ready_to_review_real_photo",
    "finalHandoffStatus": "review_before_replace_image",
    "licenseUrl": "https://creativecommons.org/licenses/by-sa/4.0/"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-07-11",
  "lastUpdatedHe": "11 ביולי 2026",
  "oneLinerHtml": "המאמר מראה כיצד ערכי משפחה משמשים מפלגות פופוליסטיות לסימון אליטות, זרים וגבולות שייכות.",
  "sections": [
    {
      "headingHe": "איך משפחה הופכת לסמל פוליטי בפופוליזם דתי?",
      "paragraphsHtml": [
        "המאמר מציב את פופוליזם, דת ומדיניות ערכי משפחה בתוך ישראל, איטליה וטורקיה. במקום להציג את הנושא כמחלוקת עקרונית בלבד, הוא עוקב אחר האופן שבו מוסדות, שחקנים וכללים יומיומיים מייצרים את הבעיה המחקרית.",
        "כך נקודת הפתיחה היא קונקרטית: מדיניות משפחה, רטוריקה של מסורת, מגדר ודת. הפרטים האלה מאפשרים להבין מדוע הדיון נוגע למבנה פעולה ולא רק לעמדה אידאולוגית."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מתמקד במדיניות ובשיח סביב שוויון מגדרי, תכנון משפחה וזכויות להט״ב בישראל, איטליה וטורקיה. נקודת המוצא היא שמפלגות פופוליסטיות ימניות מזוהות לעיתים עם דת ושמרנות, אך בפועל הן עשויות לאמץ עמדות שונות ואף ליברליות חלקית, בהתאם לקשר שלהן למוסדות דת, לתחרות פוליטית ולזהות הקהל שלהן.",
        "הניתוח משתמש בשני צירים של פופוליזם. בציר האנכי, ערכי משפחה משמשים לתקיפת אליטות תרבותיות ומשפטיות שמוצגות כמי שאינן מחויבות למסורת. בציר האופקי, אותם ערכים נקשרים לחששות דמוגרפיים, לשיעורי ילודה, להגירה ולשאלה מי נכלל בעם ומי מוצג כזר או כאיום.",
        "במקרה הישראלי, המאמר מבחין בין הליכוד לש״ס. הליכוד מתואר כמפלגה פופוליסטית מדירה כלפי אזרחים ערבים, מבקשי מקלט ואליטות, אך בעלת עמדה מתונה יחסית בנושאי משפחה, הפלות וזכויות להט״ב. ש״ס, לעומת זאת, מציבה ערכי משפחה במרכז, פונה לגרעין חרדי ולפריפריה מסורתית מזרחית, ומשלבת דרישה להכללת מזרחים עם תמיכה במדיניות מדירה כלפי מהגרי עבודה ומבקשי מקלט."
      ]
    },
    {
      "headingHe": "מה משותף לישראל, איטליה וטורקיה במדיניות ערכי משפחה?",
      "paragraphsHtml": [
        "הראיות שהמאמר נשען עליהן מדגישות את מדיניות משפחה, רטוריקה של מסורת, מגדר ודת. זהו בסיס שמחייב קריאה זהירה של מנגנונים, תהליכים ותוצאות ביניים, ולא רק של הצהרות רשמיות.",
        "במובן הזה, המחקר בודק כיצד הטענה מקבלת צורה בפועל: מי פועל, באיזה מרחב מוסדי, אילו מגבלות מופעלות, ומה משתנה כאשר הכללים מתורגמים למעשה."
      ]
    },
    {
      "headingHe": "איפה מוסדות דת ומפלגות משנים את סוג המדיניות?",
      "paragraphsHtml": [
        "לב המאמר הוא המתח בין משפחה כמרחב פרטי לכאורה מול סמל פוליטי של העם והסדר המוסרי. המתח הזה אינו רק רקע; הוא המנוע שמסביר מדוע ההכרעות בתחום מורכבות, חלקיות ולעיתים סותרות.",
        "התרומה של המאמר היא בכך שהוא מראה כיצד שני הצדדים של המתח ממשיכים להתקיים יחד. לכן התוצאה אינה פתרון נקי, אלא מפת דרכים להבנת השדה שבו הפועלים צריכים לנווט."
      ]
    },
    {
      "headingHe": "כיצד רטוריקה של מסורת מגדירה מי שייך לעם?",
      "paragraphsHtml": [
        "המושג המרכזי שעולה מן הקריאה הוא פופוליזם דתי המשתמש במשפחה כדי לסמן שייכות ואיום. הוא מאפשר לארגן את פרטי המקרה במסגרת רחבה יותר, בלי למחוק את ההקשר הישראלי או ההשוואתי שבו הם מופיעים.",
        "באמצעות המושג הזה, המאמר מעביר את הקורא משאלה תיאורית לשאלה אנליטית: לא רק מה קרה, אלא איזה סוג של כוח, אחריות או שייכות נבנה דרך התהליך."
      ]
    },
    {
      "headingHe": "מה ההשוואה מלמדת על מגדר ופופוליזם?",
      "paragraphsHtml": [
        "המשמעות הרחבה של המאמר נמצאת בהשלכות על מגדר, פלורליזם וגבולות הקהילה הפוליטית. זהו המקום שבו הניתוח מתחבר לדיון בדמוקרטיה, שלטון חוק, אזרחות או מוסדות ציבוריים, בהתאם לנושא הספציפי.",
        "המאמר משנה את נקודת המבט משום שהוא מבקש לקרוא את הזירה הנחקרת לא כפרט שולי, אלא כמקום שבו נבחנים בפועל גבולות האחריות, השוויון והלגיטימציה."
      ]
    }
  ],
  "layout": "paper",
  "title": "איך פופוליזם דתי מעצב מדיניות משפחה?",
  "permalink": "/democracy_populism_religion_and_family_values_policies_in_israel_italy_and_turkey_summary_he.html",
  "paper_url": "democracy_populism_religion_and_family_values_policies_in_israel_italy_and_turkey_summary_he.html",
  "sortKey": 202605210325
}
---
