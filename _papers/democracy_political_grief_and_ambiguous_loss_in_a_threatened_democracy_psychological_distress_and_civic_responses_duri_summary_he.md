---
{
  "version": 1,
  "file": "democracy_political_grief_and_ambiguous_loss_in_a_threatened_democracy_psychological_distress_and_civic_responses_duri_summary_he.html",
  "slug": "democracy_political_grief_and_ambiguous_loss_in_a_threatened_democracy_psychological_distress_and_civic_responses_duri_summary_he",
  "order": 309,
  "titleHe": "אבל פוליטי, אובדן עמום ומחאה אזרחית בזמן הרפורמה המשפטית",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בהרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים, וממקם את הדיון בתוך שאלות של נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי. הוא רלוונטי להבנת הדמוקרטיה בישראל משום שהוא מחבר בין מקור אקדמי לבין שאלות של זכויות, מוסדות, אמון ציבורי והגבלת כוח.",
  "summaryHe": "המאמר בוחן את הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים דרך עדשה של נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי.",
  "authorsCardHe": "Einat Yehene, Shay Ohayon",
  "paperTitle": "Political grief and ambiguous loss in a threatened democracy: psychological distress and civic responses during Israel’s judicial reform",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-9780-3974\" target=\"_blank\" rel=\"noopener noreferrer\">Einat Yehene</a>, <a href=\"https://orcid.org/0000-0001-8417-9121\" target=\"_blank\" rel=\"noopener noreferrer\">Shay Ohayon</a>",
  "journal": "Frontiers in Psychiatry",
  "dateText": "פורסם: 2026",
  "sourcePublishedDate": "2026-01-12",
  "doiUrl": "https://doi.org/10.3389/fpsyt.2025.1687951",
  "doiLabel": "https://doi.org/10.3389/fpsyt.2025.1687951",
  "authors": [
    {
      "@type": "Person",
      "name": "Einat Yehene",
      "url": "https://orcid.org/0000-0001-9780-3974"
    },
    {
      "@type": "Person",
      "name": "Shay Ohayon",
      "url": "https://orcid.org/0000-0001-8417-9121"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Einat Yehene",
      "url": "https://orcid.org/0000-0001-9780-3974"
    },
    {
      "@type": "Person",
      "name": "Shay Ohayon",
      "url": "https://orcid.org/0000-0001-8417-9121"
    }
  ],
  "topics": [
    "democratic-backsliding-populism",
    "constitutionalism-courts-rule-of-law"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "שחיקת מוסדות",
    "חוסן דמוקרטי",
    "משבר חוקתי",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_political_grief_and_ambiguous_loss_in_a_threatened_democracy_psychological_distress_and_civic_responses_duri_summary_he.jpg",
    "version": "2026-05-21-nightly-100b",
    "altHe": "איור עריכתי על הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים ומראה מדוע הדיון קשור לנסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי. הקריאה בו חשובה משום שהיא מחברת בין מקור אקדמי לבין השאלה כיצד דמוקרטיה מגבילה כוח, מגינה על זכויות ושומרת על כללי משחק הוגנים.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים, וממקם את הסוגיה בתוך הדיון הרחב על נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי.",
        "נקודת המוצא של התמצית היא השאלה כיצד עיצוב מוסדי, זכויות ואמון ציבורי משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: בהסתמך על תיאוריית האובדן והאבל המעורפלים, מחקר זה בדק כיצד נתפסת החזרה הדמוקרטית במהלך הרפורמה השיפוטית של ישראל ב-2023 השפיעה על מצוקה פסיכולוגית ומעורבות אזרחית. שיטות: נערך סקר חתך עם 453 מבוגרים ישראלים שמתנגדים לרפורמה. תוצאות: 64.9% מהמשתתפים חרגו מסף PG-13, מה שמעיד על צער פוליטי מוגבר.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים לשאלה דמוקרטית רחבה ולא רק לנושא מקצועי או נקודתי.",
        "כך המאמר מסייע להבין כיצד נסיגה דמוקרטית, פופוליזם, שחיקת מוסדות וחוסן אזרחי מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר הדיון בהרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים מתנהל בלי ביקורת מוסדית מספקת, הפגיעה עשויה להיראות תחילה צרה, אך בפועל להשפיע על אזרחים, קבוצות ומוסדות."
      ]
    },
    {
      "headingHe": "מהי המשמעות הישראלית של הדיון?",
      "paragraphsHtml": [
        "המאמר מאפשר לקרוא את הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים כחלק ממבנה רחב יותר של יחסי כוח, אחריות ציבורית והגנה על מיעוטים או מתנגדים.",
        "במובן הזה, הדיון אינו רק על אירוע או מוסד אחד, אלא על התנאים שמאפשרים לדמוקרטיה להישאר פתוחה, שוויונית ומוגבלת בכוחה."
      ]
    },
    {
      "headingHe": "איך המחקר קשור לשלטון החוק?",
      "paragraphsHtml": [
        "המחקר קשור לשלטון החוק משום שהוא בוחן כיצד <strong>הרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים</strong> מושפע מכללים, מוסדות ואפשרות ממשית להגביל כוח ציבורי.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד ממנו על זכויות?",
      "paragraphsHtml": [
        "אפשר ללמוד ממנו שזכויות אינן רק הצהרות מופשטות; הן תלויות במנגנונים שמתרגמים את הדיון בהרפורמה המשפטית, עצמאות שיפוטית והאיזונים הדמוקרטיים להגנה מעשית על יחידים וקבוצות.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "האם המאמר עוסק רק בישראל?",
      "paragraphsHtml": [
        "המאמר מתמקד במקרה הישראלי או משתמש בישראל כמקרה מרכזי, אך הערך שלו רחב יותר משום שהוא מאפשר לחשוב על הקשר בין מוסדות, זהות פוליטית וזכויות גם במקרי השוואה.",
        "אף על פי שאפשר ללמוד ממנו מעבר למקרה הישראלי, ההשוואה צריכה להישאר זהירה ולא להעביר מסקנות מהקשר אחד לאחר בלי בדיקה נוספת."
      ]
    }
  ],
  "layout": "paper",
  "title": "אבל פוליטי, אובדן עמום ומחאה אזרחית בזמן הרפורמה המשפטית",
  "permalink": "/democracy_political_grief_and_ambiguous_loss_in_a_threatened_democracy_psychological_distress_and_civic_responses_duri_summary_he.html",
  "paper_url": "democracy_political_grief_and_ambiguous_loss_in_a_threatened_democracy_psychological_distress_and_civic_responses_duri_summary_he.html",
  "sortKey": 202605210309
}
---
