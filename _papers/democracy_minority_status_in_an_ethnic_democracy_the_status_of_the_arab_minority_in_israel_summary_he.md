---
{
  "version": 1,
  "file": "democracy_minority_status_in_an_ethnic_democracy_the_status_of_the_arab_minority_in_israel_summary_he.html",
  "slug": "democracy_minority_status_in_an_ethnic_democracy_the_status_of_the_arab_minority_in_israel_summary_he",
  "order": 68,
  "titleHe": "מעמד המיעוט הערבי בדמוקרטיה אתנית",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית, וממקם את הדיון בתוך שאלות של זכויות, שוויון ומיעוטים, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית דרך עדשה של זכויות, שוויון ומיעוטים.",
  "authorsCardHe": "Sammy Smooha",
  "paperTitle": "Minority status in an ethnic democracy: The status of the Arab minority in Israel",
  "authorsHtml": "<a href=\"https://cris.haifa.ac.il/en/persons/sammy-smooha/\" target=\"_blank\" rel=\"noopener noreferrer\">Sammy Smooha</a>",
  "journal": "Ethnic and Racial Studies",
  "dateText": "פורסם: 1990",
  "doiUrl": "https://doi.org/10.1080/01419870.1990.9993679",
  "doiLabel": "https://doi.org/10.1080/01419870.1990.9993679",
  "authors": [
    {
      "@type": "Person",
      "name": "סמי סמוחה",
      "url": "https://cris.haifa.ac.il/en/persons/sammy-smooha/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Sammy Smooha",
      "url": "https://cris.haifa.ac.il/en/persons/sammy-smooha/"
    }
  ],
  "topics": [
    "rights-equality-minorities"
  ],
  "keywords": [
    "זכויות אדם",
    "שוויון",
    "מיעוטים",
    "אזרחות",
    "דמוקרטיה ליברלית",
    "מעמד המיעוט הערבי בדמוקרטיה הישראלית",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_minority_status_in_an_ethnic_democracy_the_status_of_the_arab_minority_in_israel_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על מעמד המיעוט הערבי בדמוקרטיה הישראלית",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית דרך עדשה של זכויות, שוויון ומיעוטים.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "דמוקרטיה אתנית היא מסגרת שבה מתקיימים מוסדות דמוקרטיים, אך המדינה מעניקה עדיפות לאומית או פוליטית לקבוצת רוב אתנית. בהקשר של המאמר, המושג חשוב כדי לבחון את המתח בין אזרחות שווה לבין העדפה ממוסדת של זהות לאומית אחת."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: תקציר מקובל להניח כי דמוקרטיה בחברות מפוצלות עמוקות לובשת צורה עיקרית או קונסוציאציונית. המאמר מציג את הסוג הכללי השלישי הזה, שעד כה לא מוכר, של 'דמוקרטיה אתנית' ומדגים את התועלת שלו עבור ישראל בטיפול במיעוט הערבי שלה. המתחים והסתירות באופייה הכפול של ישראל כמדינה יהודית דמוקרטית מולידים חמש דרישות ערביות שהרוב היהודי דוחה: הפיכת ישראל ללא-יהודית ולא-ציונית, קבלת הלאומיות הפלסטינית, הסרת כל ההגבלות על זכויות הפרט הערבי, הענקת זכויות לאומיות לאומיות מסוימות ושילוב ערבים במבנה הכוח הלאומי.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית מעמד המיעוט הערבי בדמוקרטיה הישראלית, וממקם את הסוגיה בתוך הדיון הרחב על זכויות, שוויון ומיעוטים.",
        "נקודת המוצא של התמצית היא השאלה כיצד שוויון אזרחי, זכויות קבוצתיות והגנה על מי שאינם חלק מן הרוב משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת מעמד המיעוט הערבי בדמוקרטיה הישראלית לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד זכויות, שוויון ומיעוטים מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר מעמד המיעוט הערבי בדמוקרטיה הישראלית משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מעמד המיעוט הערבי בדמוקרטיה אתנית",
  "permalink": "/democracy_minority_status_in_an_ethnic_democracy_the_status_of_the_arab_minority_in_israel_summary_he.html",
  "paper_url": "democracy_minority_status_in_an_ethnic_democracy_the_status_of_the_arab_minority_in_israel_summary_he.html",
  "sortKey": 202605210225
}
---
