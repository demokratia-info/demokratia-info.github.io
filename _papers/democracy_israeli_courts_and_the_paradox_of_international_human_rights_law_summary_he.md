---
{
  "version": 1,
  "file": "democracy_israeli_courts_and_the_paradox_of_international_human_rights_law_summary_he.html",
  "slug": "democracy_israeli_courts_and_the_paradox_of_international_human_rights_law_summary_he",
  "order": 65,
  "titleHe": "מה הקשר בין היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי לבין שלטון החוק בישראל?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי, וממקם את הדיון בתוך שאלות של חוקתיות, בתי משפט ושלטון החוק, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "authorsCardHe": "Natalie R Davidson; Tamar Hostovsky Brandes",
  "paperTitle": "Israeli Courts and the Paradox of International Human Rights Law",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0003-0698-5843\" target=\"_blank\" rel=\"noopener noreferrer\">Natalie R Davidson</a>; <a href=\"https://www.ono.ac.il/lecturer/dr-tamar-ostovsky-brandes/\" target=\"_blank\" rel=\"noopener noreferrer\">Tamar Hostovsky Brandes</a>",
  "journal": "European Journal of International Law",
  "dateText": "פורסם: 2022",
  "doiUrl": "https://doi.org/10.1093/ejil/chac070",
  "doiLabel": "https://doi.org/10.1093/ejil/chac070",
  "authors": [
    {
      "@type": "Person",
      "name": "Natalie R Davidson",
      "url": "https://orcid.org/0000-0003-0698-5843"
    },
    {
      "@type": "Person",
      "name": "תמר הוסטובסקי ברנדס",
      "url": "https://www.ono.ac.il/lecturer/dr-tamar-ostovsky-brandes/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Natalie R Davidson",
      "url": "https://orcid.org/0000-0003-0698-5843"
    },
    {
      "@type": "Person",
      "name": "Tamar Hostovsky Brandes",
      "url": "https://www.ono.ac.il/lecturer/dr-tamar-ostovsky-brandes/"
    }
  ],
  "topics": [
    "constitutionalism-courts-rule-of-law",
    "rights-equality-minorities"
  ],
  "keywords": [
    "חוקתיות",
    "בתי משפט",
    "שלטון החוק",
    "ביקורת שיפוטית",
    "זכויות אדם",
    "היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_israeli_courts_and_the_paradox_of_international_human_rights_law_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי",
    "fitness": "high"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי דרך עדשה של חוקתיות, בתי משפט ושלטון החוק.",
  "sections": [
    {
      "headingHe": "מושגי יסוד בקצרה",
      "paragraphsHtml": [
        "זכויות אדם הן זכויות בסיסיות שמיוחסות לכל אדם בשל אנושיותו, כגון חירות, שוויון, כבוד, הליך הוגן וחופש ביטוי. בהקשר של המאמר, המושג מסייע לבדוק אם מוסדות המדינה מגינים על יחידים וקבוצות גם כאשר הדבר מגביל כוח שלטוני או רוב פוליטי."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "הוא בוחן כיצד בתי המשפט בישראל משתמשים ב-IHRL, בהתבסס על ניתוח תוכן כמותי ואיכותי של כל ההחלטות, בכל בתי המשפט, תוך התייחסות ל-IHRL בין השנים 1990 ל-2019. הוא מגלה שבתי המשפט בישראל מגייסים את IHRL בעיקר בהתייחס לזכויות ילדים והליך הוגן, ורק לעתים רחוקות מפעילים את IHRL ביחס לשוויון אתני ומגדרי. עוד עולה כי חלק ניכר מהפניות ל-IHRL משמשות להצדקת פעולה של המדינה."
      ]
    },
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי, וממקם את הסוגיה בתוך הדיון הרחב על חוקתיות, בתי משפט ושלטון החוק.",
        "נקודת המוצא של התמצית היא השאלה כיצד בלמים מוסדיים, ביקורת שיפוטית והגבלת כוח שלטוני משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד חוקתיות, בתי משפט ושלטון החוק מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "מה הקשר בין היחס בין בתי המשפט בישראל לבין משפט זכויות האדם הבין־לאומי לבין שלטון החוק בישראל?",
  "permalink": "/democracy_israeli_courts_and_the_paradox_of_international_human_rights_law_summary_he.html",
  "paper_url": "democracy_israeli_courts_and_the_paradox_of_international_human_rights_law_summary_he.html",
  "sortKey": 202605210228
}
---
