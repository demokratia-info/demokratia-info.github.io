---
{
  "version": 1,
  "file": "democracy_politics_exclusion_localization_palestinian_minority_oslo_summary_he.html",
  "slug": "democracy_politics_exclusion_localization_palestinian_minority_oslo_summary_he",
  "order": 892,
  "titleHe": "אוסלו, הדרה ולוקליזציה בפוליטיקה של המיעוט הפלסטיני בישראל",
  "subtitleHe": "",
  "descriptionHe": "תקציר בעברית של מאמר על הדרת המיעוט הפלסטיני בישראל מתהליך אוסלו ועל המעבר לפוליטיקה מקומית של זכויות, ייצוג וצדק לצד הזדהות לאומית פלסטינית רחבה.",
  "summaryHe": "המאמר בוחן כיצד הסכמי אוסלו השפיעו על אזרחי ישראל הפלסטינים, שהיו מושפעים מן הסכסוך אך לא הוכרו כשחקן בתהליך המדיני. הוא טוען שההדרה מאוסלו האיצה מאבק מקומי לשוויון וזכויות בתוך ישראל, ובו בזמן חיזקה הזדהות עם המאבק הפלסטיני הלאומי ותמיכה בפתרון שתי המדינות.",
  "summarySourceStatus": "Based on full text",
  "authorsCardHe": "Mansour Nasasra",
  "paperTitle": "The Politics of Exclusion and Localization: The Palestinian Minority in Israel and the Oslo Accords",
  "authorsHtml": "<a href=\"https://cris.bgu.ac.il/en/persons/mansour-nasasra/\" target=\"_blank\" rel=\"noopener noreferrer\">Mansour Nasasra</a>",
  "journal": "Ethnopolitics",
  "dateText": "פורסם אונליין: 2019; כרך 20(5), 2021",
  "doiUrl": "https://doi.org/10.1080/17449057.2019.1683300",
  "doiLabel": "https://doi.org/10.1080/17449057.2019.1683300",
  "authors": [
    {
      "@type": "Person",
      "name": "Mansour Nasasra",
      "url": "https://cris.bgu.ac.il/en/persons/mansour-nasasra/"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Mansour Nasasra",
      "url": "https://cris.bgu.ac.il/en/persons/mansour-nasasra/"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "הסכמי אוסלו",
    "המיעוט הפלסטיני בישראל",
    "לוקליזציה",
    "זכויות מיעוט",
    "צדק",
    "מפלגות ערביות",
    "הרשימה המשותפת",
    "הדרה פוליטית",
    "הגדרה עצמית",
    "אזרחות"
  ],
  "image": {
    "src": "html_qa/democracy_politics_exclusion_localization_palestinian_minority_oslo_summary_he.jpg",
    "version": "2026-06-25-nightly",
    "altHe": "נוף היסטורי של נצרת כמטפורה לפוליטיקה מקומית של המיעוט הפלסטיני בישראל",
    "fitness": "standard"
  },
  "datePublished": "2026-06-25",
  "dateModified": "2026-07-13",
  "lastUpdatedHe": "13 ביולי 2026",
  "oneLinerHtml": "המאמר טוען שהדרת המיעוט הפלסטיני בישראל מאוסלו יצרה <strong>לוקליזציה פוליטית</strong> של מאבק לזכויות בתוך ישראל, בלי לנתק את הזיקה למאבק הלאומי הפלסטיני.",
  "sections": [
    {
      "headingHe": "מה עומד במרכז המאמר?",
      "paragraphsHtml": [
        "המאמר בוחן את השפעת הסכמי אוסלו על המיעוט הפלסטיני־ערבי בישראל. הוא שואל מה קורה לקבוצה אזרחית שנמצאת בלב הסכסוך אך אינה מקבלת אפיק ביטוי או הכרה בתהליך המדיני.",
        "הנושא דמוקרטי משום שהוא עוסק באזרחות, ייצוג והכרה של מיעוט לאומי בתוך מדינה. הסכם מדיני יכול לשנות לא רק יחסי חוץ אלא גם את הפוליטיקה הפנימית של אזרחים."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "המאמר מציב את המיעוט הפלסטיני בישראל כקבוצה אזרחית ולאומית שמנתה כ־20% מאוכלוסיית המדינה, אך נותרה מחוץ להסכמי אוסלו ולמשא ומתן שלאחריהם. ההדרה מיוחסת לישראל, לאש״ף ולשחקנים בינלאומיים שהתמקדו בגדה המערבית וברצועת עזה.",
        "הניתוח מראה שהתגובה הפוליטית הייתה כפולה: מצד אחד, דגש גובר על שוויון, זכויות מיעוט וצדק בתוך ישראל; מצד אחר, הזדהות מפורשת יותר עם זכויות לאומיות פלסטיניות, הגדרה עצמית ופתרון שתי המדינות.",
        "המאמר עוקב אחר הדינמיקה הזאת דרך מפלגות ערביות, התנועה האסלאמית, ארגונים קהילתיים והרשימה המשותפת. הוא קושר את הלוקליזציה גם לדפוסי הצבעה, עם מעבר יחסי לתמיכה במפלגות ערביות במקום במפלגות ציוניות, ולהמשך ההדרה של אזרחים פלסטינים מקואליציות שלטוניות בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען?",
      "paragraphsHtml": [
        "לפי המאמר, היעדר ההכרה באוסלו גרם למיעוט הפלסטיני בישראל לנסח מחדש את הפוליטיקה שלו סביב זכויות שוות כמיעוט לאומי ילידי. במקביל, נשמרה תמיכה בזכויות הלאומיות הפלסטיניות הרחבות ובהגדרה עצמית.",
        "המאמר מתאר תהליך כפול של לוקליזציה ותמיכה במאבק רחב יותר. הוא מצביע גם על לוקליזציה בדפוסי ההצבעה, עם העדפה גוברת למפלגות ערביות כגון הרשימה המשותפת במקום מפלגות ציוניות."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: לוקליזציה פוליטית",
      "paragraphsHtml": [
        "לוקליזציה פוליטית היא העברת מוקד המאבק אל הזירה המקומית והאזרחית שבה הקבוצה חיה ופועלת. היא אינה בהכרח ויתור על זיקה לאומית רחבה יותר.",
        "במאמר, הלוקליזציה מסבירה כיצד המיעוט הפלסטיני בישראל תבע זכויות וצדק בתוך ישראל דווקא בעקבות הדרה מן התהליך המדיני. המושג עוזר להבין אזרחות כמעמד משפטי וכמאבק פוליטי."
      ]
    },
    {
      "headingHe": "איפה נמצאת המשמעות הדמוקרטית?",
      "paragraphsHtml": [
        "המשמעות הדמוקרטית היא שהכרה במיעוט אינה יכולה להידחות בשם הסדרים מדיניים שמדברים מעל ראשו. כאשר אזרחים מושפעים מתהליך מדיני אך אינם מוכרים בו, נוצרת בעיית ייצוג עמוקה.",
        "המאמר מדגיש כי תביעה לשוויון אזרחי יכולה להתקיים יחד עם זהות לאומית פלסטינית. בכך הוא מאתגר תפיסות שמעמידות נאמנות אזרחית וזכויות לאומיות כסתירה הכרחית."
      ]
    },
    {
      "headingHe": "איזו שאלה המחקר מציב?",
      "paragraphsHtml": [
        "השאלה היא כיצד הסכמי אוסלו השפיעו על הפוליטיקה של המיעוט הפלסטיני בישראל. המאמר בוחן קבוצה שהייתה מושפעת מן ההסכמים אך לא הוכרה בהם כשחקן פוליטי."
      ]
    },
    {
      "headingHe": "באיזו שיטה הוא משתמש?",
      "paragraphsHtml": [
        "המאמר הוא ניתוח פוליטי והיסטורי של תהליך אוסלו ושל תגובות המיעוט הפלסטיני בישראל. הוא נשען על מסגור מושגי של הדרה, זכויות מיעוט ולוקליזציה."
      ]
    },
    {
      "headingHe": "מה הממצא המרכזי לקורא הישראלי?",
      "paragraphsHtml": [
        "הממצא המרכזי הוא שאוסלו האיץ מאבק מקומי לזכויות וצדק בתוך ישראל. במקביל, הוא לא ניתק את המיעוט מן המאבק הפלסטיני הרחב אלא יצר שילוב בין זירה אזרחית לזירה לאומית."
      ]
    },
    {
      "headingHe": "איך זה קשור לשוויון או לזכויות?",
      "paragraphsHtml": [
        "הקשר לזכויות הוא בתביעה לשוויון כמיעוט לאומי ילידי בתוך ישראל. המאמר מראה שהדרה מתהליך מדיני יכולה לחזק דרישה לזכויות אזרחיות וקולקטיביות בזירה המקומית."
      ]
    },
    {
      "headingHe": "מה המשמעות למוסדות ציבוריים?",
      "paragraphsHtml": [
        "מפלגות, כנסת ומוסדות מדינה צריכים להבין שהכרה פוליטית אינה רק עניין סמלי. כאשר מיעוט אינו מקבל אפיק ביטוי, דפוסי ההצבעה והארגון הפוליטי יכולים להשתנות בכיוון של ייצוג נפרד ומקומי יותר."
      ]
    },
    {
      "headingHe": "איזו זהירות פרשנית נדרשת כאן?",
      "paragraphsHtml": [
        "המאמר אינו מציג את המיעוט הפלסטיני בישראל כעמדה אחת פשוטה. הוא מתאר תהליך פוליטי שבו תמיכה בזכויות פלסטיניות רחבות מתקיימת לצד מאבק אזרחי בתוך ישראל."
      ]
    },
    {
      "headingHe": "מה אפשר ללמוד למדיניות ציבורית?",
      "paragraphsHtml": [
        "למדיניות ציבורית, המאמר מצביע על חשיבות הכללת אזרחים מושפעים בתהליכים מדיניים ובדיונים על זכויות מיעוט. התעלמות מהם יכולה להעמיק תחושת הדרה ולשנות את הפוליטיקה המפלגתית."
      ]
    },
    {
      "headingHe": "מה יחפש כאן מי שמתעניין בנושא?",
      "paragraphsHtml": [
        "מי שמחפש על אוסלו והמיעוט הפלסטיני בישראל ימצא כאן הסבר על לוקליזציה, זכויות מיעוט והצבעה למפלגות ערביות. זה מקור חשוב להבנת הקשר בין תהליך שלום לבין אזרחות פנימית."
      ]
    },
    {
      "headingHe": "איזה מושג צריך לזכור כדי להבין את המאמר?",
      "paragraphsHtml": [
        "המושג החשוב הוא לוקליזציה, משום שהוא מסביר כיצד מאבק לאומי יכול לקבל צורה אזרחית ומקומית. המאמר משתמש בו כדי להראות שהפוליטיקה של המיעוט אינה רק תגובה חיצונית לסכסוך אלא גם פעולה בתוך המדינה."
      ]
    },
    {
      "headingHe": "מה התרומה של המאמר למאגר?",
      "paragraphsHtml": [
        "התרומה למאגר היא חיבור בין הסכמים מדיניים לבין זכויות אזרחיות של מיעוט בתוך ישראל. הוא מוסיף הבנה של דמוקרטיה כיכולת להכיר גם באזרחים שאינם חלק מן הרוב הלאומי."
      ]
    }
  ],
  "layout": "paper",
  "title": "אוסלו, הדרה ולוקליזציה בפוליטיקה של המיעוט הפלסטיני בישראל",
  "permalink": "/democracy_politics_exclusion_localization_palestinian_minority_oslo_summary_he.html",
  "paper_url": "democracy_politics_exclusion_localization_palestinian_minority_oslo_summary_he.html",
  "sortKey": 202606250001,
  "sourcePublishedDate": "2019-12-06"
}
---
