---
{
  "version": 1,
  "file": "democracy_druze_high_schools_identity_citizenship_summary_he.html",
  "slug": "democracy_druze_high_schools_identity_citizenship_summary_he",
  "order": 941,
  "titleHe": "בתי ספר תיכוניים דרוזיים מעצבים זהות ואזרחות בישראל",
  "subtitleHe": "מחקר בשני בתי ספר מצביע על מנגנוני חינוך לערכים, שייכות קהילתית וזהות ישראלית־דרוזית",
  "descriptionHe": "תקציר בעברית של מאמר על בתי ספר תיכוניים דרוזיים בישראל, זהות תלמידים, אזרחות, תוכנית לימודים, טקסים וקשר בין חינוך לקהילה.",
  "summaryHe": "המאמר בוחן כיצד שני בתי ספר תיכוניים דרוזיים בישראל מעצבים זהות ואזרחות של מתבגרים. לפי התקציר, המחקר השתמש בראיונות עם מורים, מנהלים, קובעי מדיניות ותלמידים, לצד תצפיות וניתוח מסמכים, ומצא ארבעה מנגנונים מרכזיים: חינוך לערכים רב־ממדי, פיתוח זהות ואזרחות, למידה משמעותית וטובת הקהילה.",
  "summarySourceStatus": "Based on abstract only",
  "authorsCardHe": "Deborah Court; Randa Abbas",
  "paperTitle": "Role of Druze high schools in Israel in shaping students’ identity and citizenship",
  "authorsHtml": "<a href=\"https://orcid.org/0000-0001-6332-9637\" target=\"_blank\" rel=\"noopener noreferrer\">Deborah Court</a>; <a href=\"https://orcid.org/0000-0002-9120-8813\" target=\"_blank\" rel=\"noopener noreferrer\">Randa Abbas</a>",
  "journal": "Education, Citizenship and Social Justice",
  "dateText": "פורסם: יולי 2010; כרך 5, גיליון 2, עמ׳ 145-162",
  "doiUrl": "https://doi.org/10.1177/1746197910370725",
  "doiLabel": "https://doi.org/10.1177/1746197910370725",
  "authors": [
    {
      "@type": "Person",
      "name": "Deborah Court",
      "url": "https://orcid.org/0000-0001-6332-9637"
    },
    {
      "@type": "Person",
      "name": "Randa Abbas",
      "url": "https://orcid.org/0000-0002-9120-8813"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Deborah Court",
      "url": "https://orcid.org/0000-0001-6332-9637"
    },
    {
      "@type": "Person",
      "name": "Randa Abbas",
      "url": "https://orcid.org/0000-0002-9120-8813"
    }
  ],
  "topics": [
    "rights-equality-minorities",
    "institutions-civil-society-public-service"
  ],
  "keywords": [
    "דרוזים",
    "בתי ספר תיכוניים",
    "זהות אזרחית",
    "חינוך לערכים",
    "אזרחות",
    "מיעוטים",
    "קהילה",
    "ישראל"
  ],
  "image": {
    "src": "html_qa/democracy_druze_high_schools_identity_citizenship_summary_he.jpg",
    "version": "2026-06-30-nightly",
    "altHe": "תלמידים דרוזים ומורה בדיון כיתתי כמטפורה לעיצוב זהות ואזרחות בבתי ספר תיכוניים",
    "fitness": "high"
  },
  "datePublished": "2026-06-30",
  "dateModified": "2026-06-30",
  "lastUpdatedHe": "30 ביוני 2026",
  "oneLinerHtml": "המאמר מראה כי <strong>בית הספר</strong> יכול להיות מוסד מרכזי בעיצוב אזרחות מיעוטית שאינה מוותרת על זהות קהילתית.",
  "sections": [
    {
      "headingHe": "בית הספר כמוסד אזרחי קהילתי",
      "paragraphsHtml": [
        "המאמר מציג בתי ספר תיכוניים דרוזיים כמרחבים שבהם זהות ואזרחות נבנות באופן מכוון. בית הספר אינו רק מקום לימוד, אלא מוסד שמחבר תלמידים למסורת, למדינה ולקהילה."
      ]
    },
    {
      "headingHe": "מושגי יסוד בקצרה: אזרחות מיעוטית",
      "paragraphsHtml": [
        "אזרחות מיעוטית היא תחושת שייכות למדינה לצד שימור זהות קבוצתית מובחנת. במקרה הדרוזי, המאמר מתאר ניסיון לפתח תלמידים שמושרשים גם בזהות דרוזית וגם בזהות ישראלית."
      ]
    },
    {
      "headingHe": "מחקר רב־מקורות בשני בתי ספר",
      "paragraphsHtml": [
        "לפי התקציר, המחקר כלל ראיונות עם מורים, מנהלים, קובעי מדיניות ותלמידים, וכן תצפיות וניתוח מסמכים. שילוב המקורות מאפשר לראות כיצד מסרים על זהות ואזרחות מופיעים בתוכנית הלימודים, בשיח, בפעילויות חברתיות ובטקסים."
      ]
    },
    {
      "headingHe": "מה בודק המאמר בבתי הספר הדרוזיים?",
      "paragraphsHtml": [
        "המאמר בודק כיצד שני בתי ספר תיכוניים דרוזיים מעצבים זהות ואזרחות של מתבגרים. הוא מתעניין בדרך שבה תוכנית לימודים, הוראה, שיח, פעילות חברתית וטקסים לאומיים פועלים יחד."
      ]
    },
    {
      "headingHe": "מהם ארבעת המנגנונים שמצא המחקר?",
      "paragraphsHtml": [
        "התקציר מציין ארבעה מנגנונים: חינוך לערכים רב־ממדי, פיתוח רב־ממדי של זהות ואזרחות, למידה משמעותית וטובת הקהילה. יחד הם יוצרים מסגרת שבה בית הספר מלמד לא רק ידע אלא גם דרך להיות אזרח וחבר קהילה."
      ]
    },
    {
      "headingHe": "איך מתוארת זהות דרוזית־ישראלית?",
      "paragraphsHtml": [
        "המחקר מצא שבתי הספר מסייעים לפתח מתבגרים המושרשים גם בזהות דרוזית וגם בזהות ישראלית. ההדגשה הזאת מציעה מודל שבו שייכות למדינה אינה מחייבת מחיקה של ייחוד קהילתי."
      ]
    },
    {
      "headingHe": "מה תפקידם של טקסים ושיח בית ספרי?",
      "paragraphsHtml": [
        "טקסים ושיח בית ספרי הם דרכים שבהן מוסד חינוכי הופך ערכים למנהגים נראים. כאשר הם מעוצבים במודע, הם יכולים לחזק תחושת שייכות, אחריות וקהילתיות אצל תלמידים."
      ]
    },
    {
      "headingHe": "למה המחויבות של מחנכים דרוזים חשובה?",
      "paragraphsHtml": [
        "התקציר מציין שהמחויבות של המחנכים לבניית זהות דרוזית־ישראלית נובעת מאמונות דתיות וממעמדם הייחודי של הדרוזים בישראל. כלומר, החינוך האזרחי מושפע הן מתפיסות קהילתיות פנימיות והן ממיקום פוליטי רחב."
      ]
    },
    {
      "headingHe": "מה הקשר בין אזרחות לבין טובת הקהילה?",
      "paragraphsHtml": [
        "אזרחות במאמר אינה רק קשר בין תלמיד למדינה, אלא גם אחריות לקהילה. המנגנון של טובת הקהילה מציע שאזרחות מיעוטית יכולה להיבנות דרך שירות, שייכות וערבות פנימית."
      ]
    },
    {
      "headingHe": "איזו שאלה דמוקרטית עולה מן המחקר?",
      "paragraphsHtml": [
        "השאלה הדמוקרטית היא כיצד מערכת חינוך מאפשרת למיעוט לבנות אזרחות מורכבת ולא חד־ממדית. בית ספר שמכיר בזהות קהילתית עשוי לחזק אזרחות יציבה יותר מאשר מערכת שדורשת התאמה מלאה לרוב."
      ]
    },
    {
      "headingHe": "מה יכולים בתי ספר אחרים ללמוד מן המאמר?",
      "paragraphsHtml": [
        "בתי ספר של מיעוטים יכולים ללמוד שהוראת אזרחות צריכה להיות קשורה לחיי הקהילה ולא רק לספרי לימוד רשמיים. גם בתי ספר של רוב יכולים ללמוד שמחויבות אזרחית מתחזקת כאשר היא מכירה בזהויות ממשיות של תלמידים."
      ]
    },
    {
      "headingHe": "איך המאמר מתחבר לשוויון בחינוך?",
      "paragraphsHtml": [
        "שוויון בחינוך אינו רק חלוקת תקציבים, אלא גם הזכות לקבל חינוך שמדבר אל זהות התלמידים. המאמר מראה כיצד הכרה בזהות דרוזית יכולה להפוך את בית הספר למוסד אזרחי משמעותי יותר."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית של המאמר?",
      "paragraphsHtml": [
        "התרומה המרכזית היא תיאור מפורט של חינוך לאזרחות מתוך קהילה מיעוטית. המאמר מציע לחשוב על זהות ואזרחות כפרויקט חינוכי מתמשך שבו מוסדות מקומיים ממלאים תפקיד דמוקרטי חשוב."
      ]
    }
  ],
  "layout": "paper",
  "title": "בתי ספר תיכוניים דרוזיים מעצבים זהות ואזרחות בישראל",
  "permalink": "/democracy_druze_high_schools_identity_citizenship_summary_he.html",
  "paper_url": "democracy_druze_high_schools_identity_citizenship_summary_he.html",
  "sortKey": 202606300002
}
---
