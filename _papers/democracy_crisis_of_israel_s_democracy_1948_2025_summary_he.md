---
{
  "version": 1,
  "file": "democracy_crisis_of_israel_s_democracy_1948_2025_summary_he.html",
  "slug": "democracy_crisis_of_israel_s_democracy_1948_2025_summary_he",
  "order": 157,
  "titleHe": "כיצד משבר הדמוקרטיה בישראל לאורך זמן קשור לסיכון לנסיגה דמוקרטית?",
  "subtitleHe": "",
  "descriptionHe": "המאמר עוסק בסוגיית משבר הדמוקרטיה בישראל לאורך זמן, וממקם את הדיון בתוך שאלות של נסיגה דמוקרטית ופופוליזם, דמוקרטיה ליברלית, זכויות והגבלת כוח ציבורי.",
  "summaryHe": "המאמר בוחן את סוגיית משבר הדמוקרטיה בישראל לאורך זמן דרך עדשה של נסיגה דמוקרטית ופופוליזם.",
  "authorsCardHe": "Ilan Peleg; Ruth Amir",
  "paperTitle": "Crisis of Israel's Democracy, 1948-2025",
  "authorsHtml": "<a href=\"https://govlaw.lafayette.edu/people/ilan-peleg/\" target=\"_blank\" rel=\"noopener noreferrer\">Ilan Peleg</a>; <a href=\"https://orcid.org/0000-0003-0659-5626\" target=\"_blank\" rel=\"noopener noreferrer\">Ruth Amir</a>",
  "journal": "Bloomsbury Academic",
  "dateText": "פורסם: 2025",
  "doiUrl": "https://doi.org/10.5040/9781978746985",
  "doiLabel": "https://doi.org/10.5040/9781978746985",
  "authors": [
    {
      "@type": "Person",
      "name": "Ilan Peleg",
      "url": "https://govlaw.lafayette.edu/people/ilan-peleg/"
    },
    {
      "@type": "Person",
      "name": "Ruth Amir",
      "url": "https://orcid.org/0000-0003-0659-5626"
    }
  ],
  "sourceAuthors": [
    {
      "@type": "Person",
      "name": "Ilan Peleg",
      "url": "https://govlaw.lafayette.edu/people/ilan-peleg/"
    },
    {
      "@type": "Person",
      "name": "Ruth Amir",
      "url": "https://orcid.org/0000-0003-0659-5626"
    }
  ],
  "topics": [
    "democratic-backsliding-populism"
  ],
  "keywords": [
    "נסיגה דמוקרטית",
    "פופוליזם",
    "משבר חוקתי",
    "איזונים ובלמים",
    "ריכוז כוח",
    "משבר הדמוקרטיה בישראל לאורך זמן",
    "ישראל",
    "דמוקרטיה ליברלית"
  ],
  "image": {
    "src": "html_qa/democracy_crisis_of_israel_s_democracy_1948_2025_summary_he.jpg",
    "version": "800x600-landscape",
    "altHe": "איור עריכתי על משבר הדמוקרטיה בישראל לאורך זמן",
    "fitness": "standard"
  },
  "datePublished": "2026-05-21",
  "dateModified": "2026-06-09",
  "lastUpdatedHe": "9 ביוני 2026",
  "oneLinerHtml": "המאמר בוחן את סוגיית משבר הדמוקרטיה בישראל לאורך זמן דרך עדשה של נסיגה דמוקרטית ופופוליזם.",
  "sections": [
    {
      "headingHe": "מה המאמר בוחן?",
      "paragraphsHtml": [
        "המאמר בוחן את סוגיית משבר הדמוקרטיה בישראל לאורך זמן, וממקם את הסוגיה בתוך הדיון הרחב על נסיגה דמוקרטית ופופוליזם.",
        "נקודת המוצא של התמצית היא השאלה כיצד שחיקת נורמות דמוקרטיות, ריכוז כוח ופוליטיקה פופוליסטית משפיעים על איכות הדמוקרטיה הליברלית בישראל."
      ]
    },
    {
      "headingHe": "מה המאמר מצא או טוען בפועל?",
      "paragraphsHtml": [
        "לפי תקציר המקור, המאמר מציג כמה טענות קונקרטיות: פלג ואמיר מציעים פרספקטיבה רחבה על הגורמים הללו, המכסים את הכישלונות ההיסטוריים של מייסדי המדינה, ניתוח הסדר החוקתי וסוג המשטר של ישראל, פרשנות על הפערים הסוציו-פוליטיים והפסיכולוגיה החברתית השלטת של הישראלים, וניתוח הפופוליזם הכריזמטי של ראש ממשלת ישראל הוותיק בנימין נתניהו. הכרך מסתיים בניתוח תרחישים עתידיים ופתרונות אפשריים למשבר הדמוקרטי של ישראל. משבר הדמוקרטיה של ישראל מציע את הניתוח המקיף ביותר של הנסיגה הדמוקרטית של ישראל עד כה, בהשוואה לתהליכים דומים ברחבי העולם.",
        "לכן התרומה של העמוד אינה רק בזיהוי נושא המאמר, אלא בהצגת הטענה או הממצא שהמקור עצמו מדגיש."
      ]
    },
    {
      "headingHe": "מהי התרומה המרכזית?",
      "paragraphsHtml": [
        "התרומה המרכזית היא הפיכת משבר הדמוקרטיה בישראל לאורך זמן לשאלה מוסדית ולא רק למחלוקת פוליטית רגעית.",
        "כך המאמר מסייע להבין כיצד נסיגה דמוקרטית ופופוליזם מתורגמים לכללי פעולה, זכויות ואחריות ציבורית."
      ]
    },
    {
      "headingHe": "למה זה חשוב לדמוקרטיה ליברלית?",
      "paragraphsHtml": [
        "דמוקרטיה ליברלית אינה מסתכמת בבחירות תקופתיות; היא תלויה גם בהגבלת כוח, בהגנה על זכויות ובהכרה בלגיטימיות של מחלוקת.",
        "כאשר משבר הדמוקרטיה בישראל לאורך זמן משתנה בלי ביקורת מספקת, הפגיעה עשויה להופיע תחילה כשאלה משפטית או מקצועית צרה, אך בפועל להשפיע על תנאי ההשתתפות של אזרחים וקבוצות."
      ]
    },
    {
      "headingHe": "איך הוא קשור לשלטון החוק?",
      "paragraphsHtml": [
        "הקשר לשלטון החוק נעוץ בשאלה האם כוח ציבורי מוגבל באמצעות כללים, ביקורת, נימוקים ואפשרות מעשית לערער על החלטות.",
        "במובן זה, שלטון החוק נבחן לא רק בקיום כללים פורמליים אלא גם ביכולת להפעיל אותם על בעלי כוח באופן עקבי ומנומק."
      ]
    },
    {
      "headingHe": "איך הוא קשור לזכויות מיעוטים?",
      "paragraphsHtml": [
        "גם כאשר המאמר אינו עוסק רק במיעוטים, הוא בוחן תנאים שמעצבים את היכולת של קבוצות לא־רוביות ליהנות מהגנה, ייצוג ושייכות אזרחית.",
        "המשמעות המעשית היא לבדוק אם הזכויות מקבלות הגנה מוסדית ממשית, ולא רק מופיעות כשפה כללית של ערכים."
      ]
    },
    {
      "headingHe": "איזה מתח דמוקרטי עומד ברקע?",
      "paragraphsHtml": [
        "המתח המרכזי הוא בין הכרעת רוב או צורך שלטוני לבין ההגבלות שמונעות מן הכוח הציבורי להפוך לבלתי מבוקר.",
        "המתח צריך להיבחן דרך המנגנונים שהמאמר מתאר, ולא כהצהרה כללית על דמוקרטיה או פוליטיקה."
      ]
    }
  ],
  "layout": "paper",
  "title": "כיצד משבר הדמוקרטיה בישראל לאורך זמן קשור לסיכון לנסיגה דמוקרטית?",
  "permalink": "/democracy_crisis_of_israel_s_democracy_1948_2025_summary_he.html",
  "paper_url": "democracy_crisis_of_israel_s_democracy_1948_2025_summary_he.html",
  "sortKey": 202605210136
}
---
