...
- Private author-notice files `author_notice_queue.csv` and `author_notice_history.csv` support `/author-mailer.html`. The nightly 00:05 paper-addition phase should run `python3 scripts/prepare_author_notice_queue.py --include-all-existing --write --private-dir /path/to/private/repo` after refreshing `Authors.csv`, but it must never send email. Only `scripts/send_author_notices.py --send` sends Gmail-authenticated email, and only for editor-released `ready_to_send` rows.
...
- Page feedback: every public page footer links to `/page-feedback.html` with the Hebrew label `הצעות לתיקונים והערות`. The form accepts Hebrew or English comments, an optional suggested photo, a submitter-role choice, optional email, optional phone, and an optional editor password. It posts to `_data/site.json` `pageFeedbackEndpoint`, and the Worker appends rows to private `page_feedback_queue.csv`; original uploaded photos are stored only in the private operational repository. A correct editor password writes the row as `approved_for_update`; an empty or incorrect password is ignored and writes ordinary `pending` feedback. After the editor password is verified on that same page, a page-specific summary editor can load the source paper identified by the query string and save title/subtitle/text revisions as an approved structured feedback row. The Worker must never store the password.
